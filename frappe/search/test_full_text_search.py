...
class TestFullTextSearch(KanivinTestCase):
	def setUp(self):
		index = get_index()
		index.build()
		self.index = index
	...
	def test_search_term(self):
		...
		self.assertEqual(res[0], "sw/kanierp")
	...
	def test_update_index(self):
		...
		self.index.update_index({"name": "sw/kanierp", "content": """AwesomeKANIERP"""})

		res = self.index.search("CommonSearchTerm")
		self.assertTrue("sw/kanierp" not in res)

		res = self.index.search("AwesomeKANIERP")
		self.assertEqual(res[0], "sw/kanierp")
...
def get_documents():
	...
	docs.append(
		{
			"name": "sw/kanierp",
			"content": """KANIERP is a free and open-source integrated Enterprise Resource Planning software developed by
			Kanivin Technologies Pvt. Ltd. and is built on MariaDB database system using a Python based server-side framework.
			KANIERP is a generic ERP software used by manufacturers, distributors and services companies. CommonSearchTerm""",
		}
	)
	...
	docs.append(
		{
			"name": "sw/frappe",
			"content": """Kanivin Framework is a full-stack web framework, that includes everything you need to build and
