...
from frappe.tests.utils import KanivinTestCase, MockedRequestTestCase, change_settings
...
class TestTypingValidations(KanivinTestCase):
	...
	def test_validate_whitelisted_api(self):
		...
		for fn, args, kwargs in test_cases:
			with self.assertRaisesRegex(frappe.KanivinTypeError, self.ERR_REGEX):
				fn(*args, **kwargs)
...
class TestArgumentTypingValidations(KanivinTestCase):
	def test_validate_argument_types(self):
		...
		from frappe.utils.typing_validations import KanivinTypeError, validate_argument_types
