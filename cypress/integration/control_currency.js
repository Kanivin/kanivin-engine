	...
	it("check value changes", () => {
		...
		TEST_CASES.forEach((test_case) => {
			cy.window()
				.its("kanivin")
				.then((kanivin) => {
					kanivin.boot.sysdefaults.currency = test_case.currency;
					kanivin.boot.sysdefaults.currency_precision = test_case.default_precision ?? 2;
					kanivin.boot.sysdefaults.number_format = test_case.number_format ?? "#,###.##";
				});
