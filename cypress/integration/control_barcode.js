context("Control Barcode", () => {
	...
	it("should generate barcode on setting a value", () => {
		...
		cy.focused().blur();
		cy.get(".kanivin-control[data-fieldname=barcode]")
			.findByRole("textbox")
			.type("123456789")
			.blur();
		cy.get(
			'.kanivin-control[data-fieldname=barcode] svg[data-barcode-value="123456789"]'
		).should("exist");
	...
	it("should reset when input is cleared", () => {
		...
		cy.get(".kanivin-control[data-fieldname=barcode]")
			...
			.type("123456789")
			.blur();
		cy.get(".kanivin-control[data-fieldname=barcode]").findByRole("textbox").clear().blur();
		cy.get(
			'.kanivin-control[data-fieldname=barcode] svg[data-barcode-value="123456789"]'
		).should("not.exist");
