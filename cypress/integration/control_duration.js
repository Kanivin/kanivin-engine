context("Control Duration", () => {
	...
	it("should set duration", () => {
		...
		cy.wait(500);
		cy.get(".kanivin-control[data-fieldname=duration] input").first().click();
		...
		cy.wait(500);
		cy.get(".kanivin-control[data-fieldname=duration] input")
			.first()
			.should("have.value", "45d 30m");
		cy.get(".kanivin-control[data-fieldname=duration] input").first().blur();
	...
	it("should hide days or seconds according to duration options", () => {
		...
		cy.get(".kanivin-control[data-fieldname=duration] input").first();
