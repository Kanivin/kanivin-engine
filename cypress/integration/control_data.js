context("Data Control", () => {
	before(() => {
		...
		return cy
			...
			.its("kanivin")
			.then((kanivin) => {
				return kanivin.xcall("kanivin.tests.ui_test_helpers.create_doctype", {
	...
	it('Verifying data control by inputting different patterns for "Name" field', () => {
		...
		cy.get('.kanivin-control[data-fieldname="name1"]')
			.find("label")
			.should("have.class", "reqd");
		cy.get('.kanivin-control[data-fieldname="email"]')
			.find("label")
			.should("have.class", "reqd");
		cy.get('.kanivin-control[data-fieldname="phone"]')
			.find("label")
			.should("have.class", "reqd");
		...
		cy.get('.kanivin-control[data-fieldname="name1"]').should("have.class", "has-error");
		cy.save();
	...
	it('Verifying data control by inputting different patterns for "Email" field', () => {
		...
		cy.get('.kanivin-control[data-fieldname="email"]').should("have.class", "has-error");
		cy.save();
		cy.get(".modal-title").should("have.text", "Message");
	...
	it('Verifying data control by inputting different patterns for "Phone" field', () => {
		...
		cy.get('.kanivin-control[data-fieldname="phone"]').should("have.class", "has-error");
