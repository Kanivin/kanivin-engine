context("Control Link", () => {
	...
	it("should set the valid value", () => {
		...
		cy.intercept("POST", "/api/method/kanivin.desk.search.search_link").as("search_link");

		cy.get(".kanivin-control[data-fieldname=link] input").focus().as("input");
		cy.wait("@search_link");
		cy.get("@input").type("todo for link", { delay: 200 });
		cy.wait("@search_link");
		cy.wait(500);
		cy.get(".kanivin-control[data-fieldname=link]").findByRole("listbox").should("be.visible");
		cy.get(".kanivin-control[data-fieldname=link] input").type("{enter}", { delay: 100 });
		cy.get(".kanivin-control[data-fieldname=link] input").blur();
	...
	it("should unset invalid value", () => {
		...
		cy.intercept("POST", "/api/method/kanivin.client.validate_link").as("validate_link");

		cy.get(".kanivin-control[data-fieldname=link] input")
			.type("invalid value", { delay: 100 })
			.blur();
		cy.wait("@validate_link");
		cy.wait(500);
		cy.get(".kanivin-control[data-fieldname=link] input").should("have.value", "");
	});
	...
	it("should be possible set empty value explicitly", () => {
		...
		cy.get(".kanivin-control[data-fieldname=link] input").type("  ", { delay: 100 }).blur();
		cy.wait("@validate_link");
		cy.wait(500);
		cy.get(".kanivin-control[data-fieldname=link] input").should("have.value", "");
		cy.window()
	...
	it("should show open link button", () => {
		...
		cy.get("@todos").then((todos) => {
			cy.get(".kanivin-control[data-fieldname=link] input").as("input");
			cy.get("@input").focus();
			cy.wait("@search_link");
			cy.get("@input").type(todos[0]).blur();
			cy.wait("@validate_link");
			cy.get("@input").trigger("mouseover");
			cy.get(".kanivin-control[data-fieldname=link] .btn-open")
				.should("be.visible")
	...
	it("show title field in link", () => {
		...
		cy.window()
			...
			.then((kanivin) => {
				if (!kanivin.boot) {
					...
					kanivin.boot.link_title_doctypes = ["ToDo"];
				}
			});
		...
		cy.get("@input").type("todo for link", { delay: 200 });
		cy.wait("@search_link");
		cy.get(".kanivin-control[data-fieldname=link] ul").should("be.visible");
		cy.get(".kanivin-control[data-fieldname=link] input").type("{enter}", { delay: 100 });
		cy.get(".kanivin-control[data-fieldname=link] input").blur();
		cy.get("@dialog").then((dialog) => {
			cy.get("@todos").then((todos) => {
	...
	it("should update dependant fields (via fetch_from)", () => {
		cy.get("@todos").then((todos) => {
			...
			cy.get(".kanivin-control[data-fieldname=assigned_by] input").focus().as("input");
			cy.get("@input").clear().type(cy.config("testUser"), { delay: 300 }).blur();
			cy.wait("@validate_link");
			cy.get(".kanivin-control[data-fieldname=assigned_by_full_name] .control-value").should(
				"contain",
				"Kanivin"
			);
	...
	it("show translated text for Gender link field with language de with input in de", () => {
		cy.call("kanivin.tests.ui_test_helpers.insert_translations").then(() => {
			cy.window()
				.its("kanivin")
				.then((kanivin) => {
					cy.set_value("User", kanivin.user.name, { language: "de" });
				});
	...
	it("show text for Gender link field with language en", () => {
		cy.window()
			.its("kanivin")
			.then((kanivin) => {
				cy.set_value("User", kanivin.user.name, { language: "en" });
			});
	...
	it("show custom link option", () => {
		cy.window()
			.its("kanivin")
			.then((kanivin) => {
				kanivin.ui.form.ControlLink.link_options = (link) => {
					return [
						{
							html:
