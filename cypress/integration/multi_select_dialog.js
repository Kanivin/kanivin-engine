context("MultiSelectDialog", () => {
	...
	function open_multi_select_dialog() {
		cy.window()
			.its("kanivin")
			.then((kanivin) => {
				new kanivin.ui.form.MultiSelectDialog({
					doctype: "Contact",
					target: {},
					setters: {
						status: null,
	...
	it("checks for filters", () => {
		["search_term", "status", "gender"].forEach((fieldname) => {
			cy.get_open_dialog()
				.get(`.kanivin-control[data-fieldname="${fieldname}"]`)
				.should("exist");
		});

		// add_filters_group: 1 should add a filter group
		cy.get_open_dialog().get(`.kanivin-control[data-fieldname="filter_area"]`).should("exist");
	});
	...
	it("checks for child item selection", () => {
		...
		cy.get_open_dialog()
			.get(`.kanivin-control[data-fieldname="allow_child_item_selection"]`)
			.find('input[data-fieldname="allow_child_item_selection"]')
			.should("exist")
			.click({ force: true });

		cy.get_open_dialog()
			.get(`.kanivin-control[data-fieldname="child_selection_area"]`)
			.should("exist");
	...
	it("tests more button", () => {
		cy.get_open_dialog()
			.get(`.kanivin-control[data-fieldname="search_term"]`)
			.find('input[data-fieldname="search_term"]')
			.should("exist")
			.type("Test", { delay: 200 });
		cy.get_open_dialog()
			.get(`.kanivin-control[data-fieldname="more_child_btn"]`)
			.should("exist")
			.as("more-btn");
		...
		cy.intercept("POST", "api/method/kanivin.client.get_list").as("get-more-records");
