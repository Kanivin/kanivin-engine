...
context("Grid Search", () => {
	before(() => {
		...
		return cy
			...
			.its("kanivin")
			.then((kanivin) => {
				return kanivin.xcall(
					"kanivin.tests.ui_test_helpers.insert_doctype_with_child_table_record",
					{
	...
	it("Test search row visibility", () => {
		cy.window()
			.its("kanivin")
			.then((kanivin) => {
				kanivin.model.user_settings.save("Doctype With Child Table", "GridView", {
		...
		cy.get('.kanivin-control[data-fieldname="child_table_1"]').as("table");
