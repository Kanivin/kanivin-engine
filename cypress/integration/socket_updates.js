context("Realtime updates", () => {
	...
	it("Recieves custom messages from server", { scrollBehavior: false }, () => {
		...
		cy.window()
			.its("kanivin")
			.then((kanivin) => {
				kanivin.realtime.on(event, () => handler.handle());
			});
	...
	it("Progress bar", { scrollBehavior: false }, () => {
		...
		cy.call("kanivin.tests.ui_test_helpers.publish_progress", { title }).then(() => {
...
function publish_realtime(args) {
	return cy.call("kanivin.tests.ui_test_helpers.publish_realtime", args);
}
