context("Table MultiSelect", () => {
	...
	it("select value from multiselect dropdown", () => {
		...
		cy.get("@input").type("test@kanierp", { delay: 100 });
		cy.wait(500);
		cy.get("@input").type("{enter}");
		cy.get(
			'.frappe-control[data-fieldname="users"] .form-control .tb-selected-value .btn-link-to-form'
		).as("selected-value");
		cy.get("@selected-value").should("contain", "test@kanierp.com");
	...
	it("navigate to selected value", () => {
		...
		cy.location("pathname").should("contain", "/user/test%40kanierp.com");
	});
});
