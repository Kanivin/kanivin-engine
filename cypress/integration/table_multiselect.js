context("Table MultiSelect", () => {
	...
	it("select value from multiselect dropdown", () => {
		...
		cy.get(
			'.kanivin-control[data-fieldname="users"] .form-control .tb-selected-value .btn-link-to-form'
		).as("selected-value");
		cy.get("@selected-value").should("contain", "test@erpnext.com");

		cy.intercept("POST", "/api/method/kanivin.desk.form.save.savedocs").as("save_form");
		// trigger save
	...
	it("delete value using backspace", () => {
		...
		cy.get('.kanivin-control[data-fieldname="users"] .form-control .tb-selected-value').should(
			"not.exist"
		);
	});
	...
	it("delete value using x", () => {
		cy.go_to_list("Assignment Rule");
		cy.get(`.list-subject:contains("table multiselect")`).last().find("a").click();
		cy.get('.kanivin-control[data-fieldname="users"] .form-control .tb-selected-value').as(
			"existing_value"
		);
