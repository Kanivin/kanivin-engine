context.skip("Form Tour", () => {
	before(() => {
		...
		return cy
			...
			.its("kanivin")
			.then((kanivin) => {
				return kanivin.call("kanivin.tests.ui_test_helpers.create_form_tour");
			});
	});
	...
	it("navigates a form tour", () => {
		...
		cy.get(".kanivin-driver").should("be.visible");
		cy.get('.kanivin-control[data-fieldname="first_name"]').as("first_name");
		cy.get("@first_name").should("have.class", "driver-highlighted-element");
		cy.get(".kanivin-driver").findByRole("button", { name: "Next" }).as("next_btn");
		...
		cy.get('.kanivin-control[data-fieldname="last_name"]').as("last_name");
		...
		// assert field is highlighted
		cy.get('.kanivin-control[data-fieldname="phone_nos"]').as("phone_nos");
		...
		cy.get('.grid-row-open .kanivin-control[data-fieldname="phone"]').as("phone");
		...
		cy.wait(500);
		cy.get(".kanivin-driver").findByRole("button", { name: "Save" }).should("be.visible");
	});
});
