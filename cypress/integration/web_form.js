context("Web Form", () => {
	before(() => {
		...
		return cy
			...
			.its("kanivin")
			.then((kanivin) => {
				return kanivin.xcall("kanivin.tests.ui_test_helpers.prepare_webform_test");
			});
	});
	...
	it("Read Only", () => {
		...
		// Read Only Field
		cy.get(".web-list-table tbody tr:last").click();
		cy.get('.kanivin-control[data-fieldname="title"] .control-input').should(
			"have.css",
			"display",
			"none"
		);
	...
	it("Navigate and Submit a MultiStep WebForm", () => {
		cy.call("kanivin.tests.ui_test_helpers.update_webform_to_multistep").then(() => {
