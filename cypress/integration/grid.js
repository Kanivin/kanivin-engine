context("Grid", () => {
	...
	before(() => {
		...
		return cy
			...
			.then((kanivin) => {
				return kanivin.call(
					"kanivin.tests.ui_test_helpers.create_contact_phone_nos_records"
				);
			});
	});
	...
	it("update docfield property using toggle_display", () => {
		...
		cy.window()
			...
			.then((frm) => {
				...
				cy.get(".grid-row-open").as("table-form");
				cy.get("@table-form")
					.find('.kanivin-control[data-fieldname="is_primary_mobile_no"]')
					.should("be.hidden");
				cy.get("@table-form").find(".grid-footer-toolbar").click();
	...
	it("update docfield property using toggle_enable", () => {
		...
		cy.window()
			...
			.then((frm) => {
				...
				cy.get(".grid-row-open").as("table-form");
				cy.get("@table-form")
					.find('.kanivin-control[data-fieldname="phone"] .control-value')
