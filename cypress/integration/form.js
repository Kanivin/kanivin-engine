...
context("Form", () => {
	before(() => {
		...
		return cy
			...
			.its("kanivin")
			.then((kanivin) => {
				return kanivin.call("kanivin.tests.ui_test_helpers.create_contact_records");
			});
	});
	...
	it("create a new form", () => {
		...
		cy.intercept({
			method: "POST",
			url: "api/method/kanivin.desk.form.save.savedocs",
		}).as("form_save");
	...
	it("validates behaviour of Data options validations in child table", () => {
		...
		cy.get('.kanivin-control[data-fieldname="email_ids"]').as("table");
	...
	it("update docfield property using set_df_property in child table", () => {
		...
		cy.window()
			.its("cur_frm")
			.then((frm) => {
				cy.get('.kanivin-control[data-fieldname="phone_nos"]').as("table");
				...
				cy.get("@table-form")
					.find('.kanivin-control[data-fieldname="is_primary_phone"]')
					.should("be.hidden");
