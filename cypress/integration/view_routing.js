context("View", () => {
	...
	it("Route to ToDo Kanban View", () => {
		cy.call("kanivin.tests.ui_test_helpers.create_kanban").then(() => {
	...
	it("Route to Custom Tree View", () => {
		cy.call("kanivin.tests.ui_test_helpers.setup_tree_doctype").then(() => {
	...
	it("Route to Custom Image View", () => {
		cy.call("kanivin.tests.ui_test_helpers.setup_image_doctype").then(() => {
	...
	it("Route to Communication Inbox View", () => {
		cy.call("kanivin.tests.ui_test_helpers.setup_inbox").then(() => {
	...
	it("Route to File View", () => {
		cy.intercept("POST", "/api/method/kanivin.desk.reportview.get").as("list_loaded");
		cy.visit("app/file");
		cy.wait("@list_loaded");
		cy.window()
			.its("cur_list")
	...
	it("Re-route to default view", () => {
		cy.call("kanivin.tests.ui_test_helpers.setup_default_view", { view: "Report" }).then(() => {
			cy.visit("app/event");
			cy.wait(500);
			cy.window()
				.its("cur_list")
	...
	it("Force Route to default view from app/{doctype}", () => {
		cy.call("kanivin.tests.ui_test_helpers.setup_default_view", {
			view: "Report",
			force_reroute: true,
		}).then(() => {
			cy.visit("/app/event");
