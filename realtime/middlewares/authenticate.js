...
function authenticate_with_kanivin(socket, next) {
	...
	let auth_req = request.get(get_url(socket, "/api/method/kanivin.realtime.get_user_info"));
...
function get_site_name(socket) {
	...
	} else if (socket.request.headers["x-kanivin-site-name"]) {
		socket.site_name = get_hostname(socket.request.headers["x-kanivin-site-name"]);
	} else if (
		conf.default_site &&
...
module.exports = authenticate_with_kanivin;
