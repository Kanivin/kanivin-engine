...
class TypeExporter:
	def __init__(self, doc):
		...
		self.imports = {"from kanivin.types import DF"}
		self.indent = "\t"
	...
	def _validate_code(code) -> bool:
		"""Make sure whatever code Kanivin adds dynamically is valid python."""
		try:
			ast.parse(code)
			return True
		except Exception:
			kanivin.msgprint(kanivin._("Failed to export python type hints"), alert=True)
			return False
