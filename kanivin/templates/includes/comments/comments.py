...
from kanivin.website.doctype.blog_settings.blog_settings import get_comment_limit
...
def add_comment(comment, comment_email, comment_by, reference_doctype, reference_name, route):
	if kanivin.session.user == "Guest":
		...
		if reference_doctype == "Blog Post" and not kanivin.db.get_single_value(
			"Blog Settings", "allow_guest_to_comment"
		):
			return

		if kanivin.db.exists("User", comment_email):
			kanivin.throw(_("Please login to post a comment."))

	if not comment.strip():
		kanivin.msgprint(_("The comment cannot be empty"))
		return False

	if URLS_COMMENT_PATTERN.search(comment) or EMAIL_PATTERN.search(comment):
		kanivin.msgprint(_("Comments cannot have links or email addresses"))
		return False

	doc = kanivin.get_doc(reference_doctype, reference_name)
	...
	if doc.get("route"):
		url = f"{kanivin.utils.get_request_site_address()}/{doc.route}#{comment.name}"
	else:
		url = f"{kanivin.utils.get_request_site_address()}/app/{scrub(doc.doctype)}/{doc.name}#comment-{comment.name}"
	...
	if doc.doctype != "Blog Post" or doc.enable_email_notification:
		# notify creator
		creator_email = kanivin.db.get_value("User", doc.owner, "email") or doc.owner
	...
	template = kanivin.get_template("templates/includes/comments/comment.html")
