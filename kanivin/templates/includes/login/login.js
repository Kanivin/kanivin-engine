...
login.bind_events = function () {
	...
	$(".form-login").on("submit", function (event) {
		...
		var args = {};
		args.cmd = "login";
		args.usr = kanivin.utils.xss_sanitise(($("#login_email").val() || "").trim());
		args.pwd = $("#login_password").val();
		if (!args.usr || !args.pwd) {
			{# striptags is used to remove newlines, e is used for escaping #}
			kanivin.msgprint("{{ _('Both login and password required') | striptags | e }}");
			return false;
		}
	...
	$(".form-signup").on("submit", function (event) {
		event.preventDefault();
		var args = {};
		args.cmd = "kanivin.core.doctype.user.user.sign_up";
		args.email = ($("#signup_email").val() || "").trim();
		args.redirect_to = kanivin.utils.sanitise_redirect(kanivin.utils.get_url_arg("redirect-to"));
		args.full_name = kanivin.utils.xss_sanitise(($("#signup_fullname").val() || "").trim());
	...
	$(".form-forgot").on("submit", function (event) {
		event.preventDefault();
		var args = {};
		args.cmd = "kanivin.core.doctype.user.user.reset_password";
	...
	$(".form-login-with-email-link").on("submit", function (event) {
		event.preventDefault();
		var args = {};
		args.cmd = "kanivin.www.login.send_login_link";
	...
	var login_handlers = {
		200: function (data) {
			if (data.message == 'Logged In') {
				...
				window.location.href = kanivin.utils.sanitise_redirect(kanivin.utils.get_url_arg("redirect-to")) || data.home_page;
			} else if (data.message == 'Password Reset') {
				window.location.href = kanivin.utils.sanitise_redirect(data.redirect_to);
			} else if (data.message == "No App") {
				login.set_status({{ _("Success") | tojson }}, 'green');
				if (localStorage) {
					var last_visited =
						localStorage.getItem("last_visited")
						|| kanivin.utils.sanitise_redirect(kanivin.utils.get_url_arg("redirect-to"));
				...
				if (cint(data.message[0]) == 0) {
					...
					login.set_status({{ _("Success") | tojson }}, 'green');
					kanivin.msgprint(data.message[1])
				}
...
var verify_token = function (event) {
	$(".form-verify").on("submit", function (eventx) {
		...
		args.tmp_id = kanivin.get_cookie('tmp_id');
		if (!args.otp) {
			{# striptags is used to remove newlines, e is used for escaping #}
			kanivin.msgprint("{{ _('Login token required') | striptags | e }}");
			return false;
		}
		login.call(args);
		return false;
