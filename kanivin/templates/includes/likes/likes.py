...
from kanivin.website.doctype.blog_settings.blog_settings import get_like_limit
...
def like(reference_doctype, reference_name, like, route=""):
	like = kanivin.parse_json(like)
	ref_doc = kanivin.get_doc(reference_doctype, reference_name)
	...
	if like and ref_doc.enable_email_notification:
		...
		message = message + "<p><a href='{}/{}#likes' style='font-size: 80%'>{}</a></p>".format(
			kanivin.utils.get_request_site_address(), ref_doc.route, _("View Blog Post")
		)

		# notify creator
		kanivin.sendmail(
			recipients=kanivin.db.get_value("User", ref_doc.owner, "email") or ref_doc.owner,
			subject=subject,
			message=message,
...
def add_like(reference_doctype, reference_name):
	...
	if user == "Guest":
		like.ip_address = kanivin.local.request_ip
...
def delete_like(reference_doctype, reference_name):
	...
	kanivin.db.delete("Comment", filters)
	return False
