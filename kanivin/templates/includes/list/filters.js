function setup_list_filters() {
	...
	kanivin.call('kanivin.website.doctype.web_form.web_form.get_form_data', {
	...
	.then((r) => {
		...
		web_form_filters = web_form.web_form_fields.filter(df => {
			...
			const value = kanivin.utils.get_query_params()[df.fieldname];
			const f = kanivin.ui.form.make_control({
				df: {
					...
					is_filter: true,
					change: (e) => {
						const query_params = Object.assign(kanivin.utils.get_query_params(), {
							[df.fieldname]: f.get_value()
						})
						const query_string = kanivin.utils.make_query_string(query_params);
...
kanivin.ready(setup_list_filters);
