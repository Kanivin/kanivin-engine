kanivin.ready(() => {
	...
	kanivin.require("controls.bundle.js", () => {
...
const setup_socket_io = () => {
	kanivin.realtime.init(window.socketio_port || "9000");

	kanivin.realtime.on("publish_message", (data) => {
		publish_message(data);
	});

	kanivin.realtime.on("update_message", (data) => {
		update_message(data);
	});

	kanivin.realtime.socket.on("delete_message", (data) => {
		delete_message(data);
	});
};
...
const insert_message = (data) => {
	...
	if ($(`.discussion-on-page[data-topic=${topic.name}]`).length) {
		...
		if (topic.owner == kanivin.session.user) {
	...
	} else if (topic.owner == kanivin.session.user && document_match_found) {
...
const submit_discussion = (e) => {
	...
	if (strip_html(reply).trim() != "" || reply.includes("img")) {
		...
		kanivin.call({
			method: "kanivin.website.doctype.discussion_topic.discussion_topic.submit_discussion",
			args: {
...
const get_color_from_palette = (element) => {
	const palette = kanivin.get_palette(element.attr("title"));
...
const delete_reply = (e) => {
	kanivin.call({
		method: "kanivin.website.doctype.discussion_reply.discussion_reply.delete_message",
		args: {
...
const hide_actions_on_conditions = (template, owner) => {
	...
	kanivin.session.user != owner && $template.find(".dropdown").remove();
	return $template.prop("outerHTML");
};
...
const make_comment_editor = (element) => {
	this.comment_editor = new kanivin.ui.FieldGroup({
		fields: [
			{
	...
	element.find(".kanivin-control").removeClass("hide-control");
