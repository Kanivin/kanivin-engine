...
def validate_route_conflict(doctype, name):
	...
	for _doctype in ["Page", "Workspace", "DocType"]:
		all_names.extend(
			[slug(d) for d in kanivin.get_all(_doctype, pluck="name") if (doctype != _doctype and d != name)]
		)

	if slug(name) in all_names:
		kanivin.msgprint(kanivin._("Name already taken, please set a new name"))
		raise kanivin.NameError
...
def pop_csv_params(form_dict):
	...
	from kanivin.utils.data import cint, cstr
...
def provide_binary_file(filename: str, extension: str, content: bytes) -> None:
	...
	kanivin.response["filecontent"] = content
	kanivin.response["filename"] = f"{_(filename)}.{extension}"
