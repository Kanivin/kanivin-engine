...
def get_preview_data(doctype, docname):
	...
	preview_data = kanivin.get_list(doctype, filters={"name": docname}, fields=preview_fields, limit=1)
	...
	for key, val in preview_data.items():
		if val and meta.has_field(key) and key not in [image_field, title_field, "name"]:
			formatted_preview_data[meta.get_field(key).label] = kanivin.format(
				val,
