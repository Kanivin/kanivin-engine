...
from kanivin import _, is_whitelisted, validate_and_sanitize_search_inputs
from kanivin.database.schema import SPECIAL_CHAR_PATTERN
from kanivin.model.db_query import get_order_by
from kanivin.permissions import has_permission
from kanivin.utils import cint, cstr, unique
from kanivin.utils.data import make_filter_tuple
...
def sanitize_searchfield(searchfield: str):
	...
	if SPECIAL_CHAR_PATTERN.search(searchfield):
		kanivin.throw(_("Invalid Search Field {0}").format(searchfield), kanivin.DataError)
...
def search_widget(
	...
	standard_queries = kanivin.get_hooks().standard_queries or {}
	...
	if query:  # Query = custom search query i.e. python function
		try:
			is_whitelisted(kanivin.get_attr(query))
			return kanivin.call(
				query,
				doctype,
				txt,
		...
		except (kanivin.PermissionError, kanivin.AppNotInstalledError, ImportError):
			if kanivin.local.conf.developer_mode:
				raise
			else:
	...
	if not meta.translated_doctype:
		_txt = kanivin.db.escape((txt or "").replace("%", "").replace("@", ""))
...
def get_names_for_mentions(search_term):
	users_for_mentions = kanivin.cache.get_value("users_for_mentions", get_users_for_mentions)
	user_groups = kanivin.cache.get_value("user_groups", get_user_groups)
	...
	for mention_data in users_for_mentions + user_groups:
		...
		mention_data["link"] = kanivin.utils.get_url_to_form(
...
def get_user_groups():
	return kanivin.get_all("User Group", fields=["name as id", "name as value"], update={"is_group": True})
...
def get_link_title(doctype, docname):
	...
	if meta.show_title_field_in_link:
		return kanivin.db.get_value(doctype, docname, meta.title_field)
