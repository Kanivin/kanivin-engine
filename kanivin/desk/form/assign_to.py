...
from kanivin.desk.doctype.notification_log.notification_log import (
	enqueue_create_notification,
	get_title,
	get_title_html,
)
from kanivin.desk.form.document_follow import follow_document
from kanivin.utils.data import strip_html


class DuplicateToDoError(kanivin.ValidationError):
	pass
...
def add(args=None, *, ignore_permissions=False):
	...
	for assign_to in kanivin.parse_json(args.get("assign_to")):
		filters = {
		...
		if not ignore_permissions:
			kanivin.get_doc(args["doctype"], args["name"]).check_permission()

		if kanivin.get_all("ToDo", filters=filters):
			...
			d = kanivin.get_doc(
				{
					...
					"assigned_by": args.get("assigned_by", kanivin.session.user),
					"assignment_rule": args.get("assignment_rule"),
				}
			).insert(ignore_permissions=True)

			# set assigned_to if field exists
			if kanivin.get_meta(args["doctype"]).get_field("assigned_to"):
				kanivin.db.set_value(args["doctype"], args["name"], "assigned_to", assign_to)

			doc = kanivin.get_doc(args["doctype"], args["name"])

			# if assignee does not have permissions, share or inform
			if not kanivin.has_permission(doc=doc, user=assign_to):
				if kanivin.get_system_settings("disable_document_sharing"):
					msg = _("User {0} is not permitted to access this document.").format(
						kanivin.bold(assign_to)
					)
					msg += "<br>" + _(
						"As document sharing is disabled, please give them the required permissions before assigning."
					)
					kanivin.throw(msg, title=_("Missing Permission"))
				else:
					kanivin.share.add(doc.doctype, doc.name, assign_to)
					shared_with_users.append(assign_to)

			# make this document followed by assigned user
			if kanivin.get_cached_value("User", assign_to, "follow_assigned_documents"):
	...
	if users_with_duplicate_todo:
		...
		kanivin.msgprint(_("Already in the following Users ToDo list:{0}").format(user_list, alert=True))
...
def close(doctype: str, name: str, assign_to: str, ignore_permissions=False):
	if assign_to != kanivin.session.user:
		kanivin.throw(_("Only the assignee can complete this to-do."))
...
def set_status(doctype, name, todo=None, assign_to=None, status="Cancelled", ignore_permissions=False):
	...
	if not ignore_permissions:
		kanivin.get_doc(doctype, name).check_permission()
	try:
		if not todo:
			todo = kanivin.db.get_value(
				"ToDo",
				{
		...
		if todo:
			todo = kanivin.get_doc("ToDo", todo)
			todo.status = status
	...
	if kanivin.get_meta(doctype).get_field("assigned_to") and status in ("Cancelled", "Closed"):
		kanivin.db.set_value(doctype, name, "assigned_to", None)
...
def notify_assignment(assigned_by, allocated_to, doc_type, doc_name, action="CLOSE", description=None):
	...
	assigned_user = kanivin.db.get_value("User", allocated_to, ["language", "enabled"], as_dict=True)
	...
	user_name = kanivin.get_cached_value("User", kanivin.session.user, "full_name")
	...
	if action == "CLOSE":
		subject = _("Your assignment on {0} {1} has been removed by {2}", lang=assigned_user.language).format(
			kanivin.bold(_(doc_type)), get_title_html(title), kanivin.bold(user_name)
		)
	else:
		user_name = kanivin.bold(user_name)
		document_type = kanivin.bold(_(doc_type, lang=assigned_user.language))
