...
from kanivin.model.meta import is_single
...
def get_submitted_linked_docs(doctype: str, name: str, ignore_doctypes_on_cancel_all=None) -> list[tuple]:
	...
	* User should be able to cancel the linked documents along with the one user trying to cancel.
	...
	kanivin.has_permission(doctype, doc=name)
...
class SubmittableDocumentTree:
	def __init__(self, doctype: str, name: str):
		...
		* Node is referenced(linked) by many other documents and those are the child nodes.
		...
		NOTE: child document is a property of child node (not same as Kanivin child docs of a table field).
		"""
	...
	def get_submittable_doctypes(self) -> list[str]:
		...
		if not self._submittable_doctypes:
			self._submittable_doctypes = kanivin.get_all(
...
def get_references_across_doctypes_by_dynamic_link_field(
	...
	for doctype, fieldname, doctype_fieldname in links:
		try:
			...
			for linked_to in kanivin.get_all(doctype, pluck=doctype_fieldname, filters=filters, distinct=1):
				if linked_to:
					links_by_doctype[linked_to].append(
						{"doctype": doctype, "fieldname": fieldname, "doctype_fieldname": doctype_fieldname}
					)
		except kanivin.db.ProgrammingError:
			# TODO: FIXME
			continue
	return links_by_doctype
...
def get_referencing_documents(
	...
	if not link_info.get("is_child"):
		...
		return {from_table: kanivin.get_all(from_table, filters, pluck="name", order_by=None)}

	filters.extend(child_filters or [])
	res = kanivin.get_all(from_table, filters=filters, fields=["name", "parenttype", "parent"], order_by=None)
	...
	for parent, rows in itertools.groupby(res, key=lambda row: row["parenttype"]):
		...
		documents[parent].extend(kanivin.get_all(parent, filters=filters, pluck="name", order_by=None) or [])
	return documents
...
def cancel_all_linked_docs(docs, ignore_doctypes_on_cancel_all=None):
	...
	for i, doc in enumerate(docs, 1):
		if validate_linked_doc(doc, ignore_doctypes_on_cancel_all):
			linked_doc = kanivin.get_doc(doc.get("doctype"), doc.get("name"))
			linked_doc.cancel()
		kanivin.publish_progress(percent=i / len(docs) * 100, title=_("Cancelling documents"))
...
def validate_linked_doc(docinfo, ignore_doctypes_on_cancel_all=None):
	...
	if not kanivin.get_meta(docinfo.get("doctype")).is_submittable:
		return False
...
def get_exempted_doctypes():
	...
	return list(kanivin.get_hooks("auto_cancel_exempted_doctypes"))
...
def get_linked_docs(doctype: str, name: str, linkinfo: dict | None = None) -> dict[str, list]:
	...
	is_target_doctype_table = kanivin.get_meta(doctype).istable

	for linked_doctype, link_context in linkinfo.items():
		# Don't try to fetch linked documents if the user can't read the doctype
		if not kanivin.has_permission(linked_doctype):
			continue

		linked_doctype_meta = kanivin.get_meta(linked_doctype)
		...
		fields = [
			...
			for d in linked_doctype_meta.get(
				...
				{
					"in_list_view": 1,
					"fieldtype": ["not in", ("Image", "HTML", "Button", *kanivin.model.table_fields)],
				},
			)
		...
		if filters_ctx := link_context.get("filters"):
			ret = kanivin.get_list(doctype=linked_doctype, fields=fields, filters=filters_ctx, order_by=None)
			...
			if linked_doctype_meta.istable:
				if not (
					kanivin.db.exists("DocField", {"options": linked_doctype})
					or kanivin.db.exists(linked_doctype, {"parenttype": doctype, "parent": name})
				):
					continue
			ret = kanivin.get_list(
...
def get(doctype, docname):
	kanivin.has_permission(doctype, doc=docname)
...
def get_linked_doctypes(doctype, without_ignore_user_permissions_enabled=False):
	...
	if without_ignore_user_permissions_enabled:
		...
		return kanivin.cache.hget("linked_doctypes", doctype, lambda: _get_linked_doctypes(doctype))
...
def _get_linked_doctypes(doctype, without_ignore_user_permissions_enabled=False):
	...
	links = kanivin.get_all("DocField", fields=["parent as dt"], filters=filters)
	links += kanivin.get_all("Custom Field", fields=["dt"], filters=filters)
...
def get_linked_fields(doctype, without_ignore_user_permissions_enabled=False):
	...
	# find links of parents
	links = kanivin.get_all("DocField", fields=["parent", "fieldname"], filters=filters, as_list=1)
	links += kanivin.get_all("Custom Field", fields=["dt as parent", "fieldname"], filters=filters, as_list=1)
	...
	table_doctypes = kanivin.get_all(
	...
	for parent, options in kanivin.get_all(
...
def get_dynamic_linked_fields(doctype, without_ignore_user_permissions_enabled=False):
	...
	for df in links:
		...
		meta = kanivin.get_meta(df.doctype)
		if meta.is_virtual:
			continue

		is_child = meta.istable
		possible_link = kanivin.get_all(
			df.doctype,
