...
from kanivin.build import scrub_html_template
from kanivin.model.meta import Meta
from kanivin.model.utils import render_include
from kanivin.modules import get_module_path, load_doctype_module, scrub
from kanivin.utils import get_bench_path, get_html_format
...
def get_meta(doctype, cached=True) -> "FormMeta":
	...
	cached = cached and not kanivin.conf.developer_mode
	if cached:
		meta = kanivin.cache.hget("doctype_form_meta", doctype)
		if not meta:
			# Cache miss - explicitly get meta from DB to avoid
			meta = FormMeta(doctype, cached=False)
			kanivin.cache.hset("doctype_form_meta", doctype, meta)
	else:
...
class FormMeta(Meta):
	def __init__(self, doctype, *, cached=True):
		self.__dict__.update(kanivin.get_meta(doctype, cached=cached).__dict__)
		self.load_assets()
	...
	def add_code(self):
		...
		system_country = kanivin.get_system_settings("country")
...
"""
	...
	def add_search_fields(self):
		...
		for df in self.get("fields", {"fieldtype": "Link", "options": ["!=", "[Select]"]}):
			if df.options:
				try:
					search_fields = kanivin.get_meta(df.options).search_fields
	...
	def _show_missing_doctype_msg(self, df):
		...
		msg = _("Field {0} is referring to non-existing doctype {1}.").format(
			kanivin.bold(df.fieldname), kanivin.bold(df.options)
		)
		...
		kanivin.throw(msg, title=_("Missing DocType"))

	def add_linked_document_type(self):
		for df in self.get("fields", {"fieldtype": "Link"}):
			if df.options:
				try:
					df.linked_document_type = kanivin.get_meta(df.options).document_type
				except kanivin.DoesNotExistError:
					self._show_missing_doctype_msg(df)

	def load_print_formats(self):
		print_formats = kanivin.db.sql(
	...
	def load_workflows(self):
		...
		if workflow_name and kanivin.db.exists("Workflow", workflow_name):
			workflow = kanivin.get_doc("Workflow", workflow_name)
			workflow_docs.append(workflow)

			workflow_docs.extend(kanivin.get_doc("Workflow State", d.state) for d in workflow.get("states"))
		...
		if not self.custom:
			...
			if hasattr(module, "form_grid_templates"):
				for key, path in module.form_grid_templates.items():
					templates[key] = get_html_format(kanivin.get_app_path(app, path))
...
def get_code_files_via_hooks(hook, name):
	...
	for app_name in kanivin.get_installed_apps():
		code_hook = kanivin.get_hooks(hook, default={}, app_name=app_name)
		if not code_hook:
			continue
		...
		for file in files:
			path = kanivin.get_app_path(app_name, *file.strip("/").split("/"))
