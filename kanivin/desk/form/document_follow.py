...
def update_follow(doctype: str, doc_name: str, following: bool):
	if following:
		return follow_document(doctype, doc_name, kanivin.session.user)
	else:
		return unfollow_document(doctype, doc_name, kanivin.session.user)
...
def follow_document(doctype, doc_name, user):
	...
	if (not kanivin.get_meta(doctype).track_changes) or user == "Administrator":
		return

	if not kanivin.db.get_value("User", user, "document_follow_notify", ignore=True, cache=True):
		return

	if not is_document_followed(doctype, doc_name, user):
		doc = kanivin.new_doc("Document Follow")
...
def unfollow_document(doctype, doc_name, user):
	...
	if doc:
		kanivin.delete_doc("Document Follow", doc[0].name)
		return 1
	return 0
...
def get_user_list(frequency):
	...
	User = DocType("User")
	return (
		kanivin.qb.from_(DocumentFollow)
		.join(User)
...
def get_version(doctype, doc_name, frequency, user):
	...
	if version:
		for v in version:
			change = kanivin.parse_json(v.data)
			time = kanivin.utils.format_datetime(v.modified, "hh:mm a")
			timeline_items = []
			if change.changed:
...
def get_comments(doctype, doc_name, frequency, user):
	...
	for comment in comments:
		...
		time = kanivin.utils.format_datetime(comment.modified, "hh:mm a")
		timeline.append(
			{
...
def _get_filters(frequency, user):
	...
	if frequency == "Weekly":
		filters += [
			["modified", ">", kanivin.utils.add_days(kanivin.utils.nowdate(), -7)],
			["modified", "<", kanivin.utils.nowdate()],
		]

	elif frequency == "Daily":
		filters += [
			["modified", ">", kanivin.utils.add_days(kanivin.utils.nowdate(), -1)],
			["modified", "<", kanivin.utils.nowdate()],
		]

	elif frequency == "Hourly":
		filters += [
			["modified", ">", kanivin.utils.add_to_date(kanivin.utils.now_datetime(), hours=-1)],
			["modified", "<", kanivin.utils.now_datetime()],
		]
