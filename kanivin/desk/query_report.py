...
from kanivin.core.utils import ljust_list
from kanivin.desk.reportview import clean_params, parse_json
from kanivin.model.utils import render_include
from kanivin.modules import get_module_path, scrub
from kanivin.monitor import add_data_to_monitor
from kanivin.permissions import get_role_permissions, has_permission
from kanivin.utils import cint, cstr, flt, format_duration, get_html_format, sbool


def get_report_doc(report_name):
	doc = kanivin.get_doc("Report", report_name)
	doc.custom_columns = []
	doc.custom_filters = []
	...
	if not kanivin.has_permission(doc.ref_doctype, "report"):
		kanivin.throw(
	...
	if doc.disabled:
		kanivin.throw(_("Report {0} is disabled").format(_(report_name)))
...
def generate_report_result(
	...
	return {
		...
		"status": None,
		"execution_time": kanivin.cache.hget("report_execution_time", report.name) or 0,
	}
...
def get_script(report_name):
	...
	module = report.module or kanivin.db.get_value("DocType", report.ref_doctype, "module")
	...
	if not script:
		script = "kanivin.query_reports['%s']={}" % report_name

	return {
		"script": render_include(script),
		"html_format": html_format,
		"execution_time": kanivin.cache.hget("report_execution_time", report_name) or 0,
...
def get_reference_report(report):
	if report.report_type != "Custom Report":
		return report
	reference_report = kanivin.get_doc("Report", report.reference_report)
...
def run(
	...
	report = get_report_doc(report_name)
	if not kanivin.has_permission(report.ref_doctype, "report"):
		kanivin.msgprint(
	...
	try:
		...
		kanivin.log_error("Report Error")
		raise
...
def add_custom_column_data(custom_columns, result):
	...
	for column in custom_columns:
		if len(column["fieldname"].split("-")) > 1:
			...
			doctype_name = kanivin.unscrub(column["fieldname"].split("-")[1])
		...
		if key in custom_column_data:
			for row in result:
				...
				if isinstance(link_field, str):
					link_field = kanivin._dict({"fieldname": link_field, "names": []})
...
def get_prepared_report_result(report, filters, dn="", user=None):
	from kanivin.core.doctype.prepared_report.prepared_report import get_completed_prepared_report
	...
	doc = kanivin.get_doc("Prepared Report", dn) if dn else None
	if doc:
		try:
			...
			kanivin.msgprint(_("Prepared report render failed") + f": {e!s}")
			doc = None
...
def export_query():
	...
	form_params = kanivin._dict(kanivin.local.form_dict)
	csv_params = pop_csv_params(form_params)
	clean_params(form_params)
	parse_json(form_params)

	report_name = form_params.report_name
	kanivin.permissions.can_export(
		kanivin.get_cached_value("Report", report_name, "ref_doctype"),
		raise_exception=True,
	)

	file_format_type = form_params.file_format_type
	custom_columns = kanivin.parse_json(form_params.custom_columns or "[]")
...
def format_fields(data: kanivin._dict) -> None:
...
def get_data_for_custom_field(doctype, field, names=None):
	if not kanivin.has_permission(doctype, "read"):
		kanivin.throw(_("Not Permitted to read {0}").format(_(doctype)), kanivin.PermissionError)

	filters = {}
	if names:
		if isinstance(names, str | bytearray):
			names = kanivin.json.loads(names)
		filters.update({"name": ["in", names]})

	return kanivin._dict(kanivin.get_list(doctype, filters=filters, fields=["name", field], as_list=1))
...
def save_report(reference_report, report_name, columns, filters):
	...
	if docname:
		report = kanivin.get_doc("Report", docname)
		...
		report.save()
		kanivin.msgprint(_("Report updated successfully"))

		return docname
	else:
		new_report = kanivin.get_doc(
			{
				"doctype": "Report",
				"report_name": report_name,
		...
		kanivin.msgprint(_("{0} saved successfully").format(_(new_report.name)))
...
def get_filtered_data(ref_doctype, columns, data, user):
	...
	shared = kanivin.share.get_shared(ref_doctype, user)
	columns_dict = get_columns_dict(columns)

	role_permissions = get_role_permissions(kanivin.get_meta(ref_doctype), user)
			...
			for match_filters in filter_list:
				...
				for dt, idx in linked_doctypes.items():
					...
					if (
						...
						and kanivin.db.exists(dt, cell_value)
					):
						match = False
						break
...
def get_column_as_dict(col):
	...
	if isinstance(col, str):
		...
		col_dict["fieldname"] = kanivin.scrub(col[0])

	# dict
	else:
		col_dict.update(col)
		if "fieldname" not in col_dict:
			col_dict["fieldname"] = kanivin.scrub(col_dict["label"])
...
def get_user_match_filters(doctypes, user):
	...
	for dt in doctypes:
		filter_list = kanivin.desk.reportview.build_match_conditions(dt, user, False)
		if filter_list:
...
def validate_filters_permissions(report_name, filters=None, user=None):
	...
	report = kanivin.get_doc("Report", report_name)
