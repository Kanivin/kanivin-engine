kanivin.pages["leaderboard"].on_page_load = (wrapper) => {
	kanivin.leaderboard = new Leaderboard(wrapper);

	$(wrapper).bind("show", () => {
		// Get which leaderboard to show
		let doctype = kanivin.get_route()[1];
		kanivin.leaderboard.show_leaderboard(doctype);
	});
};
...
class Leaderboard {
	...
	get_leaderboard_config() {
		...
		kanivin
			.xcall("kanivin.desk.page.leaderboard.leaderboard.get_leaderboard_config")
			.then((config) => {
				...
				const _initial_doctype = kanivin.get_route()[1] || this.doctypes[0];
	...
	setup_leaderboard_fields() {
		this.company_select = this.page.add_field({
			...
			fieldtype: "Link",
			options: "Company",
			default: kanivin.defaults.get_default("company"),
			reqd: 1,
			change: (e) => {
				this.make_request();
			},
		...
		this.type_select = this.page.add_select(
			...
			this.options.selected_filter.map((d) => {
				return { label: __(kanivin.model.unscrub(d)), value: d };
			})
		);
	...
	create_date_range_field() {
		let timespan_field = $(this.parent).find(
			`.kanivin-control[data-original-title="${__("Timespan")}"]`
		);
		this.date_range_field = $(`<div class="from-date-field"></div>`)
			.insertAfter(timespan_field)
			.hide();

		let date_field = kanivin.ui.form.make_control({
			df: {
				fieldtype: "DateRange",
				fieldname: "selected_date_range",
				placeholder: __("Date Range"),
				default: [kanivin.datetime.month_start(), kanivin.datetime.now_date()],
	...
	render_selected_doctype() {
		this.$sidebar_list.on("click", "li", (e) => {
			...
			this.company_select.set_value(
				kanivin.defaults.get_default("company") || this.company_select.get_value()
			);
			...
			kanivin.set_route("leaderboard", this.options.selected_doctype);
			this.make_request();
		});
	}
	...
	make_request() {
		kanivin.model.with_doctype(this.options.selected_doctype, () => {
	...
	get_leaderboard(notify) {
		...
		if (!company && !this.leaderboard_config[this.options.selected_doctype].company_disabled) {
			notify(this, null);
			kanivin.show_alert(__("Please select Company"));
			return;
		}
		kanivin
			...
			.then((r) => {
				...
				kanivin.utils.make_chart(".leaderboard-graph", custom_options);
	...
	get_leaderboard_data(me, res) {
		if (res && res.message.length) {
			...
			kanivin.utils.setup_search($(me.parent), ".list-item-container", ".list-id");
		} else {
			me.$graph_area.hide();
	...
	render_list_header() {
		const _selected_filter = this.options.selected_filter.map((i) => kanivin.model.unscrub(i));
		const fields = ["rank", "name", this.options.selected_filter_item];
		const filters = fields
			.map((filter) => {
				const col = __(kanivin.model.unscrub(filter));
	...
	render_message() {
		...
		return `<div class="leaderboard-empty-state ${display_class}">
  			<div class="no-result text-center">
  				<img src="/assets/kanivin/images/ui-states/search-empty-state.svg"
  					alt="Empty State"
  					class="null-state"
  				>
	...
	get_item_html(item, index) {
		...
		const link = `/app/${kanivin.router.slug(this.options.selected_doctype)}/${item.name}`;
	...
	get_sidebar_item(item, icon) {
		let icon_html = icon ? kanivin.utils.icon(icon, "md") : "";
	...
	get_date_range() {
		...
		let current_date = kanivin.datetime.now_date();
		let date_range_map = {
			"this week": [kanivin.datetime.week_start(), kanivin.datetime.week_end()],
			"this month": [kanivin.datetime.month_start(), kanivin.datetime.month_end()],
			"this quarter": [kanivin.datetime.quarter_start(), kanivin.datetime.quarter_end()],
			"this year": [kanivin.datetime.year_start(), kanivin.datetime.year_end()],
			"last week": [kanivin.datetime.add_days(current_date, -7), current_date],
			"last month": [kanivin.datetime.add_months(current_date, -1), current_date],
			"last quarter": [kanivin.datetime.add_months(current_date, -3), current_date],
			"last year": [kanivin.datetime.add_months(current_date, -12), current_date],
			"all time": null,
			"select date range": this.selected_date_range || [
				kanivin.datetime.month_start(),
				current_date,
			],
		};
