...
from kanivin.utils import cint, get_site_path, get_url
from kanivin.utils.data import convert_utc_to_system_timezone
...
def get_scheduled_backup_limit():
	backup_limit = kanivin.db.get_singles_value("System Settings", "backup_limit")
...
def schedule_files_backup(user_email):
	...
	queued_jobs = get_jobs(site=kanivin.local.site, queue="long")
	method = "kanivin.desk.page.backups.backups.backup_files_and_notify_user"

	if method not in queued_jobs[kanivin.local.site]:
		enqueue(
			"kanivin.desk.page.backups.backups.backup_files_and_notify_user",
			queue="long",
			user_email=user_email,
		)
		kanivin.msgprint(_("Queued for backup. You will receive an email with the download link"))
	else:
		kanivin.msgprint(_("Backup job is already queued. You will receive an email with the download link"))


def backup_files_and_notify_user(user_email=None):
	from kanivin.utils.backups import backup
