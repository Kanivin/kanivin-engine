import BaseTimeline from "../../../public/js/kanivin/form/footer/base_timeline";
kanivin.provide("kanivin.energy_points");
...
class UserProfile {
	...
	show() {
		...
		this.user_id = route[1] || kanivin.session.user;
		kanivin.dom.freeze(__("Loading user profile") + "...");
		kanivin.db.exists("User", this.user_id).then((exists) => {
			kanivin.dom.unfreeze();
			if (exists) {
				this.make_user_profile();
			} else {
				kanivin.msgprint(__("User does not exist"));
			}
		});
	}

	make_user_profile() {
		this.user = kanivin.user_info(this.user_id);
		this.page.set_title(this.user.fullname);
		this.setup_user_search();
		this.main_section.empty().append(kanivin.render_template("user_profile"));
	...
	show_user_search_dialog() {
		let dialog = new kanivin.ui.Dialog({
			...
			primary_action: ({ user }) => {
				dialog.hide();
				kanivin.set_route("user-profile", user);
			},
		});
		dialog.show();
	}
	...
	render_heatmap() {
		this.heatmap = new kanivin.Chart(".performance-heatmap", {
			type: "heatmap",
	...
	update_heatmap_data(date_from) {
		kanivin
			.xcall("kanivin.desk.page.user_profile.user_profile.get_energy_points_heatmap_data", {
				user: this.user_id,
				date: date_from || kanivin.datetime.year_start(),
			})
			.then((r) => {
	...
	render_line_chart() {
		...
		this.line_chart = new kanivin.Chart(".performance-line-chart", {
			type: "line",
			height: 200,
			data: {
				labels: [],
	...
	update_line_chart_data() {
		...
		kanivin
			.xcall("kanivin.desk.doctype.dashboard_chart.dashboard_chart.get", {
	...
	render_percentage_chart(field, title) {
		kanivin
			.xcall(
				"kanivin.desk.page.user_profile.user_profile.get_energy_points_percentage_chart_data",
				{
					user: this.user_id,
					field: field,
				}
			...
			.then((chart) => {
				if (chart.labels.length) {
					this.percentage_chart = new kanivin.Chart(".performance-percentage-chart", {
						type: "percentage",
						data: {
							labels: chart.labels,
	...
	create_line_chart_filters() {
		...
		kanivin.dashboard_utils.render_chart_filters(
			filters,
			"chart-filter",
			".line-chart-options",
			1
	...
	create_heatmap_chart_filters() {
		let filters = [
			{
				label: kanivin.dashboard_utils.get_year(kanivin.datetime.now_date()),
				options: kanivin.dashboard_utils.get_years_since_creation(
					kanivin.boot.user.creation
				),
				action: (selected_item) => {
					this.update_heatmap_data(kanivin.datetime.obj_to_str(selected_item));
				},
			},
		];
		kanivin.dashboard_utils.render_chart_filters(filters, "chart-filter", ".heatmap-options");
	}

	edit_profile() {
		let edit_profile_dialog = new kanivin.ui.Dialog({
			...
			primary_action: (values) => {
				...
				kanivin
					.xcall("kanivin.desk.page.user_profile.user_profile.update_profile_info", {
						profile_info: values,
					})
					.then((user) => {
	...
	render_user_details() {
		this.sidebar.empty().append(
			kanivin.render_template("user_profile_sidebar", {
				user_image: this.user.image,
	...
	setup_user_profile_links() {
		if (this.user_id !== kanivin.session.user) {
	...
	get_user_rank() {
		return kanivin
			.xcall("kanivin.desk.page.user_profile.user_profile.get_user_rank", {
				user: this.user_id,
			})
			.then((r) => {
	...
	get_user_points() {
		return kanivin
			.xcall(
				"kanivin.social.doctype.energy_point_log.energy_point_log.get_user_energy_and_review_points",
				{
					user: this.user_id,
				}
			)
	...
	render_points_and_rank() {
		...
		const _get_stat_dom = (value, label, icon) => {
			return `<div class="user-stats-item mt-4">
				${kanivin.utils.icon(icon, "lg", "no-stroke")}
				<div>
	...
	go_to_user_settings() {
		kanivin.set_route("Form", "User", this.user_id);
	}
...
class UserProfileTimeline extends BaseTimeline {
	...
	get_user_activity_data() {
		return kanivin.xcall("kanivin.desk.page.user_profile.user_profile.get_energy_points_list", {
	...
	get_activity_timeline_item(data) {
		...
		return {
			...
			is_card: true,
			content: kanivin.energy_points.format_history_log(data),
		};
	}
...
kanivin.ui.UserProfile = UserProfile;
