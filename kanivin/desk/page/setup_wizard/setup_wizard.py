...
from kanivin.geo.country_info import get_country_info
from kanivin.permissions import AUTOMATIC_ROLES
from kanivin.translate import send_translations, set_default_language
from kanivin.utils import cint, now, strip
...
def get_setup_stages(args):  # nosemgrep
	# App setup stage functions should not include kanivin.db.commit
	# That is done by kanivin after successful completion of all stages
	stages = [
		{
...
def setup_complete(args):
	...
	if cint(kanivin.db.get_single_value("System Settings", "setup_complete")):
		return {"status": "ok"}

	args = parse_args(sanitize_input(args))
	stages = get_setup_stages(args)
	is_background_task = kanivin.conf.get("trigger_site_setup_in_background")
...
def process_setup_stages(stages, user_input, is_background_task=False):
	...
	try:
		kanivin.flags.in_setup_wizard = True
		current_task = None
		...
		kanivin.log_error(title=f"Setup failed: {message}")
		if not is_background_task:
			kanivin.response["setup_wizard_failure_message"] = message
			raise
		kanivin.publish_realtime(
			"setup_task",
		...
		if not is_background_task:
			return {"status": "ok"}
		kanivin.publish_realtime("setup_task", {"status": "ok"}, user=kanivin.session.user)
	finally:
		kanivin.flags.in_setup_wizard = False
...
def run_post_setup_complete(args):  # nosemgrep
	...
	kanivin.get_cached_doc("System Settings") and kanivin.get_doc("System Settings")


def run_setup_success(args):  # nosemgrep
	for hook in kanivin.get_hooks("setup_wizard_success"):
		kanivin.get_attr(hook)(args)
	install_fixtures.install()


def get_stages_hooks(args):  # nosemgrep
	stages = []
	for method in kanivin.get_hooks("setup_wizard_stages"):
		stages += kanivin.get_attr(method)(args)
	return stages
	...
	return [
		{
			...
			"tasks": [
				{
					"fn": kanivin.get_attr(method),
					"args": args,
					"fail_msg": "Failed to execute method",
				}
			],
		}
		for method in kanivin.get_hooks("setup_wizard_complete")
	]
...
def handle_setup_exception(args):  # nosemgrep
	...
	if args:
		...
		print(traceback)
		for hook in kanivin.get_hooks("setup_wizard_exception"):
			kanivin.get_attr(hook)(traceback, args)
...
def update_system_settings(args):  # nosemgrep
	...
	system_settings = kanivin.get_doc("System Settings", "System Settings")
	system_settings.update(
		{
			...
			"date_format": kanivin.db.get_value("Country", args.get("country"), "date_format"),
			"time_format": kanivin.db.get_value("Country", args.get("country"), "time_format"),
			"number_format": number_format,
			"enable_scheduler": 1 if not kanivin.flags.in_test else 0,
	...
	if args.get("enable_telemetry"):
		kanivin.db.set_default("session_recording_start", now())
...
def create_or_update_user(args):  # nosemgrep
	...
	if user := kanivin.db.get_value("User", email, ["first_name", "last_name"], as_dict=True):
		if user.first_name != first_name or user.last_name != last_name:
			(
				kanivin.qb.update("User")
				.set("first_name", first_name)
				.set("last_name", last_name)
				.set("full_name", args.get("full_name"))
			).run()
	else:
		_mute_emails, kanivin.flags.mute_emails = kanivin.flags.mute_emails, True
		...
		kanivin.flags.mute_emails = _mute_emails
...
def set_timezone(args):  # nosemgrep
	if args.get("timezone"):
		for name in kanivin.STANDARD_USERS:
			kanivin.db.set_value("User", name, "time_zone", args.get("timezone"))


def parse_args(args):  # nosemgrep
	if not args:
		args = kanivin.local.form_dict
...
def add_all_roles_to(name):
	user = kanivin.get_doc("User", name)
	user.append_roles(*_get_default_roles())
	user.save()
...
def _get_default_roles() -> set[str]:
	...
	return set(kanivin.get_all("Role", pluck="name")) - skip_roles


def disable_future_access():
	kanivin.db.set_default("desktop:home_page", "workspace")
	# Enable onboarding after install
	kanivin.db.set_single_value("System Settings", "enable_onboarding", 1)

	kanivin.db.set_single_value("System Settings", "setup_complete", 1)
...
def load_messages(language):
	...
	javascript files"""
	from kanivin.translate import get_messages_for_boot
...
def load_languages():
	...
	return {
		"default_language": kanivin.db.get_value("Language", kanivin.local.lang, "language_name")
		or kanivin.local.lang,
		"languages": sorted(kanivin.db.sql_list("select language_name from tabLanguage order by name")),
...
def load_country():
	...
	return get_geo_ip_country(kanivin.local.request_ip) if kanivin.local.request_ip else None


@kanivin.whitelist()
def load_user_details():
	return {
		"full_name": kanivin.cache.hget("full_name", "signup"),
		"email": kanivin.cache.hget("email", "signup"),
	}
...
def email_setup_wizard_exception(traceback, args):  # nosemgrep
	if not kanivin.conf.setup_wizard_exception_email:
		return
...
- **User:** {user}""".format(
		...
		args="\n".join(pretty_args),
		user=kanivin.session.user,
		headers=kanivin.request.headers if kanivin.request else "[no request]",
	)

	kanivin.sendmail(
		recipients=kanivin.conf.setup_wizard_exception_email,
		sender=kanivin.session.user,
		subject=f"Setup failed: {kanivin.local.site}",
		message=message,
		delayed=False,
	)
...
def get_language_code(lang):
	return kanivin.db.get_value("Language", {"language_name": lang})


def enable_twofactor_all_roles():
	all_role = kanivin.get_doc("Role", {"role_name": "All"})
	all_role.two_factor_auth = True
	all_role.save(ignore_permissions=True)


def make_records(records, debug=False):
	from kanivin import _dict
...
def show_document_insert_error():
	...
	kanivin.log_error("Exception during Setup")
