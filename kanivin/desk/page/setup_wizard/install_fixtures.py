...
def update_salutations():
	for salutation in (
		...
		doc = kanivin.new_doc("Salutation")
		doc.salutation = salutation
		doc.insert(ignore_permissions=True, ignore_if_duplicate=True)


def setup_email_linking():
	doc = kanivin.new_doc("Email Account")
...
def add_unsubscribe():
	for unsubscribe in [
		...
		if not kanivin.get_all("Email Unsubscribe", filters=unsubscribe):
			doc = kanivin.new_doc("Email Unsubscribe")
