...
def update_event(args, field_map):
	...
	args = ._dict(json.loads(args))
	field_map = ._dict(json.loads(field_map))
	w = .get_doc(args.doctype, args.name)
...
def get_event_conditions(doctype, filters=None):
	...
	from .desk.reportview import get_filters_cond

	if not .has_permission(doctype):
...
def get_events(doctype, start, end, field_map, filters=None, fields=None):
	...
	return .get_list(doctype, fields=fields, filters=filters)
