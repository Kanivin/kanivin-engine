...
from kanivin.boot import get_allowed_report_names
...
from kanivin.utils import cint, get_datetime, getdate, has_common, now_datetime, nowdate
from kanivin.utils.dashboard import cache_source
from kanivin.utils.data import format_date
from kanivin.utils.dateutils import (
...
def get_permission_query_conditions(user):
	...
	allowed_doctypes = [kanivin.db.escape(doctype) for doctype in kanivin.permissions.get_doctypes_with_read()]
	allowed_reports = [kanivin.db.escape(report) for report in get_allowed_report_names()]
	allowed_modules = [
		kanivin.db.escape(module.get("module_name")) for module in get_modules_from_all_apps_for_user()
	]
...
def has_permission(doc, ptype, user):
	...
	if doc.roles:
		...
		allowed_doctypes = kanivin.permissions.get_doctypes_with_read()
...
def get(
	...
	if chart_name:
		chart: DashboardChart = kanivin.get_doc("Dashboard Chart", chart_name)
	else:
		chart = kanivin._dict(kanivin.parse_json(chart))
	...
	filters = kanivin.parse_json(filters) or kanivin.parse_json(chart.filters_json)
	if not filters:
		filters = []
...
def create_dashboard_chart(args):
	...
	doc = kanivin.new_doc("Dashboard Chart")
	...
	if kanivin.db.exists("Dashboard Chart", args.chart_name):
...
def add_chart_to_dashboard(args):
	...
	dashboard = kanivin.get_doc("Dashboard", args.dashboard)
	dashboard_link = kanivin.new_doc("Dashboard Chart Link")
	dashboard_link.chart = args.chart_name or args.name

	if args.set_standard and dashboard.is_standard:
		chart = kanivin.get_doc("Dashboard Chart", dashboard_link.chart)
		chart.is_standard = 1
...
def get_charts_for_user(doctype, txt, searchfield, start, page_len, filters):
	or_filters = {"owner": kanivin.session.user, "is_public": 1}
...
class DashboardChart(Document):
	...
	if TYPE_CHECKING:
		...
		from kanivin.desk.doctype.dashboard_chart_field.dashboard_chart_field import DashboardChartField
	...
	def on_update(self):
		kanivin.cache.delete_key(f"chart-data:{self.name}")
	...
	def validate(self):
		if not kanivin.conf.developer_mode and self.is_standard:
			kanivin.throw(_("Cannot edit Standard charts"))
	...
	def check_required_field(self):
		if not self.document_type:
			kanivin.throw(_("Document type is required to create a dashboard chart"))

		if (
			self.document_type
			and kanivin.get_meta(self.document_type).istable
			and not self.parent_document_type
		):
			kanivin.throw(_("Parent document type is required to create a dashboard chart"))

		if self.chart_type == "Group By":
			if not self.group_by_based_on:
				kanivin.throw(_("Group By field is required to create a dashboard chart"))
			if self.group_by_type in ["Sum", "Average"] and not self.aggregate_function_based_on:
				kanivin.throw(_("Aggregate Function field is required to create a dashboard chart"))
		else:
			if not self.based_on:
				kanivin.throw(_("Time series based on is required to create a dashboard chart"))

	def check_document_type(self):
		if kanivin.get_meta(self.document_type).issingle:
			kanivin.throw(_("You cannot create a dashboard chart from single DocTypes"))
	...
	def validate_custom_options(self):
		if self.custom_options:
			try:
				...
				kanivin.throw(_("Invalid json added in the custom options: {0}").format(error))
