...
kanivin.ui.form.on("Dashboard Chart", {
	setup: function (frm) {
	...
	before_save: function (frm) {
		...
		static_filters = kanivin.dashboard_utils.remove_common_static_filter_values(
			static_filters,
			dynamic_filters
		);
	...
	refresh: function (frm) {
		...
		frm.is_disabled = !kanivin.boot.developer_mode && frm.doc.is_standard;
		...
		if (!frm.is_new()) {
			frm.add_custom_button("Add Chart to Dashboard", () => {
				const dialog = kanivin.dashboard_utils.get_add_to_dashboard_dialog(
					frm.doc.name,
					"Dashboard Chart",
					"kanivin.desk.doctype.dashboard_chart.dashboard_chart.add_chart_to_dashboard"
				);

				if (!frm.doc.chart_name) {
					kanivin.msgprint(__("Please create chart first"));
				} else {
					dialog.show();
				}
			});
	...
	set_heatmap_year_options: function (frm) {
		if (frm.doc.type == "Heatmap") {
			kanivin.db.get_doc("System Settings").then((doc) => {
				const creation_date = doc.creation;
				frm.set_df_property(
					"heatmap_year",
					"options",
					kanivin.dashboard_utils.get_years_since_creation(creation_date)
				);
			});
		}
	},
	...
	set_chart_report_filters: function (frm) {
		...
		if (report_name) {
			if (frm.doc.filters_json.length > 2) {
				...
				kanivin.report_utils.get_report_filters(report_name).then((filters) => {
					if (filters) {
						frm.chart_filters = filters;
						let filter_values = kanivin.report_utils.get_filter_values(filters);
	...
	set_chart_field_options: function (frm) {
		...
		if (frm.doc.dynamic_filters_json && frm.doc.dynamic_filters_json.length > 2) {
			filters = kanivin.dashboard_utils.get_all_filters(frm.doc);
		}
		kanivin
			.xcall("kanivin.desk.query_report.run", {
			...
			.then((data) => {
				...
				if (!frm.doc.use_report_chart) {
					if (data.result.length) {
						frm.field_options = kanivin.report_utils.get_field_options_from_report(
							data.columns,
							data
						);
						frm.set_df_property(
						...
						if (!frm.field_options.numeric_fields.length) {
							...
							let y_field_df = kanivin.meta.get_docfield(
	...
	update_options: function (frm) {
		...
		if (doctype) {
			kanivin.model.with_doctype(doctype, () => {
				...
				kanivin.get_meta(doctype).fields.map((df) => {
	...
	show_filters: function (frm) {
		frm.chart_filters = [];
		kanivin.dashboard_utils.get_filters_for_chart_type(frm.doc).then((filters) => {
			if (filters) {
				frm.chart_filters = filters;
			}
	...
	render_filters_table: function (frm) {
		...
		table.on("click", () => {
			frm.is_disabled && kanivin.throw(__("Cannot edit filters for standard charts"));
			...
			kanivin.dashboards.filters_dialog = dialog;

			if (is_document_type) {
				frm.filter_group = new kanivin.ui.FilterGroup({
			...
			if (frm.doc.chart_type == "Report") {
				...
				kanivin.query_reports[frm.doc.report_name] &&
					kanivin.query_reports[frm.doc.report_name].onload &&
					kanivin.query_reports[frm.doc.report_name].onload(kanivin.query_report);
			}
	...
	render_dynamic_filters_table(frm) {
		...
		let fields = kanivin.dashboard_utils.get_fields_for_dynamic_filter_dialog(
			is_document_type,
			filters,
			frm.dynamic_filters
		);
	...
	set_parent_document_type: async function (frm) {
		...
		kanivin.model.with_doctype(document_type, async () => {
			let doc_is_table = kanivin.get_meta(document_type).istable;
			frm.set_df_property("parent_document_type", "hidden", !doc_is_table);

			if (doc_is_table) {
				let parents = await kanivin.xcall(
					"kanivin.desk.doctype.dashboard_chart.dashboard_chart.get_parent_doctypes",
