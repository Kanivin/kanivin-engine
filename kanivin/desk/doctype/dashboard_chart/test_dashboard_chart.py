...
import kanivin
from kanivin.desk.doctype.dashboard_chart.dashboard_chart import get
from kanivin.tests.utils import KanivinTestCase
from kanivin.utils import formatdate, get_last_day, getdate
from kanivin.utils.dateutils import get_period, get_period_ending


class TestDashboardChart(KanivinTestCase):
	...
	def test_dashboard_chart(self):
		if kanivin.db.exists("Dashboard Chart", "Test Dashboard Chart"):
			kanivin.delete_doc("Dashboard Chart", "Test Dashboard Chart")
	...
	def test_empty_dashboard_chart(self):
		if kanivin.db.exists("Dashboard Chart", "Test Empty Dashboard Chart"):
			kanivin.delete_doc("Dashboard Chart", "Test Empty Dashboard Chart")

		kanivin.db.delete("Error Log")
	...
	def test_chart_wih_one_value(self):
		if kanivin.db.exists("Dashboard Chart", "Test Empty Dashboard Chart 2"):
			kanivin.delete_doc("Dashboard Chart", "Test Empty Dashboard Chart 2")

		kanivin.db.delete("Error Log")

		# create one data point
		kanivin.get_doc(dict(doctype="Error Log", creation="2018-06-01 00:00:00")).insert()
	...
	def test_group_by_chart_type(self):
		if kanivin.db.exists("Dashboard Chart", "Test Group By Dashboard Chart"):
			kanivin.delete_doc("Dashboard Chart", "Test Group By Dashboard Chart")

		kanivin.get_doc({"doctype": "ToDo", "description": "test"}).insert()
		...
		todo_status_count = kanivin.db.count("ToDo", {"status": result.get("labels")[0]})
	...
	def test_daily_dashboard_chart(self):
		...
		if kanivin.db.exists("Dashboard Chart", "Test Daily Dashboard Chart"):
			kanivin.delete_doc("Dashboard Chart", "Test Daily Dashboard Chart")
	...
	def test_weekly_dashboard_chart(self):
		...
		if kanivin.db.exists("Dashboard Chart", "Test Weekly Dashboard Chart"):
			kanivin.delete_doc("Dashboard Chart", "Test Weekly Dashboard Chart")
	...
	def test_avg_dashboard_chart(self):
		...
		if kanivin.db.exists("Dashboard Chart", "Test Average Dashboard Chart"):
			kanivin.delete_doc("Dashboard Chart", "Test Average Dashboard Chart")
	...
	def test_user_date_label_dashboard_chart(self):
		kanivin.delete_doc_if_exists("Dashboard Chart", "Test Dashboard Chart Date Label")
		...
		with patch.object(kanivin.utils.data, "get_user_date_format", return_value="dd.mm.yyyy"):
		...
		with patch.object(kanivin.utils.data, "get_user_date_format", return_value="mm-dd-yyyy"):
			result = get(chart_name="Test Dashboard Chart Date Label")
...
def create_new_communication(subject, date, rating):
	...
	comm = kanivin.get_doc(communication)
	if not kanivin.db.exists("Communication", {"subject": comm.subject}):
		comm.insert()
