...
class Note(Document):
	...
	if TYPE_CHECKING:
		from kanivin.desk.doctype.note_seen_by.note_seen_by import NoteSeenBy
	...
	def validate(self):
		if self.notify_on_login and not self.expire_notification_on:
			...
			self.expire_notification_on = kanivin.utils.add_days(self.creation, 7)
...
def mark_as_seen(note: str):
	note: Note = kanivin.get_doc("Note", note)
	note.mark_seen_by(kanivin.session.user)
...
def get_permission_query_conditions(user):
	...
	return f"(`tabNote`.owner = {kanivin.db.escape(user)} or `tabNote`.public = 1)"
