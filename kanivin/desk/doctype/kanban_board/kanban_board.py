...
class KanbanBoard(Document):
	...
	if TYPE_CHECKING:
		from kanivin.desk.doctype.kanban_board_column.kanban_board_column import KanbanBoardColumn
	...
	def on_change(self):
		kanivin.clear_cache(doctype=self.reference_doctype)
		kanivin.cache.delete_keys("_user_settings")
	...
	def validate_column_name(self):
		for column in self.columns:
			if not column.column_name:
				kanivin.msgprint(_("Column Name cannot be empty"), raise_exception=True)
...
def get_permission_query_conditions(user):
	...
	return f"""(`tabKanban Board`.private=0 or `tabKanban Board`.owner={kanivin.db.escape(user)})"""
...
def add_column(board_name, column_title):
	...
	doc = kanivin.get_doc("Kanban Board", board_name)
	for col in doc.columns:
		if column_title == col.column_name:
			kanivin.throw(_("Column <b>{0}</b> already exist.").format(column_title))
...
def update_order(board_name, order):
	...
	board = kanivin.get_doc("Kanban Board", board_name)
	doctype = board.reference_doctype
	updated_cards = []

	if not kanivin.has_permission(doctype, "write"):
	...
	for col_name, cards in order_dict.items():
		for card in cards:
			column = kanivin.get_value(doctype, {"name": card}, fieldname)
			if column != col_name:
				kanivin.set_value(doctype, card, fieldname, col_name)
...
def update_order_for_single_card(board_name, docname, from_colname, to_colname, old_index, new_index):
	...
	fieldname = board.field_name
	old_index = kanivin.parse_json(old_index)
	new_index = kanivin.parse_json(new_index)
	...
	# save updated order
	board.columns[from_col_idx].order = kanivin.as_json(from_col_order)
	board.columns[to_col_idx].order = kanivin.as_json(to_col_order)
	board.save(ignore_permissions=True)

	# update changed value in doc
	kanivin.set_value(doctype, docname, fieldname, to_colname)
...
def get_kanban_column_order_and_index(board, colname):
	for i, col in enumerate(board.columns):
		if col.column_name == colname:
			col_order = kanivin.parse_json(col.order)
			col_idx = i
...
def add_card(board_name, docname, colname):
	...
	kanivin.has_permission(board.reference_doctype, "write", throw=True)

	col_order, col_idx = get_kanban_column_order_and_index(board, colname)
	col_order.insert(0, docname)

	board.columns[col_idx].order = kanivin.as_json(col_order)
...
def quick_kanban_board(doctype, board_name, field_name, project=None):
	...
	doc = kanivin.new_doc("Kanban Board")
...
def get_order_for_column(board, colname):
	...
	if board.filters:
		filters.append(kanivin.parse_json(board.filters)[0])

	return kanivin.as_json(kanivin.get_list(board.reference_doctype, filters=filters, pluck="name"))
...
def save_settings(board_name: str, settings: str) -> Document:
	...
	resp = doc.as_dict()
	resp["fields"] = kanivin.parse_json(resp["fields"])
