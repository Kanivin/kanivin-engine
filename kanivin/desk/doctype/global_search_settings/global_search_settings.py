...
class GlobalSearchSettings(Document):
	...
	if TYPE_CHECKING:
		from kanivin.desk.doctype.global_search_doctype.global_search_doctype import GlobalSearchDocType
	...
	def validate(self):
		...
		for dt in self.allowed_in_global_search:
			...
			if kanivin.get_meta(dt.document_type).module == "Core":
				core_dts.append(dt.document_type)

			dts.append(dt.document_type)

		if core_dts:
			core_dts = ", ".join(kanivin.bold(dt) for dt in core_dts)
			kanivin.throw(_("Core Modules {0} cannot be searched in Global Search.").format(core_dts))

		if repeated_dts:
			repeated_dts = ", ".join([kanivin.bold(dt) for dt in repeated_dts])
			kanivin.throw(_("Document Type {0} has been repeated.").format(repeated_dts))

		# reset cache
		kanivin.cache.hdel("global_search", "search_priorities")


def get_doctypes_for_global_search():
	def get_from_db():
		doctypes = kanivin.get_all("Global Search DocType", fields=["document_type"], order_by="idx ASC")
		return [d.document_type for d in doctypes] or []

	return kanivin.cache.hget("global_search", "search_priorities", get_from_db)
...
def update_global_search_doctypes():
	...
	installed_apps = [app for app in kanivin.get_installed_apps() if app]
	active_domains = [domain for domain in kanivin.get_active_domains() if domain]
	active_domains.append("Default")

	for app in installed_apps:
		search_doctypes = kanivin.get_hooks(hook="global_search_doctypes", app_name=app)
	...
	doctype_list = {dt.name for dt in kanivin.get_all("DocType")}
	...
	global_search_settings = kanivin.get_single("Global Search Settings")
