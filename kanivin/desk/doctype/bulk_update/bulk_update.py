	...
	def bulk_update(self):
		...
		if self.condition:
			if ";" in self.condition:
				kanivin.throw(_("; not allowed in condition"))

			condition = f" where {self.condition}"

		docnames = kanivin.db.sql_list(
...
def submit_cancel_or_update_docs(doctype, docnames, action="submit", data=None, task_id=None):
	...
	if len(docnames) < 20:
		...
		kanivin.msgprint(_("Bulk operation is enqueued in background."), alert=True)
		kanivin.enqueue(
			_bulk_action,
			doctype=doctype,
			docnames=docnames,
		...
		kanivin.throw(_("Bulk operations only support up to 500 documents."), title=_("Too Many Documents"))


def _bulk_action(doctype, docnames, action, data, task_id=None):
	if data:
		data = kanivin.parse_json(data)
