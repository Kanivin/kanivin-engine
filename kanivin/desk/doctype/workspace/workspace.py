...
from kanivin.desk.desktop import save_new_widget
...
from kanivin.modules.export_file import delete_folder, export_to_files
...
class Workspace(Document):
	...
	if TYPE_CHECKING:
		...
		from kanivin.desk.doctype.workspace_chart.workspace_chart import WorkspaceChart
		from kanivin.desk.doctype.workspace_custom_block.workspace_custom_block import WorkspaceCustomBlock
		from kanivin.desk.doctype.workspace_link.workspace_link import WorkspaceLink
		from kanivin.desk.doctype.workspace_number_card.workspace_number_card import WorkspaceNumberCard
		from kanivin.desk.doctype.workspace_quick_list.workspace_quick_list import WorkspaceQuickList
		from kanivin.desk.doctype.workspace_shortcut.workspace_shortcut import WorkspaceShortcut
	...
	def validate(self):
		...
		if self.public and not is_workspace_manager() and not disable_saving_as_public():
			kanivin.throw(_("You need to be Workspace Manager to edit this document"))
		...
		try:
			...
			kanivin.throw(_("Content data shoud be a list"))

		for d in self.get("links"):
			if d.link_type == "Report" and d.is_query_report != 1:
				d.report_ref_doctype = kanivin.get_value("Report", d.link_to, "ref_doctype")

	def clear_cache(self):
		super().clear_cache()
		if self.for_user:
			kanivin.cache.hdel("bootinfo", self.for_user)
		else:
	...
	def on_update(self):
		...
		if kanivin.conf.developer_mode and self.public:
			if self.module:
	...
	def on_trash(self):
		if self.public and not is_workspace_manager():
			kanivin.throw(_("You need to be Workspace Manager to delete a public workspace."))

	def after_delete(self):
		if disable_saving_as_public():
			return

		if self.module and kanivin.conf.developer_mode:
			delete_folder(self.module, "Workspace", self.title)

	@staticmethod
	def get_module_wise_workspaces():
		workspaces = kanivin.get_all(
			"Workspace",
			fields=["name", "module"],
		...
		for link in self.links:
			...
			if link.type == "Card Break":
				if card_links and (
					...
					or current_card.get("only_for") == kanivin.get_system_settings("country")
				):
				...
				current_card = link
				card_links = []
			elif not link.get("only_for") or link.get("only_for") == kanivin.get_system_settings("country"):
...
def get_report_type(report):
	report_type = kanivin.get_value("Report", report, "report_type")
...
def new_page(new_page):
	...
	if page.get("public") and not is_workspace_manager():
		...
		not page.get("public") and page.get("for_user") != kanivin.session.user and not is_workspace_manager()
	):
		kanivin.throw(_("Cannot create private workspace of other users"), kanivin.PermissionError)

	elif not kanivin.has_permission(doctype="Workspace", ptype="create"):
		kanivin.flags.error_message = _("User {0} does not have the permission to create a Workspace.").format(
			kanivin.bold(kanivin.session.user)
		)
		raise kanivin.PermissionError

	doc = kanivin.new_doc("Workspace")
...
def save_page(title, public, new_widgets, blocks):
	public = kanivin.parse_json(public)

	filters = {"public": public, "label": title}

	if not public:
		filters = {"for_user": kanivin.session.user, "label": title + "-" + kanivin.session.user}
	pages = kanivin.get_all("Workspace", filters=filters)
	if pages:
		doc = kanivin.get_doc("Workspace", pages[0])
	else:
		kanivin.throw(_("Workspace not found"), kanivin.DoesNotExistError)
...
def update_page(name, title, icon, indicator_color, parent, public):
	public = kanivin.parse_json(public)
	doc = kanivin.get_doc("Workspace", name)

	if not doc.get("public") and doc.get("for_user") != kanivin.session.user and not is_workspace_manager():
		kanivin.throw(
	...
	if doc:
		child_docs = kanivin.get_all("Workspace", filters={"parent_page": doc.title, "public": doc.public})
		doc.title = title
		doc.icon = icon
		doc.indicator_color = indicator_color
		doc.parent_page = parent
		if doc.public != public:
			doc.sequence_id = kanivin.db.count("Workspace", {"public": public}, cache=True)
			doc.public = public
		doc.for_user = "" if public else doc.for_user or kanivin.session.user
		...
		if child_docs:
			for child in child_docs:
				child_doc = kanivin.get_doc("Workspace", child.name)
				child_doc.parent_page = doc.title
				if child_doc.public != public:
					child_doc.public = public
				child_doc.for_user = "" if public else child_doc.for_user or kanivin.session.user
...
def hide_unhide_page(page_name: str, is_hidden: bool):
	page = kanivin.get_doc("Workspace", page_name)

	if page.get("public") and not is_workspace_manager():
		kanivin.throw(
			_("Need Workspace Manager role to hide/unhide public workspaces"), kanivin.PermissionError
		)

	if not page.get("public") and page.get("for_user") != kanivin.session.user and not is_workspace_manager():
		kanivin.throw(_("Cannot update private workspace of other users"), kanivin.PermissionError)
...
def duplicate_page(page_name, new_page):
	...
	old_doc = kanivin.get_doc("Workspace", page_name)
	doc = kanivin.copy_doc(old_doc)
	...
	doc.module = ""
	if not doc.public:
		doc.for_user = doc.for_user or kanivin.session.user
...
def delete_page(page):
	...
	if page.get("public") and not is_workspace_manager():
		...
		workspace_owner = kanivin.get_value("Workspace", page.get("name"), "for_user")
		if workspace_owner != kanivin.session.user:
			kanivin.throw(
				_("Cannot delete private workspace of other users"),
				kanivin.PermissionError,
			)

	if kanivin.db.exists("Workspace", page.get("name")):
		kanivin.get_doc("Workspace", page.get("name")).delete(ignore_permissions=True)
...
def sort_pages(sb_public_items, sb_private_items):
	...
	workspace_private_pages = get_page_list(["name", "title"], {"for_user": kanivin.session.user})
	...
	for seq, d in enumerate(pages):
		for page in workspace_pages:
			if page.title == d.get("title"):
				doc = kanivin.get_doc("Workspace", page.name)
...
def last_sequence_id(doc):
	doc_exists = kanivin.db.exists({"doctype": "Workspace", "public": doc.public, "for_user": doc.for_user})
...
def get_page_list(fields, filters):
	return kanivin.get_all("Workspace", fields=fields, filters=filters, order_by="sequence_id asc")


def is_workspace_manager():
	return "Workspace Manager" in kanivin.get_roles()
