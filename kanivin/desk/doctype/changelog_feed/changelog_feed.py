...
def fetch_changelog_feed():
	...
	for fn in kanivin.get_hooks("get_changelog_feed"):
		try:
			cache_key = f"changelog_feed::{fn}"
			changelog_feed = kanivin.cache.get_value(cache_key, shared=True)
			if changelog_feed is None:
				changelog_feed = kanivin.call(fn, since=since)[:20] or []
			...
			for feed_item in changelog_feed:
				...
				if not kanivin.db.exists("Changelog Feed", feed):
					kanivin.new_doc("Changelog Feed").update(feed).insert()
		except Exception:
			kanivin.log_error(f"Failed to fetch changelog from {fn}")
			# don't retry if it's broken for 1 week
			kanivin.cache.set_value(cache_key, [], expires_in_sec=7 * 24 * 60 * 60, shared=True)


@redis_cache
def get_changelog_feed_items():
	"""Returns a list of latest 10 changelog feed items"""
	feed = kanivin.get_all(
		"Changelog Feed",
		fields=["title", "app_name", "link", "posting_timestamp"],
		# allow pubishing feed for many apps with single hook
		filters={"app_name": ("in", kanivin.get_installed_apps())},
		order_by="posting_timestamp desc",
		limit=20,
	)
	for f in feed:
...
def _app_title(app_name):
	try:
		return kanivin.get_hooks("app_title", app_name=app_name)[0]
	except Exception:
		return app_name


def get_feed(since):
	"""'What's New' feed implementation for Kanivin"""
	r = requests.get(f"https://kanivin.io/api/method/changelog_feed?since={since}")
	r.raise_for_status()
