...
class NotificationLog(Document):
	...
	def after_insert(self):
		kanivin.publish_realtime("notification", after_commit=True, user=self.for_user)
	...
	def clear_old_logs(days=180):
		...
		table = kanivin.qb.DocType("Notification Log")
		kanivin.db.delete(table, filters=(table.modified < (Now() - Interval(days=days))))


def get_permission_query_conditions(for_user):
	if not for_user:
		for_user = kanivin.session.user

	if for_user == "Administrator":
		return

	return f"""(`tabNotification Log`.for_user = {kanivin.db.escape(for_user)})"""


def get_title(doctype, docname, title_field=None):
	if not title_field:
		title_field = kanivin.get_meta(doctype).get_title_field()
	return docname if title_field == "name" else kanivin.db.get_value(doctype, docname, title_field)
...
def enqueue_create_notification(users: list[str] | str, doc: dict):
	...
	kanivin.enqueue(
		"kanivin.desk.doctype.notification_log.notification_log.make_notification_logs",
		doc=doc,
		users=users,
...
def _get_user_ids(user_emails):
	user_names = kanivin.db.get_values(
...
def send_notification_email(doc: NotificationLog):
	...
	from kanivin.utils import get_url_to_form, strip_html

	user = kanivin.db.get_value("User", doc.for_user, fieldname=["email", "language"], as_dict=True)
	if not user:
		return
...
def get_notification_logs(limit=20):
	notification_logs = kanivin.db.get_list(
...
def mark_all_as_read():
	unread_docs_list = kanivin.get_all(
		"Notification Log", filters={"read": 0, "for_user": kanivin.session.user}
	)
	unread_docnames = [doc.name for doc in unread_docs_list]
	if unread_docnames:
		filters = {"name": ["in", unread_docnames]}
		kanivin.db.set_value("Notification Log", filters, "read", 1, update_modified=False)
...
def mark_as_read(docname: str):
	...
	if docname:
		kanivin.db.set_value("Notification Log", str(docname), "read", 1, update_modified=False)


@kanivin.whitelist()
def trigger_indicator_hide():
	kanivin.publish_realtime("indicator_hide", user=kanivin.session.user)


def set_notifications_as_unseen(user):
	try:
		kanivin.db.set_value("Notification Settings", user, "seen", 0, update_modified=False)
