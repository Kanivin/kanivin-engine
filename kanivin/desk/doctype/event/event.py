...
from kanivin.contacts.doctype.contact.contact import get_default_contact
...
from kanivin.utils.user import get_enabled_system_users
...
class Event(Document):
	...
	if TYPE_CHECKING:
		from kanivin.desk.doctype.event_participants.event_participants import EventParticipants
	...
	def validate(self):
		...
		if self.repeat_on == "Daily" and self.ends_on and getdate(self.starts_on) != getdate(self.ends_on):
			kanivin.throw(_("Daily Events should finish on the Same Day."))

		if self.sync_with_google_calendar and not self.google_calendar:
			kanivin.throw(_("Select Google Calendar to which event should be synced."))
	...
	def on_trash(self):
		...
		for communication in communications:
			kanivin.delete_doc("Communication", communication, force=True)
	...
	def sync_communication(self):
		...
		for participant in self.event_participants:
			if communications := kanivin.get_all(
				"Communication",
				filters=[
				...
				for comm in communications:
					communication = kanivin.get_doc("Communication", comm)
					self.update_communication(participant, communication)
			else:
				meta = kanivin.get_meta(participant.reference_doctype)
	...
	def create_communication(self, participant: "EventParticipants"):
		communication = kanivin.new_doc("Communication")
		self.update_communication(participant, communication)
	...
	def set_participants_email(self):
		for participant in self.event_participants:
			...
			participant.email = (
				kanivin.get_value("Contact", participant_contact, "email_id") if participant_contact else None
			)
...
def delete_communication(event, reference_doctype, reference_docname):
	...
	deleted_participant = kanivin.get_doc(reference_doctype, reference_docname)

	comms = kanivin.get_list(
		"Communication",
		filters=[
	...
	for comm in comms:
		kanivin.delete_doc("Communication", comm)


def get_permission_query_conditions(user):
	if not user:
		user = kanivin.session.user
	return f"""(`tabEvent`.`event_type`='Public' or `tabEvent`.`owner`={kanivin.db.escape(user)})"""
...
def send_event_digest():
	...
	for user in users:
		...
		if events:
			kanivin.set_user_lang(user.name, user.language)
			...
			kanivin.sendmail(
				...
				subject=kanivin._("Upcoming Events for Today"),
				template="upcoming_events",
				args={
					"events": events,
				},
				header=[kanivin._("Events in Today's Calendar"), "blue"],
			)


@kanivin.whitelist()
def get_events(start, end, user=None, for_reminder=False, filters=None) -> list[kanivin._dict]:
	if not user:
		user = kanivin.session.user
...
def delete_events(ref_type, ref_name, delete_event=False):
	participations = kanivin.get_all(
		"Event Participants",
	...
	if participations:
		for participation in participations:
			if delete_event:
				kanivin.delete_doc("Event", participation.parent, for_reload=True)
			else:
				total_participants = kanivin.get_all(
					"Event Participants", filters={"parenttype": "Event", "parent": participation.parent}
				)

				if len(total_participants) <= 1:
					kanivin.db.delete("Event", {"name": participation.parent})
					kanivin.db.delete("Event Participants", {"name": participation.name})


# Close events if ends_on or repeat_till is less than now_datetime
def set_status_of_events():
	events = kanivin.get_list("Event", filters={"status": "Open"}, fields=["name", "ends_on", "repeat_till"])
	for event in events:
		if (event.ends_on and getdate(event.ends_on) < getdate(nowdate())) or (
			...
			kanivin.db.set_value("Event", event.name, "status", "Closed")
