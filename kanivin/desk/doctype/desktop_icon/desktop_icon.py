...
def after_doctype_insert():
	kanivin.db.add_unique("Desktop Icon", ("module_name", "owner", "standard"))
...
def get_desktop_icons(user=None):
	...
	user_icons = kanivin.cache.hget("desktop_icons", user)
	...
	if not user_icons:
		...
		blocked_doctypes = kanivin.get_all(
			"DocType",
		...
		standard_icons = kanivin.get_all("Desktop Icon", fields=fields, filters={"standard": 1})
		...
		user_icons = kanivin.get_all("Desktop Icon", fields=fields, filters={"standard": 0, "owner": user})
		...
		user_blocked_modules = kanivin.get_doc("User", user).get_blocked_modules()
		for icon in user_icons:
		...
		kanivin.cache.hset("desktop_icons", user, user_icons)
...
def add_user_icon(_doctype, _report=None, label=None, link=None, type="link", standard=0):
	...
	icon_name = kanivin.db.exists(
		"Desktop Icon", {"standard": standard, "link": link, "owner": kanivin.session.user}
	)

	if icon_name:
		if kanivin.db.get_value("Desktop Icon", icon_name, "hidden"):
			# if it is hidden, unhide it
			kanivin.db.set_value("Desktop Icon", icon_name, "hidden", 0)
			clear_desktop_icons_cache()

	else:
		idx = (
			kanivin.db.sql("select max(idx) from `tabDesktop Icon` where owner=%s", kanivin.session.user)[0][0]
			or kanivin.db.sql("select count(*) from `tabDesktop Icon` where standard=1")[0][0]
		)

		if not kanivin.db.get_value("Report", _report):
			_report = None
			userdefined_icon = kanivin.db.get_value(
		...
		except kanivin.UniqueValidationError:
			kanivin.throw(_("Desktop Icon already exists"))
	...
	for i, module_name in enumerate(new_order):
		if module_name not in ("Explore",):
			if user:
				...
				name = kanivin.db.get_value("Desktop Icon", {"standard": 1, "module_name": module_name})
				if name:
					icon = kanivin.get_doc("Desktop Icon", name)
				else:
...
def set_desktop_icons(visible_list, ignore_duplicate=True):
	...
	if not int(kanivin.defaults.get_defaults().setup_complete or 0):
		kanivin.db.delete("Desktop Icon", {"standard": 0})

	# set standard as blocked and hidden if setting first active domain
	if not kanivin.flags.keep_desktop_icons:
		kanivin.db.sql("update `tabDesktop Icon` set blocked=0, hidden=1 where standard=1")

	# set as visible if present, or add icon
	for module_name in list(visible_list):
		name = kanivin.db.get_value("Desktop Icon", {"module_name": module_name})
		if name:
			kanivin.db.set_value("Desktop Icon", name, "hidden", 0)
		else:
			if kanivin.db.exists("DocType", module_name):
				try:
					add_user_icon(module_name, standard=1)
				except kanivin.UniqueValidationError as e:
...
def set_hidden(module_name, user=None, hidden=1):
	...
	if user:
		...
		if hidden and icon.custom:
			kanivin.delete_doc(icon.doctype, icon.name, ignore_permissions=True)
			return

		# hidden by user
		icon.db_set("hidden", hidden)
	else:
		icon = kanivin.get_doc("Desktop Icon", {"standard": 1, "module_name": module_name})
...
def get_all_icons():
	return [
		d.module_name for d in kanivin.get_all("Desktop Icon", filters={"standard": 1}, fields=["module_name"])
	]


def clear_desktop_icons_cache(user=None):
	kanivin.cache.hdel("desktop_icons", user or kanivin.session.user)
	kanivin.cache.hdel("bootinfo", user or kanivin.session.user)
...
def get_user_copy(module_name, user=None):
	...
	desktop_icon_name = kanivin.db.get_value(
		"Desktop Icon", {"module_name": module_name, "owner": user, "standard": 0}
	)

	if desktop_icon_name:
		return kanivin.get_doc("Desktop Icon", desktop_icon_name)
	else:
...
def make_user_copy(module_name, user):
	...
	standard_name = kanivin.db.get_value("Desktop Icon", {"module_name": module_name, "standard": 1})

	if not standard_name:
		kanivin.throw(_("{0} not found").format(module_name), kanivin.DoesNotExistError)

	original = kanivin.get_doc("Desktop Icon", standard_name)

	desktop_icon = kanivin.get_doc(
...
def sync_from_app(app):
	...
	try:
		modules = kanivin.get_attr(app + ".config.desktop.get_data")() or {}
	except ImportError:
		return []
	...
	for i, m in enumerate(modules_list):
		...
		if desktop_icon_name:
			desktop_icon = kanivin.get_doc("Desktop Icon", desktop_icon_name)
		else:
			# new icon
			desktop_icon = kanivin.get_doc(
		...
		try:
			desktop_icon.save()
		except kanivin.exceptions.UniqueValidationError:
			pass
...
def update_icons(hidden_list, user=None):
	...
	kanivin.msgprint(kanivin._("Updated"), indicator="green", title=_("Success"), alert=True)


def get_context(context):
	context.icons = get_user_icons(kanivin.session.user)
	context.user = kanivin.session.user

	if "System Manager" in kanivin.get_roles():
		context.users = kanivin.get_all(
			"User",
...
def get_module_icons(user=None):
	...
	if not user:
		icons = kanivin.get_all("Desktop Icon", fields="*", filters={"standard": 1}, order_by="idx")
	else:
		kanivin.cache.hdel("desktop_icons", user)
		icons = get_user_icons(user)

	for icon in icons:
		icon.value = kanivin.db.escape(_(icon.label or icon.module_name))
...
def get_user_icons(user):
	...
	from kanivin.boot import get_allowed_pages
