...
kanivin.ui.form.on("Number Card", {
	...
	create_add_to_dashboard_button: function (frm) {
		frm.add_custom_button(__("Add Card to Dashboard"), () => {
			const dialog = kanivin.dashboard_utils.get_add_to_dashboard_dialog(
				frm.doc.name,
				"Number Card",
				"kanivin.desk.doctype.number_card.number_card.add_card_to_dashboard"
			);

			if (!frm.doc.name) {
				kanivin.msgprint(__("Please create Card first"));
			} else {
				dialog.show();
			}
		});
	...
	filters_config: function (frm) {
		frm.filters = eval(frm.doc.filters_config);
		const filter_values = kanivin.report_utils.get_filter_values(frm.filters);
		...
		if (doctype) {
			kanivin.model.with_doctype(doctype, () => {
				kanivin.get_meta(doctype).fields.map((df) => {
					if (kanivin.model.numeric_fieldtypes.includes(df.fieldtype)) {
	...
	set_report_filters: function (frm) {
		...
		if (report_name) {
			kanivin.report_utils.get_report_filters(report_name).then((filters) => {
				if (filters) {
					frm.filters = filters;
					const filter_values = kanivin.report_utils.get_filter_values(filters);
	...
	render_filters_table: function (frm) {
		...
		table.on("click", () => {
			if (!kanivin.boot.developer_mode && frm.doc.is_standard) {
				kanivin.throw(__("Cannot edit filters for standard number cards"));
			}
	...
	set_parent_document_type: async function (frm) {
		...
		let doc_is_table =
			document_type &&
			(await kanivin.db.get_value("DocType", document_type, "istable")).message.istable;
