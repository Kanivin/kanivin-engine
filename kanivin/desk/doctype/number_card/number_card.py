...
from kanivin.query_builder import Criterion
from kanivin.query_builder.utils import DocType
from kanivin.utils import cint, flt
...
class NumberCard(Document):
	...
	def autoname(self):
		...
		if kanivin.db.exists("Number Card", self.name):
	...
	def validate(self):
		if self.type == "Document Type":
			if not (self.document_type and self.function):
				kanivin.throw(_("Document Type and Function are required to create a number card"))

			if self.function != "Count" and not self.aggregate_function_based_on:
				kanivin.throw(_("Aggregate Field is required to create a number card"))

			if kanivin.get_meta(self.document_type).istable and not self.parent_document_type:
				kanivin.throw(_("Parent Document Type is required to create a number card"))

		elif self.type == "Report":
			if not (self.report_name and self.report_field and self.function):
				kanivin.throw(_("Report Name, Report Field and Fucntion are required to create a number card"))

		elif self.type == "Custom":
			if not self.method:
				kanivin.throw(_("Method is required to create a number card"))
...
def has_permission(doc, ptype, user):
	...
	if doc.type == "Report":
		...
		allowed_doctypes = tuple(kanivin.permissions.get_doctypes_with_read())
...
def get_result(doc, filters, to_date=None):
	...
	if not filters:
		...
		filters = kanivin.parse_json(filters)
...
def get_percentage_difference(doc, filters, result):
	doc = kanivin.parse_json(doc)
	result = kanivin.parse_json(result)

	doc = kanivin.get_doc("Number Card", doc.name)
...
def calculate_previous_result(doc, filters):
	from kanivin.utils import add_to_date

	current_date = kanivin.utils.now()
...
def create_number_card(args):
	...
	doc = kanivin.new_doc("Number Card")
...
def get_cards_for_user(doctype, txt, searchfield, start, page_len, filters):
	...
	condition_query = kanivin.qb.get_query(
		doctype,
		filters=filters,
	...
	return (
		...
		.where((numberCard.owner == kanivin.session.user) | (numberCard.is_public == 1))
...
def create_report_number_card(args):
	...
	args.name = card.name
	if args.dashboard:
		add_card_to_dashboard(kanivin.as_json(args))
...
def add_card_to_dashboard(args):
	...
	dashboard_link = kanivin.new_doc("Number Card Link")
	dashboard_link.card = args.name

	if args.set_standard and dashboard.is_standard:
		card = kanivin.get_doc("Number Card", dashboard_link.card)
		card.is_standard = 1
