...
import kanivin
from kanivin.core.doctype.scheduled_job_type.scheduled_job_type import ScheduledJobType
from kanivin.model.document import Document
from kanivin.utils.background_jobs import get_queue, get_queue_list, get_redis_conn
from kanivin.utils.caching import redis_cache
from kanivin.utils.data import add_to_date
from kanivin.utils.scheduler import (
		...
		def wrapper(*args, **kwargs):
			try:
				...
				if kanivin.flags.in_test:
					raise
				kanivin.log(kanivin.get_traceback())
				# nosemgrep
				kanivin.msgprint(
					f"System Health check step {kanivin.bold(step)} failed: {e}", alert=True, indicator="red"
				)
...
class SystemHealthReport(Document):
	...
	if TYPE_CHECKING:
		from kanivin.desk.doctype.system_health_report_errors.system_health_report_errors import (
			SystemHealthReportErrors,
		)
		from kanivin.desk.doctype.system_health_report_failing_jobs.system_health_report_failing_jobs import (
			SystemHealthReportFailingJobs,
		)
		from kanivin.desk.doctype.system_health_report_queue.system_health_report_queue import (
			SystemHealthReportQueue,
		)
		from kanivin.desk.doctype.system_health_report_tables.system_health_report_tables import (
			SystemHealthReportTables,
		)
		from kanivin.desk.doctype.system_health_report_workers.system_health_report_workers import (
	...
	def fetch_background_jobs(self):
		...
		self.test_job_id = kanivin.enqueue("kanivin.ping", at_front=True).id
		self.background_jobs_check = "queued"
		workers = kanivin.get_all("RQ Worker")
	...
	def fetch_scheduler(self):
		...
		for job_type in kanivin.get_all(
			"Scheduled Job Type",
			filters={"stopped": 0, "last_execution": ("<", threshold)},
			fields="*",
			order_by="last_execution asc",
		):
			job_type: ScheduledJobType = kanivin.get_doc(doctype="Scheduled Job Type", **job_type)
	...
	def fetch_email_stats(self):
		...
		self.total_outgoing_emails = kanivin.db.count("Email Queue", filters)
		self.pending_emails = kanivin.db.count("Email Queue", {"status": "Not Sent", **filters})
		self.failed_emails = kanivin.db.count("Email Queue", {"status": "Error", **filters})
		self.unhandled_emails = kanivin.db.count("Unhandled Email", filters)
		self.handled_emails = kanivin.db.count(
			"Communication",
	...
	def fetch_errors(self):
		...
		filters = {"creation": (">", threshold), "modified": (">", threshold)}
		self.total_errors = kanivin.db.count("Error Log", filters)

		top_errors = kanivin.db.sql(
	...
	def fetch_database_details(self):
		...
		self.database = kanivin.db.db_type
		self.db_storage_usage = sum(table.size for table in data)
		for row in data[:5]:
			self.append("top_db_tables", row)
		self.database_version = kanivin.db.sql("select version()")[0][0]

		if kanivin.db.db_type == "mariadb":
			self.bufferpool_size = kanivin.db.sql("show variables like 'innodb_buffer_pool_size'")[0][1]
			self.binary_logging = kanivin.db.sql("show variables like 'log_bin'")[0][1]

	@health_check("Cache")
	def fetch_cache_details(self):
		self.cache_keys = len(kanivin.cache.get_keys(""))
		self.cache_memory_usage = kanivin.cache.execute_command("INFO", "MEMORY").get("used_memory_human")

	@health_check("Storage")
	def fetch_storage_details(self):
		from kanivin.desk.page.backups.backups import get_context
	...
	def fetch_user_stats(self):
		...
		self.total_users = kanivin.db.count("User", {"enabled": 1})
		self.new_users = kanivin.db.count("User", {"enabled": 1, "creation": (">", threshold)})
		self.failed_logins = kanivin.db.count(
			"Activity Log",
			{
				"operation": "login",
				"status": "Failed",
		...
		self.active_sessions = kanivin.db.count("Sessions")
...
def get_job_status(job_id: str | None = None):
	kanivin.only_for("System Manager")
	try:
		return kanivin.get_doc("RQ Job", job_id).status
	except Exception:
...
def _get_directory_size(*path):
	folder = os.path.abspath(kanivin.get_site_path(*path))
