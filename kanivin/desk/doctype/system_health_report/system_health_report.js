...
kanivin.ui.form.on("System Health Report", {
	onload(frm) {
		let poll_attempts = 0;
		const interval = setInterval(() => {
			kanivin
				.xcall(
					"kanivin.desk.doctype.system_health_report.system_health_report.get_job_status",
	...
	refresh(frm) {
		...
		kanivin.realtime.on("pong", () => {
			frm.set_value("socketio_ping_check", "Pass");
			frm.set_value(
				"socketio_transport_mode",
				kanivin.realtime.socket.io?.engine?.transport?.name
			);
		});
		kanivin.realtime.emit("ping");
		frm.disable_save();
	...
	setup_highlight(frm) {
		...
		const update_fields = () => {
			if (!kanivin.get_route().includes(frm.doc.name)) {
				clearInterval(interval);
			}
