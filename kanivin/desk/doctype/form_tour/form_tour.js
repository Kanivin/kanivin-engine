...
kanivin.ui.form.on("Form Tour", {
	...
	async report_name(frm) {
		...
		let { message } = await kanivin.db.get_value("Report", frm.doc.report_name, "ref_doctype");
	...
	if (frm.doc.ui_tour) {
		frm.add_custom_button(__("Reset"), function () {
			kanivin.confirm(
				...
				function () {
					kanivin.call({
						method: "kanivin.desk.doctype.form_tour.form_tour.reset_tour",
						args: {
							tour_name: frm.doc.name,
						},
					});
					delete kanivin.boot.user.onboarding_status[frm.doc.name];
				}
			);
		});
	} else {
		frm.add_custom_button(__("Show Tour"), async () => {
			...
			if (issingle) {
				route_changed = kanivin.set_route("Form", frm.doc.reference_doctype);
			} else if (frm.doc.first_document) {
				const name = await get_first_document(frm.doc.reference_doctype);
				route_changed = kanivin.set_route("Form", frm.doc.reference_doctype, name);
			} else {
				route_changed = kanivin.set_route("Form", frm.doc.reference_doctype, "new");
			}
			route_changed.then(() => {
...
kanivin.ui.form.on("Form Tour Step", {
	...
	parent_fieldname(frm, cdt, cdn) {
		...
		const parent_fieldname_df = kanivin
		...
		frm.set_fields_as_options(
			...
			if (child_row.fieldname) {
				kanivin.model.set_value(cdt, cdn, "fieldname", child_row.fieldname);
			}
		});
	},
});
...
async function check_if_single(doctype) {
	const { message } = await kanivin.db.get_value("DocType", doctype, "issingle");
	return message.issingle || 0;
}
async function check_if_private_workspace(name) {
	const { message } = await kanivin.db.get_value("Workspace", name, "public");
	...
	switch (route[0]) {
		...
		case "Form":
			...
			if (await check_if_single(frm.doc.reference_doctype)) {
				...
				route.push("new-" + kanivin.router.slug(frm.doc.reference_doctype));
			}
			return route;
		case "Tree":
			frm.doc.workspace_name = "";
