...
from kanivin.model import child_table_fields, default_fields, get_permitted_fields, optional_fields
from kanivin.model.base_document import get_controller
from kanivin.model.db_query import DatabaseQuery
from kanivin.model.utils import is_virtual_doctype
from kanivin.utils import add_user_info, cint, format_duration
...
def get_list():
	...
	if is_virtual_doctype(args.doctype):
		...
		# uncompressed (refactored from kanivin.model.db_query.get_list)
		data = execute(**args)
...
def get_count() -> int:
	...
	if is_virtual_doctype(args.doctype):
		...
		if args.limit:
			...
			count = kanivin.db.sql(f"""select count(*) from ( {partial_query} ) p""")[0][0]
		else:
...
def get_form_params():
	...
	data = kanivin._dict(kanivin.local.form_dict)
	clean_params(data)
	validate_args(data)
	return data
	...
	if isinstance(filters, list):
		...
		for condition in filters:
			if len(condition) == 3:
				...
				if is_standard(fieldname):
					continue
				meta = kanivin.get_meta(condition[0])
...
def setup_group_by(data):
	...
	if data.group_by and data.aggregate_function:
		if data.aggregate_function.lower() not in ("count", "sum", "avg"):
			kanivin.throw(_("Invalid aggregate function"))

		if kanivin.db.has_column(data.aggregate_on_doctype, data.aggregate_on_field):
			data.fields.append(
...
def raise_invalid_field(fieldname):
	kanivin.throw(_("Field not permitted in query") + f": {fieldname}", kanivin.DataError)
...
def get_meta_and_docfield(fieldname, data):
	...
	meta = kanivin.get_meta(parenttype)
...
def get_parenttype_and_fieldname(field, data):
	if "." in field:
		...
		parenttype = parts[0]
		fieldname = parts[1]
		df = kanivin.get_meta(data.doctype).get_field(parenttype)
...
def compress(data, args=None):
	...
	from kanivin.desk.query_report import add_total_row
	...
	if args.get("add_total_row"):
		meta = kanivin.get_meta(args.doctype)
...
def save_report(name, doctype, report_settings):
	...
	if kanivin.db.exists("Report", name):
		report = kanivin.get_doc("Report", name)
		if report.is_standard == "Yes":
			kanivin.throw(_("Standard Reports cannot be edited"))

		if report.report_type != "Report Builder":
			kanivin.throw(_("Only reports of type Report Builder can be edited"))

		if report.owner != kanivin.session.user and not report.has_permission("write"):
			kanivin.throw(_("Insufficient Permissions for editing Report"), kanivin.PermissionError)
	else:
	...
	kanivin.msgprint(
		_("Report {0} saved").format(kanivin.bold(report.name)),
		indicator="green",
		alert=True,
	)
	return report.name
...
def delete_report(name):
	...
	report = kanivin.get_doc("Report", name)
	if report.is_standard == "Yes":
		kanivin.throw(_("Standard Reports cannot be deleted"))

	if report.report_type != "Report Builder":
		kanivin.throw(_("Only reports of type Report Builder can be deleted"))

	if report.owner != kanivin.session.user and not report.has_permission("delete"):
		kanivin.throw(_("Insufficient Permissions for deleting Report"), kanivin.PermissionError)

	report.delete(ignore_permissions=True)
	kanivin.msgprint(
		_("Report {0} deleted").format(kanivin.bold(report.name)),
		indicator="green",
		alert=True,
	)
...
def export_query():
	...
	from kanivin.desk.utils import get_csv_bytes, pop_csv_params, provide_binary_file
	...
	if not kanivin.permissions.can_export(doctype):
		if kanivin.permissions.can_export(doctype, is_owner=True):
			for row in ret:
				if row[-1] != kanivin.session.user:
	...
	if kanivin.local.lang == "en" or not translate_values:
	...
	if file_format_type == "CSV":
		...
		content = get_csv_bytes(
			[[handle_html(kanivin.as_unicode(v)) if isinstance(v, str) else v for v in r] for r in data],
			csv_params,
		)
	elif file_format_type == "Excel":
		from kanivin.utils.xlsxutils import make_xlsx
...
def get_field_info(fields, doctype):
	...
	for key in fields:
		...
		if parenttype == doctype and fieldname == "name":
			...
			df = kanivin.get_meta(parenttype).get_field(fieldname)
			if df and df.fieldtype in ("Data", "Select", "Small Text", "Text"):
				name = df.name
				label = _(df.label)
				fieldtype = df.fieldtype
				translatable = getattr(df, "translatable", False)
			elif df and df.fieldtype == "Link" and kanivin.get_meta(df.options).translated_doctype:
				name = df.name
				label = _(df.label)
				fieldtype = df.fieldtype
				translatable = True
...
def delete_items():
	...
	items = sorted(json.loads(kanivin.form_dict.get("items")), reverse=True)
	doctype = kanivin.form_dict.get("doctype")

	if len(items) > 10:
		kanivin.enqueue("kanivin.desk.reportview.delete_bulk", doctype=doctype, items=items)
	else:
...
def get_stats(stats, doctype, filters=None):
	...
	try:
		db_columns = kanivin.db.get_table_columns(doctype)
	except (kanivin.db.InternalError, kanivin.db.ProgrammingError):
