...
def get_notifications():
	...
	if kanivin.flags.in_install or not kanivin.db.get_single_value("System Settings", "setup_complete"):
		return out
	...
	for name in groups:
		count = kanivin.cache.hget("notification_count:" + name, kanivin.session.user)
...
def get_notifications_for_doctypes(config, notification_count):
	...
	can_read = kanivin.get_user().get_can_read()
	open_count_doctype = {}
			...
			if d in notification_count:
				...
				try:
					if isinstance(condition, dict):
						...
						result = kanivin.get_attr(condition)()

				except kanivin.PermissionError:
					kanivin.clear_messages()
					pass
					# kanivin.msgprint("Permission Error in notifications for {0}".format(d))
					...
					kanivin.cache.hset("notification_count:" + d, kanivin.session.user, result)
...
def clear_notifications(user=None):
	...
	for name in groups:
		if user:
			kanivin.cache.hdel("notification_count:" + name, user)
		else:
			kanivin.cache.delete_key("notification_count:" + name)


def clear_notification_config(user):
	kanivin.cache.hdel("notification_config", user)


def delete_notification_count_for(doctype):
	kanivin.cache.delete_key("notification_count:" + doctype)
...
def get_notification_info():
	...
	conditions = {}
	module_doctypes = {}
	doctype_info = dict(kanivin.db.sql("""select name, module from tabDocType"""))
...
def get_notification_config():
	...
	return kanivin.cache.hget("notification_config", user, _get)
...
def get_open_count(doctype: str, name: str, items=None):
	...
	if kanivin.flags.in_migrate or kanivin.flags.in_install:
		return {"count": []}
	...
	if not meta.custom:
		module = kanivin.get_meta_module(doctype)
...
def notify_mentions(ref_doctype, ref_name, content):
	if ref_doctype and ref_name and content:
		...
		sender_fullname = get_fullname(kanivin.session.user)
		...
		notification_message = _("""{0} mentioned you in a comment in {1} {2}""").format(
			kanivin.bold(sender_fullname), kanivin.bold(ref_doctype), get_title_html(title)
		)
...
def extract_mentions(txt):
	...
	for mention in soup.find_all(class_="mention"):
		if mention.get("data-is-group") == "true":
			try:
				user_group = kanivin.get_cached_doc("User Group", mention["data-id"])
