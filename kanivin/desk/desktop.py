...
# Author - Shivam Mishra <shivam@kanivin.io>

from functools import wraps
from json import dumps, loads

import kanivin
from kanivin import DoesNotExistError, ValidationError, _, _dict
from kanivin.boot import get_allowed_pages, get_allowed_reports
from kanivin.cache_manager import (
...
class Workspace:
	def __init__(self, page, minimal=False):
		...
		self.workspace_manager = "Workspace Manager" in kanivin.get_roles()

		self.user = kanivin.get_user()
		self.allowed_modules = self.get_cached("user_allowed_modules", self.get_allowed_modules)

		self.doc = kanivin.get_cached_doc("Workspace", self.page_name)
		if (
			self.doc
			and self.doc.module
		...
		self.restricted_doctypes = (
			kanivin.cache.get_value("domain_restricted_doctypes") or build_domain_restriced_doctype_cache()
		)
		self.restricted_pages = (
			kanivin.cache.get_value("domain_restricted_pages") or build_domain_restriced_page_cache()
		)
	...
	def get_cached(self, cache_key, fallback_fn):
		value = kanivin.cache.get_value(cache_key, user=kanivin.session.user)
		...
		kanivin.cache.set_value(cache_key, value, kanivin.session.user, 21600)
		return value
	...
	def get_onboarding_doc(self, onboarding):
		...
		if kanivin.db.get_value("Module Onboarding", onboarding, "is_complete"):
			return None

		doc = kanivin.get_doc("Module Onboarding", onboarding)

		# Check if user is allowed
		allowed_roles = set(doc.get_allowed_roles())
		user_roles = set(kanivin.get_roles())
	...
	def _doctype_contains_a_record(self, name):
		...
		if not exists and kanivin.db.exists(name):
			if not kanivin.db.get_value("DocType", name, "issingle"):
				exists = bool(kanivin.get_all(name, limit=1))
			else:
				exists = True
	...
	def _prepare_item(self, item):
		...
		if item.get("link_type") == "DocType":
			item["description"] = kanivin.get_meta(item.link_to).description
	...
	def is_custom_block_permitted(self, custom_block_name):
		...
		allowed = [
			d.role for d in kanivin.get_all("Has Role", fields=["role"], filters={"parent": custom_block_name})
		]
	...
	def get_links(self):
		...
		default_country = kanivin.db.get_default("country")
	...
	def get_charts(self):
		all_charts = []
		if kanivin.has_permission("Dashboard Chart", throw=False):
			charts = self.doc.charts

			for chart in charts:
				if kanivin.has_permission("Dashboard Chart", doc=chart.chart_name):
					# Translate label
	...
	def get_shortcuts(self):
		def _in_active_domains(item):
			if not item.restrict_to_domain:
				...
				return item.restrict_to_domain in kanivin.get_active_domains()
	...
	def get_onboarding_steps(self, onboarding_doc):
		...
		for doc in onboarding_doc.get_steps():
			...
			if step.action == "Create Entry":
				step.is_submittable = kanivin.db.get_value(
	...
	def get_number_cards(self):
		all_number_cards = []
		if kanivin.has_permission("Number Card", throw=False):
			number_cards = self.doc.number_cards
			for number_card in number_cards:
				if kanivin.has_permission("Number Card", doc=number_card.number_card_name):
					# Translate label
					number_card.label = (
	...
	def get_custom_blocks(self):
		all_custom_blocks = []
		if kanivin.has_permission("Custom HTML Block", throw=False):
			custom_blocks = self.doc.custom_blocks

			for custom_block in custom_blocks:
				if kanivin.has_permission("Custom HTML Block", doc=custom_block.custom_block_name):
...
def get_desktop_page(page):
	...
	try:
		...
		kanivin.log_error("Workspace Missing")
		return {}
...
def get_workspace_sidebar_items():
	...
	has_access = "Workspace Manager" in kanivin.get_roles()

	# don't get domain restricted pages
	blocked_modules = kanivin.get_cached_doc("User", kanivin.session.user).get_blocked_modules()
	blocked_modules.append("Dummy Module")

	# adding None to allowed_domains to include pages without domain restriction
	allowed_domains = [None, *kanivin.get_active_domains()]
		...
		try:
			...
			if has_access or workspace.is_permitted():
				if page.public and (has_access or not page.is_hidden) and page.title != "Welcome Workspace":
					pages.append(page)
				elif page.for_user == kanivin.session.user:
	...
	if len(pages) == 0:
		pages = [kanivin.get_doc("Workspace", "Welcome Workspace").as_dict()]
		pages[0]["label"] = _("Welcome Workspace")

	return {
		"pages": pages,
		"has_access": has_access,
		"has_create_access": kanivin.has_permission(doctype="Workspace", ptype="create"),
	}


def get_table_with_counts():
	counts = kanivin.cache.get_value("information_schema:counts")
	if not counts:
...
def update_onboarding_step(name, field, value):
	...
	kanivin.db.set_value("Onboarding Step", name, field, value)

	capture(kanivin.scrub(name), app="kanivin_onboarding", properties={field: value})
