...
def _toggle_like(doctype, name, add, user=None):
	...
	try:
		liked_by = kanivin.db.get_value(doctype, name, "_liked_by")
		...
		if add == "Yes":
			if user not in liked_by:
				...
				if kanivin.get_cached_value("User", user, "follow_liked_documents"):
		...
		if kanivin.get_meta(doctype).issingle:
			kanivin.db.set_single_value(doctype, "_liked_by", json.dumps(liked_by), update_modified=False)
		else:
			kanivin.db.set_value(doctype, name, "_liked_by", json.dumps(liked_by), update_modified=False)
