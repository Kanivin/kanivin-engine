...
def handle(request: Request):
	...
	if data is not None:
		kanivin.response["data"] = data
	return build_response("json")


# Merge all API version routing rules
from kanivin.api.v1 import url_rules as v1_rules
from kanivin.api.v2 import url_rules as v2_rules
...
def get_api_version() -> ApiVersion | None:
	if not kanivin.request:
		return

	if kanivin.request.path.startswith(f"/api/{ApiVersion.V2.value}"):
