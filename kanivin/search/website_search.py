...
from kanivin.utils import set_request, update_progress_bar
...
class WebsiteSearch(FullTextSearch):
	...
	def get_document_to_index(self, route):
		...
		try:
			...
			return kanivin._dict(title=title, content=text_content, path=route)
		except Exception:
			pass
		finally:
...
def slugs_with_web_view(_items_to_index):
	...
	doctype_with_web_views = kanivin.get_all("DocType", filters=filters, fields=fields)
	...
	for doctype in doctype_with_web_views:
		if doctype.is_published_field:
			...
			if doctype.website_search_field:
				docs = kanivin.get_all(doctype.name, filters=filters, fields=[*fields, "title"])
				for doc in docs:
					content = kanivin.utils.md_to_html(getattr(doc, doctype.website_search_field))
					soup = BeautifulSoup(content, "html.parser")
					text_content = soup.text if soup else ""
					_items_to_index += [kanivin._dict(title=doc.title, content=text_content, path=doc.route)]
			else:
				docs = kanivin.get_all(doctype.name, filters=filters, fields=fields)
