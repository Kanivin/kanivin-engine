...
from kanivin.utils import add_to_date, now

UI_TEST_USER = "kanivin@example.com"


def whitelist_for_tests(fn):
	if kanivin.request and not (kanivin.flags.in_test or getattr(kanivin.local, "dev_server", 0)):
		kanivin.throw("Cannot run UI tests. Use a development server with `bench start`")

	return kanivin.whitelist()(fn)
...
def create_if_not_exists(doc):
	...
	for doc in docs:
		...
		filters = doc.copy()
		filters.pop("doctype")
		name = kanivin.db.exists(doc.doctype, filters)
		if not name:
			d = kanivin.get_doc(doc)
...
def prepare_webform_test():
	for note in kanivin.get_all("Note", pluck="name"):
		kanivin.delete_doc("Note", note, force=True)

	kanivin.delete_doc_if_exists("Web Form", "note")
	...
	doc = kanivin.get_doc(
		{
			...
			"communication_date": kanivin.utils.now_datetime(),
		}
	)
	doc.insert()
	return doc
...
@whitelist_for_tests
def setup_workflow():
	from kanivin.workflow.doctype.workflow.test_workflow import create_todo_workflow
...
def create_contact_phone_nos_records():
	if kanivin.get_all("Contact", {"first_name": "Test Contact"}):
		return

	doc = kanivin.new_doc("Contact")
...
def create_contact_records():
	if kanivin.get_all("Contact", {"first_name": "Test Form Contact 1"}):
		return
...
def create_multiple_todo_records():
	if kanivin.get_all("ToDo", {"description": "Multiple ToDo 1"}):
		return

	values = [(f"100{i}", f"Multiple ToDo {i}") for i in range(1, 1002)]

	kanivin.db.bulk_insert("ToDo", fields=["name", "description"], values=set(values))


def insert_contact(first_name, phone_number):
	doc = kanivin.get_doc({"doctype": "Contact", "first_name": first_name})
...
def create_form_tour():
	if kanivin.db.exists("Form Tour", {"name": "Test Form Tour"}):
		return

	tour = kanivin.get_doc(
		{
			"doctype": "Form Tour",
...
def create_web_page(title, route, single_thread):
	web_page = kanivin.db.exists("Web Page", {"route": route})
	if web_page:
		return web_page
	web_page = kanivin.get_doc({"doctype": "Web Page", "title": title, "route": route, "published": True})
	web_page.save()
	...
	web_page.append(
		"page_blocks",
		{
			...
			"web_template_values": kanivin.as_json(
				{
					"title": "Discussions",
...
def update_webform_to_multistep():
	if not kanivin.db.exists("Web Form", "update-profile-duplicate"):
		doc = kanivin.get_doc("Web Form", "edit-profile")
		_doc = kanivin.copy_doc(doc)
...
def insert_doctype_with_child_table_record(name):
	if kanivin.get_all(name, {"title": "Test Grid Search"}):
		return
...
def insert_translations():
	...
	for doc in translation:
		if not kanivin.db.exists("doc"):
			kanivin.get_doc(doc).insert()


@whitelist_for_tests
def create_blog_post():
	blog_category = kanivin.get_doc(
		{"name": "general", "doctype": "Blog Category", "title": "general"}
	).insert(ignore_if_duplicate=True)

	blogger = kanivin.get_doc(
		{
...
def create_test_user(username=None):
	...
	if kanivin.db.exists("User", name):
		return

	user = kanivin.new_doc("User")
	user.email = name
	user.first_name = "Kanivin"
	user.new_password = kanivin.local.conf.admin_password
	...
	all_roles = set(kanivin.get_all("Role", pluck="name"))
...
def setup_tree_doctype():
	kanivin.delete_doc_if_exists("DocType", "Custom Tree", force=True)
	...
	if not kanivin.db.exists("Custom Tree", "All Trees"):
		kanivin.get_doc({"doctype": "Custom Tree", "tree": "All Trees"}).insert()


@whitelist_for_tests
def setup_image_doctype():
	kanivin.delete_doc_if_exists("DocType", "Custom Image", force=True)
...
def setup_default_view(view, force_reroute=None):
	kanivin.delete_doc_if_exists("Property Setter", "Event-main-default_view")
	kanivin.delete_doc_if_exists("Property Setter", "Event-main-force_re_route_to_default_view")
...
@whitelist_for_tests
def create_kanban():
	if not kanivin.db.exists("Custom Field", "Note-kanban"):
		kanivin.get_doc(
			{
				"is_system_generated": 0,
				"dt": "Note",
	...
	if not kanivin.db.exists("Kanban Board", "_Note _Kanban"):
		kanivin.get_doc(
			{
...
def create_todo(description):
	return kanivin.get_doc({"doctype": "ToDo", "description": description}).insert()
...
def create_admin_kanban():
	if not kanivin.db.exists("Kanban Board", "Admin Kanban"):
		kanivin.get_doc(
			{
				"doctype": "Kanban Board",
				"name": "Admin Kanban",
...
def publish_progress(duration=3, title=None, doctype=None, docname=None):
	...
	kanivin.enqueue(slow_task, duration=duration, title=title, doctype=doctype, docname=docname)
...
def slow_task(duration, title, doctype, docname):
	...
	for i in range(steps + 1):
		kanivin.publish_progress(i * 10, title=title, doctype=doctype, docname=docname)
