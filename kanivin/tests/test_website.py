...
import kanivin
from kanivin import get_hooks
from kanivin.tests.utils import KanivinTestCase
from kanivin.utils import set_request
from kanivin.website.page_renderers.static_page import StaticPage
from kanivin.website.serve import get_response, get_response_content
from kanivin.website.utils import build_response, clear_website_cache, get_home_page


class TestWebsite(KanivinTestCase):
	def setUp(self):
		kanivin.set_user("Guest")
		self._clearRequest()

	def tearDown(self):
		kanivin.db.delete("Access Log")
	...
	def test_home_page(self):
		...
		kanivin.db.set_value("Role", "home-page-test", "home_page", "home-page-test")
		kanivin.set_user("test-user-for-home-page@example.com")
		self.assertEqual(get_home_page(), "home-page-test")

		kanivin.set_user("Administrator")
		kanivin.db.set_value("Role", "home-page-test", "home_page", "")

		# home page via portal settings
		kanivin.db.set_single_value("Portal Settings", "default_portal_home", "test-portal-home")

		kanivin.set_user("test-user-for-home-page@example.com")
		kanivin.cache.hdel("home_page", kanivin.session.user)
		self.assertEqual(get_home_page(), "test-portal-home")

		kanivin.db.set_single_value("Portal Settings", "default_portal_home", "")
		clear_website_cache()

		# home page via website settings
		kanivin.db.set_single_value("Website Settings", "home_page", "contact")
		self.assertEqual(get_home_page(), "contact")

		kanivin.db.set_single_value("Website Settings", "home_page", None)
		clear_website_cache()
		...
		with patch.object(
			...
			"get_hooks",
			patched_get_hooks(
				"get_website_user_home_page", ["kanivin.www._test._test_home_page.get_website_user_home_page"]
			),
		):
			self.assertEqual(get_home_page(), "_test/_test_folder")

		clear_website_cache()
		with patch.object(kanivin, "get_hooks", patched_get_hooks("website_user_home_page", ["login"])):
			self.assertEqual(get_home_page(), "login")

		clear_website_cache()
		with patch.object(kanivin, "get_hooks", patched_get_hooks("home_page", ["about"])):
			self.assertEqual(get_home_page(), "about")

		clear_website_cache()
		with patch.object(
			kanivin, "get_hooks", patched_get_hooks("role_home_page", {"home-page-test": ["home-page-test"]})
		):
			self.assertEqual(get_home_page(), "home-page-test")
	...
	def test_page_load(self):
		...
		html = kanivin.safe_decode(response.get_data())
	...
	def test_app(self):
		...
		kanivin.local.session_obj = None
	...
	def test_redirect(self):
		import kanivin.hooks

		kanivin.set_user("Administrator")

		kanivin.hooks.website_redirects = [
		...
		website_settings = kanivin.get_doc("Website Settings")
		...
		delattr(kanivin.hooks, "website_redirects")
		kanivin.cache.delete_key("app_hooks")
	...
	def test_custom_page_renderer(self):
		...
		def patched_get_hooks(*args, **kwargs):
			...
			if args and args[0] == "page_renderer":
				return_value = ["kanivin.tests.test_website.CustomPageRenderer"]
			return return_value

		with patch.object(kanivin, "get_hooks", patched_get_hooks):
	...
	def test_printview_page(self):
		kanivin.db.value_cache[("DocType", "Language", "name")] = (("Language",),)
		kanivin.set_user("Administrator")
	...
	def test_json_sidebar_data(self):
		kanivin.flags.look_for_sidebar = False
		content = get_response_content("/_test/_test_folder/_test_page")
		self.assertNotIn("Test Sidebar", content)
		clear_website_cache()
		kanivin.flags.look_for_sidebar = True
		content = get_response_content("/_test/_test_folder/_test_page")
	...
	def test_caching(self):
		# to enable caching
		kanivin.flags.force_website_cache = True
	...
	def test_safe_render(self):
		...
		self.assertNotIn("kanivin.exceptions.ValidationError: Illegal template", content)

	def test_never_render(self):
		from pathlib import Path
		from random import choices

		WWW = Path(kanivin.get_app_path("kanivin")) / "www"
	...
	def test_resolve_class(self):
		from kanivin.utils.jinja_globals import resolve_class

		context = kanivin._dict(primary=True)
		...
		self.assertEqual(
			kanivin.render_template(content, context), '<a class="btn btn-default btn-primary">Test</a>'
		)
	...
	def test_app_include(self):
		...
		with patch.object(kanivin, "get_hooks", patched_get_hooks):
			kanivin.set_user("Administrator")
			kanivin.hooks.app_include_js.append("test_app_include.js")
			kanivin.hooks.app_include_css.append("test_app_include.css")
			kanivin.conf.update({"app_include_js": ["test_app_include_via_site_config.js"]})
			kanivin.conf.update({"app_include_css": ["test_app_include_via_site_config.css"]})
