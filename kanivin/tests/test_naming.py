...
from kanivin.model.naming import (
...
from kanivin.utils import now_datetime, nowdate, nowtime


class TestNaming(KanivinTestCase):
	def setUp(self):
	...
	def test_append_number_if_name_exists(self):
		...
		note = kanivin.get_doc({"doctype": DOCTYPE, "title": TITLE}).insert()
	...
	def test_field_autoname_name_sync(self):
		country = kanivin.get_last_doc("Country")
	...
	def test_child_table_naming(self):
		...
		name = kanivin.generate_hash(length=10)

		doc = kanivin.new_doc(dt_with_child_autoname.name)
	...
	def test_format_autoname_for_consecutive_week_number(self):
		...
		todo_doctype = kanivin.get_doc("DocType", doctype)
		todo_doctype.autoname = "format:TODO-{WW}-{##}"
		todo_doctype.save()

		description = "Format"

		todo = kanivin.new_doc(doctype)
	...
	def test_revert_series(self):
		...
		kanivin.db.sql("""INSERT INTO `tabSeries` (name, current) values (%s, 1)""", (series,))
		revert_series_if_last(key, name)
		current_index = kanivin.db.sql(
			"""SELECT current from `tabSeries` where name = %s""", series, as_dict=True
		)[0]

		self.assertEqual(current_index.get("current"), 0)
		kanivin.db.delete("Series", {"name": series})

		series = f"TEST-{year}-"
		key = "TEST-.YYYY.-.#####"
		name = f"TEST-{year}-00002"
		kanivin.db.sql("""INSERT INTO `tabSeries` (name, current) values (%s, 2)""", (series,))
		revert_series_if_last(key, name)
		current_index = kanivin.db.sql(
			"""SELECT current from `tabSeries` where name = %s""", series, as_dict=True
		)[0]
		...
		name = "TEST-00003"
		kanivin.db.delete("Series", {"name": series})
		kanivin.db.sql("""INSERT INTO `tabSeries` (name, current) values (%s, 3)""", (series,))
		revert_series_if_last(key, name)
		current_index = kanivin.db.sql(
			"""SELECT current from `tabSeries` where name = %s""", series, as_dict=True
		)[0]
	...
	def test_naming_for_cancelled_and_amended_doc(self):
		submittable_doctype = kanivin.get_doc(
			{
				"doctype": "DocType",
				"module": "Core",
				"custom": 1,
		...
		doc = kanivin.new_doc("Submittable Doctype")
		doc.save()
		...
		amended_doc = kanivin.copy_doc(doc)
	...
	def test_naming_validations(self):
		...
		# set name via prompt
		tag = kanivin.get_doc({"doctype": "Tag", "__newname": "Tag"})
		self.assertRaises(kanivin.NameError, tag.insert)

		# set by passing set_name as ToDo
		self.assertRaises(kanivin.NameError, make_invalid_todo)

		# name (via title field) cannot be the same as the doctype
		note = kanivin.get_doc({"doctype": "Currency", "currency_name": "Currency"})
		self.assertRaises(kanivin.NameError, note.insert)

		# case 2: set name with "New ---"
		tag = kanivin.get_doc({"doctype": "Tag", "__newname": "New Tag"})
		self.assertRaises(kanivin.NameError, tag.insert)
		...
		tag = kanivin.get_doc({"doctype": "Tag", "__newname": ""})
		self.assertRaises(kanivin.ValidationError, tag.insert)

	def test_autoincremented_naming(self):
		from kanivin.core.doctype.doctype.test_doctype import new_doctype

		doctype = "autoinc_doctype" + kanivin.generate_hash(length=5)
		dt = new_doctype(doctype, autoname="autoincrement").insert(ignore_permissions=True)

		for i in range(1, 20):
			self.assertEqual(kanivin.new_doc(doctype).save(ignore_permissions=True).name, i)
	...
	def test_naming_series_validation(self):
		dns = kanivin.get_doc("Document Naming Settings")
	...
	def test_naming_using_fields(self):
		webhook = kanivin.new_doc("Webhook")
	...
	def test_hash_collision(self):
		...
		name = kanivin.generate_hash()
		for _ in range(10):
			kanivin.flags.in_import = True
			kanivin.new_doc(doctype).update({"name": name}).insert()
		kanivin.flags.pop("in_import", None)
		...
		with patch_hooks(
			{
				"naming_series_variables": {
					"PM": ["kanivin.tests.test_naming.parse_naming_series_variable"],
				},
			},
		):
...
def make_invalid_todo():
	kanivin.get_doc({"doctype": "ToDo", "description": "Test"}).insert(set_name="ToDo")
