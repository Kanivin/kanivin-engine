...
from kanivin.query_builder.functions import Abs, Count, Ifnull, Max, Now, Timestamp
from kanivin.tests.test_query_builder import db_type_is, run_only_if
from kanivin.tests.utils import KanivinTestCase
from kanivin.utils.nestedset import get_ancestors_of, get_descendants_of
...
def create_tree_docs():
	...
	for record in records:
		d = kanivin.new_doc("Test Tree DocType")
		d.update(record)
		d.insert()


class TestQuery(KanivinTestCase):
	@run_only_if(db_type_is.MARIADB)
	def test_multiple_tables_in_filters(self):
		self.assertEqual(
			kanivin.qb.get_query(
				"DocType",
				["*"],
				[
	...
	def test_string_fields(self):
		self.assertEqual(
			kanivin.qb.get_query("User", fields="name, email", filters={"name": "Administrator"}).get_sql(),
	...
	def test_functions_fields(self):
		self.assertEqual(
			kanivin.qb.get_query("User", fields="Count(name)", filters={}).get_sql(),
			kanivin.qb.from_("User").select(Count(Field("name"))).get_sql(),
		)

		self.assertEqual(
			kanivin.qb.get_query("User", fields=["Count(name)", "Max(name)"], filters={}).get_sql(),
			kanivin.qb.from_("User").select(Count(Field("name")), Max(Field("name"))).get_sql(),
		)

		self.assertEqual(
			kanivin.qb.get_query("User", fields=["abs(name-email)", "Count(name)"], filters={}).get_sql(),
			kanivin.qb.from_("User")
		...
		self.assertEqual(
			kanivin.qb.get_query("User", fields=[Count("*")], filters={}).get_sql(),
			kanivin.qb.from_("User").select(Count("*")).get_sql(),
		)

		self.assertEqual(
			kanivin.qb.get_query("User", fields="timestamp(creation, modified)", filters={}).get_sql(),
			kanivin.qb.from_("User").select(Timestamp(Field("creation"), Field("modified"))).get_sql(),
		)
	...
	def test_qb_fields(self):
		...
		self.assertEqual(
			...
			kanivin.qb.from_(user_doctype).select(user_doctype.name, user_doctype.email).get_sql(),
		)

		self.assertEqual(
			kanivin.qb.get_query(user_doctype, fields=user_doctype.email, filters={}).get_sql(),
			kanivin.qb.from_(user_doctype).select(user_doctype.email).get_sql(),
		)

	def test_aliasing(self):
		user_doctype = kanivin.qb.DocType("User")
		self.assertEqual(
			kanivin.qb.get_query("User", fields=["name as owner", "email as id"], filters={}).get_sql(),
		...
		self.assertEqual(
			kanivin.qb.get_query(user_doctype, fields="name as owner, email as id", filters={}).get_sql(),
			kanivin.qb.from_(user_doctype)
			.select(user_doctype.name.as_("owner"), user_doctype.email.as_("id"))
			.get_sql(),
		)
	...
	def test_filters(self):
		self.assertEqual(
			kanivin.qb.get_query(
				"DocType",
				fields=["name"],
				filters={"module.app_name": "kanivin"},
			).get_sql(),
			"SELECT `tabDocType`.`name` FROM `tabDocType` LEFT JOIN `tabModule Def` ON `tabModule Def`.`name`=`tabDocType`.`module` WHERE `tabModule Def`.`app_name`='kanivin'".replace(
				"`", '"' if kanivin.db.db_type == "postgres" else "`"
			),
		)
		...
		self.assertEqual(
			...
			"SELECT `name` FROM `tabDocType`".replace("`", '"' if kanivin.db.db_type == "postgres" else "`"),
		)
	...
	def test_comment_stripping(self):
		self.assertNotIn(
			"email", kanivin.qb.get_query("User", fields=["name", "#email"], filters={}).get_sql()
		)

	def test_nestedset(self):
		kanivin.db.sql("delete from `tabDocType` where `name` = 'Test Tree DocType'")
		kanivin.db.sql_ddl("drop table if exists `tabTest Tree DocType`")
		create_tree_docs()
		descendants_result = kanivin.qb.get_query(
			"Test Tree DocType",
			fields=["name"],
		...
		ancestors_result = kanivin.qb.get_query(
			"Test Tree DocType",
			fields=["name"],
		...
		not_descendants_result = kanivin.qb.get_query(
			"Test Tree DocType",
			fields=["name"],
		...
		not_ancestors_result = kanivin.qb.get_query(
			"Test Tree DocType",
			fields=["name"],
	...
	def test_child_field_syntax(self):
		note1 = kanivin.get_doc(doctype="Note", title="Note 1", seen_by=[{"user": "Administrator"}]).insert()
		note2 = kanivin.get_doc(
			doctype="Note", title="Note 2", seen_by=[{"user": "Administrator"}, {"user": "Guest"}]
		).insert()

		result = kanivin.qb.get_query(
			"Note",
