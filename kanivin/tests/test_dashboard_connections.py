...
from kanivin.custom.doctype.customize_form.test_customize_form import TestCustomizeForm
from kanivin.desk.notifications import get_open_count
from kanivin.tests.utils import KanivinTestCase, patch_hooks


class TestDashboardConnections(KanivinTestCase):
	...
	def test_external_doctype_link_with_dashboard_override(self):
		...
		# create a test doc
		todo_doc = kanivin.get_doc(dict(doctype="ToDo", description="test")).insert()
		kanivin.get_doc(dict(doctype="Test Doctype D", title="d-001", doclink=todo_doc.name)).insert()
		kanivin.get_doc(dict(doctype="Test Doctype E", title="e-001", todo=todo_doc.name)).insert()
		...
		with patch_hooks(
			{
				"override_doctype_dashboards": {
					"ToDo": ["kanivin.tests.test_dashboard_connections.get_dashboard_for_todo"]
				}
			}
		):
			connections = get_open_count("ToDo", todo_doc.name)["count"]
...
def add_links_in_child_tables():
	test_child_table_with_link_to_doctype_a = kanivin.get_doc(
	...
	test_child_table_with_link_to_doctype_b = kanivin.get_doc(
