...
from kanivin.utils.caching import redis_cache, request_cache, site_cache
...
@site_cache
def ping() -> str:
	register_with_external_service(kanivin.local.site)
	return kanivin.local.site
...
class TestCachingUtils(KanivinTestCase):
...
class TestSiteCache(KanivinAPITestCase):
...
class TestRedisCache(KanivinAPITestCase):
	...
	def test_user_cache(self):
		...
		def calculate_area(radius: float) -> float:
			nonlocal function_call_count
			PI_APPROX = ENGINEERING_PI if kanivin.session.user == "Engineer" else PI
			function_call_count += 1
...
class TestDocumentCache(KanivinAPITestCase):
	TEST_DOCTYPE = "User"
	TEST_DOCNAME = "Administrator"
	TEST_FIELD = "middle_name"

	def setUp(self) -> None:
		self.test_value = kanivin.generate_hash()

	def test_caching(self):
		doc = kanivin.get_cached_doc(self.TEST_DOCTYPE, self.TEST_DOCNAME)

		with self.assertQueryCount(0):
			doc = kanivin.get_cached_doc(self.TEST_DOCTYPE, self.TEST_DOCNAME)

		doc.db_set(self.TEST_FIELD, self.test_value)
		new_doc = kanivin.get_cached_doc(self.TEST_DOCTYPE, self.TEST_DOCNAME)

		self.assertIsNot(doc, new_doc)  # Shouldn't be same object from kanivin.local
		self.assertEqual(new_doc.get(self.TEST_FIELD), self.test_value)  # Cache invalidated and fetched
		kanivin.db.rollback()

		doc_after_rollback = kanivin.get_cached_doc(self.TEST_DOCTYPE, self.TEST_DOCNAME)
		...
		with self.assertQueryCount(0):
			kanivin.get_cached_doc(self.TEST_DOCTYPE, self.TEST_DOCNAME)
...
class TestRedisWrapper(KanivinAPITestCase):
	def test_delete_keys(self):
		prefix = "test_del_"

		for i in range(5):
			kanivin.cache.set_value(f"{prefix}{i}", 1)

		self.assertEqual(len(kanivin.cache.get_keys(prefix)), 5)
		kanivin.cache.delete_keys(prefix)
		self.assertEqual(len(kanivin.cache.get_keys(prefix)), 0)

	def test_backward_compat_cache(self):
		self.assertEqual(kanivin.cache, kanivin.cache())
