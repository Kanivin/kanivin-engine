...
from kanivin.desk.page.setup_wizard.install_fixtures import update_global_search_doctypes
from kanivin.test_runner import make_test_objects
from kanivin.tests.utils import KanivinTestCase
from kanivin.utils import global_search, now_datetime


class TestGlobalSearch(KanivinTestCase):
	def setUp(self):
		update_global_search_doctypes()
		global_search.setup_global_search_table()
		self.assertTrue("__global_search" in kanivin.db.get_tables())
		doctype = "Event"
		global_search.reset()
	...
	def tearDown(self):
		kanivin.db.delete("Property Setter", {"doc_type": "Event"})
	...
	def test_update_doc(self):
		...
		event = kanivin.get_doc("Event", kanivin.get_all("Event")[0].name)
	...
	def test_delete_doc(self):
		self.insert_test_events()
		event_name = kanivin.get_all("Event")[0].name
		event = kanivin.get_doc("Event", event_name)
		...
		kanivin.delete_doc("Event", event_name)
		global_search.sync_global_search()
		kanivin.db.commit()
	...
	def test_insert_child_table(self):
		...
		for text in phrases:
			doc = kanivin.get_doc({"doctype": "Event", "subject": text, "starts_on": now_datetime()})
			doc.insert()
