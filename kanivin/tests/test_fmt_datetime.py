...
class TestFmtDatetime(KanivinTestCase):
	...
	def setUp(self):
		# create test domain
		self.pre_test_date_format = kanivin.db.get_default("date_format")
		self.pre_test_time_format = kanivin.db.get_default("time_format")

	def tearDown(self):
		kanivin.db.set_default("date_format", self.pre_test_date_format)
		kanivin.db.set_default("time_format", self.pre_test_time_format)
		kanivin.local.user_date_format = None
		kanivin.local.user_time_format = None
		kanivin.db.rollback()

	# Test utility functions

	def test_set_default_date_format(self):
		kanivin.db.set_default("date_format", "ZYX321")
		self.assertEqual(kanivin.db.get_default("date_format"), "ZYX321")

	def test_set_default_time_format(self):
		kanivin.db.set_default("time_format", "XYZ123")
		self.assertEqual(kanivin.db.get_default("time_format"), "XYZ123")
	...
	def test_format_date(self):
		...
		for fmt, valid_fmt in test_date_formats.items():
			kanivin.db.set_default("date_format", fmt)
			kanivin.local.user_date_format = None
	...
	def test_format_time(self):
		...
		for fmt, valid_fmt in test_time_formats.items():
			kanivin.db.set_default("time_format", fmt)
			kanivin.local.user_time_format = None
	...
	def test_format_datetime(self):
		...
		for date_fmt, valid_date in test_date_formats.items():
			kanivin.db.set_default("date_format", date_fmt)
			kanivin.local.user_date_format = None
			for time_fmt, valid_time in test_time_formats.items():
				kanivin.db.set_default("time_format", time_fmt)
				kanivin.local.user_time_format = None
