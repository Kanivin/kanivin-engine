...
class TestDefaults(KanivinTestCase):
	...
	def test_user_permission_on_defaults(self):
		...
		kanivin.delete_doc("User Permission", perm_doc.name)
	...
	def test_user_permission_defaults(self):
		...
		kanivin.db.set_value("User Permission", perm_doc.name, "is_default", 1)
		set_global_default("Country", "United States")
		self.assertEqual(get_user_default("Country"), "India")

		kanivin.db.set_value("User Permission", perm_doc.name, "is_default", 0)
		clear_user_default("Country")
