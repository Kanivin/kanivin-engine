...
import kanivin
from kanivin.auth import LoginAttemptTracker
from kanivin.kanivinclient import AuthError, KanivinClient
from kanivin.sessions import Session, get_expired_sessions, get_expiry_in_seconds
from kanivin.tests.test_api import KanivinAPITestCase
from kanivin.tests.utils import KanivinTestCase
from kanivin.utils import get_datetime, get_site_url, now
from kanivin.utils.data import add_to_date
from kanivin.www.login import _generate_temporary_login_link
...
class TestAuth(KanivinTestCase):
	@classmethod
	def setUpClass(cls):
		super().setUpClass()
		cls.HOST_NAME = kanivin.get_site_config().host_name or get_site_url(kanivin.local.site)
	...
	def tearDownClass(cls):
		kanivin.delete_doc("User", cls.test_user_email, force=True)
		kanivin.local.request_ip = None
		kanivin.form_dict.email = None
		kanivin.local.response["http_status_code"] = None

	def set_system_settings(self, k, v):
		kanivin.db.set_single_value("System Settings", k, v)
		kanivin.clear_cache()
		kanivin.db.commit()
	...
	def test_allow_login_using_mobile(self):
		...
		KanivinClient(self.HOST_NAME, self.test_user_mobile, self.test_user_password)
		KanivinClient(self.HOST_NAME, self.test_user_email, self.test_user_password)

		# login by username should fail
		with self.assertRaises(AuthError):
			KanivinClient(self.HOST_NAME, self.test_user_name, self.test_user_password)
	...
	def test_deny_multiple_login(self):
		...
		first_login = KanivinClient(self.HOST_NAME, self.test_user_email, self.test_user_password)
		first_login.get_list("ToDo")

		second_login = KanivinClient(self.HOST_NAME, self.test_user_email, self.test_user_password)
		second_login.get_list("ToDo")
		with self.assertRaises(Exception):
			first_login.get_list("ToDo")

		third_login = KanivinClient(self.HOST_NAME, self.test_user_email, self.test_user_password)
		with self.assertRaises(Exception):
			first_login.get_list("ToDo")
		with self.assertRaises(Exception):
			second_login.get_list("ToDo")
	...
	def test_disable_user_pass_login(self):
		KanivinClient(self.HOST_NAME, self.test_user_email, self.test_user_password).get_list("ToDo")
	...
	def test_correct_cookie_expiry_set(self):
		...
		client = KanivinClient(self.HOST_NAME, self.test_user_email, self.test_user_password)
...
class TestLoginAttemptTracker(KanivinTestCase):
...
class TestSessionExpirty(KanivinAPITestCase):
	def test_session_expires(self):
		sid = self.sid  # triggers login for test case login
		s: Session = kanivin.local.session_obj
