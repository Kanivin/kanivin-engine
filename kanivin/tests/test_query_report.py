...
from kanivin.desk.query_report import build_xlsx_data, export_query, run
from kanivin.tests.utils import KanivinTestCase
from kanivin.utils.xlsxutils import make_xlsx


class TestQueryReport(KanivinTestCase):
	@classmethod
	...
	def test_csv(self):
		...
		if not kanivin.db.exists("Report", REPORT_NAME):
			...
			report.query = kanivin.qb.from_(REF_DOCTYPE).select(*REPORT_COLUMNS).limit(10).get_sql()
		...
		for delimiter in (",", ";", "\t", "|"):
			for quoting in (QUOTE_ALL, QUOTE_MINIMAL, QUOTE_NONE, QUOTE_NONNUMERIC):
				...
				self.assertTrue(kanivin.response["filename"].endswith(".csv"))
				self.assertEqual(kanivin.response["type"], "binary")
				with StringIO(kanivin.response["filecontent"].decode("utf-8")) as result:
					reader = DictReader(result, delimiter=delimiter, quoting=quoting)
					row = reader.__next__()
					for column in REPORT_COLUMNS:
						self.assertIn(column, row)

		kanivin.delete_doc("Report", REPORT_NAME, delete_permanently=True)
	...
	def test_report_for_duplicate_column_names(self):
		...
		try:
			...
			for i in range(1, 3):
				kanivin.get_doc({"doctype": "Doc A", "first_name": f"John{i}", "last_name": "Doe"}).insert()
				kanivin.get_doc({"doctype": "Doc B", "last_name": f"Doe{i}", "first_name": "John"}).insert()

			if not kanivin.db.exists("Report", "Doc A Report"):
				...
				report = kanivin.get_doc("Report", "Doc A Report")
