...
class TestClient(KanivinTestCase):
	def test_set_value(self):
		todo = kanivin.get_doc(dict(doctype="ToDo", description="test")).insert()
		kanivin.set_value("ToDo", todo.name, "description", "test 1")
		self.assertEqual(kanivin.get_value("ToDo", todo.name, "description"), "test 1")

		kanivin.set_value("ToDo", todo.name, {"description": "test 2"})
		self.assertEqual(kanivin.get_value("ToDo", todo.name, "description"), "test 2")

	def test_delete(self):
		from kanivin.client import delete
		from kanivin.desk.doctype.note.note import Note

		note = kanivin.get_doc(
			doctype="Note",
			title=kanivin.generate_hash(length=8),
			content="test",
		...
		self.assertFalse(kanivin.db.exists("Note", note.name))
		self.assertRaises(kanivin.DoesNotExistError, delete, "Note", note.name)
		self.assertRaises(kanivin.DoesNotExistError, delete, "Note Seen By", child_row_name)
	...
	def test_http_valid_method_access(self):
		...
		kanivin.local.form_dict = kanivin._dict(
			{"doc": dict(doctype="ToDo", description="Valid http method"), "cmd": "kanivin.client.save"}
		)
		todo = execute_cmd("kanivin.client.save")
	...
	def test_http_invalid_method_access(self):
		...
		kanivin.local.form_dict = kanivin._dict(
			{"doc": dict(doctype="ToDo", description="Invalid http method"), "cmd": "kanivin.client.save"}
		)

		self.assertRaises(kanivin.PermissionError, execute_cmd, "kanivin.client.save")

	def test_run_doc_method(self):
		from kanivin.handler import execute_cmd

		if not kanivin.db.exists("Report", "Test Run Doc Method"):
			...
			report = kanivin.get_doc("Report", "Test Run Doc Method")
		...
		execute_cmd(kanivin.local.form_dict.cmd)
		...
		self.assertRaises(kanivin.PermissionError, execute_cmd, kanivin.local.form_dict.cmd)
	...
	def test_array_values_in_request_args(self):
		...
		kanivin.local.login_manager.login_as("Administrator")
		params = {
			"doctype": "DocType",
			"fields": ["name", "modified"],
			"sid": kanivin.session.sid,
		}
		headers = {
			"accept": "application/json",
			"content-type": "application/json",
		}
		url = get_site_url(kanivin.local.site)
		url += "/api/method/kanivin.client.get_list"
	...
	def test_client_get(self):
		from kanivin.client import get

		todo = kanivin.get_doc(doctype="ToDo", description="test").insert()
		filters = {"name": todo.name}
		filters_json = kanivin.as_json(filters)
	...
	def test_client_validatate_link(self):
		from kanivin.client import validate_link
		...
		# Permissions
		with self.set_user("Guest"), self.assertRaises(kanivin.PermissionError):
			self.assertEqual(
				validate_link("User", "Guest", fields=["enabled"]), {"name": "Guest", "enabled": 1}
			)

	def test_client_insert(self):
		from kanivin.client import insert

		def get_random_title():
			return f"test-{kanivin.generate_hash()}"
		...
		# cleanup
		kanivin.delete_doc("Note", note1.name)
		kanivin.delete_doc("Note", note2.name)

	def test_client_insert_many(self):
		from kanivin.client import insert, insert_many

		def get_random_title():
			return f"test-{kanivin.generate_hash(length=5)}"
		...
		self.assertEqual(kanivin.db.get_value("Note", docs[3], "title"), "not-a-random-title")
		self.assertEqual(kanivin.db.get_value("Note", docs[6], "title"), "another-note-title")
		self.assertIn(note1.name, docs)

		# cleanup
		for doc in docs:
			kanivin.delete_doc("Note", doc)
