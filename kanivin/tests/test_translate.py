...
from kanivin.utils import get_bench_path, set_request
...
first_lang, second_lang, third_lang, fourth_lang, fifth_lang = choices(
	...
	kanivin.get_all("Language", pluck="name", filters=[["name", "not like", "en%"], ["enabled", "=", 1]]),
	k=5,
)
...
class TestTranslate(KanivinTestCase):
	...
	def tearDown(self):
		kanivin.form_dict.pop("_lang", None)
		if self._testMethodName in self.guest_sessions_required:
	...
	def test_clear_cache(self):
		...
		self.assertIsNotNone(kanivin.cache.hget(USER_TRANSLATION_KEY, kanivin.local.lang))
		self.assertIsNotNone(kanivin.cache.hget(MERGED_TRANSLATION_KEY, kanivin.local.lang))

		clear_cache()

		self.assertIsNone(kanivin.cache.hget(USER_TRANSLATION_KEY, kanivin.local.lang))
		self.assertIsNone(kanivin.cache.hget(MERGED_TRANSLATION_KEY, kanivin.local.lang))

	def test_extract_message_from_file(self):
		data = kanivin.translate.get_messages_from_file(translation_string_file)
		bench_path = get_bench_path()
		file_path = kanivin.get_app_path("kanivin", "tests", "translation_test_file.txt")
	...
	def test_read_language_variant(self):
		...
		try:
			kanivin.local.lang = "pt-BR"
			self.assertEqual(_("Mobile No"), "Telefone Celular")
			kanivin.local.lang = "pt"
	...
	def test_request_language_resolution_with_form_dict(self):
		"""Test for kanivin.translate.get_language

		Case 1: kanivin.form_dict._lang is set
		"""

		kanivin.form_dict._lang = first_lang

		with patch.object(kanivin.translate, "get_preferred_language_cookie", return_value=second_lang):
	...
	def test_request_language_resolution_with_cookie(self):
		...
		Case 2: kanivin.form_dict._lang is not set, but preferred_language cookie is
		"""

		with patch.object(kanivin.translate, "get_preferred_language_cookie", return_value="fr"):
	...
	def test_guest_request_language_resolution_with_cookie(self):
		...
		Case 3: kanivin.form_dict._lang is not set, but preferred_language cookie is [Guest User]
		"""
	...
	def test_guest_request_language_resolution_with_request_header(self):
		...
		Case 4: kanivin.form_dict._lang & preferred_language cookie is not set, but Accept-Language header is [Guest User]
		"""
	...
	def test_request_language_resolution_with_request_header(self):
		...
		Case 5: kanivin.form_dict._lang & preferred_language cookie is not set, but Accept-Language header is
		"""
	...
	def test_load_all_translate_files(self):
		...
		verify_translation_files("kanivin")
	...
	def test_python_extractor(self):
		code = textwrap.dedent(
			...
			kanivin._("attr")
			_("name")
			kanivin._("attr with", context="attr context")
...
def verify_translation_files(app):
	...
	translations_dir = Path(kanivin.get_app_path(app)) / "translations"
