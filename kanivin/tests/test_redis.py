...
from kanivin.utils import get_bench_id
from kanivin.utils.background_jobs import get_redis_conn
...
class TestRedisAuth(KanivinTestCase):
	@skip_if_redis_version_lt("6.0")
	@patch.dict(kanivin.conf, {"bench_id": "test_bench", "use_rq_auth": False})
	...
	def test_rq_namespace(self):
		...
		# Current bench ID
		bench_id = kanivin.conf.get("bench_id")
		conn = get_redis_conn()
		...
		conn.acl_deluser(username)
		kanivin.conf.update({"bench_id": tmp_bench_id})
		_ = RedisQueue(conn).add_user(username, password)
		...
		kanivin.conf.update({"bench_id": bench_id})
		conn.acl_deluser(username)
