...
import kanivin
from kanivin.model import child_table_fields
from kanivin.tests.utils import KanivinTestCase


class TestChildTable(KanivinTestCase):
	def tearDown(self) -> None:
		try:
			kanivin.delete_doc("DocType", self.doctype_name, force=1)
		except Exception:
			pass
	...
	def test_child_table_doctype_creation_and_transitioning(self) -> None:
		...
		for column in child_table_fields:
			self.assertTrue(kanivin.db.has_column(self.doctype_name, column))
	...
	def check_valid_columns(self, assertion_method: Callable) -> None:
		valid_columns = kanivin.get_meta(self.doctype_name).get_valid_columns()
		for column in child_table_fields:
