...
import kanivin
from kanivin.model import core_doctypes_list, get_permitted_fields, is_default_field
from kanivin.model.utils import get_fetch_values
from kanivin.tests.utils import KanivinTestCase


class TestModelUtils(KanivinTestCase):
	def test_get_fetch_values(self):
		...
		full_name = kanivin.db.get_value("User", user, "full_name")
	...
	def test_get_permitted_fields(self):
		...
		todo_all_columns = kanivin.get_meta("ToDo").get_valid_columns()
		...
		with set_user("Guest"):
			...
			picked_doctype_all_columns = kanivin.get_meta(picked_doctype).get_valid_columns()
		...
		with set_user("Administrator"):
			...
			child_all_fields = kanivin.get_meta("Installed Application").get_valid_columns()
...
def set_user(user: str):
	past_user = kanivin.session.user or "Administrator"
	kanivin.set_user(user)
	yield
	kanivin.set_user(past_user)
