...
import kanivin
from kanivin.core.doctype.scheduled_job_type.scheduled_job_type import ScheduledJobType, sync_jobs
from kanivin.utils import add_days, get_datetime
from kanivin.utils.doctor import purge_pending_jobs
...
class TestScheduler(TestCase):
	def setUp(self):
		...
		purge_pending_jobs()
		if not kanivin.get_all("Scheduled Job Type", limit=1):
			sync_jobs()
	...
	def test_enqueue_jobs(self):
		kanivin.db.sql("update `tabScheduled Job Type` set last_execution = '2010-01-01 00:00:00'")

		enqueued_jobs = enqueue_events(site=kanivin.local.site)

		self.assertIn("kanivin.desk.notifications.clear_notifications", enqueued_jobs)
		self.assertIn("kanivin.utils.change_log.check_for_update", enqueued_jobs)
		self.assertIn(
	...
	@patch.object(kanivin.utils.kanivincloud, "on_kanivincloud", return_value=True)
	@patch.dict(kanivin.conf, {"developer_mode": 0})
	...
	def test_once_a_day_for_dormant(self, _mocks):
		...
		kanivin.db.truncate("Scheduled Job Log")
		...
		job = get_test_job(method="kanivin.tests.test_scheduler.test_method", frequency="Daily")
		job.execute()
		job_log = kanivin.get_doc("Scheduled Job Log", dict(scheduled_job_type=job.name))
...
def get_test_job(method="kanivin.tests.test_scheduler.test_timeout_10", frequency="All") -> ScheduledJobType:
	if not kanivin.db.exists("Scheduled Job Type", dict(method=method)):
		job = kanivin.get_doc(
			dict(
		...
		job = kanivin.get_doc("Scheduled Job Type", dict(method=method))
