...
from kanivin.tests.test_api import KanivinAPITestCase, suppress_stdout
...
class TestResourceAPIV2(KanivinAPITestCase):
	version = "v2"
	DOCTYPE = "ToDo"
	...
	def test_copy_document(self):
		doc = kanivin.get_doc(self.DOCTYPE, self.GENERATED_DOCUMENTS[0])
	...
	def test_execute_doc_method(self):
		...
		self.assertEqual(response.json["data"][0]["name"], "kanivin")
	...
	def test_delete_document_non_existing(self):
		non_existent_doc = kanivin.generate_hash(length=12)
...
class TestMethodAPIV2(KanivinAPITestCase):
	version = "v2"
	...
	def test_shorthand_controller_methods(self):
		...
		expanded_response = self.get(
			self.method("kanivin.core.doctype.user.user.get_all_roles"), {"sid": self.sid}
		)
	...
	def test_add_comment(self):
		comment_txt = kanivin.generate_hash()
		response = self.post(
...
class TestDocTypeAPIV2(KanivinAPITestCase):
	version = "v2"
