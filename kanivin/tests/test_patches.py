...
import kanivin
from kanivin.modules import patch_handler
...
execute:kanivin.db.updatedb("Item")
execute:kanivin.function(arg="1")
...
class TestPatches(KanivinTestCase):
	def test_patch_module_names(self):
		kanivin.flags.final_patches = []
		kanivin.flags.in_install = True
		for patchmodule in patch_handler.get_all_patches():
			if patchmodule.startswith("execute:"):
				...
				self.assertTrue(kanivin.get_attr(patchmodule.split(maxsplit=1)[0] + ".execute"))

		kanivin.flags.in_install = False

	def test_get_patch_list(self):
		pre = patch_handler.get_patches_from_app("kanivin", patch_handler.PatchType.pre_model_sync)
		post = patch_handler.get_patches_from_app("kanivin", patch_handler.PatchType.post_model_sync)
		all_patches = patch_handler.get_patches_from_app("kanivin")
	...
	def test_all_patches_are_marked_completed(self):
		all_patches = patch_handler.get_patches_from_app("kanivin")
		finished_patches = kanivin.db.count("Patch Log")

		self.assertGreaterEqual(finished_patches, len(all_patches))


class TestPatchReader(KanivinTestCase):
	def get_patches(self):
		return (
			patch_handler.get_patches_from_app("kanivin"),
			patch_handler.get_patches_from_app("kanivin", patch_handler.PatchType.pre_model_sync),
			patch_handler.get_patches_from_app("kanivin", patch_handler.PatchType.post_model_sync),
		)
...
def check_patch_files(app):
	...
	patch_dir = Path(kanivin.get_app_path(app)) / "patches"
	...
	for file in patch_dir.glob("**/*.py"):
		...
		try:
			patch_module = kanivin.get_module(module)
...
def _get_dotted_path(file: Path, app) -> str:
	app_path = Path(kanivin.get_app_path(app))
