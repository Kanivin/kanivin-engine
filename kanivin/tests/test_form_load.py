...
from kanivin.desk.form.load import get_docinfo, getdoc, getdoctype
from kanivin.tests.utils import KanivinTestCase
from kanivin.utils.file_manager import save_file

test_dependencies = ["Blog Category", "Blogger"]


class TestFormLoad(KanivinTestCase):
	def test_load(self):
		getdoctype("DocType")
		meta = next(filter(lambda d: d.name == "DocType", kanivin.response.docs))
		self.assertEqual(meta.name, "DocType")
		self.assertTrue(meta.get("__js"))

		kanivin.response.docs = []
		getdoctype("Event")
		meta = next(filter(lambda d: d.name == "Event", kanivin.response.docs))
		self.assertTrue(meta.get("__calendar_js"))

	def test_fieldlevel_permissions_in_load(self):
		blog = kanivin.get_doc(
			{
				"doctype": "Blog Post",
				...
				"title": f"_Test Blog Post {kanivin.utils.now()}",
				"published": 0,
			}
		)
		...
		doc = kanivin.get_doc("Blog Post", blog.name)
		doc.published = 1
		doc.save()
		...
		blog_doc.delete()
		kanivin.delete_doc(blog_post_property_setter.doctype, blog_post_property_setter.name)
	...
	def test_fieldlevel_permissions_in_load_for_child_table(self):
		...
		contact = kanivin.get_doc("Contact", "_Test Contact 1")
	...
	def test_get_doc_info(self):
		...
		get_docinfo(note)
		docinfo = kanivin.response["docinfo"]
...
def get_blog(blog_name):
	...
	return kanivin.response.docs[0]
