...
from kanivin.exceptions import DoesNotExistError
from kanivin.model.base_document import get_controller
from kanivin.model.rename_doc import bulk_rename, update_document_title
from kanivin.modules.utils import get_doc_path
...
def patch_db(endpoints: list[str] | None = None):
	...
	for point in endpoints:
		x = patch(f"kanivin.db.{point}", new=lambda: True)
		patched_endpoints.append(x)

	savepoint = "SAVEPOINT_for_test_bulk_rename"
	kanivin.db.savepoint(save_point=savepoint)
	try:
...
class TestRenameDoc(KanivinTestCase):
	@classmethod
	def setUpClass(self):
		"""Setting Up data for the tests defined under TestRenameDoc"""
		# set developer_mode to rename doc controllers
		super().setUpClass()
		self._original_developer_flag = kanivin.conf.developer_mode
		kanivin.conf.developer_mode = 1
	...
	def tearDownClass(self):
		...
		kanivin.delete_doc_if_exists("DocType", "Renamed Doc")
		kanivin.db.sql_ddl("drop table if exists `tabRenamed Doc`")

		# delete the documents created
		for docname in self.available_documents:
			kanivin.delete_doc(self.test_doctype, docname)

		for dt in self.doctype.values():
			if kanivin.db.exists("DocType", dt):
				kanivin.delete_doc("DocType", dt)
				kanivin.db.sql_ddl(f"DROP TABLE IF EXISTS `tab{dt}`")

		# reset original value of developer_mode conf
		kanivin.conf.developer_mode = self._original_developer_flag

	def setUp(self):
		kanivin.flags.link_fields = {}
		if self._testMethodName == "test_doc_rename_method":
			self.property_setter = kanivin.get_doc(
				{
	...
	def test_rename_doc(self):
		"""Rename an existing document via kanivin.rename_doc"""
		old_name = choice(self.available_documents)
		new_name = old_name + ".new"
		self.assertEqual(new_name, kanivin.rename_doc(self.test_doctype, old_name, new_name, force=True))
		self.available_documents.remove(old_name)
		self.available_documents.append(new_name)

	def test_merging_docs(self):
		"""Merge two documents via kanivin.rename_doc"""
		first_todo, second_todo = sample(self.available_documents, 2)

		second_todo_doc = kanivin.get_doc(self.test_doctype, second_todo)
		second_todo_doc.priority = "High"
		second_todo_doc.save()

		merged_todo = kanivin.rename_doc(self.test_doctype, first_todo, second_todo, merge=True, force=True)
		merged_todo_doc = kanivin.get_doc(self.test_doctype, merged_todo)
		self.available_documents.remove(first_todo)

		with self.assertRaises(DoesNotExistError):
			kanivin.get_doc(self.test_doctype, first_todo)
	...
	def test_rename_controllers(self):
		...
		self.assertNotEqual(get_controller(self.doctype.old), kanivin.model.document.Document)

		old_doctype_path = get_doc_path("Custom", "DocType", self.doctype.old)

		# rename doc via wrapper API accessible via /desk
		kanivin.rename_doc("DocType", self.doctype.old, self.doctype.new)

		# check if database and controllers are updated
		self.assertTrue(kanivin.db.exists("DocType", self.doctype.new))
		self.assertFalse(kanivin.db.exists("DocType", self.doctype.old))
		self.assertFalse(os.path.exists(old_doctype_path))

	def test_rename_doctype(self):
		"""Rename DocType via kanivin.rename_doc"""
		from kanivin.core.doctype.doctype.test_doctype import new_doctype

		if not kanivin.db.exists("DocType", "Rename This"):
			new_doctype(
				"Rename This",
				fields=[
					{
		...
		to_rename_record = kanivin.get_doc(
			{"doctype": "Rename This", "linked_to_doctype": "Rename This"}
		).insert()

		# Rename doctype
		self.assertEqual(
			"Renamed Doc", kanivin.rename_doc("DocType", "Rename This", "Renamed Doc", force=True)
		)

		# Test if Doctype value has changed in Link field
		linked_to_doctype = kanivin.db.get_value("Renamed Doc", to_rename_record.name, "linked_to_doctype")
		...
		new_name = "ToDo"
		self.assertEqual(new_name, kanivin.rename_doc("Renamed Doc", old_name, new_name, force=True))

	def test_update_document_title_api(self):
		test_doctype = "Module Def"
		test_doc = kanivin.get_doc(
			{
				"doctype": test_doctype,
				"module_name": f"Test-test_update_document_title_api-{kanivin.generate_hash()}",
				"custom": True,
			}
		)
		...
		doc_before = kanivin.get_doc(test_doctype, dn)
		return_value = update_document_title(doctype=dt, docname=dn, new_name=new_name)
		doc_after = kanivin.get_doc(test_doctype, return_value)
	...
	def test_bulk_rename(self):
		...
		with patch_db(["commit", "rollback"]), patch("kanivin.enqueue") as enqueue:
	...
	def test_doc_rename_method(self):
		...
		new_name = f"{name}-{kanivin.generate_hash(length=4)}"
		doc = kanivin.get_doc(self.test_doctype, name)
		doc.rename(new_name, merge=kanivin.db.exists(self.test_doctype, new_name))
	...
	def test_parenttype(self):
		...
		parent_a_instance = kanivin.get_doc(
			doctype=parent_a.name, test_table=[{"some_fieldname": "x"}], name="XYZ"
		).insert()

		parent_b_instance = kanivin.get_doc(
