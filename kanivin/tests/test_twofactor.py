...
import kanivin
from kanivin.auth import HTTPRequest, get_login_attempt_tracker, validate_ip_address
...
from kanivin.utils import cint, set_request

from . import get_system_setting, update_system_settings


class TestTwoFactor(KanivinTestCase):
	...
	def setUp(self):
		...
		self.login_manager = kanivin.local.login_manager
		self.user = self.login_manager.user
		update_system_settings({"allow_consecutive_login_attempts": 2})

	def tearDown(self):
		kanivin.local.response["verification"] = None
		kanivin.local.response["tmp_id"] = None
		disable_2fa()
		kanivin.clear_cache(user=self.user)
	...
	def test_authenticate_for_2factor(self):
		"""Verification obj and tmp_id should be set in kanivin.local."""
		authenticate_for_2factor(self.user)
		verification_obj = kanivin.local.response["verification"]
		tmp_id = kanivin.local.response["tmp_id"]
		self.assertTrue(verification_obj)
		self.assertTrue(tmp_id)
		for k in ["_usr", "_pwd", "_otp_secret"]:
			self.assertTrue(kanivin.cache.get(f"{tmp_id}{k}"), f"{k} not available")
	...
	def test_two_factor_is_enabled(self):
		...
		# Scenario 3
		enable_2fa()
		user = kanivin.get_doc("User", self.user)
		user.restrict_ip = kanivin.local.request_ip
		user.save()
		self.assertTrue(should_run_2fa(self.user))
	...
	def test_confirm_otp_token(self):
		...
		kanivin.flags.otp_expiry = 2
		authenticate_for_2factor(self.user)
		tmp_id = kanivin.local.response["tmp_id"]
		otp = "wrongotp"
		with self.assertRaises(kanivin.AuthenticationError):
		...
		kanivin.flags.otp_expiry = None
		if kanivin.flags.tests_verbose:
	...
	def test_render_string_template(self):
		...
		_str = kanivin.render_template(_str, args)
	...
	def test_otp_attempt_tracker(self):
		...
		with self.assertRaises(kanivin.SecurityException):
			confirm_otp_token(self.login_manager, otp=otp, tmp_id=tmp_id)
...
def create_http_request():
	...
	enable_2fa()
	kanivin.form_dict["usr"] = "test@example.com"
	kanivin.form_dict["pwd"] = "Eastern_43A1W"
	kanivin.local.form_dict["cmd"] = "login"
	return HTTPRequest()


def enable_2fa(bypass_two_factor_auth=0, bypass_restrict_ip_check=0):
	"""Enable Two factor in system settings."""
	system_settings = kanivin.get_doc("System Settings")
...
def toggle_2fa_all_role(state=None):
	...
	all_role = kanivin.get_doc("Role", "All")
