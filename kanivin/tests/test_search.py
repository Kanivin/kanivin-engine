...
from kanivin.desk.search import get_names_for_mentions, search_link, search_widget
...
class TestSearch(KanivinTestCase):
	def setUp(self):
	...
	def test_validate_and_sanitize_search_inputs(self):
		...
		# should not fail if function is called via kanivin.call with extra arguments
		args = ("Random DocType", "Random", "email", "2", "10", dict())
		kwargs = {"as_dict": False}
		self.assertListEqual(kanivin.call("kanivin.tests.test_search.get_data", *args, **kwargs), [])
	...
	def test_reference_doctype(self):
		...
		results = search_link(
			...
			query="kanivin.tests.test_search.query_with_reference_doctype",
		)
		self.assertListEqual(results, [])
	...
	def test_search_relevance(self):
		...
		kanivin.db.set_value("Language", "es", "idx", 10)
...
def setup_test_link_field_order(TestCase):
	...
	# Create Tree doctype
	if not kanivin.db.exists("DocType", TestCase.tree_doctype_name):
		TestCase.tree_doc = kanivin.get_doc(
			{
				"doctype": "DocType",
		...
		TestCase.tree_doc = kanivin.get_doc("DocType", TestCase.tree_doctype_name)

	# Create root for the tree doctype
	if not kanivin.db.exists(TestCase.tree_doctype_name, {"random": TestCase.parent_doctype_name}):
		kanivin.get_doc(
...
class TestWebsiteSearch(KanivinTestCase):
