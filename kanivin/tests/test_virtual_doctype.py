...
import kanivin
import kanivin.modules.utils
from kanivin.core.doctype.doctype.test_doctype import new_doctype
from kanivin.desk.form.save import savedocs
...
class VirtualDoctypeTest(Document):
	...
	def get_list(args):
		...
		return [kanivin._dict(doc) for name, doc in data.items()]
...
class TestVirtualDoctypes(KanivinTestCase):
	@classmethod
	def setUpClass(cls):
		kanivin.flags.allow_doctype_export = True
		cls.addClassCleanup(kanivin.flags.pop, "allow_doctype_export", None)
		...
		patch_virtual_doc = patch(
			"kanivin.controllers", new={kanivin.local.site: {TEST_DOCTYPE_NAME: VirtualDoctypeTest}}
		)
	...
	def test_insert_update_and_load_from_desk(self):
		...
		docname = kanivin.response.docs[0]["name"]

		doc = kanivin.get_doc(TEST_DOCTYPE_NAME, docname)
	...
	def test_multiple_doc_insert_and_get_list(self):
		doc1 = kanivin.new_doc(doctype=TEST_DOCTYPE_NAME)
		doc1.append("child_table", {"name": "first", "some_fieldname": "first-value"})
		doc1.insert()
		doc2 = kanivin.new_doc(doctype=TEST_DOCTYPE_NAME)
	...
	def test_delete_doc(self):
		doc = kanivin.get_doc(doctype=TEST_DOCTYPE_NAME).insert()

		kanivin.delete_doc(doc.doctype, doc.name)
