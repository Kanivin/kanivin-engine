...
class TestFixtureImport(KanivinTestCase):
	def create_new_doctype(self, DocType: str) -> None:
		file = kanivin.get_app_path("kanivin", "custom", "fixtures", f"{DocType}.json")
	...
	def insert_dummy_data_and_export(self, DocType: str, dummy_name_list: list[str]) -> str:
		for name in dummy_name_list:
			doc = kanivin.get_doc({"doctype": DocType, "member_name": name})
			doc.insert()
	...
	def test_fixtures_import(self):
		self.assertFalse(kanivin.db.exists("DocType", "temp_doctype"))
		...
		kanivin.db.truncate("temp_doctype")
		...
		self.assertEqual(kanivin.db.count("temp_doctype"), len(dummy_name_list))

		data = kanivin.get_all("temp_doctype", "member_name")
		kanivin.db.truncate("temp_doctype")
	...
	def test_singles_fixtures_import(self):
		self.assertFalse(kanivin.db.exists("DocType", "temp_singles"))
		...
		singles_doctype = kanivin.qb.DocType("Singles")
		truncate_query = (
			kanivin.qb.from_(singles_doctype).delete().where(singles_doctype.doctype == "temp_singles")
		)
		truncate_query.run()

		import_doc(path_to_exported_fixtures)

		data = kanivin.db.get_single_value("temp_singles", "member_name")
		truncate_query.run()
