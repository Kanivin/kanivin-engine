...
from kanivin.modules import export_customizations, export_module_json, get_module_path
from kanivin.modules.utils import export_doc, sync_customizations
...
class TestUtils(KanivinTestCase):
	def setUp(self):
		self._dev_mode = kanivin.local.conf.developer_mode
		kanivin.local.conf.developer_mode = True

	def tearDown(self):
		kanivin.db.rollback()
		kanivin.local.conf.developer_mode = self._dev_mode
		kanivin.local.flags.pop("in_import", None)

	def test_export_module_json_no_export(self):
		kanivin.local.flags.in_import = True
		doc = kanivin.get_last_doc("DocType")
	...
	def test_export_module_json(self):
		doc = kanivin.get_last_doc("DocType", {"issingle": 0, "custom": 0})
		...
		with open(export_doc_path) as f:
			export_doc_before = kanivin.parse_json(f.read())

		last_modified_before = os.path.getmtime(export_doc_path)
		self.addCleanup(write_file, path=export_doc_path, content=kanivin.as_json(export_doc_before))

		kanivin.flags.in_import = False
		kanivin.conf.developer_mode = True
		...
		with open(f"{export_path}.json") as f:
			kanivin.parse_json(f.read())  # export_doc_after
	...
	def test_sync_customizations(self):
		with note_customizations() as (custom_field, property_setter):
			...
			sync_customizations(app="kanivin")
	...
	def test_reload_doc(self):
		kanivin.db.set_value("DocType", "Note", "migration_hash", "", update_modified=False)
		self.assertFalse(kanivin.db.get_value("DocType", "Note", "migration_hash"))
		...
		self.assertEqual(
			kanivin.db.get_value("DocField", {"parent": "Note", "fieldname": "title"}, "fieldtype"),
			"Text",
		)
		kanivin.reload_doctype("Note")
		self.assertEqual(
			kanivin.db.get_value("DocField", {"parent": "Note", "fieldname": "title"}, "fieldtype"),
			"Data",
		)
		self.assertTrue(kanivin.db.get_value("DocType", "Note", "migration_hash"))
	...
	def test_export_doc(self):
		...
		note.title = kanivin.generate_hash(length=10)
		note.save()
		export_doc(doctype="Note", name=note.name)
		exported_doc_path = Path(
			kanivin.get_app_path("kanivin", "desk", "note", note.name, f"{note.name}.json")
		)
	...
	def test_make_boilerplate(self):
		with temp_doctype() as doctype:
			scrubbed = kanivin.scrub(doctype.name)
			path = kanivin.get_app_path("kanivin", "core", "doctype", scrubbed, f"{scrubbed}.json")
...
def temp_doctype():
	try:
		...
		kanivin.db.sql_ddl(f"DROP TABLE `tab{doctype.name}`")
...
def note_customizations():
	try:
		...
		delete_path(kanivin.get_module_path("Desk", "Note"))
