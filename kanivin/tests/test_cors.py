...
import kanivin
from kanivin.app import process_response
...
class TestCORS(KanivinTestCase):
	def make_request_and_test(self, origin="http://example.com", absent=False):
		...
		kanivin.utils.set_request(method="OPTIONS", headers=headers)
	...
	def test_valid_origin(self):
		...
		kanivin.conf.allow_cors = "*"
		self.make_request_and_test()

		kanivin.conf.allow_cors = ["http://example.com", "https://example.com"]
		self.make_request_and_test()

	def test_invalid_origin(self):
		kanivin.conf.allow_cors = "http://example1.com"
		self.make_request_and_test(absent=True)

		kanivin.conf.allow_cors = ["http://example1.com", "https://example.com"]
		self.make_request_and_test(absent=True)
