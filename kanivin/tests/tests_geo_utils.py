...
import kanivin
from kanivin.geo.utils import get_coords
from kanivin.tests.utils import KanivinTestCase


class TestGeoUtils(KanivinTestCase):
	def setUp(self):
		self.todo = kanivin.get_doc(
		...
		self.test_location = kanivin.get_doc(
	...
	def test_get_coords_from_not_existable_location(self):
		self.assertRaises(kanivin.ValidationError, get_coords, "ToDo", self.test_filter_todo, "location_field")

	def test_get_coords_from_not_existable_coords(self):
		self.assertRaises(kanivin.ValidationError, get_coords, "ToDo", self.test_filter_todo, "coordinates")
