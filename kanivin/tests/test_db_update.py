...
class TestDBUpdate(KanivinTestCase):
	def test_db_update(self):
		doctype = "User"
		kanivin.reload_doctype("User", force=True)
		kanivin.model.meta.trim_tables("User")
		...
		table_columns = kanivin.db.get_table_columns_description(f"tab{doctype}")
		...
		for field_def in field_defs:
			...
			fallback_default = (
				"0" if field_def.get("fieldtype") in kanivin.model.numeric_fieldtypes else "NULL"
			)
	...
	def test_index_and_unique_constraints(self):
		...
		kanivin.db.add_index(doctype, ["email_signature(200)"])
		kanivin.db.updatedb(doctype)
	...
	def test_bigint_conversion(self):
		...
		with self.assertRaises(kanivin.CharacterLengthExceededError):
			kanivin.get_doc(doctype=doctype.name, int_field=2**62 - 1).insert()
	...
	def test_unique_index_on_install(self):
		...
		for dt in kanivin.get_all("DocType", {"is_virtual": 0, "issingle": 0}, pluck="name"):
			doctype = kanivin.get_meta(dt)
	...
	def test_unique_index_on_alter(self):
		...
		new_field = kanivin.copy_doc(doctype.fields[0])
...
def get_fieldtype_from_def(field_def):
	fieldtuple = kanivin.db.type_map.get(field_def.fieldtype, ("", 0))
...
def get_other_fields_meta(meta):
	...
	optional_fields = kanivin.db.OPTIONAL_COLUMNS
	if meta.track_seen:
		optional_fields.append("_seen")

	child_table_fields_map = {}
	if meta.istable:
		child_table_fields_map.update({field: ("Data", 0) for field in kanivin.db.CHILD_TABLE_COLUMNS})
	...
	return [
		kanivin._dict({"fieldname": field, "fieldtype": _type, "length": _length})
