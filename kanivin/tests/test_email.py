...
from kanivin.email.doctype.email_account.test_email_account import TestEmailAccount
...
class TestEmail(KanivinTestCase):
	def setUp(self):
		kanivin.db.delete("Email Unsubscribe")
		kanivin.db.delete("Email Queue")
		kanivin.db.delete("Email Queue Recipient")
	...
	def test_send_after(self):
		...
		from kanivin.email.queue import flush

		flush()
		email_queue = kanivin.db.sql("""select name from `tabEmail Queue` where status='Sent'""", as_dict=1)
	...
	def test_flush(self):
		...
		self.assertTrue("test1@example.com" in queue_recipients)
		self.assertEqual(len(queue_recipients), 2)
		self.assertTrue("Unsubscribe" in kanivin.safe_decode(kanivin.flags.sent_mail))
	...
	def test_cc_footer(self):
		...
		self.assertTrue(
			...
			in kanivin.safe_decode(kanivin.flags.sent_mail)
		)
	...
	def test_expose(self):
		...
		email_obj = email.message_from_string(kanivin.safe_decode(kanivin.flags.sent_mail))
		for part in email_obj.walk():
			...
			if content:
				...
				query_string = re.search(
					r"(?<=/api/method/kanivin.email.queue.unsubscribe\?).*(?=" + eol + ")", content.decode()
				).group(0)
	...
	def test_sender(self):
		def _patched_assertion(email_account, assertion):
			with patch.object(QueueBuilder, "get_outgoing_email_account", return_value=email_account):
				...
				email_queue_sender = kanivin.db.get_value("Email Queue", {"status": "Sent"}, "sender")
	...
	def test_unsubscribe(self):
		from kanivin.email.queue import unsubscribe
		...
		email_queue = kanivin.db.get_value("Email Queue", {"status": "Not Sent"})
		queue_recipients = [
			r.recipient
			for r in kanivin.db.sql(
				"""select recipient from `tabEmail Queue Recipient`
		...
		kanivin.get_doc("Email Queue", email_queue).send()
		self.assertTrue("Unsubscribe" in kanivin.safe_decode(kanivin.flags.sent_mail))
	...
	def test_image_parsing(self):
		...
		kanivin.db.delete("Communication", {"sender": "sukh@yyy.com"})

		with open(kanivin.get_app_path("kanivin", "tests", "data", "email_with_image.txt")) as raw:
			messages = {
...
class TestVerifiedRequests(KanivinTestCase):
...
class TestEmailIntegrationTest(KanivinTestCase):
	...
	def setUp(self) -> None:
		# Kanivin code is configured to not attempting sending emails during test.
		kanivin.flags.testing_email = True
		requests.delete(f"{self.SMTP4DEV_WEB}/api/Messages/*")
		return super().setUp()

	def tearDown(self) -> None:
		kanivin.flags.testing_email = False
	...
	def test_store_attachments(self):
		...
		communication = kanivin.get_doc("Communication", name)

		attachments = get_attachments(communication.doctype, communication.name)
		self.assertEqual(len(attachments), 1)

		file = kanivin.get_doc("File", attachments[0].name)
