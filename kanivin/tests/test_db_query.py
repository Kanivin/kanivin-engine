...
from kanivin.core.page.permission_manager.permission_manager import add, reset, update
from kanivin.custom.doctype.property_setter.property_setter import make_property_setter
from kanivin.database.utils import DefaultOrderBy
from kanivin.desk.reportview import get_filters_cond
from kanivin.handler import execute_cmd
from kanivin.model.db_query import DatabaseQuery, get_between_date_filter
from kanivin.permissions import add_user_permission, clear_user_permissions_for_doctype
from kanivin.query_builder import Column
...
def setup_test_user(set_user=False):
	test_user = kanivin.get_doc("User", "test@example.com")
	user_roles = kanivin.get_roles()
	test_user.remove_roles(*user_roles)
	test_user.add_roles("Blogger")

	if set_user:
		kanivin.set_user(test_user.name)
...
class TestDBQuery(KanivinTestCase):
	def setUp(self):
	...
	def test_child_table_field_syntax(self):
		note = kanivin.get_doc(
			doctype="Note",
			title=f"Test {kanivin.utils.random_string(8)}",
			content="test",
			seen_by=[{"user": "Administrator"}],
		).insert()
		result = kanivin.get_all(
			"Note",
	...
	def test_child_table_join(self):
		kanivin.delete_doc_if_exists("DocType", "Parent DocType 1")
		kanivin.delete_doc_if_exists("DocType", "Parent DocType 2")
		kanivin.delete_doc_if_exists("DocType", "Child DocType")
		# child table
		kanivin.get_doc(
			{
				"doctype": "DocType",
		...
		# clear records
		kanivin.db.delete("Parent DocType 1")
		kanivin.db.delete("Parent DocType 2")
		kanivin.db.delete("Child DocType")
		...
		# test query
		results1 = kanivin.get_all("Parent DocType 1", fields=["name", "child.title as child_title"])
		results2 = kanivin.get_all("Parent DocType 2", fields=["name", "child.title as child_title"])
	...
	def test_link_field_syntax(self):
		todo = kanivin.get_doc(doctype="ToDo", description="Test ToDo", allocated_to="Administrator").insert()
		result = kanivin.get_all(
			"ToDo",
	...
	def test_build_match_conditions(self):
		...
		test2user = kanivin.get_doc("User", "test2@example.com")
		test2user.add_roles("Blogger")
		kanivin.set_user("test2@example.com")
	...
	def test_none_filter(self):
		query = kanivin.qb.get_query("DocType", fields="name", filters={"restrict_to_domain": None})
	...
	def test_between_filters_date_bounds(self):
		date_df = kanivin._dict(fieldtype="Date")
		datetime_df = kanivin._dict(fieldtype="Datetime")
		today = kanivin.utils.nowdate()
	...
	def test_ignore_permissions_for_get_filters_cond(self):
		kanivin.set_user("test2@example.com")
		self.assertRaises(kanivin.PermissionError, get_filters_cond, "DocType", dict(istable=1), [])
	...
	def test_query_fields_sanitizer(self):
		...
		with self.assertRaises(kanivin.DataError):
			DatabaseQuery("DocType").execute(
				fields=[
					"name",
					"issingle",
	...
	def test_nested_permission(self):
		...
		from kanivin.core.page.permission_manager.permission_manager import update
	...
	def test_of_not_of_descendant_ancestors(self):
		...
		data = kanivin.get_all("Nested DocType", {"name": ("descendants of", "Level 2 A")})
		self.assertTrue({"name": "Level 3 A"} in data)

		data = kanivin.get_all("Nested DocType", {"name": ("descendants of", "Level 1 A")})
		self.assertTrue({"name": "Level 3 A"} in data)
		self.assertTrue({"name": "Level 2 A"} in data)
		self.assertFalse({"name": "Level 2 B"} in data)
		self.assertFalse({"name": "Level 1 B"} in data)
		...
		data = kanivin.get_all("Nested DocType", {"name": ("ancestors of", "Level 2 A")})
		...
		data = kanivin.get_all("Nested DocType", {"name": ("ancestors of", "Level 1 A")})
		self.assertFalse({"name": "Level 3 A"} in data)
		self.assertFalse({"name": "Level 2 A"} in data)
		self.assertFalse({"name": "Level 2 B"} in data)
		self.assertFalse({"name": "Level 1 B"} in data)
		...
		data = kanivin.get_all("Nested DocType", {"name": ("not descendants of", "Level 2 A")})
		self.assertFalse({"name": "Level 3 A"} in data)
		self.assertTrue({"name": "Level 2 A"} in data)
		...
		data = kanivin.get_all("Nested DocType", {"name": ("not descendants of", "Level 1 A")})
		self.assertFalse({"name": "Level 3 A"} in data)
		self.assertFalse({"name": "Level 2 A"} in data)
		self.assertTrue({"name": "Level 2 B"} in data)
		...
		data = kanivin.get_all("Nested DocType", {"name": ("not ancestors of", "Level 2 A")})
		self.assertTrue({"name": "Level 3 A"} in data)
		self.assertTrue({"name": "Level 2 A"} in data)
		self.assertTrue({"name": "Level 2 B"} in data)
		self.assertTrue({"name": "Level 1 B"} in data)
		...
		data = kanivin.get_all("Nested DocType", {"name": ("not ancestors of", "Level 1 A")})
		self.assertTrue({"name": "Level 3 A"} in data)
		self.assertTrue({"name": "Level 2 A"} in data)
		self.assertTrue({"name": "Level 2 B"} in data)
		self.assertTrue({"name": "Level 1 B"} in data)
		...
		data = kanivin.get_all("Nested DocType", {"name": ("ancestors of", "Root")})
		self.assertTrue(len(data) == 0)
		self.assertTrue(
			len(kanivin.get_all("Nested DocType", {"name": ("not ancestors of", "Root")}))
			== len(kanivin.get_all("Nested DocType"))
		)
	...
	def test_is_set_is_not_set(self):
		...
		kanivin.db.set_value("DocType", "Property Setter", "autoname", None, update_modified=False)
	...
	def test_virtual_field_get_list(self):
		try:
			kanivin.get_list("Prepared Report", ["*"])
			kanivin.get_list("Scheduled Job Type", ["*"])
		except Exception:
	...
	def test_prepare_select_args(self):
		# kanivin.get_all inserts modified field into order_by clause
		# test to make sure this is inserted into select field when postgres
		doctypes = kanivin.get_all(
			"DocType",
	...
	def test_column_comparison(self):
		...
		users_unedited = kanivin.get_all(
			"User",
			filters={"creation": Column("modified")},
			fields=["name", "creation", "modified"],
			limit=1,
		)
		users_edited = kanivin.get_all(
			"User",
			filters={"creation": ("!=", Column("modified"))},
			fields=["name", "creation", "modified"],
			limit=1,
	...
	def test_cast_name(self):
		...
		kanivin.delete_doc_if_exists("DocType", "autoinc_dt_test")
	...
	def test_fieldname_starting_with_int(self):
		...
		kanivin.delete_doc_if_exists("DocType", "dt_with_int_named_fieldname")
		kanivin.delete_doc_if_exists("DocType", "table_dt")
		...
		dt_data = kanivin.get_doc({"doctype": "dt_with_int_named_fieldname", "1field": "10"}).insert(
			ignore_permissions=True
		)
	...
	def test_permission_query_condition(self):
		from kanivin.desk.doctype.dashboard_settings.dashboard_settings import (
		...
		dashboard_settings = kanivin.db.sql(
			f"""
				SELECT name
	...
	def test_virtual_doctype(self):
		...
		with patch(
			"kanivin.controllers",
			new={kanivin.local.site: {"Virtual DocType": VirtualDocType}},
		):
			VirtualDocType.get_list = MagicMock()

			kanivin.get_all("Virtual DocType", filters={"name": "test"}, fields=["name"], limit=1)
	...
	def test_coalesce_with_in_ops(self):
		self.assertNotIn("ifnull", kanivin.get_all("User", {"first_name": ("in", ["a", "b"])}, run=0))
		self.assertIn("ifnull", kanivin.get_all("User", {"first_name": ("in", ["a", None])}, run=0))
		self.assertIn("ifnull", kanivin.get_all("User", {"first_name": ("in", ["a", ""])}, run=0))
		self.assertIn("ifnull", kanivin.get_all("User", {"first_name": ("in", [])}, run=0))
		self.assertIn("ifnull", kanivin.get_all("User", {"first_name": ("not in", ["a"])}, run=0))
		self.assertIn("ifnull", kanivin.get_all("User", {"first_name": ("not in", [])}, run=0))
		self.assertIn("ifnull", kanivin.get_all("User", {"first_name": ("not in", [""])}, run=0))

		# primary key is never nullable
		self.assertNotIn("ifnull", kanivin.get_all("User", {"name": ("in", ["a", None])}, run=0))
		self.assertNotIn("ifnull", kanivin.get_all("User", {"name": ("in", ["a", ""])}, run=0))
		self.assertNotIn("ifnull", kanivin.get_all("User", {"name": ("in", (""))}, run=0))
		self.assertNotIn("ifnull", kanivin.get_all("User", {"name": ("in", ())}, run=0))

	def test_coalesce_with_datetime_ops(self):
		self.assertNotIn("ifnull", kanivin.get_all("User", {"last_active": (">", "2022-01-01")}, run=0))
		self.assertNotIn("ifnull", kanivin.get_all("User", {"creation": ("<", "2022-01-01")}, run=0))
		self.assertNotIn(
			"ifnull",
			kanivin.get_all("User", {"last_active": ("between", ("2022-01-01", "2023-01-01"))}, run=0),
		)
		self.assertIn("ifnull", kanivin.get_all("User", {"last_active": ("<", "2022-01-01")}, run=0))

	def test_ambiguous_linked_tables(self):
		from kanivin.desk.reportview import get

		if not kanivin.db.exists("DocType", "Related Todos"):
			kanivin.get_doc(
				{
					"doctype": "DocType",
					"custom": 1,
			...
			kanivin.db.delete("Related Todos")

		todo_one = kanivin.get_doc(
			{
				"doctype": "ToDo",
		...
		kanivin.form_dict.doctype = "Related Todos"
	...
	def test_select_star_expansion(self):
		count = kanivin.get_list("Language", ["SUM(1)", "COUNT(*)"], as_list=1, order_by=None)[0]
		self.assertEqual(count[0], kanivin.db.count("Language"))
		self.assertEqual(count[1], kanivin.db.count("Language"))


class TestReportView(KanivinTestCase):
	@run_only_if(db_type_is.MARIADB)  # TODO: postgres name casting is messed up
	def test_get_count(self):
		kanivin.local.request = kanivin._dict()
		kanivin.local.request.method = "GET"
		...
		count = execute_cmd("kanivin.desk.reportview.get_count")
		kanivin.local.form_dict = kanivin._dict(
			{
				"doctype": "DocType",
		...
		dict_filter_response = execute_cmd("kanivin.desk.reportview.get_count")
		...
		child_filter_response = execute_cmd("kanivin.desk.reportview.get_count")
		current_value = kanivin.db.sql(
	...
	def test_reportview_get(self):
		user = kanivin.get_doc("User", "test@example.com")
		add_child_table_to_blog_post()
		...
		kanivin.set_user(user.name)

		kanivin.local.request = kanivin._dict()
		kanivin.local.request.method = "POST"
		...
		response = execute_cmd("kanivin.desk.reportview.get")
	...
	def test_reportview_get_permlevel_system_users(self):
		with setup_patched_blog_post(), setup_test_user(set_user=True):
			...
			self.assertEqual(kanivin.get_list("Blog Post", "published"), [])
...
def create_event(subject="_Test Event", starts_on=None):
	...
	from kanivin.utils import get_datetime
...
def create_nested_doctype():
	if kanivin.db.exists("DocType", "Nested DocType"):
		return
...
def create_nested_doctype_records():
	...
	for r in records:
		d = kanivin.new_doc("Nested DocType")
		d.update(r)
