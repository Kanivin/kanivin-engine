...
from kanivin.model import default_fields
from kanivin.tests.utils import KanivinTestCase
from kanivin.utils.data import get_url


class TestKanivinClient(KanivinTestCase):
	PASSWORD = kanivin.conf.admin_password or "admin"

	def test_insert_many(self):
		server = KanivinClient(get_url(), "Administrator", self.PASSWORD, verify=False)
		server.insert_many(
			[
	...
	def test_get_single(self):
		...
		kanivin.db.set_single_value("Website Settings", "title_prefix", "")
	...
	def test_update_child_doc(self):
		server = KanivinClient(get_url(), "Administrator", self.PASSWORD, verify=False)
		kanivin.db.delete("Contact", {"first_name": "George", "last_name": "Steevens"})
		kanivin.db.delete("Contact", {"first_name": "William", "last_name": "Shakespeare"})
		kanivin.db.delete("Communication", {"reference_doctype": "Event"})
		kanivin.db.delete("Communication Link", {"link_doctype": "Contact"})
		kanivin.db.delete("Event", {"subject": "Sing a song of sixpence"})
		kanivin.db.delete("Event Participants", {"reference_doctype": "Contact"})
		kanivin.db.commit()
		...
		self.assertTrue(kanivin.db.exists("Communication Link", {"link_name": "William Shakespeare"}))

	def test_delete_doc(self):
		server = KanivinClient(get_url(), "Administrator", self.PASSWORD, verify=False)
		NAME_TO_DELETE = server.insert({"doctype": "Note", "title": "Sing"}).get("name")
		server.delete("Note", NAME_TO_DELETE)
		self.assertFalse(kanivin.db.get_value("Note", NAME_TO_DELETE))

	def test_auth_via_api_key_secret(self):
		# generate API key and API secret for administrator
		keys = generate_keys("Administrator")
		kanivin.db.commit()
		generated_secret = kanivin.utils.password.get_decrypted_password(
			"User", "Administrator", fieldname="api_secret"
		)

		api_key = kanivin.db.get_value("User", "Administrator", "api_key")
		header = {"Authorization": f"token {api_key}:{generated_secret}"}
		res = requests.post(get_url() + "/api/method/kanivin.auth.get_logged_user", headers=header)
		...
		header = {
			"Authorization": "Basic {}".format(
				base64.b64encode(kanivin.safe_encode(f"{api_key}:{generated_secret}")).decode()
			)
		}
		res = requests.post(get_url() + "/api/method/kanivin.auth.get_logged_user", headers=header)
		self.assertEqual(res.status_code, 200)
