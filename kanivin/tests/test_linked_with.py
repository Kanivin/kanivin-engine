...
from kanivin.desk.form import linked_with
from kanivin.tests.utils import KanivinTestCase


class TestLinkedWith(KanivinTestCase):
	def setUp(self):
	...
	def test_get_doctype_references_by_dlink_field(self):
		...
		parent_record = kanivin.get_doc({"doctype": "Parent DocType"}).insert()
	...
	def test_check_delete_integrity(self):
		...
		doc = kanivin.get_doc({"doctype": "Parent DocType"}).insert()
		doc.submit()
		doc.cancel()

		amendment = kanivin.copy_doc(doc)
		amendment.amended_from = doc.name
		amendment.docstatus = 0
		amendment.insert()
		amendment.submit()

		self.assertRaises(kanivin.LinkExistsError, doc.delete)
	...
	def test_reserved_keywords(self):
		...
		linked_doc = kanivin.new_doc(dt_name).insert().submit()

		second_doc = (
			kanivin.new_doc(dt_name, **{"from": linked_doc.doctype, "order": linked_doc.name})
			.insert()
			.submit()
		)

		with savepoint(kanivin.LinkExistsError):
