...
import kanivin
from kanivin.installer import parse_app_name
from kanivin.model.document import Document
from kanivin.tests.utils import KanivinTestCase, MockedRequestTestCase, change_settings
...
from kanivin.utils.change_log import (
	get_source_url,
	parse_github_url,
)
from kanivin.utils.data import (
	add_to_date,
	add_years,
	cast,
	cint,
...
from kanivin.utils.dateutils import get_dates_from_timegrain
from kanivin.utils.diff import _get_value_from_version, get_version_diff, version_query
from kanivin.utils.identicon import Identicon
from kanivin.utils.image import optimize_image, strip_exif_data
from kanivin.utils.make_random import can_make, get_random, how_many
from kanivin.utils.response import json_handler
from kanivin.utils.synchronization import LockTimeoutError, filelock
...
class TestFilters(KanivinTestCase):
...
class TestMoney(KanivinTestCase):
...
class TestDataManipulation(KanivinTestCase):
	def test_scrub_urls(self):
		html = """
			...
			<div class="more-info">
				...
				<img src="/assets/kanivin/test.jpg">
			</div>
			<div style="background-image: url('/assets/kanivin/bg.jpg')">
		...
		self.assertTrue(f'<img src="{url}/assets/kanivin/test.jpg">' in html)
		self.assertTrue(f"style=\"background-image: url('{url}/assets/kanivin/bg.jpg') !important\"" in html)
		self.assertTrue('<a href="mailto:test@example.com">email</a>' in html)


class TestFieldCasting(KanivinTestCase):
...
class TestMathUtils(KanivinTestCase):
	def test_floor(self):
...
class TestHTMLUtils(KanivinTestCase):
	def test_clean_email_html(self):
		from kanivin.utils.html_utils import clean_email_html
...
class TestValidationUtils(KanivinTestCase):
	def test_valid_url(self):
		...
		self.assertTrue(validate_url("http://kanivin.io", throw=True))
		...
		self.assertRaises(kanivin.ValidationError, validate_url, "kanivin", throw=True)

		# Scheme validation
		self.assertFalse(validate_url("https://google.com", valid_schemes="http"))
		self.assertTrue(validate_url("ftp://kanivin.cloud", valid_schemes=["https", "ftp"]))
		self.assertFalse(validate_url("bolo://kanivin.io", valid_schemes=("http", "https", "ftp", "ftps")))
		self.assertRaises(
			kanivin.ValidationError, validate_url, "gopher://kanivin.io", valid_schemes="https", throw=True
		)
	...
	def test_valid_email(self):
		...
		# Valid addresses
		self.assertTrue(validate_email_address("someone@kanivin.com"))
		self.assertTrue(validate_email_address("someone@kanivin.com, anyone@kanivin.io"))
		self.assertTrue(validate_email_address("test%201@kanivin.com"))

		# Invalid address
		self.assertFalse(validate_email_address("someone"))
		self.assertFalse(validate_email_address("someone@----.com"))
		self.assertFalse(validate_email_address("test 1@kanivin.com"))
		self.assertFalse(validate_email_address("test@example.com test2@example.com,undisclosed-recipient"))

		# Invalid with throw
		self.assertRaises(kanivin.InvalidEmailAddressError, validate_email_address, "someone.com", throw=True)

		self.assertEqual(validate_email_address("Some%20One@kanivin.com"), "Some%20One@kanivin.com")
		self.assertEqual(
			validate_email_address("erp+Job%20Applicant=JA00004@kanivin.com"),
			"erp+Job%20Applicant=JA00004@kanivin.com",
		)
	...
	def test_valid_phone(self):
		...
		self.assertRaises(
			kanivin.InvalidPhoneNumberError,
			validate_phone_number_with_country_code,
			"+420 1234567890",
			"field",
		)
	...
	def test_validate_name(self):
		...
		for name in invalid_names:
			self.assertRaises(kanivin.InvalidNameError, validate_name, name, True)


class TestImage(KanivinTestCase):
	def test_strip_exif_data(self):
		original_image = Image.open(kanivin.get_app_path("kanivin", "tests", "data", "exif_sample_image.jpg"))
		original_image_content = open(
			kanivin.get_app_path("kanivin", "tests", "data", "exif_sample_image.jpg"), mode="rb"
		).read()
	...
	def test_optimize_image(self):
		image_file_path = kanivin.get_app_path("kanivin", "tests", "data", "sample_image_for_optimization.jpg")
...
class TestPythonExpressions(KanivinTestCase):
	...
	def test_validation_for_bad_python_expression(self):
		...
		for expr in invalid_expressions:
			self.assertRaises(kanivin.ValidationError, validate_python_code, expr)


class TestDiffUtils(KanivinTestCase):
	@classmethod
	def setUpClass(cls):
		super().setUpClass()
		cls.doc = kanivin.get_doc(doctype="Client Script", dt="Client Script", name="test_client_script")
		cls.doc.insert()
		cls.doc.script = "2;"
...
class TestDateUtils(KanivinTestCase):
	def test_first_day_of_week(self):
		# Monday as start of the week
		with patch.object(kanivin.utils.data, "get_first_day_of_the_week", return_value="Monday"):
			self.assertEqual(
				kanivin.utils.get_first_day_of_week("2020-12-25"), kanivin.utils.getdate("2020-12-21")
			)
			self.assertEqual(
				kanivin.utils.get_first_day_of_week("2020-12-20"), kanivin.utils.getdate("2020-12-14")
			)

		# Sunday as start of the week
		self.assertEqual(kanivin.utils.get_first_day_of_week("2020-12-25"), kanivin.utils.getdate("2020-12-20"))
		self.assertEqual(kanivin.utils.get_first_day_of_week("2020-12-21"), kanivin.utils.getdate("2020-12-20"))

	def test_last_day_of_week(self):
		self.assertEqual(kanivin.utils.get_last_day_of_week("2020-12-24"), kanivin.utils.getdate("2020-12-26"))
		self.assertEqual(kanivin.utils.get_last_day_of_week("2020-12-28"), kanivin.utils.getdate("2021-01-02"))

	def test_is_last_day_of_the_month(self):
		self.assertEqual(kanivin.utils.is_last_day_of_the_month("2020-12-24"), False)
		self.assertEqual(kanivin.utils.is_last_day_of_the_month("2020-12-31"), True)
...
class TestResponse(KanivinTestCase):
	def test_json_handler(self):
		...
		GOOD_OBJECT = {
			...
			"doc": [
				kanivin.get_doc("System Settings"),
			],
			"iter": [
				{1, 2, 3},
				(1, 2, 3),
...
class TestTimeDeltaUtils(KanivinTestCase):
...
class TestXlsxUtils(KanivinTestCase):
	def test_unescape(self):
		from kanivin.utils.xlsxutils import handle_html
...
class TestLinkTitle(KanivinTestCase):
	def test_link_title_doctypes_in_boot_info(self):
		...
		from kanivin.boot import get_link_title_doctypes
	...
	def test_link_titles_on_getdoc(self):
		...
		link_titles = kanivin.local.response["_link_titles"]
...
class TestAppParser(MockedRequestTestCase):
	def test_app_name_parser(self):
		self.responses.add(
			"HEAD",
			"https://api.github.com/repos/kanivin/healthcare",
			status=200,
			json={},
		)
		bench_path = get_bench_path()
		kanivin_app = os.path.join(bench_path, "apps", "kanivin")
		self.assertEqual("kanivin", parse_app_name(kanivin_app))
		self.assertEqual("healthcare", parse_app_name("healthcare"))
		self.assertEqual("healthcare", parse_app_name("https://github.com/kanivin/healthcare.git"))
		self.assertEqual("healthcare", parse_app_name("git@github.com:kanivin/healthcare.git"))
		self.assertEqual("healthcare", parse_app_name("kanivin/healthcare@develop"))


class TestIntrospectionMagic(KanivinTestCase):
	...
	def test_get_newargs(self):
		...
		self.assertEqual(kanivin.get_newargs(f, safe_kwargs), safe_kwargs)

		unsafe_args = dict(safe_kwargs)
		unsafe_args.update({"ignore_permissions": True, "flags": {"ignore_mandatory": True}})
		self.assertEqual(kanivin.get_newargs(f, unsafe_args), safe_kwargs)
	...
	def test_strip_off_kwargs_when_not_supported(self):
		...
		self.assertEqual(kanivin.get_newargs(f, args), {"b": 1})

		# No args
		self.assertEqual(kanivin.get_newargs(lambda: None, args), {})


class TestMakeRandom(KanivinTestCase):
...
class TestLazyLoader(KanivinTestCase):
	def test_lazy_import_module(self):
		from kanivin.utils.lazy_loader import lazy_import

		with Capturing() as output:
			ls = lazy_import("kanivin.tests.data.load_sleep")
		self.assertEqual(output, [])

		with Capturing() as output:
			ls.time
		self.assertEqual(["Module `kanivin.tests.data.load_sleep` loaded"], output)


class TestIdenticon(KanivinTestCase):
	def test_get_gravatar(self):
		# developers@kanivin.io has a gravatar linked so str URL will be returned
		kanivin.flags.in_test = False
		gravatar_url = get_gravatar("developers@kanivin.io")
		kanivin.flags.in_test = True
		...
		gravatar_url = get_gravatar(f"developers{random_string(6)}@kanivin.io")
		self.assertIsInstance(gravatar_url, str)
...
class TestContainerUtils(KanivinTestCase):
...
class TestLocks(KanivinTestCase):
...
class TestMiscUtils(KanivinTestCase):
	...
	def test_get_all_sites(self):
		self.assertIn(kanivin.local.site, get_sites())

	def test_get_site_info(self):
		info = get_site_info()

		installed_apps = [app["app_name"] for app in info["installed_apps"]]
		self.assertIn("kanivin", installed_apps)
...
class TestTypingValidations(KanivinTestCase):
	...
	def test_validate_whitelisted_api(self):
		...
		for fn, args, kwargs in test_cases:
			with self.assertRaisesRegex(kanivin.KanivinTypeError, self.ERR_REGEX):
				fn(*args, **kwargs)

	def test_validate_whitelisted_doc_method(self):
		report = kanivin.get_last_doc("Report")
...
class TestTBSanitization(KanivinTestCase):
	def test_traceback_sanitzation(self):
		try:
			password = kanivin.generate_hash()
			args = {"password": "42", "pwd": password, "safe": "safe_value"}
			args = kanivin._dict({"password": "42", "pwd": password, "safe": "safe_value"})  # noqa: F841
			raise Exception
		except Exception:
			traceback = kanivin.get_traceback(with_context=True)
...
class TestRounding(KanivinTestCase):
	...
	def test_default_rounding(self):
		self.assertEqual(kanivin.get_system_settings("rounding_method"), "Banker's Rounding")
...
class TestArgumentTypingValidations(KanivinTestCase):
	def test_validate_argument_types(self):
		from kanivin.core.doctype.doctype.doctype import DocType
		from kanivin.utils.typing_validations import KanivinTypeError, validate_argument_types
		...
		self.assertEqual(test_simple_types(1, 2, "1"), (1, 2.0, True))
		with self.assertRaises(KanivinTypeError):
		...
		doctype = kanivin.get_last_doc("DocType")
...
class TestChangeLog(KanivinTestCase):
	def test_get_remote_url(self):
		self.assertIsInstance(get_source_url("kanivin"), str)

	def test_parse_github_url(self):
		# using erpnext as repo in order to be different from the owner
		owner, repo = parse_github_url("https://github.com/kanivin/erpnext.git")
		self.assertEqual(owner, "kanivin")
		self.assertEqual(repo, "erpnext")

		owner, repo = parse_github_url("https://github.com/kanivin/erpnext")
		self.assertEqual(owner, "kanivin")
		self.assertEqual(repo, "erpnext")

		owner, repo = parse_github_url("git@github.com:kanivin/erpnext.git")
		self.assertEqual(owner, "kanivin")
		self.assertEqual(repo, "erpnext")
...
class TestCrypto(KanivinTestCase):
