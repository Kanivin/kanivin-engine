...
import kanivin
from kanivin.app import make_form_dict
from kanivin.core.doctype.doctype.test_doctype import new_doctype
from kanivin.desk.doctype.note.note import Note
from kanivin.model.naming import make_autoname, parse_naming_series, revert_series_if_last
from kanivin.tests.utils import KanivinTestCase, timeout
from kanivin.utils import cint, now_datetime, set_request
...
class TestDocument(KanivinTestCase):
	def test_get_return_empty_list_for_table_field_if_none(self):
		d = kanivin.get_doc({"doctype": "User"})
	...
	def test_load_single(self):
		d = kanivin.get_doc("Website Settings", "Website Settings")
	...
	def test_insert(self):
		...
		self.assertEqual(kanivin.db.get_value("Event", d.name, "subject"), "test-doc-test-event 1")
	...
	def test_website_route_default(self):
		...
		doc = kanivin.get_doc({"doctype": parent, "child_table": [{"some_fieldname": "xasd"}]}).insert()
	...
	def test_insert_with_child(self):
		...
		d.insert()
		self.assertTrue(d.name.startswith("EV"))
		self.assertEqual(kanivin.db.get_value("Event", d.name, "subject"), "test-doc-test-event 2")
	...
	def test_update(self):
		...
		self.assertEqual(kanivin.db.get_value(d.doctype, d.name, "subject"), "subject changed")
	...
	def test_mandatory(self):
		...
		kanivin.delete_doc_if_exists("User", "test_mandatory@example.com", 1)
		...
		self.assertRaises(kanivin.MandatoryError, d.insert)

		d.set("first_name", "Test Mandatory")
		d.insert()
		self.assertEqual(kanivin.db.get_value("User", d.name), d.name)

	def test_text_editor_field(self):
		try:
			kanivin.get_doc(doctype="Activity Log", subject="test", message='<img src="test.png" />').insert()
		except kanivin.MandatoryError:
			self.fail("Text Editor false positive mandatory error")

	def test_conflict_validation(self):
		d1 = self.test_insert()
		d2 = kanivin.get_doc(d1.doctype, d1.name)
		d1.save()
		self.assertRaises(kanivin.TimestampMismatchError, d2.save)

	def test_conflict_validation_single(self):
		d1 = kanivin.get_doc("Website Settings", "Website Settings")
		d1.home_page = "test-web-page-1"

		d2 = kanivin.get_doc("Website Settings", "Website Settings")
	...
	def test_permission(self):
		...
		self.assertRaises(kanivin.PermissionError, self.test_insert)
		kanivin.set_user("Administrator")

	def test_permission_single(self):
		kanivin.set_user("Guest")
		d = kanivin.get_doc("Website Settings", "Website Settings")
		self.assertRaises(kanivin.PermissionError, d.save)
		kanivin.set_user("Administrator")

	def test_link_validation(self):
		kanivin.delete_doc_if_exists("User", "test_link_validation@example.com", 1)
		...
		self.assertRaises(kanivin.LinkValidationError, d.insert)
	...
	def test_validate(self):
		...
		self.assertRaises(kanivin.ValidationError, d.validate)
		self.assertRaises(kanivin.ValidationError, d.run_method, "validate")
		self.assertRaises(kanivin.ValidationError, d.save)
	...
	def test_new_doc_with_fields(self):
		user = kanivin.new_doc("User", first_name="wizard")
		self.assertEqual(user.first_name, "wizard")

	def test_update_after_submit(self):
		d = self.test_insert()
		d.starts_on = "2014-09-09"
		self.assertRaises(kanivin.UpdateAfterSubmitError, d.validate_update_after_submit)
	...
	def test_varchar_length(self):
		...
		self.assertRaises(kanivin.CharacterLengthExceededError, d.save)
	...
	def test_naming_series(self):
		...
		for series in data:
			...
			old_current = kanivin.db.get_value("Series", prefix, "current", order_by="name")

			revert_series_if_last(series, name)
			new_current = cint(kanivin.db.get_value("Series", prefix, "current", order_by="name"))

			self.assertEqual(cint(old_current) - 1, new_current)

	def test_non_negative_check(self):
		kanivin.delete_doc_if_exists("Currency", "Kanivin Coin", 1)

		d = kanivin.get_doc(
			{"doctype": "Currency", "currency_name": "Kanivin Coin", "smallest_currency_fraction_value": -1}
		)

		self.assertRaises(kanivin.NonNegativeError, d.insert)

		d.set("smallest_currency_fraction_value", 1)
		d.insert()
		self.assertEqual(kanivin.db.get_value("Currency", d.name), d.name)
	...
	def test_get_formatted(self):
		...
		kanivin.delete_doc_if_exists("Currency", "INR", 1)
		...
		d = kanivin.get_doc({"doctype": "Test Formatted", "currency": 100000})
	...
	def test_limit_for_get(self):
		doc = kanivin.get_doc("DocType", "DocType")
	...
	def test_virtual_fields(self):
		...
		kanivin.db.delete("Custom Field", {"dt": "Note", "fieldname": "age"})
		note = kanivin.new_doc("Note")
		note.content = "some content"
		note.title = kanivin.generate_hash(length=20)
		note.insert()

		def patch_note(class_=None):
			return patch("kanivin.controllers", new={kanivin.local.site: {"Note": class_ or CustomTestNote}})

		@contextmanager
		def customize_note(with_options=False):
			options = (
				"kanivin.utils.now_datetime() - kanivin.utils.get_datetime(doc.creation)"
				if with_options
				else ""
			)
			custom_field = kanivin.get_doc(
				{
	...
	def test_error_on_saving_new_doc_with_name(self):
		...
		doc = kanivin.get_doc(
			{
				"doctype": "ToDo",
				"description": "this should raise kanivin.DoesNotExistError",
				"name": "lets-trick-doc-save",
			}
		)

		self.assertRaises(kanivin.DoesNotExistError, doc.save)
	...
	def test_validate_from_to_dates(self):
		...
		self.assertRaises(
			kanivin.exceptions.InvalidDates, doc.validate_from_to_dates, "start_date", "end_date"
		)
	...
	def test_db_set_singles(self):
		...
		c.db_set(key, val)
		changed_val = kanivin.db.get_single_value(c.doctype, key)
		self.assertEqual(val, changed_val)


class TestDocumentWebView(KanivinTestCase):
	def get(self, path, user="Guest"):
		kanivin.set_user(user)
		set_request(method="GET", path=path)
		make_form_dict(kanivin.local.request)
	...
	def test_web_view_link_authentication(self):
		...
		# expire the key
		document_key_doc = kanivin.get_doc("Document Share Key", {"key": document_key})
	...
	def test_bulk_inserts(self):
		from kanivin.model.document import bulk_insert

		doctype = "Role Profile"
		child_field = "roles"
		child_doctype = kanivin.get_meta(doctype).get_field(child_field).options
		...
		def doc_generator():
			for _ in range(21):
				...
				doc.role_profile = kanivin.generate_hash()
		...
		all_docs = set(kanivin.get_all(doctype, pluck="name"))
		all_child_docs = set(
			kanivin.get_all(
