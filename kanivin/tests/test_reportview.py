...
import kanivin
from kanivin.desk.reportview import export_query, extract_fieldnames
...
class TestReportview(KanivinTestCase):
	def test_csv(self):
		...
		for delimiter in (",", ";", "\t", "|"):
			kanivin.local.form_dict.csv_delimiter = delimiter
			for quoting in (QUOTE_ALL, QUOTE_MINIMAL, QUOTE_NONE, QUOTE_NONNUMERIC):
				kanivin.local.form_dict.csv_quoting = quoting
