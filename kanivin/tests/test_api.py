...
from kanivin.utils import cint, get_site_url, get_test_client, get_url

try:
	_site = kanivin.local.site
except Exception:
	_site = None
...
class ThreadWithReturnValue(Thread):
	...
	def run(self):
		if self._target is not None:
			with patch("kanivin.app.get_site_name", return_value=self.site):
				header_patch = patch("kanivin.get_request_header", new=patch_request_header)
...
class KanivinAPITestCase(KanivinTestCase):
...
class TestResourceAPI(KanivinAPITestCase):
	DOCTYPE = "ToDo"
	...
	def setUpClass(cls):
		super().setUpClass()
		for _ in range(20):
			doc = kanivin.get_doc({"doctype": "ToDo", "description": kanivin.mock("paragraph")}).insert()
	...
	def tearDownClass(cls):
		for name in cls.GENERATED_DOCUMENTS:
			kanivin.delete_doc_if_exists(cls.DOCTYPE, name)
		kanivin.db.commit()
	...
	def test_get_list_dict(self):
		...
		json = kanivin._dict(response.json)
		self.assertEqual(response.status_code, 200)
	...
	def test_create_document(self):
		data = {"description": kanivin.mock("paragraph"), "sid": self.sid}
	...
	def test_update_document(self):
		generated_desc = kanivin.mock("paragraph")
	...
	def test_run_doc_method(self):
		...
		if response.status_code == 403:
			...
			self.assertEqual(
				response.json.get("exception"), "kanivin.exceptions.PermissionError: Not permitted"
			)
...
class TestMethodAPI(KanivinAPITestCase):
	def test_ping(self):
	...
	def test_get_user_info(self):
		# test 3: test for /api/method/kanivin.realtime.get_user_info
		response = self.get(self.method("kanivin.realtime.get_user_info"))
		self.assertEqual(response.status_code, 200)
		self.assertIsInstance(response.json, dict)
	...
	def test_auth_cycle(self):
		...
		response = self.get(self.method("kanivin.auth.get_logged_user"))
	...
	def test_logs(self):
		method = "kanivin.tests.test_api.test"

		def get_message(resp, msg_type):
			return kanivin.parse_json(kanivin.parse_json(kanivin.parse_json(resp.json)[msg_type])[0])
	...
	def test_array_response(self):
		method = "kanivin.tests.test_api.test_array"
...
class TestReadOnlyMode(KanivinAPITestCase):
	...
	def test_blocked_writes(self):
		with suppress_stdout():
			response = self.post(
				self.resource("ToDo"), {"description": kanivin.mock("paragraph"), "sid": self.sid}
			)
		self.assertEqual(response.status_code, 503)
		self.assertEqual(response.json["exc_type"], "InReadOnlyMode")


class TestWSGIApp(KanivinAPITestCase):
	def test_request_hooks(self):
		self.addCleanup(lambda: _test_REQ_HOOK.clear())

		with patch_hooks(
			{
				"before_request": ["kanivin.tests.test_api.before_request"],
				"after_request": ["kanivin.tests.test_api.after_request"],
			}
		):
...
class TestResponse(KanivinAPITestCase):
	def test_generate_pdf(self):
		response = self.get(
			"/api/method/kanivin.utils.print_format.download_pdf",
	...
	def test_binary_and_csv_response(self):
		def download_template(file_type):
			...
			return self.post(
				"/api/method/kanivin.core.doctype.data_import.data_import.download_template",
				{
					"sid": self.sid,
					"doctype": "User",
		...
		from kanivin.desk.utils import provide_binary_file
	...
	def test_download_private_file_with_unique_url(self):
		test_content = kanivin.generate_hash()
...
def generate_admin_keys():
	from kanivin.core.doctype.user.user import generate_keys
...
def test(*, fail=False, handled=True, message="Failed"):
	if fail:
		if handled:
			kanivin.throw(message)
		else:
			1 / 0
	else:
		kanivin.msgprint(message)
