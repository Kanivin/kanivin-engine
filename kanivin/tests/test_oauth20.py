...
import kanivin
from kanivin.integrations.oauth2 import encode_params
from kanivin.test_runner import make_test_records
from kanivin.tests.test_api import get_test_client, make_request, suppress_stdout
from kanivin.tests.utils import KanivinTestCase

if TYPE_CHECKING:
	from kanivin.integrations.doctype.social_login_key.social_login_key import SocialLoginKey


class KanivinRequestTestCase(KanivinTestCase):
	@property
	def sid(self) -> str:
		if not getattr(self, "_sid", None):
			...
			kanivin.local.login_manager.login_as("test@example.com")
			self._sid = kanivin.session.sid
...
class TestOAuth20(KanivinRequestTestCase):
	...
	def setUpClass(cls):
		...
		# Set Kanivin server URL reqired for id_token generation
		kanivin_login_key: "SocialLoginKey" = kanivin.new_doc("Social Login Key")
		kanivin_login_key.get_social_login_provider("Kanivin", initialize=True)
		kanivin_login_key.base_url = kanivin.utils.get_url()
		kanivin_login_key.enable_social_login = 0
		kanivin_login_key.insert(ignore_if_duplicate=True)
		kanivin.db.commit()

	def setUp(self):
		self.TEST_CLIENT = get_test_client()
		self.oauth_client = kanivin.new_doc("OAuth Client")
	...
	def test_login_using_authorization_code(self):
		...
		resp = self.get(
			"/api/method/kanivin.integrations.oauth2.authorize",
			{
		...
		token_response = self.post(
			"/api/method/kanivin.integrations.oauth2.get_token",
	...
	def test_revoke_token(self):
		client = kanivin.get_doc("OAuth Client", self.client_id)
		...
		revoke_token_response = self.post(
			"/api/method/kanivin.integrations.oauth2.revoke_token",
			headers=self.form_header,
	...
	def test_login_using_implicit_token(self):
		oauth_client = kanivin.get_doc("OAuth Client", self.client_id)
		...
		try:
			session.get(
				get_full_url("/api/method/kanivin.integrations.oauth2.authorize"),
...
def check_valid_openid_response(access_token=None, client: "KanivinRequestTestCase" = None):
	"""Return True for valid response."""
	# Use token in header
	headers = {}
	URL = "/api/method/kanivin.integrations.oauth2.openid_profile"
...
def get_full_url(endpoint):
	...
	return urljoin(kanivin.utils.get_url(), endpoint)
