...
from kanivin.core.page.permission_manager.permission_manager import get_roles_and_doctypes
from kanivin.desk.doctype.desktop_icon.desktop_icon import (
	add_user_icon,
...
class TestDomainification(KanivinTestCase):
	def setUp(self):
		# create test domain
	...
	def tearDown(self):
		kanivin.db.delete("Role", {"name": "_Test Role"})
		kanivin.db.delete("Has Role", {"role": "_Test Role"})
		kanivin.db.delete("Domain", {"name": ("in", ("_Test Domain 1", "_Test Domain 2"))})
		kanivin.delete_doc("DocType", "Test Domainification")
		self.remove_from_active_domains(remove_all=True)
	...
	def add_active_domain(self, domain):
		...
		domain_settings = kanivin.get_doc("Domain Settings", "Domain Settings")
	...
	def new_domain(self, domain):
		# create new domain
		kanivin.get_doc({"doctype": "Domain", "domain": domain}).insert()
	...
	def test_active_domains(self):
		self.assertTrue("_Test Domain 1" in kanivin.get_active_domains())
		self.assertFalse("_Test Domain 2" in kanivin.get_active_domains())

		self.add_active_domain("_Test Domain 2")
		self.assertTrue("_Test Domain 2" in kanivin.get_active_domains())

		self.remove_from_active_domains("_Test Domain 1")
		self.assertTrue("_Test Domain 1" not in kanivin.get_active_domains())
	...
	def test_doctype_and_role_domainification(self):
		...
		test_role = kanivin.get_doc({"doctype": "Role", "role_name": "_Test Role"}).insert()
	...
	def test_module_def_for_domainification(self):
		...
		test_module_def = kanivin.get_doc("Module Def", "Contacts")
