...
import kanivin
import kanivin.commands.scheduler
import kanivin.commands.site
import kanivin.commands.utils
import kanivin.recorder
from kanivin.installer import add_to_installed_apps, remove_app
...
from kanivin.utils import add_to_date, get_bench_path, get_bench_relative_path, now
from kanivin.utils.backups import BackupGenerator, fetch_latest_backups
from kanivin.utils.jinja_globals import bundled_asset
from kanivin.utils.scheduler import enable_scheduler, is_scheduler_inactive

_result: Result | None = None
TEST_SITE = "commands-site-O4PN2QK.test"  # added random string tag to avoid collisions
CLI_CONTEXT = kanivin._dict(sites=[TEST_SITE])
...
def missing_in_backup(doctypes: list, file: os.PathLike) -> list:
	...
	"""
	predicate = 'COPY public."tab{}"' if kanivin.conf.db_type == "postgres" else "CREATE TABLE `tab{}`"
...
@contextmanager
def maintain_locals():
	pre_site = kanivin.local.site
	pre_flags = kanivin.local.flags.copy()
	pre_db = kanivin.local.db

	try:
		yield
	finally:
		post_site = getattr(kanivin.local, "site", None)
		if not post_site or post_site != pre_site:
			kanivin.init(site=pre_site)
			kanivin.local.db = pre_db
			kanivin.local.flags.update(pre_flags)
...
def cli(cmd: Command, args: list | None = None):
	with maintain_locals():
		...
		patch_ctx = patch("kanivin.commands.pass_context", pass_test_context)
...
class BaseTestCommands(KanivinTestCase):
	@classmethod
	...
	def setup_test_site(cls):
		cmd_config = {
			"test_site": TEST_SITE,
			"admin_password": kanivin.conf.admin_password,
			"db_type": kanivin.conf.db_type,
			"db_root_username": kanivin.conf.root_login,
			"db_root_password": kanivin.conf.root_password,
		}
...
class TestCommands(BaseTestCommands):
	def test_execute(self):
		...
		self.execute("bench --site {site} execute kanivin.db.get_database_size")
		self.assertEqual(self.returncode, 0)
		self.assertIsInstance(float(self.stdout), float)

		# test 2: execute a command expecting an errored output as local won't exist
		self.execute("bench --site {site} execute kanivin.local.site")
		...
		self.execute("""bench --site {site} execute kanivin.bold --kwargs '{{"text": "DocType"}}'""")
		self.assertEqual(self.returncode, 0)
		self.assertEqual(self.stdout[1:-1], kanivin.bold(text="DocType"))
	...
	def test_restore(self):
		...
		global_config = {
			"admin_password": kanivin.conf.admin_password,
			"root_login": kanivin.conf.root_login,
			"root_password": kanivin.conf.root_password,
			"db_type": kanivin.conf.db_type,
		}
		...
		with self.switch_site(TEST_SITE):
			public_file = kanivin.new_doc(
				"File", file_name=f"test_{kanivin.generate_hash()}", content=kanivin.generate_hash()
			).insert()
			private_file = kanivin.new_doc(
				"File", file_name=f"test_{kanivin.generate_hash()}", content=kanivin.generate_hash()
			).insert()
		...
		self.execute(
			"bench --site {test_site} execute kanivin.utils.backups.fetch_latest_backups",
			site_data,
		)
		...
		with self.switch_site(TEST_SITE):
			...
			kanivin.db.sql_ddl("DROP TABLE IF EXISTS `tabToDo`")
		...
		with self.switch_site(TEST_SITE):
			self.assertTrue(kanivin.db.table_exists("ToDo", cached=False))
		...
		self.execute(
			"bench --site {test_site} execute" " kanivin.utils.backups.fetch_latest_backups --kwargs {kw}",
			site_data,
		)
	...
	def test_partial_restore(self):
			...
			kanivin.get_doc(
				{
					...
					"description": kanivin.mock("paragraph"),
				}
			).insert()
		kanivin.db.commit()
		todo_count = kanivin.db.count("ToDo")
		...
		self.assertEqual(self.returncode, 0)
		self.assertEqual(kanivin.db.count("ToDo"), todo_count)
	...
	def test_recorder(self):
		...
		kanivin.local.cache = {}
		self.assertEqual(kanivin.recorder.status(), True)

		self.execute("bench --site {site} stop-recording")
		kanivin.local.cache = {}
		self.assertEqual(kanivin.recorder.status(), False)
	...
	def test_list_apps(self):
		...
		doctype = kanivin.get_single("Installed Applications").installed_applications
		if doctype:
			installed_apps = {x.app_name for x in doctype}
		else:
			installed_apps = set(kanivin.get_installed_apps())
	...
	def test_kanivin_site_env(self):
		os.putenv("FRAPPE_SITE", kanivin.local.site)
		self.execute("bench execute kanivin.ping")
		self.assertEqual(self.returncode, 0)
	...
	def test_set_password(self):
		from kanivin.utils.password import check_password
		...
		original_password = kanivin.conf.admin_password or "admin"
	...
	@skipIf(
		not (kanivin.conf.root_password and kanivin.conf.admin_password and kanivin.conf.db_type == "mariadb"),
	...
	def test_bench_drop_site_should_archive_site(self):
		...
		self.execute(
			...
			f"--admin-password {kanivin.conf.admin_password} "
			f"--mariadb-root-password {kanivin.conf.root_password} "
			f"--db-type {kanivin.conf.db_type} "
		)
		self.assertEqual(self.returncode, 0)

		self.execute(f"bench drop-site {site} --force --root-password {kanivin.conf.root_password}")
		self.assertEqual(self.returncode, 0)
	...
	def test_force_install_app(self):
		...
		# set admin password in site_config as when kanivin force installs, we don't have the conf
		self.execute(f"bench --site {TEST_SITE} set-config admin_password {kanivin.conf.admin_password}")

		# try installing the kanivin_docs app again on test site
		self.execute(f"bench --site {TEST_SITE} install-app {app_name}")
		self.assertIn(f"{app_name} already installed", self.stdout)
		self.assertEqual(self.returncode, 0)

		# force install kanivin_docs app on the test site
...
class TestBackups(BaseTestCommands):
	...
	site_backup_path = kanivin.utils.get_site_path("private", "backups")
	...
	@skipIf(
		not (kanivin.conf.db_type == "mariadb"),
		"Only for MariaDB",
	)
	...
	def test_backup_fails_with_exit_code(self):
		...
		odb = BackupGenerator(
			kanivin.conf.db_name,
			kanivin.conf.db_name,
			kanivin.conf.db_password + "INCORRECT PASSWORD",
			db_socket=kanivin.conf.db_socket,
			db_host=kanivin.conf.db_host,
			db_port=kanivin.conf.db_port,
			db_type=kanivin.conf.db_type,
		)
	...
	def test_clear_log_table(self):
		d = kanivin.get_doc(doctype="Error Log", title="Something").insert()
		d.db_set("modified", "2010-01-01", update_modified=False)
		kanivin.db.commit()

		tables_before = kanivin.db.get_tables(cached=False)

		self.execute("bench --site {site} clear-log-table --days=30 --doctype='Error Log'")
		self.assertEqual(self.returncode, 0)
		kanivin.db.commit()

		self.assertFalse(kanivin.db.exists("Error Log", d.name))
		tables_after = kanivin.db.get_tables(cached=False)
	...
	def test_backup_only_specific_doctypes(self):
		"""Take a backup with (include) backup options set in the site config `kanivin.conf.backup.includes`"""
		self.execute(
	...
	def test_backup_excluding_specific_doctypes(self):
		"""Take a backup with (exclude) backup options set (`kanivin.conf.backup.excludes`, `--exclude`)"""
		# test 1: take a backup with kanivin.conf.backup.excludes
		self.execute(
	...
	def test_selective_backup_priority_resolution(self):
		"""Take a backup with conflicting backup options set (`kanivin.conf.excludes`, `--include`)"""
		self.execute(
	...
	def test_dont_backup_conf(self):
		"""Take a backup ignoring kanivin.conf.backup settings (with --ignore-backup-conf option)"""
...
class TestRemoveApp(KanivinTestCase):
	def test_delete_modules(self):
		from kanivin.installer import (
			_delete_doctypes,
			_delete_modules,
			_get_module_linked_doctype_field_map,
		)

		test_module = kanivin.new_doc("Module Def")

		test_module.update({"module_name": "RemoveThis", "app_name": "kanivin"})
		test_module.save()

		module_def_linked_doctype = kanivin.get_doc(
			{
				"doctype": "DocType",
		...
		self.assertFalse(kanivin.db.exists("Module Def", test_module.module_name))
		self.assertFalse(kanivin.db.exists("DocType", module_def_linked_doctype.name))

	def test_dry_run(self):
		"""Check if dry run in not destructive."""

		# nothing to assert, if this fails rest of the test suite will crumble.
		remove_app("kanivin", dry_run=True, yes=True, no_backup=True)


class TestSiteMigration(BaseTestCommands):
	def test_migrate_cli(self):
		with cli(kanivin.commands.site.migrate) as result:
...
class TestAddNewUser(BaseTestCommands):
	def test_create_user(self):
		...
		self.assertEqual(self.returncode, 0)
		user = kanivin.get_doc("User", "test@gmail.com")
...
class TestBenchBuild(BaseTestCommands):
	def test_build_assets_size_check(self):
		with cli(kanivin.commands.utils.build, "--force --production") as result:
			self.assertEqual(result.exit_code, 0)
			self.assertEqual(result.exception, None)
		...
		for chunk in default_bundle:
			abs_path = Path.cwd() / kanivin.local.sites_path / bundled_asset(chunk)[1:]
...
class TestDBUtils(BaseTestCommands):
	...
	def test_db_add_index(self):
		...
		kanivin.db.rollback()
		index_name = kanivin.db.get_index_name((field,))
		self.assertTrue(kanivin.db.has_index("tabUser", index_name))
		meta = kanivin.get_meta("User", cached=False)
...
class TestSchedulerUtils(BaseTestCommands):
	...
	def test_ready_for_migrate(self):
		with cli(kanivin.commands.scheduler.ready_for_migration) as result:
			self.assertEqual(result.exit_code, 0)


class TestCommandUtils(KanivinTestCase):
	def test_bench_helper(self):
		from kanivin.utils.bench_helper import get_app_groups

		app_groups = get_app_groups()
		self.assertIn("kanivin", app_groups)
		self.assertIsInstance(app_groups["kanivin"], click.Group)
...
class TestSchedulerCLI(BaseTestCommands):
	...
	def test_scheduler_status(self):
		...
		parsed_output = kanivin.parse_json(self.stdout)
		self.assertEqual(self.returncode, 0)
...
class TestGunicornWorker(KanivinTestCase):
	port = 8005
