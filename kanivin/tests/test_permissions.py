...
from kanivin.core.doctype.user_permission.user_permission import clear_user_permissions
from kanivin.core.page.permission_manager.permission_manager import add, remove, reset, update
from kanivin.desk.form.load import getdoc
from kanivin.permissions import (
	ALL_USER_ROLE,
	AUTOMATIC_ROLES,
	GUEST_ROLE,
	SYSTEM_USER_ROLE,
...
from kanivin.test_runner import make_test_records_for_doctype
...
class TestPermissions(KanivinTestCase):
	@classmethod
	def setUpClass(cls):
		super().setUpClass()
		kanivin.clear_cache(doctype="Blog Post")
		user = kanivin.get_doc("User", "test1@example.com")
		user.add_roles("Website Manager")
		user.add_roles("System Manager")

		user = kanivin.get_doc("User", "test2@example.com")
		user.add_roles("Blogger")

		user = kanivin.get_doc("User", "test3@example.com")
		user.add_roles("Sales User")

		user = kanivin.get_doc("User", "testperm@example.com")
		user.add_roles("Website Manager")
	...
	def setUp(self):
		...
		kanivin.db.delete("User Permission")

		kanivin.set_user("test1@example.com")

	def tearDown(self):
		kanivin.set_user("Administrator")
		kanivin.db.set_value("Blogger", "_Test Blogger 1", "user", None)
	...
	def set_strict_user_permissions(ignore):
		ss = kanivin.get_doc("System Settings")
		ss.apply_strict_user_permissions = ignore
		ss.flags.ignore_mandatory = 1
		ss.save()

	def test_basic_permission(self):
		post = kanivin.get_doc("Blog Post", "-test-blog-post")
	...
	def test_select_permission(self):
		...
		kanivin.clear_cache(doctype="Blog Post")
		kanivin.set_user("test3@example.com")
		...
		self.assertRaises(kanivin.PermissionError, post.save)

		permitted_record = kanivin.get_list("Blog Post", fields="*", limit=1)[0]
		full_record = kanivin.get_all("Blog Post", fields="*", limit=1)[0]
	...
	def test_user_permissions_in_doc(self):
		...
		post1 = kanivin.get_doc("Blog Post", "-test-blog-post-1")
	...
	def test_user_permissions_in_report(self):
		...
		kanivin.set_user("test2@example.com")
		names = [d.name for d in kanivin.get_list("Blog Post", fields=["name", "blog_category"])]

		self.assertTrue("-test-blog-post-1" in names)
		self.assertFalse("-test-blog-post" in names)

	def test_default_values(self):
		doc = kanivin.new_doc("Blog Post")
	...
	def test_user_link_match_doc(self):
		blogger = kanivin.get_doc("Blogger", "_Test Blogger 1")
		blogger.user = "test2@example.com"
		blogger.save()

		kanivin.set_user("test2@example.com")

		post = kanivin.get_doc("Blog Post", "-test-blog-post-2")
		self.assertTrue(post.has_permission("read"))
	...
	def test_user_link_match_report(self):
		...
		names = [d.name for d in kanivin.get_list("Blog Post", fields=["name", "owner"])]
	...
	def test_not_allowed_to_set_user_permissions(self):
		...
		self.assertRaises(
			kanivin.PermissionError, add_user_permission, "Blog Post", "-test-blog-post", "test2@example.com"
		)
	...
	def test_read_if_explicit_user_permissions_are_set(self):
		...
		doc = kanivin.get_doc("Blog Post", "-test-blog-post")
		self.assertTrue(doc.has_permission("read"))

		# and not this one
		doc = kanivin.get_doc("Blog Post", "-test-blog-post-1")
	...
	def test_set_standard_fields_manually(self):
		...
		fake_owner = kanivin.db.get_value("User", {"name": ("!=", kanivin.session.user)})
	...
	def test_dont_change_standard_constants(self):
		...
		user = kanivin.get_doc("User", kanivin.session.user)
		user.creation = now_datetime()
		self.assertRaises(kanivin.CannotChangeConstantError, user.save)
	...
	def test_set_only_once(self):
		blog_post = kanivin.get_meta("Blog Post")
		doc = kanivin.get_doc("Blog Post", "-test-blog-post-1")
		doc.db_set("title", "Old")
		blog_post.get_field("title").set_only_once = 1
		doc.title = "New"
		self.assertRaises(kanivin.CannotChangeConstantError, doc.save)
	...
	def test_set_only_once_child_table_rows(self):
		...
		doc = kanivin.get_doc("DocType", "Blog Post")
	...
	def test_user_permission_doctypes(self):
		...
		doc = kanivin.get_doc("Blog Post", "-test-blog-post-2")
		self.assertTrue(doc.has_permission("read"))
	...
	def test_insert_if_owner_with_user_permissions(self):
		...
		kanivin.delete_doc("Blog Post", "-test-blog-post-title")
		...
		self.assertRaises(kanivin.PermissionError, doc.insert)
		...
		kanivin.set_user("test2@example.com")
		doc = kanivin.get_doc(doc.doctype, doc.name)
		self.assertTrue(doc.has_permission("read"))
	...
	def test_strict_user_permissions(self):
		...
		kanivin.set_user("Administrator")
		kanivin.db.delete("Contact")
		kanivin.db.delete("Contact Email")
		kanivin.db.delete("Contact Phone")
		...
		allowed_contact = kanivin.get_doc("Contact", "_Test Contact For _Test Customer")
		other_contact = kanivin.get_doc("Contact", "_Test Contact For _Test Supplier")

		kanivin.set_user("test3@example.com")
		self.assertTrue(allowed_contact.has_permission("read"))
		self.assertTrue(other_contact.has_permission("read"))
		self.assertEqual(len(kanivin.get_list("Contact")), 2)
		...
		self.assertTrue(len(kanivin.get_list("Contact")), 1)
	...
	def test_contextual_user_permission(self):
		...
		posts = kanivin.get_all("Blog Post", fields=["name", "blogger"])
		...
		posts = kanivin.get_list("Blog Post", fields=["name", "blogger"])
	...
	def test_if_owner_permission_overrides_properly(self):
		...
		kanivin.set_user("test2@example.com")
		self.assertEqual(kanivin.get_list("Blog Post"), [])
	...
	def test_if_owner_permission_on_getdoc(self):
		...
		doclist = [d.name for d in kanivin.response.docs]
		self.assertTrue(doc.name in doclist)

		kanivin.set_user("test2@example.com")
		self.assertRaises(kanivin.PermissionError, getdoc, "Blog Post", doc.name)
	...
	def test_if_owner_permission_on_get_list(self):
		...
		kanivin.set_user("test2@example.com")
		self.assertIn(doc.name, kanivin.get_list("Blog Post", pluck="name"))
		...
		kanivin.set_user("test2@example.com")
		self.assertNotIn(doc.name, kanivin.get_list("Blog Post", pluck="name"))
	...
	def test_if_owner_permission_on_delete(self):
		...
		self.assertRaises(kanivin.PermissionError, kanivin.delete_doc, "Blog Post", doc.name)

		kanivin.set_user("test2@example.com")
		kanivin.delete_doc("Blog Post", "-test-blog-post-title-new-1")
	...
	def test_clear_user_permissions(self):
		...
		# reset the user
		kanivin.set_user(current_user)

	def test_child_permissions(self):
		kanivin.set_user("test3@example.com")
		self.assertIsInstance(kanivin.get_list("DefaultValue", parent_doctype="User", limit=1), list)

		# kanivin.get_list
		self.assertRaises(kanivin.PermissionError, kanivin.get_list, "DefaultValue")
		self.assertRaises(kanivin.PermissionError, kanivin.get_list, "DefaultValue", parent_doctype="ToDo")
		self.assertRaises(
			kanivin.PermissionError, kanivin.get_list, "DefaultValue", parent_doctype="DefaultValue"
		)
		...
		self.assertRaises(kanivin.PermissionError, doc.check_permission)
		...
		user = kanivin.get_doc("User", "Administrator")
		doc = user.append("defaults")
		self.assertRaises(kanivin.PermissionError, doc.check_permission)
	...
	def test_select_user(self):
		...
		for gender in ("Male", "Female"):
			if kanivin.db.exists("Gender", gender):
				continue

			kanivin.get_doc({"doctype": "Gender", "gender": gender}).insert()

		# asssign gender to test users
		kanivin.db.set_value("User", "test1@example.com", "gender", "Male")
		kanivin.db.set_value("User", "test2@example.com", "gender", "Female")
		kanivin.db.set_value("User", "test3@example.com", "gender", "Female")
		...
		kanivin.set_user("test3@example.com")
		users = kanivin.get_list("User", pluck="name")
	...
	def test_automatic_permissions(self):
		def assertHasRole(*roles: str | tuple[str, ...]):
			for role in roles:
				self.assertIn(role, kanivin.get_roles())
	...
	def test_overrides_work_as_expected(self):
		...
		custom_role = kanivin.new_doc("Role", role_name=kanivin.generate_hash()).insert().name
		with self.set_user("Administrator"):
