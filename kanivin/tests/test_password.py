...
from kanivin.utils.password import check_password, decrypt, encrypt, passlibctx, update_password


class TestPassword(KanivinTestCase):
	def setUp(self):
		kanivin.delete_doc("Email Account", "Test Email Account Password")
		kanivin.delete_doc("Email Account", "Test Email Account Password-new")
	...
	def make_email_account(self, name="Test Email Account Password"):
		if not kanivin.db.exists("Email Account", name):
			...
			return kanivin.get_doc("Email Account", name)
	...
	def test_hashed_password(self, user="test@example.com"):
		...
		self.assertRaises(kanivin.AuthenticationError, check_password, user, new_password)
	...
	def test_password_on_rename_user(self):
		...
		kanivin.rename_doc(doc.doctype, old_name, new_name)

		new_doc = kanivin.get_doc(doc.doctype, new_name)
		self.assertEqual(new_doc.get_password(), password)
		self.assertTrue(not get_password_list(doc))

		kanivin.rename_doc(doc.doctype, new_name, old_name)
