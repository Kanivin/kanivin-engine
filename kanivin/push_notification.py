...
import kanivin
from kanivin import sbool
from kanivin.utils.response import Response

from .kanivinclient import KanivinClient
...
class PushNotification:
	...
	def send_notification_to_user(
		...
		if strip_html:
			body = kanivin.utils.strip_html(body)
	...
	def is_enabled(self) -> bool:
		...
		"""
		return sbool(
			kanivin.db.get_single_value("Push Notification Settings", "enable_push_notification_relay")
		)
	...
	def _get_credential(self) -> tuple[str, str]:
		...
		"""
		notification_settings = kanivin.get_doc("Push Notification Settings")
		...
		token = kanivin.generate_hash(length=48)
		# store the token in the redis cache
		kanivin.cache().set_value(
		...
		body = {
			...
			"token": token,
			"webhook_route": "/api/method/kanivin.push_notification.auth_webhook",
		}
	...
	def _send_post_request(self, method: str, params: dict, use_authentication: bool = True):
		...
		relay_server_endpoint = kanivin.conf.get("push_relay_server_url")
		if use_authentication:
			api_key, api_secret = self._get_credential()
			client = KanivinClient(relay_server_endpoint, api_key=api_key, api_secret=api_secret)
		else:
			client = KanivinClient(relay_server_endpoint)
	...
	def _site_name(self) -> str:
		return urlparse(kanivin.utils.get_url()).hostname

	@property
	def _site_protocol(self) -> str:
		return urlparse(kanivin.utils.get_url()).scheme

	@property
	def _site_port(self) -> str:
		site_uri = urlparse(kanivin.utils.get_url())
...
def auth_webhook():
	url = urlparse(kanivin.utils.get_url()).hostname
	token = kanivin.cache().get_value(f"{url}:push_relay_registration_token")
	response = Response()
...
@kanivin.whitelist(methods=["GET"])
def subscribe(fcm_token: str, project_name: str) -> dict:
	success, message = PushNotification(project_name).add_token(kanivin.session.user, fcm_token)
	return {"success": success, "message": message}


@kanivin.whitelist(methods=["GET"])
def unsubscribe(fcm_token: str, project_name: str) -> dict:
	success, message = PushNotification(project_name).remove_token(kanivin.session.user, fcm_token)
	return {"success": success, "message": message}
