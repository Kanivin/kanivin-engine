...
from kanivin.model.document import Document, get_controller
from kanivin.utils import cint, quoted
from kanivin.website.path_resolver import resolve_path
...
def get_context(context, **dict_params):
	...
	kanivin.local.form_dict.update(dict_params)
	doctype = kanivin.local.form_dict.doctype
	context.parents = [{"route": "me", "title": _("My Account")}]
	context.meta = kanivin.get_meta(doctype)
	context.update(get_list_context(context, doctype) or {})
	context.doctype = doctype
	context.txt = kanivin.local.form_dict.txt
	context.update(get(**kanivin.local.form_dict))
...
def get(doctype, txt=None, limit_start=0, limit=20, pathname=None, **kwargs):
	...
	list_context = kanivin.flags.list_context
	...
	for doc in raw_result:
		doc.doctype = doctype
		new_context = kanivin._dict(doc=doc, meta=meta, list_view_fields=list_view_fields)

		if not list_context.get_list and not isinstance(new_context.doc, Document):
			new_context.doc = kanivin.get_doc(doc.doctype, doc.name)
			new_context.update(new_context.doc.as_dict())

		if not kanivin.flags.in_test:
			pathname = pathname or kanivin.local.request.path
			new_context["pathname"] = pathname.strip("/ ")
		new_context.update(list_context)
		set_route(new_context)
		rendered_row = kanivin.render_template(row_template, new_context, is_path=True)
...
def get_list_data(
	...
	if kanivin.is_table(doctype):
		kanivin.throw(_("Child DocTypes are not allowed"), title=_("Invalid DocType"))

	if not txt and kanivin.form_dict.search:
		txt = kanivin.form_dict.search
		del kanivin.form_dict["search"]

	controller = get_controller(doctype)
	meta = kanivin.get_meta(doctype)

	filters = prepare_filters(doctype, controller, kwargs)
	list_context = get_list_context(kanivin._dict(), doctype, web_form_name)
	...
	kanivin.flags.list_context = list_context
...
def prepare_filters(doctype, controller, kwargs):
	...
	if filters.pathname:
		...
		for key, val in kanivin.local.form_dict.items():
...
def get_list_context(context, doctype, web_form_name=None):
	...
	from kanivin.website.doctype.web_form.web_form import get_web_form_module

	list_context = context or kanivin._dict()
	meta = kanivin.get_meta(doctype)
	...
	def update_context_from_module(module, list_context):
		...
		if hasattr(module, "get_list_context"):
			out = kanivin._dict(module.get_list_context(list_context) or {})
			if out:
				list_context = out
		return list_context
	...
	if meta.custom and web_form_name:
		webform_list_contexts = kanivin.get_hooks("webform_list_context")
		if webform_list_contexts and not kanivin.get_doc("Module Def", meta.module).custom:
			out = kanivin._dict(kanivin.get_attr(webform_list_contexts[0])(meta.module) or {})
			if out:
				list_context = out
