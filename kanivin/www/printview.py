...
import kanivin
from kanivin import _, cstr, get_module_path
from kanivin.core.doctype.access_log.access_log import make_access_log
from kanivin.core.doctype.document_share_key.document_share_key import is_expired
from kanivin.utils import cint, escape_html, strip_html
from kanivin.utils.jinja_globals import is_rtl

if TYPE_CHECKING:
	from kanivin.model.document import Document
	from kanivin.printing.doctype.print_format.print_format import PrintFormat
...
def get_context(context):
	...
	if not ((kanivin.form_dict.doctype and kanivin.form_dict.name) or kanivin.form_dict.doc):
		return {
			"body": f"""
				<h1>Error</h1>
				<p>Parameters doctype and name required</p>
				<pre>{escape_html(kanivin.as_json(kanivin.form_dict, indent=2))}</pre>
				"""
		}

	if kanivin.form_dict.doc:
		doc = kanivin.form_dict.doc
	else:
		doc = kanivin.get_doc(kanivin.form_dict.doctype, kanivin.form_dict.name)

	set_link_titles(doc)

	settings = kanivin.parse_json(kanivin.form_dict.settings)

	letterhead = kanivin.form_dict.letterhead or None
	...
	body = get_rendered_template(
		...
		meta=meta,
		trigger_print=kanivin.form_dict.trigger_print,
		no_letterhead=kanivin.form_dict.no_letterhead,
		letterhead=letterhead,
		settings=settings,
	)

	make_access_log(
		doctype=kanivin.form_dict.doctype, document=kanivin.form_dict.name, file_type="PDF", method="Print"
	)

	return {
		"body": body,
		"print_style": get_print_style(kanivin.form_dict.style, print_format),
		"comment": kanivin.session.user,
		"title": kanivin.utils.strip_html(cstr(doc.get_title() or doc.name)),
		"lang": kanivin.local.lang,
		"layout_direction": "rtl" if is_rtl() else "ltr",
		"doctype": kanivin.form_dict.doctype,
		"name": kanivin.form_dict.name,
		"key": kanivin.form_dict.get("key"),
		"print_format": getattr(print_format, "name", None),
		"letterhead": letterhead,
		"no_letterhead": kanivin.form_dict.no_letterhead,
		"pdf_generator": kanivin.form_dict.get("pdf_generator", "wkhtmltopdf"),
	}
...
def get_print_format_doc(print_format_name, meta):
	...
	if not print_format_name:
		print_format_name = kanivin.form_dict.format or meta.default_print_format or "Standard"

	if print_format_name == "Standard":
		return None
	else:
		try:
			return kanivin.get_doc("Print Format", print_format_name)
...
def get_rendered_template(
	...
	if not kanivin.flags.ignore_print_permissions:
		validate_print_permission(doc)

	print_settings = kanivin.get_single("Print Settings").as_dict()
	...
	if doc.meta.is_submittable:
		if doc.docstatus.is_draft() and not cint(print_settings.allow_print_for_draft):
			kanivin.throw(_("Not allowed to print draft documents"), kanivin.PermissionError)

		if doc.docstatus.is_cancelled() and not cint(print_settings.allow_print_for_cancelled):
			kanivin.throw(_("Not allowed to print cancelled documents"), kanivin.PermissionError)
	...
	if print_format:
		...
		template = None
		if hook_func := kanivin.get_hooks("get_print_format_template"):
			template = kanivin.call(hook_func[-1], jenv=jenv, print_format=print_format)
	...
	letter_head = kanivin._dict(get_letter_head(doc, no_letterhead, letterhead) or {})

	if letter_head.content:
		letter_head.content = kanivin.utils.jinja.render_template(letter_head.content, {"doc": doc.as_dict()})
	...
	if letter_head.footer:
		letter_head.footer = kanivin.utils.jinja.render_template(letter_head.footer, {"doc": doc.as_dict()})
	...
	hook_func = kanivin.get_hooks("pdf_body_html")
	html = kanivin.get_attr(hook_func[-1])(jenv=jenv, template=template, print_format=print_format, args=args)
...
def set_title_values_for_link_and_dynamic_link_fields(meta, doc, parent_doc=None):
	...
	for field in meta.get_link_fields() + meta.get_dynamic_link_fields():
		...
		link_title = kanivin.get_cached_value(doctype, doc.get(field.fieldname), meta.title_field)
		if parent_doc:
...
def set_title_values_for_table_and_multiselect_fields(meta, doc):
	for field in meta.get_table_fields():
		...
		_meta = kanivin.get_meta(field.options)
	...
	for field in doc.meta.fields:
		if field.fieldtype == "Text Editor":
			...
			if value and "<!-- markdown -->" in value:
				doc.set(field.fieldname, kanivin.utils.md_to_html(value))
...
def get_html_and_style(
	...
	if isinstance(name, str):
		document = kanivin.get_doc(doc, name)
	else:
		document = kanivin.get_doc(json.loads(doc))
	...
	try:
		html = get_rendered_template(
			...
			settings=kanivin.parse_json(settings),
		)
	except kanivin.TemplateNotFoundError:
...
def get_rendered_raw_commands(doc: str, name: str | None = None, print_format: str | None = None):
	...
	if not print_format or (print_format and not print_format.raw_printing):
		kanivin.throw(
			_("{0} is not a raw printing format.").format(print_format), kanivin.TemplateNotFoundError
		)
...
def validate_print_permission(doc):
	for ptype in ("read", "print"):
		if kanivin.has_permission(doc.doctype, ptype, doc):
			return

	if kanivin.has_website_permission(doc):
		return

	if (key := kanivin.form_dict.key) and isinstance(key, str) and validate_key(key, doc) is not False:
		return
...
def validate_key(key, doc):
	document_key_expiry = kanivin.get_cached_value(
		"Document Share Key",
	...
	if document_key_expiry is not None:
		if is_expired(document_key_expiry[0]):
			raise kanivin.exceptions.LinkExpired
		else:
			return

	# TODO: Deprecate this! kept it for backward compatibility
	if kanivin.get_system_settings("allow_older_web_view_links") and key == doc.get_signature():
		return
...
def get_print_format(doctype, print_format):
	if print_format.disabled:
		kanivin.throw(_("Print Format {0} is disabled").format(print_format.name), kanivin.DoesNotExistError)

	# server, find template
	module = print_format.module or kanivin.db.get_value("DocType", doctype, "module")

	is_custom_module = kanivin.get_cached_value("Module Def", module, "custom")

	if not is_custom_module:
		path = os.path.join(
			get_module_path(module, "Print Format", print_format.name),
			kanivin.scrub(print_format.name) + ".html",
		)
	...
	kanivin.throw(_("No template found at path: {0}").format(path), kanivin.TemplateNotFoundError)
...
def make_layout(doc, meta, format_data=None):
	...
	for df in format_data or meta.fields:
		...
		if df.fieldtype == "Signature" and not doc.get(df.fieldname):
			placeholder_image = "/assets/kanivin/images/signature-placeholder.png"
...
def get_print_style(
	...
	css = kanivin.get_template("templates/styles/standard.css").render(context)

	if style and kanivin.db.exists("Print Style", style):
		css = css + "\n" + kanivin.db.get_value("Print Style", style, "css")
...
def get_visible_columns(data, table_meta, df):
	...
	columns = []
	doc = data[0] or kanivin.new_doc(df.options)
