...
kanivin.ui.form.on("Custom Field", {
	setup: function (frm) {
	...
	dt: function (frm) {
		...
		return kanivin.call({
			method: "kanivin.custom.doctype.custom_field.custom_field.get_fields_label",
	...
	label: function (frm) {
		if (frm.doc.label && kanivin.utils.has_special_chars(frm.doc.label)) {
			...
			frm.add_custom_button(__("Rename Fieldname"), () => {
				kanivin.prompt(
					...
					function (data) {
						kanivin
							.xcall(
								"kanivin.custom.doctype.custom_field.custom_field.rename_fieldname",
								{
...
kanivin.utils.has_special_chars = function (t) {
	var iChars = "!@#$%^&*()+=-[]\\';,./{}|\":<>?";
