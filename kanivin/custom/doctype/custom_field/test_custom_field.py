...
import kanivin
from kanivin.custom.doctype.custom_field.custom_field import (
	create_custom_field,
...
test_records = kanivin.get_test_records("Custom Field")


class TestCustomField(KanivinTestCase):
	def test_create_custom_fields(self):
		...
		self.assertTrue(kanivin.db.exists("Custom Field", "Address-_test_custom_field_1"))
		self.assertTrue(kanivin.db.exists("Custom Field", "Address-_test_custom_field_2"))
		self.assertTrue(kanivin.db.exists("Custom Field", "Contact-_test_custom_field_2"))
	...
	def test_custom_field_sorting(self):
		try:
			...
			fields = kanivin.get_meta("ToDo", cached=False).fields
	...
	def test_custom_field_renaming(self):
		def gen_fieldname():
			return "test_" + kanivin.generate_hash()

		field = create_custom_field("ToDo", {"label": gen_fieldname()}, is_system_generated=False)
		old = field.fieldname
		new = gen_fieldname()
		data = kanivin.generate_hash()
		doc = kanivin.get_doc({"doctype": "ToDo", old: data, "description": "Something"}).insert()

		rename_fieldname(field.name, new)
		field.reload()
		self.assertEqual(field.fieldname, new)

		doc = kanivin.get_doc("ToDo", doc.name)  # doc.reload doesn't clear old fields.
