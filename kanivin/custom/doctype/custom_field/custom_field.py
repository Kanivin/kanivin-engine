...
from kanivin.model import core_doctypes_list
from kanivin.model.docfield import supports_translation
from kanivin.model.document import Document
from kanivin.query_builder.functions import IfNull
from kanivin.utils import cstr, random_string
...
class CustomField(Document):
		...
		if not self.fieldname:
			...
			if not label:
				if self.fieldtype in ["Section Break", "Column Break", "Tab Break"]:
					...
					kanivin.throw(_("Label is mandatory"))
	...
	def validate(self):
		...
		from kanivin.core.doctype.doctype.doctype import check_fieldname_conflicts
		...
		if not self.fieldname:
			kanivin.throw(_("Fieldname not set for Custom Field"))
	...
	def on_update(self):
		...
		if not self.flags.ignore_validate:
			from kanivin.core.doctype.doctype.doctype import validate_fields_for_doctype

			validate_fields_for_doctype(self.dt)

		# clear cache and update the schema
		if not kanivin.flags.in_create_custom_fields:
			kanivin.clear_cache(doctype=self.dt)
			kanivin.db.updatedb(self.dt)

	def on_trash(self):
		# check if Admin owned field
		if self.owner == "Administrator" and kanivin.session.user != "Administrator":
			kanivin.throw(
				_(
					"Custom Field {0} is created by the Administrator and can only be deleted through the Administrator account."
				).format(kanivin.bold(self.label))
			)

		# delete property setter entries
		kanivin.db.delete("Property Setter", {"doc_type": self.dt, "field_name": self.fieldname})

		# update doctype layouts
		doctype_layouts = kanivin.get_all("DocType Layout", filters={"document_type": self.dt}, pluck="name")
	...
	def validate_insert_after(self, meta):
		if not meta.get_field(self.insert_after):
			kanivin.throw(
				...
				kanivin.DoesNotExistError,
			)

		if self.fieldname == self.insert_after:
			kanivin.throw(_("Insert After cannot be set as {0}").format(meta.get_label(self.insert_after)))
...
def get_fields_label(doctype=None):
	...
	if doctype in core_doctypes_list:
		return kanivin.msgprint(_("Custom Fields cannot be added to core DocTypes."))

	if meta.custom:
		return kanivin.msgprint(_("Custom Fields can only be added to a standard DocType."))

	return [
		{"value": df.fieldname or "", "label": _(df.label, context=df.parent) if df.label else ""}
		for df in kanivin.get_meta(doctype).get("fields")
	]


def create_custom_field_if_values_exist(doctype, df):
	df = kanivin._dict(df)
	if df.fieldname in kanivin.db.get_table_columns(doctype) and kanivin.db.count(
...
def create_custom_field(doctype, df, ignore_validate=False, is_system_generated=True):
	...
	if not df.fieldname and df.label:
		df.fieldname = kanivin.scrub(df.label)
	if not kanivin.db.get_value("Custom Field", {"dt": doctype, "fieldname": df.fieldname}):
...
def create_custom_fields(custom_fields: dict, ignore_validate=False, update=True):
	...
	try:
		kanivin.flags.in_create_custom_fields = True
		doctypes_to_update = set()

		if kanivin.flags.in_setup_wizard:
			...
			for doctype in doctypes:
				...
				for df in fields:
					...
					if not field:
						...
						except kanivin.exceptions.DuplicateEntryError:
							pass

					elif update:
						custom_field = kanivin.get_doc({"doctype": "Custom Field", **field})
		...
		for doctype in doctypes_to_update:
			...
			kanivin.db.updatedb(doctype)

	finally:
		kanivin.flags.in_create_custom_fields = False
...
def get_existing_custom_fields(custom_fields):
	...
	existing_fields = kanivin.get_all("Custom Field", filters={"dt": ("in", doctypes_to_fetch)}, fields="*")
...
def rename_fieldname(custom_field: str, fieldname: str):
	...
	field: CustomField = kanivin.get_doc("Custom Field", custom_field)
	...
	if field.is_system_generated:
		kanivin.throw(_("System Generated Fields can not be renamed"))
	if kanivin.db.has_column(parent_doctype, fieldname):
		kanivin.throw(_("Can not rename as column {0} is already present on DocType.").format(fieldname))
	if old_fieldname == new_fieldname:
		kanivin.msgprint(_("Old and new fieldnames are same."), alert=True)
		return

	if kanivin.db.has_column(field.dt, old_fieldname):
		kanivin.db.rename_column(parent_doctype, old_fieldname, new_fieldname)
	...
	kanivin.msgprint(_("Custom field renamed to {0} successfully.").format(fieldname), alert=True)
	kanivin.db.commit()
	kanivin.clear_cache()
...
def _update_fieldname_references(field: CustomField, old_fieldname: str, new_fieldname: str) -> None:
	...
	if field.fieldtype == "Password":
		Auth = kanivin.qb.Table("__Auth")
		kanivin.qb.update(Auth).set(Auth.fieldname, new_fieldname).where(
