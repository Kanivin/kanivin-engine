...
kanivin.ui.form.on("Client Script", {
	setup(frm) {
	...
	refresh(frm) {
		if (frm.doc.dt && frm.doc.script) {
			frm.add_custom_button(__("Go to {0}", [frm.doc.dt]), () =>
				kanivin.set_route("List", frm.doc.dt, "List")
			);
		}

		if (frm.doc.view == "Form") {
			frm.add_custom_button(__("Add script for Child Table"), () => {
				kanivin.model.with_doctype(frm.doc.dt, () => {
					const child_tables = kanivin.meta
			...
			if (!frm.is_new()) {
				frm.add_custom_button(__("Compare Versions"), () => {
					new kanivin.ui.DiffView("Client Script", "script", frm.doc.name);
				});
			}
		}
	...
	dt(frm) {
		frm.toggle_display("view", !kanivin.boot.single_types.includes(frm.doc.dt));
	...
	view(frm) {
		let has_form_boilerplate = frm.doc.script.includes("kanivin.ui.form.on");
...
kanivin.ui.form.on('Task',  'validate',  function(frm) {
...
kanivin.ui.form.on('Sales Invoice',  {
    validate: function(frm) {
