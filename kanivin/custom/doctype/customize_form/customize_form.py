...
from kanivin.custom.doctype.property_setter.property_setter import delete_property_setter
from kanivin.model import core_doctypes_list, no_value_fields
from kanivin.model.docfield import supports_translation
from kanivin.model.document import Document
from kanivin.model.meta import trim_table
...
class CustomizeForm(Document):
	...
	if TYPE_CHECKING:
		...
		from kanivin.custom.doctype.customize_form_field.customize_form_field import CustomizeFormField
	...
	def on_update(self):
		kanivin.db.delete("Singles", {"doctype": "Customize Form"})
		kanivin.db.delete("Customize Form Field")
	...
	def fetch_to_customize(self):
		...
		meta = kanivin.get_meta(self.doc_type, cached=False)
	...
	def validate_doctype(self, meta):
		...
		if self.doc_type in core_doctypes_list:
			kanivin.throw(_("Core DocTypes cannot be customized."))

		if meta.issingle:
			kanivin.throw(_("Single DocTypes cannot be customized."))

		if meta.custom:
			kanivin.throw(_("Only standard DocTypes are allowed to be customized from Customize Form."))
	...
	def get_name_translation(self):
		...
		return kanivin.get_value(
			"Translation",
			{"source_text": self.doc_type, "language": kanivin.local.lang or "en"},
	...
	def set_name_translation(self):
		...
		if not self.label:
			if current:
				# clear translation
				kanivin.delete_doc("Translation", current.name)
			return
			...
			kanivin.get_doc(
				{
					...
					"language_code": kanivin.local.lang or "en",
				}
			).insert()
			return

		if self.label != current.translated_text:
			kanivin.db.set_value("Translation", current.name, "translated_text", self.label)
			kanivin.translate.clear_cache()
	...
	def save_customization(self):
		...
		if self.flags.update_db:
			try:
				kanivin.db.updatedb(self.doc_type)
			except Exception as e:
				if kanivin.db.is_db_table_size_limit(e):
					kanivin.throw(
		...
		if not hasattr(self, "hide_success") or not self.hide_success:
			kanivin.msgprint(_("{0} updated").format(_(self.doc_type)), alert=True)
		kanivin.clear_cache(doctype=self.doc_type)
		self.fetch_to_customize()

		if self.flags.rebuild_doctype_for_global_search:
			kanivin.enqueue(
				"kanivin.utils.global_search.rebuild_for_doctype",
	...
	def allow_property_change(self, prop, meta_df, df):
		if prop == "fieldtype":
			...
			(
				kanivin.db.get_value("DocField", {"parent": self.doc_type, "fieldname": df.fieldname}, "reqd")
				== 1
			)
			and (df.get(prop) == 0)
		):
			kanivin.msgprint(_("Row {0}: Not allowed to disable Mandatory for standard fields").format(df.idx))
			return False
			...
			kanivin.msgprint(_("You cannot unset 'Read Only' for field {0}").format(df.label))
			return False

		elif prop == "options" and df.get("fieldtype") not in ALLOWED_OPTIONS_CHANGE:
			kanivin.msgprint(_("You can't set 'Options' for field {0}").format(df.label))
			return False

		elif prop == "translatable" and not supports_translation(df.get("fieldtype")):
			kanivin.msgprint(_("You can't set 'Translatable' for field {0}").format(df.label))
			return False
	...
	def set_property_setters_for_actions_and_links(self, meta):
		...
		for doctype, fieldname, field_map in (
			...
			for i, d in enumerate(self.get(fieldname) or []):
				d.idx = i
				if kanivin.db.exists(doctype, d.name) and not d.custom:
					# check property and apply property setter
					original = kanivin.get_doc(doctype, d.name)
	...
	def update_order_property_setter(self, has_custom, fieldname):
		...
		if has_custom:
			...
			kanivin.db.delete("Property Setter", dict(property=property_name, doc_type=self.doc_type))
	...
	def clear_removed_items(self, doctype, items):
		...
		"""
		if items:
			kanivin.db.delete(doctype, dict(parent=self.doc_type, custom=1, name=("not in", items)))
		else:
			kanivin.db.delete(doctype, dict(parent=self.doc_type, custom=1))
	...
	def update_custom_fields(self):
		for i, df in enumerate(self.get("fields")):
			...
			if not kanivin.db.exists("Custom Field", {"dt": self.doc_type, "fieldname": df.fieldname}):
	...
	def add_custom_field(self, df, i):
		d = kanivin.new_doc("Custom Field")
	...
	def update_in_custom_field(self, df, i):
		...
		custom_field = kanivin.get_doc("Custom Field", meta_df[0].name)
		changed = False
		for prop in docfield_properties:
	...
	def delete_custom_fields(self):
		...
		for fieldname in fields_to_remove:
			...
			if not is_standard_or_system_generated_field(df):
				kanivin.delete_doc("Custom Field", df.name)
	...
	def get_existing_property_value(self, property_name, fieldname=None):
		...
		if fieldname:
			...
			if kanivin.db.has_column("DocType", property_name):
				property_value = kanivin.db.get_value("DocType", self.doc_type, property_name)
			else:
				property_value = None
	...
	def validate_fieldtype_change(self, df, old_value, new_value):
		...
		if allowed:
			old_value_length = cint(kanivin.db.type_map.get(old_value)[1])
			new_value_length = cint(kanivin.db.type_map.get(new_value)[1])
	...
	def validate_fieldtype_length(self):
		for field in self.check_length_for_fieldtypes:
			df = field.get("df")
			max_length = cint(kanivin.db.type_map.get(df.fieldtype)[1])
			...
			label = df.label
			links_str = ", ".join(kanivin.utils.get_link_to_form(self.doc_type, doc.name) for doc in docs)

			if docs:
				kanivin.throw(
					_(
						"Value for field {0} is too long in {1}. Length should be lesser than {2} characters"
					).format(kanivin.bold(label), links_str, kanivin.bold(max_length)),
	...
	def reset_layout(self):
		...
		property_setters = kanivin.get_all(
			"Property Setter",
		...
		kanivin.db.delete("Property Setter", {"name": ("in", property_setters)})
		kanivin.clear_cache(doctype=self.doc_type)
		self.fetch_to_customize()
...
def get_orphaned_columns(doctype: str):
	...
	kanivin.db.begin(read_only=True)  # Avoid any potential bug from writing to db
	return trim_table(doctype, dry_run=True)


def reset_customization(doctype):
	setters = kanivin.get_all(
		"Property Setter",
		filters={
			"doc_type": doctype,
	...
	for setter in setters:
		kanivin.delete_doc("Property Setter", setter)
	...
	for field in custom_fields:
		kanivin.delete_doc("Custom Field", field)
