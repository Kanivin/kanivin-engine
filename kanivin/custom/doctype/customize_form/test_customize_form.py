...
import kanivin
from kanivin.core.doctype.doctype.doctype import InvalidFieldNameError
...
class TestCustomizeForm(KanivinTestCase):
	def insert_custom_field(self):
		kanivin.delete_doc_if_exists("Custom Field", "Event-custom_test_field")
		self.field = kanivin.get_doc(
			{
				...
				"options": "\nCustom 1\nCustom 2\nCustom 3",
				"default": "Custom 3",
				"insert_after": kanivin.get_meta("Event").fields[-1].fieldname,
			}
		).insert()

	def setUp(self):
		self.insert_custom_field()
		kanivin.db.delete("Property Setter", dict(doc_type="Event"))
		kanivin.db.commit()
		kanivin.clear_cache(doctype="Event")

	def tearDown(self):
		kanivin.delete_doc("Custom Field", self.field.name)
		kanivin.db.commit()
		kanivin.clear_cache(doctype="Event")

	def get_customize_form(self, doctype=None):
		d = kanivin.get_doc("Customize Form")
		if doctype:
			d.doc_type = doctype
	...
	def test_fetch_to_customize(self):
		...
		self.assertEqual(len(d.get("fields")), len(kanivin.get_doc("DocType", d.doc_type).fields) + 1)
	...
	def test_save_customization_property(self):
		d = self.get_customize_form("Event")
		self.assertEqual(
			kanivin.db.get_value("Property Setter", {"doc_type": "Event", "property": "allow_copy"}, "value"),
			None,
		)
	...
	def test_save_customization_custom_field_property(self):
		d = self.get_customize_form("Event")
		self.assertEqual(kanivin.db.get_value("Custom Field", self.field.name, "reqd"), 0)

		custom_field = d.get("fields", {"fieldname": self.field.fieldname})[0]
		custom_field.reqd = 1
		custom_field.no_copy = 1
		d.run_method("save_customization")
		self.assertEqual(kanivin.db.get_value("Custom Field", self.field.name, "reqd"), 1)
		self.assertEqual(kanivin.db.get_value("Custom Field", self.field.name, "no_copy"), 1)
		...
		d.run_method("save_customization")
		self.assertEqual(kanivin.db.get_value("Custom Field", self.field.name, "reqd"), 0)
		self.assertEqual(kanivin.db.get_value("Custom Field", self.field.name, "no_copy"), 0)
	...
	def test_save_customization_new_field(self):
		...
		self.assertEqual(
			kanivin.db.get_value("Custom Field", custom_field_name, "fieldtype"),
			"Data",
		)

		self.assertEqual(
			kanivin.db.get_value("Custom Field", custom_field_name, "insert_after"),
			last_fieldname,
		)

		kanivin.delete_doc("Custom Field", custom_field_name)
		self.assertEqual(kanivin.db.get_value("Custom Field", custom_field_name), None)
	...
	def test_save_customization_remove_field(self):
		...
		self.assertEqual(kanivin.db.get_value("Custom Field", custom_field.name), None)

		kanivin.local.test_objects["Custom Field"] = []
	...
	def test_reset_to_defaults(self):
		...
		kanivin.local.test_objects["Property Setter"] = []
	...
	def test_core_doctype_customization(self):
		self.assertRaises(kanivin.ValidationError, self.get_customize_form, "User")
	...
	def test_custom_internal_links(self):
		...
		kanivin.clear_cache()
		user_group = kanivin.get_meta("User Group")
		...
		kanivin.clear_cache()
		user_group = kanivin.get_meta("Event")
	...
	def test_system_generated_fields(self):
		...
		custom_field = kanivin.get_doc("Custom Field", {"dt": doctype, "fieldname": custom_field_name})
		...
		self.assertEqual(
			kanivin.db.get_value("Property Setter", property_setter_filters, "value"), "Test Description"
		)
	...
	def test_custom_field_order(self):
		...
		field_order_property = json.loads(
			kanivin.db.get_value("Property Setter", {"doc_type": "ToDo", "property": "field_order"}, "value")
		)

		self.assertEqual(field_order_property, [df.fieldname for df in kanivin.get_meta("ToDo").fields])
