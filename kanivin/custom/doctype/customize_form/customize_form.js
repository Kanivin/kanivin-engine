...
kanivin.provide("kanivin.customize_form");

kanivin.ui.form.on("Customize Form", {
	setup: function (frm) {
		...
		frm.set_query("doc_type", function () {
			return {
				filters: [
					...
					["DocType", "restrict_to_domain", "in", kanivin.boot.active_domains],
				],
			};
		});
	...
	refresh: function (frm) {
		...
		if (frm.doc.doc_type) {
			kanivin.model.with_doctype(frm.doc.doc_type).then(() => {
				frm.page.set_title(__("Customize Form - {0}", [__(frm.doc.doc_type)]));
				kanivin.customize_form.set_primary_action(frm);

				frm.add_custom_button(
					__("Go to {0} List", [__(frm.doc.doc_type)]),
					function () {
						kanivin.set_route("List", frm.doc.doc_type);
					},
					__("Actions")
				);
				...
				frm.add_custom_button(
					__("Set Permissions"),
					function () {
						kanivin.set_route("permission-manager", frm.doc.doc_type);
					},
					__("Actions")
				);
				...
				frm.add_custom_button(
					...
					function () {
						kanivin.customize_form.confirm(__("Remove all customizations?"), frm);
					},
					__("Actions")
				);
				...
				frm.toggle_display(
					["queue_in_background"],
					kanivin.get_meta(frm.doc.doc_type).is_submittable || 0
				);
	...
	set_default_doc_type(frm) {
		let doc_type;
		if (kanivin.route_options && kanivin.route_options.doc_type) {
			doc_type = kanivin.route_options.doc_type;
				...
				return frm.call({
					...
					callback: function (r) {
						if (!r.exc) {
							...
							kanivin.customize_form.clear_locals_and_refresh(frm);
						}
					},
				});
			}
	...
	async trim_table(frm) {
		let dropped_columns = await kanivin.xcall(
			"kanivin.custom.doctype.customize_form.customize_form.get_orphaned_columns",
			{ doctype: frm.doc.doc_type }
		);

		if (!dropped_columns?.length) {
			kanivin.toast(__("This doctype has no orphan fields to trim"));
			return;
		}
		let msg = __(
			...
			frm.add_custom_button(
				...
				function () {
					kanivin.prompt(
						...
						function (data) {
							kanivin.call({
								method: "kanivin.modules.utils.export_customizations",
								args: {
									doctype: frm.doc.doc_type,
									module: data.module,
	...
	setup_sort_order(frm) {
		...
		if (frm.doc.doc_type) {
			var fields = $.map(frm.doc.fields, function (df) {
				return kanivin.model.is_value_type(df.fieldtype) ? df.fieldname : null;
			});
	...
	setup_default_views(frm) {
		kanivin.model.set_default_views_for_doctype(frm.doc.doc_type, frm);
	},
});

// can't delete standard fields
kanivin.ui.form.on("Customize Form Field", {
	before_fields_remove: function (frm, doctype, name) {
		const row = kanivin.get_doc(doctype, name);
	...
	fields_add: function (frm, cdt, cdn) {
		var f = kanivin.model.get_doc(cdt, cdn);
...
kanivin.ui.form.on("DocType Link", {
	before_links_remove: function (frm, doctype, name) {
		let row = kanivin.get_doc(doctype, name);
		...
		if (!(row.custom || row.__islocal)) {
			kanivin.msgprint(__("Cannot delete standard link. You can hide it if you want"));
			throw "cannot delete standard link";
		}
	},
	links_add: function (frm, cdt, cdn) {
		let f = kanivin.model.get_doc(cdt, cdn);
		f.custom = 1;
	},
...
kanivin.ui.form.on("DocType Action", {
	before_actions_remove: function (frm, doctype, name) {
		...
		if (!(row.custom || row.__islocal)) {
			kanivin.msgprint(__("Cannot delete standard action. You can hide it if you want"));
...
kanivin.ui.form.on("DocType State", {
	before_states_remove: function (frm, doctype, name) {
		...
		if (!(row.custom || row.__islocal)) {
			kanivin.msgprint(__("Cannot delete standard document state."));
...
kanivin.customize_form.save_customization = function (frm) {
	if (frm.doc.doc_type) {
		return frm.call({
			doc: frm.doc,
			freeze: true,
...
kanivin.customize_form.update_fields_from_form_builder = function (frm) {
	let form_builder = kanivin.form_builder;
	if (form_builder?.store) {
		let fields = form_builder.store.update_fields();

		// if fields is a string, it means there is an error
		if (typeof fields === "string") {
			kanivin.throw(fields);
		}
		frm.refresh_fields();
	}
};

kanivin.customize_form.set_primary_action = function (frm) {
...
kanivin.customize_form.confirm = function (msg, frm) {
	...
	kanivin.customize_form.confirm.dialog = d;
	d.show();
};

kanivin.customize_form.clear_locals_and_refresh = function (frm) {
	delete frm.doc.__unsaved;
	// clear doctype from locals
	kanivin.model.clear_doc("DocType", frm.doc.doc_type);
	delete kanivin.meta.docfield_copy[frm.doc.doc_type];
	frm.refresh();
};

function render_form_builder(frm) {
	if (kanivin.form_builder && kanivin.form_builder.doctype === frm.doc.doc_type) {
		kanivin.form_builder.setup_page_actions();
		kanivin.form_builder.store.fetch();
		return;
	}

	if (kanivin.form_builder) {
		kanivin.form_builder.wrapper = $(frm.fields_dict["form_builder"].wrapper);
		kanivin.form_builder.frm = frm;
		kanivin.form_builder.doctype = frm.doc.doc_type;
		kanivin.form_builder.customize = true;
		kanivin.form_builder.init(true);
		kanivin.form_builder.store.fetch();
	} else {
		kanivin.require("form_builder.bundle.js").then(() => {
			kanivin.form_builder = new kanivin.ui.FormBuilder({
...
extend_cscript(cur_frm.cscript, new kanivin.model.DocTypeController({ frm: cur_frm }));
