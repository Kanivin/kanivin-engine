...
def get_columns():
	...
	installed_apps = kanivin.get_installed_apps(_ensure_on_bench=True)
		...
		if isinstance(values, dict):
			for k, v in values.items():
				...
				for app in installed_apps:
					if app_hooks := delist(kanivin.get_hooks(hook, app_name=app)):
						row[app] = fmt_hook_values(app_hooks.get(k))
				data.append(row)
		else:
			row = {"hook_name": hook, "hook_values": fmt_hook_values(values)}
			for app in installed_apps:
				row[app] = fmt_hook_values(kanivin.get_hooks(hook, app_name=app))
