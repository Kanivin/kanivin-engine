...
def merge_location_features_in_one(coords):
	...
	for element in coords:
		geojson_loc = kanivin.parse_json(element["location"])
		if not geojson_loc:
			continue
...
def return_location(doctype, filters_sql):
	...
	if filters_sql:
		try:
			coords = kanivin.db.sql(
				f"""SELECT name, location FROM `tab{doctype}`  WHERE {filters_sql}""", as_dict=True
			)
		except kanivin.db.InternalError:
			kanivin.msgprint(kanivin._("This Doctype does not contain location fields"), raise_exception=True)
			return
	else:
		coords = kanivin.get_all(doctype, fields=["name", "location"])
	return coords
...
def return_coordinates(doctype, filters_sql):
	...
	if filters_sql:
		try:
			...
			kanivin.msgprint(
				kanivin._("This Doctype does not contain latitude and longitude fields"), raise_exception=True
			)
			return
	else:
		coords = kanivin.get_all(doctype, fields=["name", "latitude", "longitude"])
	return coords
...
def get_coords_conditions(doctype, filters=None):
	...
	if not kanivin.has_permission(doctype):
		kanivin.throw(kanivin._("Not Permitted"), kanivin.PermissionError)
