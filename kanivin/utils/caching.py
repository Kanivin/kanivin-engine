...
def request_cache(func: Callable) -> Callable:
	...
	kanivin.local.request_cache. The cache only persists for the current request
	...
	def wrapper(*args, **kwargs):
		if not getattr(kanivin.local, "initialised", None):
			return func(*args, **kwargs)
		if not hasattr(kanivin.local, "request_cache"):
			kanivin.local.request_cache = defaultdict(dict)
		...
		try:
			return kanivin.local.request_cache[func][args_key]
		except KeyError:
			return_val = func(*args, **kwargs)
			kanivin.local.request_cache[func][args_key] = return_val
			return return_val
...
def site_cache(ttl: int | None = None, maxsize: int | None = None) -> Callable:
	...
	kanivin.utils.caching._SITE_CACHE. The cache persists on the parent process.
	...
	workers, use redis (kanivin.cache API) instead.

	Usage:
	        from kanivin.utils.caching import site_cache
	...
	def time_cache_wrapper(func: Callable | None = None) -> Callable:
		...
		def site_cache_wrapper(*args, **kwargs):
			if getattr(kanivin.local, "initialised", None):
				...
				if hasattr(func, "maxsize") and len(_SITE_CACHE[func_key][kanivin.local.site]) >= func.maxsize:
					_SITE_CACHE[func_key][kanivin.local.site].pop(
						next(iter(_SITE_CACHE[func_key][kanivin.local.site])), None
					)

				if func_call_key not in _SITE_CACHE[func_key][kanivin.local.site]:
					_SITE_CACHE[func_key][kanivin.local.site][func_call_key] = func(*args, **kwargs)

				return _SITE_CACHE[func_key][kanivin.local.site][func_call_key]
...
def redis_cache(ttl: int | None = 3600, user: str | bool | None = None, shared: bool = False) -> Callable:
	...
	def wrapper(func: Callable | None = None) -> Callable:
		...
		def clear_cache():
			kanivin.cache.delete_keys(func_key)
		...
		def redis_cache_wrapper(*args, **kwargs):
			...
			if kanivin.cache.exists(func_call_key, user=user, shared=shared):
				return kanivin.cache.get_value(func_call_key, user=user, shared=shared)
			val = func(*args, **kwargs)
			ttl = getattr(func, "ttl", 3600)
			kanivin.cache.set_value(func_call_key, val, expires_in_sec=ttl, user=user, shared=shared)
			return val
