...
def get_jenv():
	...
	if not getattr(kanivin.local, "jenv", None):
		from jinja2 import DebugUndefined
		from jinja2.sandbox import SandboxedEnvironment

		from kanivin.utils.safe_exec import UNSAFE_ATTRIBUTES, get_safe_globals

		UNSAFE_ATTRIBUTES = UNSAFE_ATTRIBUTES - {"format", "format_map"}

		class KanivinSandboxedEnvironment(SandboxedEnvironment):
		...
		# kanivin will be loaded last, so app templates will get precedence
		jenv = KanivinSandboxedEnvironment(loader=get_jloader(), undefined=DebugUndefined)
		set_filters(jenv)
		...
		kanivin.local.jenv = jenv

	return kanivin.local.jenv
...
def validate_template(html):
	...
	try:
		...
		kanivin.throw(f"Syntax error in template as line {e.lineno}: {e.message}")
...
def render_template(template, context=None, is_path=None, safe_render=True):
	...
	from kanivin import _, get_traceback, throw
...
def get_jloader():
	...
	if not getattr(kanivin.local, "jloader", None):
		from jinja2 import ChoiceLoader, PackageLoader, PrefixLoader

		apps = kanivin.get_hooks("template_apps")
		if not apps:
			apps = list(
				reversed(
					kanivin.local.flags.web_pages_apps or kanivin.get_installed_apps(_ensure_on_bench=True)
				)
			)

		if "kanivin" not in apps:
			apps.append("kanivin")

		kanivin.local.jloader = ChoiceLoader(
...
def get_jinja_hooks():
	...
	if not getattr(kanivin.local, "site", None):
		return (None, None)
	...
	def get_obj_dict_from_paths(object_paths):
		...
		for obj_path in object_paths:
			try:
				obj = kanivin.get_module(obj_path)
			except ModuleNotFoundError:
				obj = kanivin.get_attr(obj_path)
	...
	values = kanivin.get_hooks("jinja")
