...
from kanivin.utils import CallbackManager, cint, get_bench_id
from kanivin.utils.commands import log
from kanivin.utils.deprecations import deprecation_warning
from kanivin.utils.redis_queue import RedisQueue
...
def get_queues_timeout():
	common_site_config = kanivin.get_conf()
...
def enqueue(
	...
	**kwargs,
	...
	:param now: if now=True, the method is executed via kanivin.call
	...
	if deduplicate:
		if not job_id:
			kanivin.throw(_("`job_id` paramater is required for deduplication."))
		job = get_job(job_id)
		if job and job.get_status() in (JobStatus.QUEUED, JobStatus.STARTED):
			kanivin.logger().error(f"Not queueing job {job.id} because it is in queue already")
			return
		elif job:
	...
	if not is_async and not kanivin.flags.in_test:
		deprecation_warning(
			"Using enqueue with is_async=False outside of tests is not recommended, use now=True instead."
		)

	call_directly = now or (not is_async and not kanivin.flags.in_test)
	if call_directly:
		return kanivin.call(method, **kwargs)

	try:
		q = get_queue(queue, is_async=is_async)
	except ConnectionError:
		if kanivin.local.flags.in_migrate:
	...
	def enqueue_call():
		return q.enqueue_call(
			...
			kwargs=queue_args,
			at_front=at_front,
			failure_ttl=kanivin.conf.get("rq_job_failure_ttl") or RQ_JOB_FAILURE_TTL,
			result_ttl=kanivin.conf.get("rq_results_ttl") or RQ_RESULTS_TTL,
			job_id=job_id,
		)

	if enqueue_after_commit:
		kanivin.db.after_commit.add(enqueue_call)
		return
...
def enqueue_doc(doctype, name=None, method=None, queue="default", timeout=300, now=False, **kwargs):
	...
	return enqueue(
		"kanivin.utils.background_jobs.run_doc_method",
		doctype=doctype,
		name=name,
		doc_method=method,
		queue=queue,
...
def run_doc_method(doctype, name, doc_method, **kwargs):
	getattr(kanivin.get_doc(doctype, name), doc_method)(**kwargs)
...
def execute_job(site, method, event, job_name, kwargs, user=None, is_async=True, retry=0):
	...
	kanivin.local.job = kanivin._dict(
		site=site,
		method=method_name,
		job_name=job_name,
		kwargs=kwargs,
	...
	for before_job_task in kanivin.get_hooks("before_job"):
		kanivin.call(before_job_task, method=method_name, kwargs=kwargs, transaction_type="job")

	try:
		retval = method(**kwargs)

	except (kanivin.db.InternalError, kanivin.RetryBackgroundJobError) as e:
		kanivin.db.rollback()

		if retry < 5 and (
			isinstance(e, kanivin.RetryBackgroundJobError)
			or (kanivin.db.is_deadlocked(e) or kanivin.db.is_timedout(e))
		):
			# retry the job if
			# 1213 = deadlock
			# 1205 = lock wait timeout
			# or RetryBackgroundJobError is explicitly raised
			kanivin.job.after_job.reset()
			kanivin.destroy()
			time.sleep(retry + 1)
			...
			kanivin.log_error(title=method_name)
			raise
		...
		if not hasattr(kanivin.local, "site"):
			kanivin.init(site)
			kanivin.connect()
		for after_job_task in kanivin.get_hooks("after_job"):
			kanivin.call(after_job_task, method=method_name, kwargs=kwargs, result=retval)
		kanivin.local.job.after_job.run()

		if is_async:
			kanivin.destroy()


class KanivinWorker(Worker):
	def work(self, *args, **kwargs):
		self.start_kanivin_scheduler()
	...
	def start_kanivin_scheduler(self):
		from kanivin.utils.scheduler import start_scheduler
...
def start_worker_pool(
	...
	import kanivin.database.query  # sqlparse and indirect imports
	import kanivin.query_builder  # pypika
	import kanivin.utils  # common utils
	import kanivin.utils.safe_exec
	import kanivin.utils.scheduler
	...
	pool = WorkerPool(
		...
		worker_class=KanivinWorker,  # Auto starts scheduler with workerpool
	)
...
def validate_queue(queue, default_queue_list=None):
	...
	if queue not in default_queue_list:
		kanivin.throw(_("Queue should be one of {0}").format(", ".join(default_queue_list)))
...
def get_redis_conn(username=None, password=None):
	if not hasattr(kanivin.local, "conf"):
		raise Exception("You need to call kanivin.init")

	elif not kanivin.local.conf.redis_queue:
		raise Exception("redis_queue missing in common_site_config.json")

	global _redis_queue_conn

	cred = kanivin._dict()
	if kanivin.conf.get("use_rq_auth"):
		if username:
			cred["username"] = username
			cred["password"] = password
		else:
			cred["username"] = kanivin.get_site_config().rq_username or get_bench_id()
			cred["password"] = kanivin.get_site_config().rq_password
	...
	try:
		...
		log(
			f"Please make sure that Redis Queue runs @ {kanivin.get_conf().redis_queue}. Redis reported error: {e!s}",
			colour="red",
		)
		raise
...
def enqueue_test_job():
	enqueue("kanivin.utils.background_jobs.test_job", s=100)
...
def create_job_id(job_id: str) -> str:
	...
	if not job_id:
		job_id = str(uuid4())
	return f"{kanivin.local.site}::{job_id}"
...
def truncate_failed_registry(job, connection, type, value, traceback):
	...
	from kanivin.utils import create_batch

	conf = kanivin.conf if kanivin.conf else kanivin.get_conf(site=job.kwargs.get("site"))
