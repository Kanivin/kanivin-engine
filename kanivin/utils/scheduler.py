...
import kanivin
from kanivin.utils import cint, get_bench_path, get_datetime, get_sites, now_datetime
from kanivin.utils.background_jobs import set_niceness
...
def start_scheduler() -> NoReturn:
	...
	try:
		...
		kanivin.logger("scheduler").debug("Scheduler already running")
		return
...
def enqueue_events_for_all_sites() -> None:
	...
	for site in sites:
		try:
			...
			kanivin.logger("scheduler").debug(f"Failed to enqueue events for site: {site}", exc_info=True)


def enqueue_events_for_site(site: str) -> None:
	def log_exc():
		kanivin.logger("scheduler").error(f"Exception in Enqueue Events for Site {site}", exc_info=True)
	...
	try:
		...
		kanivin.logger("scheduler").debug(f"Queued events for site {site}")
	except Exception as e:
		if kanivin.db.is_access_denied(e):
			kanivin.logger("scheduler").debug(f"Access denied for site {site}")
		log_exc()
...
def enqueue_events(site: str) -> list[str] | None:
	if schedule_jobs_based_on_activity():
		enqueued_jobs = []
		all_jobs = kanivin.get_all("Scheduled Job Type", filters={"stopped": 0}, fields="*")
		random.shuffle(all_jobs)
		for job_type in all_jobs:
			job_type = kanivin.get_doc(doctype="Scheduled Job Type", **job_type)
			try:
				if job_type.enqueue():
					enqueued_jobs.append(job_type.method)
			except CroniterBadCronError:
				kanivin.logger("scheduler").error(
					f"Invalid Job on {kanivin.local.site} - {job_type.name}", exc_info=True
				)
...
def is_scheduler_inactive(verbose=True) -> bool:
	if kanivin.local.conf.maintenance_mode:
		if verbose:
			cprint(f"{kanivin.local.site}: Maintenance mode is ON")
		return True

	if kanivin.local.conf.pause_scheduler:
		if verbose:
			cprint(f"{kanivin.local.site}: kanivin.conf.pause_scheduler is SET")
		return True
...
def is_scheduler_disabled(verbose=True) -> bool:
	if kanivin.conf.disable_scheduler:
		if verbose:
			cprint(f"{kanivin.local.site}: kanivin.conf.disable_scheduler is SET")
		return True

	scheduler_disabled = not kanivin.utils.cint(
		kanivin.db.get_single_value("System Settings", "enable_scheduler")
	)
	if scheduler_disabled:
		if verbose:
			cprint(f"{kanivin.local.site}: SystemSettings.enable_scheduler is UNSET")
	return scheduler_disabled


def toggle_scheduler(enable):
	kanivin.db.set_single_value("System Settings", "enable_scheduler", int(enable))
...
def is_dormant(check_time=None):
	...
	if kanivin.conf.developer_mode or not on_kanivincloud():
		return False

	threshold = cint(kanivin.get_system_settings("dormant_days")) * 86400
	if not threshold:
		return False

	last_activity = kanivin.db.get_value(
...
def _get_last_modified_timestamp(doctype):
	timestamp = kanivin.db.get_value(doctype, filters={}, fieldname="modified", order_by="modified desc")
	if timestamp:
...
def activate_scheduler():
	...
	if kanivin.local.conf.maintenance_mode:
		kanivin.throw(kanivin._("Scheduler can not be re-enabled when maintenance mode is active."))

	if is_scheduler_disabled():
		enable_scheduler()
	if kanivin.conf.pause_scheduler:
...
def get_scheduler_tick() -> int:
	return cint(kanivin.get_conf().scheduler_tick_interval) or 60
