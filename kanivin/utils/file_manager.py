...
import kanivin
from kanivin import _, conf
from kanivin.query_builder.utils import DocType
from kanivin.utils import call_hook_method, cint, cstr, encode, get_files_path, get_hook_method
...
def get_file_url(file_data_name):
	data = kanivin.db.get_value("File", file_data_name, ["file_name", "file_url"], as_dict=True)
	return data.file_url or data.file_name


def upload():
	# get record details
	dt = kanivin.form_dict.doctype
	dn = kanivin.form_dict.docname
	file_url = kanivin.form_dict.file_url
	filename = kanivin.form_dict.filename
	kanivin.form_dict.is_private = cint(kanivin.form_dict.is_private)

	if not filename and not file_url:
		kanivin.msgprint(_("Please select a file or url"), raise_exception=True)
	...
	if dt and dn:
		...
		comment = kanivin.get_doc(dt, dn).add_comment(
			"Attachment",
...
def save_url(file_url, filename, dt, dn, folder, is_private, df=None):
	...
	# 	kanivin.msgprint("URL must start with 'http://' or 'https://'")
	# 	return None, None

	file_url = unquote(file_url)
	file_size = kanivin.form_dict.file_size
	...
	try:
		...
		return kanivin.get_doc("File", f.duplicate_entry)
	return f


def get_uploaded_content():
	# should not be unicode when reading a file, hence using kanivin.form
	if "filedata" in kanivin.form_dict:
		if "," in kanivin.form_dict.filedata:
			kanivin.form_dict.filedata = kanivin.form_dict.filedata.rsplit(",", 1)[1]
		kanivin.uploaded_content = safe_b64decode(kanivin.form_dict.filedata)
		kanivin.uploaded_filename = kanivin.form_dict.filename
		return kanivin.uploaded_filename, kanivin.uploaded_content
	else:
		kanivin.msgprint(_("No file attached"))
		return None, None
...
def get_file_data_from_hash(content_hash, is_private=0):
	for name in kanivin.get_all(
		"File", {"content_hash": content_hash, "is_private": is_private}, pluck="name"
	):
		b = kanivin.get_doc("File", name)
		return {k: b.get(k) for k in kanivin.get_hooks()["write_file_keys"]}
	return False
...
def write_file(content, fname, is_private=0):
	...
	kanivin.create_folder(file_path)
	# write the file
	if isinstance(content, str):
...
def remove_all(dt, dn, from_delete=False, delete_permanently=False):
	...
	try:
		for fid in kanivin.get_all("File", {"attached_to_doctype": dt, "attached_to_name": dn}, pluck="name"):
			if from_delete:
				# If deleting a doc, directly delete files
				kanivin.delete_doc("File", fid, ignore_permissions=True, delete_permanently=delete_permanently)
			else:
...
def remove_file(
	...
	if not (attached_to_doctype and attached_to_name):
		attached = kanivin.db.get_value("File", fid, ["attached_to_doctype", "attached_to_name", "file_name"])
		if attached:
	...
	if attached_to_doctype and attached_to_name and not from_delete:
		doc = kanivin.get_doc(attached_to_doctype, attached_to_name)
		ignore_permissions = doc.has_permission("write") or False
		if kanivin.flags.in_web_form:
			ignore_permissions = True
		if not file_name:
			file_name = kanivin.db.get_value("File", fid, "file_name")
...
def delete_file(path):
	...
	if path:
		...
		if parts[0] == "files":
			path = kanivin.utils.get_site_path("public", "files", parts[-1])

		else:
			path = kanivin.utils.get_site_path("private", "files", parts[-1])
...
def get_file_name(fname, optional_suffix):
	...
	n_records = kanivin.get_all("File", {"file_name": fname}, pluck="name")
