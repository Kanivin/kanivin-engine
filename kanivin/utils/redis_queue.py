...
import kanivin
from kanivin.utils import get_bench_id, random_string
...
class RedisQueue:
	...
	def add_user(self, username, password=None):
		...
		return kanivin._dict(user_settings) if is_created else {}

	@classmethod
	def get_connection(cls, username=None, password=None):
		if kanivin.conf.redis_queue_sentinel_enabled:
			from kanivin.utils.redis_wrapper import get_sentinel_connection

			sentinels = [tuple(node.split(":")) for node in kanivin.conf.get("redis_queue_sentinels", [])]
			sentinel = get_sentinel_connection(
				sentinels=sentinels,
				sentinel_username=kanivin.conf.get("redis_queue_sentinel_username"),
				sentinel_password=kanivin.conf.get("redis_queue_sentinel_password"),
				master_username=kanivin.conf.get("redis_queue_master_username", username),
				master_password=kanivin.conf.get("redis_queue_master_password", password),
			)
			conn = sentinel.master_for(kanivin.conf.get("redis_queue_master_service"))
			conn.ping()
			return conn
		conn = redis.from_url(kanivin.conf.redis_queue, username=username, password=password)
		conn.ping()
		return conn
