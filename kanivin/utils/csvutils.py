...
from kanivin.utils import cint, comma_or, cstr, flt


def read_csv_content_from_attached_file(doc):
	fileid = kanivin.get_all(
		"File",
		fields=["name"],
	...
	try:
		_file = kanivin.get_doc("File", fileid)
...
def read_csv_content(fcontent):
	...
	content = [kanivin.safe_decode(line) for line in fcontent.splitlines(True)]
	...
	try:
		...
		kanivin.msgprint(_("Not a valid Comma Separated Value (CSV File)"))
		raise
...
def send_csv_to_client(args):
	...
	kanivin.response["result"] = cstr(to_csv(args.data))
	kanivin.response["doctype"] = args.filename
...
def build_csv_response(data, filename):
	kanivin.response["result"] = cstr(to_csv(data))
	kanivin.response["doctype"] = filename
	kanivin.response["type"] = "csv"
...
def check_record(d):
	...
	from kanivin.utils.dateutils import parse_date
	...
	for key in d:
		...
		if docfield:
			if docfield.reqd and (val == "" or val is None):
				kanivin.msgprint(_("{0} is required").format(docfield.label), raise_exception=1)
...
def import_doc(d, doctype, overwrite, row_idx, submit=False, ignore_links=False):
	...
	if d.get("name") and kanivin.db.exists(doctype, d["name"]):
		if overwrite:
			doc = kanivin.get_doc(doctype, d["name"])
