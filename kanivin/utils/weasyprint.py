...
def download_pdf(doctype, name, print_format, letterhead=None):
	...
	kanivin.local.response.filename = "{name}.pdf".format(name=name.replace(" ", "-").replace("/", "-"))
	kanivin.local.response.filecontent = pdf
	kanivin.local.response.type = "pdf"
...
class PrintFormatGenerator:
	...
	def __init__(self, print_format, doc, letterhead=None):
		...
		"""
		self.base_url = kanivin.utils.get_url()
		self.print_format = kanivin.get_doc("Print Format", print_format)
		self.doc = doc

		if letterhead == _("No Letterhead"):
			letterhead = None
		self.letterhead = kanivin.get_doc("Letter Head", letterhead) if letterhead else None
	...
	def build_context(self):
		self.print_settings = kanivin.get_doc("Print Settings")
		...
		print_style = (
			kanivin.get_doc("Print Style", self.print_settings.print_style)
	...
	def get_main_html(self):
		self.context.css = kanivin.render_template("templates/print_format/print_format.css", self.context)
		return kanivin.render_template("templates/print_format/print_format.html", self.context)

	def get_header_footer_html(self):
		header_html = footer_html = None
		if self.letterhead:
			header_html = kanivin.render_template("templates/print_format/print_header.html", self.context)
		if self.letterhead:
			footer_html = kanivin.render_template("templates/print_format/print_footer.html", self.context)
	...
	def get_layout(self, print_format):
		layout = kanivin.parse_json(print_format.format_data)
	...
	def process_margin_texts(self, layout):
		...
		for key in margin_texts:
			...
			if text and "{{" in text:
				layout["text_" + key] = kanivin.render_template(text, self.context)
