...
from kanivin.utils import cint, strip_html_tags
from kanivin.utils.data import cstr
...
def reset():
	...
	:return:
	"""
	kanivin.db.delete("__global_search")
...
def get_doctypes_with_global_search(with_child_tables=True):
	...
	def _get():
		...
		for d in kanivin.get_all("DocType", fields=["name", "module"], filters=filters):
			meta = kanivin.get_meta(d.name)
		...
		doctypes = [
			...
			if module_app.get(kanivin.scrub(d.module)) and module_app[kanivin.scrub(d.module)] in installed_apps
		]

		return doctypes

	return kanivin.cache.get_value("doctypes_with_global_search", _get)
...
def rebuild_for_doctype(doctype):
	...
	"""
	if kanivin.local.conf.get("disable_global_search"):
		return

	def _get_filters():
		filters = kanivin._dict({"docstatus": ["!=", 2]})
	...
	if cint(meta.istable) == 1:
		parent_doctypes = kanivin.get_all(
			"DocField",
			fields="parent",
			filters={"fieldtype": ["in", kanivin.model.table_fields], "options": doctype},
		)
	...
	all_records = kanivin.get_all(doctype, fields=fieldnames, filters=_get_filters())
		...
		if content:
			...
			try:
				if hasattr(get_controller(doctype), "is_website_published") and meta.allow_guest_to_view:
					d = kanivin.get_doc(doctype, doc.name)
			...
			all_contents.append(
				{
					"doctype": kanivin.db.escape(doctype),
					"name": kanivin.db.escape(doc.name),
					"content": kanivin.db.escape(" ||| ".join(content or "")),
					"published": published,
					"title": kanivin.db.escape((title or "")[: int(kanivin.db.VARCHAR_LEN)]),
					"route": kanivin.db.escape((route or "")[: int(kanivin.db.VARCHAR_LEN)]),
				}
			)
	if all_contents:
...
def delete_global_search_records_for_doctype(doctype):
	kanivin.db.delete("__global_search", {"doctype": doctype})
...
def get_children_data(doctype, meta):
	...
	"""
	all_children = kanivin._dict()
	child_search_fields = kanivin._dict()

	for child in meta.get_table_fields():
		child_meta = kanivin.get_meta(child.options)
		...
		if search_fields:
			...
			for record in child_records:
				all_children.setdefault(record.parent, kanivin._dict()).setdefault(child.options, []).append(
					record
				)
...
def update_global_search(doc):
	...
	content = [
		...
		if doc.get(field.fieldname) and field.fieldtype not in kanivin.model.table_fields
	]
	...
	if content:
		...
		title = (cstr(doc.get_title()) or "")[: int(kanivin.db.VARCHAR_LEN)]
...
def update_global_search_for_all_web_pages():
	if kanivin.conf.get("disable_global_search"):
		return
...
def get_routes_to_index():
	...
	routes_to_index = []
	for app in apps:
		base = kanivin.get_app_path(app, "www")
		path_to_index = kanivin.get_app_path(app, "www")
...
def get_formatted_value(value, field):
	...
	if getattr(field, "fieldtype", None) in ["Text", "Text Editor"]:
		value = unescape_html(kanivin.safe_decode(value))
...
def sync_global_search():
	...
	def get_search_queue_item_generator():
		while value := kanivin.cache.rpop("global_search_queue"):
			yield value
...
def sync_values(values: list):
	...
	GlobalSearch = kanivin.qb.Table("__global_search")
	conflict_fields = ["content", "published", "title", "route"]

	query = kanivin.qb.into(GlobalSearch).columns(["doctype", "name", *conflict_fields]).insert(*values)
	...
	for field in conflict_fields:
		...
		elif kanivin.db.db_type == "postgres":
...
def sync_value_in_queue(value):
	try:
		...
		kanivin.cache.lpush("global_search_queue", json.dumps(value))
...
def delete_for_document(doc):
	...
	"""
	kanivin.db.delete("__global_search", {"doctype": doc.doctype, "name": doc.name})
...
def search(text, start=0, limit=20, doctype=""):
	...
	"""
	from kanivin.desk.doctype.global_search_settings.global_search_settings import (
		get_doctypes_for_global_search,
	)
	from kanivin.query_builder.functions import Match

	results = []
	sorted_results = []

	allowed_doctypes = set(get_doctypes_for_global_search()) & set(kanivin.get_user().get_can_read())
	...
	for doctype in allowed_doctypes:
		for r in results:
			if r.doctype == doctype and r.rank > 0.0:
				try:
					meta = kanivin.get_meta(r.doctype)
					if meta.image_field:
						r.image = kanivin.db.get_value(r.doctype, r.name, meta.image_field)
				except Exception:
...
def web_search(text: str, scope: str | None = None, start: int = 0, limit: int = 20):
	...
	for text in texts:
		...
		mariadb_conditions += "MATCH(`content`) AGAINST ({} IN BOOLEAN MODE)".format(
			kanivin.db.escape("+" + text + "*")
		)
		postgres_conditions += f'TO_TSVECTOR("content") @@ PLAINTO_TSQUERY({kanivin.db.escape(text)})'

		values = {"scope": "".join([scope, "%"]) if scope else "", "limit": limit, "start": start}

		result = kanivin.db.multisql(
			{
