...
class RedisWrapper(redis.Redis):
	...
	def __call__(self):
		"""WARNING: Added for backward compatibility to support kanivin.cache().method(...)"""
		return self
	...
	def make_key(self, key, user=None, shared=False):
		...
		return f"{kanivin.conf.db_name}|{key}".encode()
	...
	def get_value(self, key, generator=None, user=None, expires=False, shared=False):
		...
		:param expires: If the key is supposed to be with an expiry, don't store it in kanivin.local
		"""
		original_key = key
		key = self.make_key(key, user, shared)

		local_cache = kanivin.local.cache
	...
	def get_keys(self, key):
		...
		try:
			...
			return [k for k in list(kanivin.local.cache) if regex.match(cstr(k))]
	...
	def hset(
		...
		**kwargs,
		...
		# set in local
		kanivin.local.cache.setdefault(_name, {})[key] = value
	...
	def hdel(self, name, key, shared=False):
		...
		if _name in kanivin.local.cache:
			if key in kanivin.local.cache[_name]:
				del kanivin.local.cache[_name][key]
		try:
...
def setup_cache():
	if kanivin.conf.redis_cache_sentinel_enabled:
		sentinels = [tuple(node.split(":")) for node in kanivin.conf.get("redis_cache_sentinels", [])]
		sentinel = get_sentinel_connection(
			sentinels=sentinels,
			sentinel_username=kanivin.conf.get("redis_cache_sentinel_username"),
			sentinel_password=kanivin.conf.get("redis_cache_sentinel_password"),
			master_username=kanivin.conf.get("redis_cache_master_username"),
			master_password=kanivin.conf.get("redis_cache_master_password"),
		)
		return sentinel.master_for(
			kanivin.conf.get("redis_cache_master_service"),
			redis_class=RedisWrapper,
		)

	return RedisWrapper.from_url(kanivin.conf.get("redis_cache"))
