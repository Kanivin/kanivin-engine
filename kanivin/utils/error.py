...
EXCLUDE_EXCEPTIONS = (
	...
	kanivin.CSRFTokenError,  # CSRF covers OAuth too
	kanivin.SecurityException,
	kanivin.InReadOnlyMode,
)
...
def log_error(title=None, message=None, reference_doctype=None, reference_name=None, *, defer_insert=False):
	...
	from kanivin.utils.sentry import capture_exception
	...
	traceback = kanivin.as_unicode(traceback or kanivin.get_traceback(with_context=True))
	...
	if kanivin.flags.read_only or defer_insert:
...
def log_error_snapshot(exception: Exception):
	...
	logger = kanivin.logger(with_more_info=True)
	...
	def decorator_raise_error_on_no_output(func):
		...
		def wrapper_raise_error_on_no_output(*args, **kwargs):
			...
			if (not response) and raise_error:
				kanivin.throw(error_message, error_type or Exception)
			return response
...
def guess_exception_source(exception: str) -> str | None:
	...
	- For kanivin.throws the exception source is possibly present after skipping kanivin.throw frames
	...
	with suppress(Exception):
		...
		if (probably_source := apps.most_common(1)) and probably_source[0][0] != "kanivin":
