...
import kanivin
from kanivin.model.meta import get_field_currency, get_field_precision
...
def format_value(value, df=None, doc=None, currency=None, translated=False, format=None):
	...
	if not df:
		...
		default_currency = kanivin.db.get_default("currency")
		...
		if not BLOCK_TAGS_PATTERN.search(value):
			return kanivin.safe_decode(value).replace("\n", "<br>")

	elif df.get("fieldtype") == "Markdown Editor":
		return kanivin.utils.markdown(value)

	elif df.get("fieldtype") == "Table MultiSelect":
		values = []
		meta = kanivin.get_meta(df.options)
		link_field = next(df for df in meta.fields if df.fieldtype == "Link")
		for v in value:
			v.update({"__link_titles": doc.get("__link_titles")})
			formatted_value = kanivin.format_value(v.get(link_field.fieldname, ""), link_field, v)
		...
		if isinstance(value, str):
			return kanivin._(value, context=df.parent or "")
