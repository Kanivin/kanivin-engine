...
def get_fullname(user=None):
	...
	if not hasattr(kanivin.local, "fullnames"):
		kanivin.local.fullnames = {}

	if not kanivin.local.fullnames.get(user):
		p = kanivin.db.get_value("User", user, ["first_name", "last_name"], as_dict=True)
		if p:
			kanivin.local.fullnames[user] = (
				" ".join(filter(None, [p.get("first_name"), p.get("last_name")])) or user
			)
		else:
			kanivin.local.fullnames[user] = user

	return kanivin.local.fullnames.get(user)
...
def get_email_address(user=None):
	...
	return kanivin.db.get_value("User", user, "email")
...
def validate_phone_number_with_country_code(phone_number: str, fieldname: str) -> None:
	...
	try:
		...
		if not valid_number:
			kanivin.throw(
				error_message.format(kanivin.bold(phone_number), kanivin.bold(fieldname)),
				title=error_title,
				exc=kanivin.InvalidPhoneNumberError,
			)
...
def validate_phone_number(phone_number, throw=False):
	...
	if not match and throw:
		kanivin.throw(
			kanivin._("{0} is not a valid Phone Number").format(phone_number), kanivin.InvalidPhoneNumberError
		)
...
def validate_name(name, throw=False):
	...
	Note: "Name" here is name of a person, not the primary key in Kanivin doctypes.
	"""
	if not name:
		return False
	...
	if not match and throw:
		kanivin.throw(kanivin._("{0} is not a valid Name").format(name), kanivin.InvalidNameError)
...
def validate_email_address(email_str, throw=False):
	...
	def _check(e):
		...
		if not _valid:
			if throw:
				invalid_email = kanivin.utils.escape_html(e)
				kanivin.throw(
					kanivin._("{0} is not a valid Email Address").format(invalid_email),
					kanivin.InvalidEmailAddressError,
				)
			return None
		else:
			return email_id
...
def validate_url(
	...
	if not is_valid and throw:
		kanivin.throw(kanivin._("'{0}' is not a valid URL").format(kanivin.bold(txt)))
...
def has_gravatar(email: str) -> str:
	...
	if kanivin.flags.in_import or kanivin.flags.in_install or kanivin.flags.in_test:
...
def get_gravatar_url(email: str, default: Literal["mm", "404"] = "mm") -> str:
	hexdigest = hashlib.md5(kanivin.as_unicode(email).encode("utf-8"), usedforsecurity=False).hexdigest()
	...
	return Format(
		custom_var_printers=[
			...
			*[(variable_name, lambda *a, **kw: placeholder) for variable_name in blocklist],
			...
			(["_secret", kanivin._dict, lambda *a, **kw: False], dict_printer),
		],
	)


def log(event, details):
	kanivin.logger(event).info(details)
...
def get_defaults(key=None):
	...
	"""
	return kanivin.db.get_defaults(key)
...
def set_default(key, val):
	...
	"""
	return kanivin.db.set_default(key, val)
...
def execute_in_shell(cmd, verbose=False, low_priority=False, check_exit_code=False):
	...
	if failed:
		raise kanivin.CommandFailedError(
...
def get_path(*path, **kwargs):
	...
	if not base:
		base = kanivin.local.site_path
	return os.path.join(base, *path)


def get_site_base_path():
	return kanivin.local.site_path
...
def get_bench_path():
	return os.environ.get("FRAPPE_BENCH_ROOT") or os.path.realpath(
		os.path.join(os.path.dirname(kanivin.__file__), "..", "..", "..")
	)


def get_bench_id():
	return kanivin.get_conf().get("bench_id", get_bench_path().strip("/").replace("/", "-"))


def get_site_id(site=None):
	return f"{site or kanivin.local.site}@{get_bench_id()}"
...
def get_test_client(use_cookies=True) -> Client:
	"""Returns an test instance of the Kanivin WSGI"""
	from kanivin.app import application

	return Client(application, use_cookies=use_cookies)


def get_hook_method(hook_name, fallback=None):
	method = kanivin.get_hooks().get(hook_name)
	if method:
		method = kanivin.get_attr(method[0])
		return method
	if fallback:
		return fallback
...
def call_hook_method(hook, *args, **kwargs):
	out = None
	for method_name in kanivin.get_hooks(hook):
		out = out or kanivin.get_attr(method_name)(*args, **kwargs)
...
def update_progress_bar(txt, i, l, absolute=False):
	...
	if not getattr(kanivin.local, "request", None) or is_cli():  # pragma: no cover
		lt = len(txt)
		try:
...
def get_html_format(print_path):
	...
	if os.path.exists(print_path):
		...
		for include_directive, path in INCLUDE_DIRECTIVE_PATTERN.findall(html_format):
			for app_name in kanivin.get_installed_apps():
				include_path = kanivin.get_app_path(app_name, *path.split(os.path.sep))
...
def get_sites(sites_path=None):
	if not sites_path:
		sites_path = getattr(kanivin.local, "sites_path", None) or "."
...
def markdown(text, sanitize=True, linkify=True):
	html = text if is_html(text) else kanivin.utils.md_to_html(text)
...
def get_installed_apps_info():
	out = []
	from kanivin.utils.change_log import get_versions
...
def get_site_info():
	from kanivin.email.queue import get_emails_sent_this_month
	from kanivin.utils.user import get_system_managers

	# only get system users
	users = kanivin.get_all(
		"User",
		filters={"user_type": "System User", "name": ("not in", kanivin.STANDARD_USERS)},
	...
	system_settings = kanivin.db.get_singles_dict("System Settings")
	space_usage = kanivin._dict((kanivin.local.conf.limits or {}).get("space_usage", {}))
	...
	site_info = {
		...
		"last_logins": kanivin.get_all("Activity Log", **kwargs),
	}

	# from other apps
	for method_name in kanivin.get_hooks("get_site_info"):
		site_info.update(kanivin.get_attr(method_name)(site_info) or {})

	# dumps -> loads to prevent datatype conflicts
	return json.loads(kanivin.as_json(site_info))
...
def get_db_count(*args):
	...
	Example:
	        via terminal:
	                bench --site erpnext.local execute kanivin.utils.get_db_count --args "['DocType', 'Communication']"
	"""
	db_count = {}
	for doctype in args:
		db_count[doctype] = kanivin.db.count(doctype)

	return json.loads(kanivin.as_json(db_count))
...
def call(fn, *args, **kwargs):
	...
	Pass a doctype or a series of doctypes to get the count of docs in them
	Parameters:
	        fn: kanivin function to be called
	...
	Example:
	        via terminal:
	                bench --site erpnext.local execute kanivin.utils.call --args '''["kanivin.get_all", "Activity Log"]''' --kwargs '''{"fields": ["user", "creation", "full_name"], "filters":{"Operation": "Login", "Status": "Success"}, "limit": "10"}'''
	"""
	return json.loads(kanivin.as_json(kanivin.call(fn, *args, **kwargs)))
...
def get_safe_filters(filters):
	try:
		...
		if isinstance(filters, int | float):
			filters = kanivin.as_unicode(filters)
...
def set_request(**kwargs):
	...
	kanivin.local.request = Request(builder.get_environ())
...
def get_html_for_route(route):
	...
	return kanivin.safe_decode(response.get_data())
...
def get_build_version():
	try:
		return str(os.path.getmtime(os.path.join(kanivin.local.sites_path, "assets/assets.json")))
	except OSError:
		# .build can sometimes not exist
		# this is not a major problem so send fallback
		return kanivin.utils.random_string(8)


def get_assets_json():
	def _get_assets():
		# get merged assets.json and assets-rtl.json
		assets = kanivin.parse_json(kanivin.read_file("assets/assets.json"))

		if assets_rtl := kanivin.read_file("assets/assets-rtl.json"):
			assets.update(kanivin.parse_json(assets_rtl))

		return assets

	if not hasattr(kanivin.local, "assets_json"):
		if not kanivin.conf.developer_mode:
			kanivin.local.assets_json = kanivin.cache.get_value(
				"assets_json",
				_get_assets,
				shared=True,
			)
			...
			kanivin.local.assets_json = _get_assets()

	return kanivin.local.assets_json
...
def add_user_info(user: str | list[str] | set[str], user_info: dict[str, _UserInfo]) -> None:
	...
	for info in missing_info:
		user_info.setdefault(info.name, kanivin._dict()).update(
...
class CallbackManager:
	...
	Example usage: kanivin.db.after_commit
	"""
