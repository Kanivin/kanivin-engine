...
from kanivin.database.database import Database, EmptyQueryValues


class KanivinIntegration(Integration):
	identifier = "kanivin"
...
def set_scope(scope):
	if job := rq.get_current_job():
		...
		context = kanivin._dict({"scheduled": False, "wait": 0})
		...
		if kanivin.form_dict.cmd:
			path = f"/api/method/{kanivin.form_dict.cmd}"
		else:
			path = kanivin.request.path
	...
	scope.set_user({"id": kanivin.local.site})
	user = getattr(kanivin.session, "user", "Unidentified")
	scope.set_tag("kanivin_user", user)
	# Extract `X-Kanivin-Request-ID` to store as a separate field if its present
	if trace_id := kanivin.monitor.get_trace_id():
		scope.set_tag("kanivin_trace_id", trace_id)


def set_sentry_context():
	if not kanivin.get_system_settings("enable_telemetry"):
		return
	...
	try:
		...
		with hub.configure_scope() as scope:
			if (
				...
				set_scope(scope)
			if kanivin.request:
				evt_processor = _make_wsgi_event_processor(kanivin.request.environ, False)
				scope.add_event_processor(evt_processor)
				if kanivin.request.is_json:
					scope.set_context("JSON Body", kanivin.request.json)
				elif kanivin.request.form:
					scope.set_context("Form Data", kanivin.request.form)

		if client := hub.client:
			exc_info = sys.exc_info()
			if any(exc_info):
				# Don't report errors which we can't "fix" in code
				if isinstance(exc_info[1], kanivin.ValidationError | kanivin.PermissionError):
					return
		...
		kanivin.logger().error("Failed to capture exception", exc_info=True)
