...
from kanivin.utils import cstr, scrub_urls
from kanivin.utils.caching import redis_cache
from kanivin.utils.jinja_globals import bundled_asset, is_rtl
...
def get_pdf(html, options=None, output: PdfWriter | None = None):
	...
	try:
		...
		if any([error in str(e) for error in PDF_CONTENT_ERRORS]):
			if not filedata:
				print(html, options)
				kanivin.throw(_("PDF generation failed because of broken image links"))
...
def prepare_options(html, options):
	...
	# page size
	pdf_page_size = (
		options.get("page-size") or kanivin.db.get_single_value("Print Settings", "pdf_page_size") or "A4"
	)

	if pdf_page_size == "Custom":
		options["page-height"] = options.get("page-height") or kanivin.db.get_single_value(
			"Print Settings", "pdf_page_height"
		)
		options["page-width"] = options.get("page-width") or kanivin.db.get_single_value(
...
def get_cookie_options():
	options = {}
	if kanivin.session and kanivin.session.sid and hasattr(kanivin.local, "request"):
		# Use wkhtmltopdf's cookie-jar feature to set cookies and restrict them to host domain
		cookiejar = f"/tmp/{kanivin.generate_hash()}.jar"

		# Remove port from request.host
		# https://werkzeug.palletsprojects.com/en/0.16.x/wrappers/#werkzeug.wrappers.BaseRequest.host
		domain = kanivin.utils.get_host_name().split(":", 1)[0]
		with open(cookiejar, "w") as f:
			f.write(f"sid={kanivin.session.sid}; Domain={domain};\n")
...
def _get_base64_image(src):
	...
	try:
		...
		kanivin.logger("pdf").error("Failed to convert inline images to base64", exc_info=True)
...
def prepare_header_footer(soup: BeautifulSoup):
	...
	css = kanivin.read_file(os.path.join(kanivin.local.sites_path, print_css))
	...
	for html_id in ("header-html", "footer-html"):
		if content := soup.find(id=html_id):
			...
			hook_func = kanivin.get_hooks(id_map.get(html_id))
			html = kanivin.call(
				hook_func[-1],
				soup=soup,
				head=head,
			...
			# create temp file
			fname = os.path.join("/tmp", f"kanivin-pdf-{kanivin.generate_hash()}.html")
			with open(fname, "wb") as f:
				f.write(html.encode("utf-8"))

			# {"header-html": "/tmp/kanivin-pdf-random.html"}
...
def get_wkhtmltopdf_version():
	wkhtmltopdf_version = kanivin.cache.hget("wkhtmltopdf_version", None)

	if not wkhtmltopdf_version:
		try:
			res = subprocess.check_output(["wkhtmltopdf", "--version"])
			wkhtmltopdf_version = res.decode("utf-8").split(" ")[1]
			kanivin.cache.hset("wkhtmltopdf_version", None, wkhtmltopdf_version)
		except Exception:
			pass
