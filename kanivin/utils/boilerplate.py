...
def make_boilerplate(dest, app_name, no_git=False):
	...
	app_name = kanivin.scrub(app_name)
...
def _create_app_boilerplate(dest, hooks, no_git=False):
	kanivin.create_folder(
		os.path.join(dest, hooks.app_name, hooks.app_name, kanivin.scrub(hooks.app_title)),
		with_init=True,
	)
	kanivin.create_folder(os.path.join(dest, hooks.app_name, hooks.app_name, "templates"), with_init=True)
	kanivin.create_folder(os.path.join(dest, hooks.app_name, hooks.app_name, "www"))
	kanivin.create_folder(
		os.path.join(dest, hooks.app_name, hooks.app_name, "templates", "pages"), with_init=True
	)
	kanivin.create_folder(os.path.join(dest, hooks.app_name, hooks.app_name, "templates", "includes"))
	kanivin.create_folder(os.path.join(dest, hooks.app_name, hooks.app_name, "config"), with_init=True)
	kanivin.create_folder(os.path.join(dest, hooks.app_name, hooks.app_name, "public", "css"))
	kanivin.create_folder(os.path.join(dest, hooks.app_name, hooks.app_name, "public", "js"))
	...
	with open(os.path.join(dest, hooks.app_name, hooks.app_name, "__init__.py"), "w") as f:
		f.write(kanivin.as_unicode(init_template))

	with open(os.path.join(dest, hooks.app_name, "pyproject.toml"), "w") as f:
		f.write(kanivin.as_unicode(pyproject_template.format(**hooks)))
	...
	with open(os.path.join(dest, hooks.app_name, "license.txt"), "w") as f:
		f.write(kanivin.as_unicode(license_body))

	with open(os.path.join(dest, hooks.app_name, hooks.app_name, "modules.txt"), "w") as f:
		f.write(kanivin.as_unicode(hooks.app_title))
	...
	with open(os.path.join(dest, hooks.app_name, hooks.app_name, "hooks.py"), "w") as f:
		f.write(kanivin.as_unicode(hooks_template.format(**hooks)))

	with open(os.path.join(dest, hooks.app_name, hooks.app_name, "patches.txt"), "w") as f:
		f.write(kanivin.as_unicode(patches_template.format(**hooks)))
	...
	if not no_git:
		with open(os.path.join(dest, hooks.app_name, ".gitignore"), "w") as f:
			f.write(kanivin.as_unicode(gitignore_template.format(app_name=hooks.app_name)))
...
class PatchCreator:
	def __init__(self):
		self.all_apps = kanivin.get_all_apps(sites_path=".", with_internal_apps=False)
	...
	def _ask_app_name(self):
		...
		self.app_dir = pathlib.Path(kanivin.get_app_path(self.app))
	...
	def _ask_patch_meta_info(self):
		...
		default_filename = kanivin.scrub(self.docstring) + ".py"
...
dynamic = ["version"]
dependencies = [
    # "kanivin~=15.0.0" # Installed and managed by bench.
]
...
# Desk Notifications
# ------------------
# See kanivin.core.notifications.get_notification_config
...
# 	"kanivin.desk.doctype.event.event.get_events": "{app_name}.event.get_events"
# }}
#
# each overriding function accepts a `data` argument;
# generated from the base implementation of the doctype dashboard,
# along with any modifications made in other Kanivin apps
    ...
    steps:
      ...
      - name: Setup
        run: |
          ...
          bench init --skip-redis-config-generation --skip-assets --python "$(which python)" ~/kanivin-bench
          mariadb --host 127.0.0.1 --port 3306 -u root -proot -e "SET GLOBAL character_set_server = 'utf8mb4'"
          mariadb --host 127.0.0.1 --port 3306 -u root -proot -e "SET GLOBAL collation_server = 'utf8mb4_unicode_ci'"

      - name: Install
        working-directory: /home/runner/kanivin-bench
        run: |
...
# Read docs to understand patches: https://kanivinframework.com/docs/v14/user/en/database-migrations
