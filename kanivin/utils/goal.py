...
from kanivin.query_builder.functions import DateFormat, Function
from kanivin.query_builder.utils import DocType
from kanivin.utils.data import add_to_date, cstr, flt, now_datetime
...
def get_monthly_results(
	...
	date_format = "%m-%Y" if kanivin.db.db_type != "postgres" else "MM-YYYY"
...
def get_monthly_goal_graph_data(
	...
	if history and "{" in cstr(history):
		with suppress(ValueError):
			month_to_value_dict = kanivin.parse_json(history)

	if month_to_value_dict is None:  # nosemgrep
		doc_filter = {}
		with suppress(ValueError):
			doc_filter = kanivin.parse_json(filters or "{}")
