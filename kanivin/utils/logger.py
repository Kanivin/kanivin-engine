...
default_log_level = logging.WARNING if kanivin._dev_server else logging.ERROR
...
def get_logger(
	...
	if allow_site is True:
		site = getattr(kanivin.local, "site", None)
	...
	try:
		return kanivin.loggers[logger_name]
	except KeyError:
		pass
	...
	logger.setLevel(kanivin.log_level or default_log_level)
	...
	kanivin.loggers[logger_name] = logger
...
class SiteContextFilter(logging.Filter):
	...
	def filter(self, record) -> bool:
		if "Form Dict" not in str(record.msg):
			site = getattr(kanivin.local, "site", None)
			form_dict = sanitized_dict(getattr(kanivin.local, "form_dict", None))
...
def set_log_level(level: Literal["ERROR", "WARNING", "WARN", "INFO", "DEBUG"]) -> None:
	...
	kanivin.log_level = getattr(logging, (level or "").upper(), None) or default_log_level
	kanivin.loggers = {}
