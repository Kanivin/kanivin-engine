...
def main():
	...
	commands.update({"get-kanivin-commands": get_kanivin_commands, "get-kanivin-help": get_kanivin_help})
	click.Group(commands=commands)(prog_name="bench")


def get_app_groups() -> dict[str, click.Group]:
	"""Get all app groups, put them in main group "kanivin" since bench is
	designed to only handle that"""
	commands = {}
	for app in get_apps():
		if app_commands := get_app_commands(app):
			commands |= app_commands
	return dict(kanivin=click.group(name="kanivin", commands=commands)(app_group))
...
def app_group(ctx, site=False, force=False, verbose=False, profile=False):
	...
	if ctx.info_name == "kanivin":
		ctx.info_name = ""


def get_sites(site_arg: str) -> list[str]:
	if site_arg == "all":
		return kanivin.utils.get_sites()
	elif site_arg:
		return [site_arg]
	elif os.environ.get("FRAPPE_SITE"):
		return [os.environ.get("FRAPPE_SITE")]
	elif default_site := kanivin.get_conf().default_site:
		return [default_site]
	# This is not supported, just added here for warning.
	elif (site := kanivin.read_file("currentsite.txt")) and site.strip():
		click.secho(
			dedent(
				f"""
...
@click.command("get-kanivin-commands")
def get_kanivin_commands():
	commands = list(get_app_commands("kanivin"))
...
@click.command("get-kanivin-help")
def get_kanivin_help():
	print(click.Context(get_app_groups()["kanivin"]).get_help())


def get_apps():
	return kanivin.get_all_apps(with_internal_apps=False, sites_path=".")


if __name__ == "__main__":
	if not kanivin._dev_server:
