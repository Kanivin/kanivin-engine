...
def get_first_day_of_the_week() -> str:
	return kanivin.get_system_settings("first_day_of_the_week") or "Sunday"
...
def getdate(
	...
	try:
		...
		kanivin.throw(
			kanivin._("{} is not a valid date string.").format(kanivin.bold(string_date)),
			title=kanivin._("Invalid Date"),
		)
...
def add_to_date(
	...
	if isinstance(date, str):
		...
		try:
			...
			kanivin.throw(kanivin._("Please select a valid date filter"), title=kanivin._("Invalid Date"))
...
def get_system_timezone() -> str:
	...
	return kanivin.get_system_settings("time_zone") or "Asia/Kolkata"  # Default to India ?!
...
def now() -> str:
	...
	if kanivin.flags.current_date:
		return (
			getdate(kanivin.flags.current_date).strftime(DATE_FORMAT)
			+ " "
...
def get_user_date_format() -> str:
	...
	if getattr(kanivin.local, "user_date_format", None) is None:
		kanivin.local.user_date_format = kanivin.db.get_default("date_format")

	return kanivin.local.user_date_format or "yyyy-mm-dd"
...
def get_user_time_format() -> str:
	...
	if getattr(kanivin.local, "user_time_format", None) is None:
		kanivin.local.user_time_format = kanivin.db.get_default("time_format")

	return kanivin.local.user_time_format or "HH:mm:ss"
	...
	* dd/mm/yyyy
	...
	try:
		formatted_date = babel.dates.format_date(
			date, format_string, locale=(kanivin.local.lang or "").replace("-", "_")
		)
	...
	* HH:mm
	...
	try:
		formatted_time = babel.dates.format_time(
			time_, format_string, locale=(kanivin.local.lang or "").replace("-", "_")
		)
	except (UnknownLocaleError, ValueError):
	...
	* mm-dd-yyyy HH:mm
	...
	try:
		formatted_datetime = babel.dates.format_datetime(
			datetime, format_string, locale=(kanivin.local.lang or "").replace("-", "_")
		)
	except (UnknownLocaleError, ValueError):
...
def validate_duration_format(duration):
	if not DURATION_PATTERN.match(duration):
		kanivin.throw(
			kanivin._("Value {0} must be in the valid duration format: d h m s").format(kanivin.bold(duration))
		)
...
def global_date_format(date, format="long"):
	...
	date = getdate(date)
	return babel.dates.format_date(date, locale=(kanivin.local.lang or "en").replace("-", "_"), format=format)
...
def cast_fieldtype(fieldtype, value, show_warning=True):
	if show_warning:
		message = (
			"Function `kanivin.utils.data.cast_fieldtype` has been deprecated in favour"
			" of `kanivin.utils.data.cast`. Use the newer util for safer type casting."
		)
...
def cast(fieldtype, value=None):
	"""Cast the value to the Python native object of the Kanivin fieldtype provided.
	...
	Mapping of Python types => Kanivin types:
...
def flt(s: NumericType | str, precision: int | None = None, rounding_method: str | None = None) -> float:
	...
	try:
		...
		if isinstance(e, kanivin.InvalidRoundingMethod):
			raise
		num = 0.0
...
def cstr(s, encoding="utf-8"):
	return kanivin.as_unicode(s, encoding)
...
def rounded(num, precision=0, rounding_method=None):
	...
	rounding_method = (
		rounding_method or kanivin.get_system_settings("rounding_method") or "Banker's Rounding (legacy)"
	)
	...
	if rounding_method == "Banker's Rounding":
		...
		kanivin.throw(
			kanivin._("Unknown Rounding Method: {}").format(rounding_method),
			exc=kanivin.InvalidRoundingMethod,
		)
...
def round_based_on_smallest_currency_fraction(value, currency, precision=2):
	smallest_currency_fraction_value = flt(
		kanivin.db.get_value("Currency", currency, "smallest_currency_fraction_value", cache=True)
	)
...
def fmt_money(
	...
	"""
	number_format = format or kanivin.db.get_default("number_format") or "#,###.##"
	if precision is None:
		precision = cint(kanivin.db.get_default("currency_precision")) or None
	...
	if decimal_str:
		...
		if precision > 2:
			if len(decimals) < 3:
				if currency:
					fraction = kanivin.db.get_value("Currency", currency, "fraction_units", cache=True) or 100
	...
	if currency and kanivin.defaults.get_global_default("hide_currency_symbol") != "Yes":
		symbol = kanivin.db.get_value("Currency", currency, "symbol", cache=True) or currency
		symbol_on_right = kanivin.db.get_value("Currency", currency, "symbol_on_right", cache=True)

		if symbol_on_right:
			amount = f"{amount} {kanivin._(symbol)}"
		else:
			amount = f"{kanivin._(symbol)} {amount}"
...
def money_in_words(
	...
	"""
	from kanivin.utils import get_defaults
	...
	if not fraction_currency:
		fraction_currency = kanivin.db.get_value("Currency", main_currency, "fraction", cache=True) or _(
			"Cent"
		)

	number_format = (
		kanivin.db.get_value("Currency", main_currency, "number_format", cache=True)
		or kanivin.db.get_default("number_format")
		or "#,###.##"
	)
...
def in_words(integer: int, in_million=True) -> str:
	...
	locale = "en_IN" if not in_million else kanivin.local.lang
	integer = int(integer)
	try:
...
def get_thumbnail_base64_for_image(src):
	...
	from kanivin import cache, safe_decode
	from kanivin.core.doctype.file.utils import get_local_image

	if not src:
		kanivin.throw(f"Invalid source for image: {src}")
	...
	def _get_base64():
		file_path = kanivin.get_site_path("public", src.lstrip("/"))
...
def pdf_to_base64(filename: str) -> bytes | None:
	from kanivin.utils.file_manager import get_file_path
...
def pretty_date(iso_datetime: datetime.datetime | str) -> str:
	...
	locale = kanivin.local.lang.replace("-", "_") if kanivin.local.lang else None
	return format_timedelta(iso_datetime - now_dt, add_direction=True, locale=locale)


def comma_or(some_list, add_quotes=True):
	return comma_sep(some_list, kanivin._("{0} or {1}"), add_quotes)


def comma_and(some_list, add_quotes=True):
	return comma_sep(some_list, kanivin._("{0} and {1}"), add_quotes)
...
def comma_sep(some_list, pattern, add_quotes=True):
	if isinstance(some_list, list | tuple):
		...
		if not some_list:
			...
			return pattern.format(", ".join(kanivin._(s) for s in some_list[:-1]), some_list[-1])
	else:
		return some_list
...
def get_url(
	...
	host_name = kanivin.local.conf.host_name or kanivin.local.conf.hostname
	...
	if not host_name:
		...
		elif kanivin.local.site:
			protocol = "http://"

			if kanivin.local.conf.ssl_certificate:
				protocol = "https://"

			elif kanivin.local.conf.wildcard:
				domain = kanivin.local.conf.wildcard.get("domain")
				if (
					domain
					and kanivin.local.site.endswith(domain)
					and kanivin.local.conf.wildcard.get("ssl_certificate")
				):
					protocol = "https://"

			host_name = protocol + kanivin.local.site

		else:
			host_name = kanivin.db.get_single_value("Website Settings", "subdomain")
	...
	if not uri and full_address:
		uri = kanivin.get_request_header("REQUEST_URI", "")

	port = kanivin.conf.http_port or kanivin.conf.webserver_port

	if (
		not (kanivin.conf.restart_supervisor_on_update or kanivin.conf.restart_systemd_on_update)
		and host_name
...
def get_host_name_from_request() -> str:
	if hasattr(kanivin.local, "request") and kanivin.local.request and kanivin.local.request.host:
		protocol = "https://" if "https" == kanivin.get_request_header("X-Forwarded-Proto", "") else "http://"
		return protocol + kanivin.local.request.host
...
def filter_operator_is(value: str, pattern: str) -> bool:
	...
	if pattern == "set":
		...
		kanivin.throw(kanivin._(f"Invalid argument for operator 'IS': {pattern}"))
...
def get_filter(doctype: str, f: dict | list | tuple, filters_config=None) -> "kanivin._dict":
	...
	"""
	from kanivin.database.utils import NestedSetHierarchy
	from kanivin.model import child_table_fields, default_fields, optional_fields
	...
	if not isinstance(f, list | tuple):
		kanivin.throw(kanivin._("Filter must be a tuple or list (in a list)"))
	...
	if len(f) == 3:
		...
		kanivin.throw(
			kanivin._("Filter must have 4 values (doctype, fieldname, operator, value): {0}").format(str(f))
		)

	f = kanivin._dict(doctype=f[0], fieldname=f[1], operator=f[2], value=f[3])
	...
	if f.operator.lower() not in valid_operators:
		kanivin.throw(kanivin._("Operator must be one of {0}").format(", ".join(valid_operators)))
	...
	if f.doctype and (f.fieldname not in default_fields + optional_fields + child_table_fields):
		...
		meta = kanivin.get_meta(f.doctype)
		if not meta.has_field(f.fieldname):
			# try and match the doctype name from child tables
			for df in meta.get_table_fields():
				if kanivin.get_meta(df.options).has_field(f.fieldname):
					f.doctype = df.options
					break

	try:
		df = kanivin.get_meta(f.doctype).get_field(f.fieldname) if f.doctype else None
	except kanivin.exceptions.DoesNotExistError:
		df = None
...
def sanitize_column(column_name: str) -> str:
	return _sanitize_column(column_name, (kanivin.db and kanivin.db.db_type) or None)
...
def _sanitize_column(column_name: str, db_type: str) -> str:
	...
	def _raise_exception():
		kanivin.throw(_("Invalid field name {0}").format(column_name), kanivin.DataError)
...
def generate_hash(*args, **kwargs) -> str:
	return kanivin.generate_hash(*args, **kwargs)
...
def get_user_info_for_avatar(user_id: str) -> _UserInfo:
	try:
		user = kanivin.get_cached_doc("User", user_id)
...
def validate_python_code(string: str, fieldname: str | None = None, is_expression: bool = True) -> None:
	...
	try:
		...
		msg = kanivin._("{} Invalid python code on line {}").format(
			fieldname + ":" if fieldname else "", line_no + 1
		)
		msg += f"<br><pre>{error_line}</pre>"
		msg += f"<pre>{' ' * offset}^</pre>"

		kanivin.throw(msg, title=kanivin._("Syntax Error"))
	except Exception as e:
		kanivin.msgprint(
			kanivin._("{} Possibly invalid python code. <br>{}").format(fieldname + ": " or "", str(e)),
			indicator="orange",
		)
...
def format_timedelta(o: datetime.timedelta | str) -> str:
	...
	# but Kanivin doesn't - I think via babel : only allows 0..23 range for hour
...
def is_site_link(link: str) -> bool:
	...
	if link.startswith("/"):
		return True
	return urlparse(link).netloc == urlparse(kanivin.utils.get_url()).netloc
