...
from kanivin.www.printview import validate_print_permission
...
def download_multi_pdf_async(
	...
	kanivin.local.response["http_status_code"] = http.HTTPStatus.CREATED
...
def _download_multi_pdf(
	...
	if not isinstance(doctype, dict):
		...
		for idx, ss in enumerate(result):
			try:
				...
				if task_id:
					kanivin.publish_realtime(task_id=task_id, message={"message": "Failed"})
		...
		if task_id is None:
			kanivin.local.response.filename = "{doctype}.pdf".format(
		...
		for doctype_name in doctype:
			...
			for doc_name in doctype[doctype_name]:
				try:
					...
					if task_id:
						kanivin.publish_realtime(task_id=task_id, message="Failed")
					kanivin.log_error(
		...
		if task_id is None:
			kanivin.local.response.filename = f"{name}.pdf"
	...
	with BytesIO() as merged_pdf:
		...
		if task_id:
			...
			_file.save()
			kanivin.publish_realtime(f"task_complete:{task_id}", message={"file_url": _file.unique_url})
		else:
			kanivin.local.response.filecontent = merged_pdf.getvalue()
...
def download_pdf(
	...
	doc = doc or kanivin.get_doc(doctype, name)
	validate_print_permission(doc)

	with print_language(language):
		pdf_file = kanivin.get_print(
			doctype,
			name,
			format,
			doc=doc,
	...
	kanivin.local.response.filecontent = pdf_file
	kanivin.local.response.type = "pdf"
...
def report_to_pdf(html, orientation="Landscape"):
	...
	kanivin.local.response.filename = "report.pdf"
	kanivin.local.response.filecontent = get_pdf(html, {"orientation": orientation})
	kanivin.local.response.type = "pdf"
...
def print_by_server(
	...
	print_settings = kanivin.get_doc("Network Printer Settings", printer_setting)
	try:
		import cups
	except ImportError:
		kanivin.throw(_("You need to install pycups to use this feature!"))
	...
	try:
		...
		output = PdfWriter()
		output = kanivin.get_print(
			doctype, name, print_format, doc=doc, no_letterhead=no_letterhead, as_pdf=True, output=output
		)
		if not file_path:
			file_path = os.path.join("/", "tmp", f"kanivin-pdf-{kanivin.generate_hash()}.pdf")
		...
		if (
			...
			kanivin.throw(_("PDF generation failed"))
	except cups.IPPError:
		kanivin.throw(_("Printing failed"))
