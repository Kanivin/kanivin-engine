...
from kanivin.boot import get_allowed_reports
from kanivin.core.doctype.domain_settings.domain_settings import get_active_modules
from kanivin.permissions import AUTOMATIC_ROLES, get_roles, get_valid_perms
from kanivin.query_builder import DocType, Order
...
class UserPermissions:
	"""
	A user permission object can be accessed as `kanivin.get_user()`
	"""

	def __init__(self, name=""):
		self.defaults = None
		self.name = name or kanivin.session.get("user")
		self.roles = []
	...
	def setup_user(self):
		def get_user_doc():
			user = None
			try:
				user = kanivin.get_doc("User", self.name).as_dict()
			except kanivin.DoesNotExistError:
				pass
			except Exception as e:
				# install boo-boo
				if not kanivin.db.is_table_missing(e):
					raise

			return user

		if not kanivin.flags.in_install_db and not kanivin.flags.in_test:
			user_doc = kanivin.cache.hget("user_doc", self.name, get_user_doc)
			if user_doc:
				self.doc = kanivin.get_doc(user_doc)
	...
	def build_perm_map(self):
		...
		for r in get_valid_perms():
			...
			for k in kanivin.permissions.rights:
	...
	def build_permissions(self):
		...
		self.build_perm_map()
		user_shared = kanivin.share.get_shared_doctypes()
		no_list_view_link = []
		...
		if "System Manager" in self.get_roles():
			self.can_import += kanivin.get_all("DocType", {"allow_import": 1}, pluck="name")
			self.can_import += kanivin.get_all(
				"Property Setter",
				pluck="doc_type",
				filters={"property": "allow_import", "value": "1"},
			)

		kanivin.cache.hset("can_import", kanivin.session.user, self.can_import)

	def get_defaults(self):
		import kanivin.defaults

		self.defaults = kanivin.defaults.get_defaults(self.name)
	...
	def load_user(self):
		...
		if d.get("default_workspace"):
			workspace = kanivin.get_cached_doc("Workspace", d.default_workspace)
			d.default_workspace = {
				"name": workspace.name,
		...
		d.name = self.name
		d.onboarding_status = kanivin.parse_json(d.onboarding_status)
...
def get_fullname_and_avatar(user: str) -> _dict:
	first_name, last_name, avatar, name = kanivin.db.get_value(
...
def get_system_managers(only_name: bool = False) -> list[str]:
	...
	system_managers = (
		...
		.where(
			...
			& (User.docstatus < 2)
			& (User.name.notin(kanivin.STANDARD_USERS))
		)
		.select(*fields)
...
def add_role(user: str, role: str) -> None:
	kanivin.get_doc("User", user).add_roles(role)
...
def is_website_user(username: str | None = None) -> str | None:
	return kanivin.db.get_value("User", username or kanivin.session.user, "user_type") == "Website User"
...
def is_system_user(username: str | None = None) -> str | None:
	...
	return kanivin.db.get_value(
		"User",
		{
			"email": username or kanivin.session.user,
			"enabled": 1,
			"user_type": "System User",
		},
		cache=True,
