...
from kanivin.utils import cint, get_file_size, get_url, now, now_datetime
...
class BackupGenerator:
	...
	def __init__(
		...
		site = kanivin.local.site or kanivin.generate_hash(length=8)
	...
	def setup_backup_tables(self):
		...
		existing_tables = kanivin.db.get_tables()
		...
		specified_tables = get_tables(kanivin.conf.get("backup", {}).get("includes", []))
		include_tables = (specified_tables + base_tables) if specified_tables else []

		conf_tables = {
			"include": include_tables,
			"exclude": get_tables(kanivin.conf.get("backup", {}).get("excludes", [])),
		}
	...
	def get_backup(self, older_than=24, ignore_files=False, force=False):
		...
		if not (last_db and last_file and last_private_file and site_config_backup_path):
			...
			if kanivin.get_system_settings("encrypt_backup"):
	...
	def set_backup_file_name(self):
		...
		enc = "-enc" if kanivin.get_system_settings("encrypt_backup") else ""
		...
		for path in paths:
			if os.path.exists(path):
				...
				try:
					...
					kanivin.utils.execute_in_shell(command)
	...
	def print_summary(self):
		...
		print(f"Backup Summary for {kanivin.local.site} at {now()}")
	...
	def backup_files(self):
		for folder in ("public", "private"):
			files_path = kanivin.get_site_path(folder, "files")
			...
			try:
				kanivin.utils.execute_in_shell(
					cmd_string.format(backup_path, files_path),
					verbose=self.verbose,
					low_priority=True,
					check_exit_code=True,
				)
			except kanivin.CommandFailedError as e:
	...
	def copy_site_config(self):
		...
		site_config_path = os.path.join(kanivin.get_site_path(), "site_config.json")
	...
	def take_dump(self):
		...
		if not gzip_exc:
			kanivin.throw(
				_("gzip not found in PATH! This is required to take a backup."), exc=kanivin.ExecutableNotFound
			)

		if self.old_backup_metadata:
			database_header_content = [
				f"Backup generated by Kanivin {kanivin.__version__} on branch {get_app_branch('kanivin') or 'N/A'}",
				"",
			]
		else:
			database_header_content = [
				"begin kanivin metadata",
				"[kanivin]",
				f"version = {kanivin.__version__}",
				f"branch = {get_app_branch('kanivin') or 'N/A'}",
				"end kanivin metadata",
				"",
			]
		...
		if self.partial:
			...
			database_header_content.extend(
				[
					f"Partial Backup of Kanivin Site {kanivin.local.site}",
		...
		kanivin.utils.execute_in_shell(command, low_priority=True, check_exit_code=True)

	def send_email(self):
		"""
		Sends the link to backup file located at erpnext/backups
		"""
		from kanivin.email import get_system_managers
...
download only after 24 hours."""
		...
		kanivin.sendmail(recipients=recipient_list, message=msg, subject=subject)
...
def delete_temp_backups(older_than=24):
	...
	"""
	older_than = cint(kanivin.conf.keep_backups_for_hours) or older_than
	backup_path = get_backup_path()
...
def get_backup_path():
	return kanivin.utils.get_site_path(conf.get("backup_path", "private/backups"))
...
def get_or_generate_backup_encryption_key():
	...
	key = kanivin.conf.get(BACKUP_ENCRYPTION_CONFIG_KEY)
	if key:
		return key
