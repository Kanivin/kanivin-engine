...
from kanivin.query_builder.functions import Coalesce, Max
from kanivin.query_builder.terms import SubQuery
from kanivin.query_builder.utils import DocType


class NestedSetRecursionError(kanivin.ValidationError):
	pass


class NestedSetMultipleRootsError(kanivin.ValidationError):
	pass


class NestedSetChildExistsError(kanivin.ValidationError):
	pass


class NestedSetInvalidMergeError(kanivin.ValidationError):
	pass
...
def update_nsm(doc):
	...
	parent_field = "parent_" + kanivin.scrub(doc.doctype)
	...
	kanivin.db.set_value(doc.doctype, doc.name, old_parent_field, parent or "", update_modified=False)
	kanivin.clear_document_cache(doc.doctype)
...
def update_add_node(doc, parent, parent_field):
	...
	if parent:
		left, right = kanivin.db.get_value(doctype, {"name": parent}, ["lft", "rgt"], for_update=True)
	...
	kanivin.qb.update(Table).set(Table.rgt, Table.rgt + 2).where(Table.rgt >= right).run()
	kanivin.qb.update(Table).set(Table.lft, Table.lft + 2).where(Table.lft >= right).run()

	if kanivin.qb.from_(Table).select("*").where((Table.lft == right) | (Table.rgt == right + 1)).run():
		kanivin.throw(_("Nested set error. Please contact the Administrator."))

	# update index of new node
	kanivin.qb.update(Table).set(Table.lft, right).set(Table.rgt, right + 1).where(Table.name == name).run()
	return right
...
def update_move_node(doc: Document, parent_field: str):
	...
	# move to dark side
	kanivin.qb.update(Table).set(Table.lft, -Table.lft).set(Table.rgt, -Table.rgt).where(
		(Table.lft >= doc.lft) & (Table.rgt <= doc.rgt)
	).run()

	# shift left
	diff = doc.rgt - doc.lft + 1
	kanivin.qb.update(Table).set(Table.lft, Table.lft - diff).set(Table.rgt, Table.rgt - diff).where(
		Table.lft > doc.rgt
	).run()

	# shift left rgts of ancestors whose only rgts must shift
	kanivin.qb.update(Table).set(Table.rgt, Table.rgt - diff).where(
		(Table.lft < doc.lft) & (Table.rgt > doc.rgt)
	).run()
	...
	if parent:
		...
		# set parent lft, rgt
		kanivin.qb.update(Table).set(Table.rgt, Table.rgt + diff).where(Table.name == parent).run()
		...
		# new root
		max_rgt = kanivin.qb.from_(Table).select(Max(Table.rgt)).run(pluck=True)[0]
		new_diff = max_rgt + 1 - doc.lft

	# bring back from dark side
	kanivin.qb.update(Table).set(Table.lft, -Table.lft + new_diff).set(Table.rgt, -Table.rgt + new_diff).where(
		Table.lft < 0
	).run()
...
def rebuild_tree(doctype, parent_field=None):
	...
	if kanivin.request and kanivin.local.form_dict.cmd == "rebuild_tree":
		kanivin.only_for("System Manager")

	meta = kanivin.get_meta(doctype)
	if not meta.has_field("lft") or not meta.has_field("rgt"):
		kanivin.throw(
			_("Rebuilding of tree is not supported for {}").format(kanivin.bold(doctype)),
			title=_("Invalid Action"),
		)

	parent_field = meta.nsm_parent_field or f"parent_{kanivin.scrub(doctype)}"
	...
	kanivin.db.auto_commit_on_many_writes = 0
...
def rebuild_node(doctype, parent, left, parent_field):
	...
	result = (kanivin.qb.from_(table).where(column == parent).select(table.name)).run()
	...
	kanivin.db.set_value(doctype, parent, {"lft": left, "rgt": right}, update_modified=False)
...
def validate_loop(doctype, name, lft, rgt):
	...
	if name in kanivin.get_all(doctype, filters={"lft": ["<=", lft], "rgt": [">=", rgt]}, pluck="name"):
		kanivin.throw(_("Item cannot be added to its own descendants"), NestedSetRecursionError)
...
def remove_subtree(doctype: str, name: str, throw=True):
	...
	kanivin.qb.update(table).set(table.lft, table.lft - width).where(table.lft > rgt).run()
	kanivin.qb.update(table).set(table.rgt, table.rgt - width).where(table.rgt > rgt).run()

	kanivin.clear_document_cache(doctype)
...
class NestedSet(Document):
	...
	def on_trash(self, allow_root_deletion=False):
		...
		if not getattr(self, "nsm_parent_field", None):
			self.nsm_parent_field = kanivin.scrub(self.doctype) + "_parent"

		parent = self.get(self.nsm_parent_field)
		if not parent and not getattr(self, "allow_root_deletion", True):
			kanivin.throw(_("Root {0} cannot be deleted").format(_(self.doctype)))
	...
	def validate_if_child_exists(self):
		has_children = kanivin.db.count(self.doctype, filters={self.nsm_parent_field: self.name})
		if has_children:
			kanivin.throw(
	...
	def before_rename(self, olddn, newdn, merge=False, group_fname="is_group"):
		if merge and hasattr(self, group_fname):
			is_group = kanivin.db.get_value(self.doctype, newdn, group_fname)
	...
	def validate_one_root(self):
		if not self.get(self.nsm_parent_field):
			if self.get_root_node_count() > 1:
				kanivin.throw(_("""Multiple root nodes not allowed."""), NestedSetMultipleRootsError)

	def get_root_node_count(self):
		return kanivin.db.count(self.doctype, {self.nsm_parent_field: ""})
	...
	def validate_ledger(self, group_identifier="is_group"):
		if hasattr(self, group_identifier) and not bool(self.get(group_identifier)):
			if kanivin.get_all(self.doctype, {self.nsm_parent_field: self.name, "docstatus": ("!=", 2)}):
				kanivin.throw(
	...
	def get_parent(self) -> "NestedSet":
		...
		if parent_name:
			return kanivin.get_doc(self.doctype, parent_name)

	def get_children(self) -> Iterator["NestedSet"]:
		"""Return a generator that yields child Documents."""
		child_names = kanivin.get_list(self.doctype, filters={self.nsm_parent_field: self.name}, pluck="name")
		for name in child_names:
			yield kanivin.get_doc(self.doctype, name)
...
def get_root_of(doctype):
	...
	node_query = SubQuery(kanivin.qb.from_(t2).select(Count("*")).where((t2.lft < t1.lft) & (t2.rgt > t1.rgt)))
	result = kanivin.qb.from_(t1).select(t1.name).where((node_query == 0) & (t1.rgt > t1.lft)).run()
