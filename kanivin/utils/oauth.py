...
class SignupDisabledError(kanivin.PermissionError):
	...


def get_oauth2_providers() -> dict[str, dict]:
	out = {}
	providers = kanivin.get_all("Social Login Key", fields=["*"])
...
def get_oauth_keys(provider: str) -> dict[str, str]:
	...
	if keys := kanivin.conf.get(f"{provider}_login"):
		return {"client_id": keys["client_id"], "client_secret": keys["client_secret"]}

	return {
		"client_id": kanivin.db.get_value("Social Login Key", provider, "client_id"),
...
def get_oauth2_authorize_url(provider: str, redirect_to: str) -> str:
	...
	state = {
		"site": kanivin.utils.get_url(),
		"token": kanivin.generate_hash(),
...
def get_redirect_uri(provider: str) -> str:
	keys = kanivin.conf.get(f"{provider}_login")
	...
	return kanivin.utils.get_url(redirect_uri)
...
def get_info_via_oauth(provider: str, code: str, decoder: Callable | None = None, id_token: bool = False):
	...
	if not (info.get("email_verified") or info.get("email")):
		kanivin.throw(_("Email not verified with {0}").format(provider.title()))
...
def login_oauth_user(
	...
	*,
	...
	if not (state and state["token"]):
		kanivin.respond_as_web_page(_("Invalid Request"), _("Token is missing"), http_status_code=417)
		return
	...
	try:
		...
		return kanivin.respond_as_web_page(
			"Signup is Disabled",
	...
	if kanivin.utils.cint(generate_login_token):
		login_token = kanivin.generate_hash(length=32)
		kanivin.cache.set_value(f"login_token:{login_token}", kanivin.local.session.sid, expires_in_sec=120)

		kanivin.response["login_token"] = login_token

	else:
		redirect_to = state.get("redirect_to")
		redirect_post_login(
			desk_user=kanivin.local.response.get("message") == "Logged In",
			redirect_to=redirect_to,
			provider=provider,
		)
...
def get_user_record(user: str, data: dict, provider: str) -> "User":
	from kanivin.integrations.doctype.social_login_key.social_login_key import provider_allows_signup

	try:
		return kanivin.get_doc("User", user)
	except kanivin.DoesNotExistError:
		if not provider_allows_signup(provider):
			raise SignupDisabledError

	user: "User" = kanivin.new_doc("User")
	...
	user.update(
		{
			...
			"gender": gender,
			"enabled": 1,
			"new_password": kanivin.generate_hash(),
...
def update_oauth_user(user: str, data: dict, provider: str):
	...
	if not user.enabled:
		kanivin.respond_as_web_page(_("Not Allowed"), _("User {0} is disabled").format(user.email))
		return False
	...
	if update_user_record:
		...
		if default_role := kanivin.db.get_single_value("Portal Settings", "default_role"):
...
def redirect_post_login(desk_user: bool, redirect_to: str | None = None, provider: str | None = None):
	...
	if not redirect_to:
		...
		redirect_to = kanivin.utils.get_url(desk_uri if desk_user else "/me")

	kanivin.local.response["location"] = redirect_to
