...
def inspect(var, render=True):
	from kanivin.utils.jinja import get_jenv
...
def web_blocks(blocks):
	import kanivin
	from kanivin import _, _dict, throw
	from kanivin.website.doctype.web_page.web_page import get_web_blocks_html
	...
	if not kanivin.flags.web_block_scripts:
...
def include_script(path, preload=True):
	...
	if preload:
		...
		kanivin.local.preload_assets["script"].append(path)
...
def include_style(path, rtl=None, preload=True):
	...
	if preload:
		...
		kanivin.local.preload_assets["style"].append(path)

	return f'<link type="text/css" rel="stylesheet" href="{path}">'


def bundled_asset(path, rtl=None):
	from kanivin.utils import get_assets_json
	from kanivin.website.utils import abs_url
