...
import kanivin
import kanivin.database
import kanivin.utils
import kanivin.utils.user
...
from kanivin.sessions import Session, clear_sessions, delete_session, get_expiry_in_seconds
from kanivin.translate import get_language
from kanivin.twofactor import (
	authenticate_for_2factor,
	confirm_otp_token,
	get_cached_user_pass,
	should_run_2fa,
)
from kanivin.utils import cint, date_diff, datetime, get_datetime, today
from kanivin.utils.deprecations import deprecation_warning
from kanivin.utils.password import check_password, get_decrypted_password
...
class HTTPRequest:
	def __init__(self):
		...
		kanivin.local.cookie_manager.init_cookies()

	@property
	def domain(self):
		if not getattr(self, "_domain", None):
			self._domain = kanivin.request.host
	...
	def set_request_ip(self):
		if kanivin.get_request_header("X-Forwarded-For"):
			kanivin.local.request_ip = (kanivin.get_request_header("X-Forwarded-For").split(",", 1)[0]).strip()

		elif kanivin.get_request_header("REMOTE_ADDR"):
			kanivin.local.request_ip = kanivin.get_request_header("REMOTE_ADDR")
	...
	def validate_csrf_token(self):
		if (
			not kanivin.request
			or kanivin.request.method not in UNSAFE_HTTP_METHODS
			or kanivin.conf.ignore_csrf
			or not kanivin.session
			or not (saved_token := kanivin.session.data.csrf_token)
			or (
				(kanivin.get_request_header("X-Kanivin-CSRF-Token") or kanivin.form_dict.pop("csrf_token", None))
				== saved_token
			)
		):
			return

		kanivin.flags.disable_traceback = True
		kanivin.throw(_("Invalid Request"), kanivin.CSRFTokenError)

	def set_lang(self):
		kanivin.local.lang = get_language()
...
class LoginManager:
	...
	def __init__(self):
		...
		if kanivin.local.form_dict.get("cmd") == "login" or kanivin.local.request.path == "/api/method/login":
	...
	def login(self):
		if kanivin.get_system_settings("disable_user_pass_login"):
			kanivin.throw(_("Login with username and password is not allowed."), kanivin.AuthenticationError)

		# clear cache
		kanivin.clear_cache(user=kanivin.form_dict.get("usr"))
		user, pwd = get_cached_user_pass()
		self.authenticate(user=user, pwd=pwd)
		if self.force_user_to_reset_password():
			doc = kanivin.get_doc("User", self.user)
			kanivin.local.response["redirect_to"] = doc.reset_password(send_email=False, password_expired=True)
			kanivin.local.response["message"] = "Password Reset"
			return False
		...
		if should_run_2fa(self.user):
			...
			if not confirm_otp_token(self):
				return False
		kanivin.form_dict.pop("pwd", None)
		self.post_login()
	...
	def get_user_info(self):
		self.info = kanivin.get_cached_value(
			"User",
			self.user,
	...
	def setup_boot_cache(self):
		kanivin.cache_manager.build_table_count_cache()
		kanivin.cache_manager.build_domain_restriced_doctype_cache()
		kanivin.cache_manager.build_domain_restriced_page_cache()
	...
	def set_user_info(self, resume=False):
		...
		if self.info.user_type == "Website User":
			kanivin.local.cookie_manager.set_cookie("system_user", "no")
			if not resume:
				kanivin.local.response["message"] = "No App"
				kanivin.local.response["home_page"] = get_default_path() or "/" + get_home_page()
		else:
			kanivin.local.cookie_manager.set_cookie("system_user", "yes")
			if not resume:
				kanivin.local.response["message"] = "Logged In"
				default_workspace = self.info.default_workspace
				if default_workspace:
					kanivin.local.response["home_page"] = "/app/" + slug(default_workspace)
				else:
					kanivin.local.response["home_page"] = get_default_path() or "/app"

		if not resume:
			kanivin.response["full_name"] = self.full_name

		# redirect information
		if not resume and (redirect_to := kanivin.cache.hget("redirect_after_login", self.user)):
			kanivin.local.response["redirect_to"] = redirect_to
			kanivin.cache.hdel("redirect_after_login", self.user)

		kanivin.local.cookie_manager.set_cookie("full_name", self.full_name)
		kanivin.local.cookie_manager.set_cookie("user_id", self.user)
		kanivin.local.cookie_manager.set_cookie("user_image", self.info.user_image or "")

	def clear_preferred_language(self):
		kanivin.local.cookie_manager.delete_cookie("preferred_language")

	def make_session(
		self, resume: bool = False, session_end: str | None = None, audit_user: str | None = None
	):
		# start session
		kanivin.local.session_obj = Session(
			user=self.user,
			resume=resume,
		...
		self.user = kanivin.local.session_obj.user
		kanivin.local.session = kanivin.local.session_obj.data
	...
	def clear_active_sessions(self):
		...
		if not (
			cint(kanivin.conf.get("deny_multiple_sessions"))
			or cint(kanivin.db.get_system_setting("deny_multiple_sessions"))
		):
			return

		clear_sessions(kanivin.session.user, keep_current=True)

	def authenticate(self, user: str | None = None, pwd: str | None = None):
		from kanivin.core.doctype.user.user import User

		if not (user and pwd):
			user, pwd = kanivin.form_dict.get("usr"), kanivin.form_dict.get("pwd")
		if not (user and pwd):
		...
		ip_tracker = get_login_attempt_tracker(kanivin.local.request_ip)
		if not user:
		...
		ignore_tracker = should_run_2fa(user.name) and ("otp" in kanivin.form_dict)
	...
	def force_user_to_reset_password(self):
		...
		if self.user in kanivin.STANDARD_USERS:
			return False

		reset_pwd_after_days = cint(
			kanivin.db.get_single_value("System Settings", "force_user_to_reset_password")
		)

		if reset_pwd_after_days:
			last_password_reset_date = (
				kanivin.db.get_value("User", self.user, "last_password_reset_date") or today()
			)
	...
	def fail(self, message, user=None):
		if not user:
			user = _("Unknown User")
		kanivin.local.response["message"] = message
		add_authentication_log(message, user, status="Failed")
		kanivin.db.commit()
		raise kanivin.AuthenticationError

	def run_trigger(self, event="on_login"):
		for method in kanivin.get_hooks().get(event, []):
			kanivin.call(kanivin.get_attr(method), login_manager=self)

	def validate_hour(self):
		"""check if user is logging in during restricted hours"""
		login_before = int(kanivin.db.get_value("User", self.user, "login_before", ignore=True) or 0)
		login_after = int(kanivin.db.get_value("User", self.user, "login_after", ignore=True) or 0)
		...
		if login_before and current_hour >= login_before:
			kanivin.throw(_("Login not allowed at this time"), kanivin.AuthenticationError)
	...
	def impersonate(self, user):
		current_user = kanivin.session.user
		session_data = kanivin.local.session_obj.data.data
		...
		kanivin.local.session_obj.set_impersonated(current_user)
	...
	def logout(self, arg="", user=None):
		...
		if user == kanivin.session.user:
			delete_session(kanivin.session.sid, user=user, reason="User Manually Logged Out")
			self.clear_cookies()
			if kanivin.request:
				self.login_as_guest()
		else:
...
class CookieManager:
	...
	def init_cookies(self):
		if not kanivin.local.session.get("sid"):
			return

		if kanivin.session.sid:
			self.set_cookie("sid", kanivin.session.sid, max_age=get_expiry_in_seconds(), httponly=True)
	...
	def set_cookie(
		...
		if not secure and hasattr(kanivin.local, "request"):
			secure = kanivin.local.request.scheme == "https"
...
@kanivin.whitelist()
def get_logged_user():
	return kanivin.session.user


def clear_cookies():
	if hasattr(kanivin.local, "session"):
		kanivin.session.sid = ""
	kanivin.local.cookie_manager.delete_cookie(["full_name", "user_id", "sid", "user_image", "system_user"])
...
def validate_ip_address(user):
	...
	checked for those
	"""
	if hasattr(kanivin.local, "request") and kanivin.local.request.path.startswith(
		"/api/method/kanivin.realtime."
	):
		return True

	from kanivin.core.doctype.user.user import get_restricted_ip_list

	# Only fetch required fields - for perf
	user_fields = ["restrict_ip", "bypass_restrict_ip_check_if_2fa_enabled"]
	user_info = (
		kanivin.get_cached_value("User", user, user_fields, as_dict=True)
		if not kanivin.flags.in_test
		else kanivin.db.get_value("User", user, user_fields, as_dict=True)
	)
	...
	system_settings = (
		kanivin.get_cached_doc("System Settings")
		if not kanivin.flags.in_test
		else kanivin.get_single("System Settings")
	)
	...
	for ip in ip_list:
		if kanivin.local.request_ip.startswith(ip) or bypass_restrict_ip_check:
			return

	kanivin.throw(_("Access not allowed from this IP Address"), kanivin.AuthenticationError)
...
def get_login_attempt_tracker(key: str, raise_locked_exception: bool = True):
	...
	"""
	sys_settings = kanivin.get_doc("System Settings")
...
class LoginAttemptTracker:
	...
	def login_failed_count(self):
		return kanivin.cache.hget("login_failed_count", self.key)

	@login_failed_count.setter
	def login_failed_count(self, count):
		kanivin.cache.hset("login_failed_count", self.key, count)

	@login_failed_count.deleter
	def login_failed_count(self):
		kanivin.cache.hdel("login_failed_count", self.key)
	...
	def login_failed_time(self):
		...
		"""
		return kanivin.cache.hget("login_failed_time", self.key)

	@login_failed_time.setter
	def login_failed_time(self, timestamp):
		kanivin.cache.hset("login_failed_time", self.key, timestamp)

	@login_failed_time.deleter
	def login_failed_time(self):
		kanivin.cache.hdel("login_failed_time", self.key)
...
def validate_auth():
	...
	"""
	authorization_header = kanivin.get_request_header("Authorization", "").split(" ")
	...
	if len(authorization_header) == 2 and kanivin.session.user in ("", "Guest"):
		raise kanivin.AuthenticationError
...
def validate_oauth(authorization_header):
	...
	from kanivin.integrations.oauth2 import get_oauth_server
	from kanivin.oauth import get_url_delimiter

	if authorization_header[0].lower() != "bearer":
		return

	form_dict = kanivin.local.form_dict
	...
	try:
		required_scopes = kanivin.db.get_value("OAuth Bearer Token", token, "scopes").split(
			get_url_delimiter()
		)
		...
		if valid:
			kanivin.set_user(kanivin.db.get_value("OAuth Bearer Token", token, "user"))
			kanivin.local.form_dict = form_dict
	except AttributeError:
		pass
...
def validate_auth_via_api_keys(authorization_header):
	...
	try:
		...
		authorization_source = kanivin.get_request_header("Kanivin-Authorization-Source")
		if auth_type.lower() == "basic":
			api_key, api_secret = kanivin.safe_decode(base64.b64decode(auth_token)).split(":")
		...
		kanivin.throw(
			...
			kanivin.InvalidAuthorizationToken,
		)
	except (AttributeError, TypeError, ValueError):
		pass


def validate_api_key_secret(api_key, api_secret, kanivin_authorization_source=None):
	"""kanivin_authorization_source to provide api key and secret for a doctype apart from User"""
	doctype = kanivin_authorization_source or "User"
	doc = kanivin.db.get_value(doctype=doctype, filters={"api_key": api_key}, fieldname=["name"])
	if not doc:
		raise kanivin.AuthenticationError
	form_dict = kanivin.local.form_dict
	...
	if api_secret == doc_secret:
		if doctype == "User":
			user = kanivin.db.get_value(doctype="User", filters={"api_key": api_key}, fieldname=["name"])
		else:
			user = kanivin.db.get_value(doctype, doc, "user")
		if kanivin.local.login_manager.user in ("", "Guest"):
...
def validate_auth_via_hooks():
	for auth_hook in kanivin.get_hooks("auth_hooks", []):
		kanivin.get_attr(auth_hook)()
