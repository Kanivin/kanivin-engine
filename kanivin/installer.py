...
import kanivin
from kanivin.defaults import _clear_cache
from kanivin.utils import cint, is_git_url
from kanivin.utils.dashboard import sync_dashboards
from kanivin.utils.synchronization import filelock
...
def _is_scheduler_enabled(site) -> bool:
	...
	try:
		...
		kanivin.connect()
		enable_scheduler = cint(kanivin.db.get_single_value("System Settings", "enable_scheduler"))
	except Exception:
		pass
	finally:
		kanivin.db.close()
...
def _new_site(
	...
	"""Install a new Kanivin site"""

	from kanivin.utils import scheduler
	...
	if not db_name:
		...
		db_name = (
			"_"
			+ hashlib.sha1(
				os.path.realpath(kanivin.get_site_path()).encode(), usedforsecurity=False
			).hexdigest()[:16]
		)
	...
	with filelock("bench_new_site", timeout=1):
		...
		apps_to_install = ["kanivin"] + (kanivin.conf.get("install_apps") or []) + (list(install_apps) or [])
	...
	scheduler_status = "disabled" if kanivin.utils.scheduler.is_scheduler_disabled() else "enabled"
...
def install_db(
	...
	from kanivin.database import bootstrap_database, setup_database

	if not db_type:
		db_type = kanivin.conf.db_type
	...
	kanivin.flags.in_install_db = True

	kanivin.flags.root_login = root_login
	kanivin.flags.root_password = root_password
	...
	kanivin.conf.admin_password = kanivin.conf.admin_password or admin_password

	remove_missing_apps()

	kanivin.db.create_auth_table()
	kanivin.db.create_global_search_table()
	kanivin.db.create_user_settings_table()

	kanivin.flags.in_install_db = False
...
def find_org(org_repo: str) -> tuple[str, str]:
	...
	find_org()
	ref -> https://github.com/kanivin/bench/blob/develop/bench/utils/__init__.py#L390
	...
	from kanivin.exceptions import InvalidRemoteException

	for org in ["kanivin", "erpnext"]:
...
def fetch_details_from_tag(_tag: str) -> tuple[str, str, str]:
	...
	ref -> https://github.com/kanivin/bench/blob/develop/bench/utils/__init__.py#L403
...
def parse_app_name(name: str) -> str:
	...
	ref -> https://github.com/kanivin/bench/blob/develop/bench/app.py#L114
...
def install_app(name, verbose=False, set_as_patched=True, force=False):
	from kanivin.core.doctype.scheduled_job_type.scheduled_job_type import sync_jobs
	from kanivin.model.sync import sync_for
	from kanivin.modules.utils import sync_customizations
	from kanivin.utils.fixtures import sync_fixtures

	kanivin.flags.in_install = name
	kanivin.flags.ignore_in_install = False

	kanivin.clear_cache()
	app_hooks = kanivin.get_hooks(app_name=name)
	installed_apps = kanivin.get_installed_apps()
	...
	if name not in kanivin.get_all_apps():
	...
	for before_install in app_hooks.before_install or []:
		out = kanivin.get_attr(before_install)()
		if out is False:
			return

	for fn in kanivin.get_hooks("before_app_install"):
		kanivin.get_attr(fn)(name)
	...
	kanivin.get_doc("Portal Settings", "Portal Settings").sync_menu()

	if set_as_patched:
		set_all_patches_as_completed(name)

	for after_install in app_hooks.after_install or []:
		kanivin.get_attr(after_install)()

	for fn in kanivin.get_hooks("after_app_install"):
		kanivin.get_attr(fn)(name)
	...
	for after_sync in app_hooks.after_sync or []:
		kanivin.get_attr(after_sync)()  #

	kanivin.flags.in_install = False
...
def add_to_installed_apps(app_name, rebuild_website=True):
	...
	if app_name not in installed_apps:
		...
		kanivin.db.set_global("installed_apps", json.dumps(installed_apps))
		kanivin.db.commit()
		if kanivin.flags.in_install:
...
def remove_app(app_name, dry_run=False, yes=False, no_backup=False, force=False):
	...
	app_hooks = kanivin.get_hooks(app_name=app_name)

	# dont allow uninstall app if not installed unless forced
	if not force:
		if app_name not in kanivin.get_installed_apps():
	...
	for app in kanivin.get_installed_apps():
		if app != app_name:
			hooks = kanivin.get_hooks(app_name=app)
	...
	if not (dry_run or no_backup):
		from kanivin.utils.backups import scheduled_backup

		print("Backing up...")
		scheduled_backup(ignore_files=True)

	kanivin.flags.in_uninstall = True

	for before_uninstall in app_hooks.before_uninstall or []:
		kanivin.get_attr(before_uninstall)()

	for fn in kanivin.get_hooks("before_app_uninstall"):
		kanivin.get_attr(fn)(app_name)

	modules = kanivin.get_all("Module Def", filters={"app_name": app_name}, pluck="name")
	...
	if not dry_run:
		...
		kanivin.get_single("Installed Applications").update_versions()
		kanivin.db.commit()

	for after_uninstall in app_hooks.after_uninstall or []:
		kanivin.get_attr(after_uninstall)()

	for fn in kanivin.get_hooks("after_app_uninstall"):
		kanivin.get_attr(fn)(app_name)

	click.secho(f"Uninstalled App {app_name} from Site {site}", fg="green")
	kanivin.flags.in_uninstall = False
...
def _delete_modules(modules: list[str], dry_run: bool) -> list[str]:
	...
	for module_name in modules:
		...
		for doctype in kanivin.get_all(
			...
			if not dry_run:
				if doctype.issingle:
					kanivin.delete_doc("DocType", doctype.name, ignore_on_trash=True, force=True)
				else:
		...
		if not dry_run:
			kanivin.delete_doc("Module Def", module_name, ignore_on_trash=True, force=True)
...
def _delete_linked_documents(module_name: str, doctype_linkfield_map: dict[str, str], dry_run: bool) -> None:
	...
	for doctype, fieldname in doctype_linkfield_map.items():
		for record in kanivin.get_all(doctype, filters={fieldname: module_name}, pluck="name"):
			print(f"* removing {doctype} '{record}'...")
			if not dry_run:
				kanivin.delete_doc(doctype, record, ignore_on_trash=True, force=True)
...
def _get_module_linked_doctype_field_map() -> dict[str, str]:
	...
	linked_doctypes = kanivin.get_all(
		"DocField",
		filters={"fieldtype": "Link", "options": "Module Def"},
		fields=["parent", "fieldname"],
	)
	existing_linked_doctypes = [d for d in linked_doctypes if kanivin.db.exists("DocType", d.parent)]
...
def _delete_doctypes(doctypes: list[str], dry_run: bool) -> None:
	for doctype in set(doctypes):
		...
		if not dry_run:
			kanivin.delete_doc("DocType", doctype, ignore_on_trash=True, force=True)
			kanivin.db.sql_ddl(f"DROP TABLE IF EXISTS `tab{doctype}`")


def post_install(rebuild_website=False):
	from kanivin.website.utils import clear_website_cache
...
def set_all_patches_as_completed(app):
	from kanivin.modules.patch_handler import get_patches_from_app

	patches = get_patches_from_app(app)
	for patch in patches:
		kanivin.get_doc({"doctype": "Patch Log", "patch": patch}).insert(ignore_permissions=True)
	kanivin.db.commit()


def init_singles():
	singles = kanivin.get_all("DocType", filters={"issingle": True}, pluck="name")
	for single in singles:
		if kanivin.db.get_singles_dict(single):
			continue

		try:
			doc = kanivin.new_doc(single)
			doc.flags.ignore_mandatory = True
			doc.flags.ignore_validate = True
			doc.save()
		except (ImportError, kanivin.DoesNotExistError):
...
def make_conf(
	...
	sites_path = kanivin.local.sites_path
	kanivin.destroy()
	kanivin.init(site, sites_path=sites_path)
...
def make_site_config(
	...
	kanivin.create_folder(os.path.join(kanivin.local.site_path))
...
def _update_config_file(key: str, value, config_file: str):
	...
	if hasattr(kanivin.local, "conf"):
		kanivin.local.conf[key] = value


def get_site_config_path():
	return os.path.join(kanivin.local.site_path, "site_config.json")
...
def make_site_dirs():
	for dir_path in [
		...
		path = kanivin.get_site_path(dir_path)
		os.makedirs(path, exist_ok=True)


def add_module_defs(app, ignore_if_duplicate=False):
	modules = kanivin.get_module_list(app)
	for module in modules:
		d = kanivin.new_doc("Module Def")
		d.app_name = app
...
def remove_missing_apps():
	...
	apps = ("kanivin_subscription", "shopping_cart")
	installed_apps = json.loads(kanivin.db.get_global("installed_apps") or "[]")
	for app in apps:
...
def extract_files(site_name, file_path):
	...
	from kanivin.utils import get_bench_relative_path

	file_path = get_bench_relative_path(file_path)

	# Need to do kanivin.init to maintain the site locals
	kanivin.init(site=site_name)
	abs_site_path = os.path.abspath(kanivin.get_site_path())
...
def is_downgrade(sql_file_path, verbose=False):
	...
	current_version = Version(kanivin.__version__)
	...
	if verbose and is_downgrade:
		print(f"Your site is currently on Kanivin {current_version} and your backup is {backup_version}.")

	return is_downgrade


def get_old_backup_version(sql_file_path: str) -> Version | None:
	"""Return the kanivin version used to create the specified database dump.

	This methods supports older versions of Kanivin wich used a different format.
	"""
	header = get_db_dump_header(sql_file_path).split("\n")
	if match := re.search(r"Kanivin (\d+\.\d+\.\d+)", header[0]):
...
def get_backup_version(sql_file_path: str) -> Version | None:
	...
	header = get_db_dump_header(sql_file_path).split("\n")
	metadata = ""
	if "begin kanivin metadata" in header[0]:
		for line in header[1:]:
			if "end kanivin metadata" in line:
				break
			metadata += line.replace("--", "").strip() + "\n"
		parser = configparser.ConfigParser()
		parser.read_string(metadata)
		return Version(parser["kanivin"]["version"])
...
def partial_restore(sql_file_path, verbose=False):
	if kanivin.conf.db_type == "mariadb":
		from kanivin.database.mariadb.setup_db import import_db_from_sql
	elif kanivin.conf.db_type == "postgres":
		import warnings

		from kanivin.database.postgres.setup_db import import_db_from_sql
...
def validate_database_sql(path: str, _raise: bool = True) -> None:
	...
	if os.path.getsize(path):
		if (executable := which(executable_name)) is None:
			kanivin.throw(
				...
				exc=kanivin.ExecutableNotFound,
			)
		try:
			kanivin.utils.execute_in_shell(f"{executable} -m1 __Auth {path}", check_exit_code=True)
			return
		except Exception:
	...
	if _raise:
		raise kanivin.InvalidDatabaseFile
