...
from kanivin.contacts.address_and_contact import set_link_title
from kanivin.core.doctype.access_log.access_log import make_access_log
from kanivin.core.doctype.dynamic_link.dynamic_link import deduplicate_dynamic_links
from kanivin.model.document import Document
from kanivin.model.naming import append_number_if_name_exists
from kanivin.utils import cstr, has_gravatar
...
class Contact(Document):
	...
	if TYPE_CHECKING:
		from kanivin.contacts.doctype.contact_email.contact_email import ContactEmail
		from kanivin.contacts.doctype.contact_phone.contact_phone import ContactPhone
		from kanivin.core.doctype.dynamic_link.dynamic_link import DynamicLink
	...
	def autoname(self):
		...
		if kanivin.db.exists("Contact", self.name):
	...
	def validate(self):
		...
		if self.get("sync_with_google_contacts") and not self.get("google_contacts"):
			kanivin.throw(_("Select Google Contacts to which contact should be synced."))

		deduplicate_dynamic_links(self)

	def set_user(self):
		if not self.user and self.email_id:
			self.user = kanivin.db.get_value("User", {"email": self.email_id})
	...
	def add_email(self, email_id, is_primary=0, autosave=False):
		if not kanivin.db.exists("Contact Email", {"email_id": email_id, "parent": self.name}):
	...
	def add_phone(self, phone, is_primary_phone=0, is_primary_mobile_no=0, autosave=False):
		if not kanivin.db.exists("Contact Phone", {"phone": phone, "parent": self.name}):
			self.append(
				"phone_nos",
				{
					"phone": phone,
	...
	def set_primary_email(self):
		...
		if len([email.email_id for email in self.email_ids if email.is_primary]) > 1:
			kanivin.throw(_("Only one {0} can be set as primary.").format(kanivin.bold(_("Email ID"))))
	...
	def set_primary(self, fieldname):
		...
		if len(is_primary) > 1:
			kanivin.throw(
				_("Only one {0} can be set as primary.").format(kanivin.bold(kanivin.unscrub(fieldname)))
			)
...
def download_vcard(contact: str):
	...
	contact = kanivin.get_doc("Contact", contact)
	contact.check_permission()

	vcard = contact.get_vcard()
	make_access_log(doctype="Contact", document=contact.name, file_type="vcf")

	kanivin.response["filename"] = f"{contact.name}.vcf"
	kanivin.response["filecontent"] = vcard.serialize().encode("utf-8")
	kanivin.response["type"] = "binary"
...
def download_vcards(contacts: str):
	...
	from kanivin.utils.data import now

	contact_ids = kanivin.parse_json(contacts)

	vcards = []
	for contact_id in contact_ids:
		contact = kanivin.get_doc("Contact", contact_id)
		contact.check_permission()
		vcard = contact.get_vcard()
	...
	kanivin.response["filename"] = f"{timestamp} Contacts.vcf"
	kanivin.response["filecontent"] = "\n".join(vcards).encode("utf-8")
	kanivin.response["type"] = "binary"
...
def invite_user(contact: str):
	...
	if not contact.email_id:
		kanivin.throw(_("Please set Email Address"))
...
def update_contact(doc, method):
	...
	contact_name = kanivin.db.get_value("Contact", {"email_id": doc.name})
	if contact_name:
...
def contact_query(doctype, txt, searchfield, start, page_len, filters):
	from kanivin.desk.reportview import get_match_cond

	doctype = "Contact"
	if not kanivin.get_meta(doctype).get_field(searchfield) and searchfield not in kanivin.db.DEFAULT_COLUMNS:
		return []
...
def get_contact_name(email_id: str) -> str | None:
	...
	for contact_id in kanivin.get_all(
		"Contact Email", filters={"email_id": email_id, "parenttype": "Contact"}, pluck="parent"
	):
		if kanivin.db.exists("Contact", contact_id):
			return contact_id
...
def get_contacts_linked_from(doctype, docname, fields=None):
	...
	link_fields = kanivin.get_meta(doctype).get("fields", {"fieldtype": "Link", "options": "Contact"})
	if not link_fields:
		return []

	contact_names = kanivin.get_value(doctype, docname, fieldname=[f.fieldname for f in link_fields])
	if not contact_names:
		return []

	return kanivin.get_list("Contact", fields=fields, filters={"name": ("in", contact_names)})
...
def get_contact_display_list(doctype: str, name: str) -> list[dict]:
	from kanivin.contacts.doctype.address.address import get_condensed_address

	if not kanivin.has_permission("Contact", "read"):
		return []

	contact_list = kanivin.get_list(
		"Contact",
		filters=[
			["Dynamic Link", "link_doctype", "=", doctype],
	...
	for contact in contact_list:
		contact["email_ids"] = kanivin.get_all(
			"Contact Email",
			filters={"parenttype": "Contact", "parent": contact.name, "is_primary": 0},
			fields=["email_id"],
		)

		contact["phone_nos"] = kanivin.get_all(
			"Contact Phone",
			filters={
				"parenttype": "Contact",
				"parent": contact.name,
		...
		if contact.address and kanivin.has_permission("Address", "read"):
			address = kanivin.get_doc("Address", contact.address)
