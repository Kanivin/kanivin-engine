...
class AddressTemplate(Document):
	...
	def validate(self):
		...
		if not self.is_default and not self._get_previous_default():
			self.is_default = 1
			if kanivin.db.get_single_value("System Settings", "setup_complete"):
				kanivin.msgprint(_("Setting this Address Template as default as there is no other default"))

	def on_update(self):
		if self.is_default and (previous_default := self._get_previous_default()):
			kanivin.db.set_value("Address Template", previous_default, "is_default", 0)

	def on_trash(self):
		if self.is_default:
			kanivin.throw(_("Default Address Template cannot be deleted"))

	def _get_previous_default(self) -> str | None:
		return kanivin.db.get_value("Address Template", {"is_default": 1, "name": ("!=", self.name)})
