...
from kanivin.model.naming import make_autoname
...
class Address(Document):
	...
	def autoname(self):
		...
		if self.address_title:
			...
			if kanivin.db.exists("Address", self.name):
	...
	def link_address(self):
		...
		if not self.links:
			contact_name = kanivin.db.get_value("Contact", {"email_id": self.owner})
			if contact_name:
				contact = kanivin.get_cached_doc("Contact", contact_name)
...
def render_address(address: dict | str | None, check_permissions=True) -> str | None:
	...
	if not isinstance(address, dict):
		address = kanivin.get_cached_doc("Address", address)
	...
	try:
		return kanivin.render_template(template, address)
	except TemplateSyntaxError:
		kanivin.throw(_("There is an error in your Address Template {0}").format(name))
...
def get_territory_from_address(address):
	...
	for fieldname in ("city", "state", "country"):
		if address.get(fieldname):
			territory = kanivin.db.get_value("Territory", address.get(fieldname))
			if territory:
				break
...
def get_address_list(doctype, txt, filters, limit_start, limit_page_length=20, order_by=None):
	from kanivin.www.list import get_list
...
def has_website_permission(doc, ptype, user, verbose=False):
	...
	contact_name = kanivin.db.get_value("Contact", {"email_id": kanivin.session.user})
...
def get_address_templates(address):
	result = kanivin.db.get_value(
		"Address Template", {"country": address.get("country")}, ["name", "template"]
	)

	if not result:
		result = kanivin.db.get_value("Address Template", {"is_default": 1}, ["name", "template"])
...
def address_query(doctype, txt, searchfield, start, page_len, filters):
	from kanivin.desk.search import search_widget
...
def update_preferred_address(address, field):
	kanivin.db.set_value("Address", address, field, 0)


def get_address_display_list(doctype: str, name: str) -> list[dict]:
	if not kanivin.has_permission("Address", "read"):
		return []

	address_list = kanivin.get_list(
		"Address",
		filters=[
			["Dynamic Link", "link_doctype", "=", doctype],
			["Dynamic Link", "link_name", "=", name],
