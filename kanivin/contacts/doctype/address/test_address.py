...
import kanivin
from kanivin.contacts.doctype.address.address import address_query, get_address_display
from kanivin.tests.utils import KanivinTestCase


class TestAddress(KanivinTestCase):
	def test_template_works(self):
		if not kanivin.db.exists("Address Template", "India"):
			kanivin.get_doc({"doctype": "Address Template", "country": "India", "is_default": 1}).insert()

		if not kanivin.db.exists("Address", "_Test Address-Office"):
			kanivin.get_doc(
				{
		...
		address = kanivin.get_list("Address")[0].name
		display = get_address_display(kanivin.get_doc("Address", address).as_dict())
