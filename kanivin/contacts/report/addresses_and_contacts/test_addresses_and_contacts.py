...
from kanivin.contacts.report.addresses_and_contacts.addresses_and_contacts import get_data
from kanivin.tests.utils import KanivinTestCase


def get_custom_linked_doctype():
	if bool(kanivin.get_all("DocType", filters={"name": "Test Custom Doctype"})):
		return
...
def create_linked_address(link_list):
	if kanivin.flags.test_address_created:
		return

	address = kanivin.get_doc(
		{
			"doctype": "Address",
	...
	address.insert()
	kanivin.flags.test_address_created = True

	return address.name


def create_linked_contact(link_list, address):
	if kanivin.flags.test_contact_created:
		return
	...
	kanivin.flags.test_contact_created = True


class TestAddressesAndContacts(KanivinTestCase):
