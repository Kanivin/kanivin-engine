# Copyright (c) 2013, Kanivin Pvt. Ltd. and contributors
...
def get_reference_addresses_and_contact(reference_doctype, reference_name):
	data = []
	filters = None
	reference_details = kanivin._dict()
	...
	reference_list = [
		d[0] for d in kanivin.get_list(reference_doctype, filters=filters, fields=["name"], as_list=True)
	]

	for d in reference_list:
		reference_details.setdefault(d, kanivin._dict())
...
def get_reference_details(reference_doctype, doctype, reference_list, reference_details):
	...
	records = kanivin.get_list(doctype, filters=filters, fields=fields, as_list=True)
	temp_records = [d[1:] for d in records]

	if not reference_list:
		kanivin.throw(_("No records present in {0}").format(reference_doctype))

	reference_details[reference_list[0]][kanivin.scrub(doctype)] = temp_records
