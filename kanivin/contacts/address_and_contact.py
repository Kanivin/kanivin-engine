...
def load_address_and_contact(doc, key=None) -> None:
	...
	from kanivin.contacts.doctype.address.address import get_address_display_list
	from kanivin.contacts.doctype.contact.contact import get_contact_display_list
...
def has_permission(doc, ptype, user):
	...
	for df in links.get("permitted_links") + links.get("not_permitted_links"):
		...
		if name and kanivin.has_permission(doctype, ptype, doc=name):
			return True
...
def delete_contact_and_address(doctype: str, docname: str) -> None:
	for parenttype in ("Contact", "Address"):
		for name in kanivin.get_all(
			...
			doc = kanivin.get_doc(parenttype, name)
			if len(doc.links) == 1:
				doc.delete()
			else:
...
def filter_dynamic_link_doctypes(
	...
	_doctypes_from_df = kanivin.get_all(
		"DocField",
		filters=filters,
		pluck="parent",
		distinct=True,
	...
	_doctypes_from_cdf = kanivin.get_all(
...
def set_link_title(doc):
	if not doc.links:
		return
	for link in doc.links:
		linked_doc = kanivin.get_doc(link.link_doctype, link.link_name)
