...
def add_docshare(
	...
	if share_name:
		doc = kanivin.get_doc("DocShare", share_name)
	else:
		doc = kanivin.new_doc("DocShare")
	...
	if kanivin.get_cached_value("User", user, "follow_shared_documents"):
...
def remove(doctype, name, user, flags=None):
	share_name = kanivin.db.get_value("DocShare", {"user": user, "share_name": name, "share_doctype": doctype})

	if share_name:
		kanivin.delete_doc("DocShare", share_name, flags=flags)
...
def set_docshare_permission(doctype, name, user, permission_to, value=1, everyone=0, flags=None):
	...
	if not share_name:
		...
		share = kanivin.get_doc("DocShare", share_name)
		if flags:
...
def get_shared_doctypes(user=None):
	...
	if not user:
		user = kanivin.session.user
	table = kanivin.qb.DocType("DocShare")
	query = (
...
def get_share_name(doctype, name, user, everyone):
	if cint(everyone):
		share_name = kanivin.db.get_value(
...
def check_share_permission(doctype, name):
	...
	if not kanivin.has_permission(doctype, ptype="share", doc=name):
		kanivin.throw(
			_("No permission to {0} {1} {2}").format("share", _(doctype), name), kanivin.PermissionError
		)
...
def notify_assignment(shared_by, doctype, doc_name, everyone, notify=0):
	...
	reference_user = get_fullname(kanivin.session.user)
	notification_message = _("{0} shared a document {1} {2} with you").format(
		kanivin.bold(reference_user), kanivin.bold(_(doctype)), get_title_html(title)
	)
