...
import kanivin
import kanivin.model.sync
import kanivin.modules.patch_handler
import kanivin.translate
from kanivin.cache_manager import clear_global_cache
...
from kanivin.deferred_insert import save_to_db as flush_deferred_inserts
from kanivin.desk.notifications import clear_notifications
from kanivin.modules.patch_handler import PatchType
...
from kanivin.utils.connections import check_connection
...
class SiteMigration:
	...
	def setUp(self):
		...
		self.touched_tables_file = kanivin.get_site_path("touched_tables.json")
		kanivin.clear_cache()
	...
	def tearDown(self):
		...
		with open(self.touched_tables_file, "w") as f:
			json.dump(list(kanivin.flags.touched_tables), f, sort_keys=True, indent=4)

		if not self.skip_search_index:
			print(f"Queued rebuilding of search index for {kanivin.local.site}")
			kanivin.enqueue(build_index_for_all_routes, queue="long")

		kanivin.publish_realtime("version-update")
	...
	def pre_schema_updates(self):
		...
		for app in kanivin.get_installed_apps():
			for fn in kanivin.get_hooks("before_migrate", app_name=app):
				kanivin.get_attr(fn)()
	...
	def post_schema_updates(self):
		...
		* Sync Kanivin's internal language master
		...
		* Execute `after_migrate` hooks
		...
		kanivin.model.sync.remove_orphan_doctypes()

		kanivin.get_single("Portal Settings").sync_menu()
		kanivin.get_single("Installed Applications").update_versions()

		for app in kanivin.get_installed_apps():
			for fn in kanivin.get_hooks("after_migrate", app_name=app):
				kanivin.get_attr(fn)()
