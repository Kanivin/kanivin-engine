...
from kanivin.utils import cint, get_fullname, get_link_to_form, getdate
...
class EnergyPointLog(Document):
	...
	def validate(self):
		...
		if self.type in ["Appreciation", "Criticism"] and self.user == self.owner:
			kanivin.throw(_("You cannot give review points to yourself"))
	...
	def map_milestone_reference(self):
		...
		if self.reference_doctype == "Milestone":
			self.reference_doctype, self.reference_name = kanivin.db.get_value(
	...
	def after_insert(self):
		...
		kanivin.cache.hdel("energy_points", self.user)

		if self.type != "Review" and kanivin.get_cached_value(
	...
	def on_trash(self):
		if self.type == "Revert":
			reference_log = kanivin.get_doc("Energy Point Log", self.revert_of)
	...
	def revert(self, reason, ignore_permissions=False):
		...
		if self.type != "Auto":
			kanivin.throw(_("This document cannot be reverted"))
...
def get_notification_message(doc):
	...
	if doc.type == "Auto":
		owner_name = kanivin.bold("You")
		if points == 1:
			message = _("{0} gained {1} point for {2} {3}")
		else:
			message = _("{0} gained {1} points for {2} {3}")
		message = message.format(owner_name, kanivin.bold(points), doc.rule, get_title_html(title))
		...
		message = message.format(kanivin.bold(owner_name), get_title_html(title), kanivin.bold(points))
		...
		message = message.format(kanivin.bold(owner_name), get_title_html(title))

	return message


def get_alert_dict(doc):
	alert_dict = kanivin._dict()
	owner_name = get_fullname(doc.owner)
	if doc.reference_doctype:
		doc_link = get_link_to_form(doc.reference_doctype, doc.reference_name)
	points = doc.points
	bold_points = kanivin.bold(doc.points)
	if doc.type == "Auto":
		if points == 1:
...
def create_energy_points_log(ref_doctype, ref_name, doc, apply_only_once=False):
	...
	if doc.rule:
		...
		if log_exists:
			return kanivin.get_doc("Energy Point Log", log_exists)

	new_log = kanivin.new_doc("Energy Point Log")
...
def check_if_log_exists(ref_doctype, ref_name, rule, user=None):
	...
	return kanivin.db.exists("Energy Point Log", filters)
...
def get_energy_points(user):
	...
	return kanivin._dict(points.get(user, {}))
...
def review(doc, points, to_user, reason, review_type="Appreciation"):
	current_review_points = get_energy_points(kanivin.session.user).review_points
	doc = doc.as_dict() if hasattr(doc, "as_dict") else kanivin._dict(json.loads(doc))
	points = abs(cint(points))
	if current_review_points < points:
		kanivin.msgprint(_("You do not have enough review points"))
		return
...
def send_summary(timespan):
	from kanivin.social.doctype.energy_point_settings.energy_point_settings import (
	...
	if not is_email_notifications_enabled_for_type(kanivin.session.user, "Energy Point"):
		return

	from_date = kanivin.utils.add_to_date(None, weeks=-1)
	if timespan == "Monthly":
		from_date = kanivin.utils.add_to_date(None, months=-1)
