...
kanivin.ui.form.on("Energy Point Log", {
			...
			primary_action: (values) => {
				return frm
					...
					.then((res) => {
						...
						revert_dialog.hide();
						revert_dialog.clear();
						kanivin.model.docinfo[frm.doc.reference_doctype][
	...
	make_reference_name_link(frm) {
		...
		frm.fields_dict.reference_name.$input_wrapper
			.find(".control-value")
			.wrapInner(`<a href='/app/${kanivin.router.slug(dt)}/${dn}'></a>`);
	},
});
