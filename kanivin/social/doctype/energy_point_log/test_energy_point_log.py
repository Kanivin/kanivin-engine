...
import kanivin
from kanivin.desk.form.assign_to import add as assign_to
from kanivin.desk.page.user_profile.user_profile import get_energy_points_heatmap_data
...
class TestEnergyPointLog(KanivinTestCase):
	@classmethod
	def setUpClass(cls):
		super().setUpClass()
		settings = kanivin.get_single("Energy Point Settings")
		settings.enabled = 1
		settings.save()
	...
	def setUp(self):
		kanivin.cache.delete_value("energy_point_rule_map")

	def tearDown(self):
		kanivin.set_user("Administrator")
		kanivin.db.delete("Energy Point Log")
		kanivin.db.delete("Energy Point Rule")
		kanivin.cache.delete_value("energy_point_rule_map")
	...
	def test_revert_points_on_cancelled_doc(self):
		...
		energy_point_logs = kanivin.get_all("Energy Point Log")
		...
		energy_point_logs = kanivin.get_all("Energy Point Log", fields=["reference_name", "type", "reverted"])
	...
	def test_allow_creation_of_new_log_if_the_previous_log_was_reverted(self):
		...
		log_name = kanivin.db.exists("Energy Point Log", {"reference_name": created_todo.name})
		kanivin.get_doc("Energy Point Log", log_name).revert("Just for test")
...
def create_energy_point_rule_for_todo(
	...
	name = "ToDo Closed"
	point_rule_exists = kanivin.db.exists("Energy Point Rule", name)

	if point_rule_exists:
		return kanivin.get_doc("Energy Point Rule", name)
