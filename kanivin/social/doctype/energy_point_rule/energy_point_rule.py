...
from kanivin.core.doctype.user.user import get_enabled_users
from kanivin.model import log_types
from kanivin.model.document import Document
from kanivin.social.doctype.energy_point_log.energy_point_log import create_energy_points_log
...
class EnergyPointRule(Document):
	...
	def on_update(self):
		kanivin.cache_manager.clear_doctype_map("Energy Point Rule", self.reference_doctype)
	...
	def eval_condition(self, doc):
		return self.condition and kanivin.safe_eval(self.condition, None, {"doc": doc.as_dict()})
...
def get_energy_point_doctypes():
	return [
		d.reference_doctype
		for d in kanivin.get_all("Energy Point Rule", ["reference_doctype"], {"enabled": 1})
	]
