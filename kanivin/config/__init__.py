...
def get_modules_from_all_apps_for_user(user: str | None = None) -> list[dict]:
	...
	global_blocked_modules = kanivin.get_doc("User", "Administrator").get_blocked_modules()
...
def get_modules_from_app(app):
	return kanivin.get_all("Module Def", filters={"app_name": app}, fields=["module_name", "app_name as app"])


def get_all_empty_tables_by_module():
	table_rows = kanivin.qb.Field("table_rows")
	table_name = kanivin.qb.Field("table_name")
	information_schema = kanivin.qb.Schema("information_schema")

	empty_tables = (
		kanivin.qb.from_(information_schema.tables).select(table_name).where(table_rows == 0)
	).run()

	empty_tables = {r[0] for r in empty_tables}

	results = kanivin.get_all("DocType", fields=["name", "module"])
