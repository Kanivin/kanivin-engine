...
def build_missing_files():
	...
	for type in ["css", "js"]:
		folder = os.path.join(sites_path, "assets", "kanivin", "dist", type)
	...
	assets_json = kanivin.read_file("assets/assets.json")
	if assets_json:
		assets_json = kanivin.parse_json(assets_json)

		for bundle_file, output_file in assets_json.items():
			if not output_file.startswith("/assets/kanivin"):
				continue
		...
		if missing_assets:
			...
			files_to_build = ["kanivin/" + name for name in missing_assets]
			bundle(build_mode, files=files_to_build)
	else:
		# no assets.json, run full build
		bundle(build_mode, apps="kanivin")


def get_assets_link(kanivin_head) -> str:
	import requests

	tag = getoutput(
		r"cd ../apps/kanivin && git show-ref --tags -d | grep %s | sed -e 's,.*"
		r" refs/tags/,,' -e 's/\^{}//'" % kanivin_head
	)

	if tag:
		# if tag exists, download assets from github release
		url = f"https://github.com/kanivin/kanivin/releases/download/{tag}/assets.tar.gz"
	else:
		url = f"http://assets.kanivinframework.com/{kanivin_head}.tar.gz"

	if not requests.head(url):
		reference = f"Release {tag}" if tag else f"Commit {kanivin_head}"
		raise AssetsDontExistError(f"Assets for {reference} don't exist")

	return url


def fetch_assets(url, kanivin_head):
	click.secho("Retrieving assets...", fg="yellow")

	prefix = mkdtemp(prefix="kanivin-assets-", suffix=kanivin_head)
	assets_archive = download_file(url, prefix)

	if not assets_archive:
		raise AssetsNotDownloadedError(f"Assets could not be retrived from {url}")

	click.echo(click.style("✔", fg="green") + f" Downloaded Kanivin assets from {url}")
...
def setup_assets(assets_archive):
	...
	with tarfile.open(assets_archive) as tar:
		for file in tar:
			if not file.isdir():
				dest = "." + file.name.replace("./kanivin-bench/sites", "")
...
def download_kanivin_assets(verbose=True):
	"""Downloads and sets up Kanivin assets if they exist based on the current
	commit HEAD.
	Returns True if correctly setup else returns False.
	"""
	kanivin_head = getoutput("cd ../apps/kanivin && git rev-parse HEAD")

	if not kanivin_head:
		return False

	try:
		url = get_assets_link(kanivin_head)
		assets_archive = fetch_assets(url, kanivin_head)
...
def symlink(target, link_name, overwrite=False):
	...
	while True:
		temp_link_name = f"tmp{kanivin.generate_hash()}"
...
def setup():
	...
	for app in kanivin.get_all_apps(True):
		try:
			pymodules.append(kanivin.get_module(app))
		except ImportError:
			pass
	app_paths = [os.path.dirname(pymodule.__file__) for pymodule in pymodules]
	assets_path = os.path.join(kanivin.local.sites_path, "assets")
...
def bundle(
	...
	kanivin_app_path = kanivin.get_app_source_path("kanivin")
	kanivin.commands.popen(command, cwd=kanivin_app_path, env=get_node_env(), raise_err=True)

	with suppress(Exception):
		kanivin.cache.flushall()
...
def watch(apps=None):
	...
	live_reload = kanivin.utils.cint(os.environ.get("LIVE_RELOAD", kanivin.conf.live_reload))
	...
	check_node_executable()
	kanivin_app_path = kanivin.get_app_source_path("kanivin")
	kanivin.commands.popen(command, cwd=kanivin_app_path, env=get_node_env())
...
def generate_assets_map():
	...
	for app_name in kanivin.get_all_apps():
		app_doc_path = None

		pymodule = kanivin.get_module(app_name)
...
def html_to_js_template(path, content):
	"""returns HTML template content as Javascript code, adding it to `kanivin.templates`"""
	return """kanivin.templates["{key}"] = '{content}';\n""".format(
