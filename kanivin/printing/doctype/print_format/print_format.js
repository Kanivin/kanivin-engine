...
kanivin.ui.form.on("Print Format", "onload", function (frm) {
	frm.add_fetch("doc_type", "module", "module");
});

kanivin.ui.form.on("Print Format", {
	refresh: function (frm) {
		frm.set_intro("");
		frm.toggle_enable(["html", "doc_type", "module"], false);
		if (kanivin.session.user === "Administrator" || frm.doc.standard === "No") {
			frm.toggle_enable(["html", "doc_type", "module"], true);
			frm.enable_save();
		}

		if (frm.doc.standard === "Yes" && kanivin.session.user !== "Administrator") {
			frm.set_intro(__("Please duplicate this to make changes"));
		}
		frm.trigger("render_buttons");
		frm.toggle_display("standard", kanivin.boot.developer_mode);
		...
		if (!frm.is_new()) {
			if (!frm.doc.custom_format) {
				frm.add_custom_button(__("Edit Format"), function () {
					if (!frm.doc.doc_type) {
						kanivin.msgprint(__("Please select DocType first"));
						return;
					}
					if (frm.doc.print_format_builder_beta) {
						kanivin.set_route("print-format-builder-beta", frm.doc.name);
					} else {
						kanivin.set_route("print-format-builder", frm.doc.name);
					}
				});
			} else if (frm.doc.custom_format && !frm.doc.raw_printing) {
				frm.set_df_property("html", "reqd", 1);
			}
			if (kanivin.model.can_write("Customize Form")) {
				kanivin.model.with_doctype(frm.doc.doc_type, function () {
					let current_format = kanivin.get_meta(frm.doc.doc_type).default_print_format;
					...
					frm.add_custom_button(__("Set as Default"), function () {
						kanivin.call({
							method: "kanivin.printing.doctype.print_format.print_format.make_default",
							args: {
								name: frm.doc.name,
							},
							callback: function () {
	...
	hide_absolute_value_field: function (frm) {
		...
		if (doctype) {
			kanivin.model.with_doctype(doctype, () => {
				const meta = kanivin.get_meta(doctype);
