...
from kanivin.utils.weasyprint import download_pdf, get_html
...
class PrintFormat(Document):
	...
	def validate(self):
		if (
			...
			and not kanivin.flags.in_migrate
			and not kanivin.flags.in_install
			and not kanivin.flags.in_test
		):
			kanivin.throw(kanivin._("Standard Print Format cannot be updated"))

		# old_doc_type is required for clearing item cache
		self.old_doc_type = kanivin.db.get_value("Print Format", self.name, "doc_type")
		...
		if self.custom_format and self.raw_printing and not self.raw_commands:
			kanivin.throw(_("{0} are required").format(kanivin.bold(_("Raw Commands"))), kanivin.MandatoryError)

		if self.custom_format and not self.html and not self.raw_printing:
			kanivin.throw(_("{0} is required").format(kanivin.bold(_("HTML"))), kanivin.MandatoryError)
	...
	def on_update(self):
		if hasattr(self, "old_doc_type") and self.old_doc_type:
			kanivin.clear_cache(doctype=self.old_doc_type)
		if self.doc_type:
...
def make_default(name):
	...
	kanivin.has_permission("Print Format", "write")

	print_format = kanivin.get_doc("Print Format", name)

	doctype = kanivin.get_doc("DocType", print_format.doc_type)
	if doctype.custom:
	...
	kanivin.msgprint(
		kanivin._("{0} is now default print format for {1} doctype").format(
			kanivin.bold(name), kanivin.bold(print_format.doc_type)
		)
	)
