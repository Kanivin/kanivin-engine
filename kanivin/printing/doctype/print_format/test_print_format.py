...
test_records = kanivin.get_test_records("Print Format")


class TestPrintFormat(KanivinTestCase):
	def test_print_user(self, style=None):
		print_html = kanivin.get_print("User", "Administrator", style=style)
	...
	def test_export_doc(self):
		doc: "PrintFormat" = kanivin.get_doc("Print Format", test_records[0]["name"])

		# this is only to make export_doc happy
		doc.standard = "Yes"
		_before = kanivin.conf.developer_mode
		kanivin.conf.developer_mode = True
		export_path = doc.export_doc()
		kanivin.conf.developer_mode = _before
		...
		with open(exported_doc_path) as f:
			exported_doc = kanivin.parse_json(f.read())
