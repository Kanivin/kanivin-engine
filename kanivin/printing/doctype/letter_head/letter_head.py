...
from kanivin.utils import flt, is_image
...
class LetterHead(Document):
	...
	def validate_disabled_and_default(self):
		if self.disabled and self.is_default:
			kanivin.throw(_("Letter Head cannot be both disabled and default"))

		if not self.is_default and not self.disabled:
			if not kanivin.db.exists("Letter Head", dict(is_default=1)):
				self.is_default = 1
	...
	def set_image_as_html(
		...
		if not self.get(field) or not is_image(self.get(field)):
			kanivin.msgprint(failure_msg, alert=True, indicator="orange")
			return
...
{dimension}="{dimension_value}" style="{dimension}: {dimension_value}px;">
		...
		kanivin.msgprint(success_msg, alert=True)
	...
	def set_as_default(self):
		...
		if self.is_default:
			kanivin.db.sql("update `tabLetter Head` set is_default=0 where name != %s", self.name)

			set_default("letter_head", self.name)

			# update control panel - so it loads new letter directly
			kanivin.db.set_default("default_letter_head_content", self.content)
		else:
			kanivin.defaults.clear_default("letter_head", self.name)
			kanivin.defaults.clear_default("default_letter_head_content", self.content)
