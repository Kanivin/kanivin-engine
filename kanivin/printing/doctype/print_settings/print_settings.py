...
class PrintSettings(Document):
	...
	def validate(self):
		if self.pdf_page_size == "Custom" and not (self.pdf_page_height and self.pdf_page_width):
			kanivin.throw(_("Page height and width cannot be zero"))
...
def is_print_server_enabled():
	if not hasattr(kanivin.local, "enable_print_server"):
		kanivin.local.enable_print_server = cint(
			kanivin.db.get_single_value("Print Settings", "enable_print_server")
		)

	return kanivin.local.enable_print_server
