kanivin.pages["print-format-builder-beta"].on_page_load = function (wrapper) {
	...
	if (kanivin.boot.developer_mode) {
		kanivin.hot_update = kanivin.hot_update || [];
		kanivin.hot_update.push(() => load_print_format_builder_beta(wrapper));
	}
};

kanivin.pages["print-format-builder-beta"].on_page_show = function (wrapper) {
...
function load_print_format_builder_beta(wrapper) {
	...
	if (route.length > 1) {
		kanivin.require("print_format_builder.bundle.js").then(() => {
			kanivin.print_format_builder = new kanivin.ui.PrintFormatBuilder({
				wrapper: $parent,
				page: wrapper.page,
		...
		let d = new kanivin.ui.Dialog({
			...
			fields: [
				...
				},
				{
					...
					reqd: 1,
					default: kanivin.route_options ? kanivin.route_options.doctype : null,
				},
				{
			...
			primary_action({ action, doctype, print_format, print_format_name }) {
				if (action === "Edit") {
					kanivin.set_route("print-format-builder-beta", print_format);
					...
					kanivin.db
						...
						.then((doc) => {
							kanivin.set_route("print-format-builder-beta", doc.name);
						})
						.finally(() => {
