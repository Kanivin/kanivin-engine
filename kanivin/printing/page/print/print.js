kanivin.pages["print"].on_page_load = function (wrapper) {
	kanivin.ui.make_app_page({
		parent: wrapper,
	});

	let print_view = new kanivin.ui.form.PrintView(wrapper);

	$(wrapper).bind("show", () => {
		const route = kanivin.get_route();
		const doctype = route[1];
		const docname = route.slice(2).join("/");
		if (!kanivin.route_options || !kanivin.route_options.frm) {
			kanivin.model.with_doc(doctype, docname, () => {
				let frm = { doctype: doctype, docname: docname };
				frm.doc = kanivin.get_doc(doctype, docname);
				kanivin.model.with_doctype(doctype, () => {
					frm.meta = kanivin.get_meta(route[1]);
					print_view.show(frm);
				});
			});
		} else {
			print_view.frm = kanivin.route_options.frm.doctype
				? kanivin.route_options.frm
				: kanivin.route_options.frm.frm;
			kanivin.route_options.frm = null;
			print_view.show(print_view.frm);
		}
	});
};

kanivin.ui.form.PrintView = class {
	constructor(wrapper) {
	...
	make() {
		this.print_wrapper = this.page.main.empty().html(
			`<div class="print-preview-wrapper"><div class="print-preview">
				${kanivin.render_template("print_skeleton_loading")}
		...
		this.print_settings = kanivin.model.get_doc(":Print Settings", "Print Settings");
		this.setup_menu();
		this.setup_toolbar();
		this.setup_sidebar();
	...
	add_sidebar_item(df, is_dynamic) {
		...
		let field = kanivin.ui.form.make_control({
			df: df,
	...
	setup_menu() {
		...
		if (kanivin.model.can_create("Print Format")) {
	...
	show(frm) {
		...
		// print designer link
		if (Object.keys(kanivin.boot.versions).includes("print_designer")) {
			...
			this.page.add_inner_message(`
			<a style="line-height: 2.4" href="https://kanivincloud.com/marketplace/apps/print_designer?utm_source=framework-desk&utm_medium=print-view&utm_campaign=try-link">
				${__("Try the new Print Designer")}
			</a>
			`);
		}
	...
	set_breadcrumbs() {
		kanivin.breadcrumbs.add(this.frm.meta.module, this.frm.doctype);
	}
	...
	setup_additional_settings() {
		...
		kanivin
			.xcall("kanivin.printing.page.print.print.get_print_settings_to_show", {
	...
	edit_print_format() {
		...
		if (is_standard_but_editable) {
			kanivin.set_route("Form", "Print Format", print_format.name);
			return;
		}
		if (is_custom_format) {
			if (print_format.print_format_builder_beta) {
				kanivin.set_route("print-format-builder-beta", print_format.name);
			} else {
				kanivin.set_route("print-format-builder", print_format.name);
			}
			return;
		}
		...
		kanivin.prompt(
			...
			(data) => {
				...
				kanivin.set_route("print-format-builder");
	...
	setup_keyboard_shortcuts() {
		this.wrapper.find(".print-toolbar a.btn-default").each((i, el) => {
			kanivin.ui.keys.get_shortcut_group(this.frm.page).add($(el));
		});
	}
	...
	set_default_print_language() {
		let print_format = this.get_print_format();
		this.lang_code =
			this.frm.doc.language || print_format.default_print_language || kanivin.boot.lang;
	...
	preview() {
		...
		this.get_print_html((out) => {
			...
			const print_height_inches = kanivin.dom.pixel_to_inches(print_height);
	...
	setup_print_format_dom(out, $print_format) {
		...
		let base_url = kanivin.urllib.get_base_url();
		let print_css = kanivin.assets.bundled_asset(
			"print.bundle.css",
			kanivin.utils.is_rtl(this.lang_code)
		);
		this.$print_format_body
			.find("html")
			.attr("dir", kanivin.utils.is_rtl(this.lang_code) ? "rtl" : "ltr");
	...
	go_to_form_view() {
		...
		kanivin.set_route("Form", this.frm.doctype, this.frm.docname);
	}
			...
			if (me.is_raw_printing()) {
				me.get_raw_commands(function (out) {
					kanivin.ui.form
						...
						.then(kanivin.ui.form.qz_success)
						.catch((err) => {
							kanivin.ui.form.qz_fail(err);
						});
				});
			} else {
				kanivin.show_alert(
	...
	print_by_server() {
		...
		if (localStorage.getItem("network_printer")) {
			kanivin.call({
				method: "kanivin.utils.print_format.print_by_server",
				args: {
	...
	network_printer_setting_dialog(callback) {
		kanivin.call({
			method: "kanivin.printing.doctype.network_printer_settings.network_printer_settings.get_network_printer_settings",
			callback: function (r) {
				if (r.message) {
	...
	async is_wkhtmltopdf_valid() {
		const is_valid = await kanivin.xcall("kanivin.utils.pdf.is_wkhtmltopdf_valid");
	...
	render_pdf() {
		...
		if (print_format.print_format_builder_beta) {
			...
			let w = window.open(`/api/method/kanivin.utils.weasyprint.download_pdf?${params}`);
			if (!w) {
				kanivin.msgprint(__("Please enable pop-ups"));
				return;
			}
		} else {
			this.is_wkhtmltopdf_valid();
			this.render_page("/api/method/kanivin.utils.print_format.download_pdf?");
		}
	}
	...
	get_print_html(callback) {
		...
		this._req = kanivin.call({
			method: "kanivin.www.printview.get_html_and_style",
			args: {
				doc: this.frm.doc,
	...
	get_raw_commands(callback) {
		...
		kanivin.call({
			method: "kanivin.www.printview.get_rendered_raw_commands",
			args: {
				doc: this.frm.doc,
				print_format: this.selected_format(),
				_lang: this.lang_code,
	...
	set_style(style) {
		kanivin.dom.set_style(style || kanivin.boot.print_css, "print-style");
	}
	...
	printer_setting_dialog() {
		...
		this.printer_list = [];
		kanivin.ui.form.qz_get_printer_list().then((data) => {
						...
						fields: [
							{
								...
								fieldname: "print_format",
								default: 0,
								options: kanivin.meta.get_print_formats(this.frm.doctype),
								read_only: 0,
								in_list_view: 1,
								label: __("Print Format"),
							},
			...
			if (!(this.printer_list && this.printer_list.length)) {
				kanivin.throw(__("No Printer is Available."));
			}
		});
	}
};
