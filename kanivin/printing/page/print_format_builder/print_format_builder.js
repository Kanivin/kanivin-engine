kanivin.pages["print-format-builder"].on_page_load = function (wrapper) {
	kanivin.print_format_builder = new kanivin.PrintFormatBuilder(wrapper);
	kanivin.breadcrumbs.add("Setup", "Print Format");
};

kanivin.pages["print-format-builder"].on_page_show = function (wrapper) {
	var route = kanivin.get_route();
	if (route.length > 1) {
		kanivin.model.with_doc("Print Format", route[1], function () {
			kanivin.print_format_builder.print_format = kanivin.get_doc("Print Format", route[1]);
			kanivin.print_format_builder.refresh();
		});
	} else if (kanivin.route_options) {
		if (kanivin.route_options.make_new) {
			let { doctype, name, based_on, beta } = kanivin.route_options;
			kanivin.route_options = null;
			kanivin.print_format_builder.setup_new_print_format(doctype, name, based_on, beta);
		} else {
			kanivin.print_format_builder.print_format = kanivin.route_options.doc;
			kanivin.route_options = null;
			kanivin.print_format_builder.refresh();
		}
	}
...
kanivin.PrintFormatBuilder = class PrintFormatBuilder {
	constructor(parent) {
		this.parent = parent;
		this.make();
		this.refresh();
	...
	make() {
		this.page = kanivin.ui.make_app_page({
			parent: this.parent,
		...
		this.page.main = $(
			'<div class="col-md-12 border print-format-builder-main kanivin-card"></div>'
	...
	show_start() {
		this.page.main.html(kanivin.render_template("print_format_builder_start", {}));
	...
	start_edit_print_format() {
		...
		var me = this;
		this.print_format_input = kanivin.ui.form.make_control({
		...
		this.page.main.find(".btn-edit-print-format").on("click", function () {
			...
			if (!name) return;
			kanivin.model.with_doc("Print Format", name, function (doc) {
				kanivin.set_route("print-format-builder", name);
			});
		});
	}
	start_new_print_format() {
		var me = this;
		this.doctype_input = kanivin.ui.form.make_control({
		...
		this.name_input = kanivin.ui.form.make_control({
		...
		this.page.main.find(".btn-new-print-format").on("click", function () {
			...
			if (!(doctype && name)) {
				kanivin.msgprint(__("Both DocType and Name required"));
				return;
			}
	...
	setup_new_print_format(doctype, name, based_on, beta) {
		kanivin.call({
			method: "kanivin.printing.page.print_format_builder.print_format_builder.create_custom_format",
			args: {
				doctype: doctype,
				name: name,
				based_on: based_on,
	...
	setup_print_format() {
		var me = this;
		kanivin.model.with_doctype(this.print_format.doc_type, function (doctype) {
			me.meta = kanivin.get_meta(me.print_format.doc_type);
			me.setup_sidebar();
			me.render_layout();
			...
			me.page.add_inner_button(__("Edit Properties"), function () {
				kanivin.set_route("Form", "Print Format", me.print_format.name);
			});
		});
	}
	setup_sidebar() {
		...
		this.page.sidebar.html(
			$(kanivin.render_template("print_format_builder_sidebar", { fields: fields }))
		);
	...
	render_layout() {
		...
		this.prepare_data();
		$(
			kanivin.render_template("print_format_builder_layout", {
		...
		$.each(this.data, function (i, f) {
			...
			if (!f.name && f.fieldname) {
				...
				if (f.fieldname === "_custom_html") {
					...
					f = $.extend(
						kanivin.meta.get_docfield(me.print_format.doc_type, f.fieldname) || {},
						f
					);
				}
			}
			...
			if (f.fieldtype === "Section Break") {
				...
				set_column();
			} else if (!kanivin.model.layout_fields.includes(f.fieldtype)) {
	...
	init_visible_columns(f) {
		f.visible_columns = [];
		$.each(kanivin.get_meta(f.options).fields, function (i, _f) {
			if (
			...
			onAdd: function (evt) {
				...
				if (!$item.hasClass("print-format-builder-field")) {
					...
					if (fieldname === "_custom_html") {
						...
						field = kanivin.meta.get_docfield(me.print_format.doc_type, fieldname);
					}

					var html = kanivin.render_template("print_format_builder_field", {
						field: field,
						me: me,
					});
	...
	setup_add_section() {
		...
		this.page.main.find(".print-format-builder-add-section").on("click", function () {
			...
			var $section = $(
				kanivin.render_template("print_format_builder_section", {
					section: section,
					me: me,
				})
	...
	setup_column_selector() {
		...
		this.page.main.on("click", ".select-columns", function () {
			...
			var doc_fields = kanivin.get_meta(doctype).fields;
			...
			// render checkboxes
			$(
				kanivin.render_template("print_format_builder_column_selector", {
					fields: fields,
	...
	save_print_format() {
		...
		kanivin.call({
			...
			callback: function (r) {
				...
				kanivin.show_alert({ message: __("Saved"), indicator: "green" });
			},
		});
	}
};
