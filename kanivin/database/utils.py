...
from kanivin.query_builder.functions import Function
...
class LazyMogrify(LazyString):
	...
	def _setup(self) -> str:
		return kanivin.db.mogrify(self.query, self.values)
...
def dangerously_reconnect_on_connection_abort(func):
	...
	def wrapper(*args, **kwargs):
		try:
			...
			if kanivin.db.is_interface_error(e) or isinstance(e, kanivin.db.OperationalError):
				kanivin.db.connect()
				return func(*args, **kwargs)
			raise
