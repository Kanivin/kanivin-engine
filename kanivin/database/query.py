...
from kanivin.database.operator_map import OPERATOR_MAP
from kanivin.database.schema import SPECIAL_CHAR_PATTERN
from kanivin.database.utils import DefaultOrderBy, get_doctype_name
from kanivin.query_builder import Criterion, Field, Order, functions
from kanivin.query_builder.functions import Function, SqlFunctions
from kanivin.query_builder.utils import PseudoColumnMapper
from kanivin.utils.data import MARIADB_SPECIFIC_COMMENT
...
# less restrictive version of kanivin.core.doctype.doctype.doctype.START_WITH_LETTERS_PATTERN
...
class Engine:
	def get_query(
		...
		*,
		...
		self.is_mariadb = kanivin.db.db_type == "mariadb"
		self.is_postgres = kanivin.db.db_type == "postgres"
		...
		if isinstance(table, Table):
			...
			self.table = kanivin.qb.DocType(table)

		if update:
			self.query = kanivin.qb.update(self.table)
		elif into:
			self.query = kanivin.qb.into(self.table)
		elif delete:
			self.query = kanivin.qb.from_(self.table).delete()
		else:
			self.query = kanivin.qb.from_(self.table)
	...
	def validate_doctype(self):
		if not TABLE_NAME_PATTERN.match(self.doctype):
			kanivin.throw(_("Invalid DocType: {0}").format(self.doctype))
	...
	def _apply_filter(
		...
		if not isinstance(_field, str):
			...
			kanivin.throw(_("Invalid filter: {0}").format(_field))
		elif not doctype or doctype == self.doctype:
			_field = self.table[field]
		elif doctype:
			_field = kanivin.qb.DocType(doctype)[field]
		...
		if _operator in OPERATOR_MAP["nested_set"]:
			...
			_df = kanivin.get_meta(self.doctype).get_field(field)
...
class Permission:
	...
	def check_permissions(cls, query, **kwargs):
		...
		for dt in doctype:
			...
			) and not kanivin.has_permission(
				dt,
				"read",
				user=kwargs.get("user"),
				parent_doctype=kwargs.get("parent_doctype"),
			):
				kanivin.throw(_("Insufficient Permission for {0}").format(kanivin.bold(dt)))
	...
	def parse(field: str, doctype: str):
		if "." in field:
			...
			if field.startswith("`tab") or field.startswith('"tab'):
				...
				linked_field = kanivin.get_meta(doctype).get_field(linked_fieldname)
				linked_doctype = linked_field.options
				if linked_field.fieldtype == "Link":
					return LinkTableField(linked_doctype, fieldname, doctype, linked_fieldname, alias=alias)
				elif linked_field.fieldtype in kanivin.model.table_fields:
...
class ChildTableField(DynamicTableField):
	def __init__(
		...
		self.alias = alias
		self.parent_doctype = parent_doctype
		self.table = kanivin.qb.DocType(self.doctype)
		self.field = self.table[self.fieldname]

	def apply_select(self, query: QueryBuilder) -> QueryBuilder:
		table = kanivin.qb.DocType(self.doctype)
	...
	def apply_join(self, query: QueryBuilder) -> QueryBuilder:
		table = kanivin.qb.DocType(self.doctype)
		main_table = kanivin.qb.DocType(self.parent_doctype)
...
class ChildQuery:
	def __init__(
		...
		field = kanivin.get_meta(parent_doctype).get_field(fieldname)
		if field.fieldtype not in kanivin.model.table_fields:
			return
		self.fieldname = fieldname
		self.fields = fields
		self.parent_doctype = parent_doctype
...
def get_nested_set_hierarchy_result(doctype: str, name: str, hierarchy: str) -> list[str]:
	...
	table = kanivin.qb.DocType(doctype)
	try:
		lft, rgt = kanivin.qb.from_(table).select("lft", "rgt").where(table.name == name).run()[0]
	except IndexError:
		lft, rgt = None, None
