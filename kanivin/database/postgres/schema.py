...
from kanivin.database.schema import DBTable, get_definition
from kanivin.utils import cint, flt


class PostgresTable(DBTable):
	def create(self):
		varchar_len = kanivin.db.VARCHAR_LEN
		...
		if not self.meta.issingle and self.meta.autoname == "autoincrement":
			kanivin.db.create_sequence(self.doctype, check_not_exists=True)
	...
	def create_indexes(self):
		...
		if create_index_query:
			# nosemgrep
			kanivin.db.sql(create_index_query)
	...
	def alter(self):
		...
		for col in self.set_default:
			...
			if col.fieldtype in ("Check", "Int"):
				...
				col_default = f"{kanivin.db.escape(col.default)}"
		...
		try:
			if query:
				...
				# nosemgrep
				kanivin.db.sql(final_alter_query)
			if create_contraint_query:
				# nosemgrep
				kanivin.db.sql(create_contraint_query)
			if drop_contraint_query:
				# nosemgrep
				kanivin.db.sql(drop_contraint_query)
		except Exception as e:
			# sanitize
			if kanivin.db.is_duplicate_fieldname(e):
				kanivin.throw(str(e))
			elif kanivin.db.is_duplicate_entry(e):
