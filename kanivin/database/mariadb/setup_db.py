...
def get_mariadb_variables():
	return kanivin._dict(kanivin.db.sql("show variables"))
...
def setup_database(force, verbose, mariadb_user_host_login_scope=None):
	kanivin.local.session = kanivin._dict({"user": "Administrator"})

	db_name = kanivin.local.conf.db_name
	...
	dbman.create_user(db_name, kanivin.conf.db_password, **dbman_kwargs)
	if verbose:
...
def drop_user_and_database(db_name, root_login, root_password):
	kanivin.local.db = get_root_connection(root_login, root_password)
	dbman = DbManager(kanivin.local.db)
	dbman.drop_database(db_name)
...
def bootstrap_database(verbose, source_sql=None):
	...
	if "tabDefaultValue" not in kanivin.db.get_tables(cached=False):
...
def check_compatible_versions():
	try:
		...
		if version_tuple < (10, 6):
			click.secho(
				f"Warning: MariaDB version {version} is less than 10.6 which is not supported by Kanivin",
				fg="yellow",
			)
		elif version_tuple >= (10, 9):
			click.secho(
				f"Warning: MariaDB version {version} is more than 10.8 which is not yet tested with Kanivin Framework.",
				fg="yellow",
			)
	except Exception:
		click.secho(
