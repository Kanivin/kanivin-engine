...
from kanivin.database.mariadb.schema import MariaDBTable
from kanivin.utils import UnicodeWithAttrs, cstr, get_datetime, get_table_name
...
class MariaDBConnectionUtil:
	...
	def get_connection_settings(self) -> dict:
		...
		if kanivin.conf.local_infile:
			conn_settings["local_infile"] = kanivin.conf.local_infile

		if kanivin.conf.db_ssl_ca and kanivin.conf.db_ssl_cert and kanivin.conf.db_ssl_key:
			conn_settings["ssl"] = {
				"ca": kanivin.conf.db_ssl_ca,
				"cert": kanivin.conf.db_ssl_cert,
				"key": kanivin.conf.db_ssl_key,
			}
...
class MariaDBDatabase(MariaDBConnectionUtil, MariaDBExceptionUtil, Database):
	...
	def escape(s, percent=True):
		...
		# Also, given we're promoting use of the PyPika builder via kanivin.qb, the use
		# of this method should be limited.

		# pymysql expects unicode argument to escape_string with Python 3
		s = kanivin.as_unicode(escape_string(kanivin.as_unicode(s)), "utf-8").replace("`", "\\`")
	...
	def get_column_index(self, table_name: str, fieldname: str, unique: bool = False) -> kanivin._dict | None:
	...
	def add_index(self, doctype: str, fields: list, index_name: str | None = None):
		...
		if not self.has_index(table_name, index_name):
			...
			# via code or console.
			if len(fields) == 1 and not (kanivin.flags.in_install or kanivin.flags.in_migrate):
	...
	def get_tables(self, cached=True):
		...
		if cached:
			tables = kanivin.cache.get_value("db_tables")
			to_query = not tables
		...
		if to_query:
			...
			tables = (
				...
				.where(information_schema.tables.table_schema == kanivin.db.cur_db_name)
				.run(pluck=True)
			)
			kanivin.cache.set_value("db_tables", tables)
	...
	def estimate_count(self, doctype: str):
		...
		from kanivin.utils.data import cint
