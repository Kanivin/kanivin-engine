...
from kanivin.database.schema import DBTable
...
class MariaDBTable(DBTable):
	...
	def alter(self):
		...
		add_index_query = [
			...
			if not kanivin.db.get_column_index(self.table_name, col.fieldname, unique=False)
		]

		if self.meta.sort_field == "creation" and not kanivin.db.get_column_index(
		...
		for col in {*self.drop_index, *self.drop_unique}:
			...
			if unique_constraint_changed and not col.unique:
				if unique_index := kanivin.db.get_column_index(self.table_name, col.fieldname, unique=True):
			...
			if index_constraint_changed and not col.set_index:
				if index_record := kanivin.db.get_column_index(self.table_name, col.fieldname, unique=False):
