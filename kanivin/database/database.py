...
from kanivin.database.utils import (
	DefaultOrderBy,
	EmptyQueryValues,
	FallBackDateTimeStr,
	LazyMogrify,
...
from kanivin.exceptions import DoesNotExistError, ImplicitCommitError
from kanivin.monitor import get_trace_id
from kanivin.query_builder import Case
from kanivin.query_builder.functions import Count
from kanivin.utils import CallbackManager, cint, get_datetime, get_table_name, getdate, now, sbool
from kanivin.utils import cast as cast_fieldtype
from kanivin.utils.deprecations import deprecated, deprecation_warning
...
class Database:
	...
	class InvalidColumnName(kanivin.ValidationError):
		pass
	...
	def __init__(
		...
		self.value_cache = {}
		self.logger = kanivin.logger("database")
	...
	def sql(
		...
		*,
		...
		Examples:
		        ...
		        kanivin.db.sql("select name from tabCustomer", as_dict=True)

		        # return names beginning with a
		        kanivin.db.sql("select name from tabCustomer where name like %s", "a%")

		        # values as dict
		        kanivin.db.sql("select name from tabCustomer where name like %(name)s and owner=%(owner)s",
		                {"name": "a%", "owner":"test@example.com"})

		"""
		if isinstance(query, MySQLQueryBuilder | PostgreSQLQueryBuilder):
			kanivin.log("Use run method to execute SQL queries generated by Query Builder")
		...
		try:
			...
			if self.is_syntax_error(e):
				kanivin.log(f"Syntax error in query:\n{query} {values or ''}")

			elif self.is_deadlocked(e):
				raise kanivin.QueryDeadlockError(e) from e

			elif self.is_timedout(e):
				raise kanivin.QueryTimeoutError(e) from e
				...
				kanivin.throw(
					...
					exc=kanivin.InReadOnlyMode,
				)

			# TODO: added temporarily
			elif self.db_type == "postgres":
				traceback.print_stack()
				kanivin.log(f"Error in query:\n{e}")
				raise
		...
		if debug:
			time_end = time()
			kanivin.log(f"Execution time: {time_end - time_start:.2f} sec")
	...
	def _return_as_iterator(self, *, pluck, as_dict, as_list, update):
		while result := self._transform_result(self._cursor.fetchmany(SQL_ITERATOR_BATCH_SIZE)):
			if pluck:
				...
				for row in result:
					row = kanivin._dict(zip(keys, row, strict=False))
					if update:
						row.update(update)
					yield row
				...
				kanivin.throw(_("`as_iterator` only works with `as_list=True` or `as_dict=True`"))
	...
	def _log_query(
		...
		if kanivin.conf.allow_tests and kanivin.cache.get_value("flag_print_sql"):
	...
	def mogrify(self, query: Query, values: QueryValues):
		...
		try:
			...
			if isinstance(values, dict):
				return query % {
					k: kanivin.db.escape(v) if isinstance(v, str) else v for k, v in values.items()
				}
			elif isinstance(values, list | tuple):
				return query % tuple(kanivin.db.escape(v) if isinstance(v, str) else v for v in values)
			return query, values
	...
	def explain_query(self, query, values=None):
		...
		kanivin.log("--- query explain ---")
		try:
			self._cursor.execute(f"EXPLAIN {query}", values)
		except Exception as e:
			kanivin.log(f"error in query explain: {e}")
		else:
			kanivin.log(json.dumps(self.fetch_as_dict(), indent=1))
			kanivin.log("--- query explain end ---")
	...
	def sql_list(self, query, values=(), debug=False, **kwargs):
		...
		Example:
		        ...
		        doctypes = kanivin.db.sql_list("select name from DocType")
		"""
		...
		if query.lstrip()[:6].lower() in ("update", "insert", "delete"):
			...
			if self.transaction_writes > self.MAX_WRITES_PER_TRANSACTION:
				if self.auto_commit_on_many_writes:
					...
					raise kanivin.TooManyWritesError(msg)
	...
	def fetch_as_dict(self, result) -> list[kanivin._dict]:
		"""Internal. Convert results to dict."""
		if result:
			keys = [column[0] for column in self._cursor.description]

		return [kanivin._dict(zip(keys, row, strict=False)) for row in result]

	@staticmethod
	def clear_db_table_cache(query):
		if query and is_query_type(query, ("drop", "create")):
			kanivin.cache.delete_key("db_tables")
	...
	def get_value(
		...
		*,
		...
		Example:
		        ...
		        kanivin.db.get_value("Customer", {"name": ("like a%")})

		        # return last login of **User** `test@example.com`
		        kanivin.db.get_value("User", "test@example.com", "last_login")

		        last_login, last_ip = kanivin.db.get_value("User", "test@example.com",
		                ["last_login", "last_ip"])

		        # returns default date_format
		        kanivin.db.get_value("System Settings", None, "date_format")
		"""
	...
	def get_values(
		...
		*,
		...
		Example:
		        ...
		        # return first customer starting with a
		        customers = kanivin.db.get_values("Customer", {"name": ("like a%")})

		        # return last login of **User** `test@example.com`
		        user = kanivin.db.get_values("User", "test@example.com", "*")[0]
		"""
		out = None
		...
		if isinstance(filters, list):
			...
			if (filters is not None) and (filters != doctype or doctype == "DocType"):
				try:
					...
					if ignore and (
						kanivin.db.is_missing_column(e)
						or kanivin.db.is_table_missing(e)
						or str(e).startswith("Invalid DocType")
					):
						out = None
					elif (not ignore) and kanivin.db.is_table_missing(e):
	...
	def get_singles_dict(self, doctype, debug=False, *, for_update=False, cast=False):
		...
		Example:
		        ...
		        account_settings = kanivin.db.get_singles_dict("Accounts Settings")
		"""
		queried_result = kanivin.qb.get_query(
			"Singles",
		...
		if not cast:
			return kanivin._dict(queried_result)
	...
	def get_all(*args, **kwargs):
		return kanivin.get_all(*args, **kwargs)

	@staticmethod
	def get_list(*args, **kwargs):
		return kanivin.get_list(*args, **kwargs)
	...
	def _get_update_dict(
		...
		if update_modified:
			...
			modified_by = modified_by or kanivin.session.user
	...
	def set_single_value(
		...
		*,
		...
		Example:
		        ...
		        kanivin.db.set_single_value("System Settings", "deny_multiple_sessions", True)
		"""
		...
		kanivin.qb.into("Singles").columns("doctype", "field", "value").insert(*singles_data).run(debug=debug)
		kanivin.clear_document_cache(doctype, doctype)
	...
	def get_single_value(self, doctype, fieldname, cache=True):
		...
		Example:
		        ...
		        company = kanivin.db.get_single_value('Global Defaults', 'default_company')
		"""
		...
		df = kanivin.get_meta(doctype).get_field(fieldname)

		if not df:
			kanivin.throw(
				_("Field {0} does not exist on {1}").format(
					kanivin.bold(fieldname), kanivin.bold(doctype), self.InvalidColumnName
				)
			)
	...
	def set_value(
		...
		**Warning:** this function will not call Document events and should be avoided in normal cases.
		...
		"""
		from kanivin.model.utils import is_single_doctype
		...
		if isinstance(dn, str):
			kanivin.clear_document_cache(dt, dn)
		else:
			# No way to guess which documents are modified, clear all of them
			kanivin.clear_document_cache(dt)
	...
	def _build_and_run_bulk_update_query(
		...
		update_query = kanivin.qb.update(dt)
	...
	def set_default(key, val, parent="__default", parenttype=None):
		...
		kanivin.defaults.set_default(key, val, parent, parenttype)
	...
	def add_default(key, val, parent="__default", parenttype=None):
		...
		kanivin.defaults.add_default(key, val, parent, parenttype)

	@staticmethod
	def get_defaults(key=None, parent="__default"):
		"""Get all defaults"""
		defaults = kanivin.defaults.get_defaults_for(parent)
		if not key:
			return defaults
		...
		return defaults.get(kanivin.scrub(key))

	def begin(self, *, read_only=False):
		read_only = read_only or kanivin.flags.read_only
	...
	def savepoint(self, save_point):
		...
		Changes can be undone to a save point by doing kanivin.db.rollback(save_point)
	...
	def a_row_exists(self, doctype):
		...
		return kanivin.get_all(doctype, limit=1, order_by=None, as_list=True)
	...
	def count(self, dt, filters=None, debug=False, cache=False, distinct: bool = True):
		...
		if cache and not filters:
			cache_count = kanivin.cache.get_value(f"doctype:count:{dt}")
			if cache_count is not None:
				return cache_count
		count = kanivin.qb.get_query(
			table=dt,
			filters=filters,
			fields=Count("*"),
			distinct=distinct,
		...
		if not filters and cache:
			kanivin.cache.set_value(f"doctype:count:{dt}", count, expires_in_sec=86400)
		return count
	...
	def get_db_table_columns(self, table) -> list[str]:
		...
		columns = kanivin.cache.hget("table_columns", table)
		if columns is None:
			information_schema = kanivin.qb.Schema("information_schema")

			columns = (
				kanivin.qb.from_(information_schema.columns)
			...
			if columns:
				kanivin.cache.hset("table_columns", table, columns)
	...
	def get_system_setting(self, key):
		return kanivin.get_system_settings(key)
	...
	def is_column_missing(e):
		return kanivin.db.is_missing_column(e)

	def get_descendants(self, doctype, name):
		"""Return descendants of the group node in tree"""
		from kanivin.utils.nestedset import get_descendants_of
	...
	def log_touched_tables(self, query):
		if is_query_type(query, ("insert", "delete", "update", "alter", "drop", "rename")):
			...
			if kanivin.flags.touched_tables is None:
				kanivin.flags.touched_tables = set()
			kanivin.flags.touched_tables.update(tables)
	...
	def bulk_insert(
		...
		*,
		...
		query = kanivin.qb.into(table).columns(fields)
	...
	def create_sequence(self, *args, **kwargs):
		from kanivin.database.sequence import create_sequence

		return create_sequence(*args, **kwargs)

	def set_next_sequence_val(self, *args, **kwargs):
		from kanivin.database.sequence import set_next_val

		set_next_val(*args, **kwargs)

	def get_next_sequence_val(self, *args, **kwargs):
		from kanivin.database.sequence import get_next_val
	...
	def unbuffered_cursor(self):
		...
		Usage:
		        with kanivin.db.unbuffered_cursor():
		                for row in kanivin.db.sql("query with huge result", as_iterator=True):
...
def savepoint(catch: type | tuple[type, ...] = Exception):
	...
	try:
		...
		kanivin.db.release_savepoint(savepoint)
...
def get_query_execution_timeout() -> int:
	...
	if not kanivin.conf.get("enable_db_statement_timeout"):
		return 0

	# Zero means no timeout, which is the default value in db.
	timeout = 0
	with suppress(Exception):
		if getattr(kanivin.local, "request", None):
			timeout = kanivin.conf.http_timeout or 300
