...
from kanivin.utils import cint, cstr, flt
...
class DBTable:
	def __init__(self, doctype, meta=None):
		...
		self.meta = meta or kanivin.get_meta(doctype, False)
	...
	def sync(self):
		...
		if self.is_new():
			...
			kanivin.cache.hdel("table_columns", self.table_name)
			self.alter()
	...
	def get_column_definitions(self):
		column_list = [*kanivin.db.DEFAULT_COLUMNS]
		ret = []
	...
	def get_index_definitions(self):
		return [
			...
			if (
				col.set_index
				and not col.unique
				and col.fieldtype in kanivin.db.type_map
				and kanivin.db.type_map.get(col.fieldtype)[0] not in ("text", "longtext")
			)
		]
	...
	def get_columns_from_docfields(self):
		...
		if not self.meta.get("istable"):
			for fieldname in kanivin.db.OPTIONAL_COLUMNS:
	...
	def validate(self):
		...
		columns = [
			kanivin._dict({"fieldname": f, "fieldtype": "Data"}) for f in kanivin.db.STANDARD_VARCHAR_COLUMNS
		]
		if self.meta.get("istable"):
			columns += [
				kanivin._dict({"fieldname": f, "fieldtype": "Data"}) for f in kanivin.db.CHILD_TABLE_COLUMNS
			]
		...
		for col in columns:
			if len(col.fieldname) >= 64:
				kanivin.throw(
					_("Fieldname is limited to 64 characters ({0})").format(kanivin.bold(col.fieldname))
				)

			if "varchar" in kanivin.db.type_map.get(col.fieldtype, ()):
				# validate length range
				new_length = cint(col.length) or cint(kanivin.db.VARCHAR_LEN)
				if not (1 <= new_length <= 1000):
					kanivin.throw(_("Length of {0} should be between 1 and 1000").format(col.fieldname))
				...
				if cint(current_length) != cint(new_length):
					try:
						...
						max_length = kanivin.db.sql(
							f"""SELECT MAX(CHAR_LENGTH(`{col.fieldname}`)) FROM `tab{self.doctype}`"""
						)

					except kanivin.db.InternalError as e:
					...
					if max_length and max_length[0][0] and max_length[0][0] > new_length:
						...
						kanivin.msgprint(info_message)

	def is_new(self):
		return self.table_name not in kanivin.db.get_tables()

	def setup_table_columns(self):
		# TODO: figure out a way to get key data
		for c in kanivin.db.get_table_columns_description(self.table_name):
...
class DbColumn:
	...
	def get_definition(self, for_modification=False):
		...
		if self.fieldtype in ("Check", "Int"):
			...
			self.default
			and (self.default not in kanivin.db.DEFAULT_SHORTCUTS)
			and not cstr(self.default).startswith(":")
		):
			column_def += f" default {kanivin.db.escape(self.default)}"
	...
	if special_characters := SPECIAL_CHAR_PATTERN.findall(n):
		...
		kanivin.throw(
			_("Fieldname {0} cannot have special characters like {1}").format(
				kanivin.bold(cstr(n)), special_characters
			),
			kanivin.db.InvalidColumnName,
		)
	return n


def validate_column_length(fieldname):
	if len(fieldname) > kanivin.db.MAX_COLUMN_LENGTH:
		kanivin.throw(_("Fieldname is limited to 64 characters ({0})").format(fieldname))


def get_definition(fieldtype, precision=None, length=None):
	d = kanivin.db.type_map.get(fieldtype)
	...
	if fieldtype == "Int" and length and length > 11:
		...
		d = kanivin.db.type_map.get("Long Int")
	...
	if size:
		...
		if length:
			if coltype == "varchar":
				...
				# NOTE: this will only be applicable for mariadb as kanivin implements int
