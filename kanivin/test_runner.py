...
from kanivin.modules import get_module_name, load_doctype_module
...
def main(
	...
	if doctype_list_path:
		...
		with open(kanivin.get_app_path(app, doctype_list_path)) as f:
	...
	try:
		kanivin.flags.print_messages = verbose
		kanivin.flags.in_test = True

		# workaround! since there is no separate test db
		kanivin.clear_cache()
		scheduler_disabled_by_user = kanivin.utils.scheduler.is_scheduler_disabled(verbose=False)
		if not scheduler_disabled_by_user:
			kanivin.utils.scheduler.disable_scheduler()

		if not kanivin.flags.skip_before_tests:
			if verbose:
				print('Running "before_tests" hooks')
			for fn in kanivin.get_hooks("before_tests", app_name=app):
				kanivin.get_attr(fn)()
		...
		if not scheduler_disabled_by_user:
			kanivin.utils.scheduler.enable_scheduler()
...
def run_all_tests(app=None, verbose=False, profile=False, failfast=False, junit_xml_output=False):
	...
	for app in apps:
		for path, folders, files in os.walk(kanivin.get_app_path(app)):
...
def run_tests_for_doctype(
	...
	for doctype in doctypes:
		module = kanivin.db.get_value("DocType", doctype, "module")
		if not module:
		...
		if force:
			for name in kanivin.db.sql_list("select name from `tab%s`" % doctype):
				kanivin.delete_doc(doctype, name, force=True)
...
def _run_unittest(
	...
	kanivin.flags.tests_verbose = verbose
...
def _add_test(app, path, filename, verbose, test_suite=None):
	...
	app_path = kanivin.get_app_path(app)
...
def make_test_records(doctype, verbose=0, force=False, commit=False):
	if kanivin.flags.skip_test_records:
		return
	...
	for options in get_dependencies(doctype):
		...
		if options not in kanivin.local.test_objects:
			kanivin.local.test_objects[options] = []
...
def get_dependencies(doctype):
	...
	for df in meta.get_table_fields():
		link_fields.extend(kanivin.get_meta(df.options).get_link_fields())
...
def make_test_records_for_doctype(doctype, verbose=0, force=False, commit=False):
	...
	if hasattr(test_module, "_make_test_records"):
		kanivin.local.test_objects[doctype] += test_module._make_test_records(verbose)

	elif hasattr(test_module, "test_records"):
		if doctype in kanivin.local.test_objects:
			kanivin.local.test_objects[doctype] += make_test_objects(
		...
		test_records = kanivin.get_test_records(doctype)
		if test_records:
			kanivin.local.test_objects[doctype] += make_test_objects(
...
def make_test_objects(doctype, test_records=None, verbose=None, reset=False, commit=False):
	...
	for doc in test_records:
		...
		if kanivin.db.exists(d.doctype, d.name) and not reset:
			kanivin.db.rollback()
...
def add_to_test_record_log(doctype):
	...
	if doctype not in test_record_log:
		kanivin.flags.test_record_log.append(doctype)
		with open(kanivin.get_site_path(".test_log"), "w") as f:
			f.write("\n".join(filter(None, kanivin.flags.test_record_log)))


def get_test_record_log():
	"""Return the list of doctypes for which test records have been created"""
	if "test_record_log" not in kanivin.flags:
		if os.path.exists(kanivin.get_site_path(".test_log")):
			with open(kanivin.get_site_path(".test_log")) as f:
				kanivin.flags.test_record_log = f.read().splitlines()
		else:
			kanivin.flags.test_record_log = []

	return kanivin.flags.test_record_log
