...
def execute():
	...
	charts_to_modify = kanivin.get_all(
		"Dashboard Chart",
	...
	for chart in charts_to_modify:
		old_filters = kanivin.parse_json(chart.filters_json)
		...
		if chart.filters_json and isinstance(old_filters, dict):
			...
			kanivin.db.set_value("Dashboard Chart", chart.name, "filters_json", new_filters_json)
