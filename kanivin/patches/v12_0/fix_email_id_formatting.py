...
def fix_communications():
	for communication in kanivin.db.sql(
...
def fix_show_as_cc_email_queue():
	for queue in kanivin.get_all(
		"Email Queue",
		{"creation": [">", "2020-06-01"], "status": "Not Sent", "show_as_cc": ["like", "%&lt;%"]},
		["name", "show_as_cc"],
	):
		kanivin.db.set_value("Email Queue", queue["name"], "show_as_cc", format_email_id(queue["show_as_cc"]))


def fix_email_queue_recipients():
	for recipient in kanivin.db.sql(
