...
def execute():
	...
	all_tables = kanivin.db.get_tables()
	final_deletion_map = kanivin._dict()

	for table in all_tables:
		indexes_to_keep_map = kanivin._dict()
		indexes_to_delete = []
		index_info = kanivin.db.sql(
		...
		for index in index_list:
			try:
				if is_clustered_index(table_name, index):
					continue
				kanivin.db.sql_ddl(f"ALTER TABLE `{table_name}` DROP INDEX `{index}`")
			except Exception as e:
				kanivin.log_error("Failed to drop index")
