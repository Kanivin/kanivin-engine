...
import kanivin
from kanivin.permissions import get_valid_perms
from kanivin.utils import cint


def execute():
	kanivin.reload_doctype("User Permission")
	user_permissions = kanivin.get_all("User Permission", fields=["allow", "name", "user"])
	...
	for key, doctype_to_skip in doctype_to_skip_map.items():
		if not doctype_to_skip:
			continue
		if not kanivin.db.has_column("User Permission", "applicable_for") and kanivin.db.has_column(
...
def get_linked_doctypes(doctype):
	from kanivin.permissions import get_linked_doctypes

	linked_doctypes = get_linked_doctypes(doctype)
	child_doctypes = [d.options for d in kanivin.get_meta(doctype).get_table_fields()]
