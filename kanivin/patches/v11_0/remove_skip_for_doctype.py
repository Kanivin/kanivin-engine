...
from kanivin.patches.v11_0.replicate_old_user_permissions import get_doctypes_to_skip
...
def execute():
	kanivin.reload_doctype("User Permission")

	# to check if we need to migrate from skip_for_doctype
	has_skip_for_doctype = kanivin.db.has_column("User Permission", "skip_for_doctype")
	...
	for user_permission in kanivin.get_all("User Permission", fields=["*"]):
		skip_for_doctype = []
		...
		if skip_for_doctype:
			...
			kanivin.db.set_value("User Permission", user_permission.name, "apply_to_all_doctypes", 1)

	if new_user_permissions_list:
		kanivin.qb.into("User Permission").columns(
			"name",
			"user",
			"allow",
			"for_value",
	...
	if user_permissions_to_delete:
		kanivin.db.delete("User Permission", filters=(Field("name").isin(tuple(user_permissions_to_delete))))
