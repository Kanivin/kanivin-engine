...
def execute():
	...
	for name, content in kanivin.get_all(
		...
		kanivin.db.set_value("Comment", name, "content", content, update_modified=False)
	...
	for name, content in kanivin.get_all(
		...
		if content and content.startswith("Removed "):
			kanivin.db.set_value("Comment", name, "content", content[8:], update_modified=False)
