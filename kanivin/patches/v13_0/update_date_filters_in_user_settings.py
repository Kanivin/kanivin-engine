...
import kanivin
from kanivin.model.utils.user_settings import sync_user_settings, update_user_settings


def execute():
	users = kanivin.db.sql("select distinct(user) from `__UserSettings`", as_dict=True)
	...
	for user in users:
		...
		for setting in user_settings:
			data = kanivin.parse_json(setting.get("data"))
			if data:
				for key in data:
