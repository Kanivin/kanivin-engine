import kanivin
from kanivin.model.naming import is_autoincremented
...
def execute():
	...
	for doctype in possible_log_types:
		if (
			kanivin.db.exists("DocType", doctype)
			and _is_implicit_int_pk(doctype)
			and not is_autoincremented(doctype)
		):
			kanivin.db.change_column_type(
				doctype,
				"name",
				type=f"varchar({kanivin.db.VARCHAR_LEN})",
				nullable=True,
			)
...
def _is_implicit_int_pk(doctype: str) -> bool:
	...
	if kanivin.db.db_type == "mariadb":
		...
		values = (kanivin.db.cur_db_name,)

	col_type = kanivin.db.sql(query, values)
