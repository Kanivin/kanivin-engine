...
def run_all(skip_failing: bool = False, patch_type: PatchType | None = None) -> None:
	...
	executed = set(kanivin.get_all("Patch Log", filters={"skipped": 0}, fields="patch", pluck="patch"))
	...
	for patch in kanivin.flags.final_patches:
...
def get_all_patches(patch_type: PatchType | None = None) -> list[str]:
	if patch_type and not isinstance(patch_type, PatchType):
		kanivin.throw(f"Unsupported patch type specified: {patch_type}")
...
def get_patches_from_app(app: str, patch_type: PatchType | None = None) -> list[str]:
	...
	"""
	patches_file = kanivin.get_app_path(app, "patches.txt")
	...
	try:
		...
		if not patch_type or patch_type == PatchType.pre_model_sync:
			return kanivin.get_file_items(patches_file)
...
def parse_as_configfile(patches_file: str, patch_type: PatchType | None = None) -> list[str]:
	...
	if patch_type.value in parser.sections():
		...
		kanivin.throw(kanivin._("Patch type {} not found in patches.txt").format(patch_type))


def reload_doc(args):
	import kanivin.modules

	run_single(method=kanivin.modules.reload_doc, methodargs=args)


def run_single(patchmodule=None, method=None, methodargs=None, force=False):
	from kanivin import conf
...
def execute_patch(patchmodule: str, method=None, methodargs=None):
	...
	print(
		f"Executing {patchmodule or methodargs} in {kanivin.local.site} ({kanivin.db.cur_db_name}){docstring}"
	)
	...
	try:
		if patchmodule:
			if patchmodule.startswith("finally:"):
				...
				kanivin.flags.final_patches.append(patchmodule)
			else:
				if has_patch_file:
					_patch()
				else:
...
def update_patch_log(patchmodule, skipped=False):
	...
	patch = kanivin.get_doc({"doctype": "Patch Log", "patch": patchmodule})
...
def executed(patchmodule):
	...
	return kanivin.db.get_value("Patch Log", {"patch": patchmodule, "skipped": 0})


def _patch_mode(enable):
	"""stop/start execution till patch is run"""
	kanivin.local.flags.in_patch = enable
	kanivin.db.commit()
