...
from kanivin.modules import get_module_path, scrub, scrub_dt_dn
...
def export_to_files(record_list=None, record_module=None, verbose=0, create_init=None):
	...
	if record_list:
		for record in record_list:
			...
			write_document_file(
				kanivin.get_doc(record[0], record[1]),
				record_module,
...
def write_document_file(doc, record_module=None, create_init=True, folder_name=None):
	...
	is_custom_module = kanivin.db.get_value("Module Def", module, "custom")
	...
	if is_custom_module and not Path(path).resolve().is_relative_to(Path(kanivin.get_site_path()).resolve()):
		kanivin.throw("Invalid export path: " + Path(path).as_posix())
	with open(path, "w+") as txtfile:
		txtfile.write(kanivin.as_json(doc_export))
...
def strip_default_fields(doc, doc_export):
	...
	for df in doc.meta.get_table_fields():
		for d in doc_export.get(df.fieldname):
			for fieldname in kanivin.model.default_fields + kanivin.model.child_table_fields:
				if fieldname in d:
					del d[fieldname]
	...
	if hasattr(doc, "get_code_fields"):
		for key, extn in doc.get_code_fields().items():
			if doc.get(key):
				...
				if not Path(path).resolve().is_relative_to(Path(kanivin.get_site_path()).resolve()):
					kanivin.throw("Invalid export path: " + Path(path).as_posix())
				with open(path, "w+") as txtfile:
...
def get_module_name(doc):
	if doc.doctype == "Module Def":
		...
		module = kanivin.db.get_value("DocType", doc.document_type, "module")
	elif hasattr(doc, "module"):
		module = doc.module
	else:
		module = kanivin.db.get_value("DocType", doc.doctype, "module")

	return module


def delete_folder(module, dt, dn):
	if kanivin.db.get_value("Module Def", module, "custom"):
...
def get_custom_module_path(module):
	package = kanivin.db.get_value("Module Def", module, "package")
	if not package:
		kanivin.throw(f"Package must be set for custom Module <b>{module}</b>")

	path = os.path.join(get_package_path(package), scrub(module))
	if not Path(path).resolve().is_relative_to(Path(kanivin.get_site_path()).resolve()):
		kanivin.throw("Invalid module path: " + Path(path).as_posix())
...
def get_package_path(package):
	path = os.path.join(
		kanivin.get_site_path("packages"), kanivin.db.get_value("Package", package, "package_name")
	)
	if not os.path.exists(path):
		os.makedirs(path)
	return path
