...
import kanivin
from kanivin import _, get_module_path, scrub
...
def export_module_json(doc: "Document", is_standard: bool, module: str) -> str | None:
	...
	`/home/gavin/kanivin-bench/apps/kanivin/kanivin/core/print_format/_test_print_format_1/_test_print_format_1`
	"""
	if not kanivin.flags.in_import and is_standard and kanivin.conf.developer_mode:
		...
		return os.path.join(
			kanivin.get_module_path(module), scrub(doc.doctype), scrub(doc.name), scrub(doc.name)
		)
...
def get_doc_module(module: str, doctype: str, name: str) -> "ModuleType":
	...
	module_name = "{app}.{module}.{doctype}.{name}.{name}".format(
		app=kanivin.local.module_app[scrub(module)],
		doctype=scrub(doctype),
		module=scrub(module),
		name=scrub(name),
	)
	return kanivin.get_module(module_name)
...
def export_customizations(
	...
	if not kanivin.conf.developer_mode:
		kanivin.throw(_("Only allowed to export customizations in developer mode"))

	custom = {
		"custom_fields": kanivin.get_all("Custom Field", fields="*", filters={"dt": doctype}, order_by="name"),
		"property_setters": kanivin.get_all(
			"Property Setter", fields="*", filters={"doc_type": doctype}, order_by="name"
		),
		"custom_perms": [],
		"links": kanivin.get_all("DocType Link", fields="*", filters={"parent": doctype}, order_by="name"),
		"doctype": doctype,
		"sync_on_migrate": sync_on_migrate,
	}

	if with_permissions:
		custom["custom_perms"] = kanivin.get_all(
	...
	for d in kanivin.get_meta(doctype).get_table_fields():
	...
	if custom["custom_fields"] or custom["property_setters"] or custom["custom_perms"]:
		...
		with open(path, "w") as f:
			f.write(kanivin.as_json(custom))

		kanivin.msgprint(_("Customizations for <b>{0}</b> exported to:<br>{1}").format(doctype, path))
		return path
...
def sync_customizations(app=None):
	...
	for app_name in apps:
		for module_name in kanivin.local.app_modules.get(app_name) or []:
			folder = kanivin.get_app_path(app_name, module_name, "custom")
			if os.path.exists(folder):
				for fname in os.listdir(folder):
					if fname.endswith(".json"):
						...
						elif kanivin.flags.in_install and app:
							sync_customizations_for_doctype(data, folder, fname)
...
def sync_customizations_for_doctype(data: dict, folder: str, filename: str = ""):
		...
		def sync_single_doctype(doc_type):
			def _insert(data):
				if data.get(doctype_fieldname) == doc_type:
					...
					doc = kanivin.get_doc(data)
					doc.db_insert()

			match custom_doctype:
				case "Custom Field":
					for d in data[key]:
						field = kanivin.db.get_value(
						...
						if not field:
							...
							custom_field = kanivin.get_doc("Custom Field", field)
	...
	if not kanivin.db.exists("DocType", doctype):
	...
	if update_schema and not kanivin.db.get_value("DocType", doctype, "issingle"):
...
def reload_doc(
	...
	from kanivin.modules.import_file import import_files
...
def export_doc(doctype, name, module=None):
	...
	from kanivin.modules.export_file import write_document_file

	print(f"Exporting Document {doctype} {name}")
	module = module or kanivin.db.get_value("DocType", name, "module")
	write_document_file(kanivin.get_doc(doctype, name), module)


def get_doctype_module(doctype: str) -> str:
	"""Returns **Module Def** name of given doctype."""
	doctype_module_map = kanivin.cache.get_value(
		"doctype_modules",
		generator=lambda: dict(kanivin.qb.from_("DocType").select("name", "module").run()),
	)

	if module_name := doctype_module_map.get(doctype):
		return module_name
	else:
		kanivin.throw(_("DocType {} not found").format(doctype), exc=kanivin.DoesNotExistError)
...
def load_doctype_module(doctype, module=None, prefix="", suffix=""):
	...
	if key not in doctype_python_modules:
		try:
			doctype_python_modules[key] = kanivin.get_module(module_name)
...
def get_module_app(module: str) -> str:
	app = kanivin.local.module_app.get(scrub(module))
	if app is None:
		kanivin.throw(_("Module {} not found").format(module), exc=kanivin.DoesNotExistError)
	return app
...
def get_app_publisher(module: str) -> str:
	app = get_module_app(module)
	if not app:
		kanivin.throw(_("App not found for module: {0}").format(module))
	return kanivin.get_hooks(hook="app_publisher", app_name=app)[0]


def make_boilerplate(
	template: str, doc: Union["Document", "kanivin._dict"], opts: Union[dict, "kanivin._dict"] = None
):
	...
	doc = doc or kanivin._dict()
	opts = opts or kanivin._dict()
	app_publisher = get_app_publisher(doc.module)
	base_class = "Document"
	base_class_import = "from kanivin.model.document import Document"
	controller_body = "pass"

	if doc.get("is_tree"):
		base_class = "NestedSet"
		base_class_import = "from kanivin.utils.nestedset import NestedSet"
	...
	with open(target_file_path, "w") as target, open(template_file_path) as source:
		...
		target.write(kanivin.as_unicode(controller_file_content))
