...
from kanivin.modules import get_module_path, scrub_dt_dn
from kanivin.query_builder import DocType
from kanivin.utils import get_datetime, now
...
def import_file_by_path(
	...
	if docs:
		...
		for doc in docs:
			...
			db_modified_timestamp = kanivin.db.get_value(doc["doctype"], doc["name"], "modified")
			...
			if not force and db_modified_timestamp:
				...
				if doc["doctype"] == "DocType":
					try:
						stored_hash = kanivin.db.get_value(doc["doctype"], doc["name"], "migration_hash")
					except Exception:
						pass
			...
			if doc["doctype"] == "DocType":
				...
				kanivin.qb.update(doctype_table).set(doctype_table.migration_hash, calculated_hash).where(
...
def update_modified(original_modified, doc):
	...
	if doc["doctype"] == doc["name"] and doc["name"] != "DocType":
		...
		kanivin.qb.update(singles_table).set(singles_table.value, original_modified).where(
		...
		kanivin.qb.update(doctype_table).set(doctype_table.modified, original_modified).where(
			doctype_table.name == doc["name"]
		).run()
...
def import_doc(
	...
	if kanivin.db.exists(doc.doctype, doc.name):
...
def delete_old_doc(doc, reset_permissions):
	ignore = []
	old_doc = kanivin.get_doc(doc.doctype, doc.name)
	...
	# delete old
	kanivin.delete_doc(doc.doctype, doc.name, force=1, ignore_doctypes=ignore, for_reload=True)
