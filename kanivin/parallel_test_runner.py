...
class ParallelTestRunner:
	def __init__(self, app, site, build_number=1, total_builds=1, dry_run=False):
		self.app = app
		self.site = site
		self.build_number = kanivin.utils.cint(build_number) or 1
		self.total_builds = kanivin.utils.cint(total_builds)
		self.dry_run = dry_run
		self.setup_test_site()
		self.run_tests()

	def setup_test_site(self):
		kanivin.init(site=self.site)
		if not kanivin.db:
			kanivin.connect()
	...
	def before_test_setup(self):
		...
		for fn in kanivin.get_hooks("before_tests", app_name=self.app):
			kanivin.get_attr(fn)()

		test_module = kanivin.get_module(f"{self.app}.tests")
	...
	def get_module(self, path, filename):
		app_path = kanivin.get_app_path(self.app)
...
class ParallelTestWithOrchestrator(ParallelTestRunner):
	...
	def __init__(self, app, site):
		...
		if not self.orchestrator_url:
			...
			click.echo("Pass public URL after hosting https://github.com/kanivin/test-orchestrator")
			sys.exit(1)

		self.ci_build_id = os.environ.get("CI_BUILD_ID")
		self.ci_instance_id = os.environ.get("CI_INSTANCE_ID") or kanivin.generate_hash(length=10)
