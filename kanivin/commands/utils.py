...
from kanivin.coverage import CodeCoverage
from kanivin.exceptions import SiteNotSpecifiedError
from kanivin.utils import cint, update_progress_bar
...
def build(
	...
	from kanivin.build import bundle, download_kanivin_assets
	from kanivin.gettext.translate import compile_translations
	...
	with filelock("bench_build", is_global=True, timeout=10):
		...
		if not (force or apps or os.environ.get("CI")):
			# skip building kanivin if assets exist remotely
			skip_kanivin = download_kanivin_assets(verbose=verbose)
		else:
			skip_kanivin = False

		# don't minify in developer_mode for faster builds
		development = kanivin.local.conf.developer_mode or kanivin.local.dev_server
...
def watch(apps=None):
	...
	from kanivin.build import watch
...
def destroy_all_sessions(context, reason=None):
	...
	for site in context.sites:
		try:
			kanivin.init(site=site)
			kanivin.connect()
			kanivin.sessions.clear_all_sessions(reason)
			kanivin.db.commit()
		finally:
			kanivin.destroy()
	if not context.sites:
...
def show_config(context, format):
	...
	for site in context.sites:
		...
		configuration = kanivin.get_site_config(sites_path=sites_path, site_path=site)
	...
	if format == "json":
		click.echo(kanivin.as_json(sites_config))
...
def reset_perms(context):
	...
	from kanivin.permissions import reset_perms

	for site in context.sites:
		try:
			kanivin.init(site=site)
			kanivin.connect()
			for d in kanivin.db.sql_list(
				"""select name from `tabDocType`
				where istable=0 and custom=0"""
			):
				kanivin.clear_cache(doctype=d)
				reset_perms(d)
		finally:
			kanivin.destroy()
	if not context.sites:
...
def execute(context, method, args=None, kwargs=None, profile=False):
	...
	for site in context.sites:
		...
		try:
			...
			try:
				ret = kanivin.get_attr(method)(*args, **kwargs)
			except Exception:
...
def export_doc(context, doctype, docname):
	...
	for site in context.sites:
		try:
			kanivin.init(site=site)
			kanivin.connect()
			kanivin.modules.export_doc(doctype, docname)
		finally:
			kanivin.destroy()
	if not context.sites:
		raise SiteNotSpecifiedError
...
def export_json(context, doctype, path, name=None):
	...
	from kanivin.core.doctype.data_import.data_import import export_json
...
def export_csv(context, doctype, path):
	...
	from kanivin.core.doctype.data_import.data_import import export_csv
...
def export_fixtures(context, app=None):
	"Export fixtures"
	from kanivin.utils.fixtures import export_fixtures
...
def data_import(context, file_path, doctype, import_type=None, submit_after_import=False, mute_emails=True):
	...
	from kanivin.core.doctype.data_import.data_import import import_file
...
def bulk_rename(context, doctype, path):
	...
	from kanivin.model.rename_doc import bulk_rename
	from kanivin.utils.csvutils import read_csv_content
...
def postgres(context, extra_args):
	...
	site = get_site(context)
	kanivin.init(site=site)
	kanivin.conf.db_type = "postgres"
	_enter_console(extra_args=extra_args)


def _enter_console(extra_args=None):
	from kanivin.database import get_command
	from kanivin.utils import get_site_path
	...
	bin, args, bin_name = get_command(
		...
		user=kanivin.conf.db_name,
		password=kanivin.conf.db_password,
...
def jupyter(context):
	...
	jupyter_notebooks_path = os.path.abspath(kanivin.get_site_path("jupyter_notebooks"))
	sites_path = os.path.abspath(kanivin.get_site_path(".."))
...
Run the following in your first cell to connect notebook to kanivin
```
import kanivin
kanivin.init(site='{site}', sites_path='{sites_path}')
kanivin.connect()
kanivin.local.lang = kanivin.db.get_default('lang')
kanivin.db.connect()
```
	"""
	)
...
def store_logs(terminal: "InteractiveShellEmbed") -> None:
	...
	kanivin.log_level = 20  # info
	with suppress(Exception):
		logger = kanivin.logger("ipython")
...
def console(context, autoreload=False):
	...
	all_apps = kanivin.get_installed_apps()
	failed_to_import = []
...
def transform_database(context, table, engine, row_format, failfast):
	...
	if table == "all":
		...
		queried_tables = (
			...
			.where(
				...
				& (information_schema.tables.table_schema == kanivin.conf.db_name)
			)
			.run()
		)
	...
	for current, table in enumerate(tables):
		...
		try:
			kanivin.db.sql(f"ALTER TABLE `{table}`{values_to_set}")
...
def run_tests(
	...
	with CodeCoverage(coverage, app):
		import kanivin
		import kanivin.test_runner

		tests = test
		site = get_site(context)

		allow_tests = kanivin.get_conf(site).allow_tests
		...
		kanivin.init(site)  # init kanivin.flags
		kanivin.flags.skip_before_tests = skip_before_tests
		kanivin.flags.skip_test_records = skip_test_records

		ret = kanivin.test_runner.main(
			site,
			app,
			module,
			doctype,
...
@click.option("--app", help="For App", default="kanivin")
...
def run_parallel_tests(
	...
	with CodeCoverage(with_coverage, app):
		site = get_site(context)
		if use_orchestrator:
			from kanivin.parallel_test_runner import ParallelTestWithOrchestrator

			ParallelTestWithOrchestrator(app, site=site)
		else:
			from kanivin.parallel_test_runner import ParallelTestRunner
...
def run_ui_tests(
	...
	site = get_site(context)
	kanivin.init(site)
	app_base_path = kanivin.get_app_source_path(app)
	site_url = kanivin.utils.get_site_url(site)
	admin_password = kanivin.get_conf(site).admin_password
	...
	node_bin = subprocess.getoutput("(cd ../kanivin && yarn bin)")
	...
	if not (
		...
		kanivin.commands.popen(f"(cd ../kanivin && yarn add {packages} --no-lockfile)")
	...
	try:
		kanivin.commands.popen(formatted_command, cwd=app_base_path, raise_err=True)
...
def serve(
	...
	with CodeCoverage(with_coverage, "kanivin"):
		if with_coverage:
		...
		try:
			...
			if args:
				...
				if args.startswith("/api/method"):
					kanivin.local.form_dict.cmd = args.split("?", 1)[0].split("/")[-1]
			elif path:
			...
			kanivin.handler.execute_cmd(kanivin.form_dict.cmd)

			print(kanivin.response)
		finally:
			kanivin.destroy()
	if not context.sites:
		raise SiteNotSpecifiedError
...
def make_app(destination, app_name, no_git=False):
	...
	from kanivin.utils import get_sites
	...
	from kanivin.utils.boilerplate import make_boilerplate
...
def create_patch():
	...
	from kanivin.utils.boilerplate import PatchCreator
...
def get_version(output):
	...
	from kanivin.utils.change_log import get_app_branch
	from kanivin.utils.commands import render_table

	kanivin.init("")
	data = []

	for app in sorted(kanivin.get_all_apps()):
		module = kanivin.get_module(app)
		app_hooks = kanivin.get_module(app + ".hooks")

		app_info = kanivin._dict()

		try:
			app_info.commit = Repo(kanivin.get_app_source_path(app)).head.object.hexsha[:7]
...
def rebuild_global_search(context, static_pages=False):
	...
	from kanivin.utils.global_search import (
