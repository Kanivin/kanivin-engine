...
def pass_context(f):
	...
	def _func(ctx, *args, **kwargs):
		...
		try:
			ret = f(kanivin._dict(ctx.obj), *args, **kwargs)
		except kanivin.exceptions.SiteNotSpecifiedError as e:
			click.secho(str(e), fg="yellow")
			sys.exit(1)
		except kanivin.exceptions.IncorrectSitePath:
...
def get_site(context, raise_err=True):
	try:
		...
		if raise_err:
			raise kanivin.SiteNotSpecifiedError
		return None
...
def get_commands():
	...
	clickable_link = "https://kanivinframework.com/docs"
	all_commands = (
		scheduler_commands
		+ site_commands
		+ translate_commands
