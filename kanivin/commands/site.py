...
from kanivin.utils import CallbackManager
...
def new_site(
	...
	"Create a new site"
	from kanivin.installer import _new_site

	kanivin.init(site=site, new_site=True)

	if site in kanivin.get_all_apps():
		click.secho(
...
def _restore(
	*,
	...
	from kanivin.installer import extract_files
	from kanivin.utils.backups import decrypt_backup, get_or_generate_backup_encryption_key

	err, out = kanivin.utils.execute_in_shell(f"file {sql_file_path}", check_exit_code=True)
	if err:
...
def restore_backup(
	...
	from kanivin.installer import _new_site, is_downgrade, is_partial, validate_database_sql
	...
	if is_partial(sql_file_path):
		click.secho(
			"Partial Backup file detected. You cannot use a partial file to restore a Kanivin site.",
			fg="red",
		)
		click.secho(
	...
	# Check if the backup is of an older version of kanivin and the user hasn't specified force
...
def partial_restore(context, sql_file_path, verbose, encryption_key=None):
	from kanivin.installer import is_partial, partial_restore
	from kanivin.utils.backups import decrypt_backup, get_or_generate_backup_encryption_key
	...
	if "cipher" in out.decode().split(":")[-1].strip():
		...
		with decrypt_backup(sql_file_path, key):
			if not is_partial(sql_file_path):
				click.secho(
					"Full backup file detected. Use `bench restore` to restore a Kanivin Site.",
					fg="red",
				)
				sys.exit(1)
...
def _reinstall(
	...
	try:
		...
		kanivin.connect()
		kanivin.clear_cache()
		installed = kanivin.get_installed_apps()
		kanivin.clear_cache()
	except Exception:
		installed = []
	finally:
...
def install_app(context, apps, force=False):
	...
	from kanivin.installer import install_app as _install_app
	from kanivin.utils.synchronization import filelock
	...
	for site in context.sites:
		...
		with filelock("install_app", timeout=1):
			for app in apps:
				...
				except kanivin.IncompatibleApp as err:
					err_msg = f":\n{err}" if str(err) else ""
					print(f"App {app} is Incompatible with Site {site}{err_msg}")
					exit_code = 1
				except Exception as err:
					err_msg = f": {err!s}\n{kanivin.get_traceback(with_context=True)}"
...
def list_apps(context, format):
	...
	for site in context.sites:
		...
		apps = kanivin.get_single("Installed Applications").installed_applications
		if apps:
			installed_apps_info.extend(format_app(app) for app in apps)
		else:
			installed_apps_info.extend(kanivin.get_installed_apps())
	...
	if format == "json":
		click.echo(kanivin.as_json(summary_dict))
...
def add_db_index(context, doctype, column):
	...
	for site in context.sites:
		...
		kanivin.connect()
		try:
			kanivin.db.add_index(doctype, columns)
			kanivin.db.commit()
		finally:
			kanivin.destroy()
...
def describe_database_table(context, doctype, column):
	...
	from kanivin.core.doctype.recorder.recorder import _fetch_table_stats
...
def add_system_manager(context, email, first_name, last_name, send_welcome_email, password):
	...
	for site in context.sites:
		...
		kanivin.connect()
		try:
			kanivin.utils.user.add_system_manager(email, first_name, last_name, send_welcome_email, password)
			kanivin.db.commit()
		finally:
			kanivin.destroy()
	if not context.sites:
...
def disable_user(context, email):
	...
	site = get_site(context)
	with kanivin.init_site(site):
		kanivin.connect()
		user = kanivin.get_doc("User", email)
		user.enabled = 0
...
def migrate(context, skip_failing=False, skip_search_index=False):
	...
	from kanivin.migrate import SiteMigration
...
def migrate_to():
	...
	from kanivin.integrations.kanivin_providers import migrate_to
...
def run_patch(context, module, force):
	...
	import kanivin.modules.patch_handler

	for site in context.sites:
		kanivin.init(site=site)
		try:
			kanivin.connect()
			kanivin.modules.patch_handler.run_single(module, force=force or context.force)
		finally:
			kanivin.destroy()
	if not context.sites:
		raise SiteNotSpecifiedError
...
def reload_doc(context, module, doctype, docname):
	...
	for site in context.sites:
		try:
			kanivin.init(site=site)
			kanivin.connect()
			kanivin.reload_doc(module, doctype, docname, force=context.force)
			kanivin.db.commit()
		finally:
			kanivin.destroy()
	if not context.sites:
...
def reload_doctype(context, doctype):
	...
	for site in context.sites:
		try:
			kanivin.init(site=site)
			kanivin.connect()
			kanivin.reload_doctype(doctype, force=context.force)
			kanivin.db.commit()
		finally:
			kanivin.destroy()
	if not context.sites:
...
def add_to_hosts(context):
	"Add site to hosts"
	for site in context.sites:
		kanivin.commands.popen(f"echo 127.0.0.1\t{site} | sudo tee -a /etc/hosts")
	if not context.sites:
		raise SiteNotSpecifiedError
	...
	for site in context.sites:
		try:
			...
			if rollback_callback:
				...
				rollback_callback = None
			if verbose:
				print(kanivin.get_traceback(with_context=True))
			exit_code = 1
			continue
		if kanivin.get_system_settings("encrypt_backup") and kanivin.get_site_config().encryption_key:
			click.secho(
...
def remove_from_installed_apps(context, app):
	...
	ensure_app_not_kanivin(app)
	from kanivin.installer import remove_from_installed_apps
...
def uninstall(context, app, dry_run, yes, no_backup, force):
	...
	ensure_app_not_kanivin(app)
	from kanivin.installer import remove_app
	from kanivin.utils.synchronization import filelock
...
def _drop_site(
	...
	from kanivin.database import drop_user_and_database
	from kanivin.utils.backups import scheduled_backup
	...
	drop_user_and_database(kanivin.conf.db_name, db_root_username, db_root_password)

	archived_sites_path = archived_sites_path or os.path.join(
		kanivin.utils.get_bench_path(), "archived", "sites"
	)
...
def move(dest_dir, site):
	...
	kanivin.init(site)
	old_path = kanivin.utils.get_site_path()
...
def set_last_active_for_user(context, user=None):
	...
	with kanivin.init_site(site):
		...
		kanivin.db.set_value("User", user, "last_active", now_datetime())
		kanivin.db.commit()
...
def publish_realtime(context, event, message, room, user, doctype, docname, after_commit):
	...
	from kanivin import publish_realtime
...
def browse(
	...
	if site not in kanivin.utils.get_sites():
	...
	if user:
		...
		if kanivin.conf.developer_mode or user == "Administrator":
			kanivin.utils.set_request(path="/")
			kanivin.local.cookie_manager = CookieManager()
			kanivin.local.login_manager = LoginManager()
			kanivin.local.login_manager.login_as(user, session_end, user_for_audit)
			sid = f"/app?sid={kanivin.session.sid}"
		else:
			click.echo("Please enable developer mode to login as a user")

	url = f"{kanivin.utils.get_site_url(site)}{sid}"
...
def start_ngrok(context, bind_tls, use_default_authtoken):
	...
	ngrok_authtoken = kanivin.conf.ngrok_authtoken
...
def build_search_index(context):
	...
	from kanivin.search.website_search import build_index_for_all_routes
...
def clear_log_table(context, doctype, days, no_backup):
	...
	"""
	from kanivin.core.doctype.log_settings.log_settings import LOG_DOCTYPES
	from kanivin.core.doctype.log_settings.log_settings import clear_log_table as clear_logs
	from kanivin.utils.backups import scheduled_backup
	...
	if doctype not in LOG_DOCTYPES:
		raise kanivin.ValidationError(f"Unsupported logging DocType: {doctype}")
...
def trim_database(context, dry_run, format, no_backup, yes=False):
	...
	for site in context.sites:
		...
		table_name = kanivin.qb.Field("table_name").as_("name")

		database_tables: list[str] = (
			kanivin.qb.from_(information_schema.tables)
			.select(table_name)
			.where(information_schema.tables.table_schema == kanivin.conf.db_name)
			.where(information_schema.tables.table_type == "BASE TABLE")
			.run(pluck=True)
		)
		doctype_tables = kanivin.get_all("DocType", pluck="name")
		...
		if not TABLES_TO_DROP:
			...
			for table in TABLES_TO_DROP:
				...
				kanivin.db.sql_ddl(f"drop table `{table}`")

			ALL_DATA[kanivin.local.site] = TABLES_TO_DROP
		kanivin.destroy()
...
def get_standard_tables():
	...
	sql_file = os.path.join(
		...
		"kanivin",
		"database",
		kanivin.conf.db_type,
		f"framework_{kanivin.conf.db_type}.sql",
	)
...
def trim_tables(context, dry_run, format, no_backup):
	...
	from kanivin.model.meta import trim_tables
	from kanivin.utils.backups import scheduled_backup
	...
	for site in context.sites:
		...
		if not (no_backup or dry_run):
			click.secho(f"Taking backup for {kanivin.local.site}", fg="green")
		...
		try:
			...
			if format == "table" and not dry_run:
				click.secho(f"The following data have been removed from {kanivin.local.site}", fg="green")
...
def handle_data(data: dict, format="json"):
	if format == "json":
		...
		print(json.dumps({kanivin.local.site: data}, indent=1, sort_keys=True))
	else:
		from kanivin.utils.commands import render_table
...
def ensure_app_not_kanivin(app: str) -> None:
	"""
	Ensure that the app name passed is not 'kanivin'

	:param app: Name of the app
	:return: Nothing
	"""
	if app == "kanivin":
		click.secho("You cannot remove or uninstall the app `kanivin`", fg="red")
		sys.exit(1)
