...
def trigger_scheduler_event(context, event):
	...
	for site in context.sites:
		try:
			...
			kanivin.connect()
			try:
				kanivin.get_doc("Scheduled Job Type", {"method": event}).execute()
...
def scheduler(context, state: str, format: str, verbose: bool = False, site: str | None = None):
	...
	import kanivin
	from kanivin.utils.scheduler import is_scheduler_inactive, toggle_scheduler
	...
	with kanivin.init_site(site=site):
		match state:
			case "status":
				kanivin.connect()
...
def doctor(context, site=None):
	...
	from kanivin.utils.doctor import doctor as _doctor
...
def show_pending_jobs(context, site=None):
	...
	from kanivin.utils.doctor import pending_jobs as _pending_jobs
...
def start_worker(queue, quiet=False, rq_username=None, rq_password=None, burst=False, strategy=None):
	...
	from kanivin.utils.background_jobs import start_worker
...
def start_worker_pool(queue, quiet=False, num_workers=2, burst=False):
	...
	from kanivin.utils.background_jobs import start_worker_pool
