...
@click.option("--app", help="Only compile for this app. eg: kanivin")
@click.option(
	"--force",
	is_flag=True,
	default=False,
...
def compile_translations(context, app: str | None = None, locale: str | None = None, force=False):
	from kanivin.gettext.translate import compile_translations as _compile_translations
...
@click.option("--app", help="Only migrate for this app. eg: kanivin")
...
def csv_to_po(context, app: str | None = None, locale: str | None = None):
	from kanivin.gettext.translate import migrate
...
@click.option("--app", help="Only update for this app. eg: kanivin")
...
def update_po_files(context, app: str | None = None, locale: str | None = None):
	from kanivin.gettext.translate import update_po
...
@click.option("--app", help="Only create for this app. eg: kanivin")
@pass_context
def create_po_file(context, locale: str, app: str | None = None):
	"""Create PO file for lang code"""
	from kanivin.gettext.translate import new_po
...
def update_csv_from_po(app: str, locale: str) -> None:
	...
	"""
	from kanivin.gettext.translate import update_csv_from_po

	update_csv_from_po(app, locale)


def connect_to_site(site):
	from kanivin import connect
