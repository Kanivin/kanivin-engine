...
from kanivin.utils import cint, get_datetime, get_url, time_diff_in_seconds
from kanivin.utils.background_jobs import enqueue
from kanivin.utils.password import decrypt, encrypt

PARENT_FOR_DEFAULTS = "__2fa"


def get_default(key):
	return kanivin.db.get_default(key, parent=PARENT_FOR_DEFAULTS)


def set_default(key, value):
	kanivin.db.set_default(key, value, parent=PARENT_FOR_DEFAULTS)


def clear_default(key):
	kanivin.defaults.clear_default(key, parent=PARENT_FOR_DEFAULTS)
...
def toggle_two_factor_auth(state, roles=None):
	...
	for role in roles or []:
		role = kanivin.get_doc("Role", {"role_name": role})
...
def two_factor_is_enabled(user=None):
	...
	enabled = int(kanivin.db.get_single_value("System Settings", "enable_two_factor_auth") or 0)
	if enabled:
		bypass_two_factor_auth = int(
			kanivin.db.get_single_value("System Settings", "bypass_2fa_for_retricted_ip_users") or 0
		)
		if bypass_two_factor_auth and user:
			...
			if restrict_ip_list and kanivin.local.request_ip:
				for ip in restrict_ip_list:
					if kanivin.local.request_ip.startswith(ip):
						enabled = False
						break
...
def get_cached_user_pass():
	...
	user = pwd = None
	tmp_id = kanivin.form_dict.get("tmp_id")
	if tmp_id:
		user = kanivin.safe_decode(kanivin.cache.get(tmp_id + "_usr"))
		pwd = kanivin.safe_decode(kanivin.cache.get(tmp_id + "_pwd"))
	return (user, pwd)


def authenticate_for_2factor(user):
	"""Authenticate two factor for enabled user before login."""
	if kanivin.form_dict.get("otp"):
		return
	otp_secret = get_otpsecret_for_(user)
	token = int(pyotp.TOTP(otp_secret).now())
	tmp_id = kanivin.generate_hash(length=8)
	cache_2fa_data(user, token, otp_secret, tmp_id)
	verification_obj = get_verification_obj(user, token, otp_secret)
	# Save data in local
	kanivin.local.response["verification"] = verification_obj
	kanivin.local.response["tmp_id"] = tmp_id


def cache_2fa_data(user, token, otp_secret, tmp_id):
	"""Cache and set expiry for data."""
	pwd = kanivin.form_dict.get("pwd")
	...
	if verification_method in ["SMS", "Email"]:
		expiry_time = kanivin.flags.token_expiry or 300
		kanivin.cache.set(tmp_id + "_token", token)
		kanivin.cache.expire(tmp_id + "_token", expiry_time)
	else:
		expiry_time = kanivin.flags.otp_expiry or 180
	for k, v in {"_usr": user, "_pwd": pwd, "_otp_secret": otp_secret}.items():
		kanivin.cache.set(f"{tmp_id}{k}", v)
		kanivin.cache.expire(f"{tmp_id}{k}", expiry_time)
...
def two_factor_is_enabled_for_(user):
	...
	role_doctype = kanivin.qb.DocType("Role")
	no_of_users = kanivin.db.count(
		role_doctype,
...
def get_verification_method():
	return kanivin.db.get_single_value("System Settings", "two_factor_method")


def confirm_otp_token(login_manager, otp=None, tmp_id=None):
	"""Confirm otp matches."""
	from kanivin.auth import get_login_attempt_tracker

	if not otp:
		otp = kanivin.form_dict.get("otp")
	if not otp:
	...
	if not tmp_id:
		tmp_id = kanivin.form_dict.get("tmp_id")
	hotp_token = kanivin.cache.get(tmp_id + "_token")
	otp_secret = kanivin.cache.get(tmp_id + "_otp_secret")
	if not otp_secret:
	...
	if hotp_token:
		if hotp.verify(otp, int(hotp_token)):
			kanivin.cache.delete(tmp_id + "_token")
...
def get_verification_obj(user, token, otp_secret):
	otp_issuer = kanivin.db.get_single_value("System Settings", "otp_issuer_name")
	verification_method = get_verification_method()
...
def process_2fa_for_sms(user, token, otp_secret):
	...
	phone = kanivin.db.get_value("User", user, ["phone", "mobile_no"], as_dict=1)
...
def get_email_subject_for_2fa(kwargs_dict):
	...
	subject_template = _("Login Verification Code from {}").format(
		kanivin.db.get_single_value("System Settings", "otp_issuer_name")
	)
	return kanivin.render_template(subject_template, kwargs_dict)
...
def get_email_body_for_2fa(kwargs_dict):
	...
	"""
	return kanivin.render_template(body_template, kwargs_dict)
...
def get_link_for_qrcode(user, totp_uri):
	...
	key = kanivin.generate_hash(length=20)
	key_user = f"{key}_user"
	key_uri = f"{key}_uri"
	lifespan = int(kanivin.db.get_single_value("System Settings", "lifespan_qrcode_image")) or 240
	kanivin.cache.set_value(key_uri, totp_uri, expires_in_sec=lifespan)
	kanivin.cache.set_value(key_user, user, expires_in_sec=lifespan)
...
def send_token_via_sms(otpsecret, token=None, phone_no=None):
	...
	try:
		from kanivin.core.doctype.sms_settings.sms_settings import send_request
	except Exception:
		return False
	...
	ss = kanivin.get_doc("SMS Settings", "SMS Settings")
...
def send_token_via_email(user, token, otp_secret, otp_issuer, subject=None, message=None):
	...
	user_email = kanivin.db.get_value("User", user, "email")
	if not user_email:
		return False
	hotp = pyotp.HOTP(otp_secret)
...
def create_barcode_folder():
	...
	folder = kanivin.db.exists("File", {"file_name": folder_name})
	if folder:
		return folder
	folder = kanivin.get_doc({"doctype": "File", "file_name": folder_name, "is_folder": 1, "folder": "Home"})
...
def delete_qrimage(user, check_expiry=False):
	...
	for barcode in user_barcodes:
		if check_expiry and not should_remove_barcode_image(barcode):
			continue
		barcode = kanivin.get_doc("File", barcode.name)
		kanivin.delete_doc("File", barcode.name, ignore_permissions=True)


def delete_all_barcodes_for_users():
	"""Task to delete all barcodes for user."""

	users = kanivin.get_all("User", {"enabled": 1})
	for user in users:
...
def should_remove_barcode_image(barcode):
	...
	if isinstance(barcode, str):
		barcode = kanivin.get_doc("File", barcode)
	lifespan = kanivin.db.get_single_value("System Settings", "lifespan_qrcode_image") or 240
...
def disable():
	kanivin.db.set_single_value("System Settings", "enable_two_factor_auth", 0)


@kanivin.whitelist()
def reset_otp_secret(user: str):
	if kanivin.session.user != user:
		kanivin.only_for("System Manager", message=True)

	settings = kanivin.get_cached_doc("System Settings")
	...
	otp_issuer = settings.otp_issuer_name or "Kanivin Framework"
	user_email = kanivin.get_cached_value("User", user, "email")
	...
	kanivin.msgprint(_("OTP Secret has been reset. Re-registration will be required on next login."))
