...
from kanivin.utils.data import cstr, get_system_timezone, now_datetime
...
class OAuthWebRequestValidator(RequestValidator):
	...
	def validate_client_id(self, client_id, request, *args, **kwargs):
		...
		cli_id = kanivin.db.get_value("OAuth Client", {"name": client_id})
		if cli_id:
			client = kanivin.get_doc("OAuth Client", client_id)
	...
	def validate_redirect_uri(self, client_id, redirect_uri, request, *args, **kwargs):
		...
		redirect_uris = (
			cstr(kanivin.db.get_value("OAuth Client", client_id, "redirect_uris"))
			.strip()
	...
	def get_default_redirect_uri(self, client_id, request, *args, **kwargs):
		...
		return kanivin.db.get_value("OAuth Client", client_id, "default_redirect_uri")
	...
	def save_authorization_code(self, client_id, code, request, *args, **kwargs):
		...
		oac = kanivin.new_doc("OAuth Authorization Code")
	...
	def authenticate_client(self, request, *args, **kwargs):
		...
		if request.client_id:
			oc = kanivin.get_doc("OAuth Client", request.client_id)
		else:
			# Extract token, instantiate OAuth Bearer Token and use clientid from there.
			if "refresh_token" in kanivin.form_dict:
				oc = kanivin.get_doc(
					"OAuth Client",
					kanivin.db.get_value(
						"OAuth Bearer Token",
						{"refresh_token": kanivin.form_dict["refresh_token"]},
						"client",
					),
				)
			elif "token" in kanivin.form_dict:
				oc = kanivin.get_doc(
					"OAuth Client",
					kanivin.db.get_value("OAuth Bearer Token", kanivin.form_dict["token"], "client"),
				)
			else:
				oc = kanivin.get_doc(
					"OAuth Client",
					kanivin.db.get_value(
						"OAuth Bearer Token",
						kanivin.get_request_header("Authorization").split(" ")[1],
						"client",
					),
				)
		try:
		...
		return kanivin.session.user == user_id

	def authenticate_client_id(self, client_id, request, *args, **kwargs):
		cli_id = kanivin.db.get_value("OAuth Client", client_id, "name")
		if not cli_id:
			# Don't allow public (non-authenticated) clients
			return False
		else:
			request["client"] = kanivin.get_doc("OAuth Client", cli_id)
			return True
	...
	def validate_code(self, client_id, code, client, request, *args, **kwargs):
		...
		validcodes = kanivin.get_all(
			"OAuth Authorization Code",
			filters={"client": client_id, "validity": "Valid"},
		)

		if code in [vcode["name"] for vcode in validcodes]:
			request.scopes = kanivin.db.get_value("OAuth Authorization Code", code, "scopes").split(
				get_url_delimiter()
			)
			request.user = kanivin.db.get_value("OAuth Authorization Code", code, "user")
			code_challenge_method = kanivin.db.get_value(
				"OAuth Authorization Code", code, "code_challenge_method"
			)
			code_challenge = kanivin.db.get_value("OAuth Authorization Code", code, "code_challenge")

			if code_challenge and not request.code_verifier:
				if kanivin.db.exists("OAuth Authorization Code", code):
					kanivin.delete_doc("OAuth Authorization Code", code, ignore_permissions=True)
					kanivin.db.commit()
				return False
	...
	def confirm_redirect_uri(self, client_id, code, redirect_uri, client, *args, **kwargs):
		saved_redirect_uri = kanivin.db.get_value("OAuth Client", client_id, "default_redirect_uri")

		redirect_uris = kanivin.db.get_value("OAuth Client", client_id, "redirect_uris")
	...
	def save_bearer_token(self, token, request, *args, **kwargs):
		...
		otoken = kanivin.new_doc("OAuth Bearer Token")
		otoken.client = request.client["name"]
		try:
			otoken.user = (
				request.user
				if request.user
				else kanivin.db.get_value(
					"OAuth Bearer Token",
			...
			otoken.user = kanivin.session.user
		...
		return kanivin.db.get_value("OAuth Client", request.client["name"], "default_redirect_uri")

	def invalidate_authorization_code(self, client_id, code, request, *args, **kwargs):
		# Authorization codes are use once, invalidate it when a Bearer token
		# has been acquired.

		kanivin.db.set_value("OAuth Authorization Code", code, "validity", "Invalid")
		kanivin.db.commit()
	...
	def validate_bearer_token(self, token, scopes, request):
		...
		otoken = kanivin.get_doc("OAuth Bearer Token", token)
		is_token_valid = (now_datetime() < otoken.expiration_time) and otoken.status != "Revoked"
		client_scopes = kanivin.db.get_value("OAuth Client", otoken.client, "scopes").split(
			get_url_delimiter()
		)
	...
	def get_original_scopes(self, refresh_token, request, *args, **kwargs):
		...
		# request.
		obearer_token = kanivin.get_doc("OAuth Bearer Token", {"refresh_token": refresh_token})
	...
	def revoke_token(self, token, token_type_hint, request, *args, **kwargs):
		...
		if token_type_hint == "access_token":
			kanivin.db.set_value("OAuth Bearer Token", token, "status", "Revoked")
		elif token_type_hint == "refresh_token":
			kanivin.db.set_value("OAuth Bearer Token", {"refresh_token": token}, "status", "Revoked")
		else:
			kanivin.db.set_value("OAuth Bearer Token", token, "status", "Revoked")
		kanivin.db.commit()
	...
	def validate_refresh_token(self, refresh_token, client, request, *args, **kwargs):
		...
		otoken = kanivin.get_doc("OAuth Bearer Token", {"refresh_token": refresh_token, "status": "Active"})
	...
	def finalize_id_token(self, id_token, token, token_handler, request):
		# Check whether kanivin server URL is set
		id_token_header = {"typ": "jwt", "alg": "HS256"}

		user = kanivin.get_doc("User", request.user)
		...
		return kanivin.safe_decode(id_token_encoded)

	def get_authorization_code_nonce(self, client_id, code, redirect_uri, request):
		if kanivin.get_value("OAuth Authorization Code", code, "validity") == "Valid":
			return kanivin.get_value("OAuth Authorization Code", code, "nonce")

		return None

	def get_authorization_code_scopes(self, client_id, code, redirect_uri, request):
		scope = kanivin.get_value("OAuth Client", client_id, "scopes")
		if not scope:
			scope = []
		else:
	...
	def validate_id_token(self, token, scopes, request):
		try:
			id_token = kanivin.get_doc("OAuth Bearer Token", token)
	...
	def validate_jwt_bearer_token(self, token, scopes, request):
		try:
			jwt = kanivin.get_doc("OAuth Bearer Token", token)
	...
	def validate_silent_login(self, request):
		...
		- OpenIDConnectHybrid
		"""
		if kanivin.session.user == "Guest" or request.prompt.lower() == "login":
			return False
		else:
			return True
	...
	def validate_user_match(self, id_token_hint, scopes, claims, request):
		...
		if id_token_hint:
			try:
				...
				client_id, client_secret = kanivin.get_value(
					"OAuth Client",
					payload.get("aud"),
				...
				if payload.get("sub") and client_id and client_secret:
					user = kanivin.db.get_value(
						"User Social Login",
						{"userid": payload.get("sub"), "provider": "kanivin"},
						"parent",
					)
					...
					if verified_payload:
						return user.name == kanivin.session.user

			except Exception:
				return False

		elif kanivin.session.user != "Guest":
			return True
...
def delete_oauth2_data():
	kanivin.db.delete("OAuth Authorization Code", {"validity": "Invalid"})
	kanivin.db.delete("OAuth Bearer Token", {"status": "Revoked"})


def get_client_scopes(client_id):
	scopes_string = kanivin.db.get_value("OAuth Client", client_id, "scopes")
...
def get_userinfo(user):
	...
	if user.user_image:
		if kanivin.utils.validate_url(user.user_image, valid_schemes=valid_url_schemes):
			picture = user.user_image
		else:
			picture = urljoin(kanivin_server_url, user.user_image)

	return kanivin._dict(
		{
			"sub": kanivin.db.get_value(
				"User Social Login",
				{"parent": user.name, "provider": "kanivin"},
				"userid",
			),
			...
			"email": user.email,
			"picture": picture,
			"roles": kanivin.get_roles(user.name),
			"iss": kanivin_server_url,
		}
	)
...
def generate_json_error_response(e):
	...
	kanivin.local.response["http_status_code"] = getattr(e, "status_code", 500)
	return


def get_server_url():
	request_url = urlparse(kanivin.request.url)
	request_url = f"{request_url.scheme}://{request_url.netloc}"
	return kanivin.get_value("Social Login Key", "kanivin", "base_url") or request_url
