...
from kanivin.model import data_fieldtypes
from kanivin.permissions import filter_allowed_docs_for_doctype
from kanivin.utils import cstr, now_datetime, nowdate, nowtime


def get_new_doc(doctype, parent_doc=None, parentfield=None, as_dict=False):
	if doctype not in kanivin.local.new_doc_templates:
		# cache a copy of new doc as it is called
		# frequently for inserts
		kanivin.local.new_doc_templates[doctype] = make_new_doc(doctype)

	doc = copy.deepcopy(kanivin.local.new_doc_templates[doctype])
	...
	if as_dict:
		...
		return kanivin.get_doc(doc)


def make_new_doc(doctype):
	doc = kanivin.get_doc({"doctype": doctype, "__islocal": 1, "owner": kanivin.session.user, "docstatus": 0})
	...
	if not kanivin.model.meta.is_single(doctype):
		doc["__unsaved"] = 1
...
def set_user_and_static_default_values(doc):
	...
	defaults = kanivin.defaults.get_defaults()
	...
	for df in doc.meta.get("fields"):
		if df.fieldtype in data_fieldtypes:
			...
			if user_default_value is not None:
				...
				if not df.fieldtype == "Link" or kanivin.db.exists(df.options, user_default_value):
...
def set_dynamic_default_values(doc, parent_doc, parentfield):
	...
	for df in kanivin.get_meta(doc["doctype"]).get("fields"):
		if df.get("default"):
...
def get_default_based_on_another_field(df, user_permissions, parent_doc):
	...
	from kanivin.permissions import get_allowed_docs_for_doctype

	ref_doctype = df.default[1:]
	ref_fieldname = ref_doctype.lower().replace(" ", "_")
	reference_name = parent_doc.get(ref_fieldname) if parent_doc else kanivin.db.get_default(ref_fieldname)
	default_value = kanivin.db.get_value(ref_doctype, reference_name, df.fieldname)
