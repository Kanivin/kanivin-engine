...
import kanivin
from kanivin import _, bold
from kanivin.model.document import Document
from kanivin.model.dynamic_links import get_dynamic_link_map
from kanivin.model.naming import validate_name
from kanivin.model.utils.user_settings import sync_user_settings, update_user_settings_data
from kanivin.query_builder import Field
from kanivin.utils.data import sbool
from kanivin.utils.password import rename_password
from kanivin.utils.scheduler import is_scheduler_inactive
...
def update_document_title(
	...
	**kwargs,
	...
	# TODO: omit this after runtime type checking (ref: https://github.com/kanivin/kanivin/pull/14927)
	for obj in [docname, updated_title, updated_name]:
		if not isinstance(obj, str | NoneType):
			kanivin.throw(f"{obj=} must be of type str or None")
	...
	if title_updated:
		if action_enqueued and name_updated:
			...
			try:
				...
				doc.save()
				kanivin.msgprint(_("Saved"), alert=True, indicator="green")
			except Exception as e:
				if kanivin.db.is_duplicate_entry(e):
					kanivin.throw(
						_("{0} {1} already exists").format(doctype, kanivin.bold(docname)),
						title=_("Duplicate Name"),
						exc=kanivin.DuplicateEntryError,
					)
				raise
...
def rename_doc(
	...
	if not (new_usage_style or old_usage_style):
		raise TypeError(
			"{doctype, old, new} or {doc, new} are required arguments for kanivin.model.rename_doc"
		)
	...
	if validate:
		old_doc = doc or kanivin.get_doc(doctype, old)
	...
	# call after_rename
	new_doc = kanivin.get_doc(doctype, new)
	...
	if merge:
		new_doc.add_comment("Edit", _("merged {0} into {1}").format(kanivin.bold(old), kanivin.bold(new)))
	else:
		new_doc.add_comment("Edit", _("renamed from {0} to {1}").format(kanivin.bold(old), kanivin.bold(new)))

	if merge:
		kanivin.delete_doc(doctype, old)

	new_doc.clear_cache()
	kanivin.clear_cache()
	if rebuild_search:
		kanivin.enqueue("kanivin.utils.global_search.rebuild_for_doctype", doctype=doctype)
...
def update_assignments(old: str, new: str, doctype: str) -> None:
	old_assignments = kanivin.parse_json(kanivin.db.get_value(doctype, old, "_assign")) or []
	new_assignments = kanivin.parse_json(kanivin.db.get_value(doctype, new, "_assign")) or []
	...
	for user in common_assignments:
		...
		for todo in todos:
			kanivin.delete_doc("ToDo", todo.name)

	unique_assignments = list(set(old_assignments + new_assignments))
	kanivin.db.set_value(doctype, new, "_assign", kanivin.as_json(unique_assignments, indent=0))
...
def update_user_settings(old: str, new: str, link_fields: list[dict]) -> None:
	...
	UserSettings = kanivin.qb.Table("__UserSettings")

	user_settings_details = (
		kanivin.qb.from_(UserSettings)
...
def update_customizations(old: str, new: str) -> None:
	kanivin.db.set_value("Custom DocPerm", {"parent": old}, "parent", new, update_modified=False)


def update_attachments(doctype: str, old: str, new: str) -> None:
	if doctype != "DocType":
		File = kanivin.qb.DocType("File")

		kanivin.qb.update(File).set(File.attached_to_name, new).where(
			(File.attached_to_name == old) & (File.attached_to_doctype == doctype)
		).run()


def rename_versions(doctype: str, old: str, new: str) -> None:
	Version = kanivin.qb.DocType("Version")

	kanivin.qb.update(Version).set(Version.docname, new).where(
		(Version.docname == old) & (Version.ref_doctype == doctype)
	).run()


def rename_eps_records(doctype: str, old: str, new: str) -> None:
	EPL = kanivin.qb.DocType("Energy Point Log")

	kanivin.qb.update(EPL).set(EPL.reference_name, new).where(
...
def rename_parent_and_child(doctype: str, old: str, new: str, meta: "Meta") -> None:
	kanivin.qb.update(doctype).set("name", new).where(Field("name") == old).run()
...
def update_autoname_field(doctype: str, new: str, meta: "Meta") -> None:
	...
	if meta.get("autoname"):
		...
		if field and field[0] == "field":
			kanivin.qb.update(doctype).set(field[1], new).where(Field("name") == new).run()
...
def validate_rename(
	...
	if save_point:
		_SAVE_POINT = f"validate_rename_{kanivin.generate_hash(length=8)}"
		kanivin.db.savepoint(_SAVE_POINT)

	exists = kanivin.qb.from_(doctype).where(Field("name") == new).for_update().select("name").run(pluck=True)
	exists = exists[0] if exists else None

	if not kanivin.db.exists(doctype, old):
		kanivin.throw(_("Can't rename {0} to {1} because {0} doesn't exist.").format(old, new))

	if old == new:
		kanivin.throw(_("No changes made because old and new name are the same.").format(old, new))
	...
	if merge and not exists:
		kanivin.throw(_("{0} {1} does not exist, select a new target to merge").format(doctype, new))

	if not merge and exists and not ignore_if_exists:
		kanivin.throw(_("Another {0} with name {1} exists, select another name").format(doctype, new))

	kwargs = {"doctype": doctype, "ptype": "write", "raise_exception": False}
	if old_doc:
		kwargs["doc"] = old_doc

	if not (ignore_permissions or kanivin.permissions.has_permission(**kwargs)):
		kanivin.throw(_("You need write permission on {0} {1} to rename").format(doctype, old))

	if merge:
		kwargs["doc"] = kanivin.get_doc(doctype, new)
		if not (ignore_permissions or kanivin.permissions.has_permission(**kwargs)):
			kanivin.throw(_("You need write permission on {0} {1} to merge").format(doctype, new))

	if not (force or ignore_permissions) and not meta.allow_rename:
		kanivin.throw(_("{0} not allowed to be renamed").format(_(doctype)))

	# validate naming like it's done in doc.py
	new = validate_name(doctype, new)

	if save_point:
		kanivin.db.rollback(save_point=_SAVE_POINT)
...
def rename_doctype(doctype: str, old: str, new: str) -> None:
	...
	fields_with_options = ("Link", *kanivin.model.table_fields)
...
def update_link_field_values(link_fields: list[dict], old: str, new: str, doctype: str) -> None:
	for field in link_fields:
		if field["issingle"]:
			try:
				single_doc = kanivin.get_doc(field["parent"])
			...
			kanivin.db.set_value(parent, {docfield: old}, docfield, new, update_modified=False)
...
def get_link_fields(doctype: str) -> list[dict]:
	...
	if not kanivin.flags.link_fields:
		kanivin.flags.link_fields = {}

	if doctype not in kanivin.flags.link_fields:
		dt = kanivin.qb.DocType("DocType")
		df = kanivin.qb.DocType("DocField")
		cf = kanivin.qb.DocType("Custom Field")
		ps = kanivin.qb.DocType("Property Setter")
		...
		if kanivin.db.has_column("DocField", "is_virtual"):
			standard_fields_query = standard_fields_query.where(df.is_virtual == 0)

		virtual_doctypes = []
		if kanivin.db.has_column("DocType", "is_virtual"):
			virtual_doctypes = kanivin.get_all("DocType", {"is_virtual": 1}, pluck="name")
			standard_fields_query = standard_fields_query.where(dt.is_virtual == 0)

		standard_fields = standard_fields_query.run(as_dict=True)

		cf_issingle = kanivin.qb.from_(dt).select(dt.issingle).where(dt.name == cf.dt).as_("issingle")
		custom_fields = (
			kanivin.qb.from_(cf)
		...
		ps_issingle = kanivin.qb.from_(dt).select(dt.issingle).where(dt.name == ps.doc_type).as_("issingle")
		...
		kanivin.flags.link_fields[doctype] = standard_fields + custom_fields + property_setter_fields

	return kanivin.flags.link_fields[doctype]


def update_options_for_fieldtype(fieldtype: str, old: str, new: str) -> None:
	CustomField = kanivin.qb.DocType("Custom Field")
	PropertySetter = kanivin.qb.DocType("Property Setter")

	if kanivin.conf.developer_mode:
		for name in kanivin.get_all("DocField", filters={"options": old}, pluck="parent"):
			if name in (old, new):
				continue

			doctype = kanivin.get_doc("DocType", name)
			save = False
	...
	DocField = kanivin.qb.DocType("DocField")
	kanivin.qb.update(DocField).set(DocField.options, new).where(
		(DocField.fieldtype == fieldtype) & (DocField.options == old)
	).run()

	kanivin.qb.update(CustomField).set(CustomField.options, new).where(
		(CustomField.fieldtype == fieldtype) & (CustomField.options == old)
	).run()

	kanivin.qb.update(PropertySetter).set(PropertySetter.value, new).where(
...
def get_select_fields(old: str, new: str) -> list[dict]:
	...
	# get link fields from tabDocField
	st_issingle = kanivin.qb.from_(dt).select(dt.issingle).where(dt.name == df.parent).as_("issingle")
	standard_fields = (
		kanivin.qb.from_(df)
...
def update_select_field_values(old: str, new: str):
	from kanivin.query_builder.functions import Replace

	DocField = kanivin.qb.DocType("DocField")
	CustomField = kanivin.qb.DocType("Custom Field")
	PropertySetter = kanivin.qb.DocType("Property Setter")

	kanivin.qb.update(DocField).set(DocField.options, Replace(DocField.options, old, new)).where(
		(DocField.fieldtype == "Select")
		& (DocField.parent != new)
		& (DocField.options.like(f"%\n{old}%") | DocField.options.like(f"%{old}\n%"))
	).run()

	kanivin.qb.update(CustomField).set(CustomField.options, Replace(CustomField.options, old, new)).where(
		(CustomField.fieldtype == "Select")
		& (CustomField.dt != new)
		& (CustomField.options.like(f"%\n{old}%") | CustomField.options.like(f"%{old}\n%"))
	).run()

	kanivin.qb.update(PropertySetter).set(PropertySetter.value, Replace(PropertySetter.value, old, new)).where(
...
def update_parenttype_values(old: str, new: str):
	child_doctypes = kanivin.get_all(
		"DocField",
		fields=["options", "fieldname"],
		filters={"parent": new, "fieldtype": ["in", kanivin.model.table_fields]},
	)

	custom_child_doctypes = kanivin.get_all(
		"Custom Field",
		fields=["options", "fieldname"],
		filters={"dt": new, "fieldtype": ["in", kanivin.model.table_fields]},
	)

	child_doctypes += custom_child_doctypes
	fields = [d["fieldname"] for d in child_doctypes]

	property_setter_child_doctypes = kanivin.get_all(
		"Property Setter",
	...
	for doctype in child_doctypes:
		...
		kanivin.qb.update(table).set(table.parenttype, new).where(table.parenttype == old).run()


def rename_dynamic_links(doctype: str, old: str, new: str):
	Singles = kanivin.qb.DocType("Singles")
	for df in get_dynamic_link_map().get(doctype, []):
		# dynamic link in single, just one value to check
		meta = kanivin.get_meta(df.parent)
		if meta.is_virtual:
			continue
		if meta.issingle:
			refdoc = kanivin.db.get_singles_dict(df.parent)
			if refdoc.get(df.options) == doctype and refdoc.get(df.fieldname) == old:
				kanivin.qb.update(Singles).set(Singles.value, new).where(
			...
			kanivin.qb.update(parent).set(df.fieldname, new).where(
...
def bulk_rename(doctype: str, rows: list[list] | None = None, via_console: bool = False) -> list[str] | None:
	...
	if not rows:
		kanivin.throw(_("Please select a valid csv file with data"))

	if not via_console:
		max_rows = 500
		if len(rows) > max_rows:
			kanivin.throw(_("Maximum {0} rows allowed").format(max_rows))
