...
from kanivin import _, get_module_path
from kanivin.desk.doctype.tag.tag import delete_tags_for_document
from kanivin.model.docstatus import DocStatus
from kanivin.model.dynamic_links import get_dynamic_link_map
from kanivin.model.naming import revert_series_if_last
from kanivin.model.utils import is_virtual_doctype
from kanivin.utils.file_manager import remove_all
from kanivin.utils.global_search import delete_for_document
from kanivin.utils.password import delete_all_passwords_for
...
def delete_doc(
	...
	# get from form
	if not doctype:
		doctype = kanivin.form_dict.get("dt")
		name = kanivin.form_dict.get("dn")
	...
	for name in names or []:
		if is_virtual:
			kanivin.get_doc(doctype, name).delete()
			continue
		...
		if doctype == "DocType":
			if for_reload:
				...
				doc = kanivin.get_doc(doctype, name)
				if not (doc.custom or kanivin.conf.developer_mode or kanivin.flags.in_patch or force):
					kanivin.throw(_("Standard DocType can not be deleted."))

				update_flags(doc, flags, ignore_permissions)
				check_permission_and_not_submitted(doc)
				# delete custom table fields using this doctype.
				kanivin.db.delete(
					"Custom Field", {"options": name, "fieldtype": ("in", kanivin.model.table_fields)}
				)
				kanivin.db.delete("__global_search", {"doctype": name})

			delete_from_table(doctype, name, ignore_doctypes, None)

			if (
				kanivin.conf.developer_mode
				and not doc.custom
				and not (
					for_reload
			...
			try:
				kanivin.db.get_value(doctype, name, for_update=True, wait=False)
			except kanivin.QueryTimeoutError:
				kanivin.throw(
					_(
						"This document can not be deleted right now as it's being modified by another user. Please try again after some time."
					),
					exc=kanivin.QueryTimeoutError,
				)
			doc = kanivin.get_doc(doctype, name)
			...
			if not for_reload:
				...
				if not force:
					...
					except kanivin.LinkExistsError as e:
			...
			if not for_reload:
				...
				kanivin.enqueue(
					"kanivin.model.delete_doc.delete_dynamic_links",
					doctype=doc.doctype,
					name=doc.name,
					now=kanivin.flags.in_test,
...
def add_to_deleted_document(doc):
	...
	if doc.doctype != "Deleted Document" and kanivin.flags.in_install != "kanivin":
		kanivin.get_doc(
			dict(
...
def delete_from_table(doctype: str, name: str, ignore_doctypes: list[str], doc):
	if doctype != "DocType" and doctype == name:
		kanivin.db.delete("Singles", {"doctype": name})
	else:
		kanivin.db.delete(doctype, {"name": name})
	if doc:
		child_doctypes = [
			d.options for d in doc.meta.get_table_fields() if kanivin.get_meta(d.options).is_virtual == 0
		]
		...
		child_doctypes = kanivin.get_all(
			"DocField",
			fields="options",
			filters={"fieldtype": ["in", kanivin.model.table_fields], "parent": doctype},
			pluck="options",
		)

	child_doctypes_to_delete = set(child_doctypes) - set(ignore_doctypes)
	for child_doctype in child_doctypes_to_delete:
		kanivin.db.delete(child_doctype, {"parenttype": doctype, "parent": name})
...
def check_permission_and_not_submitted(doc):
	...
	if (
		...
		and kanivin.session.user != "Administrator"
		...
		kanivin.msgprint(
			...
			raise_exception=kanivin.PermissionError,
		)
...
def check_if_doc_is_linked(doc, method="Delete"):
	...
	"""
	from kanivin.model.rename_doc import get_link_fields
	...
	if method == "Delete":
		ignored_doctypes.update(kanivin.get_hooks("ignore_links_on_delete"))
	...
	for lf in link_fields:
		...
		if issingle:
			if kanivin.db.get_single_value(link_dt, link_field) == doc.name:
		...
		for item in kanivin.db.get_values(link_dt, {link_field: doc.name}, fields, as_dict=True):
...
def check_if_doc_is_dynamically_linked(doc, method="Delete"):
	"""Raise `kanivin.LinkExistsError` if the document is dynamically linked"""
	for df in get_dynamic_link_map().get(doc.doctype, []):
		ignore_linked_doctypes = doc.get("ignore_linked_doctypes") or []

		if df.parent in kanivin.get_hooks("ignore_links_on_delete") or (
		...
		if meta.issingle:
			...
			for refdoc in kanivin.db.sql(
				...
				if (method == "Delete" and not DocStatus(refdoc.docstatus).is_cancelled()) or (
					...
					if reference_doctype in kanivin.get_hooks("ignore_links_on_delete") or (
...
def clear_timeline_references(link_doctype, link_name):
	kanivin.db.delete("Communication Link", {"link_doctype": link_doctype, "link_name": link_name})
...
def delete_controllers(doctype, module):
	...
	dir_path = os.path.join(module_path, "doctype", kanivin.scrub(doctype))
