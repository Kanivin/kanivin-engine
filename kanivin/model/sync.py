...
from kanivin.modules.patch_handler import _patch_mode
...
def sync_for(app_name, force=0, reset_permissions=False):
	...
	if app_name == "kanivin":
		...
		FRAPPE_PATH = kanivin.get_app_path("kanivin")
	...
	for module_name in kanivin.local.app_modules.get(app_name) or []:
		folder = os.path.dirname(kanivin.get_module(app_name + "." + module_name).__file__)
...
def remove_orphan_doctypes():
	...
	doctype_names = kanivin.get_all("DocType", {"custom": 0}, pluck="name")
	orphan_doctypes = []
	...
	for i, name in enumerate(orphan_doctypes):
		kanivin.delete_doc("DocType", name, force=True, ignore_missing=True)
