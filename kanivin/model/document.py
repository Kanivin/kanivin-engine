...
import kanivin
from kanivin import _, is_whitelisted, msgprint
from kanivin.core.doctype.file.utils import relink_mismatched_files
from kanivin.core.doctype.server_script.server_script_utils import run_server_script_for_doc_event
from kanivin.desk.form.document_follow import follow_document
from kanivin.integrations.doctype.webhook import run_webhooks
from kanivin.model import optional_fields, table_fields
from kanivin.model.base_document import BaseDocument, get_controller
from kanivin.model.docstatus import DocStatus
from kanivin.model.naming import set_new_name, validate_name
from kanivin.model.utils import is_virtual_doctype
from kanivin.model.workflow import set_workflow_state_on_action, validate_workflow
from kanivin.types import DF
from kanivin.utils import compare, cstr, date_diff, file_lock, flt, get_datetime_str, now
from kanivin.utils.data import get_absolute_url, get_datetime, get_timedelta, getdate
from kanivin.utils.global_search import update_global_search
...
def get_doc(*args, **kwargs):
	"""returns a kanivin.model.Document object.
...
class Document(BaseDocument):
	...
	def load_from_db(self):
		...
		if not getattr(self, "_metaclass", False) and self.meta.issingle:
			single_doc = kanivin.db.get_singles_dict(self.doctype, for_update=self.flags.for_update)
			if not single_doc:
				single_doc = kanivin.new_doc(self.doctype, as_dict=True)
			...
			if not d:
				kanivin.throw(
					...
					kanivin.DoesNotExistError(doctype=self.doctype),
				)
		...
		for df in self._get_table_fields():
			...
			# During kanivin is installed, the property "is_virtual" is not available in tabDocType, so
			# we need to filter those cases for the access to kanivin.db.get_value() as it would crash otherwise.
	...
	def check_permission(self, permtype="read", permlevel=None):
		"""Raise `kanivin.PermissionError` if not permitted"""
		if not self.has_permission(permtype):
			self._handle_permission_failure(permtype)

	def has_permission(self, permtype="read", *, debug=False, user=None) -> bool:
		"""
		Call `kanivin.permissions.has_permission` if `ignore_permissions` flag isn't truthy
		...
		return kanivin.permissions.has_permission(self.doctype, permtype, self, debug=debug, user=user)
	...
	def raise_no_permission_to(self, perm_type):
		...
		kanivin.flags.error_message = _(
			"You need the '{0}' permission on {1} {2} to perform this action."
		).format(
			_(perm_type),
			kanivin.bold(_(self.doctype)),
			self.name or "",
		)
		raise kanivin.PermissionError
	...
	def insert(
		...
		if not (kanivin.flags.in_migrate or kanivin.local.flags.in_install or kanivin.flags.in_setup_wizard):
			if kanivin.get_cached_value("User", kanivin.session.user, "follow_created_documents"):
				follow_document(self.doctype, self.name, kanivin.session.user)
		return self
	...
	def check_if_locked(self):
		...
		if file_lock.lock_age(self.get_signature()) > DOCUMENT_LOCK_SOFT_EXPIRY:
			primary_action = {
				"label": "Force Unlock",
				"server_action": "kanivin.model.document.unlock_document",
		...
		kanivin.throw(
			...
			exc=kanivin.DocumentLockedError,
		)
	...
	def _save(self, ignore_permissions=None, ignore_version=None) -> "Document":
		...
		self.flags.ignore_version = kanivin.flags.in_test if ignore_version is None else ignore_version
	...
	def validate_amended_from(self):
		if kanivin.db.get_value(self.doctype, self.get("amended_from"), "docstatus") != 2:
			message = _(
				"{0} cannot be amended because it is not cancelled. Please cancel the document before creating an amendment."
			).format(kanivin.utils.get_link_to_form(self.doctype, self.get("amended_from")))
			kanivin.throw(message, title=_("Amendment Not Allowed"))
	...
	def update_child_table(self, fieldname: str, df: Optional["DocField"] = None):
		...
		if not (
			...
			or kanivin.get_meta(df.options).is_virtual == 1
		):
			existing_row_names = [row.name for row in all_rows if row.name and not row.is_new()]

			tbl = kanivin.qb.DocType(df.options)
			qry = (
				kanivin.qb.from_(tbl)
	...
	def set_new_name(self, force=False, set_name=None, set_child_names=True):
		"""Calls `kanivin.naming.set_new_name` for parent and child docs."""
	...
	def update_single(self, d):
		...
		kanivin.db.delete("Singles", {"doctype": self.doctype})
		...
		if self.doctype in kanivin.db.value_cache:
			del kanivin.db.value_cache[self.doctype]

	def set_user_and_timestamp(self):
		self._original_modified = self.modified
		self.modified = now()
		self.modified_by = kanivin.session.user
		...
		if self.is_new() and not (
			kanivin.flags.in_install or kanivin.flags.in_patch or kanivin.flags.in_migrate
		):
		...
		kanivin.flags.currently_saving.append((self.doctype, self.name))
	...
	def _validate_non_negative(self):
		def get_msg(df):
			if self.get("parentfield"):
				return "{} {} #{}: {} {}".format(
					...
					kanivin.bold(_(df.label, context=df.parent)),
				)
			else:
				return _("Value cannot be negative for {0}: {1}").format(
					_(df.parent), kanivin.bold(_(df.label, context=df.parent))
				)
		...
		for df in self.meta.get(
			...
			if flt(self.get(df.fieldname)) < 0:
				msg = get_msg(df)
				kanivin.throw(msg, kanivin.NonNegativeError, title=_("Negative Value"))
	...
	def validate_workflow(self):
		...
		if kanivin.flags.in_install == "kanivin":
			return
				...
				if fail:
					kanivin.throw(
						_("Value cannot be changed for {0}").format(
							kanivin.bold(self.meta.get_label(field.fieldname))
						),
						exc=kanivin.CannotChangeConstantError,
					)
	...
	def apply_fieldlevel_read_permissions(self):
		...
		for table_field in self.meta.get_table_fields():
			all_fields += kanivin.get_meta(table_field.options).fields or []
		...
		for table_field in self.meta.get_table_fields():
			for df in kanivin.get_meta(table_field.options).fields or []:
	...
	def validate_higher_perm_levels(self):
		...
		if self.flags.ignore_permissions or kanivin.flags.in_install:
			return
		...
		for df in self.meta.get_table_fields():
			high_permlevel_fields = kanivin.get_meta(df.options).get_high_permlevel_fields()
			if high_permlevel_fields:
	...
	def get_permissions(self):
		if self.meta.istable:
			...
			permissions = kanivin.get_meta(self.parenttype).permissions
		else:
	...
	def _set_defaults(self):
		...
		if self.is_new():
			new_doc = kanivin.new_doc(self.doctype, as_dict=True)
			self.update_if_missing(new_doc)

		# children
		for df in self.meta.get_table_fields():
			new_doc = kanivin.new_doc(df.options, parent_doc=self, parentfield=df.fieldname, as_dict=True)
	...
	def check_if_latest(self):
		...
		if cstr(previous.modified) != cstr(self._original_modified):
			kanivin.msgprint(
				...
				raise_exception=kanivin.TimestampMismatchError,
			)
	...
	def check_docstatus_transition(self, to_docstatus):
		...
		if to_docstatus == DocStatus.DRAFT:
			if self.docstatus.is_draft():
				...
				raise kanivin.DocstatusTransitionError(
					_("Cannot change docstatus from 0 (Draft) to 2 (Cancelled)")
				)
			else:
				raise kanivin.ValidationError(_("Invalid docstatus"), self.docstatus)
			...
			raise kanivin.ValidationError(_("Cannot edit cancelled document"))
	...
	def _validate_mandatory(self):
		...
		if kanivin.flags.print_messages:
			print(self.as_json().encode("utf-8"))

		raise kanivin.MandatoryError(
	...
	def _validate_links(self):
		...
		if invalid_links:
			...
			kanivin.throw(_("Could not find {0}").format(msg), kanivin.LinkValidationError)

		if cancelled_links:
			msg = ", ".join(each[2] for each in cancelled_links)
			kanivin.throw(_("Cannot link cancelled document: {0}").format(msg), kanivin.CancelledLinkError)
	...
	def run_notifications(self, method):
		...
		if (
			(kanivin.flags.in_import and kanivin.flags.mute_emails)
			or kanivin.flags.in_patch
			or kanivin.flags.in_install
		):
			return
		...
		from kanivin.email.doctype.notification.notification import evaluate_alert
		...
		if self.flags.notifications is None:
			...
			self.flags.notifications = kanivin.cache.hget("notifications", self.doctype, _get_notifications)
	...
	def _rename(self, name: str, merge: bool = False, force: bool = False, validate_rename: bool = True):
		"""Rename the document. Triggers kanivin.rename_doc, then reloads."""
	...
	def load_doc_before_save(self, *, raise_exception: bool = False):
		...
		try:
			self._doc_before_save = kanivin.get_doc(self.doctype, self.name, for_update=True)
	...
	def run_post_save_methods(self):
		...
		if (self.doctype, self.name) in kanivin.flags.currently_saving:
			kanivin.flags.currently_saving.remove((self.doctype, self.name))

	def clear_cache(self):
		kanivin.clear_document_cache(self.doctype, self.name)
	...
	def reset_seen(self):
		...
		if (
			...
			kanivin.db.set_value(
				self.doctype, self.name, "_seen", json.dumps([kanivin.session.user]), update_modified=False
			)

	def notify_update(self):
		"""Publish realtime that the current document is modified"""
		if kanivin.flags.in_patch:
			return
		...
		if not self.meta.get("read_only") and not self.meta.get("issingle") and not self.meta.get("istable"):
			data = {"doctype": self.doctype, "name": self.name, "user": kanivin.session.user}
			kanivin.publish_realtime("list_update", data, after_commit=True)
	...
	def db_set(self, fieldname, value=None, update_modified=True, notify=False, commit=False):
		...
		:param commit: default False. run kanivin.db.commit()
		"""
		...
		if update_modified and (self.doctype, self.name) not in kanivin.flags.currently_saving:
			# don't update modified timestamp if called from post save methods
			# like on_update or on_submit
			self.set("modified", now())
			self.set("modified_by", kanivin.session.user)
	...
	def db_get(self, fieldname):
		...
		return kanivin.db.get_value(self.doctype, self.name, fieldname)

	def check_no_back_links_exist(self):
		"""Check if document links to any active document before Cancel."""
		from kanivin.model.delete_doc import check_if_doc_is_dynamically_linked, check_if_doc_is_linked
	...
	def save_version(self):
		...
		if (
			...
			or kanivin.flags.in_install
			or (not self._doc_before_save and kanivin.flags.in_patch)
		):
			return

		doc_to_compare = self._doc_before_save
		if not doc_to_compare and (amended_from := self.get("amended_from")):
			doc_to_compare = kanivin.get_doc(self.doctype, amended_from)
	...
	def validate_table_has_rows(self, parentfield, raise_exception=None):
		...
		if not (isinstance(self.get(parentfield), list) and len(self.get(parentfield)) > 0):
			...
			kanivin.throw(
				_("Table {0} cannot be empty").format(label), raise_exception or kanivin.EmptyTableError
			)
	...
	def round_floats_in(self, doc, fieldnames=None):
		...
		rounding_method = kanivin.get_system_settings("rounding_method")
	...
	def add_comment(
		...
		return kanivin.get_doc(
			{
				"doctype": "Comment",
				"comment_type": comment_type,
				"comment_email": comment_email or kanivin.session.user,
	...
	def add_seen(self, user=None):
		...
		if self.meta.track_seen and not kanivin.flags.read_only and not self.meta.issingle:
			_seen = self.get("_seen") or []
			_seen = kanivin.parse_json(_seen)
			...
			if user not in _seen:
				...
				kanivin.local.flags.commit = True
	...
	def add_viewed(self, user=None, force=False, unique_views=False):
		...
		if unique_views and kanivin.db.exists(
	...
	def get_document_share_key(self, expires_on=None, no_expiry=False):
		...
		existing_key = kanivin.db.exists(
			"Document Share Key",
			{
				"reference_doctype": self.doctype,
		...
		if existing_key:
			doc = kanivin.get_doc("Document Share Key", existing_key)
		else:
			doc = kanivin.new_doc("Document Share Key")
	...
	def set_onload(self, key, value):
		if not self.get("__onload"):
			self.set("__onload", kanivin._dict())
		self.get("__onload")[key] = value

	def get_onload(self, key=None):
		if not key:
			return self.get("__onload", kanivin._dict())
	...
	def queue_action(self, action, **kwargs):
		...
		from kanivin.utils.background_jobs import enqueue
		...
		return enqueue(
			"kanivin.model.document.execute_action",
			...
			if timeout:
				for _ in range(timeout):
					...
					if not file_lock.lock_exists(signature):
						...
						break
			if lock_exists:
				raise kanivin.DocumentLockedError
		file_lock.create_lock(signature)
		kanivin.local.locked_documents.append(self)

	def unlock(self):
		"""Delete the lock file for this document"""
		file_lock.delete_lock(self.get_signature())
		if self in kanivin.local.locked_documents:
			kanivin.local.locked_documents.remove(self)
		...
		if date_diff(to_date, from_date) < 0:
			...
			if self.meta.istable:
				table_row = _("{0} row #{1}: ").format(
					_(kanivin.unscrub(self.parentfield)),
					self.idx,
				)
			...
			kanivin.throw(
				...
				+ _("{0} must be after {1}").format(
					kanivin.bold(_(self.meta.get_label(to_date_field))),
					kanivin.bold(_(self.meta.get_label(from_date_field))),
				),
				kanivin.exceptions.InvalidDates,
			)

	def get_assigned_users(self):
		assigned_users = kanivin.get_all(
			"ToDo",
			fields=["allocated_to"],
			filters={
	...
	def add_tag(self, tag):
		...
		from kanivin.desk.doctype.tag.tag import DocTags
	...
	def deferred_insert(self) -> None:
		...
		from kanivin.deferred_insert import deferred_insert
...
def execute_action(__doctype, __name, __action, **kwargs):
	...
	doc = kanivin.get_doc(__doctype, __name)
	doc.unlock()
	try:
		...
		# add a comment (?)
		if kanivin.message_log:
			msg = kanivin.message_log[-1].get("message")
		else:
			msg = "<pre><code>" + kanivin.get_traceback() + "</pre></code>"
...
def bulk_insert(
	...
	doctype_meta = kanivin.get_meta(doctype)
	...
	for child_table in doctype_meta.get_table_fields():
		valid_column_map[child_table.options] = kanivin.get_meta(child_table.options).get_valid_columns()
...
def _document_values_generator(
	...
	for doc in documents:
		...
		doc.owner = doc.modified_by = kanivin.session.user
...
def unlock_document(doctype: str | None = None, name: str | None = None, args=None):
	...
	if not doctype and not name and args:
		...
		name = str(args["name"])
	kanivin.get_doc(doctype, name).unlock()
	kanivin.msgprint(kanivin._("Document Unlocked"), alert=True)
