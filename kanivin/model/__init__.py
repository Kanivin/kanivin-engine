...
def delete_fields(args_dict, delete=0):
	...
	* If single, deletes record from `tabSingles`
	...
	for dt in args_dict:
		...
		if kanivin.db.get_value("DocType", dt, "issingle"):
			kanivin.db.delete(
				"Singles",
				{
					"doctype": dt,
			...
			existing_fields = kanivin.db.describe(dt)
...
def get_permitted_fields(
	...
	*,
	...
	if permission_type is None:
		permission_type = "select" if kanivin.only_has_select_perm(doctype, user=user) else "read"
