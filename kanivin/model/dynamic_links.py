...
def get_dynamic_link_map(for_delete=False):
	...
	"""
	if getattr(kanivin.local, "dynamic_link_map", None) is None or kanivin.flags.in_test:
		# Build from scratch
		dynamic_link_map = {}
		...
		kanivin.local.dynamic_link_map = dynamic_link_map
	return kanivin.local.dynamic_link_map
...
def get_dynamic_links():
	...
	for query in dynamic_link_queries:
		df += kanivin.db.sql(query, as_dict=True)
	return df
...
def fetch_distinct_link_doctypes(doctype: str, fieldname: str):
	...
	doctypes = kanivin.cache.get_value(key)

	if doctypes is None:
		doctypes = kanivin.db.sql(f"""select distinct `{fieldname}` from `tab{doctype}`""", pluck=True)
		kanivin.cache.set_value(key, doctypes, expires_in_sec=12 * 60 * 60)
...
def invalidate_distinct_link_doctypes(doctype: str, fieldname: str, linked_doctype: str):
	...
	if linked_doctype not in doctypes:
		...
		kanivin.db.after_commit.add(lambda: kanivin.cache.delete_value(key))
