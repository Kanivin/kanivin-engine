...
def get_user_settings(doctype, for_update=False):
	user_settings = kanivin.cache.hget("_user_settings", f"{doctype}::{kanivin.session.user}")

	if user_settings is None:
		user_settings = kanivin.db.sql(
			"""select data from `__UserSettings`
			where `user`=%s and `doctype`=%s""",
			(kanivin.session.user, doctype),
		)
...
def update_user_settings(doctype, user_settings, for_update=False):
	...
	kanivin.cache.hset("_user_settings", f"{doctype}::{kanivin.session.user}", json.dumps(current))


def sync_user_settings():
	"""Sync from cache to database (called asynchronously via the browser)"""
	for key, data in kanivin.cache.hgetall("_user_settings").items():
		key = safe_decode(key)
...
def update_user_settings_data(
	...
	if data:
		...
		if update:
			...
			kanivin.cache.hset("_user_settings", f"{user_setting.doctype}::{user_setting.user}", None)
