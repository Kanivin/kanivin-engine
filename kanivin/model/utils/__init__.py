...
def set_default(doc, key):
	...
	if not doc.is_default:
		kanivin.db.set(doc, "is_default", 1)
...
def set_field_property(filters, key, value):
	...
	docs = [
		kanivin.get_doc("DocType", d.parent)
		for d in kanivin.get_all("DocField", fields=["parent"], filters=filters)
	]
...
class InvalidIncludePath(kanivin.ValidationError):
	pass
...
def render_include(content):
	...
	for _ignore in range(5):
		if "{% include" in content:
			...
			if not paths:
				kanivin.throw(_("Invalid include path"), InvalidIncludePath)

			for path in paths:
				app, app_path = path.split("/", 1)
				with open(kanivin.get_app_path(app, app_path), encoding="utf-8") as f:
					include = f.read()
...
def get_fetch_values(doctype, fieldname, value):
	...
	db_values = kanivin.db.get_value(
...
def is_virtual_doctype(doctype: str):
	if kanivin.db.has_column("DocType", "is_virtual"):
		return kanivin.db.get_value("DocType", doctype, "is_virtual")
	return False
