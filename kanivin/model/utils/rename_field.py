...
import kanivin
from kanivin.model import no_value_fields, table_fields
from kanivin.model.utils.user_settings import sync_user_settings, update_user_settings_data
from kanivin.utils.password import rename_password_field
...
def rename_field(doctype, old_fieldname, new_fieldname, validate=True):
	...
	if validate:
		...
		if not meta.issingle and not kanivin.db.has_column(doctype, old_fieldname):
	...
	if new_field.fieldtype in table_fields:
		...
		if meta.issingle:
			...
			# copy field value
			kanivin.db.sql(f"""update `tab{doctype}` set `{new_fieldname}`=`{old_fieldname}`""")
...
def update_reports(doctype, old_fieldname, new_fieldname):
	...
	for r in reports:
		...
		if report_dict.get("updated"):
			...
			kanivin.db.sql("""update `tabReport` set `json`=%s where name=%s""", (new_val, r.name))


def update_users_report_view_settings(doctype, ref_fieldname, new_fieldname):
	user_report_cols = kanivin.db.sql(
