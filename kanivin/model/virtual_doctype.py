...
class VirtualDoctype(Protocol):
	...
	- DocType controller has to inherit from `kanivin.model.document.Document` class
	...
	@staticmethod
	def get_list(args) -> list[kanivin._dict]:
...
def validate_controller(doctype: str) -> None:
	try:
		...
		kanivin.msgprint(_("Failed to import virtual doctype {}, is controller file present?").format(doctype))
		return
		...
		if not isinstance(method, staticmethod):
			kanivin.msgprint(
				_("Virtual DocType {} requires a static method called {} found {}").format(
					kanivin.bold(doctype), kanivin.bold(m), kanivin.bold(_as_str(method))
				),
