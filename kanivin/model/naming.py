...
from kanivin.utils import cint, cstr, now_datetime
...
class InvalidNamingSeriesError(kanivin.ValidationError):
	pass
...
class NamingSeries:
	...
	def validate(self):
		if "." not in self.series:
			kanivin.throw(
				_("Invalid naming series {}: dot (.) missing").format(kanivin.bold(self.series)),
		...
		if not NAMING_SERIES_PATTERN.match(self.series):
			kanivin.throw(
				...
				).format(kanivin.bold(self.series)),
				exc=InvalidNamingSeriesError,
			)
	...
	def get_prefix(self) -> str:
		...
		if prefix is None:
			kanivin.throw(_("Invalid Naming Series: {}").format(self.series))
	...
	def update_counter(self, new_count: int) -> None:
		...
		Series = kanivin.qb.DocType("Series")
		prefix = self.get_prefix()

		# Initialize if not present in DB
		if kanivin.db.get_value("Series", prefix, "name", order_by="name") is None:
			kanivin.qb.into(Series).insert(prefix, 0).columns("name", "current").run()

		(kanivin.qb.update(Series).set(Series.current, cint(new_count)).where(Series.name == prefix)).run()

	def get_current_value(self) -> int:
		prefix = self.get_prefix()
		return cint(kanivin.db.get_value("Series", prefix, "current", order_by="name"))
...
def set_new_name(doc):
	...
	if autoname.lower() != "prompt" and not kanivin.flags.in_import:
		doc.name = None

	if is_autoincremented(doc.doctype, meta):
		doc.name = kanivin.db.get_next_sequence_val(doc.doctype)
		return
...
def set_name_from_naming_options(autoname, doc):
	...
	if _autoname.startswith("field:"):
		...
		if not doc.name:
			...
			kanivin.throw(_("{0} is required").format(doc.meta.get_label(fieldname)))
...
def set_naming_from_document_naming_rule(doc):
	...
	"""
	from kanivin.model.base_document import DOCTYPES_FOR_DOCTYPE
	...
	document_naming_rules = kanivin.cache_manager.get_doctype_map(
	...
	for d in document_naming_rules:
		kanivin.get_cached_doc("Document Naming Rule", d.name).apply(doc)
		if doc.name:
			break
...
def set_name_by_naming_series(doc):
	...
	if not doc.naming_series:
		kanivin.throw(kanivin._("Naming Series mandatory"))
...
def _generate_random_string(length=10):
	"""Better version of kanivin.generate_hash for naming.
...
def parse_naming_series(
	...
	for e in parts:
		...
		if e.startswith("#"):
			...
			part = kanivin.get_attr(method[0])(doc, e)
		else:
			part = e
...
def has_custom_parser(e):
	...
	return kanivin.get_hooks("naming_series_variables", {}).get(e)
...
def getseries(key, digits):
	# series created ?
	# Using kanivin.qb as kanivin.get_values does not allow order_by=None
	series = DocType("Series")
	current = (kanivin.qb.from_(series).where(series.name == key).for_update().select("current")).run()

	if current and current[0][0] is not None:
		current = current[0][0]
		# yes, update it
		kanivin.db.sql("UPDATE `tabSeries` SET `current` = `current` + 1 WHERE `name`=%s", (key,))
		current = cint(current) + 1
	else:
		# no, create it
		kanivin.db.sql("INSERT INTO `tabSeries` (`name`, `current`) VALUES (%s, 1)", (key,))
		current = 1
...
def revert_series_if_last(key, name, doc=None):
	...
	*Example:*
	...
	series = DocType("Series")
	current = (kanivin.qb.from_(series).where(series.name == prefix).for_update().select("current")).run()

	if current and current[0][0] == count:
		kanivin.db.sql("UPDATE `tabSeries` SET `current` = `current` - 1 WHERE `name`=%s", prefix)


def get_default_naming_series(doctype: str) -> str | None:
	"""get default value for `naming_series` property"""
	naming_series_options = kanivin.get_meta(doctype).get_naming_series_options()
...
def validate_name(doctype: str, name: int | str):
	if not name:
		kanivin.throw(_("No Name Specified for {0}").format(doctype))
	...
	if isinstance(name, int):
		if is_autoincremented(doctype):
			...
			kanivin.db.set_next_sequence_val(doctype, name, is_val_used=True)
			return name

		kanivin.throw(_("Invalid name type (integer) for varchar name column"), kanivin.NameError)

	if name.startswith("New " + doctype):
		kanivin.throw(
			_("There were some errors setting the name, please contact the administrator"), kanivin.NameError
		)
	name = name.strip()

	if not kanivin.get_meta(doctype).get("issingle") and (doctype == name) and (name != "DocType"):
		kanivin.throw(_("Name of {0} cannot be {1}").format(doctype, name), kanivin.NameError)

	special_characters = "<>"
	if re.findall(f"[{special_characters}]+", name):
		message = ", ".join(f"'{c}'" for c in special_characters)
		kanivin.throw(_("Name cannot contain special characters like {0}").format(message), kanivin.NameError)
...
def append_number_if_name_exists(doctype, value, fieldname="name", separator="-", filters=None):
	...
	exists = kanivin.db.exists(doctype, filters)
	...
	if exists:
		last = kanivin.db.sql(
			...
			WHERE `{fieldname}` {kanivin.db.REGEX_CHARACTER} %s
...
def _set_amended_name(doc):
	...
	if kanivin.db.get_value(doc.doctype, doc.amended_from, "amended_from"):
...
def _prompt_autoname(autoname, doc):
	...
	if not doc.name:
		kanivin.throw(_("Please set the document name"))
