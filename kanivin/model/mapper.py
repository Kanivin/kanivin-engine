...
from kanivin.model import child_table_fields, default_fields, table_fields
...
def make_mapped_doc(method, source_name, selected_children=None, args=None):
	...
	method = kanivin.get_attr(kanivin.override_whitelisted_method(method))

	if method not in kanivin.whitelisted:
	...
	if args:
		kanivin.flags.args = kanivin._dict(json.loads(args))

	kanivin.flags.selected_children = selected_children or None
...
def get_mapped_doc(
	...
	apply_strict_user_permissions = kanivin.get_system_settings("apply_strict_user_permissions")

	# main
	if not target_doc:
		target_doc = kanivin.new_doc(table_maps[from_doctype]["doctype"])
	elif isinstance(target_doc, str):
		target_doc = kanivin.get_doc(json.loads(target_doc))
	...
	if cached:
		source_doc = kanivin.get_cached_doc(from_doctype, from_docname)
	else:
		source_doc = kanivin.get_doc(from_doctype, from_docname)
		...
		for df in source_doc.meta.get_table_fields():
			...
			if table_map:
				...
				for source_d in source_doc.get(df.fieldname):
					...
					if (
						kanivin.flags.selected_children
						and (df.fieldname in kanivin.flags.selected_children)
						and source_d.name not in kanivin.flags.selected_children[df.fieldname]
					):
						continue
...
def map_fetch_fields(target_doc, df, no_copy_fields):
	...
	for fetch_df in target_doc.meta.get("fields", {"fetch_from": f"^{df.fieldname}."}):
		...
		if (
			...
			if not linked_doc:
				try:
					linked_doc = kanivin.get_doc(df.options, target_doc.get(df.fieldname))
				except Exception:
					return
...
def map_child_doc(source_d, target_parent, table_map, source_parent=None):
	...
	target_d = kanivin.new_doc(target_child_doctype, parent_doc=target_parent, parentfield=target_parentfield)
