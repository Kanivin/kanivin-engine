...
if TYPE_CHECKING:
	...
	from kanivin.workflow.doctype.workflow.workflow import Workflow


class WorkflowStateError(kanivin.ValidationError):
	pass


class WorkflowTransitionError(kanivin.ValidationError):
	pass


class WorkflowPermissionError(kanivin.ValidationError):
	pass


def get_workflow_name(doctype):
	workflow_name = kanivin.cache.hget("workflow", doctype)
	if workflow_name is None:
		workflow_name = kanivin.db.get_value("Workflow", {"document_type": doctype, "is_active": 1}, "name")
		kanivin.cache.hset("workflow", doctype, workflow_name or "")
...
def get_transitions(
	...
	if not isinstance(doc, Document):
		doc = kanivin.get_doc(kanivin.parse_json(doc))
		doc.load_from_db()
	...
	if not current_state:
		if raise_exception:
			...
			kanivin.throw(_("Workflow State not set"), WorkflowStateError)
...
def get_workflow_safe_globals():
	# access to kanivin.db.get_value, kanivin.db.get_list, and date time utils.
	return dict(
		kanivin=kanivin._dict(
			db=kanivin._dict(get_value=kanivin.db.get_value, get_list=kanivin.db.get_list),
			session=kanivin.session,
			utils=kanivin._dict(
				now_datetime=kanivin.utils.now_datetime,
				add_to_date=kanivin.utils.add_to_date,
				get_datetime=kanivin.utils.get_datetime,
...
def is_transition_condition_satisfied(transition, doc) -> bool:
	if not transition.condition:
		...
		return kanivin.safe_eval(transition.condition, get_workflow_safe_globals(), dict(doc=doc.as_dict()))
...
def apply_workflow(doc, action):
	...
	if not transition:
		kanivin.throw(_("Not a valid Workflow Action"), WorkflowTransitionError)

	if not has_approval_access(user, doc, transition):
		kanivin.throw(_("Self approval is not allowed"))
	...
	if doc.docstatus.is_draft() and new_docstatus.is_draft():
		...
		kanivin.throw(_("Illegal Document Status for {0}").format(next_state.state))
...
def validate_workflow(doc):
	...
	if current_state != next_state:
		bold_current = kanivin.bold(current_state)
		bold_next = kanivin.bold(next_state)
...
def get_workflow(doctype) -> "Workflow":
	return kanivin.get_cached_doc("Workflow", get_workflow_name(doctype))
...
def get_workflow_field_value(workflow_name, field):
	return kanivin.get_cached_value("Workflow", workflow_name, field)
...
def bulk_workflow_approval(docnames, doctype, action):
	...
	if len(docnames) < 20:
		...
		kanivin.msgprint(_("Bulk {0} is enqueued in background.").format(action), alert=True)
		kanivin.enqueue(
		...
		kanivin.throw(_("Bulk approval only support up to 500 documents."), title=_("Too Many Documents"))
...
def _bulk_workflow_action(docnames, doctype, action):
	...
	for idx, docname in enumerate(docnames, 1):
		...
		try:
			...
			apply_workflow(kanivin.get_doc(doctype, docname), action)
			kanivin.db.commit()
		except Exception as e:
			if not kanivin.message_log:
			...
			if not message_dict:
				if kanivin.message_log:
					messages = kanivin.get_message_log()
					for message in messages:
						kanivin.message_log.pop()
...
def print_workflow_log(messages, title, doctype, indicator):
	if messages.keys():
		...
		for doc in messages.keys():
			if len(messages[doc]):
				html = f"<details><summary>{kanivin.utils.get_link_to_form(doctype, doc)}</summary>"
	...
	try:
		for i, doc in enumerate(docs, 1):
			...
			actions = [
				...
				if has_approval_access(kanivin.session.user, doc, t)
			]
			if not actions:
				return []
...
def show_progress(docnames, message, i, description):
	n = len(docnames)
	if n >= 5:
		kanivin.publish_progress(float(i) * 100 / n, title=message, description=description)


def set_workflow_state_on_action(doc, workflow_name, action):
	workflow = kanivin.get_doc("Workflow", workflow_name)
