...
from kanivin.model.base_document import (
...
from kanivin.utils import cast, cint, cstr
from kanivin.utils.data import add_to_date, get_datetime
...
def get_meta(doctype, cached=True) -> "Meta":
	...
	if cached and (meta := kanivin.cache.hget("doctype_meta", doctype)):
		return meta

	meta = Meta(doctype)
	kanivin.cache.hset("doctype_meta", meta.name, meta)
	return meta
...
def get_table_columns(doctype):
	return kanivin.db.get_table_columns(doctype)


def load_doctype_from_file(doctype):
	fname = kanivin.scrub(doctype)
	with open(kanivin.get_app_path("kanivin", "core", "doctype", fname, fname + ".json")) as f:
...
class Meta(Document):
	...
	standard_set_once_fields = (
		kanivin._dict(fieldname="creation", fieldtype="Datetime"),
		kanivin._dict(fieldname="owner", fieldtype="Data"),
	)
	...
	def get_global_search_fields(self):
		...
		if getattr(self, "show_name_in_global_search", None):
			fields.append(kanivin._dict(fieldtype="Data", fieldname="name", label="Name"))

		return fields

	def get_valid_columns(self) -> list[str]:
		if not hasattr(self, "_valid_columns"):
			table_exists = kanivin.db.table_exists(self.name)
	...
	def add_custom_fields(self):
		if not kanivin.db.table_exists("Custom Field"):
			return

		custom_fields = kanivin.db.get_values(
			"Custom Field",
	...
	def apply_property_setters(self):
		...
		"""
		if not kanivin.db.table_exists("Property Setter"):
			return

		property_setters = kanivin.db.get_values(
			"Property Setter",
	...
	def check_if_large_table(self):
		...
		if self.istable or not kanivin.db.table_exists(self.name):  # During install, new migrate
			return

		if kanivin.db.estimate_count(self.name) > LARGE_TABLE_SIZE_THRESHOLD:
			recent_change = kanivin.db.get_value(self.name, {}, "modified", order_by="modified desc")
	...
	def set_custom_permissions(self):
		...
		if kanivin.flags.in_patch or kanivin.flags.in_install:
			return

		if not self.istable and self.name not in ("DocType", "DocField", "DocPerm", "Custom DocPerm"):
			custom_perms = kanivin.get_all(
				"Custom DocPerm",
				fields="*",
	...
	def get_fields_to_check_permissions(self, user_permission_doctypes):
		...
		if self.name in user_permission_doctypes:
			fields.append(kanivin._dict({"label": "Name", "fieldname": "name", "options": self.name}))
	...
	def get_permitted_fieldnames(
		...
		*,
		...
		if not permission_type:
			permission_type = "select" if kanivin.only_has_select_perm(self.name, user=user) else "read"
		...
		if 0 not in permlevel_access and permission_type in ("read", "select"):
			if kanivin.share.get_shared(self.name, user, rights=[permission_type], limit=1):
	...
	def get_permlevel_access(self, permission_type="read", parenttype=None, *, user=None):
		has_access_to = []
		roles = set(kanivin.get_roles(user))
	...
	def get_permissions(self, parenttype=None):
		if self.istable and parenttype:
			...
			permissions = kanivin.get_meta(parenttype).permissions
		else:
	...
	def get_dashboard_data(self):
		...
		implemented in other Kanivin applications via hooks.
		"""
		data = kanivin._dict()
		if not self.custom:
			try:
				...
				if hasattr(module, "get_data"):
					data = kanivin._dict(module.get_data())
			except ImportError:
				pass
		...
		if not self.custom:
			for hook in kanivin.get_hooks("override_doctype_dashboards", {}).get(self.name, []):
				data = kanivin._dict(kanivin.get_attr(hook)(data=data))
	...
	def get_web_template(self, suffix=""):
		...
		module_name = kanivin.scrub(self.module)
		doctype = kanivin.scrub(self.name)
		template_path = kanivin.get_module_path(
...
def is_single(doctype):
	try:
		return kanivin.db.get_value("DocType", doctype, "issingle")
	except IndexError:
		raise Exception("Cannot determine whether %s is single" % doctype)


def get_parent_dt(dt):
	if not kanivin.is_table(dt):
		return ""

	return (
		kanivin.db.get_value(
			"DocField",
			{"fieldtype": ("in", kanivin.model.table_fields), "options": dt},
			"parent",
		)
		or ""
	)
...
def set_fieldname(field_id, fieldname):
	kanivin.db.set_value("DocField", field_id, "fieldname", fieldname)
...
def get_field_currency(df, doc=None):
	...
	if not getattr(kanivin.local, "field_currency", None):
		kanivin.local.field_currency = kanivin._dict()

	if not (
		kanivin.local.field_currency.get((doc.doctype, doc.name), {}).get(df.fieldname)
		or (
			doc.get("parent")
			and kanivin.local.field_currency.get((doc.doctype, doc.parent), {}).get(df.fieldname)
		)
	):
		...
		if ":" in cstr(df.get("options")):
			...
			if len(split_opts) == 3 and doc.get(split_opts[1]):
				currency = kanivin.get_cached_value(split_opts[0], doc.get(split_opts[1]), split_opts[2])
		else:
			...
			if doc.get("parenttype"):
				if currency:
					...
					if kanivin.get_meta(doc.parenttype).has_field(df.get("options")):
						# only get_value if parent has currency field
						currency = kanivin.db.get_value(doc.parenttype, doc.parent, df.get("options"))

		if currency:
			kanivin.local.field_currency.setdefault((doc.doctype, ref_docname), kanivin._dict()).setdefault(
				df.fieldname, currency
			)

	return kanivin.local.field_currency.get((doc.doctype, doc.name), {}).get(df.fieldname) or (
		doc.get("parent") and kanivin.local.field_currency.get((doc.doctype, doc.parent), {}).get(df.fieldname)
	)


def get_field_precision(df, doc=None, currency=None):
	"""get precision based on DocField options and fieldvalue in doc"""
	from kanivin.utils import get_number_format_info

	if df.precision:
		precision = cint(df.precision)

	elif df.fieldtype == "Currency":
		precision = cint(kanivin.db.get_default("currency_precision"))
		if not precision:
			number_format = kanivin.db.get_default("number_format") or "#,###.##"
			decimal_str, comma_str, precision = get_number_format_info(number_format)
	else:
		precision = cint(kanivin.db.get_default("float_precision")) or 3
...
def get_default_df(fieldname):
	if fieldname in (default_fields + child_table_fields):
		if fieldname in ("creation", "modified"):
			return kanivin._dict(fieldname=fieldname, fieldtype="Datetime")

		elif fieldname in ("idx", "docstatus"):
			return kanivin._dict(fieldname=fieldname, fieldtype="Int")

		return kanivin._dict(fieldname=fieldname, fieldtype="Data")
...
def trim_tables(doctype=None, dry_run=False, quiet=False):
	...
	for doctype in kanivin.get_all("DocType", filters=filters, pluck="name"):
		try:
...
def trim_table(doctype, dry_run=True):
	kanivin.cache.hdel("table_columns", f"tab{doctype}")
	ignore_fields = default_fields + optional_fields + child_table_fields
	columns = kanivin.db.get_table_columns(doctype)
	fields = kanivin.get_meta(doctype, cached=False).get_fieldnames_with_value()
	...
	if columns_to_remove and not dry_run:
		...
		kanivin.db.sql_ddl(f"ALTER TABLE `tab{doctype}` {columns_to_remove}")
