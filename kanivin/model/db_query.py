...
from kanivin.database.utils import DefaultOrderBy, FallBackDateTimeStr, NestedSetHierarchy
from kanivin.model import OPTIONAL_FIELDS, get_permitted_fields, optional_fields
from kanivin.model.meta import get_table_columns
from kanivin.model.utils import is_virtual_doctype
from kanivin.model.utils.user_settings import get_user_settings, update_user_settings
from kanivin.query_builder.utils import Column
...
from kanivin.utils.data import DateTimeLikeObject, get_datetime, getdate, sbool
...
class DatabaseQuery:
	def __init__(self, doctype, user=None):
		...
		self.or_conditions = []
		self.fields = None
		self.user = user or kanivin.session.user
	...
	def doctype_meta(self):
		if not hasattr(self, "_doctype_meta"):
			self._doctype_meta = kanivin.get_meta(self.doctype)
	...
	def build_and_run(self):
		...
		if kanivin.db.db_type == "postgres" and args.order_by and args.group_by:
	...
	def prepare_args(self):
		...
		for child in self.tables[1:]:
			...
			args.tables += f" {self.join} {child} on ({child}.parenttype = {kanivin.db.escape(self.doctype)} and {child}.parent = {parent_name})"
	...
	def parse_args(self):
		...
		for field in self.fields:
			if "." in field:
				...
				linked_field = kanivin.get_meta(self.doctype).get_field(linked_fieldname)
	...
	def sanitize_fields(self):
		...
		def _raise_exception():
			kanivin.throw(_("Use of sub-query or function is restricted"), kanivin.DataError)
		...
		for field in self.fields:
			...
			if SUB_QUERY_PATTERN.match(field):
				...
				if function in blacklisted_functions:
					kanivin.throw(
						_("Use of function {0} in field is restricted").format(function), exc=kanivin.DataError
					)
			...
			if self.strict:
				if STRICT_FIELD_PATTERN.match(field):
					kanivin.throw(_("Illegal SQL Query"))
	...
	def _set_permission_map(self, doctype: str, parent_doctype: str | None = None):
		ptype = "select" if kanivin.only_has_select_perm(doctype) else "read"
		kanivin.has_permission(
			doctype,
			ptype=ptype,
	...
	def get_table_columns(self):
		...
		except kanivin.db.TableMissingError:
			if self.ignore_ddl:
				return None
			else:
				raise
	...
	def apply_fieldlevel_read_permissions(self):
		...
		Note: Does not apply to `kanivin.model.core_doctype_list`
		...
		Example:
		        ...
		        - Query: fields=["*"]
		        - Result: fields=["title", ...] // will also include Kanivin's meta field like `name`, `owner`, etc.
		"""
		from kanivin.desk.reportview import extract_fieldnames
		...
		for i, field in enumerate(self.fields):
			...
			# labels / pseudo columns or kanivin internals
			if column[0] in {"'", '"'}:
				continue
			...
			if doctype and doctype != self.doctype:
				if wrap_grave_quotes(table) not in self.query_tables:
					raise kanivin.PermissionError(doctype)
	...
	def prepare_filter_condition(self, f):
		...
		from kanivin.boot import get_additional_filters_from_hooks
		...
		if f.operator.lower() in NestedSetHierarchy:
			...
			lft, rgt = "", ""
			if f.value:
				lft, rgt = kanivin.db.get_value(ref_doctype, f.value, ["lft", "rgt"]) or (0, 0)
			...
			fallback = "''"
			value = [kanivin.db.escape((cstr(v)).strip(), percent=False) for v in nodes]
			if len(value):
				value = f"({', '.join(value)})"
			else:
				value = "('')"
			...
			fallback = "''"
			value = [kanivin.db.escape((cstr(v) or "").strip(), percent=False) for v in values]
			if len(value):
				value = f"({', '.join(value)})"
			else:
				value = "('')"
			...
			if f.operator in (">", "<", ">=", "<=") and (f.fieldname in ("creation", "modified")):
				...
				value = kanivin.db.format_date(f.value)
				fallback = "'0001-01-01'"

			elif (df and df.fieldtype == "Datetime") or isinstance(f.value, datetime.datetime):
				value = kanivin.db.format_datetime(f.value)
				fallback = f"'{FallBackDateTimeStr}'"
			...
			if isinstance(f.value, Column):
				...
				quote = '"' if kanivin.conf.db_type == "postgres" else "`"
				value = f"{tname}.{quote}{f.value.name}{quote}"

			# escape value
			elif escape and isinstance(value, str):
				value = f"{kanivin.db.escape(value, percent=False)}"
		...
		if (
			...
			if f.operator.lower() == "like" and kanivin.conf.get("db_type") == "postgres":
				f.operator = "ilike"
	...
	def build_match_conditions(self, as_condition=True) -> str | list:
		...
		if not self.user:
			self.user = kanivin.session.user

		if not self.tables:
			self.extract_tables()

		role_permissions = kanivin.permissions.get_role_permissions(self.doctype_meta, user=self.user)
		if (
			...
			only_if_shared = True
			self.shared = kanivin.share.get_shared(self.doctype, self.user)
			if not self.shared:
				kanivin.throw(_("No permission to read {0}").format(_(self.doctype)), kanivin.PermissionError)
			else:
			...
			if requires_owner_constraint(role_permissions):
				...
				self.match_conditions.append(
					f"`tab{self.doctype}`.`owner` = {kanivin.db.escape(self.user, percent=False)}"
				)

			# add user permission only if role has read perm
			elif role_permissions.get("read") or role_permissions.get("select"):
				# get user permissions
				user_permissions = kanivin.permissions.get_user_permissions(self.user)
	...
	def get_share_condition(self):
		return (
			...
			+ f" in ({', '.join(kanivin.db.escape(s, percent=False) for s in self.shared)})"
		)
	...
	def add_user_permissions(self, user_permissions):
		...
		for df in doctype_link_fields:
			...
			if user_permission_values:
				docs = []
				if kanivin.get_system_settings("apply_strict_user_permissions"):
					condition = ""
				else:
				...
				if docs:
					values = ", ".join(kanivin.db.escape(doc, percent=False) for doc in docs)
	...
	def get_permission_query_conditions(self) -> str:
		conditions = []
		hooks = kanivin.get_hooks("permission_query_conditions", {})
		condition_methods = hooks.get(self.doctype, []) + hooks.get("*", [])
		for method in condition_methods:
			if c := kanivin.call(kanivin.get_attr(method), self.user, doctype=self.doctype):
				conditions.append(c)

		if permission_script_name := get_server_script_map().get("permission_query", {}).get(self.doctype):
			script = kanivin.get_doc("Server Script", permission_script_name)
	...
	def validate_order_by_and_group_by(self, parameters: str):
		...
		if "select" in _lower and "from" in _lower:
			kanivin.throw(_("Cannot use sub-query in order by"))
		...
		for field in parameters.split(","):
			...
			if full_field_name:
				...
				if tbl not in self.tables:
					if tbl.startswith("`"):
						tbl = tbl[4:-1]
					kanivin.throw(_("Please select atleast 1 column from {0} to sort/group").format(tbl))

			# Check if the function is used anywhere in the field
			if any(func in function for func in blacklisted_sql_functions):
				kanivin.throw(_("Cannot use {0} in order/group by").format(function))
...
def check_parent_permission(parent, child_doctype):
	if parent:
		...
		if child_doctype and not (
			kanivin.db.exists("DocField", {"parent": parent, "options": child_doctype})
			or kanivin.db.exists("Custom Field", {"dt": parent, "options": child_doctype})
		):
			raise kanivin.PermissionError

		if kanivin.permissions.has_permission(parent):
			return
...
def has_any_user_permission_for_doctype(doctype, user, applicable_for):
	user_permissions = kanivin.permissions.get_user_permissions(user=user)
...
def get_between_date_filter(value, df=None):
	...
	from_date = kanivin.utils.nowdate()
	to_date = kanivin.utils.nowdate()
	...
	if fieldtype == "Datetime":
		cond = f"'{kanivin.db.format_datetime(from_date)}' AND '{kanivin.db.format_datetime(to_date)}'"
	else:
		cond = f"'{kanivin.db.format_date(from_date)}' AND '{kanivin.db.format_date(to_date)}'"
...
def get_additional_filter_field(additional_filters_config, f, value):
	...
	f = kanivin._dict(kanivin.get_attr(additional_filter["get_field"])())
	if f.query_value:
		for option in f.options:
			option = kanivin._dict(option)
