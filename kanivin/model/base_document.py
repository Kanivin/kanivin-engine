...
from kanivin.model.dynamic_links import invalidate_distinct_link_doctypes
from kanivin.model.naming import set_new_name
from kanivin.model.utils.link_count import notify_link_count
...
def get_controller(doctype):
	...
	For `custom` type, returns `kanivin.model.document.Document`.

	:param doctype: DocType name as string.
	"""

	if kanivin.local.dev_server or kanivin.flags.in_migrate:
		return import_controller(doctype)

	site_controllers = kanivin.controllers.setdefault(kanivin.local.site, {})
...
def import_controller(doctype):
	from kanivin.model.document import Document
	from kanivin.utils.nestedset import NestedSet

	module_name = "Core"
	if doctype not in DOCTYPES_FOR_DOCTYPE:
		doctype_info = kanivin.db.get_value("DocType", doctype, ("module", "custom", "is_tree"), as_dict=True)
		if doctype_info:
	...
	module_path = None
	class_overrides = kanivin.get_hooks("override_doctype_class")
	if class_overrides and class_overrides.get(doctype):
		import_path = class_overrides[doctype][-1]
		module_path, classname = import_path.rsplit(".", 1)
		module = kanivin.get_module(module_path)
...
class BaseDocument:
	...
	@cached_property
	def meta(self):
		return kanivin.get_meta(self.doctype)
	...
	def get_db_value(self, key):
		return kanivin.db.get_value(self.doctype, self.name, key)
			...
			if df:
				if is_virtual_field:
					...
					if (prop := getattr(type(self), fieldname, None)) and is_a_property(prop):
						...
						from kanivin.utils.safe_exec import get_safe_globals

						value = kanivin.safe_eval(
							code=options,
				...
				if isinstance(value, list) and df.fieldtype not in table_fields:
					kanivin.throw(_("Value for {0} cannot be a list").format(_(df.label, context=df.parent)))
	...
	def get_valid_columns(self) -> list[str]:
		if self.doctype not in kanivin.local.valid_columns:
			if self.doctype in DOCTYPES_FOR_DOCTYPE:
				from kanivin.model.meta import get_table_columns

				valid = get_table_columns(self.doctype)
			else:
				valid = self.meta.get_valid_columns()

			kanivin.local.valid_columns[self.doctype] = valid

		return kanivin.local.valid_columns[self.doctype]
	...
	def as_json(self):
		return kanivin.as_json(self.as_dict())
	...
	def db_insert(self, ignore_if_duplicate=False):
		...
		if ignore_if_duplicate and kanivin.db.db_type == "postgres":
			conflict_handler = "on conflict (name) do nothing"

		if not self.creation:
			self.creation = self.modified = now()
			self.owner = self.modified_by = kanivin.session.user
		...
		try:
			...
			if kanivin.db.is_primary_key_violation(e):
				...
				if not ignore_if_duplicate:
					kanivin.msgprint(
						_("{0} {1} already exists").format(_(self.doctype), kanivin.bold(self.name)),
						title=_("Duplicate Name"),
						indicator="red",
					)
					raise kanivin.DuplicateEntryError(self.doctype, self.name, e)

			elif kanivin.db.is_unique_key_violation(e):
				# unique constraint
	...
	def db_update(self):
		...
		try:
			...
			if kanivin.db.is_unique_key_violation(e):
				self.show_unique_validation_message(e)
			else:
				raise
	...
	def show_unique_validation_message(self, e):
		if kanivin.db.db_type != "postgres":
			...
			kanivin.msgprint(_("{0} must be unique").format(label or fieldname))

		# this is used to preserve traceback
		raise kanivin.UniqueValidationError(self.doctype, self.name, e)
	...
	def update_modified(self):
		...
		if getattr(self.meta, "issingle", False):
			kanivin.db.set_single_value(self.doctype, "modified", self.modified, update_modified=False)
		else:
			kanivin.db.set_value(self.doctype, self.name, "modified", self.modified, update_modified=False)
		...
		for df in self.meta.get_link_fields() + self.meta.get("fields", {"fieldtype": ("=", "Dynamic Link")}):
			...
			if docname:
				if df.fieldtype == "Link":
					doctype = df.options
					if not doctype:
						kanivin.throw(_("Options not set for link field {0}").format(df.fieldname))
				else:
					doctype = self.get(df.options)
					if not doctype:
						kanivin.throw(_("{0} must be set first").format(self.meta.get_label(df.options)))
				...
				if not meta.get("is_virtual"):
					if not fields_to_fetch:
						...
						values = _dict(name=kanivin.db.get_value(doctype, docname, "name", cache=True))
					else:
						...
						values = kanivin.db.get_value(doctype, docname, values_to_fetch, as_dict=True)

				if getattr(meta, "issingle", 0):
					values.name = doctype

				if meta.get("is_virtual"):
					values = kanivin.get_doc(doctype, docname).as_dict()
				...
				if values:
					...
					if not values.name:
						...
						and kanivin.get_meta(doctype).is_submittable
						and DocStatus(kanivin.db.get_value(doctype, docname, "docstatus") or 0).is_cancelled()
					):
	...
	def set_fetch_from_value(self, doctype, df, values):
		...
		if df.fieldtype in ["Small Text", "Text", "Data"]:
			from kanivin.model.meta import get_default_df

			fetch_from_df = get_default_df(fetch_from_fieldname) or kanivin.get_meta(doctype).get_field(
	...
	def _validate_selects(self):
		...
		for df in self.meta.get_select_fields():
			...
			if value not in options and not (kanivin.flags.in_test and value.startswith("_T-")):
	...
	def _validate_data_fields(self):
		# data_field options defined in kanivin.model.data_field_options
		for phone_field in self.meta.get_phone_fields():
			phone = self.get(phone_field.fieldname)
			kanivin.utils.validate_phone_number_with_country_code(phone, phone_field.fieldname)
		...
		for data_field in self.meta.get_data_fields():
			...
			if data_field_options == "Email":
				if (self.owner in kanivin.STANDARD_USERS) and (data in kanivin.STANDARD_USERS):
					continue
				for email_address in kanivin.utils.split_emails(data):
					kanivin.utils.validate_email_address(email_address, throw=True)

			if data_field_options == "Name":
				kanivin.utils.validate_name(data, throw=True)

			if data_field_options == "Phone":
				kanivin.utils.validate_phone_number(data, throw=True)

			if data_field_options == "URL":
				if not data:
					continue

				kanivin.utils.validate_url(data, throw=True)

	def _validate_constants(self):
		if kanivin.flags.in_import or self.is_new() or self.flags.ignore_validate_constants:
			return

		constants = [d.fieldname for d in self.meta.get("fields", {"set_only_once": ("=", 1)})]
		if constants:
			values = kanivin.db.get_value(self.doctype, self.name, constants, as_dict=True)
		...
		for fieldname in constants:
			...
			if self.get(fieldname) != value:
				kanivin.throw(
					...
					kanivin.CannotChangeConstantError,
				)
	...
	def _validate_code_fields(self):
		for field in self.meta.get_code_fields():
			...
			if language == "Python":
				kanivin.utils.validate_python_code(code_string, fieldname=field.label, is_expression=False)

			elif language == "PythonExpression":
				kanivin.utils.validate_python_code(code_string, fieldname=field.label)
	...
	def throw_length_exceeded_error(self, df, max_length, value):
		...
		kanivin.throw(
			_("{0}: '{1}' ({3}) will get truncated, as max characters allowed is {2}").format(
				reference, kanivin.bold(_(df.label, context=df.parent)), max_length, value
			),
			kanivin.CharacterLengthExceededError,
			title=_("Value too big"),
		)

	def _validate_update_after_submit(self):
		# get the full doc with children
		db_values = kanivin.get_doc(self.doctype, self.name).as_dict()
				...
				if self_value != db_value:
					kanivin.throw(
						_("{0} Not allowed to change {1} after submission from {2} to {3}").format(
							...
							kanivin.bold(db_value),
							kanivin.bold(self_value),
						),
						kanivin.UpdateAfterSubmitError,
	...
	def _sanitize_content(self):
		...
		for fieldname, value in self.get_valid_dict(ignore_virtual=True).items():
			...
			value = kanivin.as_unicode(value)
	...
	def _save_passwords(self):
		...
		from kanivin.utils.password import remove_encrypted_password, set_encrypted_password
	...
	def get_formatted(
		...
		from kanivin.utils.formatters import format_value
		...
		if (
			...
			currency = kanivin.db.get_value("Currency", currency_value, cache=True)
	...
	def reset_values_if_no_permlevel_access(self, has_access_to, high_permlevel_fields):
		...
		if to_reset:
			if self.is_new():
				...
				ref_doc = kanivin.new_doc(self.doctype)
			else:
	...
	def _extract_images_from_editor(self):
		from kanivin.core.doctype.file.utils import extract_images_from_doc
