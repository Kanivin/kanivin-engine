...
from kanivin.utils import cint, cstr, get_url, now_datetime
from kanivin.utils.data import getdate
...
def get_emails_sent_this_month(email_account=None):
	...
	return kanivin.db.count("Email Queue", filters=filters)
...
def get_emails_sent_today(email_account=None):
	...
	return kanivin.db.sql(q, q_args)[0][0]


def get_unsubscribe_message(unsubscribe_message: str, expose_recipients: str) -> "kanivin._dict[str, str]":
	...
	return kanivin._dict(html=html, text=text)
...
def unsubscribe(doctype, name, email):
	...
	if not kanivin.flags.in_test and not verify_request():
		return
...
def flush():
	...
	if kanivin.are_emails_muted():
		msgprint(_("Emails are muted"))

	if cint(kanivin.db.get_default("suspend_email_queue")) == 1:
		return
	...
	for row in email_queue_batch:
		try:
			email_queue: EmailQueue = kanivin.get_doc("Email Queue", row.name)
			email_queue.send()
		except Exception:
			kanivin.get_doc("Email Queue", row.name).log_error()
			...
			if (
				...
				kanivin.throw(_("Email Queue flushing aborted due to too many failures."))


def get_queue():
	batch_size = cint(kanivin.conf.email_queue_batch_size) or 500
