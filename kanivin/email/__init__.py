...
def sendmail_to_system_managers(subject, content):
	kanivin.sendmail(recipients=get_system_managers(), subject=subject, content=content)
...
def get_contact_list(txt, page_length=20, extra_filters: str | None = None) -> list[dict]:
	...
	if extra_filters:
		extra_filters = kanivin.parse_json(extra_filters)
	...
	# https://github.com/kanivin/kanivin/blob/6c6a89bcdd9454060a1333e23b855d0505c9ebc2/kanivin/public/js/kanivin/form/controls/autocomplete.js#L29-L35
	return [
		kanivin._dict(
			value=d.email_id,
			label=d.email_id,
...
def get_communication_doctype(doctype, txt, searchfield, start, page_len, filters):
	user_perms = kanivin.utils.user.UserPermissions(kanivin.session.user)
	...
	com_doctypes = []
	if len(txt) < 2:
		for name in kanivin.get_hooks("communication_doctypes"):
			try:
		...
		com_doctypes = [
			d[0] for d in kanivin.db.get_values("DocType", {"issingle": 0, "istable": 0, "hide_toolbar": 0})
		]
