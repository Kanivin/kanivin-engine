...
kanivin.ui.form.on("Email Group", {
	refresh: function (frm) {
		if (!frm.is_new()) {
			frm.add_custom_button(
				...
				function () {
					kanivin.prompt(
						...
						function (data) {
							kanivin.call({
								method: "kanivin.email.doctype.email_group.email_group.import_from",
								args: {
									name: frm.doc.name,
			...
			frm.add_custom_button(
				...
				function () {
					kanivin.prompt(
						...
						function (data) {
							kanivin.call({
								method: "kanivin.email.doctype.email_group.email_group.add_subscribers",
								args: {
									name: frm.doc.name,
			...
			frm.add_custom_button(
				__("New Newsletter"),
				function () {
					kanivin.route_options = { email_group: frm.doc.name };
					kanivin.new_doc("Newsletter");
				},
				__("Action")
			);
		}
