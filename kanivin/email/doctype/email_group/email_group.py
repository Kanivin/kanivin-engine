...
from kanivin.utils import parse_addr, validate_email_address
...
class EmailGroup(Document):
	...
	def onload(self):
		singles = [d.name for d in kanivin.get_all("DocType", "name", {"issingle": 1})]
		self.get("__onload").import_types = [
			{"value": d.parent, "label": f"{d.parent} ({d.label})"}
			for d in kanivin.get_all("DocField", ("parent", "label"), {"options": "Email"})
	...
	def import_from(self, doctype):
		...
		for user in kanivin.get_all(doctype, [email_field, unsubscribed_field or "name"]):
			with contextlib.suppress(kanivin.UniqueValidationError, kanivin.InvalidEmailAddressError):
		...
		kanivin.msgprint(_("{0} subscribers added").format(added))
	...
	def on_trash(self):
		for d in kanivin.get_all("Email Group Member", "name", {"email_group": self.name}):
			kanivin.delete_doc("Email Group Member", d.name)


@kanivin.whitelist()
def import_from(name, doctype):
	nlist = kanivin.get_doc("Email Group", name)
...
def add_subscribers(name, email_list):
	...
	template = kanivin.db.get_value("Email Group", name, "welcome_email_template")
	welcome_email = kanivin.get_doc("Email Template", template) if template else None
	...
	for email in email_list:
		...
		if parsed_email:
			if not kanivin.db.get_value("Email Group Member", {"email_group": name, "email": parsed_email}):
				kanivin.get_doc(
					{"doctype": "Email Group Member", "email_group": name, "email": parsed_email}
				).insert(ignore_permissions=kanivin.flags.ignore_permissions)
			...
			kanivin.msgprint(_("{0} is not a valid Email Address").format(email))

	kanivin.msgprint(_("{0} subscribers added").format(count))

	return kanivin.get_doc("Email Group", name).update_total_subscribers()
...
def send_welcome_email(welcome_email, email, email_group):
	...
	message = kanivin.render_template(welcome_email.response_, args)
	kanivin.sendmail(email, subject=welcome_email.subject, message=message)
