...
import kanivin
from kanivin.email.doctype.newsletter.exceptions import (
	NewsletterAlreadySentError,
	NoRecipientFoundError,
)
from kanivin.email.doctype.newsletter.newsletter import (
	Newsletter,
...
from kanivin.utils import add_days, getdate
...
class TestNewsletterMixin:
	...
	def tearDown(self):
		...
		for newsletter in newsletters:
			...
			kanivin.delete_doc("Newsletter", newsletter)
			kanivin.db.delete("Newsletter Email Group", {"parent": newsletter})
			newsletters.remove(newsletter)

	def setup_email_group(self):
		if not kanivin.db.exists("Email Group", "_Test Email Group"):
			kanivin.get_doc({"doctype": "Email Group", "title": "_Test Email Group"}).insert()
		...
		for email in emails:
			...
			try:
				...
				kanivin.db.set_value(doctype, email_filters, "unsubscribed", 0)
	...
	def get_newsletter(**kwargs) -> "Newsletter":
		...
		similar_newsletters = kanivin.get_all(doctype, newsletter_content, pluck="name")

		for similar_newsletter in similar_newsletters:
			kanivin.delete_doc(doctype, similar_newsletter)

		newsletter = kanivin.get_doc({"doctype": doctype, **newsletter_content, **kwargs})
		...
		attached_files = kanivin.get_all(
			"File",
			{
		...
		for file in attached_files:
			kanivin.delete_doc("File", file)

		return newsletter


class TestNewsletter(TestNewsletterMixin, KanivinTestCase):
	def test_send(self):
		self.send_newsletter()

		email_queue_list = [kanivin.get_doc("Email Queue", e.name) for e in kanivin.get_all("Email Queue")]
	...
	def test_unsubscribe(self):
		...
		group = kanivin.get_all("Newsletter Email Group", filters={"parent": name}, fields=["email_group"])
	...
	def test_send_scheduled_email_error_handling(self):
		...
		job_path = "kanivin.email.doctype.newsletter.newsletter.Newsletter.queue_all"
		m = MagicMock(side_effect=kanivin.OutgoingEmailError)

		with self.assertRaises(kanivin.OutgoingEmailError):
	...
	def test_retry_partially_sent_newsletter(self):
		...
		# retry
		newsletter.send_emails()
		self.assertEqual(kanivin.db.count("Email Queue"), 4)
