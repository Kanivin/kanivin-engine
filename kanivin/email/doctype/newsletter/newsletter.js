...
kanivin.ui.form.on("Newsletter", {
	refresh(frm) {
		let doc = frm.doc;
		let can_write = kanivin.boot.user.can_write.includes(doc.doctype);
		...
		if (frm.is_new() && !doc.sender_email) {
			let { fullname, email } = kanivin.user_info(doc.owner);
	...
	send_emails(frm) {
		kanivin.dom.freeze(__("Queuing emails..."));
		frm.call("send_emails").then(() => {
			frm.refresh();
			kanivin.dom.unfreeze();
			kanivin.show_alert(
				__("Queued {0} emails", [kanivin.utils.shorten_number(frm.doc.total_recipients)])
			);
		});
	},

	schedule_send_dialog(frm) {
		let hours = kanivin.utils.range(24);
	...
	update_schedule_message(frm) {
		if (!frm.doc.email_sent && frm.doc.schedule_send) {
			let datetime = kanivin.datetime.global_date_format(frm.doc.schedule_send);
