kanivin.listview_settings["Email Queue"] = {
...
function show_toggle_sending_button(list_view) {
	...
	const sending_disabled = cint(kanivin.sys_defaults.suspend_email_queue);
	const label = sending_disabled ? __("Resume Sending") : __("Suspend Sending");

	list_view.page.add_inner_button(label, async () => {
		await kanivin.xcall(
			"kanivin.email.doctype.email_queue.email_queue.toggle_sending",
		...
		kanivin.sys_defaults.suspend_email_queue = sending_disabled ? 0 : 1;
...
function add_bulk_retry_button_to_actions(list_view) {
	list_view.page.add_actions_menu_item(__("Retry Sending"), () => {
		kanivin.call({
			method: "kanivin.email.doctype.email_queue.email_queue.bulk_retry",
			args: {
