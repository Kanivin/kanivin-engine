...
import kanivin
from kanivin import _, safe_encode, task
from kanivin.core.utils import html2text
from kanivin.database.database import savepoint
from kanivin.email.doctype.email_account.email_account import EmailAccount
from kanivin.email.email_body import add_attachment, get_email, get_formatted_html
from kanivin.email.queue import get_unsubcribed_url, get_unsubscribe_message
from kanivin.email.smtp import SMTPServer
from kanivin.model.document import Document
from kanivin.query_builder import DocType, Interval
...
from kanivin.utils.verified_command import get_signed_params
...
class EmailQueue(Document):
	...
	if TYPE_CHECKING:
		from kanivin.email.doctype.email_queue_recipient.email_queue_recipient import EmailQueueRecipient
	...
	def prevent_email_queue_delete(self):
		if kanivin.session.user != "Administrator":
			kanivin.throw(_("Only Administrator can delete Email Queue"))

	def get_duplicate(self, recipients):
		values = self.as_dict()
		del values["name"]
		duplicate = kanivin.get_doc(values)
	...
	def new(cls, doc_data, ignore_permissions=False) -> "EmailQueue":
		...
		doc = kanivin.new_doc(cls.DOCTYPE)
		doc.update(data)
	...
	def update_db(self, commit=False, **kwargs):
		kanivin.db.set_value(self.DOCTYPE, self.name, kwargs)
		if commit:
			kanivin.db.commit()
	...
	def update_status(self, status, commit=False, **kwargs):
		...
		if self.communication:
			communication_doc = kanivin.get_doc("Communication", self.communication)
	...
	def get_email_account(self, raise_error=False):
		if self.email_account:
			return kanivin.get_cached_doc("Email Account", self.email_account)
	...
	def can_send_now(self):
		if (
			kanivin.are_emails_muted()
			or not self.is_to_be_sent()
			or cint(kanivin.db.get_default("suspend_email_queue")) == 1
		):
			return False
		...
		with SendMailContext(self, smtp_server_instance) as ctx:
			...
			for recipient in self.recipients:
				...
				if method := get_hook_method("override_email_send"):
					...
					if not kanivin.flags.in_test or kanivin.flags.testing_email:
			...
			if kanivin.flags.in_test and not kanivin.flags.testing_email:
				kanivin.flags.sent_mail = message
				return
	...
	def clear_old_logs(days=30):
		...
		"""
		days = days or 31
		email_queue = kanivin.qb.DocType("Email Queue")
		email_recipient = kanivin.qb.DocType("Email Queue Recipient")

		# Delete queue table
		(
			kanivin.qb.from_(email_queue).delete().where(email_queue.modified < (Now() - Interval(days=days)))
		).run()
		...
		(
			kanivin.qb.from_(email_recipient)
			.delete()
...
class SendMailContext:
	...
	def __exit__(self, exc_type, exc_val, exc_tb):
		if exc_type:
			update_fields = {"error": kanivin.get_traceback()}
	...
	def notify_failed_email(self):
		...
		notification = kanivin.new_doc("Notification Log")
	...
	def get_tracker_str(self, recipient_email) -> str:
		...
		if self.queue_doc.get("email_read_tracker_url"):
			...
			tracker_url = f"{get_url()}/api/method/kanivin.core.doctype.communication.email.mark_email_as_seen?name={self.queue_doc.communication}"
	...
	def include_attachments(self, message):
		...
		for attachment in attachments:
			...
			if file_filters:
				_file = kanivin.get_doc("File", file_filters)
				...
				print_format_file = kanivin.attach_print(**attachment)
	...
	def _store_file(self, file_name, content):
		if not kanivin.get_system_settings("store_attached_pdf_document"):
			return

		file_data = kanivin._dict(file_name=file_name, is_private=1)
		...
		if kanivin.db.exists("File", file_data):
			return

		file = kanivin.new_doc("File", **file_data)
...
def bulk_retry(queues):
	...
	email_queue = kanivin.qb.DocType("Email Queue")
	kanivin.qb.update(email_queue).set(email_queue.status, "Not Sent").set(email_queue.modified, now()).set(
		email_queue.modified_by, kanivin.session.user
...
def toggle_sending(enable):
	kanivin.only_for("System Manager")
	kanivin.db.set_default("suspend_email_queue", 0 if sbool(enable) else 1)


def on_doctype_update():
	"""Add index in `tabCommunication` for `(reference_doctype, reference_name)`"""
	kanivin.db.add_index("Email Queue", ("status", "send_after", "priority", "creation"), "index_bulk_flush")

	kanivin.db.add_index("Email Queue", ["message_id(140)"])


def get_email_retry_limit():
	return cint(kanivin.db.get_system_setting("email_retry_limit")) or 3
...
class QueueBuilder:
	...
	def __init__(
		...
		:param unsubscribe_method: URL method for unsubscribe. Default is `/api/method/kanivin.email.queue.unsubscribe`.
	...
	def unsubscribe_method(self):
		return self._unsubscribe_method or "/api/method/kanivin.email.queue.unsubscribe"
	...
	def get_unsubscribed_user_emails(self):
		...
		if len(all_ids) > 0:
			unsubscribed = (
				kanivin.qb.from_(EmailUnsubscribe)
	...
	def process(self, send_now=False) -> EmailQueue | None:
		...
		if not queue_separately:
			...
			if send_now and len(final_recipients) >= 1000:
				...
				send_now = False
			for recipients in kanivin.utils.create_batch(final_recipients, 1000):
				kanivin.enqueue(
					self.send_emails,
					queue_data=queue_data,
					final_recipients=recipients,
					job_name=kanivin.utils.get_job_name(
						"send_bulk_emails_for", self.reference_doctype, self.reference_name
					),
					now=kanivin.flags.in_test or send_now,
					queue="long",
				)
	...
	def as_dict(self, include_recipients=True):
		...
		except kanivin.InvalidEmailAddressError:
