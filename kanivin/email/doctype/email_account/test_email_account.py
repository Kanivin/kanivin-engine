...
from kanivin.email.doctype.email_account.email_account import notify_unreplied
from kanivin.email.email_body import get_message_id
from kanivin.email.receive import Email, InboundMail, SentEmailInInboxError
from kanivin.tests.utils import KanivinTestCase


class TestEmailAccount(KanivinTestCase):
	@classmethod
	def setUpClass(cls):
		super().setUpClass()
		email_account = kanivin.get_doc("Email Account", "_Test Email Account 1")
	...
	def setUp(self):
		...
		kanivin.flags.sent_mail = None
		kanivin.db.delete("Email Queue")
		kanivin.db.delete("Unhandled Email")
	...
	def test_incoming(self):
		...
		comm = kanivin.get_doc("Communication", {"sender": "test_sender@example.com"})
		self.assertTrue("test_receiver@example.com" in comm.recipients)
		# check if todo is created
		self.assertTrue(kanivin.db.get_value(comm.reference_doctype, comm.reference_name, "name"))

	def test_unread_notification(self):
		todo = kanivin.get_last_doc("ToDo")

		comm = kanivin.new_doc(
			"Communication",
	...
	def test_incoming_with_attach(self):
		...
		existing_file = kanivin.get_doc({"doctype": "File", "file_name": "erpnext-conf-14.png"})
		kanivin.delete_doc("File", existing_file.name)
	...
	def test_outgoing(self):
		...
		mail = email.message_from_string(kanivin.get_last_doc("Email Queue").message)
	...
	def test_sendmail(self):
		...
		sent_mail = email.message_from_string(kanivin.safe_decode(kanivin.flags.sent_mail))
	...
	def test_print_format(self):
		...
		sent_mail = email.message_from_string(kanivin.get_last_doc("Email Queue").message)
	...
	def test_threading(self):
		...
		sent_name = make(
			...
			doctype="ToDo",
			name=kanivin.get_last_doc("ToDo").name,
			send_email=True,
		)["name"]
		...
		sent = kanivin.get_doc("Communication", sent_name)
	...
	def test_threading_by_message_id(self):
		...
		event = kanivin.get_doc(dict(doctype="Event", subject="test-message")).insert()
		...
		last_mail = kanivin.get_doc("Email Queue", dict(reference_name=event.name))
	...
	def test_handle_bad_emails(self):
		...
		self.assertTrue(kanivin.db.get_value("Unhandled Email", {"message_id": message_id}))
	...
	def test_handle_bad_encoding(self):
		...
		self.assertTrue(kanivin.db.get_value("Unhandled Email", {"uid": uid}))
	...
	def test_append_to(self):
		...
		self.assertTrue(kanivin.db.exists(communication.reference_doctype, communication.reference_name))
	...
	def test_append_to_with_imap_folders(self):
		...
		for mail in mails:
			...
			if mail.append_to == "ToDo":
				...
				self.assertTrue(communication.reference_name)
				self.assertTrue(
					kanivin.db.exists(communication.reference_doctype, communication.reference_name)
				)
			else:
				test_folder_mails += 1
	...
	@patch("kanivin.email.receive.EmailServer.select_imap_folder", return_value=True)
	@patch("kanivin.email.receive.EmailServer.logout", side_effect=lambda: None)
	def mocked_get_inbound_mails(
		email_account, messages=None, mocked_logout=None, mocked_select_imap_folder=None
	):
		from kanivin.email.receive import EmailServer
...
class TestInboundMail(KanivinTestCase):
	@classmethod
	def setUpClass(cls):
		super().setUpClass()
		email_account = kanivin.get_doc("Email Account", "_Test Email Account 1")
	...
	def test_find_parent_communication_from_header(self):
		...
		communication = self.new_communication()
		mail_content = self.get_test_mail(fname="reply-4.raw").replace(
			"{{ message_id }}", f"<{communication.name}@{kanivin.local.site}>"
		)
...
def cleanup(sender=None):
	...
	names = kanivin.get_list("Communication", filters=filters, fields=["name"])
	for name in names:
		kanivin.delete_doc_if_exists("Communication", name.name)
		kanivin.delete_doc_if_exists("Communication Link", {"parent": name.name})
