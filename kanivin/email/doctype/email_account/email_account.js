...
function oauth_access(frm) {
	kanivin.model.with_doc("Connected App", frm.doc.connected_app, () => {
		const connected_app = kanivin.get_doc("Connected App", frm.doc.connected_app);
		return kanivin.call({
			doc: connected_app,
...
function set_default_max_attachment_size(frm) {
	if (frm.doc.__islocal && !frm.doc["attachment_limit"]) {
		kanivin.call({
			method: "kanivin.core.api.file.get_max_file_size",
			callback: function (r) {
				if (!r.exc) {
...
kanivin.ui.form.on("Email Account", {
	service: function (frm) {
		$.each(kanivin.email_defaults[frm.doc.service], function (key, value) {
			frm.set_value(key, value);
		});
		if (!frm.doc.use_imap) {
			$.each(kanivin.email_defaults_pop[frm.doc.service], function (key, value) {
				frm.set_value(key, value);
			});
		}
	},
	...
	onload: function (frm) {
		...
		frm.set_query(
			"append_to",
			"kanivin.email.doctype.email_account.email_account.get_append_to"
		);
		frm.set_query("append_to", "imap_folder", function () {
			return {
				query: "kanivin.email.doctype.email_account.email_account.get_append_to",
			};
		});
	...
	refresh: function (frm) {
		...
		if (kanivin.route_flags.delete_user_from_locals && kanivin.route_flags.linked_user) {
			delete kanivin.route_flags.delete_user_from_locals;
			delete locals["User"][kanivin.route_flags.linked_user];
		}
	},
	...
	show_oauth_authorization_message(frm) {
		if (
			...
			kanivin.call({
				method: "kanivin.integrations.doctype.connected_app.connected_app.has_token",
				args: {
	...
	domain: kanivin.utils.debounce((frm) => {
		if (frm.doc.domain) {
			kanivin.call({
	...
	email_sync_option: function (frm) {
		...
		if (frm.doc.email_sync_option == "ALL") {
			...
			kanivin.confirm(msg, null, function () {
	...
	warn_autoreply_on_incoming: function (frm) {
		if (frm.doc.enable_incoming && frm.doc.enable_auto_reply && frm.doc.__islocal) {
			...
			kanivin.confirm(msg, null, function () {
				...
				kanivin.show_alert({ message: __("Disabled Auto Reply"), indicator: "blue" });
			});
		}
	},
});
