...
import kanivin
from kanivin import _, are_emails_muted, safe_encode
from kanivin.desk.form import assign_to
from kanivin.email.doctype.email_domain.email_domain import EMAIL_DOMAIN_FIELDS
from kanivin.email.receive import EmailServer, InboundMail, SentEmailInInboxError
from kanivin.email.smtp import SMTPServer
from kanivin.email.utils import get_port
from kanivin.model.document import Document
from kanivin.utils import cint, comma_or, cstr, parse_addr, validate_email_address
from kanivin.utils.background_jobs import enqueue, get_jobs
from kanivin.utils.jinja import render_template
...
def cache_email_account(cache_name):
	def decorator_cache_email_account(func):
		...
		def wrapper_cache_email_account(*args, **kwargs):
			if not hasattr(kanivin.local, cache_name):
				setattr(kanivin.local, cache_name, {})

			cached_accounts = getattr(kanivin.local, cache_name)
...
class EmailAccount(Document):
	...
	if TYPE_CHECKING:
		from kanivin.email.doctype.imap_folder.imap_folder import IMAPFolder
	...
	def validate(self):
		...
		if self.login_id_is_different:
			if not self.login_id:
				kanivin.throw(_("Login Id is required"))
		else:
			self.login_id = None
		...
		if self.enable_incoming and self.use_imap and len(self.imap_folder) <= 0:
			kanivin.throw(_("You need to set one IMAP folder for {0}").format(kanivin.bold(self.email_id)))

		if kanivin.local.flags.in_patch or kanivin.local.flags.in_test:
			return
		...
		if not kanivin.local.flags.in_install and not self.awaiting_password:
			if validate_oauth or self.password or self.smtp_server in ("127.0.0.1", "localhost"):
				...
				if self.enable_incoming or (self.enable_outgoing and not self.no_smtp_authentication):
					if not use_oauth:
						kanivin.throw(_("Password is required or select Awaiting Password"))

		if self.notify_if_unreplied:
			if not self.send_notification_to:
				kanivin.throw(_("{0} is mandatory").format(self.meta.get_label("send_notification_to")))
			...
			for folder in self.imap_folder:
				if folder.append_to:
					...
					if folder.append_to not in valid_doctypes:
						kanivin.throw(_("Append To can be one of {0}").format(comma_or(valid_doctypes)))

	def validate_smtp_conn(self):
		if not self.smtp_server:
			kanivin.throw(_("SMTP Server is required"))
	...
	def before_save(self):
		...
		if not self.enable_incoming and self.default_incoming:
			...
			messages.append(
				_("{} has been disabled. It can only be enabled if {} is checked.").format(
					kanivin.bold(_("Default Incoming")),
					kanivin.bold(_("Enable Incoming")),
				)
			)
		if not self.enable_outgoing and self.default_outgoing:
			...
			messages.append(
				_("{} has been disabled. It can only be enabled if {} is checked.").format(
					kanivin.bold(_("Default Outgoing")),
					kanivin.bold(_("Enable Outgoing")),
				)
			)
		if messages:
			if len(messages) == 1:
	...
	def there_must_be_only_one_default(self):
		...
		for field in ("default_incoming", "default_outgoing"):
			...
			for email_account in kanivin.get_all("Email Account", filters={field: 1}):
				if email_account.name == self.name:
					continue

				email_account = kanivin.get_doc("Email Account", email_account.name)
				email_account.set(field, 0)
				email_account.save()

	@kanivin.whitelist()
	def get_domain_values(self, domain: str):
		return kanivin.db.get_value("Email Domain", domain, EMAIL_DOMAIN_FIELDS, as_dict=True)
	...
	def get_incoming_server(self, in_receive=False, email_sync_rule="UNSEEN"):
		...
		if not args.get("host"):
			kanivin.throw(_("{0} is required").format("Email Server"))

		email_server = EmailServer(kanivin._dict(args))
	...
	def check_email_server_connection(self, email_server, in_receive):
		...
		try:
			...
			if in_receive and any(map(lambda t: t in message, all_error_codes)):
				...
				kanivin.throw(cstr(e))

		except OSError:
			if in_receive:
				# timeout while connecting, see receive.py connect method
				description = kanivin.message_log.pop() if kanivin.message_log else "Socket Error"
	...
	def _password(self):
		raise_exception = not (
			self.auth_method == "OAuth" or self.no_smtp_authentication or kanivin.flags.in_test
		)
	...
	def from_record(cls, record):
		email_account = kanivin.new_doc(cls.DOCTYPE)
		email_account.update(record)
		return email_account

	@classmethod
	def find(cls, name):
		return kanivin.get_doc(cls.DOCTYPE, name)

	@classmethod
	def find_one_by_filters(cls, **kwargs) -> "EmailAccount":
		name = kanivin.db.get_value(cls.DOCTYPE, kwargs)
	...
	def get_account_details_from_site_config(cls):
		if not kanivin.conf.get("mail_server"):
			return {}
		...
		field_to_conf_name_map = {
			...
			"name": {"conf_names": ("email_sender_name",), "default": "Kanivin"},
		...
		for doc_field_name, d in field_to_conf_name_map.items():
			...
			value = [kanivin.conf.get(k) for k in conf_names if kanivin.conf.get(k)]
	...
	def handle_incoming_connect_error(self, description):
		if self.get_failed_attempts_count() > 5:
			...
			kanivin.enqueue(self._disable_broken_incoming_account, description=description)
		else:
			self.set_failed_attempts_count(self.get_failed_attempts_count() + 1)

	def _disable_broken_incoming_account(self, description):
		if kanivin.flags.in_test:
			return
	...
	def set_failed_attempts_count(self, value):
		kanivin.cache.set_value(f"{self.name}:email-account-failed-attempts", value)

	def get_failed_attempts_count(self):
		return cint(kanivin.cache.get_value(f"{self.name}:email-account-failed-attempts"))
	...
	def receive(self):
		...
		for mail in inbound_mails:
			try:
				...
				try:
					...
					if self.use_imap:
						self.handle_bad_emails(mail.uid, mail.raw_message, kanivin.get_traceback())
					exceptions.append(kanivin.get_traceback())
				except Exception:
					kanivin.db.rollback()
				else:
					kanivin.db.commit()
		...
		if exceptions:
			raise Exception(kanivin.as_json(exceptions))
	...
	def handle_bad_emails(self, uid, raw, reason):
		...
		if cint(self.use_imap):
			...
			unhandled_email = kanivin.get_doc(
				{
					"raw": raw_str,
					"uid": uid,
					"reason": reason,
	...
	def send_auto_reply(self, communication, email):
		...
		from kanivin.core.doctype.communication.email import (
		...
		if self.enable_auto_reply:
			...
			kanivin.sendmail(
				...
				or kanivin.get_template("templates/emails/auto_reply.html").render(communication.as_dict()),
	...
	def on_trash(self):
		...
		Communication = kanivin.qb.DocType("Communication")
		kanivin.qb.update(Communication).set(Communication.email_account, "").where(
	...
	def after_rename(self, old, new, merge=False):
		kanivin.db.set_value("Email Account", new, "email_account_name", new)
	...
	def check_automatic_linking_email_account(self):
		if self.enable_automatic_linking:
			if not self.enable_incoming:
				kanivin.throw(_("Automatic Linking can be activated only if Incoming is enabled."))

			if kanivin.db.exists(
				"Email Account",
				{"enable_automatic_linking": 1, "name": ("!=", self.name)},
			):
				kanivin.throw(_("Automatic Linking can be activated only for one Email Account."))
	...
	def get_oauth_token(self):
		if self.auth_method == "OAuth":
			connected_app = kanivin.get_doc("Connected App", self.connected_app)
...
def get_append_to(doctype=None, txt=None, searchfield=None, start=None, page_len=None, filters=None):
	...
	email_append_to_list = [
		dt.name for dt in kanivin.get_all("DocType", filters=filters, fields=["name", "email_append_to"])
	]
	# Set Email Append To DocTypes set via Customize Form
	email_append_to_list.extend(
		dt.doc_type
		for dt in kanivin.get_list(
			"Property Setter",
...
def notify_unreplied():
	...
	for email_account in kanivin.get_all(
		"Email Account",
		"name",
		...
		if append_to:
			...
			for comm in kanivin.get_all(
				...
				comm = kanivin.get_doc("Communication", comm.name)

				if kanivin.db.get_value(comm.reference_doctype, comm.reference_name, "status") == "Open":
...
def pull(now=False):
	...
	from kanivin.integrations.doctype.connected_app.connected_app import has_token

	doctype = kanivin.qb.DocType("Email Account")
	email_accounts = (
	...
	for email_account in email_accounts:
		...
		if now:
			...
			queued_jobs = get_jobs(site=kanivin.local.site, key="job_name")[kanivin.local.site]
...
def pull_from_email_account(email_account):
	...
	email_account = kanivin.get_doc("Email Account", email_account)
...
def setup_user_email_inbox(email_account, awaiting_password, email_id, enable_outgoing, used_oauth):
	...
	from kanivin.core.doctype.user.user import ask_pass_update
	...
	user_names = kanivin.db.get_values("User", {"email": email_id}, as_dict=True)
	if not user_names:
		return
	...
	if update_user_email_settings:
		UserEmail = kanivin.qb.DocType("User Email")
		kanivin.qb.update(UserEmail).set(UserEmail.awaiting_password, (awaiting_password or 0)).set(
		...
		users = " and ".join([kanivin.bold(user.get("name")) for user in user_names])
		kanivin.msgprint(_("Enabled email inbox for user {0}").format(users))
	ask_pass_update()
...
def remove_user_email_inbox(email_account):
	...
	for user in users:
		doc = kanivin.get_doc("User", user.get("name"))
...
def set_email_password(email_account, password):
	account = kanivin.get_doc("Email Account", email_account)
