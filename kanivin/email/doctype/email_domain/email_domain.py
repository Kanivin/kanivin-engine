...
class EmailDomain(Document):
	...
	def validate(self):
		...
		if kanivin.local.flags.in_patch or kanivin.local.flags.in_test or kanivin.local.flags.in_install:
			return
	...
	def on_update(self):
		...
		for email_account in kanivin.get_all("Email Account", filters={"domain": self.name}):
			try:
