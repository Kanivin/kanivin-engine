...
kanivin.ui.form.on("Auto Email Report", {
	refresh: function (frm) {
		...
		if (!frm.is_new()) {
			frm.add_custom_button(__("Download"), function () {
				var w = window.open(
					kanivin.urllib.get_full_url(
						"/api/method/kanivin.email.doctype.auto_email_report.auto_email_report.download?" +
							"name=" +
			...
			frm.add_custom_button(__("Send Now"), function () {
				kanivin.call({
					method: "kanivin.email.doctype.auto_email_report.auto_email_report.send_now",
					args: { name: frm.doc.name },
					callback: function () {
						kanivin.msgprint(__("Scheduled to send"));
					},
				});
			});
		} else {
			if (!frm.doc.user) {
				frm.set_value("user", kanivin.session.user);
			}
			if (!frm.doc.email_to) {
				frm.set_value("email_to", kanivin.session.user);
			}
		}
	...
	fetch_report_filters(frm) {
		if (
			...
			kanivin.call({
				method: "kanivin.desk.query_report.get_script",
				args: {
					report_name: frm.doc.report,
				},
				callback: function (r) {
					kanivin.dom.eval(r.message.script || "");
					frm.script_setup_for = frm.doc.report;
	...
	show_filters: async function (frm) {
		...
		wrapper.empty();
		let reference_report = kanivin.query_reports[frm.doc.report];
		if (!reference_report || !reference_report.filters) {
			reference_report = await kanivin.model.with_doc("Report", frm.doc.report);
		}
		if (
			...
			if (
				...
				if (frm.doc.filters) {
					...
					kanivin.db.get_value("Report", frm.doc.report, "json", (r) => {
						if (r && r.json) {
				...
				report_filters = kanivin.query_reports[frm.doc.reference_report].filters;
			} else {
				filters = JSON.parse(frm.doc.filters || "{}");
				...
				if (val.fieldtype != "Break") {
					if (val.fieldtype === "MultiSelectList") {
						val.get_data = (txt) => {
							...
							const doctype_link =
								kanivin.scrub(val.options) === val.options
									? dialog.get_value(val.options)
									: val.options;

							return doctype_link
								? kanivin.db.get_link_options(doctype_link, txt)
								: [];
						};
					}
			...
			report_filters.forEach((f) => {
				...
				$("<td>" + kanivin.format(filters[f.fieldname], f) + "</td>")
					.css(css)
					.appendTo(row);
			});
