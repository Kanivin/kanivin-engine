...
from kanivin.utils import add_to_date, get_link_to_form, today
from kanivin.utils.data import is_html

# test_records = kanivin.get_test_records('Auto Email Report')


class TestAutoEmailReport(KanivinTestCase):
	def test_auto_email(self):
		kanivin.delete_doc("Auto Email Report", "Permitted Documents For User")
...
def get_auto_email_report():
	if not kanivin.db.exists("Auto Email Report", "Permitted Documents For User"):
		auto_email_report = kanivin.get_doc(
			dict(
		...
		auto_email_report = kanivin.get_doc("Auto Email Report", "Permitted Documents For User")
