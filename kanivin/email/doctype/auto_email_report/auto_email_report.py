...
from kanivin.desk.query_report import build_xlsx_data
...
from kanivin.utils.csvutils import to_csv
...
class AutoEmailReport(Document):
	...
	def autoname(self):
		...
		if kanivin.db.exists("Auto Email Report", self.name):
	...
	def sender_email(self):
		return kanivin.db.get_value("Email Account", self.sender, "email_id")
	...
	def validate_report_count(self):
		count = kanivin.db.count("Auto Email Report", {"user": self.user, "enabled": 1})

		max_reports_per_user = (
			cint(kanivin.local.conf.max_reports_per_user)  # kept for backward compatibilty
			or cint(kanivin.db.get_single_value("System Settings", "max_auto_email_report_per_user"))
			or 20
		)
		...
		if count > max_reports_per_user + (-1 if self.flags.in_insert else 0):
			...
			kanivin.throw(msg, title=_("Report limit reached"))
		...
		if self.format not in valid_report_formats:
			kanivin.throw(
				_("{0} is not a valid report format. Report format should one of the following {1}").format(
					kanivin.bold(self.format), kanivin.bold(", ".join(valid_report_formats))
				)
			)

	def validate_mandatory_fields(self):
		# Check if all Mandatory Report Filters are filled by the User
		filters = kanivin.parse_json(self.filters) if self.filters else {}
		filter_meta = kanivin.parse_json(self.filter_meta) if self.filter_meta else {}
		throw_list = [
	...
	def get_report_content(self):
		...
		report = kanivin.get_doc("Report", self.report)

		self.filters = kanivin.parse_json(self.filters) if self.filters else {}
		...
		# add serial numbers
		columns.insert(0, kanivin._dict(fieldname="idx", label="", width="30px"))
		...
		if self.format == "HTML":
			...
			kanivin.throw(_("Invalid Output Format"))

	def get_html_table(self, columns=None, data=None):
		date_time = global_date_format(now()) + " " + format_time(now())
		report_doctype = kanivin.db.get_value("Report", self.report, "ref_doctype")

		return kanivin.render_template(
			"kanivin/templates/emails/auto_email_report.html",
			{
				"title": self.name,
	...
	def prepare_dynamic_filters(self):
		self.filters = kanivin.parse_json(self.filters)
	...
	def send(self):
		if self.filter_meta and not self.filters:
			kanivin.throw(_("Please set filters value in Report Filter table."))
...
def download(name):
	...
	auto_email_report = kanivin.get_doc("Auto Email Report", name)
	auto_email_report.check_permission()
	data = auto_email_report.get_report_content()

	if not data:
		kanivin.msgprint(_("No Data"))
		return

	kanivin.local.response.filecontent = data
	kanivin.local.response.type = "download"
	kanivin.local.response.filename = auto_email_report.get_file_name()
...
def send_daily():
	...
	enabled_reports = kanivin.get_all(
		"Auto Email Report", filters={"enabled": 1, "frequency": ("in", ("Daily", "Weekdays", "Weekly"))}
	)

	for report in enabled_reports:
		kanivin.enqueue(
			"kanivin.email.doctype.auto_email_report.auto_email_report.process_auto_email_report",
			report=report,
			queue="long",
		)
...
def process_auto_email_report(report):
	...
	auto_email_report = kanivin.get_doc("Auto Email Report", report.name)
...
def send_monthly():
	...
	for report in kanivin.get_all("Auto Email Report", {"enabled": 1, "frequency": "Monthly"}):
		kanivin.get_doc("Auto Email Report", report.name).send()
	...
	for row in data:
		...
		for col in columns:
			...
			if col.fieldtype == "Link":
				...
				doc = None
				if doc_name and col.get("parent") and not kanivin.get_meta(col.parent).istable:
					if kanivin.db.exists(col.parent, doc_name):
						doc = kanivin.get_doc(col.parent, doc_name)

				# Pass the Document to get the currency based on docfield option
				row[col.fieldname] = kanivin.format_value(row[col.fieldname], col, doc=doc)
	return columns, data
