...
kanivin.notification = {
	setup_fieldname_select: function (frm) {
		...
		kanivin.model.with_doctype(frm.doc.document_type, function () {
			...
			let fields = kanivin.get_doc("DocType", frm.doc.document_type).fields;
			let options = $.map(fields, function (d) {
				return kanivin.model.no_value_type.includes(d.fieldtype)
					? null
					: get_select_options(d);
			});
			...
			if (frm.doc.channel === "Email") {
				receiver_fields = $.map(fields, function (d) {
					...
					if (kanivin.model.table_fields.includes(d.fieldtype)) {
						let child_fields = kanivin.get_doc("DocType", d.options).fields;
...
kanivin.ui.form.on("Notification", {
	onload: function (frm) {
	...
	refresh: function (frm) {
		kanivin.notification.setup_fieldname_select(frm);
		kanivin.notification.setup_example_message(frm);
		...
		frm.get_field("is_standard").toggle(kanivin.boot.developer_mode);
		frm.trigger("event");
	},
	...
	view_properties: function (frm) {
		kanivin.route_options = { doc_type: frm.doc.document_type };
	...
	event: function (frm) {
		...
		frm.add_custom_button(__("Get Alerts for Today"), function () {
			kanivin.call({
				method: "kanivin.email.doctype.notification.notification.get_documents_for_today",
				args: {
					notification: frm.doc.name,
				},
				callback: function (r) {
					if (r.message && r.message.length > 0) {
						kanivin.msgprint(r.message.toString());
					} else {
						kanivin.msgprint(__("No alerts for today"));
					}
				},
			});
		});
