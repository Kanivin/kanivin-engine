...
def get_test_notification(config):
	try:
		notification = kanivin.get_doc(doctype="Notification", **config).insert()
		yield notification
	finally:
		notification.delete()


class TestNotification(KanivinTestCase):
	def setUp(self):
		kanivin.db.delete("Email Queue")
		kanivin.set_user("test@example.com")

		if not kanivin.db.exists("Notification", {"name": "ToDo Status Update"}, "name"):
			notification = kanivin.new_doc("Notification")
		...
		if not kanivin.db.exists("Notification", {"name": "Contact Status Update"}, "name"):
			notification = kanivin.new_doc("Notification")
	...
	def test_new_and_save(self):
		...
		self.assertEqual(kanivin.db.get_value("Communication", communication.name, "subject"), "__testing__")

	def test_condition(self):
		"""Check notification is triggered based on a condition."""
		event = kanivin.new_doc("Event")
		event.subject = "test"
	...
	def test_invalid_condition(self):
		...
		recipent = kanivin.new_doc("Notification Recipient")
		recipent.receiver_by_document_field = "owner"

		notification.recipents = recipent
		notification.condition = "test"

		self.assertRaises(kanivin.ValidationError, notification.save)
		notification.delete()
	...
	def test_date_changed(self):
		...
		kanivin.get_doc(
			"Scheduled Job Type",
			dict(method="kanivin.email.doctype.notification.notification.trigger_daily_alerts"),
		).execute()
		...
		event.starts_on = kanivin.utils.add_days(kanivin.utils.nowdate(), 2) + " 12:00:00"
		event.save()
	...
	def test_cc_jinja(self):
		kanivin.db.delete("User", {"email": "test_jinja@example.com"})
		kanivin.db.delete("Email Queue")
		kanivin.db.delete("Email Queue Recipient")

		test_user = kanivin.new_doc("User")
		...
		self.assertTrue(kanivin.db.get_value("Email Queue Recipient", {"recipient": "test_jinja@example.com"}))
	...
	def test_notification_by_child_table_field(self):
		contact = kanivin.new_doc("Contact")
	...
	def test_notification_value_change_casted_types(self):
		...
		with get_test_notification(notification) as n:
			kanivin.db.delete("Notification Log", {"subject": n.subject})

			user = kanivin.get_doc("User", "test@example.com")
			user.birth_date = kanivin.utils.add_days(user.birth_date, 1)
			user.save()

			user.reload()
			user.birth_date = kanivin.utils.getdate(user.birth_date)
			user.save()
			self.assertEqual(1, kanivin.db.count("Notification Log", {"subject": n.subject}))

	@classmethod
	def tearDownClass(cls):
		kanivin.delete_doc_if_exists("Notification", "ToDo Status Update")
		kanivin.delete_doc_if_exists("Notification", "Contact Status Update")
