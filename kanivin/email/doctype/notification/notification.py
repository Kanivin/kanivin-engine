...
from kanivin.core.doctype.role.role import get_info_based_on_role, get_user_info
from kanivin.core.doctype.sms_settings.sms_settings import send_sms
from kanivin.desk.doctype.notification_log.notification_log import enqueue_create_notification
from kanivin.integrations.doctype.slack_webhook_url.slack_webhook_url import send_slack_message
from kanivin.model.document import Document
from kanivin.modules.utils import export_module_json, get_doc_module
from kanivin.utils import add_to_date, cast, nowdate, validate_email_address
...
class Notification(Document):
	...
	if TYPE_CHECKING:
		from kanivin.email.doctype.notification_recipient.notification_recipient import NotificationRecipient
	...
	def validate(self):
		...
		if self.event in ("Days Before", "Days After") and not self.date_changed:
			kanivin.throw(_("Please specify which date field must be checked"))

		if self.event == "Value Change" and not self.value_changed:
			kanivin.throw(_("Please specify which value field must be checked"))

		self.validate_forbidden_document_types()
		self.validate_condition()
		self.validate_standard()
		kanivin.cache.hdel("notifications", self.document_type)
...
"""
	...
	def validate_standard(self):
		if self.is_standard and self.enabled and not kanivin.conf.developer_mode:
			kanivin.throw(
				_("Cannot edit Standard Notification. To edit, please disable this and duplicate it")
			)

	def validate_condition(self):
		temp_doc = kanivin.new_doc(self.document_type)
		if self.condition:
			try:
				kanivin.safe_eval(self.condition, None, get_context(temp_doc.as_dict()))
			except Exception:
				kanivin.throw(_("The Condition '{0}' is invalid").format(self.condition))

	def validate_forbidden_document_types(self):
		if self.document_type in FORBIDDEN_DOCUMENT_TYPES or (
			kanivin.get_meta(self.document_type).istable and self.event not in DATE_BASED_EVENTS
		):
	...
	def get_documents_for_today(self):
		...
		for d in doc_list:
			doc = kanivin.get_doc(self.document_type, d.name)

			if self.condition and not kanivin.safe_eval(self.condition, None, get_context(doc)):
				continue
		...
		if self.set_property_after_alert:
			...
			try:
				if allow_update and not doc.flags.in_notification_update:
					...
					if doc.meta.get_field(fieldname).fieldtype in kanivin.model.numeric_fieldtypes:
						value = kanivin.utils.cint(value)
	...
	def create_system_notification(self, doc, context):
		...
		if "{" in subject:
			subject = kanivin.render_template(self.subject, context)
		...
		notification_doc = {
			...
			"email_content": kanivin.render_template(self.message, context),
	...
	def send_an_email(self, doc, context):
		...
		from kanivin.core.doctype.communication.email import _make as make_communication
		...
		sender = None
		message = kanivin.render_template(self.message, context)
	...
	def send_sms(self, doc, context):
		send_sms(
			...
			msg=kanivin.render_template(self.message, context),
		)
	...
	def get_list_of_recipients(self, doc, context):
		...
		for recipient in self.recipients:
			if recipient.condition:
				if not kanivin.safe_eval(recipient.condition, None, context):
					continue
	...
	def get_attachment(self, doc):
		...
		print_settings = kanivin.get_doc("Print Settings", "Print Settings")
			...
			return [
				{
					...
					"lang": kanivin.db.get_value("Print Format", self.print_format, "default_print_language")
	...
	def get_template(self, md_as_html=False):
		...
		path = os.path.join(os.path.dirname(module.__file__), kanivin.scrub(self.name))
		extension = FORMATS.get(self.message_type, ".md")
		file_path = path + extension
		...
		if extension == ".md":
			return kanivin.utils.md_to_html(template)
...
def get_documents_for_today(notification):
	notification = kanivin.get_doc("Notification", notification)
...
def trigger_notifications(doc, method=None):
	if kanivin.flags.in_import or kanivin.flags.in_patch:
	...
	if method == "daily":
		...
		for d in doc_list:
			alert = kanivin.get_doc("Notification", d.name)
...
def evaluate_alert(doc: Document, alert, event):
	...
	try:
		if isinstance(alert, str):
			alert = kanivin.get_doc("Notification", alert)

		context = get_context(doc)

		if alert.condition:
			if not kanivin.safe_eval(alert.condition, None, context):
				return

		if event == "Value Change" and not doc.is_new():
			if not kanivin.db.has_column(doc.doctype, alert.value_changed):
		...
		message = _("Error while evaluating Notification {0}. Please fix your template.").format(
			kanivin.utils.get_link_to_form("Notification", alert.name)
		)
		kanivin.throw(message, title=_("Error in Notification"))
	except Exception as e:
		title = str(e)
		message = kanivin.get_traceback(with_context=True)
		kanivin.log_error(title=title, message=message)
		msg = f"<details><summary>{title}</summary>{message}</details>"
		kanivin.throw(msg, title=_("Error in Notification"))


def get_context(doc):
	Kanivin = namedtuple("kanivin", ["utils"])
	return {
		"doc": doc,
		"nowdate": nowdate,
		"kanivin": Kanivin(utils=get_safe_globals().get("kanivin").get("utils")),
	}


def get_assignees(doc):
	assignees = []
	assignees = kanivin.get_all(
		"ToDo",
...
def get_emails_from_template(template, context):
	...
	emails = kanivin.render_template(template, context) if "{" in template else template
