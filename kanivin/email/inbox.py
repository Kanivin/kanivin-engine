...
import kanivin
from kanivin.client import set_value
...
def create_email_flag_queue(names, action):
	...
	def mark_as_seen_unseen(name, action):
		doc = kanivin.get_doc("Communication", name)
		if action == "Read":
			doc.add_seen()
		else:
			_seen = json.loads(doc._seen or "[]")
			_seen = [user for user in _seen if kanivin.session.user != user]
	...
	for name in json.loads(names or []):
		uid, seen_status, email_account = kanivin.db.get_value(
		...
		if (action == "Read" and seen_status == 0) or (action == "Unread" and seen_status == 1):
			create_new = True
			email_flag_queue = kanivin.db.sql(
			...
			for queue in email_flag_queue:
				if queue.action != action:
					kanivin.delete_doc("Email Flag Queue", queue.name, ignore_permissions=True)
				elif queue.action == action:
					# Read or Unread request for email is already available
					create_new = False

			if create_new:
				flag_queue = kanivin.get_doc(
					{
						"uid": uid,
						"action": action,
						"communication": name,
				...
				kanivin.db.set_value("Communication", name, "seen", seen, update_modified=False)
...
def mark_as_spam(communication: str, sender: str):
	...
	email_rule = kanivin.db.get_value("Email Rule", {"email_id": sender})
	if not email_rule:
		kanivin.get_doc({"doctype": "Email Rule", "email_id": sender, "is_spam": 1}).insert(
