...
class EMail:
	...
	def attach_file(self, n):
		...
		_file = kanivin.get_doc("File", {"file_name": n})
	...
	def validate(self):
		...
		self.recipients = [strip(r) for r in self.recipients if r not in kanivin.STANDARD_USERS]
		self.cc = [strip(r) for r in self.cc if r not in kanivin.STANDARD_USERS]
		self.bcc = [strip(r) for r in self.bcc if r not in kanivin.STANDARD_USERS]
	...
	def make(self):
		...
		for hook in kanivin.get_hooks("make_email_body_message"):
			kanivin.get_attr(hook)(self)
...
def get_formatted_html(
	...
	email_account=None,
	header=None,
	unsubscribe_link: kanivin._dict | None = None,
	sender=None,
	with_container=False,
):
	email_account = email_account or EmailAccount.find_outgoing(match_by_email=sender)

	rendered_email = kanivin.get_template("templates/emails/standard.html").render(
		{
...
def get_email_html(template, args, subject, header=None, with_container=False):
	...
	email = kanivin.utils.jinja.get_email_from_template(template, args)
...
def inline_style_in_html(html):
	...
	css_files = kanivin.get_hooks("email_css")
...
def get_message_id():
	...
	return email.utils.make_msgid(domain=kanivin.local.site)
...
def get_footer(email_account, footer=None):
	...
	sender_address = kanivin.db.get_default("email_footer_address")

	if sender_address:
		args.update({"sender_address": sender_address})

	if not cint(kanivin.db.get_default("disable_standard_email_footer")):
		args.update({"default_mail_footer": kanivin.get_hooks("default_mail_footer")})

	footer += kanivin.utils.jinja.get_email_from_template("email_footer", args)[0]

	return footer


def replace_filename_with_cid(message):
	"""Replaces <img embed="assets/kanivin/images/filename.jpg" ...> with
	...
	while True:
		...
		# found match
		img_path = groups[0]
		img_path_escaped = kanivin.utils.html_utils.unescape_html(img_path)
...
def get_filecontent_from_path(path):
	...
	if path.startswith("assets/"):
		...
		# public file
		full_path = kanivin.get_site_path("public", path)
	elif path.startswith("private/files/"):
		# private file
		full_path = kanivin.get_site_path(path)
	else:
		full_path = path
...
def get_header(header=None):
	...
	from kanivin.utils.jinja import get_email_from_template
	...
	if not title:
		title = kanivin.get_hooks("app_title")[-1]
