...
import kanivin
from kanivin import _, safe_decode, safe_encode
from kanivin.core.doctype.file.exceptions import MaxFileSizeReachedError
from kanivin.core.doctype.file.utils import get_random_filename
from kanivin.email.oauth import Oauth
...
from kanivin.utils.user import is_system_user
...
class EmailSizeExceededError(kanivin.ValidationError):
	pass


class LoginLimitExceeded(kanivin.ValidationError):
	pass
...
class EmailServer:
	...
	def __init__(self, args=None):
		self.settings = args or kanivin._dict()
		...
		try:
			if cint(self.settings.use_ssl):
				self.imap = imaplib.IMAP4_SSL(
					...
					timeout=kanivin.conf.pop_timeout,
					ssl_context=ssl.create_default_context(),
				)
			else:
				self.imap = imaplib.IMAP4(
					self.settings.host, self.settings.incoming_port, timeout=kanivin.conf.pop_timeout
				)
			...
			kanivin.msgprint(_("Invalid Mail Server. Please rectify and try again."))
			raise
	...
	def connect_pop(self):
		...
		try:
			...
			kanivin.log_error("POP: Unable to connect")
			...
			if self.is_temporary_system_problem(e):
				...
				kanivin.msgprint(_("Invalid User Name or Support Password. Please rectify and try again."))
				raise
	...
	def check_imap_uidvalidity(self, folder):
		...
		kanivin.db.set_value("Email Account", self.settings.email_account, "uidnext", uidnext)
		...
		if not uid_validity or uid_validity != current_uid_validity:
			...
			kanivin.qb.update(Communication).set(Communication.uid, -1).where(
			...
			if self.settings.use_imap:
				...
				IMAPFolder = kanivin.qb.DocType("IMAP Folder")
				kanivin.qb.update(IMAPFolder).set(IMAPFolder.uidvalidity, current_uid_validity).set(
					IMAPFolder.uidnext, uidnext
				).where(IMAPFolder.parent == self.settings.email_account_name).where(
					IMAPFolder.folder_name == folder
				).run()
			else:
				EmailAccount = kanivin.qb.DocType("Email Account")
				kanivin.qb.update(EmailAccount).set(EmailAccount.uidvalidity, current_uid_validity).set(
	...
	def retrieve_message(self, uid, msg_num):
		try:
			...
			kanivin.log_error("Unable to fetch email", self.make_error_msg(uid, msg_num))
	...
	def get_email_seen_status(self, uid, flag_string):
		...
		for flag in imaplib.ParseFlags(flag_string) or []:
			match = WORDS_PATTERN.search(kanivin.as_unicode(flag))
...
class Email:
	...
	def set_from(self):
		...
		if _reply_to and not kanivin.db.get_value("Email Account", {"email_id": _reply_to}, "email_id"):
	...
	def decode_email(email):
		if not email:
			return
		decoded = ""
		for part, encoding in decode_header(kanivin.as_unicode(email).replace('"', " ").replace("'", " ")):
			if encoding:
	...
	def save_attachments_in_doc(self, doc):
		...
		for attachment in self.attachments:
			try:
				...
				pass
			except kanivin.FileAlreadyAttachedException:
				pass
...
class InboundMail(Email):
	...
	def _build_communication_doc(self):
		...
		communication = kanivin.get_doc(data)
	...
	def is_exist_in_system(self):
		...
		from kanivin.core.doctype.communication.communication import Communication
	...
	def is_reply_to_system_sent_mail(self):
		...
		return self.is_reply() and kanivin.local.site in self.in_reply_to
	...
	def match_record_by_subject_and_sender(self, doctype):
		...
		3. Find by matching subject alone (Special case)
		        ...
		        the system user is replying to via the common email account in Kanivin. This fix bypasses
		...
		if not record:
			...
			name = kanivin.db.get_value(self.email_account.append_to, filters=filters)
			record = self.get_doc(doctype, name, ignore_error=True) if name else None
		return record

	def _create_reference_document(self, doctype):
		"""Create reference document if it does not exist in the system."""
		parent = kanivin.new_doc(doctype)
		email_fields = self.get_email_fields(doctype)

		if email_fields.subject_field:
			parent.set(email_fields.subject_field, kanivin.as_unicode(self.subject)[:140])

		if email_fields.sender_field:
			parent.set(email_fields.sender_field, kanivin.as_unicode(self.from_email))

		if email_fields.sender_name_field:
			parent.set(email_fields.sender_name_field, kanivin.as_unicode(self.from_real_name))
		...
		try:
			...
			return kanivin.db.get_value(doctype, {email_fields.sender_field: self.from_email})

	@staticmethod
	def get_doc(doctype, docname, ignore_error=False):
		try:
			return kanivin.get_doc(doctype, docname)
	...
	def get_users_linked_to_account(email_account):
		...
		users = kanivin.get_all("User Email", filters={"email_account": email_account.name}, fields=["parent"])
	...
	def clean_subject(subject):
		...
		return kanivin.as_unicode(strip(re.sub(regex, "", subject, count=0, flags=re.IGNORECASE)))

	@staticmethod
	def get_email_fields(doctype):
		"""Returns Email related fields of a doctype."""
		fields = kanivin._dict()
	...
	def get_document(self, doctype, name):
		"""Is same as kanivin.get_doc but suppresses the DoesNotExist error."""
		try:
			return kanivin.get_doc(doctype, name)
		except kanivin.DoesNotExistError:
			return None
