...
from kanivin.utils import cint, cstr, get_traceback


class InvalidEmailCredentials(kanivin.ValidationError):
	pass
...
class SMTPServer:
	def __init__(
		...
		if not self.server:
			kanivin.msgprint(
				...
				raise_exception=kanivin.OutgoingEmailError,
			)
	...
	def session(self):
		...
		try:
			...
			if not _session:
				kanivin.msgprint(
					_("Could not connect to outgoing email server"), raise_exception=kanivin.OutgoingEmailError
				)
			...
			if self.use_oauth:
				...
				if res[0] != 235:
					kanivin.msgprint(res[1], raise_exception=kanivin.OutgoingEmailError)
	...
	def _enqueue_connection_closure(self):
		if kanivin.request and hasattr(kanivin.request, "after_response"):
			kanivin.request.after_response.add(self.quit)
		elif kanivin.job:
			kanivin.job.after_job.add(self.quit)
		elif not kanivin.flags.in_test:
			# Console?
			import atexit
