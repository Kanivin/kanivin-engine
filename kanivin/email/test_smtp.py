...
class TestSMTP(KanivinTestCase):
	...
	def test_get_email_account(self):
		existing_email_accounts = kanivin.get_all(
		...
		for email_account in existing_email_accounts:
			kanivin.db.set_value("Email Account", email_account["name"], unset_details)

		# remove mail_server config so that test@example.com is not created
		mail_server = kanivin.conf.get("mail_server")
		del kanivin.conf["mail_server"]

		kanivin.local.outgoing_email_account = {}
		...
		kanivin.conf["mail_server"] = mail_server
		for email_account in existing_email_accounts:
			set_details = {
			...
			kanivin.db.set_value("Email Account", email_account["name"], set_details)
...
def create_email_account(email_id, password, enable_outgoing, default_outgoing=0, append_to=None):
	...
	email_account = kanivin.new_doc("Email Account")
