...
import kanivin
from kanivin.query_builder.custom import GROUP_CONCAT, MATCH, STRING_AGG, TO_TSVECTOR
from kanivin.query_builder.utils import ImportMapper, db_type_is
...
class Cast_(Function):
	def __init__(self, value, as_type, alias=None):
		if kanivin.db.db_type == "mariadb" and (
...
def _aggregate(function, dt, fieldname, filters, **kwargs):
	return (
		kanivin.qb.get_query(dt, filters=filters, fields=[function(PseudoColumn(fieldname))]).run(**kwargs)[0][
			0
		]
		or 0
	)
