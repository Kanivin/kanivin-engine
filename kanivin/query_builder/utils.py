...
class ImportMapper:
	...
	def __call__(self, *args: Any, **kwds: Any) -> Callable:
		db = db_type_is(kanivin.conf.db_type)
...
def get_query(*args, **kwargs) -> QueryBuilder:
	from kanivin.database.query import Engine
...
def DocType(*args, **kwargs):
	return kanivin.qb.DocType(*args, **kwargs)


def Table(*args, **kwargs):
	return kanivin.qb.Table(*args, **kwargs)


def patch_query_execute():
	"""Patch the Query Builder with helper execute method
	This excludes the use of `kanivin.db.sql` method while
	...
	def execute_query(query, *args, **kwargs):
		...
		result = kanivin.db.sql(query, params, *args, **kwargs)  # nosemgrep
	...
	def prepare_query(query):
		...
		if kanivin.flags.in_safe_exec:
			from kanivin.utils.safe_exec import SERVER_SCRIPT_FILE_PREFIX, check_safe_sql_query
			...
			if not check_safe_sql_query(query, throw=False):
				...
				if len(callstack) >= 3 and SERVER_SCRIPT_FILE_PREFIX in callstack[2].filename:
					raise kanivin.PermissionError("Only SELECT SQL allowed in scripting")

		return query, param_collector.get_parameters()

	builder_class = kanivin.qb._BuilderClasss
...
def patch_query_aggregation():
	"""Patch aggregation functions to kanivin.qb"""
	from kanivin.query_builder.functions import _avg, _max, _min, _sum
