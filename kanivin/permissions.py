...
from kanivin import _, msgprint
...
def print_has_permission_check_logs(func):
	...
	def inner(*args, **kwargs):
		...
		self_perm_check = True if not kwargs.get("user") else kwargs.get("user") == kanivin.session.user

		if raise_exception:
			kanivin.flags["has_permission_check_logs"] = []
		...
		if not result and self_perm_check and raise_exception:
			msgprint(("<br>").join(kanivin.flags.get("has_permission_check_logs", [])))

		if raise_exception:
			kanivin.flags.pop("has_permission_check_logs", None)
		return result
...
def _debug_log(log: str):
	if not hasattr(kanivin.local, "permission_debug_log"):
		kanivin.local.permission_debug_log = []
	kanivin.local.permission_debug_log.append(log)


def _pop_debug_log() -> list[str]:
	if log := getattr(kanivin.local, "permission_debug_log", None):
		del kanivin.local.permission_debug_log
		return log
	return []
...
def has_permission(
	...
	*,
	...
	:param raise_exception:
	        ...
	        If not False, will display a message using kanivin.msgprint
	...
	if ptype == "share" and kanivin.get_system_settings("disable_document_sharing"):
	...
	if doc:
		if isinstance(doc, str | int):
			doc = kanivin.get_doc(meta.name, doc)
		...
		if not perm:
			...
			msg = _("User {0} does not have access to this document").format(kanivin.bold(user))
			if kanivin.has_permission(doc.doctype):
		...
		debug and _debug_log(
			...
			+ kanivin.as_json(role_permissions, indent=8)
		)
		...
		if not perm:
			push_perm_check_log(
				_("User {0} does not have doctype access via role permission for document {1}").format(
					kanivin.bold(user), kanivin.bold(_(doctype))
				),
				debug=debug,
			)
	...
	def false_if_not_shared():
		...
		if doc:
			...
			shared = kanivin.share.get_shared(
				doctype,
				user,
				rights=rights,
		...
		elif kanivin.share.get_shared(doctype, user, rights=rights, limit=1):
...
def get_doc_permissions(doc, user=None, ptype=None, debug=False):
	...
	meta = kanivin.get_meta(doc.doctype)
	...
	debug and _debug_log(
		"User has following permissions using role permission system: "
		+ kanivin.as_json(permissions, indent=8)
	)
	...
	if permissions.get("has_if_owner_enabled"):
		...
		debug and _debug_log(
			"User is owner of document, so permissions are updated to: " + kanivin.as_json(permissions)
		)
...
def get_role_permissions(doctype_meta, user=None, is_owner=None, debug=False):
	...
	if isinstance(doctype_meta, str):
		doctype_meta = kanivin.get_meta(doctype_meta)  # assuming doctype name was passed
	...
	if not kanivin.local.role_permissions.get(cache_key) or debug:
		perms = kanivin._dict(if_owner={})
		...
		kanivin.local.role_permissions[cache_key] = perms

	return kanivin.local.role_permissions[cache_key]


def get_user_permissions(user):
	from kanivin.core.doctype.user_permission.user_permission import get_user_permissions
...
def has_user_permission(doc, user=None, debug=False):
	...
	apply_strict_user_permissions = (
		False if doc.meta.issingle else kanivin.get_system_settings("apply_strict_user_permissions")
	)
	...
	def check_user_permission_on_link_fields(d):
		...
		meta = kanivin.get_meta(d.get("doctype"))
...
def has_controller_permissions(doc, ptype, user=None, debug=False) -> bool:
	...
	hooks = kanivin.get_hooks("has_permission")
	methods = hooks.get(doc.doctype, []) + hooks.get("*", [])

	for method in reversed(methods):
		controller_permission = kanivin.call(method, doc=doc, ptype=ptype, user=user, debug=debug)
...
def get_all_perms(role):
	...
	perms = kanivin.get_all("DocPerm", fields="*", filters=dict(role=role))
	custom_perms = kanivin.get_all("Custom DocPerm", fields="*", filters=dict(role=role))
	doctypes_with_custom_perms = kanivin.get_all("Custom DocPerm", pluck="parent", distinct=True)
...
def get_roles(user=None, with_standard=True):
	...
	def get():
		if user == "Administrator":
			return kanivin.get_all("Role", pluck="name")  # return all available roles
		else:
	...
	roles = kanivin.cache.hget("roles", user, get)
...
def get_perms_for(roles, perm_doctype="DocPerm"):
	...
	return kanivin.get_all(perm_doctype, fields=["*"], filters=filters)


def get_doctypes_with_custom_docperms():
	"""Returns all the doctypes with Custom Docperms"""

	doctypes = kanivin.get_all("Custom DocPerm", fields=["parent"], distinct=1)
...
def add_user_permission(
	...
	"""Add user permission"""
	from kanivin.core.doctype.user_permission.user_permission import user_permission_exists

	if not user_permission_exists(user, doctype, name, applicable_for):
		if not kanivin.db.exists(doctype, name):
			kanivin.throw(_("{0} {1} not found").format(_(doctype), name), kanivin.DoesNotExistError)
...
def remove_user_permission(doctype, name, user):
	user_permission_name = kanivin.db.get_value(
		"User Permission", dict(user=user, allow=doctype, for_value=name)
	)
	kanivin.delete_doc("User Permission", user_permission_name)
...
def clear_user_permissions_for_doctype(doctype, user=None):
	...
	if user:
		filters["user"] = user
	user_permissions_for_doctype = kanivin.get_all("User Permission", filters=filters)
	for d in user_permissions_for_doctype:
		kanivin.delete_doc("User Permission", d.name)


def can_import(doctype, raise_exception=False):
	if not ("System Manager" in kanivin.get_roles() or has_permission(doctype, "import")):
		if raise_exception:
			raise kanivin.PermissionError(f"You are not allowed to import: {doctype}")
		else:
			return False
	return True
...
def can_export(doctype, raise_exception=False, is_owner=False):
	if "System Manager" in kanivin.get_roles():
		return True
	else:
		role_permissions = kanivin.permissions.get_role_permissions(doctype, is_owner=is_owner)
		has_access = role_permissions.get("export") or role_permissions.get("if_owner").get("export")
		if not has_access and raise_exception:
			raise kanivin.PermissionError(_("You are not allowed to export {} doctype").format(doctype))
		return has_access
...
def update_permission_property(
	...
	from kanivin.core.doctype.doctype.doctype import validate_permissions_for_doctype

	out = setup_custom_perms(doctype)

	name = kanivin.db.get_value(
		"Custom DocPerm", dict(parent=doctype, role=role, permlevel=permlevel, if_owner=if_owner)
	)
	table = DocType("Custom DocPerm")
	kanivin.qb.update(table).set(ptype, value).where(table.name == name).run()
...
def setup_custom_perms(parent):
	...
	if not kanivin.db.exists("Custom DocPerm", dict(parent=parent)):
		copy_perms(parent)
		return True
...
def add_permission(doctype, role, permlevel=0, ptype=None):
	...
	custom_docperm = kanivin.get_doc(
		{
...
def copy_perms(parent):
	...
	for d in kanivin.get_all("DocPerm", fields="*", filters=dict(parent=parent)):
		custom_perm = kanivin.new_doc("Custom DocPerm")
...
def reset_perms(doctype):
	...
	from kanivin.desk.notifications import delete_notification_count_for

	delete_notification_count_for(doctype)
	kanivin.db.delete("Custom DocPerm", {"parent": doctype})


def get_linked_doctypes(dt: str) -> list:
	meta = kanivin.get_meta(dt)
	linked_doctypes = [dt] + [
		d.options
		for d in meta.get(
			"fields",
...
def push_perm_check_log(log, debug=False):
	...
	if kanivin.flags.get("has_permission_check_logs") is None:
		return

	kanivin.flags.get("has_permission_check_logs").append(log)
...
def has_child_permission(
	...
	*,
	...
	if isinstance(child_doc, str):
		child_doc = kanivin.db.get_value(
			child_doctype,
			child_doc,
	...
	if not parent_doctype:
		push_perm_check_log(
			_("Please specify a valid parent DocType for {0}").format(kanivin.bold(child_doctype)),
			debug=debug,
		)
		return False

	parent_meta = kanivin.get_meta(parent_doctype)
	...
	if parent_meta.istable or not (
		...
		push_perm_check_log(
			_("{0} is not a valid parent DocType for {1}").format(
				kanivin.bold(parent_doctype), kanivin.bold(child_doctype)
			),
			debug=debug,
		)
		return False
	...
	if child_doc:
		...
		if not parentfield:
			push_perm_check_log(
				_("Parentfield not specified in {0}: {1}").format(
					kanivin.bold(child_doctype), kanivin.bold(child_doc.name)
				),
				debug=debug,
			)
			return False
		...
		if parentfield not in valid_parentfields:
			push_perm_check_log(
				_("{0} is not a valid parentfield for {1}").format(
					kanivin.bold(parentfield), kanivin.bold(child_doctype)
				),
				debug=debug,
			)
			return False
		...
		if permlevel > 0 and permlevel not in accessible_permlevels:
			push_perm_check_log(
				_("Insufficient Permission Level for {0}").format(kanivin.bold(parent_doctype)), debug=debug
			)
			debug and _debug_log(
...
def is_system_user(user: str | None = None) -> bool:
	return kanivin.get_cached_value("User", user or kanivin.session.user, "user_type") == "System User"
...
def check_doctype_permission(doctype: str, ptype: str = "read") -> None:
	...
	_message_log = kanivin.local.message_log
	kanivin.local.message_log = []
	try:
		kanivin.has_permission(doctype, ptype, throw=True, ignore_share_permissions=True)
	except kanivin.PermissionError:
		kanivin.flags.disable_traceback = True
		raise

	kanivin.local.message_log = _message_log
...
def handle_does_not_exist_error(fn):
	...
	def wrapper(e, *args, **kwargs):
		if isinstance(e, kanivin.DoesNotExistError) and (doctype := getattr(e, "doctype", None)):
			try:
				check_doctype_permission(doctype)
			except kanivin.PermissionError as _e:
