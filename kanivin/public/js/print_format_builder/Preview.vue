...
function get_default_docname() {
	return kanivin.db.get_list(doctype.value, { limit: 1 }).then((doc) => {
...
let url = computed(() => {
	...
	let _url =
		type.value == "PDF" ? `/api/method/kanivin.utils.weasyprint.download_pdf` : "/printpreview";
	return `${_url}?${params.toString()}`;
});

// mounted
onMounted(() => {
	doc_select.value = kanivin.ui.form.make_control({
	...
	preview_type.value = kanivin.ui.form.make_control({
