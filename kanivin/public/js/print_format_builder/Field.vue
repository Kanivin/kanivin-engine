...
function edit_html() {
	let d = new kanivin.ui.Dialog({
		...
		primary_action: ({ html }) => {
			html = kanivin.dom.remove_script_and_style(html);
			props.df["html"] = html;
			d.hide();
		},
	});
...
function get_all_columns() {
	let meta = kanivin.get_meta(props.df.options);
	let more_columns = [
		{
			label: __("Sr No."),
			value: "idx",
	...
	return more_columns.concat(
		meta.fields
			.map((tf) => {
				if (kanivin.model.no_value_type.includes(tf.fieldtype)) {
					return;
				}
				return {
					label: tf.label,
...
function get_column_to_add(fieldname) {
	...
	return {
		...kanivin.meta.get_docfield(props.df.options, fieldname),
		width: 10,
	};
}
