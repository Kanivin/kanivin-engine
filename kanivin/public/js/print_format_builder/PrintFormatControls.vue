...
function clone_field(df) {
	...
	if (cloned.custom) {
		...
		cloned.fieldname += "_" + kanivin.utils.get_random(8);
	}
	return cloned;
}
...
let print_templates = computed(() => {
	...
	for (let template of templates) {
		let df;
		if (template.field) {
			df = kanivin.meta.get_docfield(meta.value.name, template.field);
		} else {
			df = {
				label: template.name,
				fieldname: kanivin.scrub(template.name),
			};
		}
		out.push({
...
onMounted(() => {
	let method =
		"kanivin.printing.page.print_format_builder_beta.print_format_builder_beta.get_google_fonts";
...
.control-font :deep(.kanivin-control[data-fieldname="font"] label) {
	display: none;
}
</style>
