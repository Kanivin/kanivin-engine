...
kanivin.provide("kanivin.web_form");
import EventEmitterMixin from "../../kanivin/event_emitter";

export default class WebForm extends kanivin.ui.FieldGroup {
	constructor(opts) {
		super();
		Object.assign(this, opts);
		kanivin.web_form = this;
		kanivin.web_form.events = {};
		Object.assign(kanivin.web_form.events, EventEmitterMixin);
	...
	make() {
		...
		kanivin.init_client_script && kanivin.init_client_script();
		this.setup_listeners();
		kanivin.web_form.events.trigger("after_load");
	...
	set_default_values() {
		...
		let values = kanivin.utils.get_query_params();
		delete values.new;
	...
	render_progress_dots() {
		...
		for (let i = 0; i <= this.page_breaks.length; i++) {
			let $dot = $(`<div class="slide-step">
				...
				<div class="slide-step-complete">${kanivin.utils.icon("tick", "xs")}</div>
	...
	is_next_section_empty(section) {
		...
		let visible_controls = _page.find(".kanivin-control:not(.hide-control)");
	...
	save() {
		...
		// Save
		window.saving = true;
		kanivin.form_dirty = false;

		kanivin.call({
			type: "POST",
			method: "kanivin.website.doctype.web_form.web_form.accept",
			args: {
				data: this.doc,
				web_form: this.name,
				for_payment,
			...
			callback: (response) => {
				...
				if (!response.exc) {
					...
					kanivin.web_form.events.trigger("after_save");
					...
					if (is_new && (response.message.attachment || response.message.file)) {
						kanivin.call({
							type: "POST",
							method: "kanivin.handler.upload_file",
							args: {
	...
	handle_success(data) {
		...
		if (this.success_url) {
			kanivin.utils.setup_timer(5, 0, $(".time"));
			setTimeout(() => {
