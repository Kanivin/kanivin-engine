...
kanivin.ready(function () {
	let web_form_doc = kanivin.web_form_doc;
	let reference_doc = kanivin.reference_doc;
	...
	function show_login_prompt() {
		if (kanivin.session.user != "Guest" || !web_form_doc.login_required) return;
		const login_required = new kanivin.ui.Dialog({
	...
	function show_form() {
		...
		$(".file-size").each(function () {
			$(this).text(kanivin.form.formatters.FileSize($(this).text()));
		});
	}
