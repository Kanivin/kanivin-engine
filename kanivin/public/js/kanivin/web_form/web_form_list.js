...
kanivin.provide("kanivin.web_form_list");

export default class WebFormList {
	constructor(opts) {
		Object.assign(this, opts);
		kanivin.web_form_list = this;
	...
	make_filters() {
		...
		kanivin
			.call("kanivin.website.doctype.web_form.web_form.get_web_form_filters", {
			...
			.then((response) => {
				...
				fields.forEach((field) => {
					...
					let input = kanivin.ui.form.make_control({
						df: {
	...
	fetch_data() {
		if (this.condition_json && JSON.parse(this.condition_json)) {
			let filter = kanivin.utils.get_filter_from_json(this.condition_json, this.doctype);
			filter = kanivin.utils.get_filter_as_json(filter);
			this.filters = Object.assign(this.filters, JSON.parse(filter));
		}

		let args = {
			method: "kanivin.www.list.get_list_data",
			args: {
	...
	more() {
		...
		this.fetch_data().then((res) => {
			if (res.message.length === 0) {
				kanivin.msgprint(__("No more items to display"));
			}
	...
	make_table_body() {
		if (this.data.length) {
			...
			this.wrapper.empty();
			kanivin.has_permission(this.doctype, "", "create", () => {
		...
		let empty_state = $(`
			<div class="no-result text-muted flex justify-center align-center">
				<div class="text-center">
					<div>
						<img
							src="/assets/kanivin/images/ui-states/list-empty-state.svg"
	...
	append_rows(row_data) {
		...
		row_data.forEach((data_item) => {
			...
			let row = new kanivin.ui.WebFormListRow({
				row: $row_element,
				doc: data_item,
	...
	make_actions() {
		...
		kanivin.has_permission(this.doctype, "", "delete", () => {
	...
	delete_rows() {
		...
		kanivin
			.call({
				type: "POST",
				method: "kanivin.website.doctype.web_form.web_form.delete_multiple",
				args: {
					web_form_name: this.web_form_name,
...
kanivin.ui.WebFormListRow = class WebFormListRow {
	...
	make_row() {
		...
		this.columns.forEach((field) => {
			let formatter = kanivin.form.get_formatter(field.fieldtype);
			let value =
