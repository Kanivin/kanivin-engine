...
*/
kanivin.model.DocTypeController = class DocTypeController extends kanivin.ui.form.Controller {
	setup() {
		// setup formatters for fieldtype
		kanivin.meta.docfield_map[
	...
	show_db_utilization() {
		...
		kanivin
			.xcall("kanivin.core.doctype.doctype.doctype.get_row_size_utilization", {
				doctype,
			})
			.then((r) => {
				if (r < 50.0) return;
	...
	setup_fetch_from_fields(doc, doctype, docname) {
		...
		let row = kanivin.get_doc(doctype, docname);
		...
		function update_fieldname_options() {
			...
			kanivin.model.with_doctype(link_doctype, () => {
				let fields = kanivin.meta
