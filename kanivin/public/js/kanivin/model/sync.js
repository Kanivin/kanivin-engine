...
Object.assign(kanivin.model, {
	docinfo: {},
	sync: function (r) {
		/* docs:
			extract docs, docinfo (attachments, comments, assignments)
			from incoming request and set in `locals` and `kanivin.model.docinfo`
		*/
		var isPlain;
		...
		if (r.docs) {
			for (var i = 0, l = r.docs.length; i < l; i++) {
				...
				if (locals[d.doctype] && locals[d.doctype][d.name]) {
					// update values
					kanivin.model.update_in_locals(d);
				} else {
					kanivin.model.add_to_locals(d);
				}
				...
				if (d.localname) {
					kanivin.model.rename_after_save(d, i);
				}
			}
		}

		kanivin.model.sync_docinfo(r);
		return r.docs;
	},

	rename_after_save: (d, i) => {
		kanivin.model.new_names[d.localname] = d.name;
		$(document).trigger("rename", [d.doctype, d.localname, d.name]);
		delete locals[d.doctype][d.localname];

		// update docinfo to new dict keys
		if (i === 0) {
			kanivin.model.docinfo[d.doctype][d.name] = kanivin.model.docinfo[d.doctype][d.localname];
			kanivin.model.docinfo[d.doctype][d.localname] = undefined;
		}
	},
	...
	sync_docinfo: (r) => {
		...
		if (r.docinfo) {
			...
			if (!kanivin.model.docinfo[doctype]) {
				kanivin.model.docinfo[doctype] = {};
			}
			kanivin.model.docinfo[doctype][name] = r.docinfo;

			// copy values to kanivin.boot.user_info
			Object.assign(kanivin.boot.user_info, r.docinfo.user_info);
		}
	...
	add_to_locals: function (doc) {
		...
		if (!doc.name && doc.__islocal) {
			...
			if (!doc.parentfield) kanivin.model.clear_doc(doc);

			doc.name = kanivin.model.get_new_name(doc.doctype);

			if (!doc.parentfield)
				kanivin.provide("kanivin.model.docinfo." + doc.doctype + "." + doc.name);
		}
	...
	update_in_locals: function (doc) {
		...
		for (let fieldname in doc) {
			let df = kanivin.meta.get_field(doc.doctype, fieldname);
			if (df && kanivin.model.table_fields.includes(df.fieldtype)) {
				// table
				...
				for (let i = 0; i < doc[fieldname].length; i++) {
					...
					if (local_d) {
						...
						if (!d.name) {
							...
							d.name = kanivin.model.get_new_name(doc.doctype);
						}
