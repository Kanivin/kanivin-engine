...
$.extend(kanivin.model, {
	...
	init: function () {
		...
		kanivin.realtime.on("doc_update", function (data) {
			var doc = locals[data.doctype] && locals[data.doctype][data.name];

			if (doc) {
				// current document is dirty, show message if its not me
				if (
					kanivin.get_route()[0] === "Form" &&
					cur_frm.doc.doctype === doc.doctype &&
					cur_frm.doc.name === doc.name
				) {
					if (data.modified !== cur_frm.doc.modified && !kanivin.ui.form.is_saving) {
						if (!cur_frm.is_dirty()) {
					...
					if (!doc.__unsaved) {
						...
						kanivin.model.remove_from_locals(doc.doctype, doc.name);
					} else {
	...
	is_value_type: function (fieldtype) {
		...
		return kanivin.model.no_value_type.indexOf(fieldtype) === -1;
	},

	is_non_std_field: function (fieldname) {
		return ![...kanivin.model.std_fields_list, ...kanivin.model.child_table_field_list].includes(
			fieldname
		);
	},
	...
	get_std_field: function (fieldname, ignore = false) {
		var docfield = $.map(
			[].concat(kanivin.model.std_fields).concat(kanivin.model.std_fields_table),
			function (d) {
		...
		if (!docfield.length) {
			...
			if (ignore) {
				...
				kanivin.msgprint(__("Unknown Column: {0}", [fieldname]));
			}
		}
		return docfield[0];
	},
	...
	set_in_localstorage: function (doctype, docs) {
		try {
			...
			kanivin.model.clear_local_storage();
			localStorage["_doctype:" + doctype] = JSON.stringify(docs);
		}
	},
	...
	with_doctype: function (doctype, callback, async) {
		if (locals.DocType[doctype]) {
			...
			let cached_docs = kanivin.model.get_from_localstorage(doctype);
			...
			return kanivin.call({
				method: "kanivin.desk.form.load.getdoctype",
				type: "GET",
				args: {
					doctype: doctype,
					with_parent: 1,
				...
				callback: function (r) {
					if (r.exc) {
						kanivin.msgprint(__("Unable to load: {0}", [__(doctype)]));
						throw "No doctype";
					}
					if (r.message == "use_cache") {
						kanivin.model.sync(cached_doc);
					} else {
						kanivin.model.set_in_localstorage(doctype, r.docs);
					}
					kanivin.model.init_doctype(doctype);

					if (r.user_settings) {
						// remember filters and other settings from last view
						kanivin.model.user_settings[doctype] = JSON.parse(r.user_settings);
						kanivin.model.user_settings[doctype].updated_on = moment().toString();
					}
					callback && callback(r);
				},
			});
	...
	init_doctype: function (doctype) {
		...
		if (meta.__templates) {
			$.extend(kanivin.templates, meta.__templates);
		}
	},
	...
	with_doc: function (doctype, name, callback) {
		return new Promise((resolve) => {
			...
			if (
				locals[doctype] &&
				locals[doctype][name] &&
				kanivin.model.get_docinfo(doctype, name)
			) {
				callback && callback(name);
				resolve(kanivin.get_doc(doctype, name));
			} else {
				return kanivin.call({
					method: "kanivin.desk.form.load.getdoc",
					type: "GET",
					args: {
						doctype: doctype,
						name: name,
	...
	get_docinfo: function (doctype, name) {
		return (kanivin.model.docinfo[doctype] && kanivin.model.docinfo[doctype][name]) || null;
	},

	set_docinfo: function (doctype, name, key, value) {
		if (kanivin.model.docinfo[doctype] && kanivin.model.docinfo[doctype][name]) {
			kanivin.model.docinfo[doctype][name][key] = value;
		}
	},

	get_shared: function (doctype, name) {
		return kanivin.model.get_docinfo(doctype, name).shared;
	},

	get_server_module_name: function (doctype) {
		var dt = kanivin.model.scrub(doctype);
		var module = kanivin.model.scrub(locals.DocType[doctype].module);
		var app = kanivin.boot.module_app[module];
	...
	can_create: function (doctype) {
		return kanivin.boot.user.can_create.indexOf(doctype) !== -1;
	},

	can_select: function (doctype) {
		if (kanivin.boot.user) {
			return kanivin.boot.user.can_select.indexOf(doctype) !== -1;
		}
	},

	can_read: function (doctype) {
		if (kanivin.boot.user) {
			return kanivin.boot.user.can_read.indexOf(doctype) !== -1;
		}
	},

	can_write: function (doctype) {
		return kanivin.boot.user.can_write.indexOf(doctype) !== -1;
	},

	can_get_report: function (doctype) {
		return kanivin.boot.user.can_get_report.indexOf(doctype) !== -1;
	},

	can_delete: function (doctype) {
		if (!doctype) return false;
		return kanivin.boot.user.can_delete.indexOf(doctype) !== -1;
	},

	can_submit: function (doctype) {
		if (!doctype) return false;
		return kanivin.boot.user.can_submit.indexOf(doctype) !== -1;
	},

	can_cancel: function (doctype) {
		if (!doctype) return false;
		return kanivin.boot.user.can_cancel.indexOf(doctype) !== -1;
	},

	has_workflow: function (doctype) {
		return kanivin.get_list("Workflow", { document_type: doctype, is_active: 1 }).length;
	},
	...
	is_single: function (doctype) {
		if (!doctype) return false;
		return kanivin.boot.single_types.indexOf(doctype) != -1;
	},
	...
	can_import: function (doctype, frm, meta = null) {
		...
		if (kanivin.user_roles.includes("System Manager")) return true;

		if (frm) return frm.perm[0].import === 1;
		return kanivin.boot.user.can_import.indexOf(doctype) !== -1;
	},
	...
	can_export: function (doctype, frm) {
		...
		return kanivin.boot.user.can_export.indexOf(doctype) !== -1;
	},

	can_print: function (doctype, frm) {
		if (frm) return frm.perm[0].print === 1;
		return kanivin.boot.user.can_print.indexOf(doctype) !== -1;
	},

	can_email: function (doctype, frm) {
		if (frm) return frm.perm[0].email === 1;
		return kanivin.boot.user.can_email.indexOf(doctype) !== -1;
	},

	can_share: function (doctype, frm) {
		let disable_sharing = cint(kanivin.sys_defaults.disable_document_sharing);

		if (disable_sharing && kanivin.session.user !== "Administrator") {
			return false;
		}
		...
		return kanivin.boot.user.can_share.indexOf(doctype) !== -1;
	},

	has_value: function (dt, dn, fn) {
		// return true if property has value
		var val = locals[dt] && locals[dt][dn] && locals[dt][dn][fn];
		var df = kanivin.meta.get_docfield(dt, fn, dn);
	...
	get_list: function (doctype, filters) {
		...
		return kanivin.utils.filter_dict(docsdict, filters);
	},
	...
	get_value: function (doctype, filters, fieldname, callback) {
		if (callback) {
			...
			if (
				...
				var l = kanivin.get_list(doctype, filters);
	...
	set_value: function (
		...
		$.each(to_update, (key, value) => {
			if (doc && doc[key] !== value) {
				...
				doc[key] = value;
				tasks.push(() => kanivin.model.trigger(key, value, doc, skip_dirty_trigger));
			} else {
	...
	on: function (doctype, fieldname, fn) {
		...
		*/
		/* example: kanivin.model.on("Customer", "age", function(fieldname, value, doc) {
		  if(doc.age < 16) {
		   	kanivin.msgprint("Warning, Customer must atleast be 16 years old.");
		    raise "CustomerAgeError";
		  }
		}) */
		kanivin.provide("kanivin.model.events." + doctype);
		if (!kanivin.model.events[doctype][fieldname]) {
			kanivin.model.events[doctype][fieldname] = [];
		}
		kanivin.model.events[doctype][fieldname].push(fn);
	},
		...
		function enqueue_events(events) {
			...
			for (const fn of events) {
				...
				tasks.push(() => {
					...
					// or use the default promise kanivin.after_ajax
					if (return_value && return_value.then) {
						return return_value;
					} else {
						return kanivin.after_server_call();
					}
				});
			}
		}

		if (kanivin.model.events[doc.doctype]) {
			enqueue_events(kanivin.model.events[doc.doctype][fieldname]);
			enqueue_events(kanivin.model.events[doc.doctype]["*"]);
		}
	...
	get_doc: function (doctype, name) {
		...
		if ($.isPlainObject(name)) {
			var doc = kanivin.get_list(doctype, name);
	...
	get_children: function (doctype, parent, parentfield, filters) {
		...
		if ($.isPlainObject(doctype)) {
			...
			doc = kanivin.get_doc(doctype, parent);
		}

		var children = doc[parentfield] || [];
		if (filters) {
			return kanivin.utils.filter_dict(children, filters);
		} else {
			return children;
		}
	},
	...
	remove_from_locals: function (doctype, name) {
		...
		if (kanivin.views.formview[doctype]) {
			delete kanivin.views.formview[doctype].frm.opendocs[name];
		}
	},
	...
	get_no_copy_list: function (doctype) {
		...
		var docfields = kanivin.get_doc("DocType", doctype).fields || [];
	...
	delete_doc: function (doctype, docname, callback) {
		let title = docname;
		const title_field = kanivin.get_meta(doctype).title_field;
		if (kanivin.get_meta(doctype).autoname == "hash" && title_field) {
			const value = kanivin.model.get_value(doctype, docname, title_field);
			if (value) {
				title = `${value} (${docname})`;
			}
		}
		kanivin.confirm(__("Permanently delete {0}?", [title.bold()]), function () {
			return kanivin.call({
				method: "kanivin.client.delete",
				args: {
					doctype: doctype,
					name: docname,
				},
				...
				callback: function (r, rt) {
					if (!r.exc) {
						kanivin.utils.play_sound("delete");
						kanivin.model.clear_doc(doctype, docname);
	...
	round_floats_in: function (doc, fieldnames) {
		...
		if (!fieldnames) {
			fieldnames = kanivin.meta.get_fieldnames(doc.doctype, doc.parent, {
	...
	validate_missing: function (doc, fieldname) {
		if (!doc[fieldname]) {
			kanivin.throw(
				__("Please specify") +
					": " +
					__(kanivin.meta.get_label(doc.doctype, fieldname, doc.parent || doc.name))
			);
		}
	},
	...
	is_numeric_field: function (fieldtype) {
		...
		return kanivin.model.numeric_fieldtypes.includes(fieldtype);
	},
	...
	set_default_views_for_doctype(doctype, frm) {
		kanivin.model.with_doctype(doctype, () => {
			...
			if (meta.is_calendar_and_gantt && kanivin.views.calendar[doctype]) {
			...
			if (doctype === "Communication" && kanivin.boot.email_accounts.length) {
...
// legacy
kanivin.get_doc = kanivin.model.get_doc;
kanivin.get_children = kanivin.model.get_children;
kanivin.get_list = kanivin.model.get_list;
