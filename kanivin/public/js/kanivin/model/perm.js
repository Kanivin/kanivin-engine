...
$.extend(kanivin.perm, {
	...
	has_perm: (doctype, permlevel = 0, ptype = "read", doc) => {
		const perms = kanivin.perm.get_perm(doctype, doc);
	...
	get_perm: (doctype, doc) => {
		...
		if (doc && !doc.__islocal) {
			return kanivin.perm._get_perm(doctype, doc);
		}

		return (kanivin.perm.doctype_perm[doctype] ??= kanivin.perm._get_perm(doctype));
	},

	_get_perm: (doctype, doc) => {
		let perm = [{ read: 0, permlevel: 0 }];

		let meta = kanivin.get_doc("DocType", doctype);
		const user = kanivin.session.user;

		if (user === "Administrator" || kanivin.user_roles.includes("Administrator")) {
			perm[0].read = 1;
		}

		if (!meta) return perm;

		perm = kanivin.perm.get_role_permissions(meta);
		const base_perm = perm[0];

		if (doc) {
			// apply user permissions via docinfo (which is processed server-side)
			let docinfo = kanivin.model.get_docinfo(doctype, doc.name);
			...
			// if owner
			if (doc.owner !== user) {
				for (const right of kanivin.perm.rights) {
			...
			if (docinfo && docinfo.shared) {
				for (const s of docinfo.shared) {
					...
					if (s.read) {
						...
						base_perm.email =
							kanivin.boot.user.can_email.indexOf(doctype) !== -1 ? 1 : 0;
						base_perm.print =
							kanivin.boot.user.can_print.indexOf(doctype) !== -1 ? 1 : 0;
					}
				}
			}
		}

		if (!base_perm.read && kanivin.model.can_read(doctype)) {
			// read via sharing
			base_perm.read = 1;
		}
	...
	get_role_permissions: (meta) => {
		...
		*/
		...
		(meta.permissions || []).forEach((p) => {
			...
			if (kanivin.user_roles.includes(p.role)) {
				kanivin.perm.rights.forEach((right) => {
					if (!p[right]) return;
	...
	get_match_rules: (doctype, ptype) => {
		...
		let perm = kanivin.perm.get_perm(doctype);

		let user_permissions = kanivin.defaults.get_user_permissions();

		if (user_permissions && !$.isEmptyObject(user_permissions)) {
			let rules = {};
			let fields_to_check = kanivin.meta.get_fields_to_check_permissions(doctype);
			$.each(fields_to_check, (i, df) => {
				const user_permissions_for_doctype = user_permissions[df.options] || [];
				const allowed_records = kanivin.perm.get_allowed_docs_for_doctype(
		...
		if (base_perm.read && !base_perm.rights_without_if_owner.has("read")) {
			match_rules.push({ Owner: kanivin.session.user });
		}
		return match_rules;
	},
	...
	get_field_display_status: (df, doc, perm, explain) => {
		...
		if (!perm && doc) {
			perm = kanivin.perm.get_perm(doc.doctype, doc);
		}
	...
	is_visible: (df, doc, perm) => {
		if (typeof df === "string") {
			// df is fieldname
			df = kanivin.meta.get_docfield(doc.doctype, df, doc.parent || doc.name);
		}

		let status = kanivin.perm.get_field_display_status(df, doc, perm);
	...
	get_allowed_docs_for_doctype: (user_permissions, doctype) => {
		...
		return kanivin.perm.filter_allowed_docs_for_doctype(user_permissions, doctype, false);
	},
