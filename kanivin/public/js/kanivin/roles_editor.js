kanivin.RoleEditor = class {
	constructor(wrapper, frm, disable) {
		...
		this.multicheck = kanivin.ui.form.make_control({
			...
			df: {
				...
				get_data: () => {
					return kanivin
						.xcall("kanivin.core.doctype.user.user.get_all_roles")
						.then((roles) => {
	...
	show_permissions(role) {
		...
		return kanivin
			.xcall("kanivin.core.doctype.user.user.get_perm_info", { role })
			.then((permissions) => {
						...
						<table class="user-perm">
							<thead>
								<tr>
									...
									<th> ${__("Level")} </th>
									<th> ${__("If Owner")} </th>
									${kanivin.perm.rights.map((p) => `<th> ${__(kanivin.unscrub(p))}</th>`).join("")}
								</tr>
							</thead>
							<tbody></tbody>
						</table>
					...
					permissions.forEach((perm) => {
						$body.find("tbody").append(`
							<tr>
								...
								<td>${perm.if_owner ? kanivin.utils.icon("check", "xs") : "-"}</td>
								${kanivin.perm.rights
									.map(
										(p) =>
											`<td class="text-muted bold">${
												perm[p] ? kanivin.utils.icon("check", "xs") : "-"
											}</td>`
									)
									.join("")}
							</tr>
	...
	make_perm_dialog() {
		this.perm_dialog = new kanivin.ui.Dialog({
	...
	set_roles_in_table() {
		...
		roles.map((role_doc) => {
			if (!checked_options.includes(role_doc.role)) {
				kanivin.model.clear_doc(role_doc.doctype, role_doc.name);
			}
		});
		checked_options.map((role) => {
			if (!roles.find((d) => d.role === role)) {
				let role_doc = kanivin.model.add_child(this.frm.doc, "Has Role", "roles");
				role_doc.role = role;
			}
		});
	}
