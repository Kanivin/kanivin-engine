...
kanivin.ui.Notifications = class Notifications {
	constructor() {
		this.tabs = {};
		this.notification_settings = kanivin.boot.notification_settings;
		this.make();
	}
	...
	make() {
		...
		this.user = kanivin.session.user;
	...
	setup_headers() {
		...
		$(`<span class="notification-settings pull-right" data-action="go_to_settings">
			${kanivin.utils.icon("setting-gear")}
		</span>`)
			.on("click", (e) => {
				e.stopImmediatePropagation();
				this.dropdown.dropdown("hide");
				kanivin.set_route("Form", "Notification Settings", kanivin.session.user);
			})
		...
		$(`<span class="mark-all-read pull-right" data-action="mark_all_as_read">
			${kanivin.utils.icon("mark-as-read")}
		</span>`)
	...
	mark_all_as_read(e) {
		...
		kanivin.call("kanivin.desk.doctype.notification_log.notification_log.mark_all_as_read");
	}
...
kanivin.ui.notifications = {
	get_notification_config() {
		return kanivin.xcall("kanivin.desk.notifications.get_notification_info").then((r) => {
			kanivin.ui.notifications.config = r;
			return r;
		});
	},

	show_open_count_list(doctype) {
		if (!kanivin.ui.notifications.config) {
	...
	route_to_list_with_filters(doctype) {
		let filters = kanivin.ui.notifications.config["conditions"][doctype];
		if (filters && $.isPlainObject(filters)) {
			if (!kanivin.route_options) {
				kanivin.route_options = {};
			}
			$.extend(kanivin.route_options, filters);
		}
		kanivin.set_route("List", doctype);
	},
};
...
class NotificationsView extends BaseNotificationsView {
	make() {
		...
		this.get_notifications_list(this.max_length).then((r) => {
			...
			kanivin.update_user_info(r.message.user_info);
	...
	mark_as_read(docname, $el) {
		kanivin
			.call("kanivin.desk.doctype.notification_log.notification_log.mark_as_read", {
				docname: docname,
			})
			.then(() => {
	...
	get_dropdown_item_html(notification_log) {
		...
		message = title
			? message.replace(title[1], kanivin.ellipsis(strip_html(title[1]), 100))
			: message;

		let timestamp = kanivin.datetime.comment_when(notification_log.creation);
		...
		let user_avatar = kanivin.avatar(user, "avatar-medium user-avatar");
			...
			if (this.dropdown_items.length) {
				...
				this.container.append(
					...
					<div class="text-center">
						<img src="/assets/kanivin/images/ui-states/notification-empty-state.svg" alt="Generic Empty State" class="null-state">
	...
	get_notifications_list(limit) {
		return kanivin.call(
			"kanivin.desk.doctype.notification_log.notification_log.get_notification_logs",
			{ limit: limit }
		);
	}
	...
	get_item_link(notification_doc) {
		...
		return kanivin.utils.get_form_link(link_doctype, link_docname);
	}
	...
	toggle_seen(flag) {
		kanivin.call(
			"kanivin.desk.doctype.notification_settings.notification_settings.set_seen_value",
			{
				value: cint(flag),
	...
	setup_notification_listeners() {
		kanivin.realtime.on("notification", () => {
			this.toggle_notification_icon(false);
			this.update_dropdown();
		});

		kanivin.realtime.on("indicator_hide", () => {
		...
		this.parent.on("show.bs.dropdown", () => {
			...
			if (this.notifications_icon.find(".notifications-unseen").is(":visible")) {
				this.toggle_notification_icon(true);
				kanivin.call(
					"kanivin.desk.doctype.notification_log.notification_log.trigger_indicator_hide"
				);
			}
		});
	}
...
class EventsView extends BaseNotificationsView {
	make() {
		let today = kanivin.datetime.get_today();
		kanivin
			.xcall("kanivin.desk.doctype.event.event.get_events", {
				start: today,
				end: today,
			})
			.then((event_list) => {
	...
	render_events_html(event_list) {
		...
		if (event_list.length) {
			let get_event_html = (event) => {
				let time = __("All Day");
				if (!event.all_day) {
					let start_time = kanivin.datetime.get_time(event.starts_on);
					let days_diff = kanivin.datetime.get_day_diff(event.ends_on, event.starts_on);
					let end_time = kanivin.datetime.get_time(event.ends_on);
					if (days_diff > 1) {
				...
				if (event.particpants) {
					particpants = kanivin.avatar_group(event.particpants, 3);
				}
			...
			html = `
				<div class="notification-null-state">
					<div class="text-center">
					<img src="/assets/kanivin/images/ui-states/event-empty-state.svg" alt="Generic Empty State" class="null-state">
...
class ChangelogFeedView extends BaseNotificationsView {
	make() {
		this.render_changelog_feed_html(kanivin.boot.changelog_feed || []);
	}
	...
	render_changelog_feed_html(changelog_feed) {
		...
		if (changelog_feed.length) {
			...
			const get_changelog_feed_html = (changelog_feed_item) => {
				const timestamp = kanivin.datetime.prettyDate(
