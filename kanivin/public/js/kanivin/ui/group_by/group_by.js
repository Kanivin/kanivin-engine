...
kanivin.ui.GroupBy = class {
	...
	init_group_by_popover() {
		...
		const group_by_template = $(
			kanivin.render_template("group_by", {
			...
			if (!this.aggregate_on_html.length) {
				...
				for (let doctype in this.all_fields) {
					...
					doctype_fields.forEach((field) => {
						...
						if (kanivin.model.is_numeric_field(field.fieldtype)) {
							let field_label = field.label || kanivin.model.unscrub(field.fieldname);
							let option_text =
								doctype == this.doctype
			...
			$(`<div class="group-by-selector">
				<button class="btn btn-default btn-sm group-by-button ellipsis">
					<span class="group-by-icon">
						${kanivin.utils.icon("es-line-folder-alt")}
					</span>
	...
	get_group_by_docfield() {
		...
		if (this.aggregate_function === "count") {
			...
			docfield = Object.assign(
				{},
				kanivin.meta.docfield_map[this.aggregate_on_doctype][this.aggregate_on_field]
			);
	...
	get_group_by_fields() {
		...
		const standard_fields = kanivin.model.std_fields.filter(
		...
		const standard_fields_filter = (df) =>
			!kanivin.model.no_value_type.includes(df.fieldtype) && !df.report_hide;
