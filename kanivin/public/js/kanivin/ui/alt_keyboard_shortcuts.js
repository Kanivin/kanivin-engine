kanivin.provide("kanivin.ui.keys");

let shortcut_groups = new WeakMap();
let shortcut_group_list = [];
kanivin.ui.keys.shortcut_groups = shortcut_groups;

kanivin.ui.keys.get_shortcut_group = (parent) => {
	// parent must be an object
	if (!shortcut_groups.has(parent)) {
		shortcut_groups.set(parent, new kanivin.ui.keys.AltShortcutGroup());
	}
...
kanivin.ui.keys.bind_shortcut_group_event = () => {
	...
	$(document).on("keydown", (e) => {
		let key = (kanivin.ui.keys.key_map[e.which] || "").toLowerCase();
...
kanivin.ui.keys.AltShortcutGroup = class AltShortcutGroup {
	constructor() {
		this.shortcuts_dict = {};
	...
	is_taken(letter) {
		let is_in_global_shortcut = kanivin.ui.keys.standard_shortcuts
			.filter((s) => !s.page)
