 ...
 *
 * @returns {kanivin.ui.Page}
 */

/**
 * @typedef {Object} kanivin.ui.Page
 */

kanivin.ui.make_app_page = function (opts) {
	opts.parent.page = new kanivin.ui.Page(opts);
	return opts.parent.page;
};

kanivin.ui.pages = {};

kanivin.ui.Page = class Page {
	constructor(opts) {
		$.extend(this, opts);
		...
		this.make();
		kanivin.ui.pages[kanivin.get_route_str()] = this;
	}
	...
	load_lib(callback) {
		kanivin.require(this.required_libs, callback);
	}

	add_main_section() {
		$(kanivin.render_template("page", {})).appendTo(this.wrapper);
	...
	setup_page() {
		...
		this.card_layout && this.main.addClass("kanivin-card");
	...
	setup_sidebar_toggle() {
		...
		if (this.disable_sidebar_toggle || !sidebar_wrapper.length) {
			...
			sidebar_toggle.click(() => {
				if (kanivin.utils.is_xs() || kanivin.utils.is_sm()) {
	...
	add_action_icon(icon, click, css_class = "", tooltip_label) {
		...
		if (!tooltip_label) {
			...
			tooltip_label = kanivin.unscrub(icon);
		}
	...
	get_icon_label(icon, label) {
		...
		return `${icon ? kanivin.utils.icon(icon_name, size) : ""} <span class="hidden-xs"> ${__(
			label
		)} </span>`;
	}
	...
	set_action(btn, opts) {
		...
		kanivin.ui.keys.get_shortcut_group(this).add(btn, text_span.length ? text_span : btn);
	}
	...
	add_dropdown_item({
		...
		if (icon) {
			$icon = `<span class="menu-item-icon">${kanivin.utils.icon(icon)}</span>`;
		}
		...
		if (shortcut) {
			...
			`);
			kanivin.ui.keys.add_shortcut(shortcut_obj);
		} else {
			$li = $(`
				<li>
					<a class="grey-link dropdown-item" href="#" onClick="return false;">
	...
	set_title(title, icon = null, strip = true, tab_title = "", tooltip_label = "") {
		...
		this.title = title;
		kanivin.utils.set_title(tab_title || title);
		if (icon) {
			title = `${kanivin.utils.icon(icon)} ${title}`;
		}
	...
	add_button(label, click, opts) {
		...
		let button = $(`<button
			class="btn ${opts.btn_class || "btn-default"} ${opts.btn_size || "btn-sm"} ellipsis">
				${opts.icon ? kanivin.utils.icon(opts.icon) : ""}
				${label}
		</button>`);
	...
	add_dropdown_button(parent, label, click, icon) {
		kanivin.ui.toolbar.add_dropdown_button(parent, label, click, icon);
	}
	...
	add_field(df, parent) {
		...
		var f = kanivin.ui.form.make_control({
			df: df,
