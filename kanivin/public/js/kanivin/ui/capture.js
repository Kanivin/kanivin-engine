// kanivin.ui.Capture
// Author - Achilles Rasquinha <achilles@kanivin.io>
 ...
 *
 * @example
 * kanivin._.get_data_uri(video)
 * // returns "data:image/pngbase64,..."
 */
kanivin._.get_data_uri = (element) => {
...
/**
 * @description Kanivin's Capture object.
 *
 * @example
 * const capture = kanivin.ui.Capture()
 * capture.show()
 *
 ...
 */
kanivin.ui.Capture = class {
	constructor(options = {}) {
		this.options = kanivin.ui.Capture.OPTIONS;
	...
	set_options(options) {
		this.options = { ...kanivin.ui.Capture.OPTIONS, ...options };
	...
	show() {
		...
		if (cint(kanivin.boot.sysdefaults.force_web_capture_mode_for_uploads)) {
			this.show_for_desktop();
		} else if (kanivin.is_mobile()) {
	...
	build_dialog() {
		...
		me.$template = $(kanivin.ui.Capture.TEMPLATE);
	...
	show_for_desktop() {
		...
		this.render_stream()
			...
			.catch((err) => {
				if (me.options.error) {
					kanivin.show_alert(kanivin.ui.Capture.ERR_MESSAGE, 3);
				}
	...
	setup_take_photo_action() {
		...
		this.dialog.set_primary_action(__("Take Photo"), () => {
			const data_url = kanivin._.get_data_uri(me.video);
...
kanivin.ui.Capture.OPTIONS = {
	title: __("Camera"),
	animate: false,
	error: false,
};
kanivin.ui.Capture.ERR_MESSAGE = __("Unable to load camera.");
kanivin.ui.Capture.TEMPLATE = `
<div class="kanivin-capture">
	<div class="embed-responsive embed-responsive-16by9 fc-stream-container">
		<video class="fc-stream embed-responsive-item">${kanivin.ui.Capture.ERR_MESSAGE}</video>
	</div>
