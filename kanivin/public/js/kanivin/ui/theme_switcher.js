...
kanivin.ui.ThemeSwitcher = class ThemeSwitcher {
	constructor() {
		this.setup_dialog();
		this.refresh();
	}
	...
	bind_events() {
		this.dialog.$wrapper.on("keydown", (e) => {
			...
			const key = kanivin.ui.keys.get_key(e);
			let increment_by;
	...
	toggle_theme(theme) {
		...
		kanivin.show_alert(__("Theme Changed"), 3);

		kanivin.xcall("kanivin.core.doctype.user.user.switch_theme", {
			theme: toTitle(theme),
		});
	}
...
kanivin.ui.add_system_theme_switch_listener = () => {
	kanivin.ui.dark_theme_media_query.addEventListener("change", () => {
		kanivin.ui.set_theme();
	});
};

kanivin.ui.dark_theme_media_query = window.matchMedia("(prefers-color-scheme: dark)");

kanivin.ui.set_theme = (theme) => {
	const root = document.documentElement;
	let theme_mode = root.getAttribute("data-theme-mode");
	if (!theme) {
		if (theme_mode === "automatic") {
			theme = kanivin.ui.dark_theme_media_query.matches ? "dark" : "light";
		}
	}
