kanivin.ui.SortSelector = class SortSelector {
	// parent:
	// change:
	// args:
	...
	make() {
		...
		this.wrapper = $(kanivin.render_template("sort_selector", this.args)).appendTo(this.parent);
		this.bind_events();
	}
	bind_events() {
		var me = this;
	...
	set_value(sort_by, sort_order) {
		...
		if (this.sort_order !== sort_order) {
			...
			$icon.html(kanivin.utils.icon(icon_name, "sm"));
		}
	}
	prepare_args() {
		var me = this;
		...
		if (!this.args.options) {
			...
			meta.fields.forEach(function (df) {
				if (
					...
					kanivin.perm.has_perm(me.doctype, df.permlevel, "read")
				) {
	...
	get_label(fieldname) {
		if (fieldname === "idx") {
			...
			return this.labels[fieldname] || kanivin.meta.get_label(this.doctype, fieldname);
		}
	}
	get_sql_string() {
