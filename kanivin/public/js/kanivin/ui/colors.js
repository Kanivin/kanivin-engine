...
kanivin.ui.color = {
	get: function (color_name, shade) {
		...
		return kanivin.ui.color_map;
	},
	get_color: function (color_name) {
		const color_names = Object.keys(kanivin.ui.color_map);
		if (color_names.includes(color_name)) {
			return kanivin.ui.color_map[color_name];
		} else {
	...
	all: function () {
		return Object.values(kanivin.ui.color_map).reduce((acc, curr) => acc.concat(curr), []);
	},
	names: function () {
		return Object.keys(kanivin.ui.color_map);
	},
	...
	get_color_name: function (hex) {
		for (const key in kanivin.ui.color_map) {
			const colors = kanivin.ui.color_map[key];
...
kanivin.ui.color_map = kanivin.ui.color.get_color_map();
