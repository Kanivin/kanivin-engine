kanivin.ui.LinkPreview = class {
	...
	identify_doc() {
		if (this.is_link) {
			...
			this.name = kanivin.utils.unescape_html(this.element.attr("data-name"));
	...
	setup_popover_control(e) {
		if (!(kanivin.boot.link_preview_doctypes || []).includes(this.doctype)) {
			return;
		}
	...
	get_preview_data() {
		return kanivin.xcall("kanivin.desk.link_preview.get_preview_data", {
	...
	get_image_html(preview_data) {
		let avatar_html = kanivin.get_avatar(
			"avatar-medium",
	...
	get_content_html(preview_data) {
		...
		Object.keys(preview_data).forEach((key) => {
			if (!["preview_image", "preview_title", "name"].includes(key)) {
				let value = kanivin.ellipsis(preview_data[key], 280);
				let label = key;
				content_html += `
