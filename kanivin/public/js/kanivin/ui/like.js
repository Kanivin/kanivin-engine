...
kanivin.ui.is_liked = function (doc) {
	return kanivin.ui.get_liked_by(doc).includes(kanivin.session.user);
};

kanivin.ui.get_liked_by = function (doc) {
	return doc._liked_by ? JSON.parse(doc._liked_by) : [];
};

kanivin.ui.toggle_like = function ($btn, doctype, name, callback) {
	const add = $btn.hasClass("not-liked") ? "Yes" : "No";
	// disable click
	$btn.css("pointer-events", "none");

	kanivin.call({
		method: "kanivin.desk.like.toggle_like",
		quiet: true,
		args: {
			doctype: doctype,
			name: name,
		...
		callback: function (r) {
			...
			if (doc) {
				let liked_by = kanivin.ui.get_liked_by(doc);

				if (add === "Yes" && !liked_by.includes(kanivin.session.user)) {
					liked_by.push(kanivin.session.user);
				}

				if (add === "No" && liked_by.includes(kanivin.session.user)) {
					liked_by = liked_by.filter((user) => user !== kanivin.session.user);
				}
...
kanivin.ui.click_toggle_like = function () {
	console.warn("`kanivin.ui.click_toggle_like` is deprecated and has no effect.");
};

kanivin.ui.setup_like_popover = ($parent, selector) => {
	if (kanivin.dom.is_touchscreen()) {
		return;
	}
			...
			content: () => {
				...
				liked_by.forEach((user) => {
					...
					liked_by_list.append(`
						<li data-user=${user}>${kanivin.avatar(user, "avatar-xs")}
							<span>${kanivin.user.full_name(user)}</span>
						</li>
					`);
				});
				...
				liked_by_list.children("li").click((ev) => {
					...
					kanivin.set_route(link_base + user);
				});
