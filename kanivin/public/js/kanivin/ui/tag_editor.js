kanivin.ui.TagEditor = class TagEditor {
	constructor(opts) {
		/* docs:
		Arguments
	...
	setup_tags() {
		...
		this.tags = new kanivin.ui.Tags({
			parent: this.wrapper,
			placeholder: '<svg class="es-icon icon-sm"><use href="#es-line-add"></use></svg>',
			onTagAdd: (tag) => {
				if (me.initialized && !me.refreshing) {
					return kanivin.call({
						method: "kanivin.desk.doctype.tag.tag.add_tag",
			...
			onTagRemove: (tag) => {
				if (!me.refreshing) {
					return kanivin.call({
						method: "kanivin.desk.doctype.tag.tag.remove_tag",
						args: me.get_args(tag),
						callback: function (r) {
	...
	setup_awesomplete() {
		...
		$input.on("input", function (e) {
			...
			kanivin.call({
				method: "kanivin.desk.doctype.tag.tag.get_tags",
				args: {
