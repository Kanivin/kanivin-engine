...
kanivin.ui.open_dialogs = [];

kanivin.ui.Dialog = class Dialog extends kanivin.ui.FieldGroup {
	constructor(opts) {
		super();
		this.display = false;
		this.is_dialog = true;
	...
	make() {
		this.$wrapper = kanivin.get_modal("", "");
		...
		this.$wrapper
			.on("hide.bs.modal", function () {
				...
				kanivin.ui.form.get_open_grid_form?.()?.hide_form();

				if (kanivin.ui.open_dialogs[kanivin.ui.open_dialogs.length - 1] === me) {
					kanivin.ui.open_dialogs.pop();
					if (kanivin.ui.open_dialogs.length) {
						window.cur_dialog =
							kanivin.ui.open_dialogs[kanivin.ui.open_dialogs.length - 1];
					} else {
						window.cur_dialog = null;
					}
				}
			...
			.on("shown.bs.modal", function () {
				...
				kanivin.ui.open_dialogs.push(me);
				me.focus_on_first_input();
				me.hide_scrollbar(true);
				me.on_page_show && me.on_page_show();
				$(document).trigger("kanivin.ui.Dialog:shown");
	...
	toggle_minimize() {
		...
		this.get_minimize_btn().html(kanivin.utils.icon(icon));
