...
kanivin.provide("kanivin.ui.keys.handlers");

kanivin.ui.keys.setup = function () {
	$(window).on("keydown", function (e) {
		var key = kanivin.ui.keys.get_key(e);
		if (kanivin.ui.keys.handlers[key]) {
			var out = null;
			for (var i = 0, l = kanivin.ui.keys.handlers[key].length; i < l; i++) {
				var handler = kanivin.ui.keys.handlers[key][i];
...
kanivin.ui.keys.standard_shortcuts = standard_shortcuts;
kanivin.ui.keys.add_shortcut = ({
	...
	kanivin.ui.keys.off(shortcut, page);
	// attach new handler
	kanivin.ui.keys.on(shortcut, handler);
...
kanivin.ui.keys.show_keyboard_shortcut_dialog = () => {
	if (kanivin.ui.keys.is_dialog_shown) return;
		...
		let html = shortcuts
			...
			.map((shortcut) => {
				let shortcut_label = shortcut.shortcut
					.split("+")
					.map(kanivin.utils.to_title_case)
					.join("+");
				if (kanivin.utils.is_mac()) {
	...
	let dialog = new kanivin.ui.Dialog({
		...
		on_hide() {
			kanivin.ui.keys.is_dialog_shown = false;
		},
	});
	...
	dialog.show();
	kanivin.ui.keys.is_dialog_shown = true;
};

kanivin.ui.keys.get_key = function (e) {
	var keycode = e.keyCode || e.which;
	var key = kanivin.ui.keys.key_map[keycode] || String.fromCharCode(keycode);
...
kanivin.ui.keys.on = function (key, handler) {
	if (!kanivin.ui.keys.handlers[key]) {
		kanivin.ui.keys.handlers[key] = [];
	}
	kanivin.ui.keys.handlers[key].push(handler);
};

kanivin.ui.keys.off = function (key, page) {
	let handlers = kanivin.ui.keys.handlers[key];
	if (!handlers || handlers.length === 0) return;
	kanivin.ui.keys.handlers[key] = handlers.filter((h) => {
...
kanivin.ui.keys.add_shortcut({
	shortcut: "ctrl+s",
	action: function (e) {
		kanivin.app.trigger_primary_action();
		e.preventDefault();
		return false;
	},
...
kanivin.ui.keys.on("escape", function (e) {
	handle_escape_key();
});

kanivin.ui.keys.on("esc", function (e) {
	handle_escape_key();
});

kanivin.ui.keys.on("enter", function (e) {
	if (window.cur_dialog && cur_dialog.confirm_dialog) {
		cur_dialog.get_primary_btn().trigger("click");
	}
});

kanivin.ui.keys.on("ctrl+down", function (e) {
	var grid_row = kanivin.ui.form.get_open_grid_form();
	grid_row &&
...
kanivin.ui.keys.on("ctrl+up", function (e) {
	var grid_row = kanivin.ui.form.get_open_grid_form();
	grid_row &&
		grid_row.toggle_view(false, function () {
			grid_row.open_prev();
...
"abcdefghijklmnopqrstuvwxyz".split("").forEach((letter, i) => {
	kanivin.ui.keys.key_map[65 + i] = letter;
});
