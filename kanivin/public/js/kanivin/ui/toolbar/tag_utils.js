...
kanivin.tags.utils = {
	get_tags: function (txt) {
		...
		if (!kanivin.tags.tags) {
			kanivin.tags.utils.fetch_tags();
			return [];
		}

		for (let i in kanivin.tags.tags) {
			let tag = kanivin.tags.tags[i];
			let level = kanivin.search.utils.fuzzy_search(txt, tag);
			if (level) {
				out.push({
					type: "Tag",
					label: __("#{0}", [kanivin.search.utils.bolden_match_part(__(tag), txt)]),
					value: __("#{0}", [__(tag)]),
					index: 1 + level,
					match: tag,
					onclick() {
						// Use Global Search Dialog for tag search too.
						kanivin.searchdialog.search.init_search("#".concat(tag), "tags");
					},
				});
			}
		}
	...
	fetch_tags() {
		kanivin.call({
			method: "kanivin.desk.doctype.tag.tag.get_tags_list_for_awesomebar",
			callback: function (r) {
				if (r && r.message) {
					kanivin.tags.tags = $.extend([], r.message);
				}
			},
		});
	},
	...
	get_tag_results: function (tag) {
		...
		return new Promise(function (resolve) {
			kanivin.call({
				method: "kanivin.desk.doctype.tag.tag.get_documents_for_tag",
				args: {
					tag: tag,
				},
				callback: function (r) {
