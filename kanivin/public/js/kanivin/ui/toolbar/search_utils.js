...
kanivin.search.utils = {
	setup_recent: function () {
		this.recent = JSON.parse(kanivin.boot.user.recent || "[]") || [];
	},
	...
	get_recent_pages: function (keywords) {
		...
		kanivin.route_history.forEach(function (route, i) {
			if (route[0] === "Form") {
				...
				values.push([kanivin.route_titles[route.join("/")] || route[0], route]);
			}
		});
		...
		find(values, keywords, function (match) {
			...
			if (route[0] === "Form") {
				...
				out.label = kanivin.utils.escape_html(match[0]).bold();
				out.value = match[0];
			} else {
	...
	get_frequent_links() {
		let options = [];
		kanivin.boot.frequently_visited_links.forEach((link) => {
			const label = kanivin.utils.get_route_label(link.route);
			options.push({
				route: link.route,
				label: label,
				value: label,
	...
	get_search_in_list: function (keywords) {
		...
		if (keywords.split(" ").includes("in") && keywords.slice(-2) !== "in") {
			...
			kanivin.boot.user.can_read.forEach(function (item) {
				if (kanivin.boot.user.can_search.includes(item)) {
	...
	get_creatables: function (keywords) {
		...
		if (firstKeyword.toLowerCase() === __("new")) {
			kanivin.boot.user.can_create.forEach(function (item) {
	...
	get_doctypes: function (keywords) {
		...
		kanivin.boot.user.can_read.forEach(function (item) {
			...
			if (score) {
				target = item;
				if (kanivin.boot.single_types.includes(item)) {
					out.push(option("", ["Form", item, item], 0.05));
				} else if (kanivin.boot.user.can_search.includes(item)) {
					// include 'making new' option
					if (kanivin.boot.user.can_create.includes(item)) {
						var match = item;
						out.push({
							type: "New",
							label: __("New {0}", [search_result.marked_string || __(item)]),
							...
							match: item,
							onclick: function () {
								kanivin.new_doc(match, true);
							},
						});
					}

					out.push(option("List", ["List", item], 0.05));
					if (kanivin.model.can_get_report(item)) {
	...
	get_reports: function (keywords) {
		...
		var out = [];
		var route;
		Object.keys(kanivin.boot.user.all_reports).forEach(function (item) {
			const search_result = me.fuzzy_search(keywords, item, true);
			var level = search_result.score;
			if (level > 0) {
				var report = kanivin.boot.user.all_reports[item];
	...
	get_workspaces: function (keywords) {
		var me = this;
		var out = [];
		kanivin.boot.allowed_workspaces.forEach(function (item) {
			...
			if (level > 0) {
				var ret = {
					...
					index: level,
					route: [kanivin.router.slug(item.name)],
				};
	...
	get_dashboards: function (keywords) {
		var me = this;
		var out = [];
		kanivin.boot.dashboards.forEach(function (item) {
			const search_result = me.fuzzy_search(keywords, item.name, true);
			var level = search_result.score;
			if (level > 0) {
				var ret = {
	...
	get_global_results: function (keywords, start, limit, doctype = "") {
		...
		return new Promise(function (resolve, reject) {
			kanivin.call({
				method: "kanivin.utils.global_search.search",
				args: {
					text: keywords,
					start: start,
					limit: limit,
	...
	get_marketplace_apps: function (keywords) {
		var me = this;
		var out = [];
		kanivin.boot.marketplace_apps.forEach(function (item) {
			...
			if (search_result.score > 0) {
				var ret = {
					...
					route: [
						`https://kanivincloud.com/${item.route}?utm_source=awesomebar`,
						item.name,
					],
				};
