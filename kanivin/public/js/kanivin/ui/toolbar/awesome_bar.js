...
kanivin.provide("kanivin.tags");

kanivin.search.AwesomeBar = class AwesomeBar {
	setup(element) {
		var me = this;
		...
		var awesomplete = new Awesomplete(input, {
			...
			item: function (item, term) {
				...
				let target = "#";
				if (d.route) {
					target = kanivin.router.make_url(
						kanivin.router.convert_from_standard_route(
							kanivin.router.get_route_from_arguments(
			...
			kanivin.utils.debounce(function (e) {
				...
				if (txt && txt.length > 1) {
					...
					me.options = me.options.concat(
						me.deduplicate(kanivin.search.utils.get_recent_pages(txt || ""))
					);
					me.options = me.options.concat(kanivin.search.utils.get_frequent_links());
				}
				me.add_help();
		...
		$input.on("awesomplete-select", function (e) {
			...
			if (item.route_options) {
				kanivin.route_options = item.route_options;
			}
		...
		kanivin.search.utils.setup_recent();
	}
	...
	add_help() {
		this.options.push({
			...
			onclick: function () {
				...
				kanivin.msgprint(txt, __("Search Help"));
			},
		});
	}
	...
	build_options(txt) {
		var options = kanivin.search.utils
			.get_creatables(txt)
			.concat(
				kanivin.search.utils.get_search_in_list(txt),
				kanivin.search.utils.get_doctypes(txt),
				kanivin.search.utils.get_reports(txt),
				kanivin.search.utils.get_pages(txt),
				kanivin.search.utils.get_workspaces(txt),
				kanivin.search.utils.get_dashboards(txt),
				kanivin.search.utils.get_recent_pages(txt || ""),
				kanivin.search.utils.get_executables(txt),
				kanivin.search.utils.get_marketplace_apps(txt)
			);
		if (txt.charAt(0) === "#") {
			options = kanivin.tags.utils.get_tags(txt);
		}
	...
	make_global_search(txt) {
		...
		// 		<span class="ellipsis">Search for ${kanivin.utils.xss_sanitise(txt).bold()}</span>
		// 		<kbd>↵</kbd>
		// 	</span>
		// `);

		// search_text.click(() => {
		// 	kanivin.searchdialog.search.init_search(txt, "global_search");
		// });
		...
		this.options.push({
			label: `
				<span class="flex justify-between text-medium">
					<span class="ellipsis">${__("Search for {0}", [kanivin.utils.xss_sanitise(txt).bold()])}</span>
					<kbd>↵</kbd>
				</span>
			`,
	...
	make_search_in_current(txt) {
		...
		if (route[0] === "List" && txt.indexOf(" in") === -1) {
			...
			const doctype = kanivin.container.page?.list_view?.doctype;
			if (!doctype) return;
			...
			this.options.push({
				label: __("Find {0} in {1}", [
					kanivin.utils.xss_sanitise(txt).bold(),
					__(route[1]).bold(),
				]),
		...
		if (first == parseInt(first) || first === "(" || first === "=") {
			...
			try {
				...
				this.options.push({
					...
					default: "Calculator",
					onclick: function () {
						kanivin.msgprint(formatted_value, __("Result"));
					},
				});
			} catch (e) {
				// pass
	...
	make_random(txt) {
		if (txt.toLowerCase().includes("random")) {
			this.options.push({
				...
				value: kanivin.utils.get_random(16),
				onclick: function () {
					kanivin.msgprint(kanivin.utils.get_random(16), __("Result"));
				},
			});
		}
	}
