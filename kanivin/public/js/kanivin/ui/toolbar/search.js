...
kanivin.search.SearchDialog = class {
	constructor(opts) {
		$.extend(this, opts);
		this.make();
	}
	...
	make() {
		this.search_dialog = new kanivin.ui.Dialog({
			minimizable: true,
			size: "large",
		});
		this.set_header();
		...
		this.searches = {
			global_search: {
				...
				get_results: (keywords, callback) => {
					let start = 0,
						limit = 100;
					let results = kanivin.search.utils.get_nav_results(keywords);
					kanivin.search.utils.get_global_results(keywords, start, limit).then(
						(global_results) => {
			...
			tags: {
				...
				get_results: (keywords, callback) => {
					var results = kanivin.search.utils.get_nav_results(keywords);
					kanivin.tags.utils.get_tag_results(keywords).then(
						(global_results) => {
							results = results.concat(global_results);
							callback(results, keywords);
						},
	...
	render_data(result_sets) {
		let $search_results = $(kanivin.render_template("search")).addClass("hide");
	...
	get_link(result) {
		let link = "";
		if (result.route) {
			link = `href="${kanivin.router.make_url(result.route)}"`;
	...
	render_result(type, result) {
		...
		if (result.image !== undefined) {
			let avatar_html = kanivin.get_avatar("avatar-medium", result.label, result.image);
	...
	handle_result_click(result, $result) {
		if (result.route_options) {
			kanivin.route_options = result.route_options;
		}
		$result.on("click", () => {
			...
			if (result.onclick) {
				...
				kanivin.set_route(result.route);
