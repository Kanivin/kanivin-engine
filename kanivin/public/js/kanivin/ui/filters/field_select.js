...
kanivin.ui.FieldSelect = class FieldSelect {
	...
	build_options() {
		var me = this;
		me.table_fields = [];
		var std_filters = $.map(kanivin.model.std_fields, function (d) {
		...
		// main table
		var main_table_fields = std_filters.concat(kanivin.meta.docfield_list[me.doctype]);
		$.each(kanivin.utils.sort(main_table_fields, "label", "string"), function (i, df) {
			let doctype =
				kanivin.get_meta(me.doctype).istable && me.parent_doctype
					? me.parent_doctype
					: me.doctype;

			// show fields where user has read access and if report hide flag is not set
			if (kanivin.perm.has_perm(doctype, df.permlevel, "read")) me.add_field_option(df);
		});

		// child tables
		$.each(me.table_fields, function (i, table_df) {
			if (table_df.options) {
				let child_table_fields = [].concat(kanivin.meta.docfield_list[table_df.options]);

				if (table_df.fieldtype === "Table MultiSelect") {
					const link_field = kanivin.meta
						.get_docfields(table_df.options)
						.find((df) => df.fieldtype === "Link");
					child_table_fields = link_field ? [link_field] : [];
				}

				$.each(kanivin.utils.sort(child_table_fields, "label", "string"), function (i, df) {
					let doctype =
						kanivin.get_meta(me.doctype).istable && me.parent_doctype
							? me.parent_doctype
							: me.doctype;
					...
					// show fields where user has read access and if report hide flag is not set
					if (kanivin.perm.has_perm(doctype, df.permlevel, "read"))
	...
	add_field_option(df) {
		...
		if (df.fieldname == "docstatus" && !kanivin.model.is_submittable(me.doctype)) return;
		...
		if (
			kanivin.model.no_value_type.indexOf(df.fieldtype) == -1 &&
