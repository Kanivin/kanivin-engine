kanivin.ui.Filter = class {
	constructor(opts) {
		...
		this.utils = kanivin.ui.filter_utils;
		this.set_conditions();
	...
	set_conditions_from_config() {
		if (kanivin.boot.additional_filters_config) {
			this.filters_config = kanivin.boot.additional_filters_config;
	...
	make() {
		this.filter_edit_area = $(
			kanivin.render_template("edit_filter", {
	...
	make_select() {
		this.fieldselect = new kanivin.ui.FieldSelect({
	...
	set_field(doctype, fieldname, fieldtype, condition) {
		...
		if (
			...
			if (this.filters_config[condition].data) {
				...
				kanivin.xcall(this.filters_config[condition].get_field, args).then((field) => {
	...
	make_field(df, old_fieldtype) {
		...
		let f = kanivin.ui.form.make_control({
			df: df,
			parent: field_area,
			only_input: true,
		});
	...
	toggle_nested_set_conditions(df) {
		let show_condition =
			df.fieldtype === "Link" && kanivin.boot.nested_set_doctypes.includes(df.options);
...
kanivin.ui.filter_utils = {
	get_formatted_value(field, value) {
		...
		return kanivin.format(value, field.df, { only_value: 1 });
	},
	...
	get_selected_value(field, condition) {
		...
		} else if (kanivin.boot.additional_filters_config[condition]) {
			val = field.value || val;
		}
		if (val === "%") {
			val = "";
	...
	get_default_condition(df) {
		const meta = kanivin.get_meta(df.parent);
