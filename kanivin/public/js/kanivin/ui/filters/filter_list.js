kanivin.ui.FilterGroup = class {
	...
	add_filters(filters) {
		...
		return kanivin.run_serially(promises).then(() => this.update_filters());
	}
	...
	_push_new_filter(doctype, fieldname, condition, value, hidden = false) {
		...
		let filter = new kanivin.ui.Filter(args);
	...
	filter_exists(filter_value) {
		...
		return this.filters
			...
			.some((f) => {
				...
				return kanivin.utils.arrays_equal(f_value.slice(0, 4), filter_value.slice(0, 4));
			});
	}
