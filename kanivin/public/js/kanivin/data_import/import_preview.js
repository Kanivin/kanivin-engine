...
kanivin.data_import.ImportPreview = class ImportPreview {
		...
		this.data = this.data.map((row) => {
			return row.map((cell) => {
				...
				if (typeof cell === "string") {
					cell = kanivin.utils.xss_sanitise(cell);
				}
				return cell;
			});
		});
	...
	setup_styles() {
		...
		this.datatable.style.setStyle(`svg.import-success`, {
			width: "16px",
			fill: kanivin.ui.color.get_color_shade("green", "dark"),
		});
		...
		this.datatable.style.setStyle(row_classes, {
			pointerEvents: "none",
			backgroundColor: kanivin.ui.color.get_color_shade("gray", "extra-light"),
			color: kanivin.ui.color.get_color_shade("gray", "dark"),
		});
	}
	...
	show_column_warning(_, $target) {
		...
		kanivin.utils.scroll_to($warning, true, 30);
	}
...
function get_fields_as_options(doctype, column_map) {
	let keys = [doctype];
	kanivin.meta.get_table_fields(doctype).forEach((df) => {
	...
	return [].concat(
		...keys.map((key) => {
			return column_map[key].map((df) => {
				...
				if (doctype !== key) {
					let table_field = kanivin.meta.get_docfield(doctype, key);
