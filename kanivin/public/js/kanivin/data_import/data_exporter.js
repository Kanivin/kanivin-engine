kanivin.provide("kanivin.data_import");

kanivin.data_import.DataExporter = class DataExporter {
	...
	make_dialog() {
		this.dialog = new kanivin.ui.Dialog({
			...
			fields: [
				...
				...kanivin.meta.get_table_fields(this.doctype).map((df) => {
	...
	export_records() {
		let method = "/api/method/kanivin.core.doctype.data_import.data_import.download_template";
	...
	make_select_all_buttons() {
		...
		`);
		kanivin.utils.bind_actions_with_object($select_all_buttons, this);
	...
	select_mandatory() {
		let mandatory_table_fields = kanivin.meta
	...
	update_record_count_message() {
		...
		let count_method = {
			all: () => kanivin.db.count(this.doctype),
			by_filter: () =>
				kanivin.db.count(this.doctype, {
					filters: this.get_filters(),
				}),
	...
	get_multicheck_options(doctype, child_fieldname = null) {
		...
		if (meta.autoname && meta.autoname.startsWith("field:")) {
			...
			autoname_field = kanivin.meta.get_field(doctype, fieldname);
		}
...
export function get_columns_for_picker(doctype) {
	...
	const exportable_fields = (df) => {
		let keep = true;
		if (kanivin.model.no_value_type.includes(df.fieldtype)) {
			keep = false;
		}
	...
	// parent
	let doctype_fields = kanivin.meta.get_docfields(doctype).filter(exportable_fields);
	...
	// children
	const table_fields = kanivin.meta.get_table_fields(doctype);
	table_fields.forEach((df) => {
		const cdt = df.options;
		const child_table_fields = kanivin.meta.get_docfields(cdt).filter(exportable_fields);
