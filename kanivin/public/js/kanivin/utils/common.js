...
kanivin.avatar = function (
	...
	if (user) {
		// desk
		user_info = kanivin.user_info(user);
	} else {
		// website
		let full_name = title || kanivin.get_cookie("full_name");
		user_info = {
			image: image_url === null ? kanivin.get_cookie("user_image") : image_url,
			fullname: full_name,
			abbr: kanivin.get_abbr(full_name),
			color: kanivin.get_palette(full_name),
		};
	}
...
kanivin.get_avatar = function (css_class, title, image_url = null, remove_color, data_attributes) {
	if (!css_class) {
	...
	if (image_url) {
		...
		let abbr = kanivin.get_abbr(title);
		let style = "";
		if (!remove_color) {
			let color = kanivin.get_palette(title);
...
kanivin.avatar_group = function (users, limit = 4, options = {}) {
	...
	let html = display_users
		.map((user) =>
			kanivin.avatar(user, "avatar-small " + css_class, null, null, false, options.filterable)
		)
		.join("");
		...
		html = `
			...
			<span class="avatar avatar-small ${css_class}">
				<div class="avatar-frame standard-image avatar-extra-count"
					title="${extra_users.map((u) => kanivin.user_info(u).fullname).join(", ")}">
					+${extra_users.length}
				</div>
			</span>
		`;
		...
		avatar_action_html = `
			<span class="avatar avatar-small">
				<div class="avatar-frame avatar-action">
					${kanivin.utils.icon(options.action_icon, "sm")}
				</div>
			</span>
		`;
	}
...
kanivin.ui.scroll = function (element, animate, additional_offset) {
...
kanivin.get_palette = function (txt) {
	if (!txt) return kanivin.palette[8]; // breaks when undefined
	var idx = cint((parseInt(md5(txt).substr(4, 2), 16) + 1) / 5.33);
	return kanivin.palette[idx % 8];
};

kanivin.get_abbr = function (txt, max_length) {
	if (!txt) return "";
	var abbr = "";
...
kanivin.gravatars = {};
kanivin.get_gravatar = function (email_id, size = 0) {
	var param = size ? "s=" + size : "d=retro";
	if (!kanivin.gravatars[email_id]) {
		// TODO: check if gravatar exists
		kanivin.gravatars[email_id] =
			"https://secure.gravatar.com/avatar/" + md5(email_id) + "?" + param;
	}
	return kanivin.gravatars[email_id];
};
...
kanivin.get_cookie = function getCookie(name) {
	return kanivin.get_cookies()[name];
};

kanivin.get_cookies = function getCookies() {
...
kanivin.is_mobile = function () {
	return $(document).width() < 768;
};

kanivin.is_large_screen = function () {
	return $(document).height() > 1180;
};

kanivin.utils.xss_sanitise = function (string, options) {
...
kanivin.utils.sanitise_redirect = (url) => {
	const is_external = (() => {
		return (url) => {
			function domain(url) {
	...
	url = kanivin.utils.strip_url(url);

	return is_external(url)
		? ""
		: sanitise_javascript(kanivin.utils.xss_sanitise(url, { strategies: ["js"] }));
};

kanivin.utils.strip_url = (url) => {
...
kanivin.utils.new_auto_repeat_prompt = function (frm) {
	const fields = [
		{
			fieldname: "frequency",
			fieldtype: "Select",
		...
		},
		{
			...
			label: __("Start Date"),
			reqd: 1,
			default: kanivin.datetime.nowdate(),
		},
		{
			fieldname: "end_date",
			fieldtype: "Date",
	...
	kanivin.prompt(
		...
		function (values) {
			kanivin.call({
				method: "kanivin.automation.doctype.auto_repeat.auto_repeat.make_auto_repeat",
				args: {
...
kanivin.utils.get_page_view_count = function (route) {
	return kanivin.call("kanivin.website.doctype.web_page_view.web_page_view.get_page_view_count", {
		path: route,
	});
};
