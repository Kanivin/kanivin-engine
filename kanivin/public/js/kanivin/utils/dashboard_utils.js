kanivin.dashboard_utils = {
	render_chart_filters: function (filters, button_class, container, append) {
		filters.forEach((filter) => {
			...
			if (filter.icon) {
				icon_html = kanivin.utils.icon(filter.icon);
			}
	...
	get_filters_for_chart_type: function (chart) {
		if (chart.chart_type === "Custom" && chart.source) {
			...
			return kanivin.xcall(method, { name: chart.source }).then((config) => {
				kanivin.dom.eval(config);
				return kanivin.dashboards.chart_sources[chart.source].filters;
			});
		} else if (chart.chart_type === "Report" && chart.report_name) {
			return kanivin.report_utils.get_report_filters(chart.report_name).then((filters) => {
				return filters;
			});
		} else {
	...
	create_dashboard_settings() {
		return kanivin
			.xcall(
				"kanivin.desk.doctype.dashboard_settings.dashboard_settings.create_dashboard_settings",
				{
	...
	get_years_since_creation(creation) {
		...
		let current_year = this.get_year(kanivin.datetime.now_date());
		let years_list = [];
		...
		let fields = [
			{
				...
				options: `<div>
						...
						<p>${__("For example:")}
							<code>kanivin.defaults.get_user_default("Company")</code>
						</p>
					</div>`,
			},
		];
		...
		if (Array.isArray(dynamic_filters)) {
			dynamic_filters.forEach((f) => {
				try {
					...
					kanivin.throw(__("Invalid expression set in filter {0} ({1})", [f[1], f[0]]));
				}
			});
			filters = [...filters, ...dynamic_filters];
		} else {
			for (let key of Object.keys(dynamic_filters)) {
				try {
					...
					kanivin.throw(__("Invalid expression set in filter {0}", [key]));
				}
			}
	...
	get_add_to_dashboard_dialog(docname, doctype, method) {
		...
		const dialog = new kanivin.ui.Dialog({
			...
			primary_action: (values) => {
				values.name = docname;
				values.set_standard = kanivin.boot.developer_mode;
				kanivin.xcall(method, { args: values }).then(() => {
