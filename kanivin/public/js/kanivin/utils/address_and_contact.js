kanivin.provide("kanivin.contacts");

$.extend(kanivin.contacts, {
	...
	render_address_and_contact: function (frm) {
		...
		if (frm.fields_dict["address_html"] && "addr_list" in frm.doc.__onload) {
			$(frm.fields_dict["address_html"].wrapper)
				.html(kanivin.render_template("address_list", frm.doc.__onload))
				.find(".btn-address")
		...
		if (frm.fields_dict["contact_html"] && "contact_list" in frm.doc.__onload) {
			$(frm.fields_dict["contact_html"].wrapper)
				.html(kanivin.render_template("contact_list", frm.doc.__onload))
				.find(".btn-contact")
				.on("click", () => new_record("Contact", frm.doc));
		}
	},
	get_last_doc: function (frm) {
		const reverse_routes = kanivin.route_history.slice().reverse();
	...
	get_address_display: function (frm, address_field, display_field) {
		...
		kanivin
			.xcall("kanivin.contacts.doctype.address.address.get_address_display", {
...
function new_record(doctype, source_doc) {
	...
	return kanivin.new_doc(doctype);
}
