...
window.format_number = function (v, format, decimals) {
	if (!format) {
		...
		if (decimals == null) decimals = cint(kanivin.defaults.get_default("float_precision")) || 3;
	}
...
function format_currency(v, currency, decimals) {
	...
	const show_symbol_on_right =
		kanivin.model.get_value(":Currency", currency, "symbol_on_right") ?? false;

	if (decimals === undefined) {
		decimals = kanivin.boot.sysdefaults.currency_precision || null;
	}
...
function get_currency_symbol(currency) {
	if (kanivin.boot) {
		if (kanivin.boot.sysdefaults && kanivin.boot.sysdefaults.hide_currency_symbol == "Yes")
			return null;

		if (!currency) currency = kanivin.boot.sysdefaults.currency;

		return kanivin.model.get_value(":Currency", currency, "symbol") || currency;
	} else {
		// load in template
		return kanivin.currency_symbols[currency];
	}
}

function get_number_format(currency) {
	let sysdefaults = kanivin?.boot?.sysdefaults;
	return (
		(cint(sysdefaults?.use_number_format_from_currency) &&
			currency &&
			kanivin.model.get_value(":Currency", currency, "number_format")) ||
...
function get_number_format_info(format) {
	var info = kanivin.number_format_info[format];
...
function _round(num, precision, rounding_method) {
	rounding_method =
		rounding_method || kanivin.boot.sysdefaults.rounding_method || "Banker's Rounding (legacy)";
...
function precision(fieldname, doc) {
	if (cur_frm) {
		...
		var df = kanivin.meta.get_docfield(doc.doctype, fieldname, doc.parent || doc.name);
		if (!df) console.log(fieldname + ": could not find docfield in method precision()");
		return kanivin.meta.get_field_precision(df, doc);
	} else {
		return kanivin.boot.sysdefaults.float_precision;
	}
}
...
function round_based_on_smallest_currency_fraction(value, currency, precision) {
	var smallest_currency_fraction_value = flt(
		kanivin.model.get_value(":Currency", currency, "smallest_currency_fraction_value")
	);
