function prettyDate(date, mini) {
	...
	if (typeof date == "string") {
		date = kanivin.datetime.convert_to_user_tz(date);
		date = new Date(
			(date || "")
				.replace(/-/g, "/")
				.replace(/[TZ]/g, " ")
	...
	let diff =
		(new Date(kanivin.datetime.now_datetime().replace(/-/g, "/")).getTime() - date.getTime()) /
		1000;
...
window.comment_when = function (datetime, mini) {
	var timestamp = kanivin.datetime.str_to_user ? kanivin.datetime.str_to_user(datetime) : datetime;
	return (
		'<span class="kanivin-timestamp ' +
		(mini ? " mini" : "") +
		'" data-timestamp="' +
		datetime +
		'" title="' +
...
kanivin.datetime.comment_when = comment_when;
kanivin.datetime.prettyDate = prettyDate;

kanivin.datetime.refresh_when = function () {
	if (jQuery) {
		$(".kanivin-timestamp").each(function () {
