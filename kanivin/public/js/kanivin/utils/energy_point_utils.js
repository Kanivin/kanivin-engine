...
Object.assign(kanivin.energy_points, {
	get_points(points) {
	...
	format_history_log(log) {
		...
		const separator = `<span>&nbsp;-&nbsp;</span>`;
		const route = kanivin.utils.get_form_link(log.reference_doctype, log.reference_name);
		return `<div class="flex">
  			...
  			${log.reason ? separator + log.reason : ""}
  			${separator + kanivin.datetime.comment_when(log.creation)}
  		</div>`;
	},
	get_history_log_message(log) {
		const owner_name = kanivin.user.full_name(log.owner).bold();
	...
	get_form_log_message(log) {
		// redundant code to honor readability and to avoid confusion
		const owner_name = kanivin.user.full_name(log.owner).bold();
		const user = kanivin.user.full_name(log.user).bold();
		if (log.type === "Appreciation") {
