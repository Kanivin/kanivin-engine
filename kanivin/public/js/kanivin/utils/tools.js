...
kanivin.provide("kanivin.tools");

kanivin.tools.downloadify = function (data, roles, title) {
	if (roles && roles.length && !has_common(roles, roles)) {
		kanivin.msgprint(
			__("Export not allowed. You need {0} role to export.", [kanivin.utils.comma_or(roles)])
		);
		return;
	}

	var filename = title + ".csv";
	var csv_data = kanivin.tools.to_csv(data);
...
kanivin.markdown = function (txt) {
	if (!kanivin.md2html) {
		kanivin.md2html = new showdown.Converter();
	}
	...
	return kanivin.md2html.makeHtml(txt);
};

kanivin.tools.to_csv = function (data) {
	var res = [];
