...
kanivin.realtime.on("build_event", (data) => {
	if (data.success) {
			...
			for (let file of changed_files) {
				if (file.includes(".bundle.")) {
					...
					if (parts.length === 2) {
						...
						kanivin.assets._executed = kanivin.assets._executed.filter(
		...
		// update assets json
		kanivin.call("kanivin.sessions.get_boot_assets_json").then((r) => {
			if (r.message) {
				kanivin.boot.assets_json = r.message;

				if (kanivin.hot_update) {
					kanivin.hot_update.forEach((callback) => {
						callback();
					});
				}
			}
