...
kanivin.dom = {
	...
	get_unique_id: function () {
		const id = "unique-" + kanivin.dom.id_count;
		kanivin.dom.id_count++;
		return id;
	},
	set_unique_id: function (ele) {
		...
		var id = "unique-" + kanivin.dom.id_count;
		$ele.attr("id", id);
		kanivin.dom.id_count++;
		return id;
	},
	...
	add: function (parent, newtag, className, cs, innerHTML, onclick) {
		if (parent && parent.substr) parent = kanivin.dom.by_id(parent);
	...
	freeze: function (msg, css_class) {
		...
		kanivin.dom.freeze_count++;
	},
	unfreeze: function () {
		if (!kanivin.dom.freeze_count) return; // anything open?
		kanivin.dom.freeze_count--;
		if (!kanivin.dom.freeze_count) {
	...
	scroll_to_section(section_name) {
		setTimeout(() => {
			...
			if (section.length) {
				...
				kanivin.ui.scroll(section.parent().parent());
			}
		}, 200);
	},
...
kanivin.ellipsis = function (text, max) {
	if (!max) max = 20;
	text = cstr(text);
	if (text.length > max) {
...
kanivin.run_serially = function (tasks) {
...
kanivin.load_image = (src, onload, onerror, preprocess = () => {}) => {
...
kanivin.timeout = (seconds) => {
	return new Promise((resolve) => {
		setTimeout(() => resolve(), seconds * 1000);
	});
};

kanivin.scrub = function (text, spacer = "_") {
	return text.replace(/ /g, spacer).toLowerCase();
};

kanivin.unscrub = function (txt) {
	return kanivin.model.unscrub(txt);
};

kanivin.get_data_pill = (
	label,
	target_id = null,
	remove_action = null,
	image = null,
	...
	let color = "",
		style = "";
	if (colored) {
		color = kanivin.get_palette(label);
	}
...
kanivin.get_modal = function (title, content) {
				...
				<div class="modal-header">
					...
					<div class="modal-actions">
						<button class="btn btn-modal-minimize btn-link hide">
							${kanivin.utils.icon("collapse")}
						</button>
						<button class="btn btn-modal-close btn-link" data-dismiss="modal">
							${kanivin.utils.icon("close-alt", "sm", "close-alt")}
						</button>
					</div>
				</div>
...
kanivin.is_online = function () {
	if (kanivin.boot.developer_mode == 1) {
...
kanivin.create_shadow_element = function (wrapper, html, css, js) {
	let random_id = "custom-block-" + kanivin.utils.get_random(5).toLowerCase();
	...
	class CustomBlock extends HTMLElement {
		constructor() {
			...
			div.innerHTML = kanivin.dom.remove_script_and_style(html);

			// link global desk css
			let link = document.createElement("link");
			link.rel = "stylesheet";
			link.href = kanivin.assets.bundled_asset("desk.bundle.css");
