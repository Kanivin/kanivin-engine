...
export default class ListFilter {
	constructor({ wrapper, doctype }) {
		...
		this.can_add_global = kanivin.user.has_role(["System Manager", "Administrator"]);
		this.filters = [];
		this.make();
		this.bind();
		this.refresh();
	...
	make() {
		...
		this.filter_input = kanivin.ui.form.make_control({
			df: {
				fieldtype: "Data",
		...
		this.is_global_input = kanivin.ui.form.make_control({
			df: {
				fieldtype: "Check",
				label: __("Is Global"),
			},
	...
	filter_template(filter) {
		return `<div class="list-link filter-pill list-sidebar-button btn btn-default" data-name="${
			...
			<a class="remove">${kanivin.utils.icon("close")}</a>
		</div>`;
	}
	...
	save_filter(filter_name) {
		return kanivin.db.insert({
			doctype: "List Filter",
			reference_doctype: this.list_view.doctype,
			filter_name,
			for_user: this.is_global_input.get_value() ? "" : kanivin.session.user,
	...
	remove_filter(name) {
		if (!name) return;
		return kanivin.db.delete_doc("List Filter", name);
	}
