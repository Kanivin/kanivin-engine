...
kanivin.views.ListView = class ListView extends kanivin.views.BaseList {
	static load_last_view() {
		const route = kanivin.get_route();
		const doctype = route[1];

		if (route.length === 2) {
			const user_settings = kanivin.get_user_settings(doctype);
			const last_view = user_settings.last_view;
			kanivin.set_route(
				"list",
				kanivin.router.doctype_layout || doctype,
				kanivin.views.is_valid(last_view) ? last_view.toLowerCase() : "list"
			);
			return true;
		}
		return false;
	...
	constructor(opts) {
		super(opts);
		this.show();
		const meta = kanivin.get_meta(this.doctype);
		this.is_large_table = meta?.is_large_table;

		this.debounced_refresh = kanivin.utils.debounce(
	...
	has_permissions() {
		return kanivin.perm.has_perm(this.doctype, 0, "read");
	}
	...
	check_permissions() {
		if (!this.has_permissions()) {
			kanivin.set_route("");
			kanivin.throw(__("Not permitted to view {0}", [this.doctype]));
		}
	}
	...
	validate_filters(filters) {
		...
		valid_fields = valid_fields.concat(kanivin.model.std_fields_list);
	...
	show_restricted_list_indicator_if_applicable() {
		const match_rules_list = kanivin.perm.get_match_rules(this.doctype);
		if (match_rules_list.length) {
			this.restricted_list = $(
				`<button class="btn btn-xs restricted-button flex align-center">
					${kanivin.utils.icon("restriction", "xs")}
				</button>`
			)
	...
	show_restrictions(match_rules_list = []) {
		kanivin.msgprint(
			kanivin.render_template("list_view_permission_restrictions", {
	...
	async set_fields() {
		...
		await Promise.all(
			fields.map((f) => {
				return new Promise((resolve) => {
					const df =
						typeof f === "string" ? kanivin.meta.get_docfield(this.doctype, f) : f;
					if (
						df &&
						df.fieldtype == "Link" &&
						kanivin.boot.link_title_doctypes.includes(df.options)
					) {
						kanivin.model.with_doctype(df.options, () => {
							const meta = kanivin.get_meta(df.options);
		...
		this.fields.forEach((f) => {
			const df = kanivin.meta.get_docfield(f[1], f[0]);
	...
	patch_refresh_and_load_lib() {
		...
		this.refresh = kanivin.utils.throttle(this.refresh, 1000);
		this.load_lib = new Promise((resolve) => {
			if (this.required_libs) {
				kanivin.require(this.required_libs, resolve);
			} else {
				resolve();
			}
		});
	...
	set_primary_action() {
		if (this.can_create && !kanivin.boot.read_only) {
			const doctype_name = __(kanivin.router.doctype_layout) || __(this.doctype);
	...
	make_new_doc() {
		...
		this.filter_area.get().forEach((f) => {
			if (allowed_filter_types.includes(f[2]) && kanivin.model.is_non_std_field(f[1])) {
				options[f[1]] = f[3];
			}
		});
		kanivin.new_doc(doctype, options);
	}
	...
	setup_columns() {
		...
		const get_df = kanivin.meta.get_docfield.bind(null, this.doctype);
		...
		if (kanivin.has_indicator(this.doctype)) {
			// indicator
			this.columns.push({
				type: "Status",
			});
		...
		this.columns = this.columns.concat(
			fields_in_list_view
				.filter((df) => {
					if (kanivin.has_indicator(this.doctype) && df.fieldname === "status") {
						return false;
					}
	...
	get_no_result_message() {
		...
		let empty_state_image =
			...
			"/assets/kanivin/images/ui-states/list-empty-state.svg";
	...
	before_refresh() {
		if (kanivin.route_options && this.filter_area) {
	...
	before_render() {
		...
		kanivin.model.user_settings.save(this.doctype, "last_view", this.view_name);
		...
		const $columns = this.columns
			.map((col) => {
				let classes = [
					...
					kanivin.model.is_numeric_field(col.df) ? "text-right" : "",
				].join(" ");
		...
		const right_html = `
			...
			<span class="level-item list-liked-by-me hidden-xs">
				<span title="${__("Liked by me")}">
					${kanivin.utils.icon("es-solid-heart", "sm", "like-icon")}
				</span>
			</span>
		`;
	...
	get_column_html(col, doc) {
		...
		let translated_doctypes = kanivin.boot?.translated_doctypes || [];
		...
		const format = () => {
			if (df.fieldtype === "Percent") {
				...
				return kanivin.format(value, df, null, doc);
			}
		};
		...
		const field_html = () => {
			...
			if (strip_html_required) {
				...
				_value =
					typeof value_display === "string"
						? kanivin.utils.escape_html(value_display)
						: value_display;
			}
			...
			if (df.fieldtype === "Image") {
				html = df.options
					...
					: `<div class="missing-image small">
						${kanivin.utils.icon("restriction")}
					</div>`;
			} else if (df.fieldtype === "Select") {
				html = `<span class="filterable indicator-pill ${kanivin.utils.guess_colour(
					_value
				)} ellipsis"
			...
			} else if (kanivin.model.html_fieldtypes.includes(df.fieldtype)) {
				html = `<span class="ellipsis">
					${_value}
				</span>`;
			} else {
				html = `<a class="filterable ellipsis"
					data-filter="${fieldname},=,${kanivin.utils.escape_html(value)}">
					${format()}
				</a>`;
			}

			return `<span class="ellipsis"
				title="${__(label)}: ${kanivin.utils.escape_html(_value)}">
				${html}
			</span>`;
		};
		...
		const css_class = [
			"list-row-col ellipsis",
			class_map[col.type],
			kanivin.model.is_numeric_field(df) ? "text-right" : "",
		].join(" ");
	...
	get_meta_html(doc) {
		...
		if (assigned_users.length) {
			assigned_to = `<div class="list-assignments d-flex align-items-center">
					${kanivin.avatar_group(assigned_users, 3, { filterable: true })[0].outerHTML}
				</div>`;
		}
		...
		if (this.list_view_settings && !this.list_view_settings.disable_comment_count) {
			comment_count = `<span class="comment-count d-flex align-items-center">
				${kanivin.utils.icon("es-line-chat-alt")}
	...
	get_form_link(doc) {
		...
		return `/app/${encodeURIComponent(
			kanivin.router.slug(kanivin.router.doctype_layout || this.doctype)
		)}/${encodeURIComponent(cstr(doc.name))}`;
	}

	get_seen_class(doc) {
		const seen_by = doc._seen ? JSON.parse(doc._seen) : [];
		return seen_by.includes(kanivin.session.user) ? "" : "bold";
	}

	get_like_html(doc) {
		const liked_by = doc._liked_by ? JSON.parse(doc._liked_by) : [];
		const is_liked = liked_by.includes(kanivin.session.user);
		const title = liked_by.map((u) => kanivin.user_info(u).fullname).join(", ");
	...
	get_subject_text(doc, title) {
		...
		if (kanivin.model.html_fieldtypes.includes(subject_field.fieldtype)) {
			// NOTE: this is very slow, so only do it for HTML fields
			return kanivin.utils.html2text(value);
		} else {
			return value;
		}
	}
	...
	get_indicator_html(doc, show_workflow_state) {
		const indicator = kanivin.get_indicator(doc, this.doctype, show_workflow_state);
	...
	get_indicator_dot(doc) {
		const indicator = kanivin.get_indicator(doc, this.doctype);
		if (!indicator) return "";
	...
	get_image_url(doc) {
		...
		if (window.cordova && !kanivin.utils.is_url(url)) {
			url = kanivin.base_url + url;
		}
		return url || null;
	}
	...
	setup_filterable() {
		...
		this.$result.on("click", ".filterable", (e) => {
			...
			const filters_to_apply = filters.map((f) => {
				f = f.split(",");
				if (f[2] === "Today") {
					f[2] = kanivin.datetime.get_today();
				} else if (f[2] == "User") {
					f[2] = kanivin.session.user;
				}
	...
	setup_list_click() {
		this.$result.on("click", ".list-row, .image-view-header, .file-header", (e) => {
			...
			if (link) {
				kanivin.set_route(link.pathname);
				return false;
			}
		});
	}
	...
	setup_like() {
		this.$result.on("click", ".like-action", (e) => {
			...
			kanivin.ui.toggle_like($this, doctype, name);
	...
	setup_realtime_updates() {
		...
		kanivin.realtime.doctype_subscribe(this.doctype);
		kanivin.realtime.off("list_update");
		kanivin.realtime.on("list_update", (data) => {
	...
	disable_realtime_updates() {
		kanivin.realtime.doctype_unsubscribe(this.doctype);
	...
	process_document_refreshes() {
		...
		kanivin.call(call_args).then(({ message }) => {
			if (!message) return;
			const data = kanivin.utils.dict(message.keys, message.values);
	...
	save_view_user_settings(obj) {
		return kanivin.model.user_settings.save(this.doctype, this.view_name, obj);
	}

	on_update() {}

	update_url_with_filters() {
		if (kanivin.get_route_str() == this.page_name && !this.report_name) {
	...
	get_menu_items() {
		...
		if (kanivin.model.can_import(doctype, null, this.meta)) {
			items.push({
				label: __("Import", null, "Button in list view menu"),
				action: () =>
					kanivin.set_route("list", "data-import", {
		...
		if (kanivin.user_roles.includes("System Manager")) {
			items.push({
				...
				action: () =>
					kanivin.set_route("permission-manager", {
						doctype,
					}),
				standard: true,
			});
		...
		if (
			kanivin.model.can_create("Custom Field") &&
			kanivin.model.can_create("Property Setter")
		) {
			items.push({
				...
				action: () => {
					if (!this.meta) return;
					if (this.meta.custom) {
						kanivin.set_route("form", "doctype", doctype);
					} else if (!this.meta.custom) {
						kanivin.set_route("form", "customize-form", {
							doc_type: doctype,
						});
					}
				},
		...
		if (kanivin.user.has_role("System Manager") && kanivin.boot.developer_mode === 1) {
			// edit doctype
			items.push({
				label: __("Edit DocType", null, "Button in list view menu"),
				action: () => kanivin.set_route("form", "doctype", doctype),
				standard: true,
			});
		}
	...
	show_list_settings() {
		kanivin.model.with_doctype(this.doctype, () => {
			new ListSettings({
				listview: this,
				doctype: this.doctype,
				settings: this.list_view_settings,
				meta: kanivin.get_meta(this.doctype),
			});
		});
	}
	...
	get_workflow_action_menu_items() {
		...
		if (kanivin.model.has_workflow(this.doctype)) {
			const actions = kanivin.workflow.get_all_transition_actions(this.doctype);
			actions.forEach((action) => {
				workflow_actions.push({
					...
					action: () => {
						...
						kanivin
							.xcall("kanivin.model.workflow.bulk_workflow_approval", {
	...
	toggle_workflow_actions() {
		if (!kanivin.model.has_workflow(this.doctype)) return;
		...
		kanivin
			.xcall("kanivin.model.workflow.get_common_transition_actions", {
				docs: checked_items,
				doctype: this.doctype,
			})
			.then((actions) => {
	...
	get_actions_menu_items() {
		...
		const is_field_editable = (field_doc) => {
			return (
				field_doc.fieldname &&
				kanivin.model.is_value_type(field_doc) &&
		...
		const bulk_edit = () => {
			return {
				...
				action: () => {
					...
					kanivin.meta.get_docfields(doctype).forEach((field_doc) => {
		...
		// bulk printing
		if (kanivin.model.can_print(doctype)) {
			actions_menu_items.push(bulk_printing());
		}

		// bulk submit
		if (
			kanivin.model.is_submittable(doctype) &&
			has_submit_permission(doctype) &&
			!kanivin.model.has_workflow(doctype)
		) {
			actions_menu_items.push(bulk_submit());
		}

		// bulk cancel
		if (kanivin.model.can_cancel(doctype) && !kanivin.model.has_workflow(doctype)) {
			actions_menu_items.push(bulk_cancel());
		}

		// bulk delete
		if (kanivin.model.can_delete(doctype) && !kanivin.model.has_workflow(doctype)) {
	...
	parse_filters_from_route_options() {
		...
		if (!params.toString() && kanivin.route_options) {
			params = new Map(Object.entries(kanivin.route_options));
		}
		...
		params.forEach((value, field) => {
			...
			if (!doctype) {
				doctype = kanivin.meta.get_doctype_for_field(this.doctype, field);
			}
...
kanivin.get_list_view = (doctype) => {
	let route = `List/${doctype}/List`;
	return kanivin.views.list_view[route];
};
...
class ElementFactory {
	...
	create_like_element(doctype) {
		...
		like.innerHTML = kanivin.utils.icon("es-solid-heart", "sm", "like-icon");
