...
kanivin.views.BaseList = class BaseList {
	constructor(opts) {
	...
	setup_defaults() {
		...
		this.page_title = this.page_title || kanivin.router.doctype_layout || __(this.doctype);
		this.meta = kanivin.get_meta(this.doctype);
		this.settings = kanivin.listview_settings[this.doctype] || {};
		this.user_settings = kanivin.get_user_settings(this.doctype);

		this.start = 0;
		this.page_length = kanivin.is_large_screen() ? 100 : 20;
		this.data = [];
		this.method = "kanivin.desk.reportview.get";

		this.can_create = kanivin.model.can_create(this.doctype);
		this.can_write = kanivin.model.can_write(this.doctype);
	...
	get_list_view_settings() {
		return kanivin
			.call("kanivin.desk.listview.get_list_settings", {
	...
	async set_fields() {
		let fields = [].concat(kanivin.model.std_fields_list, this.meta.title_field);
	...
	_add_field(fieldname, doctype) {
		...
		const is_valid_field =
			kanivin.model.std_fields_list.includes(fieldname) ||
			kanivin.meta.has_field(doctype, fieldname) ||
			fieldname === "_seen";
	...
	set_stats() {
		...
		this.workflow_state_fieldname = kanivin.workflow.get_state_fieldname(this.doctype);
		if (this.workflow_state_fieldname) {
			if (!kanivin.workflow.workflows[this.doctype]["override_status"]) {
	...
	fetch_meta() {
		return kanivin.model.with_doctype(this.doctype);
	}
	...
	setup_page() {
		...
		!this.hide_card_layout && this.page.main.addClass("kanivin-card");
	...
	setup_view_menu() {
		if (kanivin.boot.desk_settings.view_switcher && !this.meta.force_re_route_to_default_view) {
			const icon_map = {
				Image: "image-view",
				List: "list",
				Report: "small-file",
			...
			this.views_list = new kanivin.views.ListViewSelect({
				doctype: this.doctype,
	...
	set_breadcrumbs() {
		kanivin.breadcrumbs.add(this.meta.module, this.doctype);
	}
	...
	setup_side_bar() {
		...
		this.list_sidebar = new kanivin.views.ListSidebar({
			doctype: this.doctype,
			stats: this.stats,
	...
	setup_list_wrapper() {
		this.$kanivin_list = $('<div class="kanivin-list">').appendTo(this.page.main);
	}
	...
	setup_result_area() {
		...
		this.$kanivin_list.append(this.$result);
	}
	...
	setup_no_result_area() {
		...
		`).hide();
		this.$kanivin_list.append(this.$no_result);
	}

	setup_freeze_area() {
		this.$freeze = $('<div class="freeze"></div>').hide();
		this.$kanivin_list.append(this.$freeze);
	}
	...
	setup_paging_area() {
		...
		this.$kanivin_list.append(this.$paging_area);
	...
	get_fields() {
		...
		return this.fields.map((f) => kanivin.model.get_full_column_name(f[0], f[1]));
	}

	get_group_by() {
		let name_field = this.fields && this.fields.find((f) => f[0] == "name");
		if (name_field) {
			return kanivin.model.get_full_column_name(name_field[0], name_field[1]);
		}
		return null;
	}
	...
	refresh() {
		...
		return kanivin.call(args).then((r) => {
			// render
			this.prepare_data(r);
	...
	prepare_data(r) {
		...
		Object.assign(kanivin.boot.user_info, data.user_info);
		delete data.user_info;

		data = !Array.isArray(data) ? kanivin.utils.dict(data.keys, data.values) : data;
...
class FilterArea {
	constructor(list_view) {
		...
		this.debounced_refresh_list_view = kanivin.utils.debounce(
	...
	clear(refresh = true) {
		...
		return kanivin.run_serially(promises).then(() => {
			this.trigger_refresh = true;
			...
			doctype_fields
				.filter(
					(df) =>
						...
						(df.in_standard_filter && kanivin.model.is_value_type(df.fieldtype))
				)
				.map((df) => {
					...
					if (
						df.fieldtype == "Link" &&
						df.options &&
						kanivin.boot.treeviews.includes(df.options)
					) {
						condition = "descendants of (inclusive)";
					}
	...
	make_filter_list() {
			...
			<div class="btn-group">
				<button class="btn btn-default btn-sm filter-button">
					<span class="filter-icon">
						${kanivin.utils.icon("es-line-filter")}
					</span>
				...
				<button class="btn btn-default btn-sm filter-x-button" title="${__("Clear all filters")}">
					<span class="filter-icon">
						${kanivin.utils.icon("es-small-close")}
					</span>
				</button>
			</div>
		...
		this.filter_list = new kanivin.ui.FilterGroup({
...
kanivin.views.is_valid = (view_mode) => kanivin.views.view_modes.includes(view_mode);
