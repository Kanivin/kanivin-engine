...
kanivin.views.ListGroupBy = class ListGroupBy {
	constructor(opts) {
		$.extend(this, opts);
		this.make_wrapper();
	...
	render_group_by_items() {
		let get_item_html = (fieldname) => {
			...
			if (fieldname === "assigned_to") {
				...
				label = kanivin.meta.get_label(this.doctype, fieldname);
				let docfield = kanivin.meta.get_docfield(this.doctype, fieldname);
				if (!docfield) {
					return;
				}
			...
			return `<li class="group-by-field list-link">
					...
					href="#" onclick="return false;">
						...
						<span>${kanivin.utils.icon("select", "xs")}</span>
					</a>
	...
	setup_search($dropdown) {
		kanivin.utils.setup_search($dropdown, ".group-by-item", ".group-by-value", "data-name");
	}
	...
	get_group_by_count(field) {
		...
		return kanivin.call("kanivin.desk.listview.get_group_by_count", args).then((r) => {
			let field_counts = r.message || [];
			field_counts = field_counts.filter((f) => f.count !== 0);
			let current_user = field_counts.find((f) => f.name === kanivin.session.user);
			field_counts = field_counts.filter(
				(f) => !["Guest", "Administrator", kanivin.session.user].includes(f.name)
			);
			// Set kanivin.session.user on top of the list
	...
	get_dropdown_html(field, fieldtype, applied = false) {
		...
		if (field.name == null) {
			label = __("Not Set");
		} else if (field.name === kanivin.session.user) {
			label = __("Me");
		...
		let applied_html = applied
			? `<span class="applied"> ${kanivin.utils.icon("tick", "xs")} </span>`
			: "";
