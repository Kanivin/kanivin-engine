export default class ListSettings {
	constructor({ listview, doctype, meta, settings }) {
		if (!doctype) {
			kanivin.throw("DocType required");
		}
		...
		kanivin.model.with_doctype("List View Settings", () => {
			this.make();
	...
	make() {
		...
		let list_view_settings = kanivin.get_meta("List View Settings");
		...
		me.dialog.set_primary_action(__("Save"), () => {
			...
			kanivin.call({
				method: "kanivin.desk.doctype.list_view_settings.list_view_settings.save_listview_settings",
				args: {
					doctype: me.doctype,
		...
		for (let idx in me.fields) {
			...
			fields += `
				<div class="control-input flex align-center form-control fields_order ${is_sortable}"
					...
					<div class="row">
						<div class="col-1">
							${kanivin.utils.icon("drag", "xs", "", "", "sortable-handle " + show_sortable_handle)}
						</div>
		...
		d.set_primary_action(__("Save"), () => {
			...
			for (let idx in values) {
				...
				if (me.fields.length === parseInt(me.dialog.get_values().total_fields)) {
					...
					let field = kanivin.meta.get_docfield(me.doctype, value);
					if (field) {
						me.fields.push({
	...
	reset_listview_fields(dialog) {
		...
		kanivin
			.xcall(
				"kanivin.desk.doctype.list_view_settings.list_view_settings.get_default_listview_fields",
				{
					doctype: me.doctype,
				}
			)
	...
	set_list_view_fields(meta) {
		...
		meta.fields.forEach((field) => {
			if (
				field.in_list_view &&
				!kanivin.model.no_value_type.includes(field.fieldtype) &&
	...
	set_subject_field(meta) {
		...
		if (meta.title_field) {
			let field = kanivin.meta.get_docfield(me.doctype, meta.title_field.trim());
	...
	set_status_field() {
		...
		if (kanivin.has_indicator(me.doctype)) {
			me.fields.push({
				type: "Status",
				label: __("Status"),
	...
	get_doctype_fields(meta, fields) {
		...
		meta.fields.forEach((field) => {
			if (!kanivin.model.no_value_type.includes(field.fieldtype)) {
