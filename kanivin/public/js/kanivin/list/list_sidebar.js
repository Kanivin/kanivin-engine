...
kanivin.views.ListSidebar = class ListSidebar {
	constructor(opts) {
		$.extend(this, opts);
		this.make();
	}
	...
	make() {
		var sidebar_content = kanivin.render_template("list_sidebar", { doctype: this.doctype });
	...
	setup_views() {
		...
		if (this.doctype === "Communication" && kanivin.boot.email_accounts.length) {
			this.sidebar.find('.list-link[data-view="Inbox"]').removeClass("hide");
			show_list_link = true;
		}

		if (kanivin.treeview_settings[this.doctype] || kanivin.get_meta(this.doctype).is_tree) {
			this.sidebar.find(".tree-link").removeClass("hide");
		}

		this.current_view = "List";
		var route = kanivin.get_route();
		if (route.length > 2 && kanivin.views.view_modes.includes(route[2])) {
	...
	setup_kanban_boards() {
		...
		kanivin.views.KanbanView.setup_dropdown_in_sidebar(this.doctype, $dropdown);
	}

	setup_keyboard_shortcuts() {
		this.sidebar.find(".list-link > a, .list-link > .btn-group > a").each((i, el) => {
			kanivin.ui.keys.get_shortcut_group(this.page).add($(el));
		});
	}

	setup_list_group_by() {
		this.list_group_by = new kanivin.views.ListGroupBy({
			doctype: this.doctype,
			sidebar: this,
			list_view: this.list_view,
			page: this.page,
	...
	get_stats() {
		...
		kanivin.call({
			method: "kanivin.desk.reportview.get_sidebar_stats",
			type: "GET",
			args: {
				stats: me.stats,
				doctype: me.doctype,
			...
			callback: function (r) {
				...
				kanivin.utils.setup_search(stats_dropdown, ".stat-link", ".stat-label");
			},
		});
	}
	...
	render_stat(stats) {
		...
		let tag_list = $(kanivin.render_template("list_sidebar_stat", args)).on(
			"click",
			".stat-link",
			(e) => {
	...
	add_insights_banner() {
		...
		const link = "https://kanivin.io/s/insights";
		const cta = "Kanivin Insights";
	...
	add_crm_banner() {
		...
		const message = "";
		const link =
			"https://kanivin.io/crm?utm_source=crm-sidebar&utm_medium=sidebar&utm_campaign=kanivin-ad";
		const cta = __("Switch to Kanivin CRM for smarter sales");
		this.add_banner(message, link, cta);
	}
	...
	add_helpdesk_banner() {
		...
		const message = "";
		const link =
			"https://kanivin.io/helpdesk?utm_source=support-sidebar&utm_medium=sidebar&utm_campaign=kanivin-ad";
		const cta = __("Upgrade your support experience with Kanivin Helpdesk");
		this.add_banner(message, link, cta);
	}
};
