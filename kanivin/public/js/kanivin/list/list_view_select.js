...
kanivin.views.ListViewSelect = class ListViewSelect {
	constructor(opts) {
		$.extend(this, opts);
	...
	set_current_view() {
		...
		const view_name = kanivin.utils.to_title_case(route[2] || "");
		if (route.length > 2 && kanivin.views.view_modes.includes(view_name)) {
	...
	set_route(view, calendar_name) {
		...
		if (search_params) {
			kanivin.route_options = Object.fromEntries(search_params);
		}
	...
	setup_views() {
		const views = {
			...
			Calendar: {
				condition: kanivin.views.calendar[this.doctype],
			...
			Inbox: {
				condition: this.doctype === "Communication" && kanivin.boot.email_accounts.length,
				action: () => this.set_route("inbox"),
				current_view_handler: () => {
					const accounts = this.get_email_accounts();
					let default_action;
					if (has_common(kanivin.user_roles, ["System Manager", "Administrator"])) {
						default_action = {
							label: __("New Email Account"),
							action: () => kanivin.new_doc("Email Account"),
						};
					}
			...
			Tree: {
				condition:
					kanivin.treeview_settings[this.doctype] ||
					kanivin.get_meta(this.doctype).is_tree,
		...
		kanivin.views.view_modes.forEach((view) => {
	...
	setup_kanban_switcher(kanbans) {
		...
		if (can_create) {
			this.page.add_custom_menu_item(
				...
				() => kanivin.views.KanbanView.show_kanban_dialog(this.doctype),
				true
			);
		}
	}
	...
	get_page_name() {
		return kanivin.utils.to_title_case(kanivin.get_route().slice(-1)[0] || "");
	}
	...
	get_reports() {
		...
		var reports =
			Object.values(kanivin.boot.user.all_reports).sort((a, b) =>
	...
	setup_kanban_boards() {
			...
			kanivin.db.get_value(
				...
				(board) => {
					if (!$.isEmptyObject(board)) {
						kanivin.set_route("list", doctype, "kanban", board.name);
					} else {
						kanivin.views.KanbanView.show_kanban_dialog(doctype);
					}
				}
			);
		}
		...
		const last_opened_kanban =
			kanivin.model.user_settings[this.doctype]["Kanban"]?.last_kanban_board;
		if (!last_opened_kanban) {
			fetch_kanban_board(this.doctype);
		} else {
			kanivin.db.exists("Kanban Board", last_opened_kanban).then((exists) => {
				if (exists) {
					kanivin.set_route("list", this.doctype, "kanban", last_opened_kanban);
				} else {
					fetch_kanban_board(this.doctype);
				}
			});
	...
	get_calendars() {
		...
		return kanivin.db
			...
			.then((result) => {
				...
				if (kanivin.views.calendar[this.doctype]) {
	...
	get_email_accounts() {
		let accounts_to_add = [];
		let accounts = kanivin.boot.email_accounts;
	...
	slug() {
		return kanivin.router.slug(kanivin.router.doctype_layout || this.doctype);
	}
};
