export default class BulkOperations {
	constructor({ doctype }) {
		if (!doctype) kanivin.throw(__("Doctype required"));
		this.doctype = doctype;
	}

	print(docs) {
		const print_settings = kanivin.model.get_doc(":Print Settings", "Print Settings");
		const allow_print_for_draft = cint(print_settings.allow_print_for_draft);
		const is_submittable = kanivin.model.is_submittable(this.doctype);
		...
		if (invalid_docs.length > 0) {
			kanivin.msgprint(__("You selected Draft or Cancelled documents"));
			return;
		}

		if (valid_docs.length === 0) {
			kanivin.msgprint(__("Select atleast 1 record for printing"));
			return;
		}
		...
		const dialog = new kanivin.ui.Dialog({
			...
			fields: [
				...
				},
				{
					...
					fieldname: "print_sel",
					options: kanivin.meta.get_print_formats(this.doctype),
					default: kanivin.get_meta(this.doctype).default_print_format,
				},
				{
					fieldtype: "Select",
					label: __("Page Size"),
					fieldname: "page_size",
					options: kanivin.meta.get_print_sizes(),
		...
		dialog.set_primary_action(__("Print"), (args) => {
			if (!args) return;
			const default_print_format = kanivin.get_meta(this.doctype).default_print_format;
			...
			if (args.page_size === "Custom") {
				if (args.page_height === 0 || args.page_width === 0) {
					kanivin.throw(__("Page height and width cannot be zero"));
				}
			...
			if (args.background_print) {
				kanivin
					.call("kanivin.utils.print_format.download_multi_pdf_async", {
					...
					.then((response) => {
						...
						kanivin.realtime.task_subscribe(task_id);
						kanivin.realtime.on(`task_complete:${task_id}`, (data) => {
							kanivin.msgprint({
							...
							kanivin.realtime.task_unsubscribe(task_id);
							kanivin.realtime.off(`task_complete:${task_id}`);
						});
					});
			} else {
				const w = window.open(
					"/api/method/kanivin.utils.print_format.download_multi_pdf?" +
						"doctype=" +
	...
	get_letterhead_options() {
		...
		kanivin.call({
			method: "kanivin.client.get_list",
			args: {
				doctype: "Letter Head",
	...
	delete(docnames, done = null) {
		kanivin
			.call({
				method: "kanivin.desk.reportview.delete_items",
				freeze: true,
				freeze_message:
					docnames.length <= 10
	...
	assign(docnames, done) {
		if (docnames.length > 0) {
			const assign_to = new kanivin.ui.form.AssignToDialog({
				obj: this,
				method: "kanivin.desk.form.assign_to.add_multiple",
				doctype: this.doctype,
				docname: docnames,
				bulk_assign: true,
				re_assign: true,
			...
			kanivin.msgprint(__("Select records for assignment"));
		}
	}
	...
	clear_assignment(docnames, done) {
		if (docnames.length > 0) {
			kanivin
				.call({
					method: "kanivin.desk.form.assign_to.remove_multiple",
					args: {
						doctype: this.doctype,
						names: docnames,
			...
			kanivin.msgprint(__("Select records for removing assignment"));
		}
	}
	...
	apply_assignment_rule(docnames, done) {
		if (docnames.length > 0) {
			kanivin
				.call("kanivin.automation.doctype.assignment_rule.assignment_rule.bulk_apply", {
					doctype: this.doctype,
					docnames: docnames,
				})
				.then(() => done());
	...
	submit_or_cancel(docnames, action = "submit", done = null) {
		...
		kanivin.realtime.task_subscribe(task_id);
		return kanivin
			.xcall("kanivin.desk.doctype.bulk_update.bulk_update.submit_cancel_or_update_docs", {
				doctype: this.doctype,
				action: action,
				docnames: docnames,
				task_id: task_id,
			...
			.then((failed_docnames) => {
				if (failed_docnames?.length) {
					const comma_separated_records = kanivin.utils.comma_and(failed_docnames);
					switch (action) {
						case "submit":
							kanivin.throw(__("Cannot submit {0}.", [comma_separated_records]));
							break;
						case "cancel":
							kanivin.throw(__("Cannot cancel {0}.", [comma_separated_records]));
							break;
						default:
							kanivin.throw(__("Cannot {0} {1}.", [action, comma_separated_records]));
					}
				}
				if (failed_docnames?.length < docnames.length) {
					kanivin.utils.play_sound(action);
					if (done) done();
				}
			})
			.finally(() => {
	...
	edit(docnames, field_mappings, done) {
		...
		const dialog = new kanivin.ui.Dialog({
			...
			primary_action: ({ value }) => {
				...
				kanivin
					.call({
						method: "kanivin.desk.doctype.bulk_update.bulk_update.submit_cancel_or_update_docs",
						args: {
							doctype: this.doctype,
							freeze: true,
							docnames: docnames,
					...
					.then((r) => {
						...
						dialog.hide();
						kanivin.show_alert(__("Updated successfully"));
					});
			},
		...
		const dialog = new kanivin.ui.Dialog({
			...
			fields: [
				{
					...
					get_data: function (txt) {
						return kanivin.db.get_link_options("Tag", txt);
					},
				},
			],
			...
			primary_action: () => {
				...
				if (args && args.tags) {
					...
					kanivin.call({
						method: "kanivin.desk.doctype.tag.tag.add_tags",
						args: {
							tags: args.tags,
							dt: this.doctype,
							docs: docnames,
	...
	export(doctype, docnames) {
		kanivin.require("data_import_tools.bundle.js", () => {
			const data_exporter = new kanivin.data_import.DataExporter(
				doctype,
				"Insert New Records"
			);
