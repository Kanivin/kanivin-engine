class WidgetDialog {
		...
		this.dialog = new kanivin.ui.Dialog({
			...
			primary_action: (data) => {
				...
				if (!this.editing && !data.name) {
					data.name = `${this.type}-${this.label}-${kanivin.utils.get_random(20)}`;
				}
	...
	get_title() {
		...
		let label = (action = action + " " + kanivin.model.unscrub(this.type));
		return __(label);
	}
...
class QuickListDialog extends WidgetDialog {
	...
	generate_filter_from_json() {
		if (this.values && this.values.quick_list_filter) {
			this.filters = kanivin.utils.get_filter_from_json(
...
class ShortcutDialog extends WidgetDialog {
				...
				onchange: () => {
					if (this.dialog.get_value("type") == "DocType") {
						this.dialog.fields_dict.link_to.get_query = () => {
							return {
								query: "kanivin.core.report.permitted_documents_for_user.permitted_documents_for_user.query_doctypes",
								filters: {
			...
			},
			{
				...
				onchange: () => {
					...
					if (doctype && this.dialog.get_value("type") == "DocType") {
						kanivin.model.with_doctype(doctype, async () => {
							let meta = kanivin.get_meta(doctype);

							if (doctype && kanivin.boot.single_types.includes(doctype)) {
								this.hide_filters();
							} else if (doctype) {
							...
							if (kanivin.boot.calendars.includes(doctype)) views.push("Calendar");

							const response = await kanivin.db.get_value(
								"Kanban Board",
			...
			},
			{
				...
				depends_on: (state) => {
					if (this.dialog) {
						...
						let is_single = kanivin.boot.single_types.includes(doctype);
	...
	process_data(data) {
		...
		data.label = data.label ? data.label : kanivin.model.unscrub(data.link_to);

		if (data.url) {
			let _url = data.url;
			if (data.url.startsWith("/")) {
				_url = kanivin.urllib.get_base_url() + data.url;
			}
			!validate_url(_url) &&
				kanivin.throw({
...
class NumberCardDialog extends WidgetDialog {
			...
			return [
				{
					...
					get_query: () => {
						return {
							query: "kanivin.desk.doctype.number_card.number_card.get_cards_for_user",
							filters: {
	...
	set_aggregate_function_fields() {
		...
		if (this.document_type && kanivin.get_meta(this.document_type)) {
			kanivin.get_meta(this.document_type).fields.map((df) => {
		...
		return [
			{
				...
				get_query: () => {
					return {
						query: "kanivin.desk.doctype.custom_html_block.custom_html_block.get_custom_blocks_for_user",
					};
				},
			},
		];
