...
export default class OnboardingWidget extends Widget {
	async refresh() {
		kanivin.utils.load_video_player();
	...
	add_step(step, index) {
		...
		let $step = $(`<a class="onboarding-step ${status}">
				<div class="step-title">
					<div class="step-index step-pending">${kanivin.utils.icon(
						"es-line-success",
						"md",
						"",
						"",
					...
					<div class="step-index step-skipped">${kanivin.utils.icon(
					...
					<div class="step-index step-complete">${kanivin.utils.icon(
						"es-solid-success",
						"md",
						"",
						"",
	...
	show_step(step) {
		...
		let actions = {
			...
			"Create Entry": (step) => {
				if (step.is_complete) {
					kanivin.set_route(`/app/List/${step.reference_document}`);
				} else {
		...
		const set_description = () => {
			let content = __(step.description)
				? kanivin.markdown(__(step.description))
				: `<h1>${__(step.title)}</h1>`;

			if (step.action === "Create Entry") {
				// add a secondary action to view list
				content += `<p>
					<a href='/app/${kanivin.router.slug(step.reference_document)}'>
		...
		const toggle_video = () => {
			...
			let plyr = new kanivin.Plyr(video[0], {
				hideControls: true,
				resetOnEnd: true,
			});
	...
	go_to_page(step) {
		...
		kanivin.set_route(step.path).then(() => {
			let message =
			...
			let msg_dialog = kanivin.msgprint({
				message: message,
				title: title,
				primary_action: {
					action: () => {
	...
	open_report(step) {
		let route = kanivin.utils.generate_route({
		...
		let current_route = kanivin.get_route();

		kanivin.set_route(route).then(() => {
			let msg_dialog = kanivin.msgprint({
				message: __(step.report_description),
				title: __(step.reference_report),
				primary_action: {
					action: () => {
						kanivin.set_route(current_route).then(() => {
							this.mark_complete(step);
						});
						msg_dialog.hide();
					},
			...
			kanivin.msg_dialog.custom_onhide = () => this.mark_complete(step);
		});
	}
	...
	show_form_tour(step) {
		let route;
		if (step.is_single) {
			route = kanivin.router.slug(step.reference_document);
		} else {
			route = `${kanivin.router.slug(step.reference_document)}/new`;
		}

		let current_route = kanivin.get_route();

		kanivin.route_hooks = {};
		kanivin.route_hooks.after_load = (frm) => {
			const on_finish = () => {
				let msg_dialog = kanivin.msgprint({
	...
	update_settings(step) {
		...
		kanivin.route_hooks.after_save = (frm) => {
			let success = false;
			let args = {};
			...
			if (success) {
				...
				args.secondary_action = {
					action: () => kanivin.set_route(current_route),
					label: __("Go Back"),
				};
			...
			kanivin.msgprint(args);
			kanivin.msg_dialog.custom_onhide = () => custom_onhide();
		};

		kanivin.set_route("Form", step.reference_document);
	}
	...
	async create_entry(step) {
		...
		if (step.is_submittable) {
			kanivin.route_hooks.after_save = () => {
				kanivin.msgprint({
					message: __("Submit this document to complete this step."),
					title: __("Document Saved"),
				});
			};
			kanivin.route_hooks.after_submit = callback;
		} else {
			kanivin.route_hooks.after_save = callback;
		}

		kanivin.set_route("Form", step.reference_document, docname);
	}

	show_quick_entry(step) {
		let current_route = kanivin.get_route_str();
		kanivin.ui.form.make_quick_entry(
			step.reference_document,
			() => {
				if (kanivin.get_route_str() != current_route) {
					let success_dialog = kanivin.msgprint({
						message: __("Let's take you back to onboarding"),
						title: __("Document Saved"),
						primary_action: {
							action: () => {
	...
	show_video(step) {
		kanivin.help.show_video(step.video_url, step.title);
		this.mark_complete(step);
	}
	...
	update_step_status(step, status, value, callback) {
		...
		kanivin
			.call("kanivin.desk.desktop.update_onboarding_step", {
				name: step.name,
				field: status,
				value: value,
			})
	...
	show_success() {
		...
		let success_state_image =
			this.success_state_image || "/assets/kanivin/images/ui-states/success-color.png";
		let documentation = "";
		if (this.docs_url) {
			documentation = __(
	...
	is_dismissed() {
		...
		if (Object.keys(dismissed).includes(this.title)) {
			...
			let today = new Date();
			let diff = kanivin.datetime.get_hour_diff(today, last_hidden);
			return diff < 24;
		}
		return false;
	}
	...
	set_actions() {
		...
		dismiss.on("click", () => {
			let dismissed = JSON.parse(localStorage.getItem("dismissed-onboarding") || "{}");
			dismissed[this.title] = kanivin.datetime.now_datetime();

			localStorage.setItem("dismissed-onboarding", JSON.stringify(dismissed));
			this.delete(true, true);
			this.widget.closest(".ce-block").hide();
			kanivin.telemetry.capture("dismissed_" + kanivin.scrub(this.title), "kanivin_onboarding");
		});
		...
		return kanivin.model
			...
			.then((onboarding_doc) => {
				if (onboarding_doc) {
					...
					const method =
						"kanivin.desk.doctype.onboarding_step.onboarding_step.get_onboarding_steps";
					return kanivin
	...
	async get_first_document(doctype) {
		const { message } = await kanivin.db.get_value(
			"Form Tour",
		...
		if (message.first_document) {
			await kanivin.db.get_list(doctype, { order_by: "creation" }).then((res) => {
