...
kanivin.provide("kanivin.dashboards");
kanivin.provide("kanivin.dashboards.chart_sources");
...
export default class ChartWidget extends Widget {
	...
	render_time_series_filters() {
		...
		kanivin.dashboard_utils.render_chart_filters(filters, "chart-actions", this.action_area, 0);
	}
			...
			filters = [
				{
					...
					options: kanivin.dashboard_utils.get_years_since_creation(
						kanivin.boot.user.creation
					),
	...
	render_date_range_field() {
		if (!this.date_field_wrapper || !this.date_field_wrapper.is(":visible")) {
			...
			this.date_range_field = kanivin.ui.form.make_control({
				df: {
					fieldtype: "DateRange",
					fieldname: "from_date",
	...
	get_report_chart_data(result) {
		if (result.chart && this.chart_doc.use_report_chart) {
			...
			let columns = result.columns.map((col) => {
				return kanivin.report_utils.prepare_field_from_column(col);
			});

			return kanivin.report_utils.make_chart_options(columns, result, chart_fields).data;
		}
	}
	...
	prepare_chart_actions() {
		let actions = [
			...
			},
			{
				...
				action: "action-edit",
				handler: () => {
					kanivin.set_route("Form", "Dashboard Chart", this.chart_doc.name);
				},
			},
			{
		...
		if (this.chart_doc.document_type) {
			actions.push({
				...
				action: "action-list",
				handler: () => {
					kanivin.set_route("List", this.chart_doc.document_type);
				},
			});
			...
			actions.push({
				...
				action: "action-list",
				handler: () => {
					kanivin.set_route("query-report", this.chart_doc.report_name, this.filters);
				},
			});
		}
	...
	setup_filter_dialog(fields) {
		...
		if (this.chart_doc.chart_type == "Report") {
			...
			kanivin.query_report = new kanivin.views.QueryReport({ filters: dialog.fields_list });
			kanivin.query_reports[this.chart_doc.report_name].onload &&
				kanivin.query_reports[this.chart_doc.report_name].onload(kanivin.query_report);
		}
	...
	save_chart_config_for_user(config, reset = 0) {
		...
		kanivin.xcall(
			"kanivin.desk.doctype.dashboard_settings.dashboard_settings.save_chart_config",
			{
				reset: reset,
	...
	create_filter_group_and_add_filters() {
		this.filter_group = new kanivin.ui.FilterGroup({
		...
		this.filters &&
			kanivin.model.with_doctype(this.chart_doc.document_type, () => {
	...
	fetch(filters, refresh = false, args) {
		...
		return kanivin.xcall(method, args);
	}
	...
	async render() {
		let setup_dashboard_chart = () => {
			...
			if (!this.dashboard_chart) {
				this.dashboard_chart = kanivin.utils.make_chart(this.chart_wrapper[0], chart_args);
			} else {
		...
		if (!this.data || !this.data.labels || !Object.keys(this.data).length) {
			...
			if (this.chart_doc.document_type) {
				kanivin.model.with_doctype(this.chart_doc.document_type, setup_dashboard_chart);
			} else {
	...
	get_chart_args() {
		...
		if (this.chart_doc.document_type) {
			let doctype_meta = kanivin.get_meta(this.chart_doc.document_type);
	...
	update_chart_object() {
		kanivin.db.get_doc("Dashboard Chart", this.chart_doc.name).then((doc) => {
			this.chart_doc = doc;
	...
	prepare_chart_object() {
		if (this.chart_doc.type == "Heatmap" && !this.chart_doc.heatmap_year) {
			this.chart_doc.heatmap_year = kanivin.dashboard_utils.get_year(
				kanivin.datetime.now_date()
			);
		}
	...
	set_chart_filters() {
		...
		let chart_saved_filters = kanivin.dashboard_utils.get_all_filters(this.chart_doc);

		if (this.chart_doc.chart_type == "Report") {
			return kanivin.dashboard_utils
				.get_filters_for_chart_type(this.chart_doc)
				.then((filters) => {
					...
					this.filters =
						kanivin.utils.parse_array(user_saved_filters) ||
						kanivin.utils.parse_array(this.filters) ||
						kanivin.utils.parse_array(chart_saved_filters);
				});
		} else {
			this.filters =
				kanivin.utils.parse_array(user_saved_filters) ||
	...
	get_settings() {
		return kanivin.model.with_doc("Dashboard Chart", this.chart_name).then((chart_doc) => {
			if (chart_doc) {
				this.chart_doc = chart_doc;
				if (this.chart_doc.chart_type == "Custom") {
					// custom source
					if (kanivin.dashboards.chart_sources[this.chart_doc.source]) {
						this.settings = kanivin.dashboards.chart_sources[this.chart_doc.source];
						return Promise.resolve();
					} else {
						const method =
							"kanivin.desk.doctype.dashboard_chart_source.dashboard_chart_source.get_config";
						return kanivin
							.xcall(method, { name: this.chart_doc.source })
							.then((config) => {
								kanivin.dom.eval(config);
								this.settings =
									kanivin.dashboards.chart_sources[this.chart_doc.source];
							});
					}
				} else if (this.chart_doc.chart_type == "Report") {
					this.settings = {
					...
					this.settings = {
						method: "kanivin.desk.doctype.dashboard_chart.dashboard_chart.get",
					};
					return Promise.resolve();
				}
			}
