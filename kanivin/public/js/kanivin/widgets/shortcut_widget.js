...
export default class ShortcutWidget extends Widget {
	...
	setup_events() {
		this.widget.click((e) => {
			...
			let filters = kanivin.utils.get_filter_from_json(this.stats_filter);
	...
	set_actions() {
		if (this.in_customize_mode) return;
		let icon_to_append = kanivin.utils.icon("es-line-arrow-up-right", "xs", "", "", "ml-2");
		if (kanivin.utils.is_rtl(kanivin.boot.lang)) {
			icon_to_append = kanivin.utils.icon("es-line-arrow-up-left", "xs", "", "", "ml-2");
		}
		...
		let filters = kanivin.utils.process_filter_expression(this.stats_filter);
	...
	set_count(count) {
		...
		$(kanivin.utils.icon("es-line-arrow-up-right", "xs", "", "", "ml-2")).appendTo(
			this.action_area
		);
	}
}
