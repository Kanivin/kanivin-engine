...
export default class NumberCardWidget extends Widget {
	...
	make_card() {
		kanivin.model.with_doc("Number Card", this.number_card_name || this.name).then((card) => {
			if (!card) {
	...
	create_number_card() {
		this.set_doc_args();
		return kanivin
			.xcall("kanivin.desk.doctype.number_card.number_card.create_number_card", {
				args: this.card_doc,
			})
			.then((doc) => {
				this.name = doc.name;
	...
	set_route() {
		...
		const route = kanivin.utils.generate_route({
			name: name,
		...
		if (is_document_type) {
			...
			kanivin.route_options = filters.reduce((acc, filter) => {
	...
	set_route_for_custom_card() {
		...
		if (this.data.route_options) {
			kanivin.route_options = this.data.route_options;
		}

		kanivin.set_route(this.data.route);
	}
	...
	get_settings(type) {
		...
		const settings_map = {
			...
			"Document Type": {
				method: "kanivin.desk.doctype.number_card.number_card.get_result",
				args: {
					doc: this.card_doc,
					filters: this.filters,
				},
	...
	get_filters() {
		return kanivin.dashboard_utils.get_all_filters(this.card_doc);
	}
	...
	async get_data() {
		this.data = await kanivin.xcall(this.settings.method, this.settings.args);
	...
	get_number_for_doctype_card(res) {
		...
		if (this.card_doc.function !== "Count") {
			return kanivin.model.with_doctype(this.card_doc.document_type, () => {
				const based_on_df = kanivin.meta.get_docfield(
	...
	get_number_for_report_card(res) {
		...
		this.number = kanivin.report_utils.get_result_of_fn(this.card_doc.report_function, vals);
	...
	set_formatted_number(df, doc) {
		...
		const shortened_number = kanivin.utils.shorten_number(this.number, default_country, 5);
		...
		const formatted_number = kanivin.format(number_parts[0], df, null, doc);
		...
		return this.get_percentage_stats().then(() => {
			if (this.percentage_stat == 0 || this.percentage_stat == undefined) {
				...
				caret_html = `<span class="indicator-pill-round green">
						${kanivin.utils.icon("es-line-arrow-up-right", "xs")}
					</span>`;
				color_class = "green-stat";
			} else {
				caret_html = `<span class="indicator-pill-round red">
						${kanivin.utils.icon("arrow-down-right", "xs")}
					</span>`;
	...
	get_percentage_stats() {
		return kanivin
			.xcall("kanivin.desk.doctype.number_card.number_card.get_percentage_difference", {
				doc: this.card_doc,
				filters: this.filters,
				result: this.number,
			})
			.then((res) => {
				if (res !== undefined) {
					this.percentage_stat = kanivin.utils.shorten_number(res);
				}
			});
	}
		...
		let actions = [
			...
			},
			{
				...
				handler: () => {
					...
					kanivin.set_route("Form", "Number Card", number_card);
				},
			},
		];
