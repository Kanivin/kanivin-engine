...
export default class QuickListWidget extends Widget {
	...
	set_actions() {
		...
		if (kanivin.model.can_create(this.document_type)) {
	...
	setup_refresh_list_button() {
		this.refresh_list = $(
			`<div class="refresh-list btn btn-xs pull-right" title="${__("Refresh List")}">
				${kanivin.utils.icon("es-line-reload", "sm")}
			</div>`
		);
	...
	setup_filter(doctype) {
		...
		this.filters = kanivin.utils.process_filter_expression(this.quick_list_filter);
	...
	setup_quick_list_item(doc) {
		const indicator = kanivin.get_indicator(doc, this.document_type);
			...
			<div class="quick-list-item">
				<div class="ellipsis left">
					...
					<div class="timestamp text-muted">
						${kanivin.datetime.prettyDate(doc.modified)}
					</div>
				</div>
			</div>
		`);
		...
		let icon_to_append = `<div class="right-arrow">${kanivin.utils.icon("right", "xs")}</div>`;
		if (kanivin.utils.is_rtl(kanivin.boot.lang)) {
			icon_to_append = `<div class="left-arrow">${kanivin.utils.icon("left", "xs")}</div>`;
		}
		$(icon_to_append).appendTo($quick_list_item);

		$quick_list_item.click((e) => {
			if (e.ctrlKey || e.metaKey) {
				kanivin.open_in_new_tab = true;
			}
			kanivin.set_route(`${kanivin.utils.get_form_link(this.document_type, doc.name)}`);
		});
	...
	set_body() {
		...
		kanivin.model.with_doctype(this.document_type, () => {
			let fields = ["name"];

			// get name of title field
			if (!this.title_field_name) {
				let meta = kanivin.get_meta(this.document_type);
			...
			this.has_status_field = kanivin.meta.has_field(this.document_type, "status");
			...
			let workflow_fieldname = kanivin.workflow.get_state_fieldname(this.document_type);
			workflow_fieldname && fields.push(workflow_fieldname);
			fields.push("modified");

			let add_fields = kanivin.listview_settings?.[this.document_type]?.add_fields;
			if (Array.isArray(add_fields)) {
				fields.push(...add_fields);
				fields = [...new Set(fields)];
			}

			let quick_list_filter = kanivin.utils.process_filter_expression(this.quick_list_filter);

			let args = {
				method: "kanivin.desk.reportview.get",
				args: {
	...
	set_footer() {
		...
		let filters = kanivin.utils.get_filter_from_json(this.quick_list_filter);
		let route = kanivin.utils.generate_route({ type: "doctype", name: this.document_type });
		this.see_all_button = $(`
