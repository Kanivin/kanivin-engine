...
kanivin.breadcrumbs = {
	...
	add(module, doctype, type) {
		...
		this.all[kanivin.breadcrumbs.current_page()] = obj;
		this.update();
	},

	current_page() {
		return kanivin.get_route_str();
	},

	update() {
		var breadcrumbs = this.all[kanivin.breadcrumbs.current_page()];
	...
	get last_route() {
		return kanivin.route_history.slice(-2)[0];
	},
	...
	set_workspace_breadcrumb(breadcrumbs) {
		...
		if (
			...
			(breadcrumbs.module_info.blocked ||
				!kanivin.visible_modules.includes(breadcrumbs.module_info.module))
		) {
			return;
		}

		this.append_breadcrumb_element(
			`/app/${kanivin.router.slug(breadcrumbs.workspace)}`,
	...
	set_workspace(breadcrumbs) {
		...
		if (this.last_route?.[0] == "Workspaces") {
			...
			if (
				breadcrumbs.module &&
				kanivin.boot.module_wise_workspaces[breadcrumbs.module]?.includes(last_workspace)
			) {
		...
		if (breadcrumbs.module) {
			...
			breadcrumbs.module_info = kanivin.get_module(breadcrumbs.module);

			// set workspace
			if (
				breadcrumbs.module_info &&
				kanivin.boot.module_wise_workspaces[breadcrumbs.module]
			) {
				breadcrumbs.workspace = kanivin.boot.module_wise_workspaces[breadcrumbs.module][0];
			}
		}
	},
	...
	set_list_breadcrumb(breadcrumbs) {
		...
		const doctype_meta = kanivin.get_doc("DocType", doctype);
		if (
			(doctype === "User" && !kanivin.user.has_role("System Manager")) ||
			doctype_meta?.issingle
		) {
			// no user listview for non-system managers and single doctypes
		} else {
			let route;
			const doctype_route = kanivin.router.slug(kanivin.router.doctype_layout || doctype);
			if (doctype_meta?.is_tree) {
				let view = kanivin.model.user_settings[doctype].last_view || "Tree";
	...
	set_form_breadcrumb(breadcrumbs, view) {
		const doctype = breadcrumbs.doctype;
		let docname = kanivin.get_route().slice(2).join("/");
		let docname_title;
		...
		let form_route = `/app/${kanivin.router.slug(doctype)}/${encodeURIComponent(docname)}`;
		...
		if (view === "form") {
			...
			last_crumb.click((event) => {
				...
				kanivin.utils.copy_to_clipboard(last_crumb.text());
			});
		}
	},
	...
	set_dashboard_breadcrumb(breadcrumbs) {
		const doctype = breadcrumbs.doctype;
		const docname = kanivin.get_route()[1];
		let dashboard_route = `/app/${kanivin.router.slug(doctype)}/${docname}`;
		$(`<li><a href="${dashboard_route}">${__(docname)}</a></li>`).appendTo(this.$breadcrumbs);
	},

	setup_modules() {
		if (!kanivin.visible_modules) {
			kanivin.visible_modules = $.map(kanivin.boot.allowed_workspaces, (m) => {
				return m.module;
			});
		}
	},
	...
	rename(doctype, old_name, new_name) {
		...
		delete kanivin.breadcrumbs.all[old_route_str];
		this.update();
	},
