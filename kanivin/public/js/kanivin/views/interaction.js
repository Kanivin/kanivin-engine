...
kanivin.provide("kanivin.interaction_settings");

kanivin.views.InteractionComposer = class InteractionComposer {
	constructor(opts) {
		$.extend(this, opts);
		this.make();
	}
	...
	get_event_categories() {
		let me = this;
		kanivin.model.with_doctype("Event", () => {
			let categories = kanivin.meta
	...
	setup_attach() {
		...
		attach.find(".add-more-attachments a").on("click", () => new kanivin.ui.FileUploader(args));
		this.render_attach();
	}
	...
	create_interaction(btn, form_values, selected_attachments) {
		...
		if (!("owner" in interaction_values)) {
			interaction_values["owner"] = kanivin.session.user;
		}
		if (!("assigned_by" in interaction_values) && interaction_values["doctype"] == "ToDo") {
			interaction_values["assigned_by"] = kanivin.session.user;
		}
		return kanivin.call({
	...
	add_attachments(doc, attachments) {
		kanivin.call({
			method: "kanivin.utils.file_manager.add_attachments",
			args: {
				doctype: doc.doctype,
				name: doc.name,
