...
kanivin.provide("kanivin.treeview_settings");
kanivin.provide("kanivin.views.trees");
window.cur_tree = null;

kanivin.views.TreeFactory = class TreeFactory extends kanivin.views.Factory {
	make(route) {
		kanivin.model.with_doctype(route[1], function () {
			var options = {
				doctype: route[1],
				meta: kanivin.get_meta(route[1]),
			};

			if (
				!kanivin.treeview_settings[route[1]] &&
				!kanivin.meta.get_docfield(route[1], "is_group")
			) {
				kanivin.msgprint(__("Tree view is not available for {0}", [route[1]]));
				return false;
			}
			$.extend(options, kanivin.treeview_settings[route[1]] || {});
			kanivin.views.trees[options.doctype] = new kanivin.views.TreeView(options);
		});
	}
	...
	on_show() {
		 ...
		 * treeview is visible.
		 */
		let route = kanivin.get_route();
		let treeview = kanivin.views.trees[route[1]];
...
kanivin.views.TreeView = class TreeView {
	constructor(opts) {
		var me = this;
		...
		this.get_tree_nodes = me.opts.get_tree_nodes || "kanivin.desk.treeview.get_children";
	...
	get_permissions() {
		this.can_read = kanivin.model.can_read(this.doctype);
		this.can_create =
			kanivin.boot.user.can_create.indexOf(this.doctype) !== -1 ||
			kanivin.boot.user.in_create.indexOf(this.doctype) !== -1;
		this.can_write = kanivin.model.can_write(this.doctype);
		this.can_delete = kanivin.model.can_delete(this.doctype);
	}
	make_page() {
		var me = this;
		if (!this.opts || !this.opts.do_not_make_page) {
			this.parent = kanivin.container.add_page(this.page_name);
			kanivin.ui.make_app_page({ parent: this.parent, single_column: true });
			this.page = this.parent.page;
			kanivin.container.change_to(this.page_name);
			kanivin.breadcrumbs.add(
			...
			this.page.main.addClass("kanivin-card");
		} else {
			this.page = this.opts.page;
			$(this.page[0]).addClass("kanivin-card");
		}
	...
	make_filters() {
		var me = this;
		kanivin.treeview_settings.filters = [];
		$.each(this.opts.filters || [], function (i, filter) {
			if (kanivin.route_options && kanivin.route_options[filter.fieldname]) {
				filter.default = kanivin.route_options[filter.fieldname];
			}
	...
	rebuild_tree() {
		...
		kanivin.call({
			method: "kanivin.utils.nestedset.rebuild_tree",
			args: {
				doctype: me.doctype,
			},
			callback: function (r) {
	...
	select_node(node) {
		...
		if (this.opts.view_template) {
			...
			$(
				kanivin.render_template(me.opts.view_template, {
					data: node.data,
					doctype: me.doctype,
				})
		...
		var toolbar = [
			{
				...
				click: function (node) {
					kanivin.set_route("Form", me.doctype, node.label);
				},
			},
			{
				label: __("Add Child"),
			...
			},
			{
				...
				condition: function (node) {
					...
					if (me.doctype && kanivin.get_meta(me.doctype)) {
						if (!kanivin.get_meta(me.doctype).allow_rename) allow_rename = false;
					}
					return !node.is_root && me.can_write && allow_rename;
				},
				click: function (node) {
					kanivin.model.rename_doc(me.doctype, node.label, function (new_name) {
			...
			},
			{
				...
				click: function (node) {
					kanivin.model.delete_doc(me.doctype, node.label, function () {
						node.parent.remove();
					});
				},
				btnClass: "hidden-xs",
	...
	new_node() {
		...
		if (!(node && node.expandable)) {
			kanivin.msgprint(__("Select a group node first."));
			return;
		}
		...
		d.set_primary_action(__("Create New"), function () {
			...
			d.hide();
			kanivin.dom.freeze(__("Creating {0}", [me.doctype]));

			$.extend(args, v);
			return kanivin.call({
				method: me.opts.add_tree_node || "kanivin.desk.treeview.add_node",
				args: args,
				callback: function (r) {
					if (!r.exc) {
	...
	print_tree() {
		if (!kanivin.model.can_print(this.doctype)) {
			kanivin.msgprint(__("You are not allowed to print this report"));
			return false;
		}
		var tree = $(".tree:visible").html();
		var me = this;
		kanivin.ui.get_print_settings(false, function (print_settings) {
			var title = __(me.docname || me.doctype);
			kanivin.render_tree({ title: title, tree: tree, print_settings: print_settings });
			kanivin.call({
				method: "kanivin.core.doctype.access_log.access_log.make_access_log",
				args: {
					doctype: me.doctype,
	...
	set_menu_item() {
		...
		this.menu_items = [
			{
				label: __("View List"),
				action: function () {
					kanivin.set_route("List", me.doctype);
				},
			},
			{
				label: __("Print"),
		...
		if (
			...
			kanivin.meta.has_field(me.doctype, "lft") &&
			kanivin.meta.has_field(me.doctype, "rgt")
		) {
