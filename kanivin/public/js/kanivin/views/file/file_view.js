...
kanivin.views.FileView = class FileView extends kanivin.views.ListView {
	static load_last_view() {
		const route = kanivin.get_route();
		if (route.length === 2) {
			const view_user_settings = kanivin.get_user_settings("File", "File");
			kanivin.set_route(
				"List",
				"File",
				view_user_settings.last_folder || kanivin.boot.home_folder
			);
			return true;
		}
	...
	setup_view() {
		...
		this.page.add_button(__("Toggle Grid View"), () => {
			kanivin.views.FileView.grid_view = !kanivin.views.FileView.grid_view;
			this.refresh();
		});
	}
	...
	get_args() {
		...
		if (kanivin.views.FileView.grid_view) {
			Object.assign(args, {
		...
		return [
			{
				label: __("Home"),
				action: () => {
					kanivin.set_route("List", "File", "Home");
				},
			},
			{
				label: __("New Folder"),
					...
					kanivin.prompt(
						...
						(values) => {
							if (values.value.indexOf("/") > -1) {
								kanivin.throw(__("Folder name should not include '/' (slash)"));
							}
							const data = {
							...
							kanivin.call({
								method: "kanivin.core.api.file.create_new_folder",
								args: data,
							});
						},
			...
			},
			{
				...
				action: () => {
					new kanivin.ui.FileUploader({
						...
						on_success: (file) => {
							kanivin.show_alert(__("Unzipping files..."));
							kanivin
								.call("kanivin.core.api.file.unzip_file", {
									name: file.name,
								})
								.then((r) => {
									if (r.message) {
										kanivin.show_alert(__("Unzipped {0} files", [r.message]));
									}
								});
						},
					});
	...
	add_file_action_buttons() {
		this.$cut_button = this.page
			.add_button(__("Cut"), () => {
				kanivin.file_manager.cut(this.get_checked_items(), this.current_folder);
				this.$checks.parents(".file-wrapper").addClass("cut");
			})
			.hide();

		this.$paste_btn = this.page
			.add_button(__("Paste"), () => kanivin.file_manager.paste(this.current_folder))
			.hide();

		this.page.add_actions_menu_item(__("Export as zip"), () => {
			let docnames = this.get_checked_items(true);
			if (docnames.length) {
				open_url_post("/api/method/kanivin.core.api.file.zip_files", {
	...
	set_fields() {
		this.fields = this.meta.fields
			.filter((df) => kanivin.model.is_value_type(df.fieldtype) && !df.hidden)
			.map((df) => df.fieldname)
	...
	prepare_datum(d) {
		...
		if (d.is_folder) {
			...
			type = "folder";
		} else if (kanivin.utils.is_image_file(d.file_name)) {
			icon_class = "image";
			type = "image";
		} else {
			icon_class = "file";
	...
	before_render() {
		super.before_render();
		kanivin.model.user_settings.save("File", "grid_view", kanivin.views.FileView.grid_view);
	...
	get_header_html() {
		...
		let header_selector_html = !kanivin.views.FileView.grid_view
			? `<input class="level-item list-check-all hidden-xs" type="checkbox" title="${__(
					"Select All"
			  )}">`
			: "";

		let header_columns_html = !kanivin.views.FileView.grid_view
	...
	get_creation_date(file) {
		...
		let created_on;
		if (date === kanivin.datetime.now_date()) {
			created_on = comment_when(file.creation);
		} else {
			created_on = kanivin.datetime.str_to_user(date);
		}
		return created_on;
	}
	...
	get_left_html(file) {
		...
		const file_size = file.file_size ? kanivin.form.formatters.FileSize(file.file_size) : "";
		...
		return `
			<div class="list-row-col ellipsis list-subject level">
				...
				<span class="level-item  ellipsis" title="${kanivin.utils.escape_html(file.file_name)}">
					<a class="ellipsis" href="${route_url}" title="${kanivin.utils.escape_html(file.file_name)}">
						${file.subject_html}
					</a>
				</span>
			</div>
		...
		this.$result.on("drop", (e) => {
			...
			if (dataTransfer.files && dataTransfer.files.length > 0) {
				...
				if ($el.parents(".folders").length !== 0) {
					...
					kanivin.file_manager.paste(folder_name);
					kanivin.show_alert(`File ${file_name} moved to ${folder_name}`);
				}
			}
		});
	}
	...
	toggle_cut_paste_buttons() {
		const hide_paste_btn =
			!kanivin.file_manager.can_paste ||
			kanivin.file_manager.old_folder === this.current_folder;
...
kanivin.views.FileView.grid_view = kanivin.get_user_settings("File").grid_view || false;
