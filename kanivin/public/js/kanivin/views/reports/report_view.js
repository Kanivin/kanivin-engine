...
kanivin.views.ReportView = class ReportView extends kanivin.views.ListView {
	get view_name() {
		return "Report";
	}
	...
	setup_events() {
		...
		kanivin.realtime.on("list_update", (data) => this.on_update(data));
	}
	...
	setup_sort_selector() {
		this.sort_selector = new kanivin.ui.SortSelector({
		...
		this.group_by_control = new kanivin.ui.GroupBy(this);
	...
	before_refresh() {
		if (this.report_doc) {
			// don't parse kanivin.route_options if this is a Custom Report
	...
	set_dirty_state_for_custom_report() {
		...
		if (!kanivin.utils.deep_equal(current_settings, report_settings)) {
	...
	prepare_data(r) {
		let data = r.message || {};
		data = kanivin.utils.dict(data.keys, data.values);
	...
	on_update(data) {
		if (this.doctype === data.doctype && data.name) {
			...
			const flash_row = data.user !== kanivin.session.user;
	...
	setup_charts() {
		...
		for (let col of this.columns) {
			...
			// numeric values in y
			if (col.docfield && kanivin.model.numeric_fieldtypes.includes(col.docfield.fieldtype)) {
				y_fields.push({
					label: col.content,
					fieldname: col.id,
					value: col.id,
			...
			fields: [
				...
				},
				{
					...
					default: defaults.chart_type
						? kanivin.utils.to_title_case(defaults.chart_type)
						: "Bar",
				},
			],
			primary_action: (data) => {
	...
	make_chart() {
		...
		this.chart = new kanivin.Chart(this.$charts_wrapper.find(".charts-inner-wrapper")[0], {
			...
			tooltipOptions: {
				formatTooltipY: (value) => {
					return kanivin.format(value, get_df(this.chart_args.y_axes[0]), {
	...
	render_editing_input(colIndex, value, parent) {
		...
		if (col.docfield.fieldtype === "Text Editor") {
			...
			// make control
			control = kanivin.ui.form.make_control({
				df: col.docfield,
				parent: parent,
				render_input: true,
			});
	...
	evaluate_read_only_depends_on(expression, data) {
		...
		if (typeof expression === "boolean") {
			...
			try {
				out = kanivin.utils.eval(expression.substr(5), { doc: data });
				if (parent && parent.istable && expression.includes("is_submittable")) {
					out = true;
				}
			} catch (e) {
				kanivin.throw(__('Invalid "depends_on" expression'));
			}
	...
	is_editable(df, data) {
		if (
			df &&
			kanivin.model.can_write(this.doctype) &&
			...
			kanivin.model.is_non_std_field(df.fieldname)
		) {
	...
	set_default_fields() {
		...
		const fields = this.meta.fields.filter((df) => {
			return (
				...
				kanivin.perm.has_perm(this.doctype, df.permlevel, "read") &&
				kanivin.model.is_value_type(df.fieldtype) &&
				!df.report_hide
			);
		});
	...
	get_fields() {
		let fields = this.fields.map((f) => {
			let column_name = kanivin.model.get_full_column_name(f[0], f[1]);
			if (f[1] !== this.doctype) {
				// child table field
		...
		const cdt_name_fields = this.get_unique_cdt_in_view().map(
			(cdt) => kanivin.model.get_full_column_name("name", cdt) + " as " + `'${cdt}:name'`
		);
	...
	add_currency_column(fieldname, doctype, col_index) {
		...
		const df = kanivin.meta.get_docfield(doctype, fieldname);
		if (
			df &&
			df.fieldtype === "Currency" &&
			df.options &&
			!df.options.includes(":") &&
			kanivin.meta.has_field(doctype, df.options)
		) {
			...
			const field_label = kanivin.meta.get_label(doctype, field[0]);
			kanivin.show_alert(
	...
	remove_column_from_datatable(column) {
		...
		if (field[0] === "name") {
			this.refresh();
			kanivin.throw(__("Cannot remove ID field"));
		}
	...
	get_columns_for_picker() {
		...
		const standard_fields_filter = (df) => !kanivin.model.no_value_type.includes(df.fieldtype);

		let doctype_fields = kanivin.meta
			.get_docfields(this.doctype)
			.filter(standard_fields_filter);

		// filter out docstatus field from picker
		let std_fields = kanivin.model.std_fields.filter(
		...
		if (this.data) {
			has_status_values = kanivin.get_indicator(this.data[0], this.doctype);
		}

		if (!kanivin.meta.has_field(this.doctype, "status") && has_status_values) {
			doctype_fields = [
				{
					label: __("Status"),
					fieldname: "docstatus",
		...
		const table_fields = kanivin.meta.get_table_fields(this.doctype);

		table_fields.forEach((df) => {
			const cdt = df.options;
			const child_table_fields = kanivin.meta
				.get_docfields(cdt)
				.filter(standard_fields_filter);
	...
	get_dialog_fields() {
		...
		const table_fields = kanivin.meta.get_table_fields(this.doctype).filter((df) => !df.hidden);
		...
		for (let f of this.fields) {
			...
			if (f[0] !== "docstatus") {
				...
				if (
					...
					!kanivin.meta.has_field(this.doctype, "status")
				) {
	...
	build_column(c) {
		...
		let docfield = kanivin.meta.docfield_map[doctype || this.doctype][fieldname];
		...
		if (!docfield) {
			docfield = kanivin.model.get_std_field(fieldname, true);
			...
			if (docfield) {
				...
				if (fieldname == "docstatus" && !kanivin.meta.has_field(this.doctype, "status")) {
		...
		const editable =
			kanivin.model.is_non_std_field(fieldname) &&
			!docfield.read_only &&
			!docfield.is_virtual;

		const align = (() => {
			const is_numeric = kanivin.model.is_numeric_field(docfield);
			if (is_numeric) {
				return "right";
			}
		...
		return {
			...
			format: (value, row, column, data) => {
				...
				return kanivin.format(value, column.docfield, { always_show_decimals: true }, doc);
			},
		};
	}
		...
		if (this.add_totals_row) {
			...
			const totals_row = this.columns.map((col, i) => {
				return {
					...
					format: (value) => {
						let formatted_value = kanivin.format(
							value,
							col.docfield,
							{
								always_show_decimals: true,
	...
	format_total_cell(formatted_value, df) {
		...
		if (kanivin.model.is_numeric_field(df.docfield)) {
	...
	build_row(d) {
		return this.columns.map((col) => {
			...
			if (col.field === "docstatus" && !kanivin.meta.has_field(this.doctype, "status")) {
				// get status from docstatus
				let status = kanivin.get_indicator(d, this.doctype);
				if (status) {
	...
	save_report(save_type) {
		const _save_report = (name) => {
			...
			return kanivin.call({
				method: "kanivin.desk.reportview.save_report",
				args: {
					name: name,
					doctype: this.doctype,
				...
				callback: (r) => {
					if (r.exc) {
						kanivin.msgprint(__("Report was not saved (there were errors)"));
						return;
					}
					if (r.message != this.report_name) {
						// Rerender the reports dropdown,
						// so that this report is included in the dropdown as well.
						kanivin.boot.user.all_reports[r.message] = {
							ref_doctype: this.doctype,
							report_type: "Report Builder",
							title: r.message,
						};

						kanivin.set_route("List", this.doctype, "Report", r.message);
						return;
					}
	...
	delete_report() {
		return kanivin.call({
			method: "kanivin.desk.reportview.delete_report",
	...
	get_report_doc() {
		return new Promise((resolve) => {
			kanivin.model.with_doc("Report", this.report_name, () => {
				resolve(kanivin.get_doc("Report", this.report_name));
			});
		});
	}
		...
		return (
			...
			filters
				.map((f) => {
					...
					const docfield = kanivin.meta.get_docfield(doctype, fieldname);
					const label = `<b>${__(kanivin.meta.get_label(doctype, fieldname))}</b>`;
					switch (condition) {
						case "=":
							return __("{0} is equal to {1}", [
								label,
								kanivin.format(value, docfield),
							]);
						case "!=":
						...
						case "Between":
							return __("{0} is between {1} and {2}", [
								__(label),
								kanivin.format(value[0], docfield),
								kanivin.format(value[1], docfield),
							]);
						case "Timespan":
						...
						case "in":
							return __("{0} is one of {1}", [
								__(label),
								kanivin.utils.comma_or(
									value.map((v) => kanivin.format(v, docfield))
								),
							]);
						case "not in":
	...
	get_columns_totals(data) {
		...
		this.columns.forEach((col, i) => {
			const totals = data.reduce((totals, d) => {
				if (col.id in d && kanivin.model.is_numeric_field(col.docfield)) {
					totals += flt(d[col.id]);
					return totals;
				}
			}, 0);
	...
	report_menu_items() {
		let items = [
			...
			},
			{
				...
				action: () => {
					...
					kanivin.ui.get_print_settings(false, (print_settings) => {
			...
			},
			{
				...
				action: () => {
					...
					kanivin.utils.setup_search(d.$body, ".unit-checkbox", ".label-area");
					d.show();
				},
			},
		];

		if (kanivin.model.can_export(this.doctype)) {
			items.push({
				label: __("Export"),
				action: () => {
					...
					const d = kanivin.report_utils.get_export_dialog(
						__(this.doctype),
						extra_fields,
						(data) => {
							args.cmd = "kanivin.desk.reportview.export_query";
		...
		items.push({
			...
			action: () => {
				if (this.report_name) {
					...
					kanivin.msgprint(__("Please save the report first"));
				}
			},
		});
		...
		const can_edit_or_delete = (action) => {
			...
			return (
				...
				(kanivin.model[method]("Report") || this.report_doc.owner === kanivin.session.user)
			);
		};
		...
		if (can_edit_or_delete("delete")) {
			items.push({
				label: __("Delete"),
				action: () =>
					kanivin.confirm("Are you sure you want to delete this report?", () =>
						this.delete_report()
					),
		...
		// user permissions
		if (this.report_name && kanivin.user.has_role("System Manager")) {
			items.push({
				...
				action: () => {
					...
					kanivin.set_route("List", "User Permission", args);
				},
			});
		}
	...
	parse_filters_from_route_options() {
		if (kanivin.route_options?._group_by) {
			try {
				let config = JSON.parse(kanivin.route_options._group_by);
				this.group_by_control.apply_settings({
					group_by: config[0],
					aggregate_on: config[1],
					aggregate_function: config[2],
				});
				delete kanivin.route_options["_group_by"];
			} catch (e) {
