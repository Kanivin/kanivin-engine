kanivin.provide("kanivin.report_utils");

kanivin.report_utils = {
	make_chart_options: function (
		...
		let datasets = y_fields.map((y_field) => ({
			name: kanivin.model.unscrub(y_field),
		...
		function get_options(fields) {
			return fields.map((field) => {
				if (field.fieldname) {
					...
					field = kanivin.report_utils.prepare_field_from_column(field);
					return { label: field.label, value: field.fieldname };
				}
			});
		}
	...
	get_report_filters: function (report_name) {
		if (kanivin.query_reports[report_name]) {
			let filters = kanivin.query_reports[report_name].filters;
		...
		return kanivin
			...
			.then((r) => {
				kanivin.dom.eval(r.script || "");
				return kanivin.after_ajax(() => {
					if (
						kanivin.query_reports[report_name] &&
						!kanivin.query_reports[report_name].filters &&
						r.filters
					) {
						return (kanivin.query_reports[report_name].filters = r.filters);
					}
					return (
						kanivin.query_reports[report_name] &&
						kanivin.query_reports[report_name].filters
	...
	get_export_dialog(report_name, extra_fields, callback) {
		...
		function update_csv_preview(dialog) {
			const is_query_report = kanivin.get_route()[0] === "query-report";
			const report = is_query_report ? kanivin.query_report : cur_list;
			...
			dialog.set_value(
				"csv_preview",
				kanivin.report_utils.get_csv_preview(
					PREVIEW_DATA,
	...
	get_csv_preview(data, quoting, delimiter) {
		...
		if (delimiter.length > 1) {
			kanivin.throw(__("Delimiter must be a single character"));
		}

		if (0 > quoting || quoting > 3) {
			kanivin.throw(__("Quoting must be between 0 and 3"));
		}
