...
kanivin.provide("kanivin.widget.utils");
kanivin.provide("kanivin.views");
kanivin.provide("kanivin.query_reports");

kanivin.standard_pages["query-report"] = function () {
	var wrapper = kanivin.container.add_page("query-report");
	...
	kanivin.query_report = new kanivin.views.QueryReport({
		parent: wrapper,
	});

	$(wrapper).bind("show", function () {
		kanivin.query_report.show();
	});
};

kanivin.views.QueryReport = class QueryReport extends kanivin.views.BaseList {
	show() {
	...
	init() {
		...
		this.init_promise = kanivin.run_serially(tasks);
		return this.init_promise;
	}
	...
	setup_defaults() {
		...
		this.refresh = kanivin.utils.throttle(this.refresh, 300);
	...
	update_url_with_filters() {
		if (kanivin.get_route_str() == this.page_name) {
	...
	get_no_result_message() {
		return `<div class="msg-box no-border">
			<div>
				<img src="/assets/kanivin/images/ui-states/list-empty-state.svg" alt="Generic Empty State" class="null-state">
			</div>
	...
	setup_events() {
		kanivin.realtime.on("report_generated", (data) => {
			...
			if (data.report_name) {
				...
				if (data.name == this.prepared_report_doc_name) {
					...
					kanivin.show_alert({ message: alert_message, indicator: "orange" });
				}
			}
		});
	...
	load() {
		if (kanivin.get_route().length < 2) {
			this.toggle_nothing_to_show(true);
			return;
		}

		let route_options = {};
		route_options = Object.assign(route_options, kanivin.route_options);

		if (this.report_name !== kanivin.get_route()[1]) {
			// different report
			this.load_report(route_options);
		} else if (kanivin.has_route_options()) {
	...
	add_card_button_to_toolbar() {
		if (!kanivin.model.can_create("Number Card")) return;
	...
	add_chart_buttons_to_toolbar(show) {
		if (!kanivin.model.can_create("Dashboard Chart")) return;
		if (show) {
	...
	add_card_to_dashboard() {
		let field_options = kanivin.report_utils.get_field_options_from_report(
			this.columns,
			this.raw_data
		);
		const dashboard_field = kanivin.dashboard_utils.get_dashboard_link_field();
		const set_standard = kanivin.boot.developer_mode;

		const dialog = new kanivin.ui.Dialog({
	...
	add_chart_to_dashboard() {
		if (this.chart_fields || this.chart_options) {
			...
			kanivin.msgprint(__("Please Set Chart"));
		}
	}
	...
	create_number_card(values, dashboard_name, card_name, set_standard) {
		...
		this.add_to_dashboard(
			"kanivin.desk.doctype.number_card.number_card.create_report_number_card",
			args,
			dashboard_name,
			card_name,
			"Number Card"
	...
	create_dashboard_chart(chart_args, dashboard_name, chart_name, set_standard) {
		let args = {
			...
			chart_type: "Report",
			report_name: this.report_name,
			type: chart_args.chart_type || kanivin.model.unscrub(chart_args.type),
		...
		this.add_to_dashboard(
			"kanivin.desk.doctype.dashboard_chart.dashboard_chart.create_report_chart",
			args,
			dashboard_name,
			chart_name,
			"Dashboard Chart"
	...
	add_to_dashboard(method, args, dashboard_name, name, doctype) {
		kanivin.xcall(method, { args: args }).then(() => {
			let message;
			if (dashboard_name) {
			...
			kanivin.msgprint(message, __("New {0} Created", [__(doctype)]));
		});
	}
	...
	get_report_doc() {
		return kanivin.model
			.with_doc("Report", this.report_name)
			.then((doc) => {
				this.report_doc = doc;
			})
			.then(() => kanivin.model.with_doctype(this.report_doc?.ref_doctype));
	}

	get_report_settings() {
		return new Promise((resolve, reject) => {
			if (kanivin.query_reports[this.report_name]) {
				this.report_settings = kanivin.query_reports[this.report_name];
				resolve();
			} else {
				kanivin
					.xcall("kanivin.desk.query_report.get_script", {
						report_name: this.report_name,
					})
					.then((settings) => {
						kanivin.dom.eval(settings.script || "");
						kanivin.after_ajax(() => {
							...
							kanivin.query_reports[this.report_name] = this.report_settings;
	...
	get_local_report_settings(custom_report_name) {
		let report_script_name =
			this.report_doc.report_type === "Custom Report"
				...
				: this.report_name;
		return kanivin.query_reports[report_script_name] || {};
	}
	...
	setup_progress_bar() {
		...
		this.interval = setInterval(function () {
			seconds_elapsed += 1;
			kanivin.show_progress(__("Preparing Report"), seconds_elapsed, execution_time);
		}, 1000);
	}
	...
	evaluate_depends_on_value(expression, filter_label) {
		...
		if (doc) {
			if (typeof expression === "boolean") {
				...
				try {
					out = kanivin.utils.eval(expression.substr(5), { doc });
				} catch (e) {
					kanivin.throw(
	...
	set_route_filters(route_options) {
		if (!route_options) route_options = kanivin.route_options;
		...
		if (route_options) {
			...
			return kanivin.run_serially(promises);
		}
	}
	...
	refresh(have_filters_changed) {
		...
		return new Promise((resolve) => {
			this.last_ajax = kanivin.call({
				method: "kanivin.desk.query_report.run",
				type: "GET",
				args: {
					report_name: this.report_name,
					filters: filters,
			...
			.then((r) => {
				...
				if (data.result && data.result.length) {
					...
					if (this.chart_options) {
						...
						this.$chart.empty();
						if (this.chart_fields) {
							this.chart_options = kanivin.report_utils.make_chart_options(
								this.columns,
								this.raw_data,
								this.chart_fields
							);
	...
	render_summary(data) {
		data.forEach((summary) => {
			kanivin.utils.build_summary_item(summary).appendTo(this.$summary);
		});
	...
	get_query_params() {
		const query_string = kanivin.utils.get_query_string(kanivin.get_route_str());
		return kanivin.utils.get_query_params(query_string);
	}
	...
	add_prepared_report_buttons(doc) {
		if (doc) {
			this.page.add_inner_button(
				...
				function () {
					window.open(
						kanivin.urllib.get_full_url(
							"/api/method/kanivin.core.doctype.prepared_report.prepared_report.download_attachment?" +
								"dn=" +
			...
			let pretty_diff = kanivin.datetime.comment_when(doc.report_end_time);
			const days_old = kanivin.datetime.get_day_diff(
				kanivin.datetime.now_datetime(),
				doc.report_end_time
			);
			if (days_old > 1) {
		...
		this.primary_action_map = {
			...
			Edit: {
				label: __("Edit"),
				click: () => {
					kanivin.set_route(kanivin.get_route());
				},
			},
			Rebuild: {
				label: __("Rebuild"),
	...
	show_warning_or_generate_report() {
		kanivin
			.xcall(
				"kanivin.core.doctype.prepared_report.prepared_report.get_reports_in_queued_state",
				{
			...
			.then((reports) => {
				...
				if (reports.length) {
					...
					this.prepared_report_dialog = kanivin.warn(
					...
					kanivin.utils.bind_actions_with_object(
	...
	delete_old_queued_reports() {
		...
		kanivin
			.xcall("kanivin.core.doctype.prepared_report.prepared_report.delete_prepared_reports", {
	...
	generate_background_report() {
		...
		if (!missing_mandatory.length) {
			...
			return new Promise((resolve) =>
				kanivin.call({
					method: "kanivin.core.doctype.prepared_report.prepared_report.make_prepared_report",
					args: {
						report_name: this.report_name,
						filters: filters,
					},
				...
				let alert_message =
					...
					`</a>`;
				kanivin.show_alert({ message: alert_message, indicator: "orange" }, 10);
				this.toggle_nothing_to_show(true);
			});
		}
	}
	...
	render_datatable() {
		...
		if (data.length > 1000000) {
			...
			this.toggle_message(true, `${kanivin.utils.icon("solid-warning")} ${msg}`);
			return;
		}
		...
		if (
			...
			let datatable_options = {
				...
				data: data,
				inlineFilters: true,
				language: kanivin.boot.lang,
				translations: kanivin.utils.datatable.get_translations(),
				treeView: this.tree_report,
				layout: "fixed",
				cellHeight: 33,
				showTotalRow: this.raw_data.add_total_row && !this.report_settings.tree,
				direction: kanivin.utils.is_rtl() ? "rtl" : "ltr",
				hooks: {
					columnTotal: kanivin.utils.report_column_total,
				},
			};
	...
	get_chart_options(data) {
		...
		options.axisOptions = {
			...
			numberFormatter: kanivin.utils.format_chart_axis_number,
		};
		options.height = 280;
		return options;
	}
	...
	render_chart(options) {
		this.$chart.empty();
		this.$chart.show();
		this.chart = new kanivin.Chart(this.$chart[0], options);
	}
	...
	open_create_chart_dialog() {
		...
		function preview_chart() {
			...
			if (values.x_field && values.y_fields.length) {
				let options = kanivin.report_utils.make_chart_options(
					me.columns,
					me.raw_data,
					values
				);
				...
				wrapper.empty();
				new kanivin.Chart(wrapper[0], options);
	...
	prepare_columns(columns) {
		...
		return columns.map((column) => {
			column = kanivin.report_utils.prepare_field_from_column(column);
			...
			if (column.fieldtype === "Date") {
				compareFn = (cell, keyword) => {
					...
					const keywordValue = kanivin.datetime.user_to_obj(keyword);
					const cellValue = kanivin.datetime.str_to_obj(cell.content);
	...
	set_breadcrumbs() {
		...
		const ref_doctype = kanivin.get_meta(this.report_doc.ref_doctype);
		kanivin.breadcrumbs.add(ref_doctype.module);
	}

	make_access_log(method, file_format) {
		kanivin.call("kanivin.core.doctype.access_log.access_log.make_access_log", {
	...
	pdf_report(print_settings) {
		const base_url = kanivin.urllib.get_base_url();
		const print_css = kanivin.boot.print_css;
		const landscape = print_settings.orientation == "Landscape";
		...
		const content = kanivin.render_template(template, {
			title: __(this.report_name),
			subtitle: filters_html,
		...
		const html = kanivin.render_template("print_template", {
			title: __(this.report_name),
			content: content,
			base_url: base_url,
			print_css: print_css,
			...
			landscape: landscape,
			columns: columns,
			lang: kanivin.boot.lang,
			layout_direction: kanivin.utils.is_rtl() ? "rtl" : "ltr",
			can_use_smaller_font: this.report_doc.is_standard === "Yes" && custom_format ? 0 : 1,
		});
		...
		kanivin.render_pdf(html, print_settings);
	}
	...
	get_filters_html_for_print() {
		...
		return Object.keys(applied_filters)
			.map((fieldname) => {
				const docfield = kanivin.query_report.get_filter(fieldname).df;
				...
				return `<div class="filter-row">
					<b>${__(docfield.label, null, docfield.parent)}:</b> ${kanivin.format(value, docfield)}
				</div>`;
			})
			.join("");
	}
	...
	export_report() {
		...
		this.export_dialog = kanivin.report_utils.get_export_dialog(
			__(this.report_name),
			extra_fields,
			({
				file_format,
				...
				for (const [key, value] of Object.entries(filters)) {
					const df = kanivin.query_report.get_filter(key).df;
				...
				const args = {
					cmd: "kanivin.desk.query_report.export_query",
					report_name: this.report_name,
				...
				open_url_post(kanivin.request.url, args);
		...
		return rows.map((row) => {
			...
			return row.slice(standard_column_count).map((cell, i) => {
				if (cell.column.fieldtype === "Duration") {
					cell.content = kanivin.utils.get_formatted_duration(cell.content);
				}
	...
	get_menu_items() {
		let items = [
			...
			{
				label: __("Edit"),
				action: () => kanivin.set_route("Form", "Report", this.report_name),
				condition: () => kanivin.user.is_report_manager(),
				standard: true,
			},
			{
				label: __("Print"),
				action: () => {
					let dialog = kanivin.ui.get_print_settings(
						false,
				...
				condition: () => kanivin.model.can_print(this.report_doc.ref_doctype),
				standard: true,
			},
			{
				label: __("PDF"),
			...
			},
			{
				...
				condition: () => kanivin.model.can_export(this.report_doc.ref_doctype),
				standard: true,
			},
			{
				label: __("Setup Auto Email"),
				action: () =>
					kanivin.set_route("List", "Auto Email Report", { report: this.report_name }),
				standard: true,
			},
			{
				label: __("Add Column"),
				action: () => {
					let d = new kanivin.ui.Dialog({
						...
						fields: [
							{
								...
								options: this.linked_doctypes?.map((df) => ({
									label: df.doctype + " (" + kanivin.unscrub(df.fieldname) + ")",
								...
								change: () => {
									...
									kanivin.model.with_doctype(doctype, () => {
										let options = kanivin.meta
											.get_docfields(doctype)
											.filter(kanivin.model.is_value_type)
											.map((df) => ({
												label: df.label,
												value: df.fieldname,
											}));
						...
						primary_action: (values) => {
							...
							let df = kanivin.meta.get_docfield(values.doctype, values.field);
							...
							custom_columns.push({
								fieldname: this.columns
									...
									? df.fieldname + "-" + kanivin.scrub(values.doctype)
									: df.fieldname,
							...
							kanivin.call({
								method: "kanivin.desk.query_report.get_data_for_custom_field",
								args: {
									field: values.field,
									doctype: values.doctype,
									names: Array.from(
			...
			},
			{
				...
				action: () =>
					kanivin.set_route("List", "User Permission", {
						doctype: "Report",
						name: this.report_name,
					}),
				condition: () => kanivin.user.has_role("System Manager"),
				standard: true,
			},
		];

		if (kanivin.user.is_report_manager()) {
			items.push({
				label: __("Save"),
				action: () => {
					let d = new kanivin.ui.Dialog({
						...
						primary_action: (values) => {
							kanivin.call({
								method: "kanivin.desk.query_report.save_report",
								args: {
								...
								callback: function (r) {
									this.show_save = false;
									d.hide();
									kanivin.set_route("query-report", r.message);
								},
							});
						},
					});
	...
	add_custom_column(custom_column, custom_data, new_column_data, insert_after_index) {
		...
		this.data.forEach((row) => {
			if (column[0].fieldname.includes("-")) {
				row[column_field + "-" + kanivin.scrub(new_column_data.doctype)] =
