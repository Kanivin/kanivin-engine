...
(function () {
	var method_prefix = "kanivin.desk.doctype.kanban_board.kanban_board.";
	...
	var store = createStore({
		...
		actions: {
			...
			add_column: function (context, col) {
				if (kanivin.model.can_create("Custom Field")) {
			...
			add_card: function (context, { card_title, column_title }) {
				var state = context.state;
				var doc = kanivin.model.get_new_doc(state.doctype);
				...
				if (field && !quick_entry) {
					...
					kanivin.new_doc(state.doctype, doc);
				}
			},
	...
	kanivin.views.KanbanBoard = function (opts) {
		var self = {};
		...
		function prepare() {
			...
			if (self.$kanban_board.length === 0) {
				self.$kanban_board = $(kanivin.render_template("kanban_board"));
		...
		function make_columns() {
			...
			columns.filter(is_active_column).map(function (col) {
				kanivin.views.KanbanBoardColumn(col, self.$kanban_board, self.board_perms);
			});
		}
	...
	kanivin.views.KanbanBoardColumn = function (column, wrapper, board_perms) {
		var self = {};
		var filtered_cards = [];
		...
		function make_dom() {
			self.$kanban_column = $(
				kanivin.render_template("kanban_column", {
					title: column.title,
					doctype: store.state.doctype,
					indicator: kanivin.scrub(column.indicator, "-"),
				})
			).appendTo(wrapper);
			// add task, archive
			...
			if (order) {
				...
				filtered_cards.forEach(function (card) {
					if (order.indexOf(card.name) === -1) {
						kanivin.views.KanbanBoardCard(card, self.$kanban_cards);
					}
				});
				order.forEach(function (name) {
					if (!filtered_cards_names.includes(name)) return;
					kanivin.views.KanbanBoardCard(get_card(name), self.$kanban_cards);
				});
			} else {
		...
		function setup_sortable() {
			...
			if (!kanivin.model.can_write(store.state.doctype)) return;
		...
		function bind_add_card() {
			...
			if (!kanivin.model.can_create(store.state.doctype)) {
		...
		function bind_options() {
			...
			get_column_indicators(function (indicators) {
				let html = `<li class="button-group">${indicators
					.map((indicator) => {
						let classname = kanivin.scrub(indicator, "-");
	...
	kanivin.views.KanbanBoardCard = function (card, wrapper) {
		var self = {};
		...
		function make_dom() {
			var opts = {
				name: card.name,
				title: kanivin.utils.html2text(card.title),
				disable_click: card._disable_click ? "disable-click" : "",
				creation: card.creation,
				doc_content: get_doc_content(card),
				image_url: cur_list.get_image_url(card),
				form_link: kanivin.utils.get_form_link(card.doctype, card.name),
			};

			self.$card = $(kanivin.render_template("kanban_card", opts)).appendTo(wrapper);

			if (!kanivin.model.can_write(card.doctype)) {
		...
		function get_doc_content(card) {
			...
			for (let field_name of cur_list.board.fields) {
				let field =
					kanivin.meta.docfield_map[card.doctype]?.[field_name] ||
					kanivin.model.get_std_field(field_name);
				let label = cur_list.board.show_labels
					? `<span>${__(field.label, null, field.parent)}: </span>`
					: "";
				let value = kanivin.format(card.doc[field_name], field);
				fields.push(`
		...
		function render_card_meta() {
			...
			if (card.color && kanivin.ui.color.validate_hex(card.color)) {
				const $div = $("<div>");
				$("<div></div>")
					.css({
						width: "30px",
		...
		function get_assignees_group() {
			return kanivin.avatar_group(card.assigned_list, 3, {
		...
		function show_assign_to_dialog(e) {
			e.preventDefault();
			e.stopPropagation();
			self.assign_to = new kanivin.ui.form.AssignToDialog({
				obj: self,
				method: "kanivin.desk.form.assign_to.add",
	...
	function fetch_customization(doctype) {
		return new Promise(function (resolve) {
			kanivin.model.with_doc("Customize Form", "Customize Form", function () {
				var doc = kanivin.get_doc("Customize Form");
	...
	function insert_doc(doc) {
		return kanivin.call({
			method: "kanivin.client.insert",
			args: {
				doc: doc,
			},
			callback: function () {
				kanivin.model.clear_doc(doc.doctype, doc.name);
				kanivin.show_alert({ message: __("Saved"), indicator: "green" }, 1);
			},
		});
	}
	...
	function get_column_indicators(callback) {
		kanivin.model.with_doctype("Kanban Board Column", function () {
			var meta = kanivin.get_meta("Kanban Board Column");
			var indicators;
