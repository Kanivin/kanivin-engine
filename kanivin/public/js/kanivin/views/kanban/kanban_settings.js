export default class KanbanSettings {
	constructor({ kanbanview, doctype, meta, settings }) {
		if (!doctype) {
			kanivin.throw(__("DocType required"));
		}
	...
	make() {
		...
		this.dialog.set_primary_action(__("Save"), () => {
			...
			kanivin.call({
				method: "kanivin.desk.doctype.kanban_board.kanban_board.save_settings",
				args: {
		...
		for (let fieldname of this.fields) {
			...
			fields += `
				<div class="control-input flex align-center form-control fields_order sortable"
					...
					<div class="row">
						<div class="col-md-1">
							${kanivin.utils.icon("drag", "xs", "", "", "sortable-handle")}
						</div>
	...
	get_docfield(field_name) {
		return (
			kanivin.meta.get_docfield(this.doctype, field_name) ||
	...
	get_multiselect_fields() {
		...
		const ignore_fieldtypes = [
			...
			"Code",
			"Color",
			...kanivin.model.no_value_type,
		];

		return kanivin.model.std_fields
