...
kanivin.views.GanttView = class GanttView extends kanivin.views.ListView {
	get view_name() {
		return "Gantt";
	}
	...
	prepare_tasks() {
		...
		this.tasks = this.data.map(function (item) {
			...
			if (item.color && kanivin.ui.color.validate_hex(item.color)) {
	...
	render_gantt() {
		...
		this.gantt = new Gantt(this.$result[0], this.tasks, {
			...
			on_click: (task) => {
				kanivin.set_route("Form", task.doctype, task.id);
			},
			on_date_change: (task, start, end) => {
				if (!me.can_write) return;
				kanivin.db.set_value(task.doctype, task.id, {
	...
	set_colors() {
		...
		let style = classes
			.map((c) => {
				...
				const progress_color = kanivin.ui.color.get_contrast_color(bar_color);
				return `
	...
	get required_libs() {
		return [
			"assets/kanivin/node_modules/frappe-gantt/dist/frappe-gantt.css",
			"assets/kanivin/node_modules/frappe-gantt/dist/frappe-gantt.min.js",
		];
	}
};
