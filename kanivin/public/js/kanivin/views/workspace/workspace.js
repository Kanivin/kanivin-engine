...
kanivin.standard_pages["Workspaces"] = function () {
	var wrapper = kanivin.container.add_page("Workspaces");
	...
	kanivin.workspace = new kanivin.views.Workspace(wrapper);
	$(wrapper).bind("show", function () {
		kanivin.workspace.show();
	});
};

kanivin.views.Workspace = class Workspace {
	constructor(wrapper) {
		this.wrapper = $(wrapper);
		this.page = wrapper.page;
		this.blocks = kanivin.workspace_block.blocks;
	...
	async setup_pages(reload) {
		...
		if (this.all_pages) {
			...
			for (let page of this.all_pages) {
				kanivin.workspaces[kanivin.router.slug(page.name)] = {
					title: page.title,
					public: page.public,
				};
			}
	...
	get_pages() {
		return kanivin.xcall("kanivin.desk.desktop.get_workspace_sidebar_items");
	}
			...
			>
				<div class="desk-sidebar-item standard-sidebar-item ${item.selected ? "selected" : ""}">
					<a
						href="/app/${
							item.public
								? kanivin.router.slug(item.title)
								: "private/" + kanivin.router.slug(item.title)
						}"
					...
					>
						<span class="sidebar-item-icon" item-icon=${item.icon || "folder-normal"}>
							${
								item.public
									? kanivin.utils.icon(item.icon || "folder-normal", "md")
	...
	make_sidebar() {
		...
		this.sidebar.find(".selected").length &&
			!kanivin.dom.is_element_in_viewport(this.sidebar.find(".selected")) &&
	...
	build_sidebar_section(category, root_pages) {
		...
		let $title = $(`<button class="btn-reset standard-sidebar-label">
			<span>${kanivin.utils.icon("es-line-down", "xs")}</span>
	...
	append_item(item, container) {
		let is_current_page =
			kanivin.router.slug(item.title) == kanivin.router.slug(this.get_page_to_show().name) &&
	...
	add_drop_icon(item, sidebar_control, item_container) {
		...
		let $drop_icon = $(`<button class="btn-reset drop-icon hidden">`)
			.html(kanivin.utils.icon(drop_icon, "sm"))
	...
	show() {
		...
		if (!kanivin.router.current_route[0]) {
			kanivin.route_flags.replace_route = true;
			kanivin.set_route(kanivin.router.slug(page.public ? page.name : "private/" + page.name));
			return;
		}
	...
	get_data(page) {
		return kanivin
			.call("kanivin.desk.desktop.get_desktop_page", {
				page: page,
			})
			.then((data) => {
				...
				return kanivin.dashboard_utils.get_dashboard_settings().then((settings) => {
					if (settings) {
	...
	get_page_to_show() {
		...
		if (kanivin.boot.user.default_workspace) {
			default_page = {
				name: kanivin.boot.user.default_workspace.title,
				public: kanivin.boot.user.default_workspace.public,
			};
		} else if (
	...
	async show_page(page) {
		...
		if (this.all_pages.length) {
			...
			if (this.pages && this.pages[current_page.name]) {
				...
				await kanivin.after_ajax(() => this.get_data(current_page));
			}
	...
	setup_customization_buttons(page) {
		...
		this.page.set_secondary_action(__("Discard"), async () => {
			...
			this.reload();
			kanivin.show_alert({ message: __("Customizations Discarded"), indicator: "info" });
		});

		if (page.name && this.has_access) {
			this.page.add_inner_button(__("Settings"), () => {
				kanivin.set_route(`workspace/${page.name}`);
			});
		}
	}
	...
	add_sidebar_actions(item, sidebar_control, is_new) {
		if (!item.is_editable) {
			...
			kanivin.utils.add_custom_button(
				kanivin.utils.icon("es-line-duplicate", "sm"),
			...
			kanivin.utils.add_custom_button(
				kanivin.utils.icon("es-line-preview", "sm"),
			...
			kanivin.utils.add_custom_button(
				kanivin.utils.icon("es-line-drag", "xs"),
				null,
				"drag-handle",
				__("Drag"),
				null,
	...
	edit_page(item) {
		...
		const d = new kanivin.ui.Dialog({
			...
			primary_action: (values) => {
				...
				kanivin.call({
					method: "kanivin.desk.doctype.workspace.workspace.update_page",
					args: {
						name: old_item.name,
						title: values.title,
						icon: values.icon || "",
					...
					callback: function (res) {
						if (res.message) {
							...
							kanivin.show_alert({ message: message, indicator: "green" });
						}
					},
				});
				...
				if (this.make_page_selected) {
					...
					let route = pre_url + kanivin.router.slug(values.title);
	...
	update_cached_values(old_item, new_item, duplicate, new_page) {
		...
		// update kanivin.workspaces
		if (kanivin.workspaces[kanivin.router.slug(old_item.name)] || new_page) {
			!duplicate && delete kanivin.workspaces[kanivin.router.slug(old_item.name)];
			if (new_item) {
				kanivin.workspaces[kanivin.router.slug(new_item.name)] = { title: new_item.title };
			}
		}
	...
	add_settings_button(item, sidebar_control) {
		this.dropdown_list = [
			{
				...
				icon: kanivin.utils.icon("es-line-edit", "sm"),
				action: () => this.edit_page(item),
			},
			{
				label: __("Duplicate"),
				title: __("Duplicate Workspace"),
				icon: kanivin.utils.icon("es-line-duplicate", "sm"),
				action: () => this.duplicate_page(item),
			},
			{
				label: __("Hide"),
				title: __("Hide Workspace"),
				icon: kanivin.utils.icon("es-line-hide", "sm"),
		...
		if (this.is_item_deletable(item)) {
			this.dropdown_list.push({
				...
				icon: kanivin.utils.icon("delete-active", "sm"),
		...
		let $button = $(`
			<div class="btn btn-xs setting-btn dropdown-btn" title="${__("Setting")}">
				${kanivin.utils.icon("es-line-dot-horizontal", "xs")}
			</div>
	...
	is_item_deletable(item) {
		...
		if (
			!item.public ||
			(item.public && (!item.module || (item.module && kanivin.boot.developer_mode)))
		)
			return true;
		return false;
	}
	...
	delete_page(page) {
		kanivin.confirm(
			...
			() => {
				kanivin.call({
					method: "kanivin.desk.doctype.workspace.workspace.delete_page",
					args: { page: page },
					callback: function (res) {
						if (res.message) {
	...
	duplicate_page(page) {
		...
		const d = new kanivin.ui.Dialog({
			...
			primary_action: (values) => {
				...
				d.hide();
				kanivin.call({
					method: "kanivin.desk.doctype.workspace.workspace.duplicate_page",
					args: {
						page_name: page.name,
						new_page: values,
					},
				...
				new_page.name = values.title + (new_page.public ? "" : "-" + kanivin.session.user);
				...
				new_page.for_user = new_page.public ? "" : kanivin.session.user;
	...
	hide_unhide_workspace(page, event, hide) {
		...
		kanivin.call({
			method: "kanivin.desk.doctype.workspace.workspace." + method,
			args: {
				page_name: page.name,
			},
			callback: (r) => {
	...
	update_sorted_sidebar() {
		if (this.sorted_public_items || this.sorted_private_items) {
			kanivin.call({
				method: "kanivin.desk.doctype.workspace.workspace.sort_pages",
				args: {
				...
				callback: function (res) {
					if (res.message) {
						...
						kanivin.show_alert({ message: __(message), indicator: "green" });
					}
				},
			});
		}
	...
	initialize_new_page() {
		...
		const d = new kanivin.ui.Dialog({
			...
			primary_action: (values) => {
				...
				let name = values.title + (values.is_public ? "" : "-" + kanivin.session.user);
				let blocks = [
					{
						type: "header",
				...
				let new_page = {
					...
					title: values.title,
					public: values.is_public || 0,
					for_user: values.is_public ? "" : kanivin.session.user,
					icon: values.icon,
					indicator_color: values.indicator_color,
				...
				this.editor
					...
					.then(async () => {
						...
						kanivin.call({
							method: "kanivin.desk.doctype.workspace.workspace.new_page",
							args: {
								new_page: new_page,
							},
							callback: function (res) {
						...
						let route = pre_url + kanivin.router.slug(new_page.title);
						kanivin.set_route(route);
	...
	validate_page(new_page, old_page) {
		...
		if (kanivin.router.doctype_route_exist(kanivin.router.slug(new_page.title))) {
	...
	initialize_editorjs(blocks) {
		this.tools = {
			...
			HeaderSize: kanivin.workspace_block.tunes["header_size"],
		};
	...
	save_page(page) {
		...
		return this.editor
			...
			.then((outputData) => {
				...
				page.content = JSON.stringify(blocks);
				kanivin.call({
					method: "kanivin.desk.doctype.workspace.workspace.save_page",
					args: {
						title: page.title,
	...
	create_page_skeleton() {
		...
		this.body.prepend(kanivin.render_template("workspace_loading_skeleton"));
	...
	create_sidebar_skeleton() {
		...
		$(kanivin.render_template("workspace_sidebar_loading_skeleton")).insertBefore(this.sidebar);
	...
	register_awesomebar_shortcut() {
		"abcdefghijklmnopqrstuvwxyz".split("").forEach((letter) => {
			...
			kanivin.ui.keys.add_shortcut({ shortcut: letter, ...default_shortcut });
			kanivin.ui.keys.add_shortcut({ shortcut: `shift+${letter}`, ...default_shortcut });
		});
	}
};
