...
export default class Block {
	...
	make(block, block_name, widget_type = block) {
		let block_data = this.config.page_data[block + "s"].items.find((obj) => {
			return (
				kanivin.utils.unescape_html(obj.label) == kanivin.utils.unescape_html(__(block_name))
			);
		});
		if (!block_data) return false;
		this.wrapper.innerHTML = "";
		block_data.in_customize_mode = !this.readOnly;
		this.block_widget = new kanivin.widget.SingleWidgetGroup({
	...
	new(block, widget_type = block) {
		...
		this.dialog = new dialog_class({
			...
			primary_action: (widget) => {
				...
				this.block_widget = kanivin.widget.make_widget({
					...widget,
	...
	add_new_block_button() {
		let $new_button = $(`
			<div class="new-block-button">${kanivin.utils.icon("add-round", "lg")}</div>
		`);
	...
	add_settings_button() {
		...
		this.dropdown_list = [
			...
			},
			{
				label: "Expand",
				title: "Expand Block",
				icon: kanivin.utils.icon("expand-alt", "sm"),
				action: () => this.increase_width(),
			},
			{
				label: "Shrink",
				title: "Shrink Block",
				icon: kanivin.utils.icon("shrink", "sm"),
				action: () => this.decrease_width(),
			},
			{
				label: "Move Up",
				title: "Move Up",
				icon: kanivin.utils.icon("up-arrow", "sm"),
				action: () => this.move_block("up"),
			},
			{
				label: "Move Down",
				title: "Move Down",
				icon: kanivin.utils.icon("down-arrow", "sm"),
		...
		let $button = $(`
			<div class="dropdown-btn">
				<button class="btn btn-secondary btn-xs setting-btn" title="${__("Setting")}">
					${kanivin.utils.icon("dot-horizontal", "xs")}
				</button>
