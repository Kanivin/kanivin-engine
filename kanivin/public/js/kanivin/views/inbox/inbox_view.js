/**
 * kanivin.views.InboxView
 */

kanivin.provide("kanivin.views");

kanivin.views.InboxView = class InboxView extends kanivin.views.ListView {
	static load_last_view() {
		const route = kanivin.get_route();
		if (!route[3] && kanivin.boot.email_accounts.length) {
			let email_account;
			if (kanivin.boot.email_accounts[0].email_id == "All Accounts") {
				email_account = "All Accounts";
			} else {
				email_account = kanivin.boot.email_accounts[0].email_account;
			}
			kanivin.set_route("List", "Communication", "Inbox", email_account);
			return true;
		...
		function is_valid(email_account) {
			return kanivin.boot.email_accounts.find((d) => d.email_account === email_account);
		}
	}
	...
	setup_defaults() {
		...
		this.email_account = kanivin.get_route()[3];
	...
	get_seen_class(doc) {
		return Boolean(doc.seen) || JSON.parse(doc._seen || "[]").includes(kanivin.session.user)
			? ""
			: "bold";
	}
	...
	get_meta_html(email) {
		...
		if (email.reference_doctype && email.reference_doctype !== this.doctype) {
			link = `<a class="text-muted grey"
				href="${kanivin.utils.get_form_link(email.reference_doctype, email.reference_name)}"
	...
	get_inbox_filters() {
		...
		if (email_account === "Sent") {
			...
			filters = default_filters.concat([
				...
				["Communication", "email_account", "in", kanivin.boot.all_accounts, true],
			]);
		} else {
			var op = "=";
			if (email_account == "All Accounts") {
				op = "in";
				email_account = kanivin.boot.all_accounts;
			}
	...
	get_no_result_message() {
		...
		} else if (!email_account && !kanivin.boot.email_accounts.length) {
		...
		const html = kanivin.model.can_create(args.doctype)
			? `<p>${args.msg}</p>
			<p>
	...
	make_new_doc() {
		if (!this.email_account && !kanivin.boot.email_accounts.length) {
			kanivin.route_options = {
				email_id: kanivin.session.user_email,
			};
			kanivin.new_doc("Email Account");
		} else {
