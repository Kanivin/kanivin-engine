...
kanivin.provide("kanivin.views.calendar");
kanivin.provide("kanivin.views.calendars");

kanivin.views.CalendarView = class CalendarView extends kanivin.views.ListView {
	static load_last_view() {
		const route = kanivin.get_route();
		if (route.length === 3) {
			const doctype = route[1];
			const user_settings = kanivin.get_user_settings(doctype)["Calendar"] || {};
	...
	setup_defaults() {
		return super.setup_defaults().then(() => {
			...
			this.calendar_settings = kanivin.views.calendar[this.doctype] || {};
			this.calendar_name = kanivin.get_route()[3];
		});
	}
	...
	render() {
		...
		this.load_lib
			...
			.then((options) => {
				this.calendar = new kanivin.views.Calendar(options);
			});
	}
	...
	get_calendar_preferences() {
		...
		return new Promise((resolve) => {
			if (calendar_name === "default") {
				Object.assign(options, kanivin.views.calendar[this.doctype]);
				resolve(options);
			} else {
				kanivin.model.with_doc("Calendar View", calendar_name, () => {
					const doc = kanivin.get_doc("Calendar View", calendar_name);
					if (!doc) {
						kanivin.show_alert(
						...
						kanivin.set_route("List", this.doctype, "Calendar", "default");
						return;
					}
	...
	get required_libs() {
		let assets = [
			"assets/kanivin/js/lib/fullcalendar/fullcalendar.min.css",
			"assets/kanivin/js/lib/fullcalendar/fullcalendar.min.js",
		];
		let user_language = kanivin.boot.lang;
		if (user_language && user_language !== "en") {
			assets.push("assets/kanivin/js/lib/fullcalendar/locale-all.js");
		}
		return assets;
	}
};

kanivin.views.Calendar = class Calendar {
	constructor(options) {
		$.extend(this, options);
	...
	make_page() {
		...
		$.each(kanivin.boot.calendars, function (i, doctype) {
			if (kanivin.model.can_read(doctype)) {
				me.page.add_menu_item(__(doctype), function () {
					kanivin.set_route("List", doctype, "Calendar");
				});
			}
		});
	...
	make() {
		...
		this.footnote_area = kanivin.utils.set_footnote(
			this.footnote_area,
			this.$wrapper,
	...
	set_css() {
		...
		this.$wrapper
			...
			.attr("class", "")
			.html(kanivin.utils.icon("left"));
		this.$wrapper
			.find(".fc-next-button span")
			.attr("class", "")
			.html(kanivin.utils.icon("right"));

		this.$wrapper.find(".fc-today-button").prepend(kanivin.utils.icon("today"));
	...
	get_system_datetime(date) {
		date._offset = moment(date).tz(kanivin.sys_defaults.time_zone)._offset;
		return kanivin.datetime.convert_to_system_tz(moment(date).locale("en"));
	}
	setup_options(defaults) {
		var me = this;
		defaults.meridiem = "false";
		this.cal_options = {
			locale: kanivin.boot.lang,
			header: {
			...
			events: function (start, end, timezone, callback) {
				return kanivin.call({
					method: me.get_events_method || "kanivin.desk.calendar.get_events",
					type: "GET",
			...
			eventClick: function (event) {
				...
				if (kanivin.model.can_read(doctype)) {
					kanivin.set_route("Form", doctype, event.name);
				}
			},
			...
			select: function (startDate, endDate, jsEvent, view) {
				...
				var event = kanivin.model.get_new_doc(me.doctype);
				...
				kanivin.set_route("Form", me.doctype, event.name);
			},
	...
	prepare_events(events) {
		...
		return (events || []).map((d) => {
			d.id = d.name;
			d.editable = kanivin.model.can_write(d.doctype || me.doctype);
			...
			if (d.convertToUserTz) {
				d.start = kanivin.datetime.convert_to_user_tz(d.start);
				d.end = kanivin.datetime.convert_to_user_tz(d.end);
			}

			// show event on single day if start or end date is invalid
			if (!kanivin.datetime.validate(d.start) && d.end) {
				d.start = kanivin.datetime.add_days(d.end, -1);
			}

			if (d.start && !kanivin.datetime.validate(d.end)) {
				d.end = kanivin.datetime.add_days(d.start, 1);
			}

			me.fix_end_date_for_event_render(d);
			me.prepare_colors(d);

			d.title = kanivin.utils.html2text(d.title);
	...
	prepare_colors(d) {
		...
		if (this.get_css_class) {
			...
			if (color_name.startsWith("#")) {
				color_name = kanivin.ui.color.validate_hex(color_name) ? color_name : "blue";
			}

			d.backgroundColor = kanivin.ui.color.get(color_name, "extra-light");
			d.textColor = kanivin.ui.color.get(color_name, "dark");
		} else {
			color = d.color;
			if (!kanivin.ui.color.validate_hex(color) || !color) {
				color = kanivin.ui.color.get("blue", "extra-light");
			}
			d.backgroundColor = color;
			d.textColor = kanivin.ui.color.get_contrast_color(color);
		}
		return d;
	}
	update_event(event, revertFunc) {
		var me = this;
		kanivin.model.remove_from_locals(me.doctype, event.name);
		return kanivin.call({
			method: me.update_event_method || "kanivin.desk.calendar.update_event",
			args: me.get_update_args(event),
			callback: function (r) {
				if (r.exc) {
					kanivin.show_alert(__("Unable to update event"));
					revertFunc();
				}
			},
			error: function () {
