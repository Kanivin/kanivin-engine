...
kanivin.last_edited_communication = {};
const separator_element = "<div>---</div>";

kanivin.views.CommunicationComposer = class {
	constructor(opts) {
		$.extend(this, opts);
		if (!this.doc) {
	...
	make() {
		...
		this.dialog = new kanivin.ui.Dialog({
	...
	get_fields() {
		...
		const fields = [
			...
			{
				fieldtype: "Button",
				label: kanivin.utils.icon("down", "xs"),
			...
			},
			{
				...
				fieldname: "content",
				onchange: kanivin.utils.debounce(this.save_as_draft.bind(this), 300),
			},
			{
				fieldtype: "Button",
			...
			{ fieldtype: "Section Break" },
			{
				...
				default: kanivin.boot.user.send_me_a_copy,
			},
			{
		...
		const email_accounts = kanivin.boot.email_accounts.filter((account) => {
			return (
		...
		if (email_accounts.length) {
			...
			} else if (this.user_email_accounts.includes(kanivin.session.user_email)) {
				this["sender"] = kanivin.session.user_email;
			}
		}
	...
	guess_language() {
		...
		if (print_format != "Standard") {
			print_format_lang = kanivin.get_doc(
				"Print Format",
				print_format
			)?.default_print_language;
		}

		let lang = document_lang || print_format_lang || kanivin.boot.lang;
	...
	toggle_more_options(show_options) {
		...
		const label = kanivin.utils.icon(show_options ? "up-line" : "down", "xs");
	...
	setup_multiselect_queries() {
		["recipients", "cc", "bcc"].forEach((field) => {
			this.dialog.fields_dict[field].get_data = () => {
				...
				kanivin.call({
					method: "kanivin.email.get_contact_list",
					args: args,
					callback: (r) => {
	...
	setup_email_template() {
		...
		function add_template() {
			...
			kanivin.call({
				method: "kanivin.email.doctype.email_template.email_template.get_email_template",
				args: {
		...
		kanivin.utils.add_select_group_button(clear_and_add_template, email_template_actions);
	}
	...
	get_last_edited_communication(clear) {
		if (!kanivin.last_edited_communication[this.doctype]) {
			kanivin.last_edited_communication[this.doctype] = {};
		}

		if (clear || !kanivin.last_edited_communication[this.doctype][this.key]) {
			kanivin.last_edited_communication[this.doctype][this.key] = {};
		}

		return kanivin.last_edited_communication[this.doctype][this.key];
	}
	...
	async set_values() {
		...
		const subject = this.subject ? kanivin.utils.html2text(this.subject) : "";
	...
	setup_print() {
		...
		if (this.frm) {
			const print_formats = kanivin.meta.get_print_formats(this.frm.meta.name);
	...
	setup_attach() {
		...
		$(`
			...
			<p class='add-more-attachments'>
				<button class='btn btn-xs btn-default'>
					${kanivin.utils.icon("small-add", "xs")}&nbsp;
					${__("Add Attachment")}
				</button>
			</p>
		...
		attach
			...
			.on("click", () => new kanivin.ui.FileUploader(args));
			...
			if (files.length) {
				$.each(files, (i, f) => {
					...
					if (!attachment_rows.find(`[data-file-name="${f.name}"]`).length) {
						f.file_url = kanivin.urllib.get_full_url(f.file_url);
	...
	get_attachment_row(attachment, checked) {
		return $(`<p class="checkbox flex">
			<label title="${attachment.file_name}" style="max-width: 100%">
				...
				>
					${kanivin.utils.icon("link-url", "sm")}
				</a>
			</label>
		</p>`);
	}
	...
	setup_email() {
		...
		$(fields.send_me_a_copy.input).on("click", () => {
			...
			kanivin.db.set_value("User", kanivin.session.user, "send_me_a_copy", val);
			kanivin.boot.user.send_me_a_copy = val;
		});
	}
	...
	send_email(btn, form_values, selected_attachments, print_html, print_format) {
		...
		if (!form_values.recipients) {
			kanivin.msgprint(__("Enter Email Recipient(s)"));
			return;
		}
		...
		if (this.frm && !kanivin.model.can_email(this.doc.doctype, this.frm)) {
			kanivin.msgprint(__("You are not allowed to send emails related to this document"));
			return;
		}

		return kanivin.call({
			method: "kanivin.core.doctype.communication.email.make",
			args: {
				...
				sender_full_name: form_values.sender ? kanivin.user.full_name() : undefined,
			...
			callback(r) {
				if (!r.exc) {
					kanivin.utils.play_sound("email");

					if (r.message["emails_not_sent_to"]) {
						kanivin.msgprint(
							__("Email not sent to {0} (unsubscribed / disabled)", [
								kanivin.utils.escape_html(r.message["emails_not_sent_to"]),
							])
						);
					}
	...
	is_print_letterhead_checked() {
		if (this.frm && $(this.frm.wrapper).find(".form-print-wrapper").is(":visible")) {
			...
			return (
				kanivin.model.get_doc(":Print Settings", "Print Settings") || { with_letterhead: 1 }
			).with_letterhead
				? 1
				: 0;
		}
	...
	async get_signature(sender_email) {
		let signature = kanivin.boot.user.email_signature;
		...
		if (!signature) {
			...
			const email_accounts = await kanivin.db.get_list("Email Account", {
				filters: filters,
		...
		if (!kanivin.utils.is_html(signature)) {
	...
	get_earlier_reply() {
		...
		const communication_date = kanivin.datetime.global_date_format(
	...
	html2text(html) {
		...
		const text = kanivin.utils.html2text(html);
		return text.replace(/\n{3,}/g, "\n\n");
	}
};
