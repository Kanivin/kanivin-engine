/**
 * kanivin.views.ImageView
 */
kanivin.provide("kanivin.views");

kanivin.views.ImageView = class ImageView extends kanivin.views.ListView {
	get view_name() {
		return "Image";
	}
	...
	item_html(item) {
		...
		const escaped_title = kanivin.utils.escape_html(title);
	...
	get_attached_images() {
		return kanivin
			.call({
				method: "kanivin.core.api.file.get_attached_images",
				args: {
	...
	get_header_html() {
		...
		// 				${kanivin.utils.icon('heart', 'sm', 'like-icon')}
		// 			</div>
		// 			<div>${__('Liked')}</div>
		// 		</div>
		// 	</div>
	...
	setup_gallery() {
		var me = this;
		this.gallery = new kanivin.views.GalleryView({
			doctype: this.doctype,
			items: this.items,
			wrapper: this.$result,
...
kanivin.views.GalleryView = class GalleryView {
	constructor(opts) {
		$.extend(this, opts);
		var me = this;
	...
	prepare() {
		...
		if (this.pswp_root.length === 0) {
			var pswp = kanivin.render_template("photoswipe_dom");
	...
	load_lib() {
		return new Promise((resolve) => {
			var asset_dir = "assets/kanivin/js/lib/photoswipe/";
			kanivin.require(
				[
