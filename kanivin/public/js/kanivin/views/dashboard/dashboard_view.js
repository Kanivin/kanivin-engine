...
kanivin.views.DashboardView = class DashboardView extends kanivin.views.ListView {
	get view_name() {
		return "Dashboard";
	}
	...
	setup_defaults() {
		return super.setup_defaults().then(() => {
			...
			this.dashboard_settings =
				kanivin.get_user_settings(this.doctype)["dashboard_settings"] || null;
		});
	}
	...
	render_empty_state() {
		...
		const empty_state_image = "/assets/kanivin/images/ui-states/list-empty-state.svg";
	...
	save_dashboard_customization() {
		...
		kanivin.model.user_settings.save(
			this.doctype,
			"dashboard_settings",
			this.dashboard_settings
		);
	...
	reset_dashboard_customization() {
		kanivin.confirm(__("Are you sure you want to reset all customizations?"), () => {
			this.dashboard_settings = null;
			kanivin.model.user_settings
		...
		const dialog = new kanivin.ui.Dialog({
			...
			fields: [
				...
				},
				{
					...
					get_query: () => {
						return {
							query: "kanivin.desk.doctype.dashboard_chart.dashboard_chart.get_charts_for_user",
							filters: {
								document_type: this.doctype,
							},
						};
			...
			primary_action: (values) => {
				...
				if (chart.new_or_existing == "New Chart") {
					...
					kanivin
						.xcall(
							"kanivin.desk.doctype.dashboard_chart.dashboard_chart.create_dashboard_chart",
							{ args: chart }
						)
						.then((doc) => {
	...
	get_field_options() {
		...
		kanivin.get_meta(this.doctype).fields.map((df) => {
