...
kanivin.ui.form.AssignTo = class AssignTo {
	constructor(opts) {
		$.extend(this, opts);
	...
	render(assignments) {
		...
		let avatar_group = kanivin.avatar_group(assigned_users, 5, {
			align: "left",
			overlap: true,
		});
		...
		avatar_group.click(() => {
			new kanivin.ui.form.AssignmentDialog({
	...
	add() {
		...
		if (this.frm.is_new()) {
			kanivin.throw(__("Please save the document before assignment"));
			return;
		}

		if (!me.assign_to) {
			me.assign_to = new kanivin.ui.form.AssignToDialog({
	...
	remove(owner) {
		if (this.frm.is_new()) {
			kanivin.throw(__("Please save the document before removing assignment"));
			return;
		}

		return kanivin
			.xcall("kanivin.desk.form.assign_to.remove", {
...
kanivin.ui.form.AssignToDialog = class AssignToDialog {
	constructor(opts) {
		$.extend(this, opts);
	...
	make() {
		...
		me.dialog = new kanivin.ui.Dialog({
	...
	assign_to_me() {
		...
		if (me.dialog.get_value("assign_to_me")) {
			assign_to.push(kanivin.session.user);
		}
		...
		return [
			...
			},
			{
				...
				get_data: function (txt) {
					return kanivin.db.get_link_options("User", txt, {
...
kanivin.ui.form.AssignmentDialog = class {
	constructor(opts) {
		this.frm = opts.frm;
	...
	make() {
		...
		this.assignment_list.removeClass("kanivin-control");
	...
	add_assignment(assignment) {
		return kanivin
			.xcall("kanivin.desk.form.assign_to.add", {
				doctype: this.frm.doctype,
				name: this.frm.docname,
	...
	remove_assignment(assignment) {
		return kanivin.xcall("kanivin.desk.form.assign_to.remove", {
			doctype: this.frm.doctype,
			name: this.frm.docname,
			assign_to: assignment,
		});
	...
	close_assignment(assignment) {
		return kanivin.xcall("kanivin.desk.form.assign_to.close", {
			doctype: this.frm.doctype,
			name: this.frm.docname,
			assign_to: assignment,
		});
	...
	get_assignment_row(assignment) {
		const row = $(`
			<div class="dialog-assignment-row" data-user="${assignment}">
				<div class="assignee">
					${kanivin.avatar(assignment)}
					${kanivin.user.full_name(assignment)}
				</div>
		...
		if (assignment === kanivin.session.user || this.frm.perm[0].write) {
			btn_group.append(`
				<button type="button" class="btn btn-default remove-btn" title="${__("Cancel")}">
				${kanivin.utils.icon("close")}
				</button>
			`);
