kanivin.ui.form.SidebarUsers = class {
	constructor(opts) {
		$.extend(this, opts);
	}
	...
	show_in_sidebar(users, type, show_alert) {
		...
		users.current.forEach((username) => {
			if (username === kanivin.session.user) {
				// current user
				return;
			}

			var user_info = kanivin.user_info(username);
			sidebar_users.push({
		...
		if (sidebar_users.length) {
			...
			this.parent.append(
				kanivin.render_template("users_in_sidebar", { users: sidebar_users })
			);
		} else {
	...
	show_alert(users, message) {
		if (users.length) {
			if (users.length === 1) {
				kanivin.show_alert(__("{0} is currently {1}", [users[0], message]));
			} else {
				kanivin.show_alert(
					__("{0} are currently {1}", [kanivin.utils.comma_and(users), message])
				);
			}
		}
	}
