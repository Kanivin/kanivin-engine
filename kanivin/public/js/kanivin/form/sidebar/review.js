...
kanivin.ui.form.Review = class Review {
	constructor({ parent, frm }) {
		this.parent = parent;
		this.frm = frm;
		this.points = kanivin.boot.points;
	...
	update_points() {
		return kanivin
			.xcall("kanivin.social.doctype.energy_point_log.energy_point_log.get_energy_points", {
				user: kanivin.session.user,
			})
			.then((data) => {
				kanivin.boot.points = data;
				this.points = data;
			});
	}
	...
	show_review_dialog() {
		...
		const review_dialog = new kanivin.ui.Dialog({
			title: __("Add Review"),
			fields: [
				{
					fieldname: "to_user",
			...
			primary_action: (values) => {
				...
				if (values.points > this.points.review_points) {
					return kanivin.msgprint(__("You do not have enough points"));
				}
				kanivin
					.xcall("kanivin.social.doctype.energy_point_log.energy_point_log.review", {
						doc: {
	...
	setup_detail_popover(el, data) {
		let subject = "";
		let fullname = kanivin.user.full_name(data.user);
		let timestamp = `<span class="text-muted">${kanivin.datetime.comment_when(
			data.creation
		)}</span>`;
			...
			content: () => {
				return `
					<div class="text-medium">
						...
						<div class="subject">
							${kanivin.utils.icon("review")}
							${subject}

							<p class="mt-1">
								<!-- ${kanivin.avatar("shivam@example.com", "avatar-xs")} -->
								<span>${kanivin.user.full_name(data.owner)}</span> - ${timestamp}
							</p>
						</div>
					</div>
				`;
