kanivin.ui.form.set_user_image = function (frm) {
	...
	if (image) {
		image = window.cordova && image.indexOf("http") === -1 ? kanivin.base_url + image : image;
		...
		image_section
			...
			.removeClass("hide")
			.find(".standard-image")
			.html(kanivin.get_abbr(title));

		title_image.css("background-image", "").html(kanivin.get_abbr(title));
...
kanivin.ui.form.setup_user_image_event = function (frm) {
	// re-draw image on change of user image
	if (frm.meta.image_field) {
		kanivin.ui.form.on(frm.doctype, frm.meta.image_field, function (frm) {
			kanivin.ui.form.set_user_image(frm);
		});
	}
