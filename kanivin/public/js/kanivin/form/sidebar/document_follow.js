...
kanivin.ui.form.DocumentFollow = class DocumentFollow {
	constructor(opts) {
		$.extend(this, opts);
		if (!kanivin.boot.user.document_follow_notify) {
	...
	render_sidebar() {
		...
		const document_follow_enabled = kanivin.boot.user.document_follow_notify;
		const document_can_be_followed = kanivin.get_meta(this.frm.doctype).track_changes;
		if (
	...
	bind_events() {
		this.follow_document_link.on("click", () => {
			...
			kanivin.call({
				method: "kanivin.desk.form.document_follow.follow_document",
				args: {
		...
		this.unfollow_document_link.on("click", () => {
			...
			kanivin.call({
				method: "kanivin.desk.form.document_follow.unfollow_document",
				args: {
					doctype: this.frm.doctype,
					doc_name: this.frm.doc.name,
					user: kanivin.session.user,
	...
	get_followed_user() {
		...
		return new Promise((resolve) => {
			kanivin
				.call({
					method: "kanivin.desk.form.document_follow.get_follow_users",
					args: {
						doctype: this.frm.doctype,
						doc_name: this.frm.doc.name,
					},
				...
				.then((r) => {
					...
					for (var d in r.message) {
						...
						if (this.count_others < 4) {
							html += kanivin.avatar(r.message[d].user, "avatar-small");
						}
