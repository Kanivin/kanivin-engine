...
kanivin.ui.form.Sidebar = class {
	constructor(opts) {
		$.extend(this, opts);
	}

	make() {
		var sidebar_content = kanivin.render_template("form_sidebar", {
			doctype: this.frm.doctype,
			frm: this.frm,
			can_write: kanivin.model.can_write(this.frm.doctype, this.frm.docname),
		});
		...
		kanivin.ui.form.setup_user_image_event(this.frm);
	...
	bind_events() {
		...
		this.comments.on("click", function () {
			kanivin.utils.scroll_to(me.frm.footer.wrapper.find(".comment-box"), true);
		});

		this.like_icon.on("click", function () {
			kanivin.ui.toggle_like(me.like_wrapper, me.frm.doctype, me.frm.doc.name, function () {
				me.refresh_like();
			});
		});
	}
	...
	setup_keyboard_shortcuts() {
		...
		kanivin.ui.keys.get_shortcut_group(this.page).add(assignment_link);
	}
	...
	refresh() {
		if (this.frm.doc.__islocal) {
			...
			if (this.frm.doc.route && cint(kanivin.boot.website_tracking_enabled)) {
				...
				kanivin.utils.get_page_view_count(route).then((res) => {
					this.sidebar
						.find(".pageview-count")
			...
			kanivin.ui.form.set_user_image(this.frm);
		}
	}
			...
			kanivin.call({
				...
				callback: function (res) {
					...
					me.sidebar.find(".auto-repeat-status").on("click", function () {
						kanivin.set_route("Form", "Auto Repeat", me.frm.doc.auto_repeat);
					});
				},
			});
		}
	...
	make_tags() {
		...
		this.frm.tags = new kanivin.ui.TagEditor({
			parent: tags_parent,
	...
	make_attachments() {
		var me = this;
		this.frm.attachments = new kanivin.ui.form.Attachments({
	...
	make_assignments() {
		this.frm.assign_to = new kanivin.ui.form.AssignTo({
	...
	make_shared() {
		this.frm.shared = new kanivin.ui.form.Share({
			frm: this.frm,
	...
	make_like() {
		...
		kanivin.ui.setup_like_popover(this.sidebar.find(".form-stats-likes"), ".like-icon");
	}
	...
	make_follow() {
		...
		this.follow_button.on("click", () => {
			...
			kanivin
				.call("kanivin.desk.form.document_follow.update_follow", {
					doctype: this.frm.doctype,
					doc_name: this.frm.doc.name,
					following: !is_followed,
				})
				.then(() => {
					kanivin.model.set_docinfo(
						this.frm.doctype,
						this.frm.doc.name,
	...
	refresh_like() {
		...
		const liked = kanivin.ui.is_liked(this.frm.doc);
		this.like_wrapper
	...
	make_review() {
		...
		if (kanivin.boot.energy_points_enabled && !this.frm.is_new()) {
			this.frm.reviews = new kanivin.ui.form.Review({
	...
	reload_docinfo(callback) {
		kanivin.call({
			method: "kanivin.desk.form.load.get_docinfo",
			args: {
				doctype: this.frm.doctype,
