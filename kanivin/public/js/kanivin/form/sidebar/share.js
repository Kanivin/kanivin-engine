...
kanivin.ui.form.Share = class Share {
	constructor(opts) {
		$.extend(this, opts);
	...
	render_sidebar() {
		...
		this.shares.show();
		let avatar_group = kanivin.avatar_group(shared_users, 5, { align: "left", overlap: true });
	...
	show() {
		...
		kanivin.call({
			method: "kanivin.share.get_users",
			args: {
	...
	render_shared(shared) {
		...
		$(
			kanivin.render_template("set_sharing", {
				frm: this.frm,
				shared: this.shared,
				everyone: everyone,
			})
		...
		if (kanivin.model.can_share(null, this.frm)) {
	...
	make_user_input() {
		// make add-user input
		this.dialog.share_with = kanivin.ui.form.make_control({
	...
	set_edit_share_events() {
		...
		$(d.body)
			...
			.on("click", function () {
				...
				kanivin.call({
					method: "kanivin.share.set_permission",
					args: {
						doctype: me.frm.doctype,
						name: me.frm.doc.name,
						user: user,
