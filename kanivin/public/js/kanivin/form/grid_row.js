...
export default class GridRow {
	...
	set_docfields(update = false) {
		if (this.doc && this.parent_df.options) {
			kanivin.meta.make_docfield_copy_for(
				this.parent_df.options,
				this.doc.name,
				this.docfields
			);
			const docfields = kanivin.meta.get_docfields(this.parent_df.options, this.doc.name);
			if (update) {
			...
			if (this.frm) {
				kanivin
					.run_serially([
						...
						() => {
							kanivin.model.clear_doc(this.doc.doctype, this.doc.name);
		...
		kanivin.prompt(
			...
			function (values) {
				if (me.doc._sortable === false) {
					kanivin.msgprint(__("Cannot move row"));
					return;
				}
	...
	render_template() {
		...
		this.row_display = $('<div class="row-data sortable-handle template-row"></div>')
			.appendTo(this.row)
			.html(
				kanivin.render(this.grid.template, {
					doc: this.doc ? kanivin.get_format_helper(this.doc) : null,
					frm: this.frm,
					row: this,
				})
			);
	...
	render_row(refresh) {
		...
		if (!this.row_index && !this.show_search) {
			...
			kanivin.utils.only_allow_num_decimal(this.row_index.find("input"));
		}
				...
				if (!this.configure_columns) {
					...
					this.open_form_button = $(`
						<div class="btn-open-row" data-toggle="tooltip" data-placement="right" title="${edit_msg}">
							<a>${kanivin.utils.icon("edit", "xs")}</a>
						</div>
					`)
		...
		if (this.configure_columns && this.frm) {
			this.configure_columns_button = $(`
				<div class="col grid-static-col d-flex justify-content-center" style="cursor: pointer;">
					<a>${kanivin.utils.icon("setting-gear", "sm", "", "filter: opacity(0.5)")}</a>
				</div>
			`)
				.appendTo(this.row)
				.on("click", () => {
	...
	configure_dialog_for_columns_selector() {
		this.grid_settings_dialog = new kanivin.ui.Dialog({
		...
		d.set_primary_action(__("Add"), () => {
			...
			if (selected_fields) {
				selected_fields.forEach((selected_column) => {
					let docfield = kanivin.meta.get_docfield(this.grid.doctype, selected_column);
	...
	prepare_columns_for_dialog(selected_fields) {
		...
		const blocked_fields = kanivin.model.no_value_type;
	...
	render_selected_columns() {
		...
		if (this.selected_columns_for_grid) {
			this.selected_columns_for_grid.forEach((d) => {
				let docfield = kanivin.meta.get_docfield(this.grid.doctype, d.fieldname);
					...
					<div class='control-input flex align-center form-control fields_order sortable-handle sortable'
						...
						<div class='row'>
							<div class='col-1' style='padding-top: 4px;'>
								<a style='cursor: grabbing;'>${kanivin.utils.icon("drag", "xs")}</a>
							</div>
		...
		$(this.fields_html_wrapper)
			...
			.change((event) => {
				if (cint(event.target.value) === 0) {
					...
					kanivin.throw(__("Column width cannot be zero."));
				}
	...
	validate_columns_width() {
		...
		if (total_column_width && total_column_width > 10) {
			kanivin.throw(__("The total column width cannot be more than 10."));
		}
	}
		...
		$(this.fields_html_wrapper)
			...
			.click((event) => {
				...
				if (selected_columns_for_grid && selected_columns_for_grid.length === 0) {
					kanivin.throw(__("At least one column is required to show in the grid."));
				}
	...
	update_user_settings_for_grid() {
		...
		kanivin.model.user_settings.save(this.frm.doctype, "GridView", value).then((r) => {
			kanivin.model.user_settings[this.frm.doctype] = r.message || r;
			this.grid.reset_grid();
		});
	}

	reset_user_settings_for_grid() {
		kanivin.model.user_settings.save(this.frm.doctype, "GridView", null).then((r) => {
			kanivin.model.user_settings[this.frm.doctype] = r.message || r;
			this.grid.reset_grid();
		});
	}
	...
	setup_columns() {
		...
		this.grid.visible_columns.forEach((col, ci) => {
			...
			let txt = this.doc
				? kanivin.format(this.doc[df.fieldname], df, null, this.doc)
	...
	evaluate_depends_on_value(expression) {
		...
		if (typeof expression === "boolean") {
			...
			try {
				out = kanivin.utils.eval(expression.substr(5), { doc, parent });
	...
	make_search_column(df, colsize) {
		...
		["Currency", "Float", "Int", "Percent", "Rating"].includes(df.fieldtype) &&
			kanivin.utils.only_allow_num_decimal($search_input);
		...
		var $col = $(
			...
			.on("touchmove", function (event) {
				...
				if (horizontal) {
					...
					grid.style.left = `${kanivin.utils.is_rtl() ? "" : "-"}${grid_start}px`;
				}
			})
			...
			.on("click", function (event) {
				if (kanivin.ui.form.editable_row !== me) {
	...
	toggle_editable_row(show) {
		...
		if (
			...
			if (kanivin.ui.form.editable_row && kanivin.ui.form.editable_row !== this) {
				kanivin.ui.form.editable_row.toggle_editable_row(false);
			}
			...
			kanivin.ui.form.editable_row = this;
			return false;
		} else {
			...
			kanivin.ui.form.editable_row = null;
		}
	}
	...
	make_control(column) {
		...
		var field = kanivin.ui.form.make_control({
			df: df,
			parent: parent,
			only_input: true,
			with_link_btn: true,
	...
	set_arrow_keys(field) {
		...
		if (field.$input) {
			field.$input.on("keydown", function (e) {
				var { TAB, UP: UP_ARROW, DOWN: DOWN_ARROW } = kanivin.ui.keyCode;
	...
	show_form() {
		...
		kanivin.dom.freeze("", "dark grid-form");
		if (cur_frm) cur_frm.cur_grid = this;
		this.wrapper.addClass("grid-row-open");
		if (
			!kanivin.dom.is_element_in_viewport(this.wrapper) &&
			!kanivin.dom.is_element_in_modal(this.wrapper)
		) {
			// -15 offset to make form look visually centered
			kanivin.utils.scroll_to(this.wrapper, true, -15);
		}
	...
	hide_form() {
		...
		if (!kanivin.dom.is_element_in_modal(this.row)) {
			kanivin.utils.scroll_to(this.row, true, 15);
		}
		this.refresh();
	...
	refresh_field(fieldname, txt) {
		...
		if (df && this.doc) {
			txt = kanivin.format(this.doc[fieldname], df, null, this.doc);
		}

		if (!txt && this.frm) {
			txt = kanivin.format(this.doc[fieldname], df, null, this.frm.doc);
		}
	...
	get_visible_columns(blacklist = []) {
		...
		var visible_columns = $.map(this.docfields, function (df) {
			var visible =
				...
				!kanivin.model.layout_fields.includes(df.fieldtype) &&
