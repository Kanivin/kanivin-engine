kanivin.ui.form.ControlCode = class ControlCode extends kanivin.ui.form.ControlText {
	make_input() {
	...
	set_copy_button() {
		...
		if ((codeField && this.df.read_only === 1) || (codeField && this.frm.doc.docstatus > 0)) {
			...
			this.button.on("click", () => {
				kanivin.utils.copy_to_clipboard(
					kanivin.model.get_value(this.doctype, this.docname, this.df.fieldname)
				);
			});
	...
	make_ace_editor() {
		...
		let change_content = kanivin.utils.debounce(() => {
	...
	load_lib() {
		...
		if (kanivin.boot.developer_mode) {
			this.root_lib_path = "/assets/kanivin/node_modules/ace-builds/src-noconflict/";
		} else {
			this.root_lib_path = "/assets/kanivin/node_modules/ace-builds/src-min-noconflict/";
		}

		this.library_loaded = new Promise((resolve) => {
			kanivin.require(this.root_lib_path + "ace.js", () => {
