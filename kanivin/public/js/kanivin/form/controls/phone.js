...
kanivin.ui.form.ControlPhone = class ControlPhone extends kanivin.ui.form.ControlData {
	async make_input() {
	...
	async setup_country_codes() {
		...
		if (data) {
			...
			const data = await kanivin.xcall("kanivin.geo.country_info.get_country_timezone_info");
	...
	input_events() {
		this.$input.keydown((e) => {
			...
			if ([kanivin.ui.keyCode.BACKSPACE].includes(key_code)) {
	...
	setup_country_code_picker() {
		...
		if (!this.selected_icon.length) {
			this.selected_icon = $(
				`<div class="selected-phone">${kanivin.utils.icon("down", "sm")}</div>`
			);
	...
	get_country_flag(country) {
		...
		return kanivin.utils.flag(code);
	}
