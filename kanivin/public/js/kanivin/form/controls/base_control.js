kanivin.ui.form.Control = class BaseControl {
	constructor(opts) {
		$.extend(this, opts);
		this.make();
	...
	make_wrapper() {
		this.$wrapper = $("<div class='kanivin-control'></div>").appendTo(this.parent);
	...
	get_status(explain) {
		...
		var status = kanivin.perm.get_field_display_status(
			this.df,
			kanivin.model.get_doc(this.doctype, this.docname),
		...
		let value = kanivin.model.get_value(this.doctype, this.docname, this.df.fieldname);
	...
	show_translatable_button(value) {
		...
		if (
			...
			!kanivin.model.can_write("Translation") ||
			!value
		)
			return;

		// Disable translation in website
		if (!kanivin.views || !kanivin.views.TranslationManager) return;
		...
		$(translation_btn)
			...
			.on("click", () => {
				if (!this.doc.__islocal) {
					new kanivin.views.TranslationManager({
						df: this.df,
