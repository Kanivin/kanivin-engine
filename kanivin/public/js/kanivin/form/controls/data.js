kanivin.provide("kanivin.phone_call");

kanivin.ui.form.ControlData = class ControlData extends kanivin.ui.form.ControlInput {
	...
	make_input() {
		...
		this.$input.on("paste", (e) => {
			...
			if (maxlength && pasted_data.length > maxlength) {
				...
				if (this.frm && kanivin.model.can_write(this.frm.doctype)) {
					let doctype_edit_link = null;
					if (this.frm.meta.custom) {
						doctype_edit_link = kanivin.utils.get_form_link(
							"DocType",
							this.frm.doctype,
							true,
							__("this form")
		...
		if (this.df.with_copy_button) {
			this.$wrapper
				...
				.append(
					...
					${kanivin.utils.icon("clipboard", "sm")}
				</button>`
				)
				.find(".action-btn")
				.click(() => {
					kanivin.utils.copy_to_clipboard(this.value);
				});
		}
	}
	...
	bind_change_event() {
		...
		if (this.constructor.trigger_change_on_input_event && !this.in_grid()) {
			...
			this.$input.on("input", kanivin.utils.debounce(change_handler, 500));
		}
	}
	setup_autoname_check() {
		if (!this.df.parent) return;
		this.meta = kanivin.get_meta(this.df.parent);
		if (
			this.meta &&
			((this.meta.autoname &&
	...
	validate(v) {
		...
		if (this.df.options == "Phone") {
			...
			var email_list = kanivin.utils.split_emails(v);
			if (!email_list) {
				return "";
			} else {
...
kanivin.ui.form.ControlReadOnly = kanivin.ui.form.ControlData;
