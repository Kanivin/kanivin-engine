...
kanivin.ui.form.ControlTextEditor = class ControlTextEditor extends kanivin.ui.form.ControlCode {
	make_wrapper() {
		super.make_wrapper();
	}
	...
	bind_events() {
		this.quill.on(
			"text-change",
			kanivin.utils.debounce((delta, oldDelta, source) => {
		...
		$(this.quill.root).on("keydown", (e) => {
			const key = kanivin.ui.keys && kanivin.ui.keys.get_key(e);
	...
	get_mention_options() {
		...
		return {
			...
			source: kanivin.utils.debounce(async function (search_term, renderList) {
				let method =
					me.mention_search_method || "kanivin.desk.search.get_names_for_mentions";
				let values = await kanivin.xcall(method, {
					search_term,
				});
			...
			renderItem(item) {
				...
				return `${value} ${item.is_group ? kanivin.utils.icon("users") : ""}`;
			},
		};
	}
	...
	parse(value) {
		...
		return kanivin.dom.remove_script_and_style(value);
	}
