kanivin.ui.form.ControlPassword = class ControlPassword extends kanivin.ui.form.ControlData {
	...
	make_input() {
		...
		this.toggle_password.on("click", () => {
			if (this.$input.attr("type") === "password") {
				...
				this.toggle_password.html(kanivin.utils.icon("hide", "sm"));
			} else {
				this.$input.attr("type", "password");
				this.toggle_password.html(kanivin.utils.icon("unhide", "sm"));
			}
		});
	...
	get_password_strength(value) {
		...
		kanivin.call({
			type: "POST",
			method: "kanivin.core.doctype.user.user.test_password_strength",
			args: {
				new_password: value || "",
			},
			callback: function (r) {
