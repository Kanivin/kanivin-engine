kanivin.ui.form.ControlCurrency = class ControlCurrency extends kanivin.ui.form.ControlFloat {
	...
	get_precision() {
		...
		if (typeof this.df.precision != "number" && !this.df.precision) {
			if (kanivin.boot.sysdefaults.currency_precision) {
				this.df.precision = kanivin.boot.sysdefaults.currency_precision;
			} else {
