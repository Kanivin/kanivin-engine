...
kanivin.ui.form.ControlGeolocation = class ControlGeolocation extends kanivin.ui.form.ControlData {
	...
	set_disp_area(value) {
		...
		if (this.frm) {
			...
			$(document).on("kanivin.ui.Dialog:shown", () => {
				this.make_map();
			});
		}
	}
	...
	bind_leaflet_map() {
		...
		this.map.setView(kanivin.utils.map_defaults.center, kanivin.utils.map_defaults.zoom);
	...
	bind_leaflet_draw_control() {
		if (
			!kanivin.perm.has_perm(this.doctype, this.df.permlevel, "write", this.doc) ||
			this.df.read_only
		) {
			return;
		}
	...
	get_leaflet_controls() {
		return new L.Control.Draw({
			...
			draw: {
				polyline: {
					shapeOptions: {
						color: kanivin.ui.color.get("blue"),
						weight: 10,
					},
				},
				polygon: {
					...
					drawError: {
						color: kanivin.ui.color.get("orange"), // Color the shape will turn when intersects
