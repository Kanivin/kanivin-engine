kanivin.ui.form.ControlTableMultiSelect = class ControlTableMultiSelect extends (
	kanivin.ui.form.ControlLink
) {
	make_input() {
		super.make_input();
		...
		this.$input_area.on("click", ".btn-link-to-form", (e) => {
			...
			const value = decodeURIComponent($value.data().value);
			const link_field = this.get_link_field();
			kanivin.set_route("Form", link_field.options, value);
		});
		this.$input.on("keydown", (e) => {
			// if backspace key pressed on empty input, delete last value
			if (e.keyCode == kanivin.ui.keyCode.BACKSPACE && e.target.value === "") {
	...
	parse(value, label) {
		...
		if (value) {
			if (this.frm) {
				const new_row = kanivin.model.add_child(
					this.frm.doc,
					this.df.options,
					this.df.fieldname
				);
			...
			kanivin.utils.add_link_title(link_field.options, value, label);
		}
	...
	get_pill_html(value) {
		...
		const pill_name = kanivin.utils.get_link_title(link_field.options, value) || value;
		return `
			<button class="data-pill btn tb-selected-value" data-value="${encoded_value}">
				<span class="btn-link-to-form">${__(pill_name)}</span>
				<span class="btn-remove">${kanivin.utils.icon("close")}</span>
			</button>
		`;
	}
	get_options() {
	...
	get_link_field() {
		if (!this._link_field) {
			const meta = kanivin.get_meta(this.df.options);
