kanivin.ui.form.ControlSelect = class ControlSelect extends kanivin.ui.form.ControlData {
	...
	set_icon(is_xs_input) {
		const select_icon_html = `<div class="select-icon ${is_xs_input ? "xs" : ""}">
				${kanivin.utils.icon("select", is_xs_input ? "xs" : "sm")}
			</div>`;
		if (this.only_input) {
	...
	get_file_attachment_list() {
		if (!this.frm) return;
		var fl = kanivin.model.docinfo[this.frm.doctype][this.frm.docname];
...
kanivin.ui.form.add_options = function (input, options_list, sort, doctype) {
	let $select = $(input);
...
(function ($) {
	$.fn.add_options = function (options_list, sort) {
		return kanivin.ui.form.add_options(this.get(0), options_list, sort);
	};
