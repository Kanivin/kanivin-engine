kanivin.ui.form.ControlMultiSelectList = class ControlMultiSelectList extends (
	kanivin.ui.form.ControlData
) {
	...
	set_selectable_items(options) {
		let html = options
			.map((option) => {
				...
				<div class="multiselect-check">${kanivin.utils.icon("tick", "xs")}</div>
			</li>`;
			})
			.join("");
		if (!html) {
