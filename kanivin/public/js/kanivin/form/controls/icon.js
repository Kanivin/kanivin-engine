...
kanivin.ui.form.ControlIcon = class ControlIcon extends kanivin.ui.form.ControlData {
	make_input() {
	...
	get_all_icons() {
		...
		$("#all-symbols > svg > symbol[id]").each(function () {
			this.id.includes("icon-") && kanivin.symbols.push(this.id.replace("icon-", ""));
		});
	}
	...
	make_icon_input() {
		...
		if (!this.selected_icon) {
			this.selected_icon = $(
				`<div class="selected-icon">${kanivin.utils.icon("folder-normal", "md")}</div>`
			);
