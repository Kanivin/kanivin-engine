...
kanivin.ui.form.recent_link_validations = {};

kanivin.ui.form.ControlLink = class ControlLink extends kanivin.ui.form.ControlData {
		...
		$(`<div class="link-field ui-front" style="position: relative;">
			...
			<span class="link-btn">
				<a class="btn-clear" style="display: inline-block;" title="${__("Clear Link")}">
					${kanivin.utils.icon("close", "xs", "es-icon")}
				</a>
				<a class="btn-open" style="display: inline-block;" title="${__("Open Link")}">
					${kanivin.utils.icon("arrow-right", "xs")}
				</a>
			</span>
	...
	show_link_and_clear_buttons() {
		if (this.$input.val() && this.get_options()) {
			...
			this.$link_open.attr("href", kanivin.utils.get_form_link(doctype, name));
	...
	get_reference_doctype() {
		...
		if (this.doctype) return this.doctype;
			...
			return kanivin.get_route && kanivin.get_route()[0] === "List"
				? kanivin.get_route()[1]
				: null;
		}
	}
	...
	is_translatable() {
		return (kanivin.boot?.translated_doctypes || []).includes(this.get_options());
	}
	is_title_link() {
		return (kanivin.boot?.link_title_doctypes || []).includes(this.get_options());
	}
	async set_link_title(value) {
		...
		const link_title =
			kanivin.utils.get_link_title(doctype, value) ||
			(await kanivin.utils.fetch_link_title(doctype, value));
	...
	parse_validate_and_set_in_model(value, e, label) {
		...
		if (label) {
			...
			kanivin.utils.add_link_title(this.df.options, value, label);
		}
	...
	new_doc() {
		...
		if (df && df.get_route_options_for_new_doc) {
			kanivin.route_options = df.get_route_options_for_new_doc(this);
		} else {
			kanivin.route_options = {};
		}

		// partially entered name field
		kanivin.route_options.name_field = this.get_label_value();

		// reference to calling link
		kanivin._from_link = kanivin.utils.deep_clone(this);
		kanivin._from_link_scrollY = $(document).scrollTop();

		kanivin.ui.form.make_quick_entry(doctype, (doc) => {
	...
	setup_awesomeplete() {
		...
		this.awesomplete = new Awesomplete(me.input, {
			...
			item: function (item) {
				...
				return $(`<div role="option">`)
					...
					.html(`<p title="${kanivin.utils.escape_html(_label)}">${html}</p>`)
					.get(0);
			},
			sort: function () {
				return 0;
		...
		this.$input.on(
			"input",
			kanivin.utils.debounce(function (e) {
				...
				var args = {
					...
					page_length: cint(kanivin.boot.sysdefaults?.link_field_results_limit) || 10,
				};
					...
					callback: function (r) {
						...
						if (!me.df.only_select) {
							...
							let custom__link_options =
								kanivin.ui.form.ControlLink.link_options &&
								kanivin.ui.form.ControlLink.link_options(me);
						...
						r.message.forEach((item) => {
							kanivin.utils.add_link_title(doctype, item.value, item.label);
						});
					},
				});
			}, 500)
		...
		this.$input.on("awesomplete-select", function (e) {
			...
			if (me.df.remember_last_selected_value) {
				kanivin.boot.user.last_selected_values[me.df.options] = item.value;
			}
	...
	toggle_href(doctype) {
		if (kanivin.model.can_select(doctype) && !kanivin.model.can_read(doctype)) {
	...
	get_filter_description(filters) {
		...
		function get_filter_description(filter) {
			...
			let docfield = kanivin.meta.get_docfield(doctype, fieldname);
			let label = docfield ? docfield.label : kanivin.model.unscrub(fieldname);
	...
	set_custom_query(args) {
		const is_valid_value = (value, key) => {
			...
			if (this.frm) {
				let field = kanivin.meta.get_docfield(this.frm.doctype, key);
	...
	parse_filters(link_filters) {
		...
		link_filters.forEach((filter) => {
			...
			if (value?.startsWith?.("eval:")) {
				...
				value = kanivin.utils.eval(value, context);
			}
	...
	validate_link_and_fetch(value) {
		...
		if (value) {
			return kanivin
				.xcall("kanivin.client.validate_link", {
					doctype: options,
					docname: value,
