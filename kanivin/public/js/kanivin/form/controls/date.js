kanivin.ui.form.ControlDate = class ControlDate extends kanivin.ui.form.ControlData {
	...
	set_formatted_input(value) {
		...
		if (should_refresh) {
			this.datepicker.selectDate(kanivin.datetime.str_to_obj(value));
		}
	}
	set_date_options() {
		// webformTODO:
		...
		let lang = "en";
		kanivin.boot.user && (lang = kanivin.boot.user.language);
		...
		this.date_format = kanivin.defaultDateFormat;
		this.datepicker_options = {
			...
			firstDay: kanivin.datetime.get_first_day_of_the_week_index(),
			onSelect: () => {
	...
	get_now_date() {
		return kanivin.datetime
			.convert_to_system_tz(kanivin.datetime.now_date(true), false)
			.toDate();
	}
	set_t_for_today() {
		var me = this;
		this.$input.on("keydown", function (e) {
			if (e.which === 84) {
				...
				if (me.df.fieldtype == "Date") {
					me.set_value(kanivin.datetime.nowdate());
				}
				if (me.df.fieldtype == "Datetime") {
					me.set_value(kanivin.datetime.now_datetime());
				}
				if (me.df.fieldtype == "Time") {
					me.set_value(kanivin.datetime.now_time());
				}
				return false;
			}
		});
	...
	parse(value) {
		if (value) {
			...
			return kanivin.datetime.user_to_str(value, false, true);
		}
	}
	format_for_input(value) {
		if (value) {
			return kanivin.datetime.str_to_user(value, false, true);
		}
		return "";
	}
	validate(value) {
		if (value && !kanivin.datetime.validate(value)) {
			let sysdefaults = kanivin.sys_defaults;
			let date_format =
				sysdefaults && sysdefaults.date_format ? sysdefaults.date_format : "yyyy-mm-dd";
			kanivin.msgprint(__("Date {0} must be in format: {1}", [value, date_format]));
			return "";
		}
		return value;
	}
