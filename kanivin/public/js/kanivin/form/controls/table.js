...
kanivin.ui.form.ControlTable = class ControlTable extends kanivin.ui.form.Control {
	make() {
		super.make();
		...
		this.$wrapper.on("paste", ":text", (e) => {
			...
			let data = kanivin.utils.csv_to_array(pasted_data, "\t");
			...
			if (this.get_field(data[0][0])) {
				data[0].forEach((column) => {
					...
					const df = kanivin.meta.get_docfield(doctype, this.get_field(column));
				...
				visible_columns.forEach((column) => {
					...
					if (
						...
						const df = kanivin.meta.get_docfield(doctype, column.fieldname);
						fieldtypes.push(df ? df.fieldtype : "");
