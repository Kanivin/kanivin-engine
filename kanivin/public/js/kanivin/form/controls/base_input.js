kanivin.ui.form.ControlInput = class ControlInput extends kanivin.ui.form.Control {
	...
	make_wrapper() {
		if (this.only_input) {
			this.$wrapper = $('<div class="form-group kanivin-control">').appendTo(this.parent);
		} else {
			this.$wrapper = $(
				`<div class="kanivin-control">
	...
	refresh_input() {
		...
		if (me.disp_status != "None") {
			// refresh value
			if (me.frm) {
				me.value = kanivin.model.get_value(me.doctype, me.docname, me.df.fieldname);
			} else if (me.doc) {
				...
				if (is_fetch_from_read_only) {
					$(me.disp_area).attr(
						...
						__(
							...
							me.df.fetch_from.split(".").map((value) => __(kanivin.unscrub(value)))
						)
					);
				}
			}
	...
	set_disp_area(value) {
		...
		if (this.df.fieldtype === "Data") {
			value = kanivin.utils.escape_html(value);
		}
		let doc = this.doc || (this.frm && this.frm.doc);
		let display_value = kanivin.format(value, this.df, { no_icon: true, inline: true }, doc);
