...
kanivin.ui.form.States = class FormStates {
	constructor(opts) {
		$.extend(this, opts);
		this.state_fieldname = kanivin.workflow.get_state_fieldname(this.frm.doctype);

		// no workflow?
		if (!this.state_fieldname) return;

		this.update_fields = kanivin.workflow.get_update_fields(this.frm.doctype);
	...
	setup_help() {
		...
		this.frm.page.add_action_item(
			__("Help"),
			function () {
				kanivin.workflow.setup(me.frm.doctype);
				var state = me.get_state();
				var d = new kanivin.ui.Dialog({
					title: "Workflow: " + kanivin.workflow.workflows[me.frm.doctype].name,
				});

				kanivin.workflow.get_transitions(me.frm.doc).then((transitions) => {
					const next_actions =
						$.map(
							transitions,
					...
					const document_editable_by = kanivin.workflow
	...
	show_actions() {
		...
		kanivin.workflow.get_transitions(this.frm.doc).then((transitions) => {
			this.frm.page.clear_actions_menu();
			transitions.forEach((d) => {
				if (kanivin.user_roles.includes(d.allowed) && has_approval_access(d)) {
					added = true;
					me.frm.page.add_action_item(__(d.action), function () {
						...
						me.frm.script_manager.trigger("before_workflow_action").then(() => {
							kanivin
								.xcall("kanivin.model.workflow.apply_workflow", {
									doc: me.frm.doc,
									action: d.action,
								})
								.then((doc) => {
	...
	set_default_state() {
		var default_state = kanivin.workflow.get_default_state(
			this.frm.doctype,
