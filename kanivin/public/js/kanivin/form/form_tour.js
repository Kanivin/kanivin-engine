kanivin.ui.form.FormTour = class FormTour {
	constructor({ frm }) {
		this.frm = frm;
		this.driver_steps = [];
	}
	...
	init_driver() {
		...
		kanivin.router.on("change", () => this.driver.reset());
	...
	async init({ tour_name, on_finish }) {
		if (tour_name) {
			...
			const doctype_tour_exists = await kanivin.db.exists("Form Tour", this.frm.doctype);
			if (doctype_tour_exists) {
				this.tour = await kanivin.db.get_doc("Form Tour", this.frm.doctype);
			} else {
				this.tour = { steps: kanivin.tour[this.frm.doctype] };
			}
		}
	...
	get_step(step_info, on_next, on_prev) {
		...
		let element = `.kanivin-control[data-fieldname='${fieldname}']`;
		...
		if (is_table_field) {
			...
			element = `.grid-row-open .kanivin-control[data-fieldname='${fieldname}']`;
		}
	...
	add_new_row_step(step) {
		const $add_row = `.kanivin-control[data-fieldname='${step.fieldname}'] .grid-add-row`;
		const add_row_step = {
			element: $add_row,
	...
	expand_row_and_proceed(step, start_from) {
		...
		kanivin.utils.sleep(300).then(() => this.driver.start(start_from));
	}
	...
	open_first_row_of(fieldname) {
		...
		$($close_row).one("click", () => {
			...
			kanivin.utils.scroll_to(next_element, true, 150, null, () => {
				this.driver.moveNext();
				kanivin.flags.disable_auto_scroll = false;
			});
			kanivin.flags.disable_auto_scroll = true;
		});
	}
