...
kanivin.provide("kanivin.ui.form.handlers");
...
kanivin.ui.form.get_event_handler_list = function (doctype, fieldname) {
	if (!kanivin.ui.form.handlers[doctype]) {
		kanivin.ui.form.handlers[doctype] = {};
	}
	if (!kanivin.ui.form.handlers[doctype][fieldname]) {
		kanivin.ui.form.handlers[doctype][fieldname] = [];
	}
	return kanivin.ui.form.handlers[doctype][fieldname];
};

kanivin.ui.form.on = kanivin.ui.form.on_change = function (doctype, fieldname, handler) {
	var add_handler = function (fieldname, handler) {
		var handler_list = kanivin.ui.form.get_event_handler_list(doctype, fieldname);
...
kanivin.ui.form.off = function (doctype, fieldname, handler) {
	var handler_list = kanivin.ui.form.get_event_handler_list(doctype, fieldname);
...
kanivin.ui.form.trigger = function (doctype, fieldname) {
	cur_frm.script_manager.trigger(fieldname, doctype);
};

kanivin.ui.form.ScriptManager = class ScriptManager {
	constructor(opts) {
		$.extend(this, opts);
	}
	...
	trigger(event_name, doctype, name) {
		...
		this.frm.selected_doc = kanivin.get_doc(doctype, name);
	...
	get_handlers(event_name, doctype) {
		...
		if (kanivin.ui.form.handlers[doctype] && kanivin.ui.form.handlers[doctype][event_name]) {
			$.each(kanivin.ui.form.handlers[doctype][event_name], function (i, fn) {
	...
	setup() {
		...
		$.each(this.frm.fields, function (i, field) {
			...
			if (kanivin.model.table_fields.includes(field.df.fieldtype)) {
				$.each(
					kanivin.meta.get_docfields(field.df.options, me.frm.docname),
					function (i, df) {
						setup_add_fetch(df);
					}
				);
		...
		// css
		doctype.__css && kanivin.dom.set_style(doctype.__css);

		this.trigger("setup");
	}
	log_error(caller, e) {
		kanivin.show_alert({ message: __("Error in Client Script."), indicator: "error" });
