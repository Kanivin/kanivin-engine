...
kanivin.provide("kanivin.form.formatters");

kanivin.form.link_formatters = {};

kanivin.form.formatters = {
	...
	_apply_custom_formatter: function (value, df) {
		...
		example:
			kanivin.meta.docfield_map[df.parent][df.fieldname].formatter = (value) => {
		...
		*/
		...
		if (df) {
			const std_df =
				kanivin.meta.docfield_map[df.parent] &&
				kanivin.meta.docfield_map[df.parent][df.fieldname];
	...
	Data: function (value, df) {
		...
		return kanivin.form.formatters._apply_custom_formatter(value, df);
	},
	Autocomplete: function (value, df) {
		return __(kanivin.form.formatters["Data"](value, df));
	},
	...
	Float: function (value, docfield, options, doc) {
		...
		var precision =
			docfield.precision ||
			cint(kanivin.boot.sysdefaults && kanivin.boot.sysdefaults.float_precision) ||
			null;
		if (docfield.options && docfield.options.trim()) {
			...
			return kanivin.form.formatters.Currency(value, docfield, options, doc);
		} else {
			// show 1.000000 as 1
			...
			return kanivin.form.formatters._right(format_number(value, null, precision), options);
		}
	},
	Int: function (value, docfield, options) {
		...
		if (cstr(docfield.options).trim() === "File Size") {
			return kanivin.form.formatters.FileSize(value);
		}
		return kanivin.form.formatters._right(value == null ? "" : cint(value), options);
	},
	Percent: function (value, docfield, options) {
		...
		const precision =
			...
			cint(kanivin.boot.sysdefaults && kanivin.boot.sysdefaults.float_precision) ||
			2;
		return kanivin.form.formatters._right(flt(value, precision) + "%", options);
	},
	...
	Currency: function (value, docfield, options, doc) {
		...
		var currency = kanivin.meta.get_field_currency(docfield, doc);
		...
		if (typeof docfield.precision == "number") {
			...
			precision = cint(
				docfield.precision || kanivin.boot.sysdefaults.currency_precision || 2
			);
		}
		...
		if (precision > 2) {
			...
			if (decimals.length < 3 || decimals.length < precision) {
				const fraction =
					kanivin.model.get_value(":Currency", currency, "fraction_units") || 100; // if not set, minimum 2.
		...
		if (options && options.only_value) {
			...
			return kanivin.form.formatters._right(value, options);
		}
	},
	...
	Link: function (value, docfield, options, doc) {
		...
		let link_title = kanivin.utils.get_link_title(doctype, value);
		...
		if (kanivin.form.link_formatters[doctype]) {
			// don't apply formatters in case of composite (parent field of same type)
			if (doc && doctype !== doc.doctype) {
				value = kanivin.form.link_formatters[doctype](value, doc, docfield);
			}
		}
		...
		if (docfield && docfield.link_onclick) {
			...
			if (kanivin.model.can_read(doctype)) {
				const a = document.createElement("a");
				a.href = `/app/${encodeURIComponent(
					kanivin.router.slug(doctype)
	...
	Date: function (value) {
		if (!kanivin.datetime.str_to_user) {
			return value;
		}
		if (value) {
			value = kanivin.datetime.str_to_user(value, false, true);
			// handle invalid date
	...
	DateRange: function (value) {
		if (Array.isArray(value)) {
			return __("{0} to {1}", [
				kanivin.datetime.str_to_user(value[0]),
				kanivin.datetime.str_to_user(value[1]),
			]);
		} else {
			return value || "";
		}
	...
	Datetime: function (value) {
		if (value) {
			return moment(kanivin.datetime.convert_to_user_tz(value)).format(
				kanivin.boot.sysdefaults.date_format.toUpperCase() +
					" " +
					(kanivin.boot.sysdefaults.time_format || "HH:mm:ss")
			);
		} else {
			return "";
		}
	...
	Text: function (value, df) {
		if (value) {
			...
			if (!match) {
				value = kanivin.utils.replace_newlines(value);
			}
		}

		return kanivin.form.formatters.Data(value, df);
	},
	Time: function (value) {
		if (value) {
			value = kanivin.datetime.str_to_user(value, true);
		}
	...
	Duration: function (value, docfield) {
		if (value) {
			let duration_options = kanivin.utils.get_duration_options(docfield);
			value = kanivin.utils.get_formatted_duration(value, duration_options);
		}
	...
	SmallText: function (value) {
		return kanivin.form.formatters.Text(value);
	},
	TextEditor: function (value) {
		let formatted_value = kanivin.form.formatters.Text(value);
	...
	WorkflowState: function (value) {
		var workflow_state = kanivin.get_doc("Workflow State", value);
		if (workflow_state) {
			return repl(
	...
	TableMultiSelect: function (rows, df, options) {
		...
		const formatted_values = rows.map((row) => {
			...
			return `<span class="text-nowrap">
				${kanivin.format(value, link_field, options, row)}
			</span>`;
		});
	...
	Icon: (value) => {
		return value
			? `<div>
			<div class="selected-icon">${kanivin.utils.icon(value, "md")}</div>
...
kanivin.form.get_formatter = function (fieldtype) {
	if (!fieldtype) fieldtype = "Data";
	return kanivin.form.formatters[fieldtype.replace(/ /g, "")] || kanivin.form.formatters.Data;
};

kanivin.format = function (value, df, options, doc) {
	...
	var formatter = df.formatter || kanivin.form.get_formatter(fieldtype);

	var formatted = formatter(value, df, options, doc);

	if (typeof formatted == "string") formatted = kanivin.dom.remove_script_and_style(formatted);

	return formatted;
};

kanivin.get_format_helper = function (doc) {
	var helper = {
		get_formatted: function (fieldname) {
			var df = kanivin.meta.get_docfield(doc.doctype, fieldname);
			if (!df) {
				console.log("fieldname not found: " + fieldname);
			}
			return kanivin.format(doc[fieldname], df, { inline: 1 }, doc);
		},
	};
	$.extend(helper, doc);
	return helper;
...
kanivin.form.link_formatters["User"] = function (value, doc, docfield) {
