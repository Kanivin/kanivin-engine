...
kanivin.ui.form.Toolbar = class Toolbar {
	constructor(opts) {
		$.extend(this, opts);
		this.refresh();
	...
	set_title() {
		...
		if (this.frm.is_new()) {
			...
			if (
				...
				this.page.$sub_title_area.on("click", (event) => {
					...
					kanivin.utils.copy_to_clipboard(this.frm.docname);
				});
			}
		} else {
			title = this.frm.docname;
		...
		if (this.frm.meta.title_field) {
			kanivin.utils.set_title(title + " - " + this.frm.docname);
		}
	...
	rename_document_title(input_name, input_title, merge = false) {
		...
		let rename_document = () => {
			if (input_name != docname) kanivin.realtime.doctype_subscribe(doctype, input_name);
			return kanivin
				.xcall("kanivin.model.rename_doc.update_document_title", {
					doctype,
					docname,
					name: input_name,
					title: input_title,
	...
	set_indicator() {
		var indicator = kanivin.get_indicator(this.frm.doc);
		if (
	...
	make_menu_items() {
		...
		const is_submittable = kanivin.model.is_submittable(this.frm.doc.doctype);
		...
		if (
			...
			if (kanivin.model.can_print(null, me.frm) && !this.frm.meta.issingle) {
		...
		// email
		if (kanivin.model.can_email(null, me.frm) && me.frm.doc.docstatus < 2) {
			this.page.add_menu_item(
				__("Email"),
				function () {
					me.frm.email_doc();
		...
		// duplicate
		if (kanivin.boot.user.can_create.includes(me.frm.doctype) && !me.frm.meta.allow_copy) {
			this.page.add_menu_item(
				__("Duplicate"),
				function () {
					me.frm.copy_doc();
		...
		this.page.add_menu_item(
			...
			function () {
				kanivin.utils.copy_to_clipboard(JSON.stringify(me.frm.doc));
			},
			true
		);
		...
		if (
			...
			!kanivin.model.is_single(me.frm.doctype) &&
			kanivin.model.can_delete(me.frm.doctype)
		) {
			this.page.add_menu_item(
				__("Delete"),
				function () {
		...
		if (this.can_repeat()) {
			this.page.add_menu_item(
				__("Repeat"),
				function () {
					kanivin.utils.new_auto_repeat_prompt(me.frm);
				},
				true
			);
		}
		...
		if (p[CREATE] && !this.frm.meta.issingle && !this.frm.meta.in_create) {
			this.page.add_menu_item(
				...
				function () {
					kanivin.new_doc(me.frm.doctype, true);
				},
				true,
				{
					shortcut: "Ctrl+B",
	...
	make_customize_buttons() {
		...
		if (kanivin.model.can_create("DocType")) {
			if (kanivin.boot.developer_mode === 1 && !is_doctype_form) {
				// edit doctype
				this.page.add_menu_item(
					__("Edit DocType"),
					() => {
						kanivin.set_route("Form", "DocType", this.frm.doctype);
					},
					true
				);
			}
	...
	has_workflow() {
		if (this._has_workflow === undefined)
			this._has_workflow = kanivin.get_list("Workflow", {
	...
	show_linked_with() {
		if (!this.frm.linked_with) {
			this.frm.linked_with = new kanivin.ui.form.LinkedWith({
				frm: this.frm,
			});
		}
	...
	set_page_actions(status) {
		...
		if (status === "Edit") {
			...
			if (this.has_workflow()) {
				kanivin
					.xcall("kanivin.model.workflow.can_cancel_document", {
