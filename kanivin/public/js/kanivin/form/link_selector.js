...
kanivin.ui.form.LinkSelector = class LinkSelector {
	constructor(opts) {
		...
		if (this.doctype != "[Select]") {
			kanivin.model.with_doctype(this.doctype, function (r) {
				me.make();
			});
		} else {
			this.make();
				...
				if (results.length) {
					...
					$(
						'<p><br><span class="text-muted">' +
							__("No Results") +
							"</span>" +
							(kanivin.model.can_create(me.doctype)
	...
	set_in_grid(value) {
		return new Promise((resolve) => {
			if (this.qty_fieldname) {
						...
						if (!updated) {
							...
							kanivin.run_serially([
								...
								() => {
									...
									return kanivin.model.set_value(d.doctype, d.name, args);
								},
								() => kanivin.show_alert(__("Added {0} ({1})", [value, data.qty])),
								() => resolve(),
							]);
						}
					},
				...
				kanivin.model.set_value(d.doctype, d.name, this.fieldname, value).then(() => {
					kanivin.show_alert(__("{0} {1} added", [this.dynamic_link_reference, value]));
					resolve();
				});
			} else {
				let d = this.target.add_new_row();
				kanivin.model.set_value(d.doctype, d.name, this.fieldname, value).then(() => {
					kanivin.show_alert(__("{0} added", [value]));
					resolve();
				});
			}
		});
...
kanivin.link_search = function (doctype, args, callback, btn) {
	if (!args) {
		args = {
			txt: "",
		};
