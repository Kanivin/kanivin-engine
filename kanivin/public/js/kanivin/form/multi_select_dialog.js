kanivin.ui.form.MultiSelectDialog = class MultiSelectDialog {
	constructor(opts) {
		...
		if (!this.for_select) {
			kanivin.model.with_doctype(this.doctype, () => this.init());
		} else {
			this.init();
		}
	}
	...
	make_new_document(e) {
		...
		if (e) {
			...
			kanivin.new_doc(this.doctype, true);
		}
	}
	...
	setup_child_datatable() {
		...
		this.child_datatable = new kanivin.DataTable(this.$child_wrapper.get(0), {
	...
	get_primary_filters() {
		...
		if ($.isArray(this.setters)) {
			...
			Object.keys(this.setters).forEach((setter, index) => {
				let df_prop = kanivin.meta.docfield_map[this.doctype][setter];
		...
		if (this.allow_child_item_selection) {
			this.child_doctype = kanivin.meta.get_docfield(
				this.doctype,
	...
	bind_events() {
		...
		this.$parent.find(".input-with-feedback").on("change", () => {
			kanivin.flags.auto_scroll = false;
			if (this.is_child_selection_enabled()) {
				this.show_child_results();
			} else {
				this.get_results();
				...
				${
					head
						? `<span class="ellipsis text-muted" title="${__(
								kanivin.model.unscrub(column)
						  )}">${__(kanivin.model.unscrub(column))}</span>`
						: column !== "name"
						? `<span class="ellipsis result-row" title="${__(
								result[column] || ""
						  )}">${__(result[column] || "")}</span>`
						: `<a href="${
								"/app/" + kanivin.router.slug(me.doctype) + "/" + result[column] ||
								""
	...
	render_result_list(results, more = 0, empty = true) {
		...
		if (!kanivin.flags.auto_scroll && empty) {
			this.empty_list();
		}
		more_btn.hide();
	...
	async perform_search(args) {
		const res = await kanivin.call({
			type: "GET",
			method: "kanivin.desk.search.search_widget",
			no_spinner: true,
			args: args,
		});
