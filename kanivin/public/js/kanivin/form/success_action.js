...
kanivin.provide("kanivin.success_action");

kanivin.ui.form.SuccessAction = class SuccessAction {
	constructor(form) {
		this.form = form;
		this.load_setting();
	}
	...
	load_setting() {
		this.setting = kanivin.boot.success_action.find(
	...
	show_alert() {
		kanivin.db.get_list(this.form.doctype, { limit: 2 }).then((result) => {
	...
	get default_actions() {
		return {
			new: {
				label: __("New"),
				action: (frm) => kanivin.new_doc(frm.doctype),
			},
			print: {
				label: __("Print"),
			...
			list: {
				label: __("View All"),
				action: (frm) => {
					kanivin.set_route("List", frm.doctype);
				},
			},
		};
	}
