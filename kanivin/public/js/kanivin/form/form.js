...
kanivin.provide("kanivin.model.docinfo");
...
kanivin.ui.form.Controller = class FormController {
	constructor(opts) {
		$.extend(this, opts);
	}
};

kanivin.ui.form.Form = class KanivinForm {
	constructor(doctype, parent, in_form, doctype_layout_name) {
		...
		this.sections = [];
		this.grids = [];
		this.cscript = new kanivin.ui.form.Controller({ frm: this });
		this.events = {};
		this.fetch_dict = {};
		this.parent = parent;
		this.doctype_layout = kanivin.get_doc("DocType Layout", doctype_layout_name);
		this.undo_manager = new UndoManager({ frm: this });
		this.setup_meta(doctype);
		this.debounced_reload_doc = kanivin.utils.debounce(this.reload_doc.bind(this), 1000);
	...
	setup_meta() {
		this.meta = kanivin.get_doc("DocType", this.doctype);

		if (this.meta.istable) {
			this.meta.in_dialog = 1;
		}

		this.perm = kanivin.perm.get_perm(this.doctype); // for create
	...
	setup() {
		this.fields = [];
		this.fields_dict = {};
		this.state_fieldname = kanivin.workflow.get_state_fieldname(this.doctype);
		...
		this.toolbar = new kanivin.ui.form.Toolbar({
			frm: this,
			page: this.page,
		});

		this.viewers = new kanivin.ui.form.FormViewers({
			frm: this,
		...
		this.script_manager = new kanivin.ui.form.ScriptManager({
			frm: this,
		});
		this.script_manager.setup();
		this.watch_model_updates();

		if (!this.meta.hide_toolbar && kanivin.boot.desk_settings.timeline) {
			// this.footer_tab = new kanivin.ui.form.Tab(this.layout, {
			// 	label: __("Activity"),
			// 	fieldname: 'timeline'
			// });

			this.footer = new kanivin.ui.form.Footer({
				frm: this,
	...
	add_form_keyboard_shortcuts() {
		...
		kanivin.ui.keys.add_shortcut({
			...
			condition: () => kanivin.model.can_print(this.doctype, this) && !this.meta.issingle,
		});
	...
	setup_std_layout() {
		...
		// layout
		this.layout = new kanivin.ui.form.Layout({
			parent: this.body,
		...
		this.dashboard = new kanivin.ui.form.Dashboard(dashboard_parent, this);

		this.tour = new kanivin.ui.form.FormTour({
			frm: this,
		});

		// workflow state
		this.states = new kanivin.ui.form.States({
			frm: this,
		});
	}
	...
	watch_model_updates() {
		...
		// on table fields
		var table_fields = kanivin.get_children("DocType", me.doctype, "fields", {
			fieldtype: ["in", kanivin.model.table_fields],
		});

		// using $.each to preserve df via closure
		$.each(table_fields, function (i, df) {
			kanivin.model.on(df.options, "*", function (fieldname, value, doc) {
	...
	setup_file_drop() {
		...
		this.$wrapper.on("dragenter dragover", false).on("drop", function (e) {
			...
			if (me.doc.__islocal) {
				kanivin.msgprint(__("Please save before attaching."));
				throw "attach error";
			}
		...
		this.fields.map((field) => {
			if (field.df.fieldtype === "Markdown Editor") {
				this.set_df_property(field.df.fieldname, "autocompletions", () => {
					...
					return attachments
						.filter((file) => kanivin.utils.is_image_file(file.file_url))
						.map((file) => {
							return {
	...
	refresh(docname) {
		...
		if (this.docname) {
			...
			// set the doc
			this.doc = kanivin.get_doc(this.doctype, this.docname);

			// check permissions
			this.fetch_permissions();
			if (!this.has_read_permission()) {
				kanivin.show_not_permitted(__(this.doctype) + " " + __(cstr(this.docname)));
				return;
			}
			...
			// read only (workflow)
			this.read_only = kanivin.workflow.is_read_only(this.doctype, this.docname);
			if (this.read_only) {
				this.set_read_only();
			}
	...
	setup_doctype_actions() {
		if (this.meta.actions) {
			for (let action of this.meta.actions) {
				kanivin.ui.form.on(this.doctype, "refresh", () => {
					if (!this.is_new()) {
						if (!action.hidden) {
	...
	execute_action(action) {
		if (typeof action === "string") {
			...
			if (typeof action === "string") {
				kanivin.throw(`Action ${action} not found`);
			}
		}
		if (action.action_type === "Server Action") {
			return kanivin.xcall(action.action, { doc: this.doc }).then((doc) => {
				if (doc.doctype) {
					// document is returned by the method,
					// apply the changes locally and refresh
					kanivin.model.sync(doc);
					this.refresh();
				}
			...
			return kanivin.set_route(action.action);
		}
	}
	...
	switch_doc(docname) {
		...
		kanivin.ui.form.close_grid_form();
	...
	initialize_new_doc() {
		...
		this.script_manager.trigger("before_load", this.doctype, this.docname).then(() => {
			...
			kanivin.after_ajax(function () {
				me.trigger_link_fields();
			});

			kanivin.breadcrumbs.add(me.meta.module, me.doctype);
		});
	...
	render_form(switched) {
		if (!this.meta.istable) {
			...
			if (kanivin.boot.desk_settings.form_sidebar) {
				this.sidebar = new kanivin.ui.form.Sidebar({
					frm: this,
					page: this.page,
				});
				this.sidebar.make();
	...
	run_after_load_hook() {
		if (kanivin.route_hooks.after_load) {
			let route_callback = kanivin.route_hooks.after_load;
			delete kanivin.route_hooks.after_load;
	...
	refresh_header(switched) {
		...
		if (!this.meta.in_dialog || this.in_form) {
			kanivin.utils.set_title(this.meta.issingle ? this.doctype : this.docname);
		}
	...
	validate_and_save(save_action, callback, btn, on_error, resolve, reject) {
		...
		var after_save = function (r) {
			...
			if (!r.exc) {
				if (["Save", "Update", "Amend"].indexOf(save_action) !== -1) {
					kanivin.utils.play_sound("click");
				}

				me.script_manager.trigger("after_save");

				if (kanivin.route_hooks.after_save) {
					let route_callback = kanivin.route_hooks.after_save;
					delete kanivin.route_hooks.after_save;
		...
		if (save_action != "Update") {
			// validate
			kanivin.validated = true;
			kanivin
				.run_serially([
					() => this.script_manager.trigger("validate"),
					() => this.script_manager.trigger("before_save"),
					() => {
						if (!kanivin.validated) {
							fail();
							return;
						}

						kanivin.ui.form.save(me, save_action, after_save, btn);
					},
				])
				.catch(fail);
		} else {
						...
						me.save(
							...
							function (r) {
								if (r.exc) {
									...
									kanivin.utils.play_sound("submit");
									callback && callback();
									me.script_manager
										.trigger("on_submit")
										.then(() => resolve(me))
										.then(() => {
											if (kanivin.route_hooks.after_submit) {
												let route_callback =
													kanivin.route_hooks.after_submit;
												delete kanivin.route_hooks.after_submit;
												route_callback(me);
											}
										});
								}
	...
	savecancel(btn, callback, on_error) {
		...
		kanivin
			.call({
				method: "kanivin.desk.form.linked_with.get_submitted_linked_docs",
				args: {
	...
	_cancel_all(r, btn, callback, on_error) {
		...
		for (let doctype of doctypes) {
			if (!me.ignore_doctypes_on_cancel_all.includes(doctype)) {
				let docnames = links
					...
					.map((link) => kanivin.utils.get_form_link(link.doctype, link.name, true))
					.join(", ");
		...
		let can_cancel = links.every((link) => kanivin.model.can_cancel(link.doctype));
		if (can_cancel) {
		...
		let d = new kanivin.ui.Dialog(
			{
				...
				fields: [
					{
						fieldtype: "HTML",
						options: `<p class="kanivin-confirm-message">${confirm_message}</p>`,
					},
				],
			},
			() => me.handle_save_fail(btn, on_error)
		...
		if (can_cancel) {
			d.set_primary_action(__("Cancel All"), () => {
				d.hide();
				kanivin.call({
					method: "kanivin.desk.form.linked_with.cancel_all_linked_docs",
					args: {
						docs: links,
						ignore_doctypes_on_cancel_all: me.ignore_doctypes_on_cancel_all || [],
					},
			...
			me.script_manager.trigger("before_cancel").then(() => {
				...
				var after_cancel = function (r) {
					if (r.exc) {
						...
						kanivin.utils.play_sound("cancel");
						me.refresh();
						callback && callback();
						me.script_manager.trigger("after_cancel");
					}
				};
				kanivin.ui.form.save(me, "cancel", after_cancel, btn);
			});
		};
	...
	savetrash() {
		...
		kanivin.model.delete_doc(this.doctype, this.docname, function () {
			window.history.back();
		});
	}
	...
	amend_doc() {
		if (!this.fields_dict["amended_from"]) {
			kanivin.msgprint(__('"amended_from" field must be present to do an amendment.'));
			return;
		}

		kanivin
			.xcall("kanivin.client.is_document_amended", {
			...
			.then((is_amended) => {
				...
				var fn = function (newdoc) {
					...
					if (me.fields_dict && me.fields_dict["amendment_date"])
						newdoc.amendment_date = kanivin.datetime.obj_to_str(new Date());
				};
				this.copy_doc(fn, 1);
				kanivin.utils.play_sound("click");
			});
	...
	validate_form_action(action, resolve) {
		...
		var perms = kanivin.perm.get_perm(this.doc.doctype)[0];
		...
		if (
			(kanivin.workflow.is_read_only(this.doctype, this.docname) &&
	...
	show_submit_message() {
		if (
			...
			!this.is_dirty() &&
			!this.is_new() &&
			!kanivin.model.has_workflow(this.doctype) && // show only if no workflow
	...
	fetch_permissions() {
		...
		this.perm = kanivin.perm.get_perm(dt, this.doc);
	}
	...
	check_doctype_conflict(docname) {
		if (this.doctype == "DocType" && docname == "DocType") {
			kanivin.msgprint(__("Allowing DocType, DocType. Be careful!"));
		} else if (this.doctype == "DocType") {
			if (kanivin.views.formview[docname] || kanivin.pages["List/" + docname]) {
				window.location.reload();
			}
		} else {
			if (
				kanivin.views.formview.DocType &&
				kanivin.views.formview.DocType.frm.opendocs[this.doctype]
			) {
				window.location.reload();
			}
		}
	...
	rename_notify(dt, old, name) {
		...
		// cleanup
		if (this && this.opendocs[old] && kanivin.meta.docfield_copy[dt]) {
			// delete docfield copy
			kanivin.meta.docfield_copy[dt][name] = kanivin.meta.docfield_copy[dt][old];
			delete kanivin.meta.docfield_copy[dt][old];
		}
		...
		kanivin.re_route[kanivin.router.get_sub_path()] = `${encodeURIComponent(
			kanivin.router.slug(this.doctype)
		)}/${encodeURIComponent(name)}`;
		!kanivin._from_link && kanivin.set_route("Form", this.doctype, name);
	}
	...
	print_doc() {
		...
		kanivin.set_route("print", this.doctype, this.doc.name);
	}

	navigate_records(prev) {
		let filters, sort_field, sort_order;
		let list_view = kanivin.get_list_view(this.doctype);
		if (list_view) {
			filters = list_view.get_filters_for_args();
			sort_field = list_view.sort_by;
			sort_order = list_view.sort_order;
		} else {
			let list_settings = kanivin.get_user_settings(this.doctype)["List"];
			if (list_settings) {
		...
		kanivin.call("kanivin.desk.form.utils.get_next", args).then((r) => {
			if (r.message) {
				kanivin.set_route("Form", this.doctype, r.message);
	...
	rename_doc() {
		kanivin.model.rename_doc(this.doctype, this.docname, () => this.refresh_header());
	}
	...
	email_doc(message) {
		new kanivin.views.CommunicationComposer({
			doc: this.doc,
			frm: this,
	...
	copy_doc(onload, from_amend) {
		...
		var newdoc = kanivin.model.copy_doc(this.doc, from_amend);
	...
	reload_doc() {
		...
		if (!this.doc.__islocal) {
			kanivin.model.remove_from_locals(this.doctype, this.docname);
			return kanivin.model.with_doc(this.doctype, this.docname, () => {
				this.refresh();
			});
		}
	}
	...
	has_perm(ptype) {
		return kanivin.perm.has_perm(this.doctype, 0, ptype, this.doc);
	}
	...
	get_docinfo() {
		return kanivin.model.docinfo[this.doctype][this.docname];
	}
	...
	set_footnote(txt) {
		this.footnote_area = kanivin.utils.set_footnote(this.footnote_area, this.body, txt);
	}
	...
	scroll_to_element() {
		if (kanivin.route_options && kanivin.route_options.scroll_to) {
			var scroll_to = kanivin.route_options.scroll_to;
			delete kanivin.route_options.scroll_to;
			...
			if (selector.length) {
				kanivin.utils.scroll_to(selector);
			}
		} else if (window.location.hash) {
			if ($(window.location.hash).length) {
				kanivin.utils.scroll_to(window.location.hash, true, 200, null, null, true);
			} else {
	...
	show_success_action() {
		...
		const success_action = new kanivin.ui.form.SuccessAction(this);
		success_action.show();
	}
	...
	set_currency_labels(fields_list, currency, parentfield) {
		...
		$.each(fields_list, function (i, fname) {
			var docfield = kanivin.meta.docfield_map[doctype][fname];
			if (docfield) {
	...
	field_map(fnames, fn) {
		...
		for (var i = 0, l = fnames.length; i < l; i++) {
			...
			var field = kanivin.meta.get_docfield(this.doctype, fieldname, this.docname);
			if (field) {
				fn(field);
	...
	get_docfield(fieldname1, fieldname2) {
		if (fieldname2) {
			...
			return kanivin.meta.get_docfield(doctype, fieldname2, this.docname);
		} else {
			// for parent
			return kanivin.meta.get_docfield(this.doctype, fieldname1, this.docname);
		}
	}
	...
	set_df_property(fieldname, property, value, docname, table_field, table_row_name = null) {
		...
		if (!docname || !table_field) {
			...
			const filtered_fields = kanivin.utils.filter_dict(grid.docfields, {
				fieldname: table_field,
			});
			if (filtered_fields.length) {
				df = kanivin.meta.get_docfield(
	...
	get_files() {
		return this.attachments
			? kanivin.utils.sort(this.attachments.get_attachments(), "file_name", "string")
			: [];
	}
	...
	clear_table(fieldname) {
		kanivin.model.clear_table(this.doc, fieldname);
	}

	add_child(fieldname, values) {
		var doc = kanivin.model.add_child(
			this.doc,
			kanivin.meta.get_docfield(this.doctype, fieldname).options,
			fieldname
		);
		if (values) {
	...
	set_value(field, value, if_missing, skip_dirty_trigger = false) {
		...
		var _set = function (f, v) {
			...
			if (fieldobj) {
				if (!if_missing || !kanivin.model.has_value(me.doctype, me.doc.name, f)) {
					if (
						kanivin.model.table_fields.includes(fieldobj.df.fieldtype) &&
						$.isArray(v)
					) {
						// set entire child table from specified array as value
						kanivin.model.clear_table(me.doc, fieldobj.df.fieldname);

						const standard_fields = [
							...kanivin.model.std_fields_list,
							...kanivin.model.child_table_field_list,
						];
						v.forEach((d, idx) => {
							let child = kanivin.model.add_child(
								me.doc,
								fieldobj.df.options,
						...
						return kanivin.model.set_value(
							me.doctype,
							me.doc.name,
							f,
							v,
				...
				kanivin.msgprint(__("Field {0} not found.", [f]));
				throw "frm.set_value";
			}
		};
		...
		if (typeof field == "string") {
			...
			return kanivin.run_serially(tasks);
		}
	}
	...
	call(opts, args, callback) {
		...
		if (!opts.doc) {
			if (opts.method.indexOf(".") === -1)
				opts.method = kanivin.model.get_server_module_name(me.doctype) + "." + opts.method;
					...
					if (opts.child) {
						...
						if (opts.child) {
							var std_field_list = ["doctype"]
								.concat(kanivin.model.std_fields_list)
								.concat(kanivin.model.child_table_field_list);
	...
	set_read_only() {
		const docperms = kanivin.perm.get_perm(this.doc.doctype);
	...
	get_formatted(fieldname) {
		return kanivin.format(
			this.doc[fieldname],
			kanivin.meta.get_docfield(this.doctype, fieldname, this.docname),
			{ no_icon: true },
			this.doc
		);
	}
	...
	open_grid_row() {
		return kanivin.ui.form.get_open_grid_form();
	}
	...
	get_selected() {
		...
		var selected = {},
			me = this;
		kanivin.meta.get_table_fields(this.doctype).forEach(function (df) {
	...
	set_indicator_formatter(fieldname, get_color, get_text) {
		...
		var doctype;
		if (kanivin.meta.docfield_map[this.doctype][fieldname]) {
			doctype = this.doctype;
		} else {
			kanivin.meta.get_table_fields(this.doctype).every(function (df) {
				if (kanivin.meta.docfield_map[df.options][fieldname]) {
					doctype = df.options;
					return false;
				} else {
					return true;
		...
		kanivin.meta.docfield_map[doctype][fieldname].formatter = function (
			value,
			df,
			options,
			doc
			...
			if (value) {
				...
				if (get_text) {
					label = get_text(doc);
				} else if (kanivin.form.link_formatters[df.options]) {
					label = kanivin.form.link_formatters[df.options](value, doc, df);
				} else {
					label = value;
				}
				...
				return `
						<a class="indicator ${get_color(doc || {})}"
							href="/app/${kanivin.router.slug(df.options)}/${escaped_name}"
							data-doctype="${df.options}"
							data-name="${kanivin.utils.escape_html(value)}">
							${label}
						</a>
					`;
			} else {
	...
	can_create(doctype) {
		...
		if (!kanivin.model.can_create(doctype)) {
			return false;
		}
	...
	make_new(doctype) {
		...
		if (this.make_methods && this.make_methods[doctype]) {
			...
			kanivin.model.with_doctype(doctype, function () {
				let new_doc = kanivin.model.get_new_doc(doctype, null, null, true);

				// set link fields (if found)
				me.set_link_field(doctype, new_doc);

				kanivin.ui.form.make_quick_entry(doctype, null, null, new_doc);
				// kanivin.set_route('Form', doctype, new_doc.name);
			});
		}
	}
	...
	set_link_field(doctype, new_doc) {
		let me = this;
		kanivin.get_meta(doctype).fields.forEach(function (df) {
	...
	update_in_all_rows(table_fieldname, fieldname, value) {
		...
		kanivin.model
			...
			.filter((child) => !kanivin.model.has_value(child.doctype, child.name, fieldname))
			.forEach((child) =>
				kanivin.model.set_value(child.doctype, child.name, fieldname, value)
			);
	}
	...
	scroll_to_field(fieldname, focus = true) {
		...
		// scroll to input
		kanivin.utils.scroll_to($el, true, 15);
		...
		let control_element = $el.closest(".kanivin-control");
	...
	setup_docinfo_change_listener() {
		...
		if (this.doc && !this.is_new()) {
			kanivin.realtime.doc_subscribe(doctype, docname);
		}
		kanivin.realtime.off("docinfo_update");
		kanivin.realtime.on("docinfo_update", ({ doc, key, action = "update" }) => {
			if (
			...
			let doc_list = kanivin.model.docinfo[doctype][docname][key] || [];
			let docindex = doc_list.findIndex((old_doc) => {
				return old_doc.name === doc.name;
			});

			if (action === "add") {
				kanivin.model.docinfo[doctype][docname][key].push(doc);
			}
			if (docindex > -1) {
				if (action === "update") {
					kanivin.model.docinfo[doctype][docname][key].splice(docindex, 1, doc);
				}
				if (action === "delete") {
					kanivin.model.docinfo[doctype][docname][key].splice(docindex, 1);
				}
			}
			...
			if (
				!(
					...
					doc.doctype === "Comment" &&
					doc.owner === kanivin.session.user
				)
			) {
	...
	set_fields_as_options(
		...
		return new Promise((resolve) => {
			kanivin.model.with_doctype(reference_doctype, () => {
				kanivin.get_meta(reference_doctype).fields.map((df) => {
					filter_function(df) &&
	...
	get_involved_users() {
		...
		return involved_users
			.uniqBy((u) => u)
			.filter((user) => !["Administrator", kanivin.session.user].includes(user))
			.filter(Boolean);
	}
	...
	show_submission_queue_banner() {
		...
		kanivin
			.call({
				method: "kanivin.core.doctype.submission_queue.submission_queue.get_latest_submissions",
	...
	show_workflow_read_only_banner() {
		...
		if (this.dashboard) {
			...
			kanivin.after_ajax(_show_read_only_banner);
		}
	}
};
