...
kanivin.ui.form.save = function (frm, action, callback, btn) {
	...
	var save = function () {
		...
		if ((action !== "Save" || frm.is_dirty()) && check_mandatory()) {
			_call({
				method: "kanivin.desk.form.save.savedocs",
			...
			!frm.is_dirty() &&
				kanivin.show_alert({ message: __("No changes in document"), indicator: "orange" });
	...
	var remove_empty_rows = function () {
		...
		*/
		const docs = kanivin.model.get_all_docs(frm.doc);

		// we should only worry about table data
		const tables = docs.filter((d) => {
			return kanivin.model.is_table(d.doctype);
		});

		let modified_table_fields = [];

		tables.map((doc) => {
			const cells = kanivin.meta.docfield_list[doc.doctype] || [];
			...
			if (is_empty_row(in_list_view_cells)) {
				kanivin.model.clear_doc(doc.doctype, doc.name);
	...
	var cancel = function () {
		...
		var workflow_state_fieldname = kanivin.workflow.get_state_fieldname(frm.doctype);
		...
		_call({
			method: "kanivin.desk.form.save.cancel",
			args: args,
			callback: function (r) {
				$(document).trigger("save", [frm.doc]);
				callback(r);
	...
	var check_mandatory = function () {
		...
		$.each(kanivin.model.get_all_docs(frm.doc), function (i, doc) {
			var error_fields = [];
			var folded = false;

			$.each(kanivin.meta.docfield_list[doc.doctype] || [], function (i, docfield) {
				if (docfield.fieldname) {
					const df = kanivin.meta.get_docfield(doc.doctype, docfield.fieldname, doc.name);
					...
					if (
						...
						!kanivin.model.has_value(doc.doctype, doc.name, df.fieldname)
					) {
						has_errors = true;
			...
			if (error_fields.length) {
				let meta = kanivin.get_meta(doc.doctype);
				let message;
				if (meta.istable) {
					const table_field = kanivin.meta.docfield_map[doc.parenttype][doc.parentfield];

					const table_label = __(
						table_field.label || kanivin.unscrub(table_field.fieldname)
					).bold();
	...
	let is_docfield_mandatory = function (doc, df) {
		...
		let parent = kanivin.get_meta(df.parent);
		...
		if (typeof expression === "boolean") {
			...
			try {
				...
				kanivin.throw(__('Invalid "mandatory_depends_on" expression'));
			}
		} else {
	...
	var _call = function (opts) {
		...
		if (frm.is_new()) {
			kanivin.ui.form.remove_old_form_route();
		}
		kanivin.ui.form.is_saving = true;
		...
		return kanivin.call({
			...
			always: function (r) {
				$(btn).prop("disabled", false);
				kanivin.ui.form.is_saving = false;

				if (r) {
					var doc = r.docs && r.docs[0];
					if (doc) {
						kanivin.ui.form.update_calling_link(doc);
					}
				}
			},
		});
...
kanivin.ui.form.remove_old_form_route = () => {
	let current_route = kanivin.get_route().join("/");
	kanivin.route_history = kanivin.route_history.filter(
		(route) => route.join("/") !== current_route
	);
};

kanivin.ui.form.update_calling_link = (newdoc) => {
	if (!kanivin._from_link) return;
	var doc = kanivin.get_doc(kanivin._from_link.doctype, kanivin._from_link.docname);

	let is_valid_doctype = () => {
		if (kanivin._from_link.df.fieldtype === "Link") {
			return newdoc.doctype === kanivin._from_link.df.options;
		} else {
			// dynamic link, type is dynamic
			return newdoc.doctype === doc[kanivin._from_link.df.options];
		}
	};

	if (is_valid_doctype()) {
		kanivin.model.with_doctype(newdoc.doctype, () => {
			let meta = kanivin.get_meta(newdoc.doctype);
			// set value
			if (doc && doc.parentfield) {
				//update values for child table
				$.each(
					kanivin._from_link.frm.fields_dict[doc.parentfield].grid.grid_rows,
					function (index, field) {
						if (field.doc && field.doc.name === kanivin._from_link.docname) {
							if (meta.title_field && meta.show_title_field_in_link) {
								kanivin.utils.add_link_title(
									newdoc.doctype,
									newdoc.name,
									newdoc[meta.title_field]
								);
							}
							kanivin._from_link.set_value(newdoc.name);
						}
					}
				);
			} else {
			...
			// refresh field
			kanivin._from_link.refresh();

			// if from form, switch
			if (kanivin._from_link.frm) {
				kanivin
					.set_route(
						"Form",
						kanivin._from_link.frm.doctype,
						kanivin._from_link.frm.docname
					)
					.then(() => {
						kanivin.utils.scroll_to(kanivin._from_link_scrollY);
					});
			}

			kanivin._from_link = null;
		});
	}
};
