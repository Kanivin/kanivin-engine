...
kanivin.ui.form.Dashboard = class FormDashboard {
	...
	setup_dashboard_sections() {
		...
		this.heatmap_area = this.make_section({
			...
			is_dashboard_section: 1,
			body_html: `
				<div id="heatmap-${kanivin.model.scrub(this.frm.doctype)}" class="heatmap"></div>
	...
	refresh() {
		this.reset();
		if (this.frm.doc.__islocal || !kanivin.boot.desk_settings.dashboard) {
			return;
		}
	...
	render_links() {
		...
		$(kanivin.render_template("form_links", this.data)).appendTo(transactions_area_body);
	...
	render_report_links() {
		...
		if (this.data.reports && this.data.reports.length) {
			$(kanivin.render_template("report_links", this.data)).appendTo(parent);
			// bind reports
	...
	open_report($link) {
		...
		kanivin.provide("kanivin.route_options");
		kanivin.route_options[fieldname] = this.frm.doc.name;
		kanivin.set_route("query-report", report);
	}
	...
	open_document_list($link, show_open) {
		...
		if (
			...
			if (names.length) {
				kanivin.route_options = { name: ["in", names] };
			} else {
				return false;
			}
		} else if (this.data.fieldname) {
			kanivin.route_options = this.get_document_filter(doctype);
			if (show_open && kanivin.ui.notifications) {
				kanivin.ui.notifications.show_open_count_list(doctype);
			}
		}

		kanivin.set_route("List", doctype, "List");
	}
	...
	set_open_count() {
		...
		let method = this.data.method || "kanivin.desk.notifications.get_open_count";
		kanivin.call({
			type: "GET",
			method: method,
			args: {
	...
	// heatmap
	render_heatmap() {
		this.heatmap = new kanivin.Chart("#heatmap-" + kanivin.model.scrub(this.frm.doctype), {
			type: "heatmap",
	...
	render_graph(args) {
		...
		this.chart = new kanivin.Chart(".form-graph", args);
		if (!this.chart) {
			this.hide();
		}
	}
