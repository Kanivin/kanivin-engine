...
kanivin.ui.form.get_open_grid_form = function () {
	return $(".grid-row-open").data("grid_row");
};

kanivin.ui.form.close_grid_form = function () {
	var open_form = kanivin.ui.form.get_open_grid_form();
	open_form && open_form.hide_form();

	// hide editable row too
	if (kanivin.ui.form.editable_row) {
...
export default class Grid {
	constructor(opts) {
		...
		this.debounced_refresh = kanivin.utils.debounce(this.debounced_refresh, 100);
	}
	...
	make() {
					...
					<div class="form-grid">
						...
						<div class="grid-body">
							...
							<div class="grid-empty text-center">
								<img
									src="/assets/kanivin/images/ui-states/grid-empty-state.svg"
		...
		kanivin.utils.bind_actions_with_object(this.wrapper, this);
	...
	set_doc_url() {
		let unsupported_fieldtypes = kanivin.model.no_value_type.filter(
			(x) => kanivin.model.table_fields.indexOf(x) === -1
		);
	...
	delete_rows() {
		...
		selected_children.forEach((doc) => {
			...
			tasks.push(() => kanivin.timeout(0.1));
		});
	...
	delete_all_rows() {
		kanivin.confirm(__("Are you sure you want to delete all rows?"), () => {
	...
	scroll_to_top() {
		kanivin.utils.scroll_to(this.wrapper);
	}
	...
	refresh() {
		...
		if (this.frm) {
			this.display_status = kanivin.perm.get_field_display_status(
				this.df,
				this.frm.doc,
				this.perm
			);
	...
	setup_fields() {
		...
		if (this.frm && this.frm.docname) {
			...
			this.df = kanivin.meta.get_docfield(
				this.frm.doctype,
				this.df.fieldname,
				this.frm.docname
			);
			...
			if (this.df.options) {
				this.df =
					kanivin.meta.get_docfield(this.df.options, this.df.fieldname) ||
					this.df ||
					null;
			}
		}
		...
		if (this.doctype && this.frm) {
			this.docfields = kanivin.meta.get_docfields(this.doctype, this.frm.docname);
		} else {
	...
	get_data_based_on_fieldtype(df, data, value) {
		...
		if (fieldtype === "Check") {
			value = kanivin.utils.string_to_boolean(value);
			...
			let formatted_duration = kanivin.utils.get_formatted_duration(fieldvalue);
			...
			let user_formatted_date = kanivin.datetime.str_to_user(fieldvalue);
				...
				row.columns_list.forEach((column) => {
					...
					if (column.df.fieldname == fieldname) {
						if (show) {
							...
							if (row != kanivin.ui.form.editable_row) {
	...
	get_docfield(fieldname) {
		return kanivin.meta.get_docfield(
			this.doctype,
			fieldname,
	...
	duplicate_row(d, copy_doc) {
		...
		const docfields = kanivin.get_meta(this.doctype).fields || [];
	...
	setup_visible_columns() {
		...
		for (var ci in fields) {
			...
			if (
				...
				if (
					...
					const docfield = kanivin.meta.docfield_map[df.parent][df.fieldname];
		...
		while (total_colsize < 11 && passes < 12) {
			for (var i in this.visible_columns) {
				...
				if (colsize > 1 && colsize < 11 && kanivin.model.is_non_std_field(df.fieldname)) {
					if (
						passes < 3 &&
	...
	setup_user_defined_columns() {
		...
		let user_settings = kanivin.get_user_settings(this.frm.doctype, "GridView");
		if (user_settings && user_settings[this.doctype] && user_settings[this.doctype].length) {
			this.user_defined_columns = user_settings[this.doctype]
				.map((row) => {
					let column = kanivin.meta.get_docfield(this.doctype, row.fieldname);
	...
	set_multiple_add(link, qty) {
		...
		var link_field = kanivin.meta.get_docfield(this.df.options, link);
	...
	setup_allow_bulk_edit() {
		...
		if (this.frm && this.frm.get_docfield(this.df.fieldname)?.allow_bulk_edit) {
			...
			const value_formatter_map = {
				Date: (val) => (val ? kanivin.datetime.user_to_str(val) : val),
				Int: (val) => cint(val),
				Check: (val) => cint(val),
				Float: (val) => flt(val),
			...
			// upload
			kanivin.flags.no_socketio = true;
			$(this.wrapper)
				.find(".grid-upload")
				.removeClass("hidden")
				.on("click", () => {
					new kanivin.ui.FileUploader({
						...
						on_success(file) {
							var data = kanivin.utils.csv_to_array(
								kanivin.utils.get_decoded_string(file.dataurl)
							);
								...
								if (i > 6) {
									...
									if (!blank_row) {
										...
										$.each(row, (ci, value) => {
											...
											var df = kanivin.meta.get_docfield(
												me.df.options,
												fieldname
											);
											if (df) {
	...
	setup_download() {
		let title = this.df.label || kanivin.model.unscrub(this.df.fieldname);
		$(this.wrapper)
			.find(".grid-download")
			.removeClass("hidden")
			.on("click", () => {
				...
				data.push(["------"]);
				$.each(kanivin.get_meta(this.df.options).fields, (i, df) => {
					// don't include the read-only field in the template
					if (kanivin.model.is_value_type(df.fieldtype)) {
						data[1].push(df.label);
						data[2].push(df.fieldname);
						let description = (df.description || "") + " ";
						if (df.fieldtype === "Date") {
							description += kanivin.boot.sysdefaults.date_format;
						}
				...
				$.each(this.frm.doc[this.df.fieldname] || [], (i, d) => {
					...
					$.each(data[2], (i, fieldname) => {
						...
						if (docfields[i].fieldtype === "Date" && value) {
							value = kanivin.datetime.str_to_user(value);
						}
				...
				kanivin.tools.downloadify(data, null, title);
				return false;
			});
	}
