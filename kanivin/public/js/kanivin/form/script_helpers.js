...
window.refresh_field = function (n, docname, table_field) {
	...
	if (n && typeof n === "string" && table_field) {
		var grid = cur_frm.fields_dict[table_field].grid,
			field = kanivin.utils.filter_dict(grid.docfields, { fieldname: n }),
			grid_row = grid.grid_rows_by_docname[docname];

		if (field && field.length) {
			field = field[0];
			var meta = kanivin.meta.get_docfield(field.parent, field.fieldname, docname);
			$.extend(field, meta);
			if (grid_row) {
...
window.toggle_field = function (n, hidden) {
	var df = kanivin.meta.get_docfield(cur_frm.doctype, n, cur_frm.docname);
	if (df) {
		df.hidden = hidden;
		refresh_field(n);
	} else {
