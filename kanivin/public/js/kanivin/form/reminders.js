export class ReminderManager {
	...
	_update_datetime_selector() {
		...
		this.dialog.fields_dict.remind_at.datepicker?.update({
			minDate: kanivin.datetime.str_to_obj(kanivin.datetime.now_datetime()),
		});
	}
	...
	_convert_period_to_absolute_time() {
		...
		const now_time = kanivin.datetime.str_to_obj(kanivin.datetime.now_datetime());
		let [magnitude, unit] = period.split("_");

		let time_to_set = moment(now_time)
			.add(magnitude, unit)
			.format(kanivin.defaultDatetimeFormat);
		this.dialog.set_value("remind_at", time_to_set);
	}

	create_reminder() {
		kanivin
			.xcall("kanivin.automation.doctype.reminder.reminder.create_new_reminder", {
			...
			.then((reminder) => {
				kanivin.show_alert(
					__("Reminder set at {0}", [kanivin.datetime.str_to_user(reminder.remind_at)])
				);
			});
	}
}
