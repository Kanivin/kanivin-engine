kanivin.ui.form.FormViewers = class FormViewers {
	...
	refresh() {
		...
		let avatar_group = kanivin.avatar_group(this.active_users, 5, {
			align: "left",
			overlap: true,
		});
	...
	setup_events() {
		let me = this;
		kanivin.realtime.off("doc_viewers");
		kanivin.realtime.on("doc_viewers", function (data) {
			me.update_users(data);
		});
	}

	async update_users({ doctype, docname, users = [] }) {
		users = users.filter((u) => u != kanivin.session.user);
	...
	async fetch_user_info(users) {
		...
		for (let user of users) {
			if (!kanivin.boot.user_info[user]) unknown_users.push(user);
		}
		if (!unknown_users.length) return;

		const data = await kanivin.xcall("kanivin.desk.form.load.get_user_info_for_viewers", {
			users: unknown_users,
		});
		Object.assign(kanivin.boot.user_info, data);
	}
};
