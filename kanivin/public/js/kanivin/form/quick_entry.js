...
kanivin.quick_edit = function (doctype, name) {
	kanivin.db.get_doc(doctype, name).then((doc) => {
		kanivin.ui.form.make_quick_entry(doctype, null, null, doc);
	});
};

kanivin.ui.form.make_quick_entry = (doctype, after_insert, init_callback, doc, force) => {
	var trimmed_doctype = doctype.replace(/ /g, "");
	var controller_name = "QuickEntryForm";

	if (kanivin.ui.form[trimmed_doctype + "QuickEntryForm"]) {
		controller_name = trimmed_doctype + "QuickEntryForm";
	}

	kanivin.quick_entry = new kanivin.ui.form[controller_name](
		doctype,
		after_insert,
		init_callback,
		doc,
	...
	return kanivin.quick_entry.setup();
};

kanivin.ui.form.QuickEntryForm = class QuickEntryForm {
	...
	check_quick_entry_doc() {
		if (!this.doc) {
			this.doc = kanivin.model.get_new_doc(this.doctype, null, null, true);
		}
	}
	...
	render_dialog() {
		...
		this.dialog.onhide = () => (kanivin.quick_entry = null);
		this.dialog.show();
			...
			kanivin.call({
				...
				callback: function (r) {
					if (
						...
						kanivin.model.can_submit(me.doctype) &&
						!kanivin.model.has_workflow(me.doctype)
					) {
						...
						me.dialog.hide();
						// delete the old doc
						kanivin.model.clear_doc(me.dialog.doc.doctype, me.dialog.doc.name);
						me.dialog.doc = r.message;
						if (kanivin._from_link) {
							kanivin.ui.form.update_calling_link(me.dialog.doc);
						} else {
							if (me.after_insert) {
	...
	open_form_if_not_list() {
		...
		if (route && !(route[0] === "List" && route[1] === doc.doctype)) {
			kanivin.run_serially([() => kanivin.set_route("Form", doc.doctype, doc.name)]);
		}
	}
	...
	open_doc(set_hooks) {
		...
		if (set_hooks && this.after_insert) {
			kanivin.route_options = kanivin.route_options || {};
			kanivin.route_options.after_save = (frm) => {
				this.after_insert(frm);
			};
		}
		this.doc.__run_link_triggers = false;
		kanivin.set_route("Form", this.doctype, this.doc.name);
	}
