...
class BaseTimeline {
	...
	add_action_button(label, action, icon = null, btn_class = null) {
		let icon_element = icon ? kanivin.utils.icon(icon, "xs") : null;
	...
	get_timeline_item(item) {
		...
		if (item.icon) {
			timeline_item.append(`
				<div class="timeline-badge" title='${item.title || kanivin.utils.to_title_case(item.icon)}'>
					${kanivin.utils.icon(item.icon, item.icon_size || "md", item.icon_class || "")}
				</div>
			`);
		...
		timeline_item.append(
			`<div class="timeline-content ${item.is_card ? "kanivin-card" : ""}">`
		);
