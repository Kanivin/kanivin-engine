...
class FormTimeline extends BaseTimeline {
	...
	setup_timeline_actions() {
		if (kanivin.model.can_email(null, this.frm)) {
		...
		if (this.frm.meta.allow_events_in_timeline) {
			let create_event = () => {
				const args = {
					...
					txt: kanivin.markdown(this.frm.comment_box.get_value()),
				};
				return new kanivin.views.InteractionComposer(args);
			};
	...
	setup_document_email_link() {
		...
		if (doc_info.document_email) {
			...
			this.document_email_link_wrapper.find(".document-email-link").on("click", (e) => {
				...
				kanivin.utils.copy_to_clipboard(text);
			});
		}
	}
	...
	render_timeline_items() {
		...
		kanivin.utils.bind_actions_with_object(this.timeline_items_wrapper, this);
	}

	add_web_page_view_count() {
		if (this.frm.doc.route && cint(kanivin.boot.website_tracking_enabled)) {
			kanivin.utils.get_page_view_count(this.frm.doc.route).then((res) => {
	...
	async get_more_communication_timeline_contents() {
		...
		let response = await kanivin.call({
			method: "kanivin.desk.form.load.get_communications",
			args: {
	...
	get_communication_timeline_content(doc, allow_reply = true) {
		doc._url = kanivin.utils.get_form_link("Communication", doc.name);
		...
		doc.content = kanivin.dom.remove_script_and_style(doc.content);
		let communication_content = $(kanivin.render_template("timeline_message_box", { doc }));
		if (allow_reply) {
	...
	get_comment_timeline_content(doc) {
		doc.content = kanivin.dom.remove_script_and_style(doc.content);
		const comment_content = $(kanivin.render_template("timeline_message_box", { doc }));
	...
	get_milestone_timeline_contents() {
		...
		(this.doc_info.milestones || []).forEach((milestone_log) => {
			const field = kanivin.meta.get_label(this.frm.doctype, milestone_log.track_field);
	...
	get_custom_timeline_contents() {
		...
		(this.doc_info.additional_timeline_content || []).forEach((custom_item) => {
			custom_timeline_contents.push({
				...
				content:
					custom_item.content ||
					kanivin.render_template(custom_item.template, custom_item.template_data),
			});
		});
	...
	get_energy_point_timeline_contents() {
		...
		(this.doc_info.energy_point_logs || []).forEach((log) => {
			...
			energy_point_timeline_contents.push({
				...
				content: kanivin.energy_points.format_form_log(log),
			});
		});
	...
	setup_reply(communication_box, communication_doc) {
		...
		let reply = $(`<a class="action-btn reply">${kanivin.utils.icon("reply", "md")}</a>`).click(
			() => {
				this.compose_mail(communication_doc);
			}
		);
		let reply_all = $(
			`<a class="action-btn reply-all">${kanivin.utils.icon("reply-all", "md")}</a>`
		).click(() => {
	...
	compose_mail(communication_doc = null, reply_all = false) {
		const args = {
			...
			frm: this.frm,
			recipients:
				communication_doc && communication_doc.sender != kanivin.session.user_email
		...
		const email_accounts = kanivin.boot.email_accounts
			.filter((account) => {
				return (
		...
		if (communication_doc && args.is_a_reply) {
			args.cc = "";
			if (
				email_accounts.includes(kanivin.session.user_email) &&
				communication_doc.sender != kanivin.session.user_email
			) {
				...
				args.cc =
					recipients.filter((r) => r != kanivin.session.user_email).join(", ") + ", ";
			}
			if (reply_all) {
		...
		if (this.frm.doctype === "Communication") {
			...
			const comment_value = kanivin.markdown(this.frm.comment_box.get_value());
			args.message = strip_html(comment_value) ? comment_value : "";
		}

		new kanivin.views.CommunicationComposer(args);
	}
	...
	setup_comment_actions(comment_wrapper, doc) {
		...
		if (
			kanivin.model.can_delete("Comment") &&
			(kanivin.session.user == doc.owner || kanivin.user.has_role("System Manager"))
		) {
			const delete_option = $(`
				<li>
		...
		let edit_button = $();
		let current_user = kanivin.session.user;
	...
	make_editable(container) {
		return kanivin.ui.form.make_control({
			parent: container,
			df: {
				fieldtype: "Comment",
				fieldname: "comment",
	...
	update_comment(name, content) {
		return kanivin
			.xcall("kanivin.desk.form.utils.update_comment", { name, content })
			.then(() => {
	...
	delete_comment(comment_name) {
		kanivin.confirm(__("Delete comment?"), () => {
			return kanivin
				.xcall("kanivin.client.delete", {
					doctype: "Comment",
					name: comment_name,
				})
				.then(() => {
	...
	copy_link(ev) {
		let doc_link = kanivin.urllib.get_full_url(
			kanivin.utils.get_form_link(this.frm.doctype, this.frm.docname)
		);
		let element_id = $(ev.currentTarget).closest(".timeline-content").attr("id");
		kanivin.utils.copy_to_clipboard(`${doc_link}#${element_id}`);
	}
}
