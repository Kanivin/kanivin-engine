...
kanivin.ui.form.Footer = class FormFooter {
	constructor(opts) {
		$.extend(this, opts);
		this.make();
	...
	make() {
		this.wrapper = $(kanivin.render_template("form_footer", {})).appendTo(this.parent);
		this.wrapper.find(".btn-save").click(() => {
			this.frm.save("Save", null, this);
		});
	}
	make_comment_box() {
		this.frm.comment_box = kanivin.ui.form.make_control({
			...
			on_submit: (comment) => {
				if (strip_html(comment).trim() != "" || comment.includes("img")) {
					...
					kanivin
						.xcall("kanivin.desk.form.utils.add_comment", {
							reference_doctype: this.frm.doctype,
							reference_name: this.frm.docname,
							content: comment,
							comment_email: kanivin.session.user,
							comment_by: kanivin.session.user_fullname,
						})
						.then((comment) => {
							let comment_item =
