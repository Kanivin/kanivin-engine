...
function get_version_timeline_content(version_doc, frm) {
	...
	if (!$.isEmptyObject(updater_reference)) {
		...
		if (doctype && docname) {
			updater_reference_link = kanivin.utils.get_form_link(doctype, docname, true, label);
		} else {
	...
	if (data.changed && data.changed.length) {
		...
		data.changed.every(function (p) {
			if (p[0] === "docstatus") {
				...
				const df = kanivin.meta.get_docfield(frm.doctype, p[0], frm.docname);
				if (df && (!df.hidden || df.show_on_timeline)) {
					const field_display_status = kanivin.perm.get_field_display_status(
						df,
						null,
						frm.perm
					);
	...
	if (data.row_changed && data.row_changed.length) {
		...
		data.row_changed.every(function (row) {
			row[3].every(function (p) {
				...
				if (df && (!df.hidden || df.show_on_timeline)) {
					var field_display_status = kanivin.perm.get_field_display_status(
						df,
						null,
						frm.perm
					);
					...
					if (
						...
						parts.push(
							__("{0} from {1} to {2} in row #{3}", [
								kanivin.meta.get_label(frm.fields_dict[row[0]].grid.doctype, p[0]),
								format_content_for_timeline(p[1]),
								format_content_for_timeline(p[2]),
								row[1],
							])
	...
	["added", "removed"].forEach(function (key) {
		if (data[key] && data[key].length) {
			let parts = (data[key] || []).map(function (p) {
				var df = kanivin.meta.get_docfield(frm.doctype, p[0], frm.docname);
				...
				if (df && (!df.hidden || df.show_on_timeline)) {
					...
					if (
						...
						return __(kanivin.meta.get_label(frm.doctype, p[0]));
					}
				}
			});
...
function get_version_comment(version_doc, text) {
	...
	if (text.includes("<a")) {
		...
		try {
			...
			if (unlinked_content) {
				version_comment += kanivin.utils.get_form_link(
					"Version",
					version_doc.name,
					true,
					unlinked_content
	...
	return kanivin.utils.get_form_link("Version", version_doc.name, true, text);
}
...
function format_content_for_timeline(content) {
	...
	// escapes HTML
	// and makes it bold
	content = kanivin.utils.html2text(content);
	content = kanivin.ellipsis(content, 40) || '""';
	content = kanivin.utils.escape_html(content);
	return content.bold();
}

function get_user_link(user) {
	const user_display_text = kanivin.user_info(user).fullname || "";
	return kanivin.utils.get_form_link("User", user, true, user_display_text);
}

function get_user_message(user, message_self, message_other) {
	return kanivin.utils.is_current_user(user) ? message_self : message_other;
}
