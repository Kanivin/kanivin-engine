kanivin.ui.get_print_settings = function (pdf, callback, letter_head, pick_columns) {
	var print_settings = locals[":Print Settings"]["Print Settings"];

	var company = kanivin.defaults.get_default("company");
	...
	return kanivin.prompt(
		...
		function (data) {
			...
			if (data.letter_head) {
				data.letter_head = kanivin.boot.letter_heads[print_settings.letter_head];
			}
			callback(data);
		},
		__("Print Settings")
...
kanivin.ui.form.qz_connect = function () {
	return new Promise(function (resolve, reject) {
		kanivin.ui.form.qz_init().then(() => {
			if (qz.websocket.isActive()) {
				// if already active, resolve immediately
				// kanivin.show_alert({message: __('QZ Tray Connection Active!'), indicator: 'green'});
				resolve();
			} else {
				// try to connect once before firing the mimetype launcher
				kanivin.show_alert({
...
kanivin.ui.form.qz_init = function () {
	...
	return new Promise((resolve) => {
		if (typeof qz === "object" && typeof qz.version === "string") {
			...
			let qz_required_assets = [
				"/assets/kanivin/node_modules/js-sha256/build/sha256.min.js",
				"/assets/kanivin/node_modules/qz-tray/qz-tray.js",
			];
			kanivin.require(qz_required_assets, () => {
			...
			// note 'kanivin.require' does not have callback on fail. Hence, any failure cannot be communicated to the user.
		}
	});
};

kanivin.ui.form.qz_get_printer_list = function () {
	// returns the list of printers that are available to the QZ Tray
	return kanivin.ui.form
		.qz_connect()
		.then(function () {
...
kanivin.ui.form.qz_success = function () {
...
kanivin.ui.form.qz_fail = function (e) {
	// notify qz errors
	kanivin.show_alert(
		{
