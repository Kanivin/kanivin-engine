...
kanivin.ui.form.Layout = class Layout {
	constructor(opts) {
		this.views = {};
		this.pages = [];
		this.tabs = [];
	...
	get_doctype_fields() {
		...
		if (this.doctype_layout) {
			...
			fields = fields.concat(
				kanivin.meta.sort_docfields(kanivin.meta.docfield_map[this.doctype])
			);
		}
	...
	get_fields_from_layout() {
		...
		for (let f of this.doctype_layout.fields) {
			const docfield = copy_dict(kanivin.meta.docfield_map[this.doctype][f.fieldname]);
	...
	show_message(html, color, permanent = false) {
		...
		const close_message = $(`<div class="close-message">${kanivin.utils.icon("close")}</div>`);
		if (!permanent) {
	...
	init_field(df, parent, render = false) {
		const fieldobj = kanivin.ui.form.make_control({
			df: df,
	...
	make_page(df) {
		...
		this.fold_btn = head.find(".btn-fold").on("click", function () {
			...
			if (page.hasClass("hide")) {
				...
				kanivin.utils.scroll_to($(this), true, 30);
				me.folded = false;
			} else {
	...
	is_numeric_field_active() {
		const control = $(document.activeElement).closest(".kanivin-control");
	...
	refresh_sections() {
		...
		this.wrapper.find(".form-section:not(.hide-control)").each(function () {
			...
			if (section.find(".kanivin-control:not(.hide-control)").length) {
	...
	attach_doc_and_docfields(refresh) {
		...
		for (let i = 0, l = this.fields_list.length; i < l; i++) {
			...
			if (me.doc) {
				...
				fieldobj.df =
					kanivin.meta.get_docfield(me.doc.doctype, fieldobj.df.fieldname, me.doc.name) ||
					fieldobj.df;
			}
		...
		for (let i = start_idx + 1, len = fields.length; i < len; i++) {
			...
			if (this.is_visible(field)) {
				if (field.df.fieldtype === "Table") {
					...
					!kanivin.model.no_value_type.includes(field.df.fieldtype)
				) {
