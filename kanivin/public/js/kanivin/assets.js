...
// depends on kanivin.versions to manage versioning

kanivin.require = function (items, callback) {
	if (typeof items === "string") {
		items = [items];
	}
	items = items.map((item) => kanivin.assets.bundled_asset(item));

	return new Promise((resolve) => {
		kanivin.assets.execute(items, () => {
			resolve();
			callback && callback();
		});
	});
...
class AssetManager {
	constructor() {
		...
		this._handlers = {
			js: function (txt) {
				kanivin.dom.eval(txt);
			},
			css: function (txt) {
				kanivin.dom.set_style(txt);
			},
		};
	}
	check() {
	...
	init_local_storage() {
		...
		if (kanivin.boot) localStorage.metadata_version = kanivin.boot.metadata_version;
	}
	...
	execute(items, callback) {
		...
		const version_string =
			kanivin.boot.developer_mode || window.dev_server ? Date.now() : window._version_number;
...
kanivin.assets = new AssetManager();
