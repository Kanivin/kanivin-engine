	...
	<div class="tree-node" :class="{ opened: node.open }">
		...
		>
			...
			<a
				...
				v-html="kanivin.utils.icon('external-link', 'sm')"
				@click.stop
			/>
		</span>
		<div v-if="node.file_url && kanivin.utils.is_image_file(node.file_url)">
...
let icon = computed(() => {
	let icons = {
		open: kanivin.utils.icon("folder-open", "md"),
		closed: kanivin.utils.icon("folder-normal", "md"),
		leaf: kanivin.utils.icon("primitive-dot", "xs"),
		search: kanivin.utils.icon("search"),
	};
...
let open_file = (filename) => {
	return kanivin.utils.get_form_link("File", filename);
};
