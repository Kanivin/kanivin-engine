		...
		<div class="file-browser-list">
			<div class="file-filter">
				<input
					...
					v-model="search_text"
					@input="kanivin.utils.debounce(search_by_name(), 300)"
				/>
			</div>
			<TreeNode
...
function get_files_in_folder(folder, start) {
	return kanivin
		.call("kanivin.core.api.file.get_files_in_folder", {
			folder,
			start,
...
function search_by_name() {
	...
	kanivin
		.call("kanivin.core.api.file.get_files_by_search_text", {
...
function make_file_node(file) {
	...
	let label = kanivin.utils.file_name_ellipsis(filename, 40);
	return {
		label: label,
		filename: filename,
