				...
				<div class="mt-2 text-center">
					...
					>
						<svg width="30" height="30">
							<image
								href="/assets/kanivin/icons/social/google_drive.svg"
								width="30"
								height="30"
							/>
						</svg>
...
if (kanivin.user_id !== "Guest") {
	kanivin.call({
		// method only available after login
		method: "kanivin.integrations.doctype.google_settings.google_settings.get_file_picker_settings",
		callback: (resp) => {
			if (!resp.exc) {
...
if (props.restrictions.max_file_size == null) {
	kanivin.call("kanivin.core.api.file.get_max_file_size").then((res) => {
		props.restrictions.max_file_size = Number(res.message);
	});
}
if (props.restrictions.max_number_of_files == null && props.doctype) {
	props.restrictions.max_number_of_files = kanivin.get_meta(props.doctype)?.max_attachments;
}
...
function upload_files(dialog) {
	...
	if (!files.value.length) {
		kanivin.msgprint(__("Please select a file first."));
	...
	return kanivin.run_serially(files.value.map((file, i) => () => upload_file(file, i)));
}
function upload_via_file_browser() {
	let selected_file = file_browser.value.selected_node;
	if (!selected_file.value) {
		kanivin.msgprint(__("Click on a file to select it."));
...
function upload_via_web_link() {
	...
	if (!file_url) {
		kanivin.msgprint(__("Invalid URL"));
		close_dialog.value = true;
		return Promise.reject();
	}
...
function return_as_dataurl() {
	let promises = files.value.map((file) =>
		kanivin.dom.file_to_base64(file.file_obj).then((dataurl) => {
...
function capture_image() {
	const capture = new kanivin.ui.Capture({
		animate: false,
		error: true,
	});
	capture.show();
	capture.submit((data_urls) => {
		data_urls.forEach((data_url) => {
			let filename = `capture_${kanivin.datetime
				.now_datetime()
				.replaceAll(/[: -]/g, "_")}.png`;
