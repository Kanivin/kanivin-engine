...
kanivin.provide("kanivin.realtime");
...
class RealTimeClient {
	...
	get_host(port = 9000) {
		...
		if (window.dev_server) {
			...
			port = kanivin.boot.socketio_port || port.toString() || "9000";
			if (parts.length > 2) {
				host = parts[0] + ":" + parts[1];
			}
			host = host + ":" + port;
		}
		return host + `/${kanivin.boot.sitename}`;
	}
	...
	doc_subscribe(doctype, docname) {
		if (kanivin.flags.doc_subscribe) {
		...
		kanivin.flags.doc_subscribe = true;

		// throttle to 1 per sec
		setTimeout(function () {
			kanivin.flags.doc_subscribe = false;
		}, 1000);
...
kanivin.realtime = new RealTimeClient();

// backward compatbility
kanivin.socketio = kanivin.realtime;
