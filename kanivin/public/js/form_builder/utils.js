...
export async function load_doctype_model(doctype) {
	await kanivin.call("kanivin.desk.form.load.getdoctype", { doctype });
}

export async function get_table_columns(df, child_doctype) {
	let table_columns = [];

	if (!kanivin.get_meta(df.options)) {
		await load_doctype_model(df.options);
	}
	if (!child_doctype) {
		child_doctype = kanivin.get_meta(df.options);
	}
	...
	for (let tf of table_fields) {
		if (!kanivin.model.layout_fields.includes(tf.fieldtype) && tf.in_list_view && tf.label) {
			let colsize;
	...
	function adjust_column_size(total_colsize, increase) {
		...
		while (start_condition && passes < 11) {
			for (var i in table_columns) {
				...
				if (colsize > 1 && colsize < 11 && kanivin.model.is_non_std_field(_df.fieldname)) {
					if (
						passes < 3 &&
		...
		if (d.fieldtype) {
			if (!d.fieldname) {
				if (d.label) {
					...
					if (kanivin.model.restricted_fields.includes(d.fieldname)) {
					...
					d.fieldname =
						...
						"_" +
						kanivin.utils.get_random(4);
				}
			} else {
				if (kanivin.model.restricted_fields.includes(d.fieldname)) {
					kanivin.throw(__("Fieldname {0} is restricted", [d.fieldname]));
				}
			}
...
export function clone_field(field) {
	...
	cloned_field.df.name = kanivin.utils.get_random(8);
	return cloned_field;
}
