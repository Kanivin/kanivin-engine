...
function remove_field() {
	if (store.is_customize_form && props.field.df.is_custom_field == 0) {
		kanivin.msgprint(__("Cannot delete standard field. You can hide it if you want"));
...
function duplicate_field() {
	...
	duplicate_field.df.owner = kanivin.session.user;
...
function make_dialog(frm) {
	frm.dialog = new kanivin.ui.Dialog({
	...
	if (frm.doctype === "Customize Form") {
		...
		kanivin.db.exists("Property Setter", property_setter_id).then((exits) => {
			if (exits) {
				frm.dialog.set_secondary_action_label(__("Reset To Default"));
				frm.dialog.set_secondary_action(() => {
					kanivin.call({
						method: "kanivin.custom.doctype.customize_form.customize_form.get_link_filters_from_doc_without_customisations",
						args: {
...
function edit_filters() {
	...
	kanivin.model.with_doctype(field_doctype, () => {
		frm.dialog.show();
		add_existing_filter(frm, props.field.df);
	});
}
			...
			<template #label>
				<div class="field-label">
					...
					<div
						...
						v-html="kanivin.utils.icon('help', 'sm')"
					></div>
				</div>
			</template>
			<template #actions>
				<div class="field-actions" :hidden="store.read_only">
					<button
						...
						@click="edit_filters"
					>
						<div v-html="kanivin.utils.icon('filter', 'sm')"></div>
					</button>
					<AddFieldButton ref="add_field_ref" :column="column" :field="field">
						<div v-html="kanivin.utils.icon('add', 'sm')" />
					</AddFieldButton>
					<button
					...
					>
						<div v-html="kanivin.utils.icon('move', 'sm')"></div>
					</button>
					<button
						class="btn btn-xs btn-icon"
					...
					>
						<div v-html="kanivin.utils.icon('duplicate', 'sm')"></div>
					</button>
					<button
						class="btn btn-xs btn-icon"
					...
					>
						<div v-html="kanivin.utils.icon('remove', 'sm')"></div>
					</button>
				</div>
			</template>
		</component>
