...
function get_options() {
	...
	if (props.df.fieldname == "fieldtype") {
		if (!in_list(kanivin.model.layout_fields, props.modelValue)) {
			options =
				options &&
				options.filter((opt) => !in_list(kanivin.model.layout_fields, opt.value));
		} else {
...
<template>
	<div v-if="slots.label" class="control kanivin-control" :class="{ editable: slots.label }">
		<!-- label -->
		...
		<div class="select-input">
			...
			<div class="select-icon" v-html="kanivin.utils.icon('select', 'sm')"></div>
		</div>
