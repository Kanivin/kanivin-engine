...
if (props.df.fieldtype === "Datetime") {
	let time_zone_text = kanivin.boot.time_zone
		? kanivin.boot.time_zone.user
		: kanivin.sys_defaults.time_zone;
...
<template>
	<div class="control kanivin-control" :class="{ editable: slots.label }">
		<!-- label -->
		...
		<div
			...
			v-html="kanivin.utils.icon('folder-normal', 'md')"
		/>
		<!-- phone selector icon -->
		<div
			v-if="df.fieldtype == 'Phone'"
			class="selected-phone no-value"
			v-html="kanivin.utils.icon('down', 'sm')"
		/>
	</div>
</template>
