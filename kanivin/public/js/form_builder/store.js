...
export const useStore = defineStore("form-builder-store", () => {
	...
	function get_df(fieldtype, fieldname = "", label = "") {
		...
		let df = kanivin.model.get_new_doc(docfield);
		df.name = kanivin.utils.get_random(8);
	...
	async function fetch() {
		...
		if (doctype.value.startsWith("new-doctype-") && !doc.value.fields?.length) {
			kanivin.model.with_doctype("DocType").then(() => {
		...
		if (!get_docfields.value.length) {
			let docfield = is_customize_form.value ? "Customize Form Field" : "DocField";
			if (!kanivin.get_meta(docfield)) {
				await load_doctype_model(docfield);
			}
			let df = kanivin.get_meta(docfield).fields;
		...
		nextTick(() => {
			...
			read_only.value =
				!is_customize_form.value && !kanivin.boot.developer_mode && !doc.value.custom;
			preview.value = false;
		});
	...
	function validate_fields(fields, is_table) {
		...
		let not_allowed_in_list_view = ["Attach Image", ...kanivin.model.no_value_type];
		if (is_table) {
		...
		fields.forEach((df) => {
			...
			if (["Link", ...kanivin.model.table_fields].includes(df.fieldtype) && !df.options) {
				error_message = __(
			...
			if (df.in_global_search && kanivin.model.no_value_type.includes(df.fieldtype)) {
				error_message = __(
