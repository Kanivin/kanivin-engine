kanivin.ui.OnboardingTour = class OnboardingTour {
	constructor() {
		this.driver_steps = [];
	...
	init_driver() {
		this.driver = new kanivin.Driver({
			className: "kanivin-driver",
			allowClose: false,
			padding: 10,
			...
			opacity: 0.5,
			onHighlighted: (step) => {
				kanivin.ui.next_form_tour = step.options.step_info?.next_form_tour;
				const wait_for_node = setInterval(() => {
					...
					if (step.popover.closeBtnNode) {
						step.popover.closeBtnNode.onclick = () => {
							...
							!kanivin.boot.user.onboarding_status[this.tour.name] &&
								(kanivin.boot.user.onboarding_status[this.tour.name] = {});
							kanivin.boot.user.onboarding_status[this.tour.name].is_complete = true;
							if (!this.driver.hasNextStep()) {
								kanivin.boot.user.onboarding_status[
									this.tour.name
								].all_steps_completed = true;
							}

							kanivin.call({
								method: "kanivin.desk.doctype.form_tour.form_tour.update_user_status",
								args: {
									value: JSON.stringify(kanivin.boot.user.onboarding_status),
				...
				if ($input) kanivin.utils.sleep(200).then(() => $input.focus());
			},
		});
	}

	async init({ tour_name, start_step }) {
		this.tour = await kanivin.db.get_doc("Form Tour", tour_name);
		this.init_driver();
		this.build_steps();
	...
	get_step(step_info, on_next) {
		...
		return {
			...
			popover: {
				title,
				description,
				position: kanivin.router.slug(position || "Bottom"),
			},
			onNext: on_next,
			step_info: step_info,
			showButtons,
...
kanivin.ui.init_onboarding_tour = () => {
	...
	typeof kanivin.boot.onboarding_tours == "undefined" && kanivin.boot.onboarding_tours == [];
	typeof kanivin.boot.user.onboarding_status == "undefined" &&
		kanivin.boot.user.onboarding_status == {};
	let route = kanivin.router.current_route;
	if (route?.[0] === "") return;
	...
	if (route[0] != "dashboard-view") {
		kanivin.boot.onboarding_tours &&
			kanivin.boot.onboarding_tours.forEach((tour) => {
	...
	matching_tours = matching_tours.filter((tour) => {
		if (kanivin.boot.user.onboarding_status[tour[0]]?.is_complete == true) return false;
		return true;
	});
	matching_tours = matching_tours.map((tour) => {
		if (kanivin.boot.user.onboarding_status[tour[0]]?.steps_complete != undefined) {
			tour.push(kanivin.boot.user.onboarding_status[tour[0]].steps_complete);
		}
		return tour;
	});
	if (matching_tours.length == 0) return;
	let current_tour = matching_tours.find(
		(tour) => tour[0] == kanivin.ui.currentTourInstance?.tour?.name
	);
	let next_tour = matching_tours.find((tour) => tour[0] == kanivin.ui.next_form_tour);
	if (current_tour) {
		...
		kanivin.ui.next_form_tour = undefined;
	} else {
	...
	if (!tour_name) return;
	if (kanivin.ui.currentTourInstance?.driver) {
		kanivin.ui.currentTourInstance.driver_steps = [];
		kanivin.ui.currentTourInstance.driver.reset(true);
		kanivin.ui.currentTourInstance.update_driver_steps();
	}
	const tour = (kanivin.ui.currentTourInstance = new kanivin.ui.OnboardingTour());
