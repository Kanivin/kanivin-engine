...
export const useStore = defineStore("workflow-builder-store", () => {
	...
	async function fetch() {
		await kanivin.model.clear_doc("Workflow", workflow_name.value);
		await kanivin.model.with_doc("Workflow", workflow_name.value);

		workflow_doc.value = kanivin.get_doc("Workflow", workflow_name.value);
		await kanivin.model.with_doctype(workflow_doc.value.document_type);

		if (!workflowfields.value.length) {
			await kanivin.model.with_doctype("Workflow");
			workflowfields.value = kanivin.get_meta("Workflow").fields;
		}

		if (!statefields.value.length) {
			await kanivin.model.with_doctype("Workflow Document State");
			statefields.value = kanivin.get_meta("Workflow Document State").fields;
		}

		if (!transitionfields.value.length) {
			await kanivin.model.with_doctype("Workflow Transition");
			transitionfields.value = kanivin.get_meta("Workflow Transition").fields;
		}

		if (!workflow_doc_fields.value.length) {
			let doc_type = workflow_doc.value.document_type;
			await kanivin.model.with_doctype(doc_type);
			workflow_doc_fields.value = kanivin.meta
				.get_docfields(doc_type, null, {
					fieldtype: ["not in", kanivin.model.no_value_type],
				})
				.sort((a, b) => {
					if (a.label && b.label) {
	...
	async function save_changes() {
		...
		try {
			...
			await kanivin.call("kanivin.client.save", { doc });
			kanivin.toast(__("Workflow updated successfully"));
			fetch();
		} catch (e) {
			console.error(e);
		} finally {
	...
	function setup_breadcrumbs() {
		...
		`;
		kanivin.breadcrumbs.clear();
		kanivin.breadcrumbs.$breadcrumbs.append(breadcrumbs);
	}
