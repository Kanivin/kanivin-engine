let kanivinCloudBaseEndpoint = "https://kanivincloud.com";
let isFCUser = false;

$(document).ready(function () {
	if (
		kanivin.boot.is_fc_site &&
		kanivin.boot.setup_complete === 1 &&
		!kanivin.is_mobile() &&
		kanivin.user.has_role("System Manager")
	) {
		kanivin.call({
			method: "kanivin.integrations.kanivin_providers.kanivincloud_billing.current_site_info",
			callback: (r) => {
				if (!r?.message) return;

				const response = r.message;
				const trial_end_date = new Date(response.trial_end_date);
				kanivinCloudBaseEndpoint = response.base_url;
...
function openKanivinCloudDashboard() {
	window.open(`${kanivinCloudBaseEndpoint}/dashboard/sites/${kanivin.boot.sitename}`, "_blank");
}
