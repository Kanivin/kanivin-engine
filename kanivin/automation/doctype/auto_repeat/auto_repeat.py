...
from kanivin.automation.doctype.assignment_rule.assignment_rule import get_repeated
from kanivin.contacts.doctype.contact.contact import (
	get_contacts_linked_from,
	get_contacts_linking_to,
)
from kanivin.core.doctype.communication.email import make
...
from kanivin.utils.background_jobs import get_jobs
...
class AutoRepeat(Document):
	...
	if TYPE_CHECKING:
		from kanivin.automation.doctype.auto_repeat_day.auto_repeat_day import AutoRepeatDay
	...
	def after_save(self):
		kanivin.get_doc(self.reference_doctype, self.reference_document).notify_update()

	def on_trash(self):
		kanivin.db.set_value(self.reference_doctype, self.reference_document, "auto_repeat", "")
		kanivin.get_doc(self.reference_doctype, self.reference_document).notify_update()
	...
	def set_dates(self):
		if self.disabled:
			...
			if self.end_date and getdate(self.end_date) < getdate(self.next_schedule_date):
				kanivin.throw(_("The Next Scheduled Date cannot be later than the End Date."))
	...
	def validate_reference_doctype(self):
		if kanivin.flags.in_test or kanivin.flags.in_patch:
			return
		if not kanivin.get_meta(self.reference_doctype).allow_auto_repeat:
			kanivin.throw(
	...
	def validate_submit_on_creation(self):
		if self.submit_on_creation and not kanivin.get_meta(self.reference_doctype).is_submittable:
			kanivin.throw(
				_("Cannot enable {0} for a non-submittable doctype").format(
					kanivin.bold(_("Submit on Creation"))
				)
			)
		...
		if self.end_date == self.start_date:
			kanivin.throw(
				_("{0} should not be same as {1}").format(
					kanivin.bold(_("End Date")), kanivin.bold(_("Start Date"))
				)
			)
			...
			if self.recipients:
				...
				for email in email_list:
					if not validate_email_address(email):
						kanivin.throw(_("{0} is an invalid email address in 'Recipients'").format(email))
			else:
				kanivin.throw(_("'Recipients' not specified"))
		...
		if not len(set(auto_repeat_days)) == len(auto_repeat_days):
			...
			kanivin.throw(
				_("Auto Repeat Day{0} {1} has been repeated.").format(
					plural, kanivin.bold(", ".join(repeated_days))
				)
			)

	def update_auto_repeat_id(self):
		# check if document is already on auto repeat
		auto_repeat = kanivin.db.get_value(self.reference_doctype, self.reference_document, "auto_repeat")
		if auto_repeat and auto_repeat != self.name and not kanivin.flags.in_patch:
			kanivin.throw(
				_("The {0} is already on auto repeat {1}").format(self.reference_document, auto_repeat)
			)
		else:
			kanivin.db.set_value(self.reference_doctype, self.reference_document, "auto_repeat", self.name)
	...
	def create_documents(self):
		try:
			...
			if self.reference_document and not kanivin.flags.in_test:
				self.notify_error_to_user(error_log)

	def make_new_document(self):
		reference_doc = kanivin.get_doc(self.reference_doctype, self.reference_document)
		new_doc = kanivin.copy_doc(reference_doc, ignore_no_copy=False)
	...
	def update_doc(self, new_doc, reference_doc):
		...
		auto_repeat_doc = kanivin.get_doc("Auto Repeat", self.name)
	...
	def send_notification(self, new_doc):
		...
		if not self.subject:
			...
			subject = kanivin.render_template(self.subject, {"doc": new_doc})
		...
		try:
			...
			).format(kanivin.bold(_("Note")), kanivin.bold(_("Allow Print for Draft")))
			attachments = None
		...
		if error_string:
			...
			message = kanivin.render_template(self.message, {"doc": new_doc})
	...
	def fetch_linked_contacts(self):
		if self.reference_doctype and self.reference_document:
			...
			if not email_ids:
				kanivin.msgprint(_("No contacts linked to document"), alert=True)
			else:
				self.recipients = ", ".join(email_ids)

	def disable_auto_repeat(self):
		kanivin.db.set_value("Auto Repeat", self.name, "disabled", 1)
	...
	def notify_error_to_user(self, error_log):
		...
		form_link = kanivin.utils.get_link_to_form(self.reference_doctype, self.reference_document)
		auto_repeat_failed_for = _("Auto Repeat failed for {0}").format(form_link)

		error_log_link = kanivin.utils.get_link_to_form("Error Log", error_log.name)
...
def make_auto_repeat_entry():
	enqueued_method = "kanivin.automation.doctype.auto_repeat.auto_repeat.create_repeated_entries"
	jobs = get_jobs()

	if not jobs or enqueued_method not in jobs[kanivin.local.site]:
		date = getdate(today())
		data = get_auto_repeat_entries(date)
		kanivin.enqueue(enqueued_method, data=data)
...
def create_repeated_entries(data):
	for d in data:
		doc = kanivin.get_doc("Auto Repeat", d.name)
		...
		if schedule_date == current_date and not doc.disabled:
			...
			if schedule_date and not doc.disabled:
				kanivin.db.set_value("Auto Repeat", doc.name, "next_schedule_date", schedule_date)
...
def get_auto_repeat_entries(date=None):
	...
	auto_repeat = kanivin.qb.DocType("Auto Repeat")
	query = kanivin.qb.from_(auto_repeat)
...
def set_auto_repeat_as_completed(auto_repeat):
	for entry in auto_repeat:
		doc = kanivin.get_doc("Auto Repeat", entry.name)
		if doc.is_completed():
...
def make_auto_repeat(doctype, docname, frequency="Daily", start_date=None, end_date=None):
	...
	doc = kanivin.new_doc("Auto Repeat")
...
def update_reference(docname: str, reference: str):
	doc = kanivin.get_doc("Auto Repeat", str(docname))
...
def generate_message_preview(reference_dt, reference_doc, message=None, subject=None):
	kanivin.has_permission("Auto Repeat", "write", throw=True)
	doc = kanivin.get_doc(reference_dt, reference_doc)
	doc.check_permission()
	subject_preview = _("Please add a subject to your email")
	msg_preview = kanivin.render_template(message, {"doc": doc})
	if subject:
		subject_preview = kanivin.render_template(subject, {"doc": doc})
