...
import kanivin
from kanivin.automation.doctype.auto_repeat.auto_repeat import (
...
from kanivin.utils import add_days, add_months, getdate, today

if TYPE_CHECKING:
	from kanivin.custom.doctype.custom_field.custom_field import CustomField
...
def add_custom_fields() -> "CustomField":
	...
	return create_custom_field("ToDo", df) or kanivin.get_doc(
		"Custom Field", dict(fieldname=df["fieldname"], dt="ToDo")
	)


class TestAutoRepeat(KanivinTestCase):
	@classmethod
	def setUpClass(cls):
	...
	def test_daily_auto_repeat(self):
		...
		todo = kanivin.get_doc(doc.reference_doctype, doc.reference_document)
		self.assertEqual(todo.auto_repeat, doc.name)

		new_todo = kanivin.db.get_value("ToDo", {"auto_repeat": doc.name, "name": ("!=", todo.name)}, "name")

		new_todo = kanivin.get_doc("ToDo", new_todo)
	...
	def monthly_auto_repeat(self, doctype, docname, start_date, end_date=None):
		...
		data = get_auto_repeat_entries(getdate(today()))
		create_repeated_entries(data)
		docnames = kanivin.get_all(doc.reference_doctype, {"auto_repeat": doc.name})
		self.assertEqual(len(docnames), 1)

		doc = kanivin.get_doc("Auto Repeat", doc.name)
	...
	def test_email_notification(self):
		...
		email_queue = kanivin.db.exists("Email Queue", dict(reference_doctype="ToDo", reference_name=new_todo))
	...
	def test_submit_on_creation(self):
		...
		submittable_doc = kanivin.get_doc(dict(doctype=doctype, test="test submit on creation")).insert()
	...
	return kanivin.get_doc(
		{
			...
			"reference_document": args.reference_document or kanivin.db.get_value("ToDo", "name"),
...
def create_submittable_doctype(doctype, submit_perms=1):
	if kanivin.db.exists("DocType", doctype):
		return
	else:
		doc = kanivin.get_doc(
			{
