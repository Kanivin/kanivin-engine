...
from kanivin.utils.data import add_to_date, get_datetime, now_datetime
...
class Reminder(Document):
	...
	def clear_old_logs(days=30):
		...
		table = kanivin.qb.DocType("Reminder")
		kanivin.db.delete(table, filters=(table.remind_at < (Now() - Interval(days=days))))

	def validate(self):
		self.user = kanivin.session.user
		if get_datetime(self.remind_at) < now_datetime():
			kanivin.throw(_("Reminder cannot be created in past."))
...
def create_new_reminder(
	...
	reminder = kanivin.new_doc("Reminder")
...
def send_reminders():
	...
	pending_reminders = kanivin.get_all(
		"Reminder",
		filters=[
	...
	for reminder in pending_reminders:
		kanivin.get_doc("Reminder", reminder).send_reminder()
