...
class AssignmentRule(Document):
	...
	if TYPE_CHECKING:
		from kanivin.automation.doctype.assignment_rule_day.assignment_rule_day import AssignmentRuleDay
		from kanivin.automation.doctype.assignment_rule_user.assignment_rule_user import (
			AssignmentRuleUser,
		)
	...
	def validate_document_types(self):
		if self.document_type == "ToDo":
			kanivin.throw(
				_("Assignment Rule is not allowed on document type {0}").format(kanivin.bold(_("ToDo")))
			)
		...
		if user:
			assign_to.add(
				dict(
					...
					name=doc.get("name"),
					description=kanivin.render_template(self.description, doc),
	...
	def get_user_based_on_field(self, doc):
		...
		if kanivin.db.exists("User", val):
			return val

	def safe_eval(self, fieldname, doc):
		try:
			if self.get(fieldname):
				return kanivin.safe_eval(self.get(fieldname), None, doc)
		except Exception as e:
			# when assignment fails, don't block the document as it may be
			# a part of the email pulling
			kanivin.msgprint(kanivin._("Auto assignment failed: {0}").format(str(e)), indicator="orange")
	...
	def is_rule_not_applicable_today(self):
		today = kanivin.flags.assignment_day or kanivin.utils.get_weekday()
		assignment_days = self.get_assignment_days()
...
def reopen_closed_assignment(doc):
	...
	for todo in todo_list:
		todo_doc = kanivin.get_doc("ToDo", todo)
...
def apply(doc=None, method=None, doctype=None, name=None):
	...
	assignment_rule_docs: list[AssignmentRule] = [
		kanivin.get_cached_doc("Assignment Rule", d.get("name")) for d in assignment_rules
	]
	...
	if assignments:
		for assignment_rule in assignment_rule_docs:
			...
			if not new_apply:
				...
				if to_close_todos:
					# close todo status
					todos_to_close = kanivin.get_all(
						"ToDo",
						filters={
							"reference_type": doc.doctype,
							"reference_name": doc.name,
					...
					for todo in todos_to_close:
						_todo = kanivin.get_doc("ToDo", todo)
						_todo.status = "Closed"
...
def update_due_date(doc, state=None):
	...
	for rule in assignment_rules:
		rule_doc = kanivin.get_cached_doc("Assignment Rule", rule.get("name"))
		...
		if field_updated:
			assignment_todos = kanivin.get_all(
				"ToDo",
				filters={
...
def get_assignment_rules() -> list[str]:
	return kanivin.get_all("Assignment Rule", filters={"disabled": 0}, pluck="document_type")
