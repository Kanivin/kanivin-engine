...
import kanivin
from kanivin.test_runner import make_test_records
from kanivin.tests.utils import KanivinTestCase

TEST_DOCTYPE = "Assignment Test"


class TestAutoAssign(KanivinTestCase):
	@classmethod
	def setUpClass(cls):
		super().setUpClass()
		kanivin.db.delete("Assignment Rule")
	...
	def test_load_balancing(self):
		...
		for user in ("test@example.com", "test1@example.com", "test2@example.com"):
			self.assertEqual(
				len(kanivin.get_all("ToDo", dict(allocated_to=user, reference_type=TEST_DOCTYPE))), 10
			)
		...
		for d in kanivin.get_all(
			...
			kanivin.db.delete("ToDo", {"name": d.name})
	...
	def check_assignment_rule_scheduling(self):
		...
		kanivin.flags.assignment_day = "Monday"
		note = _make_test_record(public=1)
		...
		kanivin.flags.assignment_day = "Friday"
		note = _make_test_record(public=1)
	...
	def test_assignment_rule_condition(self):
		...
		assignment_rule = kanivin.get_doc(
			dict(
		...
		expiry_date = kanivin.utils.add_days(kanivin.utils.nowdate(), 2)
		note1 = _make_test_record(expiry_date=expiry_date)
		note2 = _make_test_record(expiry_date=expiry_date)

		note1_todo = kanivin.get_all(
			"ToDo", filters=dict(reference_type=TEST_DOCTYPE, reference_name=note1.name, status="Open")
		)[0]

		note1_todo_doc = kanivin.get_doc("ToDo", note1_todo.name)
		self.assertEqual(kanivin.utils.get_date_str(note1_todo_doc.date), expiry_date)

		# due date should be updated if the reference doc's date is updated.
		note1.expiry_date = kanivin.utils.add_days(expiry_date, 2)
		note1.save()
		note1_todo_doc.reload()
		self.assertEqual(kanivin.utils.get_date_str(note1_todo_doc.date), note1.expiry_date)

		# saving one note's expiry should not update other note todo's due date
		note2_todo = kanivin.get_all(
			"ToDo",
			filters=dict(reference_type=TEST_DOCTYPE, reference_name=note2.name, status="Open"),
			fields=["name", "date"],
		)[0]
		self.assertNotEqual(kanivin.utils.get_date_str(note2_todo.date), note1.expiry_date)
		self.assertEqual(kanivin.utils.get_date_str(note2_todo.date), expiry_date)
		assignment_rule.delete()
		kanivin.db.commit()  # undo changes commited by DDL
	...
	def test_submittable_assignment(self):
		...
		dt = kanivin.get_doc("DocType", submittable_doctype)
		dt.is_submittable = 1
		dt.save()

		# create a rule for the submittable doctype
		assignment_rule = kanivin.new_doc("Assignment Rule")
		...
		doc = kanivin.new_doc(submittable_doctype)
		doc.save()
		doc.submit()
...
def clear_assignments():
	kanivin.db.delete("ToDo", {"reference_type": TEST_DOCTYPE})


def get_assignment_rule(days, assign=None):
	kanivin.delete_doc_if_exists("Assignment Rule", f"For {TEST_DOCTYPE} 1")
	...
	kanivin.delete_doc_if_exists("Assignment Rule", f"For {TEST_DOCTYPE} 2")
...
def _make_test_record(
	...
	**kwargs,
	...
	doc = kanivin.new_doc(TEST_DOCTYPE)
