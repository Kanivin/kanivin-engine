...
import 
from .utils import get_request_site_address

CALLBACK_METHOD = "/api/method/.integrations.google_oauth.callback"
_SCOPES = {
...
_DOMAIN_CALLBACK_METHODS = {
	"mail": ".email.oauth.authorize_google_access",
	"contacts": ".integrations.doctype.google_contacts.google_contacts.authorize_access",
	"drive": ".integrations.doctype.google_drive.google_drive.authorize_access",
	"indexing": ".website.doctype.website_settings.google_indexing.authorize_access",
}
...
class GoogleOAuth:
	...
	def __init__(self, domain: str, validate: bool = True):
		self.google_settings = .get_single("Google Settings")
	...
	def validate_google_settings(self):
		...
		if not self.google_settings.enable:
			.throw(._("Please enable {} before continuing.").format(google_settings))

		if not (self.google_settings.client_id and self.google_settings.client_secret):
			.throw(._("Please update {} before continuing.").format(google_settings))
...
def handle_response(
	...
	if "error" in response:
		.log_error(._(error_title), ._(response.get("error_description", error_message)))

		if raise_err:
			.throw(._(error_title), GoogleAuthenticationError, ._(error_message))
...
def callback(state: str, code: str | None = None, error: str | None = None) -> None:
	...
	Invokes functions using `.get_attr` and also adds required (keyworded) arguments
	along with committing and redirecting us back to  site."""
	...
	if not error:
		if (domain := state.pop("domain")) in _DOMAIN_CALLBACK_METHODS:
			...
			.get_attr(_DOMAIN_CALLBACK_METHODS[domain])(**state)
