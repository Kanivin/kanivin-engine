...
import kanivin
from kanivin.utils import cint, split_emails
...
def send_email(success, service_name, doctype, email_field, error_status=None):
	...
	if success:
		if not kanivin.db.get_single_value(doctype, "send_email_for_successful_backup"):
			return
...
<p>Please contact your system manager for more information.</p>"""
	...
	kanivin.sendmail(recipients=recipients, subject=subject, message=message)


def get_recipients(doctype, email_field):
	return split_emails(kanivin.db.get_value(doctype, None, email_field))


def get_latest_backup_file(with_files=False):
	from kanivin.utils.backups import BackupGenerator
