...
import 
from  import _, _lt
from .integrations.google_oauth import GoogleOAuth
...
from .utils.password import set_encrypted_password

if TYPE_CHECKING:
	from .desk.doctype.event.event import Event
...
class GoogleCalendar(Document):
	...
	def validate(self):
		...
		if not google_settings.enable:
			.throw(_("Enable Google API in Google Settings."))

		if not google_settings.client_id or not google_settings.client_secret:
			.throw(_("Enter Client Id and Client Secret in Google Settings."))
	...
	def get_access_token(self):
		...
		if not self.refresh_token:
			raise .ValidationError(
				_("Click on {0} to generate Refresh Token.").format(.bold(allow_google_calendar_label))
			)
		...
		try:
			...
			.throw(
				...
				).format(.bold(allow_google_calendar_label))
			)
...
def authorize_access(g_calendar: str, reauthorize: bool = False):
	...
	"""
	google_calendar = .get_doc("Google Calendar", g_calendar)
	...
	if not google_calendar.authorization_code or reauthorize:
		.cache.hset("google_calendar", "google_calendar", google_calendar.name)
	...
	if "refresh_token" in r:
		.db.set_value("Google Calendar", google_calendar.name, "refresh_token", r["refresh_token"])
		.db.commit()

	.local.response["type"] = "redirect"
	.local.response["location"] = google_calendar.get_url()

	.msgprint(_("Google Calendar has been configured."), indicator="green")
...
def google_callback(code=None):
	...
	"""
	google_calendar = .cache.hget("google_calendar", "google_calendar")
	.db.set_value("Google Calendar", google_calendar, "authorization_code", code)
	.db.commit()
...
def sync(g_calendar: str | None = None):
	...
	for g in .get_list("Google Calendar", filters=filters, pluck="name"):
...
def get_google_calendar_object(g_calendar):
	...
	google_settings = .get_cached_doc("Google Settings")
	account: GoogleCalendar = .get_doc("Google Calendar", g_calendar)
...
def check_google_calendar(account: GoogleCalendar, google_calendar):
	...
	calendar = {
		...
		"timeZone": .get_system_settings("time_zone"),
	}
	try:
...
def sync_events_from_google_calendar(g_calendar, method=None):
	...
	while True:
		try:
			...
			if err.resp.status == 410:
				...
				.msgprint(msg, title="Invalid Sync Token", indicator="blue")
			else:
				.throw(msg)
	...
	for idx, event in enumerate(results):
		.publish_realtime(
			"import_google_calendar", {"progress": idx + 1, "total": len(results)}, user=.session.user
		)
		...
		if event.get("status") == "confirmed":
			...
			if not .db.exists("Event", {"google_calendar_event_id": event.get("id")}):
...
def insert_event_to_calendar(account, event, recurrence=None):
	"""
	Inserts event in Kanivin Calendar during Sync
	"""
	calendar_event = {
		"doctype": "Event",
	...
	e: Event = .get_doc(calendar_event)
...
def update_participants_in_event(calendar_event: "Event", google_event: dict):
	...
	in_system_participants = .get_all(
...
def update_event_in_calendar(account, event, recurrence=None):
	"""
	Updates Event in Kanivin Calendar if any existing Google Calendar Event is updated
	"""
	calendar_event = .get_doc("Event", {"google_calendar_event_id": event.get("id")})
...
def insert_event_in_google_calendar(doc, method=None):
	...
	if (
		...
		or not .db.exists("Google Calendar", {"name": doc.google_calendar})
	):
		return
	...
	try:
		...
		.msgprint(_("Event Synced with Google Calendar."))
	except HttpError as err:
		.throw(
...
def update_event_in_google_calendar(doc, method=None):
	"""
	Updates Events in Google Calendar if any existing event is modified in Kanivin Calendar
	"""
...
def delete_event_from_google_calendar(doc, method=None):
	"""
	Delete Events from Google Calendar if Kanivin Event is deleted.
	"""

	if not .db.exists("Google Calendar", {"name": doc.google_calendar, "push_to_google_calendar": 1}):
		return
