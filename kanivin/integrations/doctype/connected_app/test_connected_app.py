...
import kanivin
from kanivin.integrations.doctype.social_login_key.test_social_login_key import (
...
def get_connected_app():
	...
	connected_app = kanivin.new_doc(doctype)
	connected_app.provider_name = "kanivin"
...
def get_oauth_client():
	oauth_client = kanivin.new_doc("OAuth Client")
...
class TestConnectedApp(KanivinTestCase):
	def setUp(self):
		"""Set up a Connected App that connects to our own oAuth provider.

		Kanivin comes with it's own oAuth2 provider that we can test against. The
		...
		Kanivin's "Authorization URL" and "Access Token URL" (actually they're
	...
	def test_web_application_flow(self):
		...
		resp = oauth2_session.get(urljoin(self.base_url, "/api/method/kanivin.auth.get_logged_user"))
	...
	def tearDown(self):
		...
		if getattr(self, "oauth_client", None):
			tokens = kanivin.get_all("OAuth Bearer Token", filters={"client": self.oauth_client.name})
			for token in tokens:
				doc = kanivin.get_doc("OAuth Bearer Token", token.name)
				doc.delete()

			codes = kanivin.get_all("OAuth Authorization Code", filters={"client": self.oauth_client.name})
			for code in codes:
				doc = kanivin.get_doc("OAuth Authorization Code", code.name)
				doc.delete()
