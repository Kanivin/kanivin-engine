...
class BaseUrlNotSetError(kanivin.ValidationError):
	pass


class AuthorizeUrlNotSetError(kanivin.ValidationError):
	pass


class AccessTokenUrlNotSetError(kanivin.ValidationError):
	pass


class RedirectUrlNotSetError(kanivin.ValidationError):
	pass


class ClientIDNotSetError(kanivin.ValidationError):
	pass


class ClientSecretNotSetError(kanivin.ValidationError):
	pass
...
class SocialLoginKey(Document):
	...
	def autoname(self):
		self.name = kanivin.scrub(self.provider_name)

	def validate(self):
		self.set_icon()
		if self.custom_base_url and not self.base_url:
			kanivin.throw(_("Please enter Base URL"), exc=BaseUrlNotSetError)
		if not self.authorize_url:
			kanivin.throw(_("Please enter Authorize URL"), exc=AuthorizeUrlNotSetError)
		if not self.access_token_url:
			kanivin.throw(_("Please enter Access Token URL"), exc=AccessTokenUrlNotSetError)
		if not self.redirect_url:
			kanivin.throw(_("Please enter Redirect URL"), exc=RedirectUrlNotSetError)
		if self.enable_social_login and not self.client_id:
			kanivin.throw(_("Please enter Client ID before social login is enabled"), exc=ClientIDNotSetError)
	...
	def set_icon(self):
		...
		if self.provider_name in icon_map:
			...
			self.icon = f"/assets/kanivin/icons/social/{icon_file}"
	...
	def get_social_login_provider(self, provider, initialize=False):
		...
		providers["Office 365"] = {
			...
			"redirect_url": "/api/method/kanivin.integrations.oauth2_logins.login_via_office365",
			"api_endpoint": None,
		...
		providers["GitHub"] = {
			...
			"redirect_url": "/api/method/kanivin.integrations.oauth2_logins.login_via_github",
			"api_endpoint": "user",
			"api_endpoint_args": None,
		...
		providers["Google"] = {
			...
			"redirect_url": "/api/method/kanivin.integrations.oauth2_logins.login_via_google",
		...
		providers["Facebook"] = {
			...
			"redirect_url": "/api/method/kanivin.integrations.oauth2_logins.login_via_facebook",
		...
		providers["Kanivin"] = {
			...
			"enable_social_login": 1,
			"custom_base_url": 1,
			"icon": "/assets/kanivin/images/kanivin-favicon.svg",
			"redirect_url": "/api/method/kanivin.integrations.oauth2_logins.login_via_kanivin",
			"api_endpoint": "/api/method/kanivin.integrations.oauth2.openid_profile",
			"api_endpoint_args": None,
			"authorize_url": "/api/method/kanivin.integrations.oauth2.authorize",
			"access_token_url": "/api/method/kanivin.integrations.oauth2.get_token",
			"auth_url_data": json.dumps({"response_type": "code", "scope": "openid"}),
		}
		...
		providers["Salesforce"] = {
			...
			"redirect_url": "/api/method/kanivin.integrations.oauth2_logins.login_via_salesforce",
		...
		providers["fairlogin"] = {
			...
			"custom_base_url": 0,
			"icon": "fa fa-key",
			"redirect_url": "/api/method/kanivin.integrations.oauth2_logins.login_via_fairlogin",
		...
		providers["Keycloak"] = {
			...
			"enable_social_login": 1,
			"custom_base_url": 1,
			"redirect_url": "/api/method/kanivin.integrations.oauth2_logins.login_via_keycloak/keycloak",
...
def provider_allows_signup(provider: str) -> bool:
	from kanivin.website.utils import is_signup_disabled

	sign_up_config = kanivin.db.get_value("Social Login Key", provider, "sign_ups")
