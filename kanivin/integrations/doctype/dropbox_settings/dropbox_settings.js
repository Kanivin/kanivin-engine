...
.ui.form.on("Dropbox Settings", {
	...
	allow_dropbox_access: function (frm) {
		if (!frm.events.are_keys_present(frm)) {
			.msgprint(__("App Access Key and/or Secret Key are not present."));
			return;
		}

		.call({
			method: ".integrations.doctype.dropbox_settings.dropbox_settings.get_dropbox_authorize_url",
			freeze: true,
			callback: function (r) {
				if (!r.exc) {
	...
	take_backup: function (frm) {
		if (frm.doc.enabled && (frm.doc.dropbox_refresh_token || frm.doc.dropbox_access_token)) {
			frm.add_custom_button(__("Take Backup Now"), function () {
				.call({
					method: ".integrations.doctype.dropbox_settings.dropbox_settings.take_backup",
					freeze: true,
				});
			});
		}
