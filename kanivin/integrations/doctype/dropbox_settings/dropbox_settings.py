...
import 
from  import _
from .integrations.offsite_backup_utils import (
	get_chunk_site,
...
from .utils import cint, encode, get_backups_path, get_files_path, get_request_site_address
from .utils.background_jobs import enqueue
from .utils.backups import new_backup
...
def take_backup():
	...
	enqueue(
		".integrations.doctype.dropbox_settings.dropbox_settings.take_backup_to_dropbox",
		queue="long",
		timeout=1500,
	)
	.msgprint(_("Queued for backup. It may take a few minutes to an hour."))
...
def take_backups_if(freq):
	if .db.get_single_value("Dropbox Settings", "backup_frequency") == freq:
...
def take_backup_to_dropbox(retry_count=0, upload_db_backup=True):
	...
	try:
		if cint(.db.get_single_value("Dropbox Settings", "enabled")):
			validate_file_size()
			...
			if cint(.db.get_single_value("Dropbox Settings", "send_email_for_successful_backup")):
	...
	for f in .get_all(
		...
		for file_metadata in response.entries:
			try:
				...
				error_log.append(.get_traceback())
...
def update_file_dropbox_status(file_name):
	.db.set_value("File", file_name, "uploaded_to_dropbox", 1, update_modified=False)


def is_fresh_upload():
	file_name = .db.get_value("File", {"uploaded_to_dropbox": 1}, "name")
...
def get_dropbox_settings(redirect_uri=False):
	...
	settings = .get_doc("Dropbox Settings")
	app_details = {
		"app_key": settings.app_access_key or .conf.dropbox_access_key,
		"app_secret": settings.get_password(fieldname="app_secret_key", raise_exception=False)
		if settings.app_secret_key
		else .conf.dropbox_secret_key,
		...
		app_details.update(
			{
				...
				+ "/api/method/.integrations.doctype.dropbox_settings.dropbox_settings.dropbox_auth_finish"
			}
		)
...
def dropbox_auth_finish():
	...
	if not callback.state or not callback.code:
		.respond_as_web_page(
			...
			indicator_color="red",
			http_status_code=.AuthenticationError.http_status_code,
		)
		return
	...
	.local.response["location"] = "/app/dropbox-settings"
...
def set_dropbox_token(access_token, refresh_token=None):
	...
	dropbox_settings = .get_single("Dropbox Settings")
