...
class GoogleContacts(Document):
	...
	def validate(self):
		if not .db.get_single_value("Google Settings", "enable"):
			.throw(_("Enable Google API in Google Settings."))

	def get_access_token(self):
		if not self.refresh_token:
			button_label = .bold(_("Allow Google Contacts Access"))
...
def authorize_access(g_contact, reauthorize=False, code=None):
	...
	"""
	contact = .get_doc("Google Contacts", g_contact)
...
def get_google_contacts_object(g_contact):
	...
	"""
	account = .get_doc("Google Contacts", g_contact)
	oauth_obj = GoogleOAuth("contacts")
...
def sync(g_contact=None):
	...
	google_contacts = .get_list("Google Contacts", filters=filters)
...
def sync_contacts_from_google_contacts(g_contact):
	...
	.db.set_value("Google Contacts", account.name, "last_sync_on", .utils.now_datetime())

	for idx, connection in enumerate(results):
		.publish_realtime(
			"import_google_contacts", dict(progress=idx + 1, total=len(results)), user=.session.user
		)
		# Work-around to fix
		# https://github.com///issues/22648
...
def insert_contacts_to_google_contacts(doc, method=None):
	...
	"""
	if (
		not .db.exists("Google Contacts", {"name": doc.google_contacts})
	...
	try:
		...
		.db.set_value("Contact", doc.name, "google_contacts_id", contact.get("resourceName"))
	except HttpError as err:
		.msgprint(
...
def update_contacts_to_google_contacts(doc, method=None):
	...
	try:
		...
		.msgprint(_("Contact Synced with Google Contacts."))
	except HttpError as err:
		.msgprint(
