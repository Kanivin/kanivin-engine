...
.ui.form.on("Google Contacts", {
	refresh: function (frm) {
		...
		.realtime.on("import_google_contacts", (data) => {
			if (data.progress) {
		...
		if (frm.doc.refresh_token) {
			let sync_button = frm.add_custom_button(__("Sync Contacts"), function () {
				.show_alert({
					...
					.call({
						method: ".integrations.doctype.google_contacts.google_contacts.sync",
						args: {
	...
	authorize_google_contacts_access: function (frm) {
		.call({
			method: ".integrations.doctype.google_contacts.google_contacts.authorize_access",
			args: {
				g_contact: frm.doc.name,
