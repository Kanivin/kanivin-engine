...
import 
from  import _, safe_encode
from .model.document import Document
from .twofactor import authenticate_for_2factor, confirm_otp_token, should_run_2fa

if TYPE_CHECKING:
	from .core.doctype.user.user import User
...
class LDAPSettings(Document):
	...
	if TYPE_CHECKING:
		from .integrations.doctype.ldap_group_mapping.ldap_group_mapping import LDAPGroupMapping
	...
	def connect_to_ldap(self, base_dn, password, read_only=True) -> ldap3.Connection:
		try:
			...
			.throw(_("Invalid username or password"))
	...
	def get_ldap_client_settings() -> dict:
		...
		ldap = .get_cached_doc("LDAP Settings")
		if ldap.enabled:
			result["enabled"] = True
			result["method"] = ".integrations.doctype.ldap_settings.ldap_settings.login"
		return result
	...
	def create_or_update_user(self, user_data: dict, groups: list | None = None):
		...
		if .db.exists("User", user_data["email"]):
			user = .get_doc("User", user_data["email"])
		...
		if self.default_user_type == "System User":
			...
			role = .db.get_value("User Type", user.user_type, "role")
	...
	def authenticate(self, username: str, password: str):
		...
		try:
			...
			.throw(_("Please use a valid LDAP search filter"), title=_("Misconfigured"))
		...
		if conn.search(
			...
			if conn.entries and conn.entries[0]:
				...
				if conn.modify(entry_dn, changes=changes):
					...
					.msgprint(_("Password changed successfully."))
				else:
					.throw(_("Failed to change password."))
			else:
				.throw(_("No Entry for the User {0} found within LDAP!").format(user))
		else:
			.throw(_("No LDAP User found for email: {0}").format(user))
...
def login():
	# LDAP LOGIN LOGIC
	args = .form_dict
	ldap: LDAPSettings = .get_doc("LDAP Settings")

	user = ldap.authenticate(.as_unicode(args.usr), .as_unicode(args.pwd))

	.local.login_manager.user = user.name
	if should_run_2fa(user.name):
		authenticate_for_2factor(user.name)
		if not confirm_otp_token(.local.login_manager):
			return False

	.form_dict.pop("pwd", None)
	.local.login_manager.post_login()
