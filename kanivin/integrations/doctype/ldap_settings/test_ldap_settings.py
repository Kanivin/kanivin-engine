...
import 
from .exceptions import MandatoryError, ValidationError
from .integrations.doctype.ldap_settings.ldap_settings import LDAPSettings
...
class LDAP_TestCase:
	...
	def mock_ldap_connection(f):
		...
		def wrapped(self, *args, **kwargs):
			with mock.patch(
				".integrations.doctype.ldap_settings.ldap_settings.LDAPSettings.connect_to_ldap",
	...
	def clean_test_users():
		with contextlib.suppress(Exception):
			.get_doc("User", "posix.user1@unit.testing").delete()
		with contextlib.suppress(Exception):
			.get_doc("User", "posix.user2@unit.testing").delete()
		with contextlib.suppress(Exception):
			.get_doc("User", "website_ldap_user@test.com").delete()

	@classmethod
	def setUpClass(cls):
		cls.clean_test_users()
		# Save user data for restoration in tearDownClass()
		cls.user_ldap_settings = .get_doc("LDAP Settings")
	...
	def test_connect_to_ldap(self):
		...
		prevent_connection_parameters = {
			"mode": {
				"IP_V4_ONLY": "Locks the user to IPv4 without  providing a way to configure",
				"IP_V6_ONLY": "Locks the user to IPv6 without  providing a way to configure",
			},
			"auto_bind": {
	...
	def test_update_user_fields(self):
		...
		test_user = .get_doc("User", test_user_data["email"])
		self.test_class.update_user_fields(test_user, test_user_data)
		updated_user = .get_doc("User", test_user_data["email"])
		...
		self.assertIn(self.test_class.default_role, .get_roles(updated_user.name))
	...
	def test_create_website_user(self):
		...
		new_user = .get_doc("User", new_test_user_data["email"])
	...
	def test_sync_roles(self):
		...
		role_to_group_map = {
			...
			"All": "_default_all",
			"Guest": "_default_guest",
			"Desk User": "_default_desk_user",
		}

		# re-create user1 to ensure clean
		.get_doc("User", "posix.user1@unit.testing").delete()
		user = .get_doc(self.user1doc)
		user.insert(ignore_permissions=True)

		for test_user in test_user_data:
			test_user_doc = .get_doc("User", f"{test_user}@unit.testing")
			test_user_roles = .get_roles(f"{test_user}@unit.testing")
			...
			.get_doc("User", f"{test_user}@unit.testing")
			updated_user_roles = .get_roles(f"{test_user}@unit.testing")
	...
	def test_create_or_update_user(self):
		...
		.get_doc("User", f"{test_user}@unit.testing").delete()
		...
		with mock.patch(
			".integrations.doctype.ldap_settings.ldap_settings.LDAPSettings.update_user_fields"
		) as update_user_fields_method:
			with mock.patch(
				".integrations.doctype.ldap_settings.ldap_settings.LDAPSettings.sync_roles"
	...
	def test_authenticate(self):
		with mock.patch(
			".integrations.doctype.ldap_settings.ldap_settings.LDAPSettings.fetch_ldap_groups"
		...
		for username, password in enumerate(invalid_users):
			with self.assertRaises(.exceptions.ValidationError) as display_massage:
