...
from kanivin.integrations.offsite_backup_utils import (
...
class S3BackupSettings(Document):
	...
	def validate(self):
		...
		try:
			...
			bucket_name = kanivin.bold(self.bucket)
			if error_code == "403":
...
def take_backup():
	...
	enqueue(
		"kanivin.integrations.doctype.s3_backup_settings.s3_backup_settings.take_backups_s3",
		queue="long",
		timeout=1500,
	)
	kanivin.msgprint(_("Queued for backup. It may take a few minutes to an hour."))
...
def take_backups_if(freq):
	if cint(kanivin.db.get_single_value("S3 Backup Settings", "enabled")):
		if kanivin.db.get_single_value("S3 Backup Settings", "frequency") == freq:
			take_backups_s3()
...
def notify():
	error_message = kanivin.get_traceback()
	send_email(False, "Amazon S3", "S3 Backup Settings", "notify_email", error_message)


def backup_to_s3():
	from kanivin.utils import get_backups_path
	from kanivin.utils.backups import new_backup

	doc = kanivin.get_single("S3 Backup Settings")
	bucket = doc.bucket
	...
	if kanivin.flags.create_new_backup:
		backup = new_backup(
			ignore_files=False,
			backup_path_db=None,
