...
from .utils import get_backups_path, get_bench_path
...
class GoogleDrive(Document):
	...
	def get_access_token(self):
		if not self.refresh_token:
			button_label = .bold(_("Allow Google Drive Access"))
			raise .ValidationError(_("Click on {0} to generate Refresh Token.").format(button_label))
...
def authorize_access(reauthorize=False, code=None):
	...
	oauth_code = .db.get_single_value("Google Drive", "authorization_code") if not code else code
	oauth_obj = GoogleOAuth("drive")

	if not oauth_code or reauthorize:
		if reauthorize:
			.db.set_single_value("Google Drive", "backup_folder_id", "")
...
def check_for_folder_in_google_drive():
	...
	def _create_folder_in_google_drive(google_drive, account):
		...
		try:
			...
			.db.set_single_value("Google Drive", "backup_folder_id", folder.get("id"))
			.db.commit()
	...
	try:
		...
		.throw(_("Google Drive - Could not find folder in Google Drive - Error Code {0}").format(e))

	for f in google_drive_folders.get("files"):
		if f.get("name") == account.backup_folder_name:
			.db.set_single_value("Google Drive", "backup_folder_id", f.get("id"))
			.db.commit()
...
def take_backup():
	...
	enqueue(
		".integrations.doctype.google_drive.google_drive.upload_system_backup_to_google_drive",
		queue="long",
		timeout=1500,
	)
...
def upload_system_backup_to_google_drive():
	...
	for fileurl in file_urls:
		...
		try:
			...
			.throw(_("Google Drive - Could not locate - {0}").format(e))
	...
	.db.set_single_value("Google Drive", "last_backup_on", .utils.now_datetime())
	send_email(True, "Google Drive", "Google Drive", "email")
	return _("Google Drive Backup Successful.")


def daily_backup():
	drive_settings = .db.get_singles_dict("Google Drive", cast=True)
