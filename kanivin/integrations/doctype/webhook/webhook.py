...
from kanivin.utils.background_jobs import get_queues_timeout
from kanivin.utils.jinja import validate_template
from kanivin.utils.safe_exec import get_safe_globals

WEBHOOK_SECRET_HEADER = "X-Kanivin-Webhook-Signature"
...
class Webhook(Document):
	...
	if TYPE_CHECKING:
		from kanivin.integrations.doctype.webhook_data.webhook_data import WebhookData
		from kanivin.integrations.doctype.webhook_header.webhook_header import WebhookHeader
	...
	def on_update(self):
		kanivin.cache.delete_value("webhooks")

	def validate_docevent(self):
		if self.webhook_doctype:
			is_submittable = kanivin.get_value("DocType", self.webhook_doctype, "is_submittable")
			if not is_submittable and self.webhook_docevent in [
				"on_submit",
				"on_cancel",
				"on_update_after_submit",
			]:
				kanivin.throw(_("DocType must be Submittable for the selected Doc Event"))

	def validate_condition(self):
		temp_doc = kanivin.new_doc(self.webhook_doctype)
		if self.condition:
			try:
				kanivin.safe_eval(self.condition, eval_locals=get_context(temp_doc))
			except Exception as e:
				kanivin.throw(_("Invalid Condition: {}").format(e))
	...
	def validate_request_url(self):
		try:
			...
			kanivin.throw(_("Check Request URL"), exc=e)
	...
	def validate_repeating_fields(self):
		...
		if len(webhook_data) != len(set(webhook_data)):
			kanivin.throw(_("Same Field is entered more than once"))
	...
	def validate_secret(self):
		if self.enable_security:
			try:
				...
				kanivin.throw(_("Invalid Webhook Secret"))
	...
	def meets_condition(self):
		...
		try:
			doc = kanivin.get_cached_doc(self.webhook_doctype, self.preview_document)
			met_condition = kanivin.safe_eval(self.condition, eval_locals=get_context(doc))
		except Exception as e:
	...
	def preview_request_body(self):
		...
		try:
			doc = kanivin.get_cached_doc(self.webhook_doctype, self.preview_document)
			return kanivin.as_json(get_webhook_data(doc, self))
		except Exception as e:
			return _("Failed to compute request body: {}").format(e)


def get_context(doc):
	return {"doc": doc, "utils": get_safe_globals().get("kanivin").get("utils")}


def enqueue_webhook(doc, webhook) -> None:
	request_url = headers = data = r = None
	try:
		webhook: Webhook = kanivin.get_doc("Webhook", webhook.get("name"))
		request_url = webhook.request_url
		if webhook.is_dynamic_url:
			request_url = kanivin.render_template(webhook.request_url, get_context(doc))
		headers = get_webhook_headers(doc, webhook)
		data = get_webhook_data(doc, webhook)

	except Exception as e:
		kanivin.logger().debug({"enqueue_webhook_error": e})
	...
	for i in range(3):
		try:
			...
			r.raise_for_status()
			kanivin.logger().debug({"webhook_success": r.text})
			log_request(webhook.name, doc.name, request_url, headers, data, r)
			break

		except requests.exceptions.ReadTimeout as e:
			kanivin.logger().debug({"webhook_error": e, "try": i + 1})
			log_request(webhook.name, doc.name, request_url, headers, data)
...
def log_request(
	...
	request_log = kanivin.get_doc(
		{
			"doctype": "Webhook Request Log",
			"webhook": webhook,
			"reference_document": docname,
			"user": kanivin.session.user if kanivin.session.user else None,
			"url": url,
			"headers": kanivin.as_json(headers) if headers else None,
			"data": kanivin.as_json(data) if data else None,
			"response": res.text if res is not None else None,
			"error": kanivin.get_traceback(),
		}
	)
...
def get_webhook_data(doc, webhook):
	...
	if webhook.webhook_data:
		...
		data = kanivin.render_template(webhook.webhook_json, get_context(doc))
