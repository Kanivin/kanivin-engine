...
def run_webhooks(doc, method):
	...
	kanivin_flags = kanivin.local.flags

	if kanivin_flags.in_import or kanivin_flags.in_patch or kanivin_flags.in_install or kanivin_flags.in_migrate:
		return

	# load all webhooks from cache / DB
	webhooks = kanivin.cache.get_value("webhooks", get_all_webhooks)
	...
	from kanivin.integrations.doctype.webhook.webhook import get_context
	...
	for webhook in webhooks_for_doc:
		...
		elif kanivin.safe_eval(webhook.condition, eval_locals=get_context(doc)):
			trigger_webhook = True
...
def _add_webhook_to_queue(webhook, doc):
	...
	if not getattr(kanivin.local, "_webhook_queue", None):
		kanivin.local._webhook_queue = []
		kanivin.db.after_commit.add(flush_webhook_execution_queue)

	kanivin.local._webhook_queue.append(kanivin._dict(doc=doc, webhook=webhook))
...
def flush_webhook_execution_queue():
	...
	# reverse
	kanivin.local._webhook_queue.reverse()

	# deduplicate on (doc.name, webhook.name)
	# 'doc' holds the last instance values
	for execution in kanivin.local._webhook_queue:
	...
	del kanivin.local._webhook_queue
	...
	for instance in unique_last_instances:
		kanivin.enqueue(
			"kanivin.integrations.doctype.webhook.webhook.enqueue_webhook",
			doc=instance.doc,
