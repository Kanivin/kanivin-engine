...
import kanivin
from kanivin.integrations.doctype.webhook import flush_webhook_execution_queue
from kanivin.integrations.doctype.webhook.webhook import (
	enqueue_webhook,
	get_webhook_data,
	get_webhook_headers,
)
...
def get_test_webhook(config):
	wh = kanivin.get_doc(config)
	if not wh.name:
		wh.name = kanivin.generate_hash()
	wh.insert()
	wh.reload()
	try:
		yield wh
...
class TestWebhook(KanivinTestCase):
	@classmethod
	def setUpClass(cls):
		# delete any existing webhooks
		kanivin.db.delete("Webhook")
		# Delete existing logs if any
		kanivin.db.delete("Webhook Request Log")
		super().setUpClass()
	...
	def setUp(self):
		...
		if kanivin.db.exists("Webhook", webhook_fields):
			self.webhook = kanivin.get_doc("Webhook", webhook_fields)
		else:
			self.webhook = kanivin.new_doc("Webhook")
			self.webhook.update(webhook_fields)

		# create a User document
		self.user = kanivin.new_doc("User")
		self.user.first_name = kanivin.mock("name")
		self.user.email = kanivin.mock("email")
		self.user.save()

		# Create another test user specific to this test
		self.test_user = kanivin.new_doc("User")
	...
	def test_webhook_trigger_with_enabled_webhooks(self):
		...
		webhooks = kanivin.cache.get_value("webhooks")
		self.assertTrue("User" in webhooks)
		self.assertEqual(len(webhooks.get("User")), 1)

		# only 1 hook (enabled) must be queued
		self.assertEqual(len(kanivin.local._webhook_queue), 1)
		execution = kanivin.local._webhook_queue[0]
	...
	def test_validate_doc_events(self):
		...
		self.assertRaises(kanivin.ValidationError, self.webhook.save)
	...
	def test_webhook_req_log_creation(self):
		...
		if not kanivin.db.get_value("User", "user2@integration.webhooks.test.com"):
			user = kanivin.get_doc(
				{"doctype": "User", "email": "user2@integration.webhooks.test.com", "first_name": "user2"}
			).insert()
		else:
			user = kanivin.get_doc("User", "user2@integration.webhooks.test.com")

		webhook = kanivin.get_doc("Webhook", {"webhook_doctype": "User"})
		enqueue_webhook(user, webhook)

		self.assertTrue(kanivin.get_all("Webhook Request Log", pluck="name"))
	...
	def test_webhook_with_array_body(self):
		...
		doc = kanivin.new_doc("Note")
		doc.title = "Test Webhook Note"
		final_title = kanivin.generate_hash()
		...
		with get_test_webhook(wh_config):
			...
			log = kanivin.get_last_doc("Webhook Request Log")
