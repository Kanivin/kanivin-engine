...
kanivin.webhook = {
	set_fieldname_select: (frm) => {
		if (frm.doc.webhook_doctype) {
			kanivin.model.with_doctype(frm.doc.webhook_doctype, () => {
				// get doctype fields
				let fields = $.map(
					kanivin.get_doc("DocType", frm.doc.webhook_doctype).fields,
					(d) => {
						if (
							kanivin.model.no_value_type.includes(d.fieldtype) &&
							!kanivin.model.table_fields.includes(d.fieldtype)
						) {
							return null;
						} else {
							return {
				...
				// add meta fields
				for (let field of kanivin.model.std_fields) {
...
kanivin.ui.form.on("Webhook", {
	refresh: (frm) => {
		kanivin.webhook.set_fieldname_select(frm);
		frm.set_query(
			"background_jobs_queue",
			"kanivin.integrations.doctype.webhook.webhook.get_all_queues"
		);
	},

	request_structure: (frm) => {
		kanivin.webhook.set_request_headers(frm);
	},
...
kanivin.ui.form.on("Webhook Data", {
	fieldname: (frm, cdt, cdn) => {
		...
		if (!df.length) {
			...
			df = kanivin.model.std_fields.filter((field) => field.fieldname == row.fieldname);
		}
