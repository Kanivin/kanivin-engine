...
"""
Kanivin - Low Code Open Source Framework in Python and JS

Kanivin, pronounced fra-pay, is a full stack, batteries-included, web
...
Read the documentation: https://kanivinframework.com/docs
"""
...
import kanivin
from kanivin.query_builder import (
	get_query,
	get_query_builder,
	patch_query_aggregation,
	patch_query_execute,
)
from kanivin.utils.caching import request_cache
from kanivin.utils.data import cint, cstr, sbool

# Local application imports
from .exceptions import *
from .types.kanivindict import _dict
...
if TYPE_CHECKING:  # pragma: no cover
	...
	from kanivin.database.mariadb.database import MariaDBDatabase
	from kanivin.database.postgres.database import PostgresDatabase
	from kanivin.email.doctype.email_queue.email_queue import EmailQueue
	from kanivin.model.document import Document
	from kanivin.query_builder.builder import MariaDB, Postgres
	from kanivin.utils.redis_wrapper import RedisWrapper
...
def _(msg: str, lang: str | None = None, context: str | None = None) -> str:
	...
	"""
	from kanivin.translate import get_all_translations
	from kanivin.utils import is_html, strip_html_tags
...
def set_user_lang(user: str, user_language: str | None = None) -> None:
	"""Guess and set user language for the session. `kanivin.local.lang`"""
	from kanivin.translate import get_user_lang
...
def init(site: str, sites_path: str = ".", new_site: bool = False, force=False) -> None:
	"""Initialize kanivin for the current site. Reset thread locals `kanivin.local`"""
	...
	setup_module_map(include_all_apps=not (kanivin.request or kanivin.job or kanivin.flags.in_migrate))
...
def connect(site: str | None = None, db_name: str | None = None, set_admin_as_user: bool = True) -> None:
	...
	:param site: If site is given, calls `kanivin.init`.
	:param db_name: Optional. Will use from `site_config.json`.
	:param set_admin_as_user: Set Administrator as current user.
	"""
	from kanivin.database import get_db
...
def setup_redis_cache_connection():
	"""Defines `kanivin.cache` as `RedisWrapper` instance"""
	global cache

	if not cache:
		from kanivin.utils.redis_wrapper import setup_cache
...
def get_traceback(with_context: bool = False) -> str:
	...
	from kanivin.utils import get_traceback
	...
	*,
	...
	def _raise_exception():
		if raise_exception:
			...
			if out.__kanivin_exc_id:
				exc.__kanivin_exc_id = out.__kanivin_exc_id
			raise exc
	...
	if raise_exception:
		...
		out.__kanivin_exc_id = generate_hash()
...
def throw(
	...
	:param msg: Message.
	:param exc: Exception class. Default `kanivin.ValidationError`
	:param title: [optional] Message title. Default: "Message".
	:param is_minimizable: [optional] Allow users to minimize the modal
	:param wide: [optional] Show wide modal
	:param as_list: [optional] If `msg` is a list, render as un-ordered list.
...
def create_folder(path, with_init=False):
	...
	from kanivin.utils import touch_file
...
def get_user():
	from kanivin.utils.user import UserPermissions
...
def get_roles(username=None) -> list[str]:
	...
	return kanivin.permissions.get_roles(username or local.session.user)
...
def sendmail(
	...
	if as_markdown:
		from kanivin.utils import md_to_html
	...
	from kanivin.email.doctype.email_queue.email_queue import QueueBuilder
...
def whitelist(allow_guest=False, xss_safe=False, methods=None):
	...
	def innerfn(fn):
		from kanivin.utils.typing_validations import validate_argument_types
...
def read_only():
	def innfn(fn):
		...
		def wrapper_fn(*args, **kwargs):
			# kanivin.read_only could be called from nested functions, in such cases don't swap the
			# connection again.
...
def only_for(roles: list[str] | tuple[str] | str, message=False):
	"""
	Raises `kanivin.PermissionError` if the user does not have any of the permitted roles.
...
def clear_cache(user: str | None = None, doctype: str | None = None):
	...
	import kanivin.utils.caching
	from kanivin.website.router import clear_routing_cache

	if doctype:
		kanivin.cache_manager.clear_doctype_cache(doctype)
		reset_metadata_version()
	elif user:
		kanivin.cache_manager.clear_user_cache(user)
	else:  # everything
		# Delete ALL keys associated with this site.
		keys_to_delete = set(kanivin.cache.get_keys(""))
		for key in kanivin.get_hooks("persistent_cache_keys"):
			keys_to_delete.difference_update(kanivin.cache.get_keys(key))
		kanivin.cache.delete_value(list(keys_to_delete), make_keys=False)
	...
	kanivin.utils.caching._SITE_CACHE.clear()
	local.role_permissions = {}
...
def only_has_select_perm(doctype, user=None, ignore_permissions=False):
	...
	from kanivin.permissions import get_role_permissions
...
def has_permission(
	...
	*,
	...
	Raises `kanivin.PermissionError` if user isn't permitted and `throw` is truthy
	...
	out = kanivin.permissions.has_permission(
		doctype,
		ptype,
		doc=doc,
		user=user,
	...
	if throw and not out:
		if doc:
			kanivin.permissions.check_doctype_permission(doctype, ptype)

		document_label = f"{_(doctype)} {doc if isinstance(doc, str) else doc.name}" if doc else _(doctype)
		kanivin.flags.error_message = _("No permission for {0}").format(document_label)
...
def has_website_permission(doc=None, ptype="read", user=None, verbose=False, doctype=None):
	"""Raises `kanivin.PermissionError` if not permitted.
...
def get_precision(
	...
	from kanivin.model.meta import get_field_precision
...
def new_doc(
	...
	**kwargs,
	...
	from kanivin.model.create_new import get_new_doc
...
def set_value(doctype, docname, fieldname, value=None):
	"""Set document value. Calls `kanivin.client.set_value`"""
	import kanivin.client

	return kanivin.client.set_value(doctype, docname, fieldname, value)
...
def get_doc(**kwargs: dict) -> "_NewDocument":
	...
	Not recommended. Use `kanivin.new_doc` instead."""
	pass
...
def get_doc(*args, **kwargs):
	"""Return a `kanivin.model.document.Document` object of the given type and name.
	...
	Examples:
	        ...
	        todo = kanivin.get_doc({"doctype":"ToDo", "description": "test"})
	        todo.insert()

	        # open an existing document
	        todo = kanivin.get_doc("ToDo", "TD0001")

	"""
	import kanivin.model.document

	return kanivin.model.document.get_doc(*args, **kwargs)
...
def get_single(doctype):
	"""Return a `kanivin.model.document.Document` object of the given Single doctype."""
	return get_doc(doctype, doctype)


def get_meta(doctype, cached=True):
	"""Get `kanivin.model.meta.Meta` instance of given doctype name."""
	import kanivin.model.meta

	return kanivin.model.meta.get_meta(doctype, cached=cached)


def get_meta_module(doctype):
	import kanivin.modules

	return kanivin.modules.load_doctype_module(doctype)
...
def delete_doc(
	...
	"""Delete a document. Calls `kanivin.model.delete_doc.delete_doc`.
	...
	import kanivin.model.delete_doc

	return kanivin.model.delete_doc.delete_doc(
		doctype,
		name,
		force,
		ignore_doctypes,
...
def reload_doc(
	...
	return kanivin.modules.reload_doc(module, dt, dn, force=force, reset_permissions=reset_permissions)
...
def rename_doc(
	...
	*,
	...
	Calls `kanivin.model.rename_doc.rename_doc`
	"""
...
def get_file_items(path, raise_not_found=False, ignore_empty_lines=True):
	...
	if content:
		content = kanivin.utils.strip(content)
...
def import_doc(path):
	...
	from kanivin.core.doctype.data_import.data_import import import_doc
...
def redirect(url):
	...
	from kanivin.exceptions import Redirect
...
def redirect_to_message(title, html, http_status_code=None, context=None, indicator_color=None):
	...
	Example Usage:
	        kanivin.redirect_to_message(_('Thank you'), "<div><p>You will receive an email at test@example.com</p></div>")
...
def build_match_conditions(doctype, as_condition=True):
	...
	import kanivin.desk.reportview

	return kanivin.desk.reportview.build_match_conditions(doctype, as_condition=as_condition)


def get_list(doctype, *args, **kwargs):
	"""List database query via `kanivin.model.db_query`. Will also check for permissions.
	...
	Example usage:
	        ...
	        # simple dict filter
	        kanivin.get_list("ToDo", fields=["name", "description"], filters = {"owner":"test@example.com"})

	        # filter as a list of lists
	        kanivin.get_list("ToDo", fields="*", filters = [["modified", ">", "2014-01-01"]])
	"""
	import kanivin.model.db_query

	return kanivin.model.db_query.DatabaseQuery(doctype).execute(*args, **kwargs)


def get_all(doctype, *args, **kwargs):
	"""List database query via `kanivin.model.db_query`. Will **not** check for permissions.
	Parameters are same as `kanivin.get_list`
	...
	Example usage:
	        ...
	        # simple dict filter
	        kanivin.get_all("ToDo", fields=["name", "description"], filters = {"owner":"test@example.com"})

	        # filter as a list of lists
	        kanivin.get_all("ToDo", fields=["*"], filters = [["modified", ">", "2014-01-01"]])
	"""
...
def get_value(*args, **kwargs):
	...
	Alias for `kanivin.db.get_value`
...
def get_test_records(doctype):
	...
	from kanivin.modules import get_doctype_module, get_module_path
...
def format_value(*args, **kwargs):
	...
	import kanivin.utils.formatters

	return kanivin.utils.formatters.format_value(*args, **kwargs)
...
def attach_print(
	...
	from kanivin.translate import print_language
	from kanivin.utils import scrub_urls
	from kanivin.utils.pdf import get_pdf
...
def publish_progress(*args, **kwargs):
	...
	"""
	import kanivin.realtime

	return kanivin.realtime.publish_progress(*args, **kwargs)
...
def publish_realtime(*args, **kwargs):
	...
	return kanivin.realtime.publish_realtime(*args, **kwargs)


def local_cache(namespace, key, generator, regenerate_if_none=False):
	"""A key value store for caching within a request

	:param namespace: kanivin.local.cache[namespace]
	:param key: kanivin.local.cache[namespace][key] used to retrieve value
...
def enqueue(*args, **kwargs):
	...
	"""
	import kanivin.utils.background_jobs

	return kanivin.utils.background_jobs.enqueue(*args, **kwargs)
...
def enqueue_doc(*args, **kwargs):
	...
	return kanivin.utils.background_jobs.enqueue_doc(*args, **kwargs)
...
def logger(module=None, with_more_info=False, allow_site=True, filter=None, max_size=100_000, file_count=20):
	...
	from kanivin.utils.logger import get_logger
...
def safe_eval(code, eval_globals=None, eval_locals=None):
	...
	from kanivin.utils.safe_exec import safe_eval
...
def get_active_domains():
	from kanivin.core.doctype.domain_settings.domain_settings import get_active_domains
...
def get_version(doctype, name, limit=None, head=False, raise_err=True):
	...
	Example
	>>> kanivin.get_version("User", "foobar@gmail.com")
	>>>
	[
	        {
	...
	if meta.track_changes:
		...
		from kanivin.utils import dictify, safe_json_loads, squashify
...
def parse_json(val):
	from kanivin.utils import parse_json
...
def mock(type, size=1, locale="en"):
	...
	from kanivin.utils import squashify
...
def validate_and_sanitize_search_inputs(fn):
	...
	def wrapper(*args, **kwargs):
		from kanivin.desk.search import sanitize_searchfield
...
from kanivin.utils.error import log_error
from kanivin.utils.print_utils import get_print

if _tune_gc:
	# generational GC gets triggered after certain allocs (g0) which is 700 by default.
	# This number is quite small for kanivin where a single query can potentially create 700+
	# objects easily.
