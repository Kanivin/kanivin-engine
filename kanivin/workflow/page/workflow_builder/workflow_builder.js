kanivin.pages["workflow-builder"].on_page_load = function (wrapper) {
	...
	if (kanivin.boot.developer_mode) {
		...
		kanivin.hot_update.push(() => load_workflow_builder(wrapper));
	}
};

kanivin.pages["workflow-builder"].on_page_show = function (wrapper) {
...
function load_workflow_builder(wrapper) {
	...
	if (route.length > 1) {
		kanivin.require("workflow_builder.bundle.js").then(() => {
			kanivin.workflow_builder = new kanivin.ui.WorkflowBuilder({
				wrapper: $parent,
				page: wrapper.page,
				workflow: route[1],
			});
		...
		let d = new kanivin.ui.Dialog({
			...
			primary_action({ action, doctype, workflow, workflow_name }) {
				if (action === "Edit") {
					kanivin.set_route("workflow-builder", workflow);
