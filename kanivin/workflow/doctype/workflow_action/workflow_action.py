...
from kanivin.desk.form.utils import get_pdf_link
from kanivin.desk.notifications import clear_doctype_notifications
...
from kanivin.model.workflow import (
	apply_workflow,
	get_workflow_name,
...
from kanivin.utils import get_datetime, get_url
from kanivin.utils.background_jobs import enqueue
from kanivin.utils.data import get_link_to_form
from kanivin.utils.user import get_users_with_role
...
class WorkflowAction(Document):
	...
	if TYPE_CHECKING:
		...
		from kanivin.workflow.doctype.workflow_action_permitted_role.workflow_action_permitted_role import (
...
def on_doctype_update():
	...
	kanivin.db.add_index("Workflow Action", ["reference_name", "reference_doctype", "status"])
...
def get_permission_query_conditions(user):
	...
	permitted_workflow_actions = (
		kanivin.qb.from_(WorkflowAction)
	...
	return f"""(`tabWorkflow Action`.`name` in ({permitted_workflow_actions})
		or `tabWorkflow Action`.`user`={kanivin.db.escape(user)})
...
def has_permission(doc, user):
	...
	return not permitted_roles.isdisjoint(kanivin.get_roles(user))
...
def confirm_action(doctype, docname, user, action):
	...
	logged_in_user = kanivin.session.user
	...
	if logged_in_user == "Guest":
		kanivin.set_user(logged_in_user)
...
def return_success_page(doc):
	kanivin.respond_as_web_page(
		...
		_("{0}: {1} is set to state {2}").format(
			doc.get("doctype"), kanivin.bold(doc.get("name")), kanivin.bold(get_doc_workflow_state(doc))
		),
...
def return_link_expired_page(doc, doc_workflow_state):
	kanivin.respond_as_web_page(
		...
		_("Document {0} has been set to state {1} by {2}").format(
			kanivin.bold(doc.get("name")),
			kanivin.bold(doc_workflow_state),
			kanivin.bold(kanivin.get_value("User", doc.get("modified_by"), "full_name")),
		),
		indicator_color="blue",
	)
...
def get_allowed_roles(user, workflow, workflow_state):
	user = user if user else kanivin.session.user

	allowed_roles = kanivin.get_all(
		"Workflow Transition",
		fields="allowed",
	...
	user_roles = set(kanivin.get_roles(user))
...
def update_completed_workflow_actions_using_role(user=None, workflow_action=None):
	...
	(
		kanivin.qb.update(WorkflowAction)
...
def clear_old_workflow_actions_using_user(doc, user=None):
	...
	if kanivin.db.has_column("Workflow Action", "user"):
		kanivin.db.delete(
			"Workflow Action",
			{
...
def get_users_next_action_data(transitions, doc):
	...
	@kanivin.request_cache
	def user_has_permission(user: str) -> bool:
		from kanivin.permissions import has_permission
	...
	for transition in transitions:
		...
		for user in filtered_users:
			if not user_data_map.get(user):
				user_data_map[user] = kanivin._dict(
					{
						"possible_actions": [],
						"email": kanivin.db.get_value("User", user, "email"),
					}
				)
...
def create_workflow_actions_for_roles(roles, doc):
	if not roles:
		return
	workflow_action = kanivin.get_doc(
		{
...
def get_workflow_action_url(action, doc, user):
	apply_action_method = "/api/method/kanivin.workflow.doctype.workflow_action.workflow_action.apply_action"
...
def get_confirm_workflow_action_url(doc, action, user):
	confirm_action_method = (
		"/api/method/kanivin.workflow.doctype.workflow_action.workflow_action.confirm_action"
	)
...
def get_common_email_args(doc):
	...
	lang = doc.get("language") or (
		kanivin.get_cached_value("Print Format", print_format, "default_print_language")
		if print_format
		else None
	)
...
def get_email_template_from_workflow(doc):
	...
	template_name = kanivin.db.get_value(
...
def get_state_optional_field_value(workflow_name, state):
	return kanivin.get_cached_value(
