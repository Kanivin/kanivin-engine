kanivin.provide("kanivin.core");

kanivin.ui.form.on("Workflow", {
	onload: function (frm) {
	...
	refresh: function (frm) {
		...
		let message = `
		<div class="flex">
			<div class="mr-3"><img style="border-radius: var(--border-radius-md)" width="600" src="/assets/kanivin/images/workflow-builder.gif"></div>
			<div>
				...
				<div>
					<a class="btn btn-primary btn-sm" href="${workflow_builder_url}">
						${__("Workflow Builder")} ${kanivin.utils.icon("right", "xs")}
					</a>
				</div>
			</div>
		</div>
		...
		if (frm.doc.document_type) {
			frm.add_custom_button(__("Go to {0} List", [frm.doc.document_type]), () => {
				kanivin.set_route("List", frm.doc.document_type);
			});
		}
	...
	update_field_options: function (frm) {
		...
		kanivin.model.with_doctype(doc.document_type, () => {
			const fieldnames = kanivin
				.get_meta(doc.document_type)
				.fields.filter((field) => !kanivin.model.no_value_type.includes(field.fieldtype))
	...
	set_table_html: function (frm) {
		const promises = frm.states.map((r) => {
			...
			return kanivin.utils.get_indicator_color(state).then((color) => {
				return `<tr>
				<td>
	...
	get_orphaned_states_and_count: function (frm) {
		...
		return kanivin
			.xcall("kanivin.workflow.doctype.workflow.workflow.get_workflow_state_count", {
		...
		if (frm.states && frm.states.length) {
			...
			$(frm.state_table)
				...
				.on("click", (e) => {
					...
					kanivin.set_route("List", frm.doc.document_type, filters);
				});
		}
	},
});

kanivin.ui.form.on("Workflow Document State", {
...
kanivin.ui.form.on("Workflow Transition", {
	state: function (_, cdt, cdn) {
		var row = locals[cdt][cdn];
		delete row.workflow_builder_id;
	},
