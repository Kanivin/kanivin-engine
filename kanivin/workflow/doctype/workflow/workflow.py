...
class Workflow(Document):
	...
	if TYPE_CHECKING:
		...
		from kanivin.workflow.doctype.workflow_document_state.workflow_document_state import (
			WorkflowDocumentState,
		)
		from kanivin.workflow.doctype.workflow_transition.workflow_transition import WorkflowTransition
	...
	def on_update(self):
		kanivin.clear_cache(doctype=self.document_type)

	def create_custom_field_for_workflow_state(self):
		kanivin.clear_cache(doctype=self.document_type)
		meta = kanivin.get_meta(self.document_type)
	...
	def validate_docstatus(self):
		def get_state(state):
			...
			kanivin.throw(kanivin._("{0} not a valid State").format(state))
		...
		for t in self.transitions:
			...
			if state.doc_status == "2":
				kanivin.throw(
					kanivin._("Cannot change state of Cancelled Document. Transition row {0}").format(t.idx)
				)
			...
			if state.doc_status == "0" and next_state.doc_status == "2":
				kanivin.throw(kanivin._("Cannot cancel before submitting. See Transition {0}").format(t.idx))
...
def get_workflow_state_count(doctype, workflow_state_field, states):
	kanivin.has_permission(doctype=doctype, ptype="read", throw=True)
	states = kanivin.parse_json(states)

	if workflow_state_field in kanivin.get_meta(doctype).get_valid_columns():
