...
class TestWorkflow(KanivinTestCase):
	@classmethod
	def setUpClass(cls):
		super().setUpClass()
	...
	def setUp(self):
		self.patcher = patch("kanivin.attach_print", return_value={})
		self.patcher.start()
		kanivin.db.delete("Workflow Action")
		self.workflow = create_todo_workflow()

	def tearDown(self):
		kanivin.set_user("Administrator")
		self.patcher.stop()
		kanivin.delete_doc("Workflow", "Test ToDo")
	...
	def test_if_workflow_actions_were_processed_using_role(self):
		...
		workflow_actions = kanivin.get_all("Workflow Action", fields=["*"])
	...
	def test_if_workflow_actions_were_processed_using_user(self):
		...
		kanivin.qb.update(WorkflowAction).set(WorkflowAction.user, "test2@example.com").run()
		kanivin.qb.update(WorkflowActionPermittedRole).set(WorkflowActionPermittedRole.role, "").run()

		self.test_approve(doc)

		user.remove_roles("Test Approver", "System Manager")
		workflow_actions = kanivin.get_all("Workflow Action", fields=["status"])
		self.assertEqual(len(workflow_actions), 1)
		self.assertEqual(workflow_actions[0].status, "Completed")
		kanivin.set_user("Administrator")
...
def create_todo_workflow():
	from kanivin.tests.ui_test_helpers import UI_TEST_USER

	if kanivin.db.exists("Workflow", "Test ToDo"):
		kanivin.delete_doc("Workflow", "Test ToDo")

	TEST_ROLE = "Test Approver"

	if not kanivin.db.exists("Role", TEST_ROLE):
		kanivin.get_doc(dict(doctype="Role", role_name=TEST_ROLE)).insert(ignore_if_duplicate=True)
		if kanivin.db.exists("User", UI_TEST_USER):
			kanivin.get_doc("User", UI_TEST_USER).add_roles(TEST_ROLE)

	workflow = kanivin.new_doc("Workflow")
...
def create_new_todo():
	return kanivin.get_doc(dict(doctype="ToDo", description="workflow " + random_string(10))).insert()
