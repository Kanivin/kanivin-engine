...
def new_catalog(app: str, locale: str | None = None) -> Catalog:
	def get_hook(hook, app):
		return kanivin.get_hooks(hook, [None], app)[0]
...
def get_po_dir(app: str) -> Path:
	return Path(kanivin.get_app_path(app)) / PO_DIR
...
def get_method_map(app: str):
	file_path = Path(kanivin.get_app_path(app)).parent / "babel_extractors.csv"
	if file_path.exists():
...
def generate_pot(target_app: str | None = None):
	...
	apps = [target_app] if target_app else kanivin.get_all_apps(True)
	default_method_map = get_method_map("kanivin")

	keywords = DEFAULT_KEYWORDS.copy()
	keywords["_lt"] = None

	for app in apps:
		app_path = kanivin.get_pymodule_path(app, "..")
		...
		method_map = [] if app == "kanivin" else get_method_map(app)
...
def get_is_gitignored_function_for_app(app: str | None):
	...
	repo = git.Repo(kanivin.get_app_source_path(app), search_parent_directories=True)
...
def new_po(locale, target_app: str | None = None):
	...
	for app in apps:
		...
		print(
			"You will need to add the language in kanivin/geo/languages.csv, if you haven't done it already."
		)
...
def migrate(app: str | None = None, locale: str | None = None):
	apps = [app] if app else kanivin.get_all_apps(True)
...
def csv_to_po(app: str, locale: str):
	csv_file = Path(kanivin.get_app_path(app)) / "translations" / f"{locale.replace('_', '-')}.csv"
...
def get_translations_from_mo(lang, app):
	...
	translations = {}
	lang = lang.replace("-", "_")  # Kanivin uses dash, babel uses underscore.
