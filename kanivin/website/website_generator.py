...
from kanivin.modules import get_module_name
from kanivin.search.website_search import remove_document_from_index, update_index_for_path
from kanivin.website.utils import cleanup_page_name, clear_cache


class WebsiteGenerator(Document):
	website = kanivin._dict()
	...
	def on_trash(self):
		...
		if self.allow_website_search_indexing():
			kanivin.enqueue(remove_document_from_index, path=self.route, enqueue_after_commit=True)
	...
	def send_indexing_request(self, operation_type="URL_UPDATED"):
		...
		if (
			kanivin.db.get_single_value("Website Settings", "enable_google_indexing")
			and self.is_website_published()
			and self.meta.allow_guest_to_view
		):
			url = kanivin.utils.get_url(self.route)
			kanivin.enqueue(
				"kanivin.website.doctype.website_settings.google_indexing.publish_site",
				url=url,
	...
	def remove_old_route_from_index(self):
		...
		if self.allow_website_search_indexing() or kanivin.flags.in_test:
			return
	...
	def update_website_search_index(self):
		...
		"""
		if not self.allow_website_search_indexing() or kanivin.flags.in_test:
			return

		if self.is_website_published():
			kanivin.enqueue(update_index_for_path, path=self.route, enqueue_after_commit=True)
		elif self.route:
