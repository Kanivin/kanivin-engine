...
kanivin.query_reports["Website Analytics"] = {
	filters: [
		{
			fieldname: "from_date",
			label: __("From Date"),
			fieldtype: "Date",
			default: kanivin.datetime.add_days(kanivin.datetime.now_date(true), -100),
		},
		{
			fieldname: "to_date",
			label: __("To Date"),
			fieldtype: "Date",
			default: kanivin.datetime.now_date(true),
		},
		{
			fieldname: "range",
			label: __("Range"),
	...
	formatter: function (value, row, column, data, default_formatter) {
		if (
			kanivin.query_report.get_filter_value("group_by") === "source" &&
			column.id === "source"
		) {
			if (value) {
				try {
					...
					return kanivin.utils.get_form_link(doctype, name, true, value);
				} catch (e) {
