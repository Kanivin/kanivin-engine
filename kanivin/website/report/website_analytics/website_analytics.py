...
import kanivin
from kanivin.query_builder.functions import Coalesce, Count
...
class WebsiteAnalytics:
	def __init__(self, filters=None):
		self.filters = kanivin._dict(filters or {})

		if not self.filters.to_date:
			self.filters.to_date = datetime.now()

		if not self.filters.from_date:
			self.filters.from_date = kanivin.utils.add_days(self.filters.to_date, -7)

		if not self.filters.range:
			self.filters.range = "Daily"

		self.filters.to_date = kanivin.utils.add_days(self.filters.to_date, 1)
	...
	def get_columns(self):
		meta = kanivin.get_meta("Web Page View")
	...
	def get_data(self):
		WebPageView = kanivin.qb.DocType("Web Page View")
		count_all = Count("*").as_("count")
		case = kanivin.qb.terms.Case().when(WebPageView.is_unique == "1", "1")
		count_is_unique = Count(case).as_("unique_count")

		return (
			kanivin.qb.from_(WebPageView)
			...
			.orderby("count", order=kanivin.qb.desc)
		).run()
	...
	def get_chart_data(self):
		current_dialect = kanivin.db.db_type or "mariadb"
		...
		self.chart_data = kanivin.db.sql(query, values=values, as_dict=1)
	...
	def prepare_chart_data(self, data):
		...
		if self.filters.range == "Monthly":
			date_range = [kanivin.utils.add_days(dd, 1) for dd in date_range]
