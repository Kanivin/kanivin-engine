...
from kanivin.website.utils import (
	clear_cache,
	find_first_image,
	get_comment_list,
...
class BlogPost(WebsiteGenerator):
	...
	def make_route(self):
		if not self.route:
			return (
				kanivin.db.get_value("Blog Category", self.blog_category, "route")
				+ "/"
	...
	def validate(self):
		...
		if self.featured:
			if not self.meta_image:
				kanivin.throw(_("A featured post must have a cover image"))
			self.reset_featured_for_other_blogs()

		self.set_read_time()

	def reset_featured_for_other_blogs(self):
		all_posts = kanivin.get_all("Blog Post", {"featured": 1})
		for post in all_posts:
			kanivin.db.set_value("Blog Post", post.name, "featured", 0)
	...
	def get_context(self, context):
		...
		context.enable_cta = not self.hide_cta and kanivin.db.get_single_value(
			"Blog Settings", "show_cta_in_blog", cache=True
		)

		if self.blogger:
			context.blogger_info = kanivin.get_doc("Blogger", self.blogger).as_dict()
		...
		context.category = kanivin.db.get_value(
		...
		context.guest_allowed = kanivin.db.get_single_value("Blog Settings", "allow_guest_to_comment")

	def fetch_cta(self):
		if kanivin.db.get_single_value("Blog Settings", "show_cta_in_blog", cache=True):
			blog_settings = kanivin.get_cached_doc("Blog Settings")
	...
	def fetch_social_links_info(self):
		if not kanivin.db.get_single_value("Blog Settings", "enable_social_sharing", cache=True):
			return []

		url = kanivin.local.site + "/" + self.route
	...
	def load_likes(self, context):
		...
		context.like_count = kanivin.db.count("Comment", filters) or 0

		filters["comment_email"] = user

		if user == "Guest":
			filters["ip_address"] = kanivin.local.request_ip

		context.like = kanivin.db.count("Comment", filters) or 0
...
def get_list_context(context=None):
	...
	blog_settings = kanivin.get_doc("Blog Settings").as_dict(no_default_fields=True)
	list_context.update(blog_settings)

	category_name = kanivin.utils.escape_html(
		kanivin.local.form_dict.blog_category or kanivin.local.form_dict.category
	)
	if category_name:
		category = kanivin.get_doc("Blog Category", category_name)
	...
	elif kanivin.local.form_dict.blogger:
		blogger = kanivin.db.get_value("Blogger", {"name": kanivin.local.form_dict.blogger}, "full_name")
		list_context.sub_title = _("Posts by {0}").format(blogger)
		list_context.title = blogger

	elif kanivin.local.form_dict.txt:
		list_context.sub_title = _('Filtered by "{0}"').format(sanitize_html(kanivin.local.form_dict.txt))
...
def get_blog_categories():
	...
	post, category = kanivin.qb.DocType("Blog Post"), kanivin.qb.DocType("Blog Category")
	return (
		kanivin.qb.from_(category)
		.select(category.name, category.route, category.title)
		.where(
			(category.published == 1)
			& ExistsCriterion(
				kanivin.qb.from_(post)
				.select("name")
...
def clear_blog_cache():
	for blog in kanivin.db.get_list("Blog Post", fields=["route"], pluck="route", filters={"published": True}):
		clear_cache(blog)
...
def get_blog_list(doctype, txt=None, filters=None, limit_start=0, limit_page_length=20, order_by=None):
	...
	if filters and filters.get("blog_category"):
		...
		category = kanivin.utils.escape_html(
			kanivin.local.form_dict.blog_category or kanivin.local.form_dict.category
		)

	if filters and filters.get("blogger"):
		conditions.append("t1.blogger=%s" % kanivin.db.escape(filters.get("blogger")))

	if category:
		conditions.append("t1.blog_category=%s" % kanivin.db.escape(category))

	if txt:
		conditions.append(
			'(t1.content like {0} or t1.title like {0}")'.format(kanivin.db.escape("%" + txt + "%"))
		)

	if conditions:
		kanivin.local.no_cache = 1
	...
	posts = kanivin.db.sql(query, as_dict=1)
	...
	for post in posts:
		...
		post.category = kanivin.db.get_value(
