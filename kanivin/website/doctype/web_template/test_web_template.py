...
class TestWebTemplate(KanivinTestCase):
	def test_render_web_template_with_values(self):
		doc = kanivin.get_doc("Web Template", "Hero with Right Image")
		values = {
			"title": "Test Hero",
	...
	def test_custom_stylesheet(self):
		...
		kanivin.get_doc("Website Theme", "Standard").set_as_default()

	def create_web_page(self):
		if not kanivin.db.exists("Web Page", "test-web-template"):
			kanivin.get_doc(
				{
					"doctype": "Web Page",
	...
	def create_website_theme(self):
		if not kanivin.db.exists("Website Theme", "Custom"):
			theme = kanivin.get_doc({"doctype": "Website Theme", "theme": "Custom"}).insert()
		else:
			theme = kanivin.get_doc("Website Theme", "Custom")
		return theme
