...
class Blogger(Document):
	...
	def validate(self):
		if self.user and not kanivin.db.exists("User", self.user):
			# for data import
			kanivin.get_doc(
	...
	def on_update(self):
		...
		from kanivin.website.doctype.blog_post.blog_post import clear_blog_cache

		clear_blog_cache()

		if self.user:
			for blog in kanivin.db.sql_list(
				"""select name from `tabBlog Post` where owner=%s
				and ifnull(blogger,'')=''""",
				self.user,
			):
				b = kanivin.get_doc("Blog Post", blog)
				b.blogger = self.name
				b.save()

			kanivin.permissions.add_user_permission("Blogger", self.name, self.user)
