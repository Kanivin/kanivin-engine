...
from kanivin.utils import get_datetime, get_fullname, time_diff_in_hours
...
class PersonalDataDeletionRequest(Document):
	...
	if TYPE_CHECKING:
		...
		from kanivin.website.doctype.personal_data_deletion_step.personal_data_deletion_step import (
	...
	def __init__(self, *args, **kwargs):
		...
		self.user_data_fields = kanivin.get_hooks("user_data_fields")
	...
	def autoname(self):
		from kanivin.model.naming import set_name_from_naming_options
		...
		domain = kanivin.local.site.replace("_", "-")
		...
		kanivin.utils.validate_email_address(self.email, throw=True)

	def after_insert(self):
		self.send_verification_mail()

	def generate_url_for_confirmation(self):
		params = {"email": self.email, "name": self.name, "host_name": kanivin.local.site}
		api = kanivin.utils.get_url(
			"/api/method/kanivin.website.doctype.personal_data_deletion_request"
	...
	def disable_user(self):
		user = kanivin.get_doc("User", self.email)
		user.enabled = False
		user.save()
	...
	def send_verification_mail(self):
		...
		kanivin.sendmail(
			...
			args={
				"email": self.email,
				"name": self.name,
				"host_name": kanivin.utils.get_url(),
				"link": url,
			},
	...
	def notify_system_managers(self):
		...
		kanivin.sendmail(
			...
			args={"user": self.email, "url": kanivin.utils.get_url(self.get_url())},
	...
	def validate_data_anonymization(self):
		...
		if self.status != "Pending Approval":
			kanivin.throw(_("This request has not yet been approved by the user."))
	...
	def generate_anonymization_dict(self, ref):
		...
		meta = kanivin.get_meta(ref["doctype"])
	...
	def redact_doc(self, doc, ref):
		...
		if ref.get("rename") and doc["name"] != self.anon:
			kanivin.rename_doc(ref["doctype"], doc["name"], self.anon, force=True, show_alert=False)
	...
	def _anonymize_data(self, email=None, anon=None, set_data=True, commit=False):
		...
		kanivin.rename_doc("User", email, anon, force=True, show_alert=False)
	...
	def __redact_partial_match_data(self, doctype):
		...
		for df in kanivin.get_meta(doctype["doctype"]).fields:
...
def process_data_deletion_request():
	auto_account_deletion = kanivin.db.get_single_value("Website Settings", "auto_account_deletion")
	if auto_account_deletion < 1:
		return

	requests = kanivin.get_all(
		"Personal Data Deletion Request", filters={"status": "Pending Approval"}, pluck="name"
	)

	for request in requests:
		doc = kanivin.get_doc("Personal Data Deletion Request", request)
...
def confirm_deletion(email, name, host_name):
	...
	doc = kanivin.get_doc("Personal Data Deletion Request", name)
	host_name = kanivin.utils.get_url()
