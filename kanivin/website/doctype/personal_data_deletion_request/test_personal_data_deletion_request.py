...
from kanivin.website.doctype.personal_data_deletion_request.personal_data_deletion_request import (
	process_data_deletion_request,
	remove_unverified_record,
)
from kanivin.website.doctype.personal_data_download_request.test_personal_data_download_request import (
	create_user_if_not_exists,
)


class TestPersonalDataDeletionRequest(KanivinTestCase):
	def setUp(self):
		create_user_if_not_exists(email="test_delete@example.com")
		self.delete_request = kanivin.get_doc(
	...
	def test_delete_request(self):
		email_queue = kanivin.get_all("Email Queue", fields=["*"], order_by="creation desc", limit=1)
	...
	def test_unverified_record_removal(self):
		...
		remove_unverified_record()
		self.assertFalse(kanivin.db.exists("Personal Data Deletion Request", self.delete_request.name))

	def test_process_auto_request(self):
		kanivin.db.set_single_value("Website Settings", "auto_account_deletion", "1")
		date_time_obj = datetime.strptime(self.delete_request.creation, "%Y-%m-%d %H:%M:%S.%f") + timedelta(
			hours=-2
		)
		self.delete_request.db_set("creation", date_time_obj)
