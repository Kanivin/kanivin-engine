...
class PersonalDataDownloadRequest(Document):
	...
	def generate_file_and_send_mail(self, personal_data):
		...
		file_link = (
			kanivin.utils.get_url("/api/method/kanivin.utils.file_manager.download_file")
			+ "?"
			+ get_signed_params({"file_url": f.file_url})
		)
		host_name = kanivin.local.site
		kanivin.sendmail(
...
def get_user_data(user):
	...
	hooks = kanivin.get_hooks("user_data_fields")
	data = {}
	for hook in hooks:
		d = data.get(hook.get("doctype"), [])
		d += kanivin.get_all(hook.get("doctype"), {hook.get("filter_by", "owner"): user}, ["*"])
		if d:
