kanivin.ui.form.on("Web Form", {
	setup: function () {
		kanivin.meta.docfield_map["Web Form Field"].fieldtype.formatter = (value) => {
			const prefix = {
				"Page Break": "--red-600",
		...
		kanivin.meta.docfield_map["Web Form Field"].fieldname.formatter = (value) => {
			if (!value) return;
			return kanivin.unscrub(value);
		};

		kanivin.meta.docfield_map["Web Form List Column"].fieldname.formatter = (value) => {
			if (!value) return;
			return kanivin.unscrub(value);
		};
	},
	...
	refresh: function (frm) {
		...
		frm.sidebar
			...
			.attr("href", "#")
			.on("click", () => {
				const url = kanivin.urllib.get_full_url(frm.doc.route);
				const code = `<iframe src="${url}" style="border: none; width: 100%; height: inherit;"></iframe>`;
				kanivin.utils.copy_to_clipboard(code, __("Embed code copied"));
			});

		if (frm.doc.is_standard && !kanivin.boot.developer_mode) {
			frm.disable_form();
			kanivin.show_alert(
	...
	validate: function (frm) {
		...
		if (!frm.doc.web_form_fields) {
			...
			kanivin.throw(__("Atleast one field is required in Web Form Fields Table"));
		}
		...
		if (page_break_count >= 10) {
			kanivin.throw(__("There can be only 9 Page Break fields in a Web Form"));
		}
	},
	...
	add_get_fields_button(frm) {
		frm.add_custom_button(__("Get Fields"), () => {
			let webform_fieldtypes = kanivin.meta
...
kanivin.ui.form.on("Web Form List Column", {
	fieldname: function (frm, doctype, name) {
		let doc = kanivin.get_doc(doctype, name);
		let df = kanivin.meta.get_docfield(frm.doc.doc_type, doc.fieldname);
		if (!df) return;
...
kanivin.ui.form.on("Web Form Field", {
...
function get_fields_for_doctype(doctype) {
	return new Promise((resolve) => kanivin.model.with_doctype(doctype, resolve)).then(() => {
		return kanivin.meta.get_docfields(doctype).filter((df) => {
			return (
