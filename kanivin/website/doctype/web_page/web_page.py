...
from kanivin.utils import get_datetime, now, quoted, strip_html
from kanivin.utils.caching import redis_cache
from kanivin.utils.jinja import render_template
from kanivin.utils.safe_exec import safe_exec
from kanivin.website.doctype.website_slideshow.website_slideshow import get_slideshow
...
class WebPage(WebsiteGenerator):
	...
	if TYPE_CHECKING:
		...
		from kanivin.website.doctype.web_page_block.web_page_block import WebPageBlock
	...
	def get_context(self, context):
		...
		if self.context_script:
			_locals = dict(context=kanivin._dict())
	...
	def render_dynamic(self, context):
		...
		if is_jinja:
			kanivin.flags.web_block_scripts = {}
			kanivin.flags.web_block_styles = {}
			try:
	...
	def check_for_redirect(self, context):
		if "<!-- redirect:" in context.main_section:
			kanivin.local.flags.redirect_location = (
	...
	def validate_dates(self):
		if self.end_date:
			if self.start_date and get_datetime(self.end_date) < get_datetime(self.start_date):
				kanivin.throw(_("End Date cannot be before Start Date!"))
			...
			if self.published and now() > self.end_date:
				...
				kanivin.msgprint(_("Clearing end date, as it cannot be in the past for published pages."))


def check_publish_status():
	# called via daily scheduler
	web_pages = kanivin.get_all("Web Page", fields=["name", "published", "start_date", "end_date"])
	...
	for page in web_pages:
		...
		if page.published:
			...
			if (start_date and now_date < start_date) or (end_date and now_date > end_date):
				kanivin.db.set_value("Web Page", page.name, "published", 0)
		else:
			# Publish pages that are inside the set date ranges
			if start_date:
				if not end_date or (end_date and now_date < end_date):
					kanivin.db.set_value("Web Page", page.name, "published", 1)


def get_web_blocks_html(blocks):
	"""Converts a list of blocks into Raw HTML and extracts out their scripts for deduplication"""

	out = kanivin._dict(html="", scripts={}, styles={})
	extracted_scripts = {}
	extracted_styles = {}
	for block in blocks:
		web_template = kanivin.get_cached_doc("Web Template", block.web_template)
		rendered_html = kanivin.render_template(
...
def get_dynamic_web_pages() -> dict[str, str]:
	...
	get_web_pages_with_dynamic_routes = kanivin.get_hooks("get_web_pages_with_dynamic_routes") or []
	for method in get_web_pages_with_dynamic_routes:
		pages.extend(kanivin.get_attr(method)())
	return pages
