...
kanivin.ui.form.on("Web Page", {
	layout: function (frm) {
		if (frm.is_new()) {
			if (frm.doc.insert_code) {
				if (!frm.doc.javascript) {
					frm.set_value("javascript", `kanivin.ready(() => {\n\t\n});`);
				}
			}
		}
	},
	...
	validate: function (frm) {
		if (frm.doc.title && !frm.doc.route) {
			frm.set_value("route", kanivin.scrub(frm.doc.title, "-"));
		}
	},
	...
	published: function (frm) {
		...
		if (!frm.doc.published && frm.doc.end_date) {
			var end_date = kanivin.datetime.str_to_obj(kanivin.datetime.now_datetime());
	...
	set_meta_tags(frm) {
		kanivin.utils.set_meta_tag(frm.doc.route);
	},
});

kanivin.ui.form.on("Web Page Block", {
	edit_values(frm, cdt, cdn) {
		...
		open_web_template_values_editor(row.web_template, values).then((new_values) => {
			kanivin.model.set_value(cdt, cdn, "web_template_values", JSON.stringify(new_values));
		});
	},
});

kanivin.tour["Web Page"] = [
	{
		fieldname: "title",
	...
	},
	{
		...
		description: `${__("You can select one from the following,")} <br>
					<ul>
						...
						<li><b>${__("Page Builder")}</b>: ${__("Kanivin page builder using components")}</li>
					</ul>
					`,
	},
	{
