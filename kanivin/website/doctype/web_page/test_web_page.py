...
test_records = kanivin.get_test_records("Web Page")


class TestWebPage(KanivinTestCase):
	def setUp(self):
		kanivin.db.delete("Web Page")
		for t in test_records:
			kanivin.get_doc(t).insert()
	...
	def test_dynamic_route(self):
		web_page = kanivin.get_doc(
			dict(
				...
				content_type="HTML",
				dynamic_template=1,
				main_section_html="<div>{{ kanivin.form_dict.doctype }}</div>",
			)
		).insert()
		try:
			from kanivin.utils import get_html_for_route
