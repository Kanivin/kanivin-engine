...
from kanivin.website.doctype.website_theme.website_theme import (
	after_migrate,
	get_active_theme,
	get_scss_paths,
)
...
def website_theme_fixture(**theme):
	...
	kanivin.delete_doc_if_exists("Website Theme", test_theme)
	theme = kanivin.get_doc(doctype="Website Theme", theme=test_theme, **theme)
	theme.insert()
	yield theme
	kanivin.db.set_single_value("Website Settings", "website_theme", "Standard")
	theme.delete()


class TestWebsiteTheme(KanivinTestCase):
	def test_website_theme(self):
		with website_theme_fixture(
			google_font="Inter",
			custom_scss="body { font-size: 16.5px; }",  # this will get minified!
		) as theme:
			theme_path = kanivin.get_site_path("public", theme.theme_url[1:])
	...
	def test_get_scss_paths(self):
		self.assertIn("kanivin/public/scss/website.bundle", get_scss_paths())

	def test_imports_to_ignore(self):
		with website_theme_fixture(ignored_apps=[{"app": "kanivin"}]) as theme:
			self.assertTrue('@import "kanivin/public/scss/website"' not in theme.theme_scss)
