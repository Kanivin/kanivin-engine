...
class WebsiteTheme(Document):
	...
	if TYPE_CHECKING:
		...
		from kanivin.website.doctype.website_theme_ignore_app.website_theme_ignore_app import (
	...
	def on_update(self):
		if (
			not self.custom
			and kanivin.local.conf.get("developer_mode")
	...
	def is_standard_and_not_valid_user(self):
		return (
			...
			and not (kanivin.flags.in_import or kanivin.flags.in_test or kanivin.flags.in_migrate)
		)

	def on_trash(self):
		if self.is_standard_and_not_valid_user():
			kanivin.throw(_("You are not allowed to delete a standard Website Theme"), kanivin.PermissionError)

	def validate_if_customizable(self):
		if self.is_standard_and_not_valid_user():
			kanivin.throw(_("Please Duplicate this Website Theme to customize."))
	...
	def clear_cache_if_current_theme(self):
		if kanivin.flags.in_install == "kanivin":
			return
		website_settings = kanivin.get_doc("Website Settings", "Website Settings")
	...
	def generate_bootstrap_theme(self):
		...
		folder_path = abspath(kanivin.utils.get_files_path("website_theme", is_private=False))
		# create folder if not exist
		kanivin.create_folder(folder_path)

		if self.custom:
			self.delete_old_theme_files(folder_path)

		# add a random suffix
		suffix = kanivin.generate_hash(length=8) if self.custom else "style"
		file_name = kanivin.scrub(self.name) + "_" + suffix + ".css"
		...
		process = Popen(command, cwd=kanivin.get_app_source_path("kanivin"), stdout=PIPE, stderr=PIPE)

		stderr = process.communicate()[1]

		if stderr:
			stderr = kanivin.safe_decode(stderr)
			stderr = stderr.replace("\n", "<br>")
			kanivin.throw(f'<div style="font-family: monospace;">{stderr}</div>')
		else:
			self.theme_url = "/files/website_theme/" + file_name

		kanivin.msgprint(_("Compiled Successfully"), alert=True)

	def delete_old_theme_files(self, folder_path):
		import os

		for fname in os.listdir(folder_path):
			if fname.startswith(kanivin.scrub(self.name) + "_") and fname.endswith(".css"):
...
def get_active_theme() -> Optional["WebsiteTheme"]:
	if website_theme := kanivin.get_website_settings("website_theme"):
		try:
			return kanivin.get_cached_doc("Website Theme", website_theme)
...
def get_scss(website_theme):
	...
	return kanivin.render_template("kanivin/website/doctype/website_theme/website_theme_template.scss", context)
...
def get_scss_paths():
	...
	If `$BENCH_PATH/apps/kanivin/kanivin/public/scss/website[.bundle].scss` exists, the
	returned set will contain 'kanivin/public/scss/website[.bundle]'.
	"""
	import_path_list = []
	...
	for app in kanivin.get_installed_apps():
		for scss_file in scss_files:
			full_path = kanivin.get_app_path(app, scss_file)
...
def after_migrate():
	...
	"""
	website_theme = kanivin.db.get_single_value("Website Settings", "website_theme")
	if not website_theme or website_theme == "Standard":
		return

	doc = kanivin.get_doc("Website Theme", website_theme)
