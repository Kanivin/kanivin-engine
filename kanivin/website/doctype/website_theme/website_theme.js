...
kanivin.ui.form.on("Website Theme", {
	...
	refresh(frm) {
		...
		frm.toggle_display(["module", "custom"], kanivin.boot.developer_mode);

		frm.trigger("set_default_theme_button_and_indicator");
		frm.trigger("make_app_theme_selector");

		if (!frm.doc.custom && !kanivin.boot.developer_mode) {
			frm.set_read_only();
			frm.disable_save();
		} else {
			frm.enable_save();
	...
	set_default_theme_button_and_indicator(frm) {
		kanivin.db.get_single_value("Website Settings", "website_theme").then((value) => {
	...
	make_app_theme_selector(frm) {
		...
		frm.events.get_installed_apps(frm).then((apps) => {
			...
			let form = new kanivin.ui.FieldGroup({
				fields: [
					{
