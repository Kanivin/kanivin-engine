...
def authorize_access(reauthorize=False, code=None):
	...
	oauth_code = (
		kanivin.db.get_single_value("Website Settings", "indexing_authorization_code") if not code else code
	)
...
def get_google_indexing_object():
	...
	account = kanivin.get_doc("Website Settings")
	oauth_obj = GoogleOAuth("indexing")
...
def publish_site(url, operation_type="URL_UPDATED"):
	...
	try:
		...
		kanivin.log_error(message=e, title="API Indexing Issue")
