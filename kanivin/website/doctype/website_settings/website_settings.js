kanivin.ui.form.on("Website Settings", {
	setup(frm) {
	...
	set_banner_from_image: function (frm) {
		if (!frm.doc.banner_image) {
			kanivin.msgprint(__("Select a Brand Image first."));
		}
	...
	authorize_api_indexing_access: function (frm) {
		kanivin.call({
			method: "kanivin.website.doctype.website_settings.google_indexing.authorize_access",
			args: {
	...
	enable_view_tracking: function (frm) {
		kanivin.boot.website_tracking_enabled = frm.doc.enable_view_tracking;
	},

	set_parent_options: function (frm, doctype, name) {
		var item = kanivin.get_doc(doctype, name);
	...
	edit_template_values(frm, template_field) {
		...
		if (!template) {
			kanivin.show_alert(__("Please select {0}", [frm.get_docfield(template_field).label]));
			return;
		}
