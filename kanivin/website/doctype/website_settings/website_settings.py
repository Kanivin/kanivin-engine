...
from kanivin.utils import encode, get_request_site_address
from kanivin.website.utils import get_boot_data
...
class WebsiteSettings(Document):
	...
	if TYPE_CHECKING:
		...
		from kanivin.website.doctype.top_bar_item.top_bar_item import TopBarItem
		from kanivin.website.doctype.website_route_redirect.website_route_redirect import (
	...
	def validate_google_settings(self):
		if self.enable_google_indexing and not kanivin.get_cached_value("Google Settings", None, "enable"):
			kanivin.throw(_("Enable Google API in Google Settings."))
		...
		for idx, row in enumerate(self.route_redirects):
			try:
				...
				if not kanivin.flags.in_migrate:
					kanivin.throw(_("Invalid redirect regex in row #{}: {}").format(idx, str(e)))
	...
	def get_access_token(self):
		from kanivin.integrations.google_oauth import GoogleOAuth

		if not self.indexing_refresh_token:
			button_label = kanivin.bold(_("Allow API Indexing Access"))
			raise kanivin.ValidationError(_("Click on {0} to generate Refresh Token.").format(button_label))
...
def get_website_settings(context=None):
	...
	context = kanivin._dict(context or {})
	settings: "WebsiteSettings" = kanivin.get_cached_doc("Website Settings")
	...
	if context.disable_website_theme:
		context.theme = kanivin._dict()

	else:
		from kanivin.website.doctype.website_theme.website_theme import get_active_theme

		context.theme = get_active_theme() or kanivin._dict()

	if not context.get("favicon"):
		context["favicon"] = "/assets/kanivin/images/kanivin-favicon.svg"
	...
	context.read_only_mode = kanivin.flags.read_only
...
def get_auto_account_deletion():
	return kanivin.db.get_single_value("Website Settings", "auto_account_deletion")
