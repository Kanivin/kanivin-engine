...
class WebsiteSlideshow(Document):
	...
	if TYPE_CHECKING:
		...
		from kanivin.website.doctype.website_slideshow_item.website_slideshow_item import (
	...
	def validate_images(self):
		...
		if files:
			result = kanivin.get_all("File", filters={"file_url": ("in", list(files))}, fields="is_private")
			if any(file.is_private for file in result):
				kanivin.throw(_("All Images attached to Website Slideshow should be public"))
...
def get_slideshow(doc):
	...
	slideshow = kanivin.get_doc("Website Slideshow", doc.slideshow)
