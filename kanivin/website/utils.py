...
def delete_page_cache(path):
	kanivin.cache.delete_value("full_index")
	groups = ("website_page", "page_context")
	if path:
		for name in groups:
			kanivin.cache.hdel(name, path)
	else:
		for name in groups:
			kanivin.cache.delete_key(name)
...
def can_cache(no_cache=False):
	if kanivin.flags.force_website_cache:
		return True
	if kanivin.conf.disable_website_cache or kanivin.conf.developer_mode:
		return False
	if getattr(kanivin.local, "no_cache", False):
		return False
	return not no_cache


def get_comment_list(doctype, name):
	comments = kanivin.get_all(
		"Comment",
		...
		or_filters=[["owner", "=", kanivin.session.user], ["published", "=", 1]],
	)

	communications = kanivin.get_all(
		"Communication",
		fields=[
			"name",
			"creation",
		...
		or_filters=[
			["recipients", "like", f"%{kanivin.session.user}%"],
			["cc", "like", f"%{kanivin.session.user}%"],
			["bcc", "like", f"%{kanivin.session.user}%"],
		],
	)
...
def get_home_page():
	if kanivin.local.flags.home_page and not kanivin.flags.in_test:
		return kanivin.local.flags.home_page
	...
	def _get_home_page():
		...
		if kanivin.session.user != "Guest":
			# by role
			for role in kanivin.get_roles():
				home_page = kanivin.db.get_value("Role", role, "home_page")
				if home_page:
					break

			# portal default
			if not home_page:
				home_page = kanivin.db.get_single_value("Portal Settings", "default_portal_home")
		...
		# global
		if not home_page:
			home_page = kanivin.db.get_single_value("Website Settings", "home_page")

		if not home_page:
			home_page = "login" if kanivin.session.user == "Guest" else "me"

		home_page = home_page.strip("/")

		return home_page

	if kanivin.local.dev_server:
		# dont return cached homepage in development
		return _get_home_page()

	return kanivin.cache.hget("home_page", kanivin.session.user, _get_home_page)


def get_home_page_via_hooks():
	home_page = None

	home_page_method = kanivin.get_hooks("get_website_user_home_page")
	if home_page_method:
		home_page = kanivin.get_attr(home_page_method[-1])(kanivin.session.user)
	elif kanivin.get_hooks("website_user_home_page"):
		home_page = kanivin.get_hooks("website_user_home_page")[-1]

	if not home_page:
		role_home_page = kanivin.get_hooks("role_home_page")
		if role_home_page:
			for role in kanivin.get_roles():
	...
	if not home_page:
		home_page = kanivin.get_hooks("home_page")
		if home_page:
...
def get_boot_data():
	from kanivin.integrations.kanivin_providers.kanivincloud_billing import is_fc_site

	apps = get_apps() or []
	return {
		"lang": kanivin.local.lang or "en",
		"apps_data": {
			"apps": apps,
		...
		"sysdefaults": {
			"float_precision": cint(kanivin.get_system_settings("float_precision")) or 3,
			"date_format": kanivin.get_system_settings("date_format") or "yyyy-mm-dd",
			"time_format": kanivin.get_system_settings("time_format") or "HH:mm:ss",
			"first_day_of_the_week": kanivin.get_system_settings("first_day_of_the_week") or "Sunday",
			"number_format": kanivin.get_system_settings("number_format") or "#,###.##",
			"currency": kanivin.get_system_settings("currency"),
		},
		"time_zone": {
			"system": get_system_timezone(),
			"user": kanivin.db.get_value("User", kanivin.session.user, "time_zone") or get_system_timezone(),
		},
		"assets_json": get_assets_json(),
		"sitename": kanivin.local.site,
		"is_fc_site": 1 if is_fc_site() else 0,
	}


def is_signup_disabled():
	return kanivin.get_website_settings("disable_signup")
...
def get_toc(route, url_prefix=None, app=None):
	...
	return kanivin.get_template("templates/includes/full_index.html").render(
...
def get_full_index(route=None, app=None):
	...
	from kanivin.website.router import get_pages

	if not kanivin.local.flags.children_map:
		...
		children_map = kanivin.cache.get_value("website_full_index", _build)

		kanivin.local.flags.children_map = children_map

	return kanivin.local.flags.children_map
...
def clear_cache(path=None):
	...
	for key in (
		...
		kanivin.cache.delete_value(key)

	clear_routing_cache()

	kanivin.cache.delete_value("website_404")
	if path:
		kanivin.cache.hdel("website_redirects", path)
		delete_page_cache(path)
	else:
		clear_sitemap()
		kanivin.clear_cache("Guest")
		for key in (
			"portal_menu_items",
			"home_page",
			"website_route_rules",
	...
	for method in kanivin.get_hooks("website_clear_cache"):
		kanivin.get_attr(method)(path)
...
def get_sidebar_items(parent_sidebar, basepath=None):
	...
	hooks = kanivin.get_hooks("look_for_sidebar_json")
	look_for_sidebar_json = hooks[0] if hooks else kanivin.flags.look_for_sidebar
...
def get_portal_sidebar_items():
	sidebar_items = kanivin.cache.hget("portal_menu_items", kanivin.session.user)
	if sidebar_items is None:
		sidebar_items = []
		roles = kanivin.get_roles()
		portal_settings = kanivin.get_doc("Portal Settings", "Portal Settings")
		...
		items_via_hooks = kanivin.get_hooks("portal_menu_items")
		if items_via_hooks:
			for i in items_via_hooks:
				i["enabled"] = 1
			add_items(sidebar_items, items_via_hooks)

		kanivin.cache.hset("portal_menu_items", kanivin.session.user, sidebar_items)
...
def get_sidebar_items_from_sidebar_file(basepath, look_for_sidebar_json):
	...
	with open(sidebar_json_path) as sidebarfile:
		try:
			...
			kanivin.throw("Invalid Sidebar JSON at " + sidebar_json_path)
...
def cache_html(func):
	...
	def cache_html_decorator(*args, **kwargs):
		if can_cache():
			html = None
			page_cache = kanivin.cache.hget("website_page", args[0].path)
			if page_cache and kanivin.local.lang in page_cache:
				html = page_cache[kanivin.local.lang]
			if html:
				kanivin.local.response.from_cache = True
				return html
		html = func(*args, **kwargs)
		context = args[0].context
		if can_cache(context.no_cache):
			page_cache = kanivin.cache.hget("website_page", args[0].path) or {}
			page_cache[kanivin.local.lang] = html
			kanivin.cache.hset("website_page", args[0].path, page_cache)
...
def build_response(path, data, http_status_code, headers: dict | None = None):
	...
	response.headers["X-From-Cache"] = kanivin.local.response.from_cache or False
...
def add_preload_for_bundled_assets(response):
	links = [f"<{css}>; rel=preload; as=style" for css in kanivin.local.preload_assets["style"]]
	links.extend(f"<{js}>; rel=preload; as=script" for js in kanivin.local.preload_assets["script"])

	version = get_build_version()
	# include_icons = kanivin.get_hooks().get("app_include_icons", [])
	links.extend(
		f"</assets/{svg}?v={version}>; rel=preload; as=fetch; crossorigin"
		for svg in kanivin.local.preload_assets["icons"]
	)
