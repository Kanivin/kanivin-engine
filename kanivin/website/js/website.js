...
kanivin.provide("website");
kanivin.provide("kanivin.awesome_bar_path");
window.cur_frm = null;
...
$.extend(kanivin, {
	...
	require: async function (links, callback) {
		...
		links = links.map((link) => kanivin.bundled_asset(link));
	...
	bundled_asset(path, is_rtl = null) {
		if (!path.startsWith("/assets") && path.includes(".bundle.")) {
			...
			path = kanivin.boot.assets_json[path] || path;
			return path;
		}
		return path;
	},
	add_asset_to_head(link) {
		return new Promise((resolve) => {
			if (kanivin._assets_loaded.includes(link)) return resolve();
			let el;
			...
			el.onload = () => {
				kanivin._assets_loaded.push(link);
				resolve();
			};
		});
	},
	...
	call: function (opts) {
		...
		return $.ajax({
			...
			statusCode: opts.statusCode || {
				404: function () {
					kanivin.msgprint(__("Not found"));
				},
				403: function () {
					kanivin.msgprint(__("Not permitted"));
				},
				200: function (data) {
			...
			kanivin.process_response(opts, data);
		});
	},
	...
	process_response: function (opts, data) {
		...
		if (data._server_messages) {
			...
			if (opts.error_msg) {
				...
				kanivin.msgprint(server_messages);
			}
		}
	...
	has_permission: function (doctype, docname, perm_type, callback) {
		return kanivin.call({
			type: "GET",
			method: "kanivin.client.has_permission",
			no_spinner: true,
	...
	render_user: function () {
		if (kanivin.is_user_logged_in()) {
			$(".btn-login-area").toggle(false);
			$(".logged-in").toggle(true);
			$(".user-image").attr("src", kanivin.get_cookie("user_image"));

			$(".user-image-wrapper").html(
				kanivin.avatar(null, "avatar-medium", null, null, null, true)
			);
			$(".user-image-sidebar").html(
				kanivin.avatar(null, "avatar-medium", null, null, null, true)
			);
			$(".user-image-myaccount").html(
				kanivin.avatar(null, "avatar-large", null, null, null, true)
			);
		}
	},
	freeze_count: 0,
	freeze: function (msg) {
		...
		kanivin.freeze_count++;
	},
	unfreeze: function () {
		if (!kanivin.freeze_count) return; // anything open?
		kanivin.freeze_count--;
		if (!kanivin.freeze_count) {
	...
	trigger_ready: function () {
		kanivin.ready_events.forEach(function (fn) {
			fn();
		});
	},
	...
	bind_filters: function () {
		...
		$(".filter").each(function () {
			...
			var val = kanivin.utils.get_url_arg(key).replace(/\+/g, " ");
	...
	bind_navbar_search: function () {
		kanivin.get_navbar_search().on("keypress", function (e) {
			var val = $(this).val();
			if (e.which === 13 && val) {
				$(this).val("").blur();
				kanivin.do_search(val);
				return false;
			}
		});
	},
	do_search: function (val) {
		var path =
			(kanivin.awesome_bar_path && kanivin.awesome_bar_path[location.pathname]) ||
			window.search_path ||
			location.pathname;
	...
	set_search_path: function (path) {
		kanivin.awesome_bar_path[location.pathname] = path;
	},
	...
	is_user_logged_in: function () {
		return kanivin.get_cookie("user_id") !== "Guest" && kanivin.session.user !== "Guest";
	},
	...
	show_language_picker() {
		if (kanivin.session.user === "Guest" && window.show_language_picker) {
			kanivin
				.call("kanivin.translate.get_all_languages", {
					with_language_name: true,
				})
				.then((res) => {
					let language_list = res.message;
					let language = kanivin.get_cookie("preferred_language");
					let language_codes = [];
...
kanivin.setup_search = function (target, search_scope) {
	...
	$input.on(
		...
		kanivin.utils.debounce(() => {
			...
			kanivin
				.call({
					method: "kanivin.search.web_search",
					args: {
...
window.valid_email = function (id) {
	// copied regex from kanivin/utils.js validate_type
...
$(document).ready(function () {
	window.full_name = kanivin.get_cookie("full_name");
	var logged_in = kanivin.is_user_logged_in();
	$("#website-login").toggleClass("hide", logged_in ? true : false);
	$("#website-post-login").toggleClass("hide", logged_in ? false : true);
	$(".logged-in").toggleClass("hide", logged_in ? false : true);

	kanivin.bind_navbar_search();

	// add apps link
	let apps = kanivin.boot?.apps_data?.apps;
	let obj = {
		label: __("Apps"),
		route: "/apps",
	};
	if (apps?.length) {
		...
		let is_desk_apps = kanivin.boot?.apps_data?.is_desk_apps;
		!is_desk_apps && kanivin.add_apps(obj);
	}

	// switch to app link
	if (kanivin.get_cookie("system_user") === "yes" && logged_in) {
		kanivin.add_switch_to_desk();
	}
...
$(document).on("page-change", function () {
	...
	$.extend(kanivin, kanivin.get_cookies());
	kanivin.session = { user: kanivin.user_id };

	kanivin.datetime.refresh_when();
	kanivin.trigger_ready();
	kanivin.bind_filters();
	kanivin.highlight_code_blocks();
	kanivin.add_link_to_headings();
	kanivin.make_navbar_active();
	// scroll to hash
...
kanivin.ready(function () {
	kanivin.show_language_picker();
	kanivin.setup_videos();
	kanivin.realtime.init(window.socketio_port, true); // lazy connection
});
