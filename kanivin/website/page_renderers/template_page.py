...
import kanivin
from kanivin.website.page_renderers.base_template_page import BaseTemplatePage
from kanivin.website.router import get_base_template, get_page_info
...
class TemplatePage(BaseTemplatePage):
	...
	def set_template_path(self):
		...
		for app in reversed(kanivin.get_installed_apps()):
	...
	def get_index_path_options(search_path):
		return (
			kanivin.as_unicode(f"{search_path}{d}") for d in ("", ".html", ".md", "/index.html", "/index.md")
		)
	...
	def update_context(self):
		...
		self.context.build_version = kanivin.utils.get_build_version()

		if self.pymodule_name:
			self.pymodule = kanivin.get_module(self.pymodule_name)
		...
		self.context.developer_mode = kanivin.conf.developer_mode
	...
	def render_template(self):
		if self.template_path.endswith("min.js"):
			...
			html = kanivin.render_template(self.source, self.context, safe_render=safe_render)
	...
	def get_raw_template(self):
		return kanivin.get_jloader().get_source(kanivin.get_jenv(), self.context.template)[0]
	...
	def convert_from_markdown(self):
		if self.template_path.endswith(".md"):
			self.source = kanivin.utils.md_to_html(self.source)
	...
	def set_standard_path(self, path):
		self.app = "kanivin"
		self.app_path = kanivin.get_app_path("kanivin")
		self.path = path
	...
	def set_user_info(self):
		from kanivin.utils.user import get_fullname_and_avatar

		info = get_fullname_and_avatar(kanivin.session.user)
