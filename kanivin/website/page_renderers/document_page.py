import kanivin
from kanivin.model.document import get_controller
...
from kanivin.website.router import (
	get_doctypes_with_web_view,
	get_page_info_from_web_page_with_dynamic_routes,
)
from kanivin.website.utils import cache_html
...
class DocumentPage(BaseTemplatePage):
	...
	def search_in_doctypes_with_web_view(self):
		if document := _find_matching_document_webview(self.path):
			...
			doc = kanivin.get_cached_doc(self.doctype, self.docname)
			return doc.meta.allow_guest_to_view or doc.has_permission() or kanivin.has_website_permission(doc)
	...
	@cache_html
	def get_html(self):
		self.doc = kanivin.get_doc(self.doctype, self.docname)
		self.init_context()
		self.update_context()
		self.post_process_context()
		return kanivin.get_template(self.template_path).render(self.context)
...
def _find_matching_document_webview(route: str) -> tuple[str, str] | None:
	for doctype in get_doctypes_with_web_view():
		...
		try:
			docname = None
			if meta.is_virtual:
				if doclist := kanivin.get_all(doctype, filters=filters, fields=["name"], limit=1):
					docname = doclist[0].get("name")
			else:
				docname = kanivin.db.get_value(doctype, filters, "name")
