...
from kanivin.website.utils import can_cache
...
class NotFoundPage(TemplatePage):
	def __init__(self, path, http_status_code=None):
		...
		self.request_url = kanivin.local.request.url if hasattr(kanivin.local, "request") else ""
		path = "404"
	...
	def render(self):
		if self.can_cache_404():
			kanivin.cache.hset("website_404", self.request_url, True)
		return super().render()
