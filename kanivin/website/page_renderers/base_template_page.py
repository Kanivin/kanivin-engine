...
from kanivin.website.page_renderers.base_renderer import BaseRenderer
from kanivin.website.website_components.metatags import MetaTags
...
class BaseTemplatePage(BaseRenderer):
	...
	def init_context(self):
		self.context = kanivin._dict()
		self.context.update(get_website_settings())
		self.context.update(kanivin.local.conf.get("website_context") or {})

	def add_csrf_token(self, html):
		if kanivin.local.session:
			csrf_token = kanivin.local.session.data.csrf_token
			return html.replace(
				"<!-- csrf_token -->", f'<script>kanivin.csrf_token = "{csrf_token}";</script>'
			)
	...
	def set_base_template_if_missing(self):
		if not self.context.base_template_path:
			app_base = kanivin.get_hooks("base_template")
	...
	def set_missing_values(self):
		# set using kanivin.respond_as_web_page
		if hasattr(kanivin.local, "response") and kanivin.local.response.get("context"):
			self.context.update(kanivin.local.response.context)

		# to be able to inspect the context dict
		# Use the macro "inspect" from macros.html
		self.context.canonical = kanivin.utils.get_url(kanivin.utils.escape_html(self.path))
		...
		self.context.pathname = getattr(kanivin.local, "path", None) if hasattr(kanivin, "local") else self.path

	def update_website_context(self):
		# apply context from hooks
		update_website_context = kanivin.get_hooks("update_website_context")
		for method in update_website_context:
			values = kanivin.get_attr(method)(self.context)
			if values:
