...
import kanivin
from kanivin.website.utils import extract_title, get_frontmatter
...
def get_page_info_from_web_page_with_dynamic_routes(path):
	...
	"""
	from kanivin.website.doctype.web_page.web_page import get_dynamic_web_pages
...
def get_page_info_from_web_form(path):
	...
	from kanivin.website.doctype.web_form.web_form import get_published_web_forms
	...
	if end_point:
		if path.endswith("/list"):
			kanivin.form_dict.is_list = True
		elif path.endswith("/new"):
			kanivin.form_dict.is_new = True
		elif path.endswith("/edit"):
			kanivin.form_dict.is_edit = True
		else:
			kanivin.form_dict.is_read = True
		return page_info[end_point]
...
def evaluate_dynamic_routes(rules, path):
	...
	if hasattr(kanivin.local, "request") and kanivin.local.request.environ:
		urls = route_map.bind_to_environ(kanivin.local.request.environ)
		try:
			endpoint, args = urls.match("/" + path)
			if args:
				# don't cache when there's a query string!
				kanivin.local.no_cache = 1
				kanivin.local.form_dict.update(args)
		except NotFound:
			pass
...
def get_pages(app=None):
	...
	def _build(app):
		...
		if app:
			...
			apps = kanivin.local.flags.web_pages_apps or kanivin.get_installed_apps()
	...
	return kanivin.cache.get_value("website_pages", lambda: _build(app))
...
def get_pages_from_path(start, app, app_path):
	...
	if os.path.exists(start_path):
		for basepath, folders, files in os.walk(start_path):  # noqa: B007
			for fname in files:
				fname = kanivin.utils.cstr(fname)
				if "." not in fname:
					continue
...
def setup_source(page_info):
	...
	if page_info.template.endswith((".md", ".html")):
		...
		if page_info.template.endswith(".md"):
			source = kanivin.utils.md_to_html(source)
...
def get_base_template(path=None):
	...
	"""
	if not path:
		path = kanivin.local.request.path

	base_template_map = kanivin.get_hooks("base_template_map") or {}
...
def load_properties_from_controller(page_info):
	...
	module = kanivin.get_module(page_info.controller)
	if not module:
		return
...
def get_doctypes_with_web_view():
	...
	def _get():
		...
		doctypes = kanivin.get_hooks("website_generators")
		doctypes_with_web_view = kanivin.get_all(
			"DocType", fields=["name", "module"], filters=dict(has_web_view=1)
		)
		module_app_map = kanivin.local.module_app
		doctypes += [
			d.name
			for d in doctypes_with_web_view
			if module_app_map.get(kanivin.scrub(d.module)) in installed_apps
		]
		return doctypes

	return kanivin.cache.get_value("doctypes_with_web_view", _get)


def get_start_folders():
	return kanivin.local.flags.web_pages_folders or ("www", "templates/pages")


def clear_routing_cache():
	from kanivin.website.doctype.web_form.web_form import get_published_web_forms
	from kanivin.website.doctype.web_page.web_page import get_dynamic_web_pages
	from kanivin.website.page_renderers.document_page import _find_matching_document_webview
	from kanivin.www.sitemap import get_public_pages_from_doctypes
	...
	kanivin.cache.delete_value("home_page")
