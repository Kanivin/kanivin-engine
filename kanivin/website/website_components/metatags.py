...
class MetaTags:
	def __init__(self, path, context):
		...
		self.tags = kanivin._dict(self.context.get("metatags") or {})
	...
	def init_metatags_from_context(self):
		...
		if self.tags.get("image"):
			self.tags["image"] = kanivin.utils.get_url(self.tags["image"])

		self.tags["language"] = kanivin.local.lang or "en"
	...
	def set_metatags_from_website_route_meta(self):
		...
		if route == "":
			# homepage
			route = kanivin.db.get_single_value("Website Settings", "home_page")

		route_exists = (
			route and not route.endswith((".js", ".css")) and kanivin.db.exists("Website Route Meta", route)
		)

		if route_exists:
			website_route_meta = kanivin.get_doc("Website Route Meta", route)
