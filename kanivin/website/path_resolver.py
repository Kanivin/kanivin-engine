...
import kanivin
from kanivin.website.page_renderers.document_page import DocumentPage
from kanivin.website.page_renderers.list_page import ListPage
from kanivin.website.page_renderers.not_found_page import NotFoundPage
from kanivin.website.page_renderers.print_page import PrintPage
...
from kanivin.website.page_renderers.web_form import WebFormPage
from kanivin.website.router import evaluate_dynamic_routes
from kanivin.website.utils import can_cache, get_home_page
...
class PathResolver:
	...
	def resolve(self):
		...
		request = kanivin._dict()
		if hasattr(kanivin.local, "request"):
			request = kanivin.local.request or request

		# check if the request url is in 404 list
		if request.url and can_cache() and kanivin.cache.hget("website_404", request.url):
			return self.path, NotFoundPage(self.path)

		try:
			resolve_redirect(self.path, request.query_string)
		except kanivin.Redirect as e:
			return kanivin.flags.redirect_location, RedirectPage(self.path, e.http_status_code)

		if kanivin.get_hooks("website_path_resolver"):
			for handler in kanivin.get_hooks("website_path_resolver"):
				endpoint = kanivin.get_attr(handler)(self.path)
		else:
			try:
				endpoint = resolve_path(self.path)
			except werkzeug.routing.exceptions.RequestRedirect as e:
				kanivin.flags.redirect_location = e.new_url
				return kanivin.flags.redirect_location, RedirectPage(e.new_url, e.code)
	...
	def get_custom_page_renderers():
		custom_renderers = []
		for renderer_path in kanivin.get_hooks("page_renderer") or []:
			try:
				renderer = kanivin.get_attr(renderer_path)
...
def resolve_redirect(path, query_string=None):
	...
	"""
	redirects = kanivin.get_hooks("website_redirects")
	redirects += kanivin.get_all(
	...
	redirect_to = kanivin.cache.hget("website_redirects", path)

	if redirect_to:
		if isinstance(redirect_to, dict):
			kanivin.flags.redirect_location = redirect_to["path"]
			raise kanivin.Redirect(redirect_to["status_code"])
		kanivin.flags.redirect_location = redirect_to
	...
	for rule in redirects:
		...
		if query_string and rule.get("match_with_query_string"):
			path_to_match = path + "?" + kanivin.safe_decode(query_string)

		try:
			match = re.match(pattern, path_to_match)
		except re.error:
			kanivin.log_error("Broken Redirect: " + pattern)
		...
		if match:
			...
			raise kanivin.Redirect(status_code)
...
def get_website_rules():
	...
	def _get():
		rules = kanivin.get_hooks("website_route_rules")
		for d in kanivin.get_all("DocType", "name, route", dict(has_web_view=1)):
			if d.route:
	...
	return kanivin.cache.get_value("website_route_rules", _get)


def validate_path(path: str):
	if not PathResolver(path).is_valid_path():
		kanivin.throw(kanivin._("Path {0} it not a valid path").format(kanivin.bold(path)))
