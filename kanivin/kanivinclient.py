"""
KanivinClient is a library that helps you connect with other kanivin systems
"""
import base64
import json
...
class KanivinException(Exception):
	pass
...
class KanivinClient:
	def __init__(
		...
		api_key=None,
		api_secret=None,
		kanivin_authorization_source=None,
	):
		import requests
		...
		self.kanivin_authorization_source = kanivin_authorization_source
	...
	def setup_key_authentication_headers(self):
		if self.api_key and self.api_secret:
			...
			if self.kanivin_authorization_source:
				auth_source = {"Kanivin-Authorization-Source": self.kanivin_authorization_source}
	...
	def insert(self, doc):
		...
		res = self.session.post(
			...
			data={"data": kanivin.as_json(doc)},
			verify=self.verify,
			headers=self.headers,
		)
		return kanivin._dict(self.post_process(res))
	...
	def insert_many(self, docs):
		...
		return self.post_request({"cmd": "kanivin.client.insert_many", "docs": kanivin.as_json(docs)})
	...
	def update(self, doc):
		...
		res = self.session.put(
			url, data={"data": kanivin.as_json(doc)}, verify=self.verify, headers=self.headers
		)
		return kanivin._dict(self.post_process(res))
	...
	def bulk_update(self, docs):
		...
		return self.post_request({"cmd": "kanivin.client.bulk_update", "docs": kanivin.as_json(docs)})
	...
	def delete(self, doctype, name):
		...
		return self.post_request({"cmd": "kanivin.client.delete", "doctype": doctype, "name": name})

	def submit(self, doc):
		"""Submit remote document

		:param doc: dict or Document object to be submitted remotely"""
		return self.post_request({"cmd": "kanivin.client.submit", "doc": kanivin.as_json(doc)})
	...
	def get_value(self, doctype, fieldname=None, filters=None):
		...
		return self.get_request(
			{
				"cmd": "kanivin.client.get_value",
				"doctype": doctype,
				"fieldname": fieldname or "name",
				"filters": kanivin.as_json(filters),
			}
		)
	...
	def cancel(self, doctype, name):
		...
		return self.post_request({"cmd": "kanivin.client.cancel", "doctype": doctype, "name": name})
	...
	def migrate_doctype(self, doctype, filters=None, update=None, verbose=1, exclude=None, preprocess=None):
		...
		for doc in docs:
			...
			if doctype != "User" and not kanivin.db.exists("User", doc.get("owner")):
				kanivin.get_doc(
					{
						"doctype": "User",
						"email": doc.get("owner"),
	...
	def migrate_single(self, doctype):
		...
		doc.modified = kanivin.db.get_single_value(doctype, "modified")
	...
	def post_process(self, response):
		...
		if rjson and ("exc" in rjson) and rjson["exc"]:
			try:
				...
				exc = "KanivinClient Request Failed\n\n" + exc
			except Exception:
				exc = rjson["exc"]

			raise KanivinException(exc)
		if "message" in rjson:
...
class KanivinOAuth2Client(KanivinClient):
