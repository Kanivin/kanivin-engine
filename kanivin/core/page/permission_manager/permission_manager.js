kanivin.pages["permission-manager"].on_page_load = (wrapper) => {
	let page = kanivin.ui.make_app_page({
		parent: wrapper,
	...
	$(kanivin.render_template("permission_manager_help", {})).appendTo(page.main);
	wrapper.permission_engine = new kanivin.PermissionEngine(wrapper);
};

kanivin.pages["permission-manager"].refresh = function (wrapper) {
	wrapper.permission_engine.set_from_route();
};

kanivin.PermissionEngine = class PermissionEngine {
	constructor(wrapper) {
	...
	setup_page() {
		this.doctype_select = this.wrapper.page.add_field({
			...
			options: "DocType",
			change: function () {
				kanivin.set_route("permission-manager", this.get_value());
			},
		});
		...
		this.page.add_inner_button(__("Set User Permissions"), () => {
			return kanivin.set_route("List", "User Permission");
		});
		this.set_from_route();
	}
	...
	set_from_route() {
		...
		if (kanivin.get_route()[1]) {
			this.doctype_select.set_value(kanivin.get_route()[1]);
		} else if (kanivin.route_options) {
			if (kanivin.route_options.doctype) {
				this.doctype_select.set_value(kanivin.route_options.doctype);
			}
			if (kanivin.route_options.role) {
				this.role_select.set_value(kanivin.route_options.role);
			}
	...
	reset_std_permissions(data) {
		let doctype = this.get_doctype();
		let d = kanivin.confirm(__("Reset Permissions for {0}?", [__(doctype)]), () => {
			return kanivin
				.call({
					module: "kanivin.core",
					page: "permission_manager",
		...
		data.message.forEach((d) => {
			let rights = this.rights
				.filter((r) => d[r])
				.map((r) => {
					return __(toTitle(kanivin.unscrub(r)));
				});
	...
	add_delete_button(row, d) {
		$(
			`<button class='btn btn-danger btn-remove-perm btn-xs'>${kanivin.utils.icon(
				"delete"
			)}</button>`
		)
			...
			.on("click", () => {
				return kanivin.call({
					...
					callback: (r) => {
						if (r.exc) {
							kanivin.msgprint(__("Did not remove"));
						} else {
							this.refresh();
						}
					},
	...
	add_check_events() {
		...
		this.body.on("click", ".show-user-permissions", () => {
			kanivin.route_options = { allow: this.get_doctype() || "" };
			...
			() => {
				...
				d.set_primary_action(__("Add"), () => {
					...
					kanivin.call({
						...
						callback: (r) => {
							if (r.exc) {
								kanivin.msgprint(__("Did not add"));
							} else {
								this.refresh();
							}
						},
	...
	get_link_fields(doctype) {
		return kanivin.get_children("DocType", doctype, "fields", {
			fieldtype: "Link",
