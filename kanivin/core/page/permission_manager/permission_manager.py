...
from kanivin.modules.import_file import get_file_path, read_doc_from_file
...
from kanivin.utils.user import get_users_with_role as _get_user_with_role
...
def get_roles_and_doctypes():
	...
	if kanivin.session.user != "Administrator":
		custom_user_type_roles = kanivin.get_all("User Type", filters={"is_standard": 0}, fields=["role"])
...
def get_permissions(doctype: str | None = None, role: str | None = None):
	...
	if role:
		...
		if kanivin.session.user != "Administrator":
			custom_roles = kanivin.get_all("Role", filters={"is_custom": 1}, pluck="name")
			filters["role"] = ["not in", custom_roles]

		out = kanivin.get_all("Custom DocPerm", fields="*", filters=filters, order_by="permlevel")
		if not out:
			out = kanivin.get_all("DocPerm", fields="*", filters=filters, order_by="permlevel")
	...
	for d in out:
		...
		if meta := kanivin.get_meta(d.parent):
...
def update(doctype, role, permlevel, ptype, value=None, if_owner=0):
	...
	if ptype == "report" and value == "1" and if_owner == "1":
		kanivin.throw(_("Cannot set 'Report' permission if 'Only If Creator' permission is set"))
	...
	kanivin.db.after_commit.add(clear_cache)
...
def remove(doctype, role, permlevel, if_owner=0):
	...
	if not kanivin.get_all("Custom DocPerm", {"parent": doctype}):
		kanivin.throw(_("There must be atleast one permission rule."), title=_("Cannot Remove"))
