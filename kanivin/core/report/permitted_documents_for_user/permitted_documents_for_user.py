...
from kanivin.permissions import rights
...
def execute(filters=None):
	...
	data = kanivin.get_list(doctype, fields=fields, as_list=True, user=user)

	if show_permissions:
		columns = columns + [kanivin.unscrub(right) + ":Check:80" for right in rights]
		data = list(data)
		for i, doc in enumerate(data):
			permission = kanivin.permissions.get_doc_permissions(kanivin.get_doc(doctype, doc[0]), user)
...
def get_columns_and_fields(doctype):
	...
	fields = ["name"]
	for df in kanivin.get_meta(doctype).fields:
...
def query_doctypes(doctype, txt, searchfield, start, page_len, filters):
	...
	user_perms = kanivin.utils.user.UserPermissions(user)
	...
	single_doctypes = [d[0] for d in kanivin.db.get_values("DocType", {"issingle": 1})]

	return [
		[dt]
		for dt in can_read
		if txt.lower().replace("%", "") in kanivin._(dt).lower()
