...
from kanivin.cache_manager import clear_controller_cache, clear_user_cache
from kanivin.custom.doctype.custom_field.custom_field import create_custom_field
from kanivin.custom.doctype.property_setter.property_setter import make_property_setter
from kanivin.database import savepoint
from kanivin.database.schema import validate_column_length, validate_column_name
from kanivin.desk.notifications import delete_notification_count_for, get_filters_for
from kanivin.desk.utils import validate_route_conflict
from kanivin.model import (
	child_table_fields,
	data_field_options,
	default_fields,
	no_value_fields,
...
from kanivin.model.base_document import get_controller
from kanivin.model.docfield import supports_translation
from kanivin.model.document import Document
from kanivin.model.meta import Meta
from kanivin.modules import get_doc_path, make_boilerplate
from kanivin.modules.import_file import get_file_path
from kanivin.permissions import ALL_USER_ROLE, AUTOMATIC_ROLES, SYSTEM_USER_ROLE
from kanivin.query_builder.functions import Concat
from kanivin.utils import cint, flt, get_datetime, is_a_property, random_string
from kanivin.website.utils import clear_cache

if TYPE_CHECKING:
	from kanivin.custom.doctype.customize_form.customize_form import CustomizeForm
...
class InvalidFieldNameError(kanivin.ValidationError):
	pass


class UniqueFieldnameError(kanivin.ValidationError):
	pass


class IllegalMandatoryError(kanivin.ValidationError):
	pass


class DoctypeLinkError(kanivin.ValidationError):
	pass


class WrongOptionsDoctypeLinkError(kanivin.ValidationError):
	pass


class HiddenAndMandatoryWithoutDefaultError(kanivin.ValidationError):
	pass


class NonUniqueError(kanivin.ValidationError):
	pass


class CannotIndexedError(kanivin.ValidationError):
	pass


class CannotCreateStandardDoctypeError(kanivin.ValidationError):
	pass
...
class DocType(Document):
	...
	if TYPE_CHECKING:
		from kanivin.core.doctype.docfield.docfield import DocField
		from kanivin.core.doctype.docperm.docperm import DocPerm
		from kanivin.core.doctype.doctype_action.doctype_action import DocTypeAction
		from kanivin.core.doctype.doctype_link.doctype_link import DocTypeLink
		from kanivin.core.doctype.doctype_state.doctype_state import DocTypeState
	...
	def validate(self):
		...
		if not self.is_new():
			self.before_update = kanivin.get_doc("DocType", self.name)
		...
		if self.default_print_format and not self.custom:
			kanivin.throw(_("Standard DocType cannot have default print format, use Customize Form"))
	...
	def check_indexing_for_dashboard_links(self):
		...
		for d in self.fields:
			if d.fieldtype == "Link" and not (d.unique or d.search_index):
				referred_as_link = kanivin.db.exists(
					"DocType Link",
	...
	def check_developer_mode(self):
		...
		if kanivin.flags.in_patch or kanivin.flags.in_test:
			return

		if not kanivin.conf.get("developer_mode") and not self.custom:
			kanivin.throw(
		...
		if self.is_virtual and self.custom:
			kanivin.throw(_("Not allowed to create custom Virtual DocType."), CannotCreateStandardDoctypeError)

		if kanivin.conf.get("developer_mode"):
	...
	def setup_fields_to_fetch(self):
		...
		try:
			old_meta = kanivin.get_meta(kanivin.get_doc("DocType", self.name), cached=False)
			old_fields_to_fetch = [df.fieldname for df in old_meta.get_fields_to_fetch()]
		except kanivin.DoesNotExistError:
			old_fields_to_fetch = []

		new_meta = kanivin.get_meta(self, cached=False)
	...
	def validate_website(self):
		...
		if self.has_web_view:
			...
			if "route" not in [d.fieldname for d in self.fields]:
				kanivin.throw(_('Field "route" is mandatory for Web Views'), title="Missing Field")
	...
	def validate_virtual_doctype_methods(self):
		...
		from kanivin.model.virtual_doctype import validate_controller
	...
	def ensure_minimum_max_attachment_limit(self):
		...
		from kanivin.model import attachment_fieldtypes
		...
		if total_attach_fields > self.max_attachments:
			...
			field_label = kanivin.bold(self.meta.get_field("max_attachments").label)
			kanivin.msgprint(
	...
	def change_modified_of_parent(self):
		...
		if kanivin.flags.in_import:
			return
		parent_list = kanivin.get_all(
			"DocField", "parent", dict(fieldtype=["in", kanivin.model.table_fields], options=self.name)
		)
		for p in parent_list:
			kanivin.db.set_value("DocType", p.parent, {})
	...
	def on_update(self):
		...
		try:
			kanivin.db.updatedb(self.name, Meta(self))
		...
		allow_doctype_export = (
			not self.custom
			and not kanivin.flags.in_import
			and (kanivin.conf.developer_mode or kanivin.flags.allow_doctype_export)
		)
		...
		kanivin.clear_cache(doctype=self.name)

		# clear user cache so that on the next reload this doctype is included in boot
		clear_user_cache(kanivin.session.user)

		if not kanivin.flags.in_install and hasattr(self, "before_update"):
	...
	def setup_autoincrement_and_sequence(self):
		...
		name_type = f"varchar({kanivin.db.VARCHAR_LEN})"

		if self.autoname == "autoincrement":
			name_type = "bigint"
			kanivin.db.create_sequence(self.name, check_not_exists=True)
	...
	def sync_global_search(self):
		...
		if set(global_search_fields_before_update) != set(global_search_fields_after_update):
			now = (not kanivin.request) or kanivin.flags.in_test or kanivin.flags.in_install
			kanivin.enqueue("kanivin.utils.global_search.rebuild_for_doctype", now=now, doctype=self.name)
	...
	def set_base_class_for_controller(self):
		...
		scrubbed_name = kanivin.scrub(self.name)
		scrubbed_module = kanivin.scrub(self.module)
		controller_path = kanivin.get_module_path(
			scrubbed_module, "doctype", scrubbed_name, f"{scrubbed_name}.py"
		)

		document_cls_tag = f"class {despaced_name}(Document)"
		document_import_tag = "from kanivin.model.document import Document"
		website_generator_cls_tag = f"class {despaced_name}(WebsiteGenerator)"
		website_generator_import_tag = "from kanivin.website.website_generator import WebsiteGenerator"
	...
	def run_module_method(self, method):
		from kanivin.modules import load_doctype_module
	...
	def before_rename(self, old, new, merge=False):
		...
		if not self.custom and kanivin.session.user != "Administrator":
			kanivin.throw(_("DocType can only be renamed by Administrator"))

		self.check_developer_mode()
		self.validate_name(new)

		if merge:
			kanivin.throw(_("DocType can not be merged"))
	...
	def after_rename(self, old, new, merge=False):
		...
		if self.issingle:
			kanivin.db.sql("""update tabSingles set doctype=%s where doctype=%s""", (new, old))
			kanivin.db.sql(
			...
			kanivin.db.rename_table(old, new)
			kanivin.db.commit()

		# Do not rename and move files and folders for custom doctype
		if not self.custom:
			if not kanivin.flags.in_patch:
	...
	def rename_files_and_folders(self, old, new):
		...
		for fname in os.listdir(new_path):
			if kanivin.scrub(old) in fname:
				old_file_name = os.path.join(new_path, fname)
				new_file_name = os.path.join(new_path, fname.replace(kanivin.scrub(old), kanivin.scrub(new)))
				shutil.move(old_file_name, new_file_name)

		self.rename_inside_controller(new, old, new_path)
		kanivin.msgprint(_("Renamed files and replaced code in controllers, please check!"))

	def rename_inside_controller(self, new, old, new_path):
		for fname in ("{}.js", "{}.py", "{}_list.js", "{}_calendar.js", "test_{}.py", "test_{}.js"):
			fname = os.path.join(new_path, fname.format(kanivin.scrub(new)))
				...
				with open(fname, "w") as f:
					if fname.endswith(".js"):
						...
						file_content = code.replace(
							kanivin.scrub(old), kanivin.scrub(new)
		...
		doctype_json_path = os.path.join(new_path, f"{kanivin.scrub(new)}.json")
		current_data = kanivin.get_file_json(doctype_json_path)
	...
	def preserve_naming_series_options_in_property_setter(self):
		...
		if not (
			kanivin.db.table_exists(self.name)
			and kanivin.get_all(self.name, fields=["name"], limit=1, as_list=True)
		):
			return

		existing_property_setter = kanivin.db.get_value(
	...
	def make_controller_template(self):
		...
		if self.has_web_view:
			templates_path = kanivin.get_module_path(
				kanivin.scrub(self.module), "doctype", kanivin.scrub(self.name), "templates"
			)
	...
	def export_types_to_controller(self):
		from kanivin.modules.utils import get_module_app
		from kanivin.types.exporter import TypeExporter
		...
		if any(kanivin.get_hooks("export_python_type_annotations", app_name=app)):
			TypeExporter(self).export_types()

	def make_amendable(self):
		"""If is_submittable is set, add amended_from docfields."""
		if self.is_submittable:
			docfield_exists = kanivin.get_all(
	...
	def make_repeatable(self):
		...
		if self.allow_auto_repeat:
			if not kanivin.db.exists(
				"Custom Field", {"fieldname": "auto_repeat", "dt": self.name}
			) and not kanivin.db.exists("DocField", {"fieldname": "auto_repeat", "parent": self.name}):
	...
	def validate_nestedset(self):
		...
		if not self.nsm_parent_field:
			field_label = kanivin.bold(_("Parent Field (Tree)"))
			kanivin.throw(_("{0} is a mandatory field").format(field_label), kanivin.MandatoryError)

		# check if field is valid
		fieldnames = [df.fieldname for df in self.fields]
		if self.nsm_parent_field and self.nsm_parent_field not in fieldnames:
			kanivin.throw(_("Parent Field must be a valid fieldname"), InvalidFieldNameError)
	...
	def add_nestedset_fields(self):
		...
		parent_field_name = kanivin.scrub(parent_field_label)
		self.append(
			"fields",
			{
	...
	def get_max_idx(self):
		...
		max_idx = kanivin.db.sql("""select max(idx) from `tabDocField` where parent = %s""", self.name)
	...
	def validate_name(self, name=None):
		...
		max_length = kanivin.db.MAX_COLUMN_LENGTH - 3
		...
		if WHITESPACE_PADDING_PATTERN.search(name):
			kanivin.throw(_("DocType's name should not start or end with whitespace"), kanivin.NameError)
	...
	def check_pending_migration(self) -> bool:
		...
		file = Path(get_file_path(kanivin.scrub(self.module), self.doctype, self.name))
...
def validate_series(dt, autoname=None, name=None):
	...
	if not autoname and dt.get("fields", {"fieldname": "naming_series"}):
		...
		if not dt.get("fields", {"fieldname": fieldname}):
			kanivin.throw(
				_("Fieldname called {0} must exist to enable autonaming").format(kanivin.bold(fieldname)),
	...
	if autoname and autoname.startswith("field:"):
		...
		if not field or field not in [df.fieldname for df in dt.fields]:
			kanivin.throw(_("Invalid fieldname '{0}' in autoname").format(field))
		else:
			for df in dt.fields:
	...
	if (
		...
		used_in = (
			kanivin.qb.from_(doctype)
			.select(doctype.name)
			.where(doctype.autoname.like(Concat(prefix, ".%")))
			.where(doctype.name != name)
		).run()
		if used_in:
			kanivin.throw(_("Series {0} already used in {1}").format(prefix, used_in[0][0]))
...
def validate_autoincrement_autoname(dt: Union[DocType, "CustomizeForm"]) -> bool:
	...
	def get_autoname_before_save(dt: Union[DocType, "CustomizeForm"]) -> str:
		if dt.doctype == "Customize Form":
			property_value = kanivin.db.get_value(
			...
			if not property_value:
				return kanivin.db.get_value("DocType", dt.doc_type, "autoname") or ""
	...
	if not dt.is_new():
		...
		if (
			...
			if dt.doctype == "Customize Form":
				kanivin.throw(_("Cannot change to/from autoincrement autoname in Customize Form"))

			if kanivin.get_meta(dt.name).issingle:
				return False

			if not kanivin.get_all(dt.name, limit=1):
...
def change_name_column_type(doctype_name: str, type: str) -> None:
	...
	kanivin.db.change_column_type(*args)


def validate_links_table_fieldnames(meta):
	"""Validate fieldnames in Links table"""
	if not meta.links or kanivin.flags.in_patch or kanivin.flags.in_fixtures or kanivin.flags.in_migrate:
		return
	...
	for index, link in enumerate(meta.links, 1):
		...
		if not link.parent_doctype:
			...
			kanivin.throw(message, kanivin.ValidationError, _("Parent Missing"))

		if not link.table_fieldname:
			message = _("Document Links Row #{0}: Table Fieldname is mandatory for internal links").format(
				index
			)
			kanivin.throw(message, kanivin.ValidationError, _("Table Fieldname Missing"))

		if meta.name == link.parent_doctype:
			field_exists = link.table_fieldname in fieldnames
		else:
			field_exists = kanivin.get_meta(link.parent_doctype).has_field(link.table_fieldname)

		if not field_exists:
			message = _("Document Links Row #{0}: Could not find field {1} in {2} DocType").format(
				index, kanivin.bold(link.table_fieldname), kanivin.bold(meta.name)
			)
			kanivin.throw(message, kanivin.ValidationError, _("Invalid Table Fieldname"))
...
def _test_connection_query(doctype, field, idx):
	...
	try:
		kanivin.get_all(doctype, filters=filters, limit=1, distinct=True, ignore_ifnull=True)
	except Exception as e:
		kanivin.clear_last_message()
		msg = _("Document Links Row #{0}: Invalid doctype or fieldname.").format(idx)
		msg += "<br>" + str(e)
		kanivin.throw(msg, InvalidFieldNameError)


def validate_fields_for_doctype(doctype):
	meta = kanivin.get_meta(doctype, cached=False)
...
def validate_fields(meta: Meta):
	...
	:param meta: `kanivin.model.meta.Meta` object to check."""
		...
		if fieldname in Document._reserved_keywords:
			kanivin.throw(
				_("{0}: fieldname cannot be set to reserved keyword {1}").format(
					kanivin.bold(docname),
					kanivin.bold(fieldname),
				),
	...
	def check_link_table_options(docname, d):
		if kanivin.flags.in_patch or kanivin.flags.in_fixtures:
			return
		...
		if d.fieldtype in ("Link", *table_fields):
			...
			if d.options != d.parent:
				options = kanivin.db.get_value("DocType", d.options, "name")
				if not options:
					kanivin.throw(
	...
	def check_hidden_and_mandatory(docname, d):
		if d.hidden and d.reqd and not d.default and not kanivin.flags.in_migrate:
			kanivin.throw(
	...
	def check_width(d):
		if d.fieldtype == "Currency" and cint(d.width) < 100:
			kanivin.throw(_("Max width for type Currency is 100px in row {0}").format(d.idx))
	...
	def check_precision(d):
		if (
			...
			kanivin.throw(_("Precision should be between 1 and 6"))
	...
	def check_unique_and_text(docname, d):
		...
		if getattr(d, "unique", False):
			...
			if not d.get("__islocal") and kanivin.db.has_column(d.parent, d.fieldname):
				has_non_unique_values = kanivin.db.sql(
	...
	def check_fold(fields):
		...
		for i, f in enumerate(fields):
			if f.fieldtype == "Fold":
				if fold_exists:
					kanivin.throw(_("There can be only one Fold in a form"))
				fold_exists = True
				if i < len(fields) - 1:
					nxt = fields[i + 1]
					if nxt.fieldtype != "Section Break":
						kanivin.throw(_("Fold must come before a Section Break"))
				else:
					kanivin.throw(_("Fold can not be at the end of the form"))
	...
	def check_search_fields(meta, fields):
		...
		for fieldname in search_fields:
			...
			if (fieldtype_mapper.get(fieldname) in no_value_fields) or (fieldname not in fieldname_list):
				kanivin.throw(_("Search field {0} is not valid").format(fieldname))
	...
	def check_title_field(meta):
		...
		if meta.title_field not in fieldname_list:
			kanivin.throw(_("Title field must be a valid fieldname"), InvalidFieldNameError)
	...
	def check_image_field(meta):
		...
		if not df:
			kanivin.throw(_("Image field must be a valid fieldname"), InvalidFieldNameError)
		if df[0].fieldtype != "Attach Image":
			kanivin.throw(_("Image field must be of type Attach Image"), InvalidFieldNameError)
	...
	def check_is_published_field(meta):
		...
		if meta.is_published_field not in fieldname_list:
			kanivin.throw(_("Is Published Field must be a valid fieldname"), InvalidFieldNameError)
	...
	def check_website_search_field(meta):
		...
		if meta.website_search_field not in fieldname_list:
			kanivin.throw(_("Website Search Field must be a valid fieldname"), InvalidFieldNameError)
	...
	def check_timeline_field(meta):
		...
		if meta.timeline_field not in fieldname_list:
			kanivin.throw(_("Timeline field must be a valid fieldname"), InvalidFieldNameError)

		df = meta.get("fields", {"fieldname": meta.timeline_field})[0]
		if df.fieldtype not in ("Link", "Dynamic Link"):
			kanivin.throw(_("Timeline field must be a Link or Dynamic Link"), InvalidFieldNameError)
	...
	def check_illegal_depends_on_conditions(docfield):
		...
		for field in depends_on_fields:
			...
			if depends_on and ("=" in depends_on) and DEPENDS_ON_PATTERN.match(depends_on):
				kanivin.throw(_("Invalid {0} condition").format(kanivin.unscrub(field)), kanivin.ValidationError)
	...
	def validate_fetch_from(field):
		...
		if source_field == field.fieldname:
			...
			kanivin.throw(msg, title=_("Recursive Fetch From"))
	...
	def validate_data_field_type(docfield):
		...
		if docfield.fieldtype == "Data" and not (docfield.oldfieldtype and docfield.oldfieldtype != "Data"):
			if docfield.options and (docfield.options not in data_field_options):
				df_str = kanivin.bold(_(docfield.label, context=docfield.parent))
				text_str = (
				...
				kanivin.msgprint(text_str + df_options_str, title="Invalid Data Field", alert=True)

	def check_child_table_option(docfield):
		if kanivin.flags.in_fixtures:
			return
		if docfield.fieldtype not in ["Table MultiSelect", "Table"]:
			return

		doctype = docfield.options
		child_doctype_meta = kanivin.get_meta(doctype)

		if not child_doctype_meta.istable:
			kanivin.throw(
				_("Option {0} for field {1} is not a child table").format(
					kanivin.bold(doctype), kanivin.bold(docfield.fieldname)
				),
	...
	def check_max_height(docfield):
		if getattr(docfield, "max_height", None) and (docfield.max_height[-2:] not in ("px", "em")):
			kanivin.throw(f"Max for {kanivin.bold(docfield.fieldname)} height must be in px, em, rem")
	...
	def check_no_of_ratings(docfield):
		if docfield.fieldtype == "Rating":
			if docfield.options and (int(docfield.options) > 10 or int(docfield.options) < 3):
				kanivin.throw(_("Options for Rating field can range from 3 to 10"))
	...
	for d in fields:
		...
		if not kanivin.flags.in_migrate:
...
def validate_permissions_for_doctype(doctype, for_remove=False, alert=False):
	...
	doctype = kanivin.get_doc("DocType", doctype)
...
def validate_permissions(doctype, for_remove=False, alert=False):
	...
	if not permissions and alert:
		kanivin.msgprint(_("No Permissions Specified"), alert=True, indicator="orange")
	...
	def check_atleast_one_set(d):
		if not d.select and not d.read and not d.write and not d.submit and not d.cancel and not d.create:
			kanivin.throw(_("{0}: No basic permissions set").format(get_txt(d)))
	...
	def check_permission_dependency(d):
		if d.cancel and not d.submit:
			kanivin.throw(_("{0}: Cannot set Cancel without Submit").format(get_txt(d)))

		if (d.submit or d.cancel or d.amend) and not d.write:
			kanivin.throw(_("{0}: Cannot set Submit, Cancel, Amend without Write").format(get_txt(d)))
		if d.amend and not d.write:
			kanivin.throw(_("{0}: Cannot set Amend without Cancel").format(get_txt(d)))
		if d.get("import") and not d.create:
			kanivin.throw(_("{0}: Cannot set Import without Create").format(get_txt(d)))
	...
	def remove_rights_for_single(d):
		...
		if d.report:
			kanivin.msgprint(_("Report cannot be set for Single types"))
			d.report = 0
			d.set("import", 0)
			d.set("export", 0)
	...
	def check_if_submittable(d):
		if d.submit and not issubmittable:
			kanivin.throw(_("{0}: Cannot set Assign Submit if not Submittable").format(get_txt(d)))
		elif d.amend and not issubmittable:
			kanivin.throw(_("{0}: Cannot set Assign Amend if not Submittable").format(get_txt(d)))

	def check_if_importable(d):
		if d.get("import") and not isimportable:
			kanivin.throw(_("{0}: Cannot set import as {1} is not importable").format(get_txt(d), doctype))
		...
		if doctype.custom:
			if d.role in AUTOMATIC_ROLES:
				kanivin.throw(
					...
					).format(d.idx, kanivin.bold(_(d.role))),
					title=_("Permissions Error"),
				)

			roles = [row.name for row in kanivin.get_all("Role", filters={"is_custom": 1})]
...
def make_module_and_roles(doc, perm_fieldname="permissions"):
	...
	try:
		if (
			...
			and not kanivin.db.exists("Domain", doc.restrict_to_domain)
		):
			kanivin.get_doc(dict(doctype="Domain", domain=doc.restrict_to_domain)).insert()

		if "tabModule Def" in kanivin.db.get_tables() and not kanivin.db.exists("Module Def", doc.module):
			m = kanivin.get_doc({"doctype": "Module Def", "module_name": doc.module})
			if kanivin.scrub(doc.module) in kanivin.local.module_app:
				m.app_name = kanivin.local.module_app[kanivin.scrub(doc.module)]
			else:
				m.app_name = "kanivin"
			m.flags.ignore_mandatory = m.flags.ignore_permissions = True
			if kanivin.flags.package:
				m.package = kanivin.flags.package.name
				m.custom = 1
			m.insert()
		...
		for role in list(set(roles)):
			if kanivin.db.table_exists("Role", cached=False) and not kanivin.db.exists("Role", role):
				r = kanivin.new_doc("Role")
				r.role_name = role
				r.desk_access = 1
		...
		pass
	except kanivin.db.ProgrammingError as e:
		if kanivin.db.is_table_missing(e):
			pass
		else:
			raise
...
def check_fieldname_conflicts(docfield):
	...
	doc = kanivin.get_doc({"doctype": docfield.dt})
	...
	if docfield.fieldname in method_list + property_list:
		kanivin.msgprint(msg, raise_exception=not docfield.is_virtual)


def clear_linked_doctype_cache():
	kanivin.cache.delete_value("linked_doctypes_without_ignore_user_permissions_enabled")
...
def check_email_append_to(doc):
	...
	if doc.subject_field and not subject_field:
		kanivin.throw(_("Select a valid Subject field for creating documents from Email"))
	...
	if subject_field and subject_field.fieldtype not in [
		...
		kanivin.throw(_("Subject Field type should be Data, Text, Long Text, Small Text, Text Editor"))
	...
	if doc.sender_field and not sender_field:
		kanivin.throw(_("Select a valid Sender Field for creating documents from Email"))

	if not sender_field.options == "Email":
		kanivin.throw(_("Sender Field should have Email in options"))
...
def get_row_size_utilization(doctype: str) -> float:
	...
	try:
		return flt(kanivin.db.get_row_size(doctype) / kanivin.db.MAX_ROW_SIZE_LIMIT * 100, 2)
	except Exception:
		return 0.0
