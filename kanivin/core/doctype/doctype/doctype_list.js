kanivin.listview_settings["DocType"] = {
	primary_action: function () {
		this.new_doctype_dialog();
	},

	new_doctype_dialog() {
		let non_developer = kanivin.session.user !== "Administrator" || !kanivin.boot.developer_mode;
		let fields = [
			{
				label: __("Name"),
				fieldname: "name",
		...
		let new_d = new kanivin.ui.Dialog({
			...
			primary_action(values) {
				...
				kanivin.db
					...
					.then((doc) => {
						kanivin.set_route("Form", "DocType", doc.name);
					});
			},
			secondary_action_label: __("Cancel"),
			secondary_action() {
				new_d.hide();
				if (kanivin.get_route()[0] === "Form") {
