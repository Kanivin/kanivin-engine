...
kanivin.ui.form.on("DocType", {
	onload: function (frm) {
		if (frm.is_new() && !frm.doc?.fields) {
			kanivin.listview_settings["DocType"].new_doctype_dialog();
		}
	...
	after_save: function (frm) {
		if (
			kanivin.form_builder &&
			kanivin.form_builder.doctype === frm.doc.name &&
			kanivin.form_builder.store
		) {
	...
	refresh: function (frm) {
		frm.set_query("role", "permissions", function (doc) {
			if (doc.custom && kanivin.session.user != "Administrator") {
				return {
					query: "kanivin.core.doctype.role.role.role_query",
					filters: [["Role", "name", "!=", "All"]],
				};
			}
		});

		if (kanivin.session.user !== "Administrator" || !kanivin.boot.developer_mode) {
			if (frm.is_new()) {
		...
		if (!frm.is_new() && !frm.doc.istable) {
			if (frm.doc.issingle) {
				frm.add_custom_button(__("Go to {0}", [__(frm.doc.name)]), () => {
					window.open(`/app/${kanivin.router.slug(frm.doc.name)}`);
				});
			} else {
		...
		if (!kanivin.boot.developer_mode && !frm.doc.custom) {
		...
		} else if (kanivin.boot.developer_mode) {
			frm.dashboard.clear_comment();
			let msg = __(
	...
	setup_default_views: (frm) => {
		kanivin.model.set_default_views_for_doctype(frm.doc.name, frm);
	},
...
function render_form_builder(frm) {
	if (kanivin.form_builder && kanivin.form_builder.doctype === frm.doc.name) {
	...
	if (kanivin.form_builder) {
		...
		kanivin.form_builder.doctype = frm.doc.name;
		kanivin.form_builder.customize = false;
