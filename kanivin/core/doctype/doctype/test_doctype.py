...
import kanivin
from kanivin.cache_manager import clear_doctype_cache
from kanivin.core.doctype.doctype.doctype import (
	CannotIndexedError,
	DocType,
	DoctypeLinkError,
...
from kanivin.core.doctype.rq_job.test_rq_job import wait_for_completion
from kanivin.custom.doctype.custom_field.custom_field import create_custom_fields
from kanivin.desk.form.load import getdoc
from kanivin.model.delete_doc import delete_controllers
from kanivin.model.sync import remove_orphan_doctypes
from kanivin.tests.utils import KanivinTestCase


class TestDocType(KanivinTestCase):
	def tearDown(self):
		kanivin.db.rollback()

	def test_validate_name(self):
		self.assertRaises(kanivin.NameError, new_doctype("_Some DocType").insert)
		self.assertRaises(kanivin.NameError, new_doctype("8Some DocType").insert)
		self.assertRaises(kanivin.NameError, new_doctype("Some (DocType)").insert)
		self.assertRaises(
			kanivin.NameError,
		...
		for name in ("Some DocType", "Some_DocType", "Some-DocType"):
			if kanivin.db.exists("DocType", name):
				kanivin.delete_doc("DocType", name)

			doc = new_doctype(name).insert()
			doc.delete()

	def test_making_sequence_on_change(self):
		kanivin.delete_doc_if_exists("DocType", self._testMethodName)
		...
		self.assertEqual(
			...
			"varchar" if kanivin.db.db_type == "mariadb" else "character varying",
		)

	def test_doctype_unique_constraint_dropped(self):
		if kanivin.db.exists("DocType", "With_Unique"):
			kanivin.delete_doc("DocType", "With_Unique")

		dt = new_doctype("With_Unique", unique=1)
		dt.insert()

		doc1 = kanivin.new_doc("With_Unique")
		doc2 = kanivin.new_doc("With_Unique")
		...
		doc1.insert()
		self.assertRaises(kanivin.UniqueValidationError, doc2.insert)
		kanivin.db.rollback()
	...
	def test_validate_search_fields(self):
		...
		self.assertRaises(kanivin.ValidationError, doc.save)
	...
	@unittest.skipUnless(
		os.access(kanivin.get_app_path("kanivin"), os.W_OK), "Only run if kanivin app paths is writable"
	)
	def test_sync_field_order(self):
		...
		kanivin.delete_doc_if_exists("DocType", "Test Field Order DocType")
		if os.path.isfile(path):
			os.remove(path)

		try:
			kanivin.flags.allow_doctype_export = 1
			test_doctype.save()

			# assert that field_order list is being created with the default order
			test_doctype_json = kanivin.get_file_json(path)
			...
			with open(path, "w+") as txtfile:
				txtfile.write(kanivin.as_json(test_doctype_json))
			...
			kanivin.reload_doctype(test_doctype.name, force=True)
			test_doctype.reload()
			...
			kanivin.flags.allow_doctype_export = 0
	...
	def test_cancel_link_doctype(self):
		...
		from kanivin.desk.form.linked_with import cancel_all_linked_docs, get_submitted_linked_docs
		...
		# create doctype data
		data_link_doc = kanivin.new_doc("Test Linked Doctype")
		data_link_doc.some_fieldname = "Data1"
		data_link_doc.insert()
		data_link_doc.save()
		data_link_doc.submit()

		data_doc = kanivin.new_doc("Test Doctype")
	...
	def test_ignore_cancelation_of_linked_doctype_during_cancel(self):
		...
		data_doc = kanivin.new_doc("Test Doctype 2")
		data_doc.some_fieldname = "Data1"
		...
		self.assertRaises(kanivin.LinkExistsError, data_link_doc_1.cancel)
	...
	def test_create_virtual_doctype(self):
		...
		virtual_doc.reload()
		doc = kanivin.get_doc("DocType", "Test Virtual Doctype")

		self.assertDictEqual(doc.as_dict(), virtual_doc.as_dict())
		self.assertEqual(doc.is_virtual, 1)
		self.assertFalse(kanivin.db.table_exists("Test Virtual Doctype"))

	def test_create_virtual_doctype_as_child_table(self):
		"""Test virtual DocType as Child Table below a normal DocType."""
		kanivin.delete_doc_if_exists("DocType", "Test Parent Virtual DocType", force=1)
		kanivin.delete_doc_if_exists("DocType", "Test Virtual DocType as Child Table", force=1)
		...
		doc = kanivin.get_doc("DocType", "Test Virtual DocType as Child Table")

		self.assertEqual(doc.is_virtual, 1)
		self.assertEqual(doc.istable, 1)
		self.assertFalse(kanivin.db.table_exists("Test Virtual DocType as Child Table"))
		...
		self.assertRaises(kanivin.exceptions.ValidationError, parent_doc.insert)
		parent_doc.is_virtual = 1
		parent_doc.insert(ignore_permissions=True)
		self.assertFalse(kanivin.db.table_exists("Test Parent Virtual DocType"))
	...
	def test_autoincremented_doctype_transition(self):
		kanivin.delete_doc_if_exists("DocType", "testy_autoinc_dt")
		dt = new_doctype("testy_autoinc_dt", autoname="autoincrement").insert(ignore_permissions=True)
		dt.autoname = "hash"

		dt.save(ignore_permissions=True)

		dt_data = kanivin.get_doc({"doctype": dt.name, "some_fieldname": "test data"}).insert(
		...
		try:
			dt.save(ignore_permissions=True)
		except kanivin.ValidationError as e:
			self.assertEqual(
				e.args[0],
	...
	def test_json_field(self):
		...
		json_doc.insert()
		json_doc.save()
		doc = kanivin.get_doc("DocType", "Test Json Doctype")
		...
		test_json = kanivin.get_doc("Test Json Doctype", doc.name)
	...
	def test_no_delete_doc(self):
		self.assertRaises(kanivin.ValidationError, kanivin.delete_doc, "DocType", "Address")

	@unittest.skipUnless(
		os.access(kanivin.get_app_path("kanivin"), os.W_OK), "Only run if kanivin app paths is writable"
	)
	@patch.dict(kanivin.conf, {"developer_mode": 1})
	...
	def test_custom_field_deletion(self):
		...
		kanivin.delete_doc("DocType", child)
		self.assertFalse(kanivin.get_meta(doctype).get_field(field))
	...
	def test_delete_doctype_with_customization(self):
		...
		# Create 1 record
		original_doc = kanivin.get_doc(doctype=doctype, custom_field_name="wat").insert()
		self.assertEqual(original_doc.some_fieldname, "DELETETHIS")

		# delete doctype
		kanivin.delete_doc("DocType", doctype)
		...
		restored_doc = kanivin.get_last_doc(doctype)
		...
		self.assertEqual(kanivin.get_meta(doctype).get_field(field).default, "DELETETHIS")
		kanivin.delete_doc("DocType", doctype)
	...
	def test_delete_orphaned_doctypes(self):
		...
		job = kanivin.enqueue(remove_orphan_doctypes)
		wait_for_completion(job)

		kanivin.db.rollback()
		self.assertFalse(kanivin.db.exists("DocType", doctype.name))
	...
	def test_not_in_list_view_for_not_allowed_mandatory_field(self):
		...
		kanivin.delete_doc("DocType", doctype.name)
	...
	def test_no_recursive_fetch(self):
		...
		self.assertRaises(kanivin.ValidationError, recursive_dt.insert)
