...
from kanivin.database.schema import SPECIAL_CHAR_PATTERN
from kanivin.model.document import Document
from kanivin.permissions import SYSTEM_USER_ROLE, get_doctypes_with_read
from kanivin.utils import call_hook_method, cint, get_files_path, get_hook_method, get_url
from kanivin.utils.file_manager import is_safe_path
...
class File(Document):
	...
	def autoname(self):
		...
		if self.is_folder:
			...
			self.name = kanivin.generate_hash(length=10)
	...
	def before_insert(self):
		...
		if self.is_remote_file:
			...
			kanivin.db.after_rollback.add(self.on_rollback)
	...
	def validate(self):
		...
		self.file_size = kanivin.form_dict.file_size or self.file_size
	...
	def validate_attachment_references(self):
		...
		if not self.attached_to_name or not isinstance(self.attached_to_name, str | int):
			kanivin.throw(_("Attached To Name must be a string or an integer"), kanivin.ValidationError)

		if self.attached_to_field and SPECIAL_CHAR_PATTERN.search(self.attached_to_field):
			kanivin.throw(_("The fieldname you've specified in Attached To Field is invalid"))
	...
	def on_trash(self):
		if self.is_home_folder or self.is_attachments_folder:
			kanivin.throw(_("Cannot delete Home and Attachments folders"))
	...
	def get_successors(self):
		return kanivin.get_all("File", filters={"folder": self.name}, pluck="name")
	...
	def handle_is_private_changed(self):
		...
		private_file_path = Path(kanivin.get_site_path("private", "files", file_name))
		public_file_path = Path(kanivin.get_site_path("public", "files", file_name))
		...
		if kanivin.get_meta(self.attached_to_doctype).issingle:
	...
	def fetch_attached_to_field(self, old_file_url):
		...
		reference_dict = kanivin.get_doc(self.attached_to_doctype, self.attached_to_name).as_dict()
	...
	def validate_attachment_limit(self):
		...
		if self.attached_to_doctype and self.attached_to_name:
			attachment_limit = cint(kanivin.get_meta(self.attached_to_doctype).max_attachments)
			...
			if current_attachment_count >= attachment_limit:
				kanivin.throw(
					_("Maximum Attachment Limit of {0} has been reached for {1} {2}.").format(
						kanivin.bold(attachment_limit), self.attached_to_doctype, self.attached_to_name
					),
	...
	def set_folder_name(self):
		...
		if self.attached_to_doctype:
			self.folder = kanivin.db.get_value("File", {"is_attachments_folder": 1})
	...
	def validate_file_on_disk(self):
		...
		if not os.path.exists(full_path):
			kanivin.throw(_("File {0} does not exist").format(self.file_url), IOError)

	def validate_file_extension(self):
		# Only validate uploaded files, not generated by code/integrations.
		if not self.file_type or not kanivin.request:
			return

		allowed_extensions = kanivin.get_system_settings("allowed_file_extensions")
		if not allowed_extensions:
			return

		if self.file_type not in allowed_extensions.splitlines():
			kanivin.throw(_("File type of {0} is not allowed").format(self.file_type), exc=FileTypeNotAllowed)
	...
	def validate_duplicate_entry(self):
		if not self.flags.ignore_duplicate_entry_error and not self.is_folder:
			...
			duplicate_file = kanivin.db.get_value("File", filters, ["name", "file_url"], as_dict=1)

			if duplicate_file:
				duplicate_file_doc = kanivin.get_cached_doc("File", duplicate_file.name)
	...
	def set_file_name(self):
		if not self.file_name and not self.file_url:
			kanivin.throw(
				_("Fields `file_name` or `file_url` must be set for File"), exc=kanivin.MandatoryError
			)
	...
	def generate_content_hash(self):
		...
		try:
			...
			kanivin.throw(_("File {0} does not exist").format(file_path))
	...
	def make_thumbnail(
		...
		path = os.path.abspath(kanivin.get_site_path("public", thumbnail_url.lstrip("/")))
		...
		try:
			...
			kanivin.msgprint(_("Unable to write file format for {0}").format(path))
			return
	...
	def validate_empty_folder(self):
		...
		if self.is_folder and kanivin.get_all("File", filters={"folder": self.name}, limit=1):
			kanivin.throw(_("Folder {0} is not empty").format(self.name), FolderNotEmpty)

	def _delete_file_on_disk(self):
		"""If file not attached to any other record, delete it"""
		on_disk_file_not_shared = self.content_hash and not kanivin.get_all(
			"File",
			filters={
				"content_hash": self.content_hash,
				"name": ["!=", self.name],
	...
	def unzip(self) -> list["File"]:
		...
		if not self.file_url.endswith(".zip"):
			kanivin.throw(_("{0} is not a zip file").format(self.file_name))
		...
		with zipfile.ZipFile(zip_path) as z:
			for file in z.filelist:
				...
				file_doc = kanivin.new_doc("File")
				try:
					file_doc.content = z.read(file.filename)
				except zipfile.BadZipFile:
					kanivin.throw(_("{0} is a not a valid zip file").format(self.file_name))
		...
		kanivin.delete_doc("File", self.name)
		return files
	...
	def get_content(self) -> bytes:
		if self.is_folder:
			kanivin.throw(_("Cannot get file contents of a Folder"))
	...
	def get_full_path(self):
		...
		if file_path.startswith("/private/files/"):
			...
			kanivin.throw(_("There is some problem with the file url: {0}").format(file_path))

		if not is_safe_path(file_path):
			kanivin.throw(_("Cannot access file path {0}").format(file_path))

		if os.path.sep in self.file_name:
			kanivin.throw(_("File name cannot have {0}").format(os.path.sep))
	...
	def save_file(
		...
		if (
			...
			and kanivin.get_system_settings("strip_exif_metadata_from_uploaded_images")
		):
		...
		if not ignore_existing_file_check:
			duplicate_file = kanivin.get_value(
				"File",
		...
		if duplicate_file:
			file_doc: "File" = kanivin.get_cached_doc("File", duplicate_file.name)
	...
	def check_max_file_size(self):
		...
		if file_size > max_file_size:
			...
			if kanivin.has_permission("System Settings", "write"):
				msg += ".<br>" + _("You can increase the limit from System Settings.")
			kanivin.throw(msg, exc=MaxFileSizeReachedError)
	...
	def create_attachment_record(self):
		...
		file_url = quote(kanivin.safe_encode(self.file_url), safe="/:") if self.file_url else self.file_name
	...
	def add_comment_in_reference_doc(self, comment_type, text):
		if self.attached_to_doctype and self.attached_to_name:
			try:
				doc = kanivin.get_doc(self.attached_to_doctype, self.attached_to_name)
	...
	def zip_files(files):
		...
		for _file in files:
			if isinstance(_file, str):
				_file = kanivin.get_doc("File", _file)
...
def on_doctype_update():
	kanivin.db.add_index("File", ["attached_to_doctype", "attached_to_name"])
	kanivin.db.add_index("File", ["file_url(100)"])
...
def has_permission(doc, ptype=None, user=None, debug=False):
	...
	if ptype == "create":
		return kanivin.has_permission("File", "create", user=user, debug=debug)
	...
	if doc.attached_to_doctype and doc.attached_to_name:
		...
		try:
			ref_doc = kanivin.get_doc(attached_to_doctype, attached_to_name)
...
def get_permission_query_conditions(user: str | None = None) -> str:
	...
	if SYSTEM_USER_ROLE not in kanivin.get_roles(user):
		return f""" `tabFile`.`owner` = {kanivin.db.escape(user)} """

	readable_doctypes = ", ".join(repr(dt) for dt in get_doctypes_with_read())
	return f"""
		(`tabFile`.`is_private` = 0)
		OR (`tabFile`.`attached_to_doctype` IS NULL AND `tabFile`.`owner` = {kanivin.db.escape(user)})
