...
from kanivin.utils import cint, cstr, encode, get_files_path, random_string, strip
from kanivin.utils.file_manager import safe_b64decode
...
def setup_folder_path(filename: str, new_parent: str) -> None:
	file: "File" = kanivin.get_doc("File", filename)
...
def get_local_image(file_url: str) -> tuple["ImageFile", str, str]:
	...
	file_path = kanivin.get_site_path(*file_url_path)

	try:
		image = Image.open(file_path)
	except OSError:
		kanivin.throw(_("Unable to read file format for {0}").format(file_url))
...
def get_web_image(file_url: str) -> tuple["ImageFile", str, str]:
	...
	file_url = kanivin.utils.get_url(file_url)
	r = requests.get(file_url, stream=True)
	try:
		r.raise_for_status()
	except requests.exceptions.HTTPError as e:
		if "404" in e.args[0]:
			kanivin.msgprint(_("File '{0}' not found").format(file_url))
		else:
			kanivin.msgprint(_("Unable to read file format for {0}").format(file_url))
		raise

	try:
		image = Image.open(BytesIO(r.content))
	except Exception as e:
		kanivin.msgprint(_("Image link '{0}' is not valid").format(file_url), raise_exception=e)
...
def remove_file_by_url(file_url: str, doctype: str | None = None, name: str | None = None) -> "Document":
	if doctype and name:
		fid = kanivin.db.get_value(
			"File", {"file_url": file_url, "attached_to_doctype": doctype, "attached_to_name": name}
		)
	else:
		fid = kanivin.db.get_value("File", {"file_url": file_url})

	if fid:
		from kanivin.utils.file_manager import remove_file
...
def get_file_name(fname: str, optional_suffix: str | None = None) -> str:
	...
	suffix = optional_suffix or kanivin.generate_hash(length=6)
...
def extract_images_from_html(doc: "Document", content: str, is_private: bool = False):
	...
	def _save_file(match):
		...
		try:
			...
			kanivin.flags.has_dataurl = True
...
def update_existing_file_docs(doc: "File") -> None:
	...
	file_doctype = kanivin.qb.DocType("File")
	(
		kanivin.qb.update(file_doctype)
...
def attach_files_to_document(doc: "Document", event) -> None:
	...
	for df in attach_fields:
		...
		unattached_file = kanivin.db.exists(
			"File",
			{
				"file_url": value,
...
def relink_files(doc, fieldname, temp_doc_name):
	...
	if not temp_doc_name:
		return
	from kanivin.utils.data import add_to_date, now_datetime

	mislinked_file = kanivin.db.get_value(
		"File",
		{
...
def find_file_by_url(path: str, name: str | None = None) -> Optional["File"]:
	...
	files = kanivin.get_all("File", filters=filters, fields="*")

	# this file might be attached to multiple documents
	# if the file is accessible from any one of those documents
	# then it should be downloadable
	for file_data in files:
		file: "File" = kanivin.get_doc(doctype="File", **file_data)
