kanivin.ui.form.on("File", {
	refresh: function (frm) {
		if (frm.doc.file_url) {
			frm.add_custom_button(__("View File"), () => {
				if (!kanivin.utils.is_url(frm.doc.file_url)) {
	...
	preview_file: function (frm) {
		...
		if (kanivin.utils.is_image_file(frm.doc.file_url)) {
			$preview = $(`<div class="img_preview">
				<img
					class="img-responsive"
					src="${kanivin.utils.escape_html(frm.doc.file_url)}"
					onerror="${frm.toggle_display("preview", false)}"
				/>
			</div>`);
		} else if (kanivin.utils.is_video_file(frm.doc.file_url)) {
			$preview = $(`<div class="img_preview">
				<video width="480" height="320" controls>
					<source src="${kanivin.utils.escape_html(frm.doc.file_url)}">
			...
			$preview = $(`<div class="img_preview">
				<object style="background:#323639;" width="100%">
					<embed
						...
						width="100%"
						height="1190"
						src="${kanivin.utils.escape_html(frm.doc.file_url)}" type="application/pdf"
					>
				</object>
			</div>`);
			...
			$preview = $(`<div class="img_preview">
				<audio width="480" height="60" controls>
					<source src="${kanivin.utils.escape_html(frm.doc.file_url)}" type="audio/mpeg">
	...
	optimize: function (frm) {
		kanivin.show_alert(__("Optimizing image..."));
		frm.call("optimize_file").then(() => {
			kanivin.show_alert(__("Image optimized"));
		});
	},

	unzip: function (frm) {
		kanivin.call({
			method: "kanivin.core.api.file.unzip_file",
			args: {
				name: frm.doc.name,
			},
			callback: function () {
