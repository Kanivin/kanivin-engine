...
from kanivin.core.api.file import (
	create_new_folder,
	get_attached_images,
	get_files_in_folder,
	move_file,
...
from kanivin.core.doctype.file.exceptions import FileTypeNotAllowed
from kanivin.core.doctype.file.utils import get_corrupted_image_msg, get_extension
from kanivin.desk.form.utils import add_comment
from kanivin.exceptions import ValidationError
from kanivin.tests.utils import KanivinTestCase, change_settings
from kanivin.utils import get_files_path, set_request
...
def make_test_doc(ignore_permissions=False):
	d = kanivin.new_doc("ToDo")
	d.description = "Test"
	d.assigned_by = kanivin.session.user
...
def make_test_image_file(private=False):
	file_path = kanivin.get_app_path("kanivin", "tests/data/sample_image_for_optimization.jpg")
	with open(file_path, "rb") as f:
		file_content = f.read()

	test_file = kanivin.get_doc(
		{
			"doctype": "File",
	...
	# remove those flags
	_test_file: "File" = kanivin.get_doc("File", test_file.name)
...
class TestSimpleFile(KanivinTestCase):
	def setUp(self):
	...
	def test_save(self):
		_file = kanivin.get_doc("File", {"file_url": self.saved_file_url})
		content = _file.get_content()
		self.assertEqual(content, self.test_content)


class TestFSRollbacks(KanivinTestCase):
	def test_rollback_from_file_system(self):
		file_name = content = kanivin.generate_hash()
		file = kanivin.new_doc("File", file_name=file_name, content=content).insert()
...
class TestExtensionValidations(KanivinTestCase):
	@change_settings("System Settings", {"allowed_file_extensions": "JPG\nCSV"})
	def test_allowed_extension(self):
		set_request(method="POST", path="/")
		file_name = content = kanivin.generate_hash()
		bad_file = kanivin.new_doc("File", file_name=f"{file_name}.png", content=content)
		self.assertRaises(FileTypeNotAllowed, bad_file.insert)

		bad_file = kanivin.new_doc("File", file_name=f"{file_name}.csv", content=content).insert()
		kanivin.db.rollback()
		self.assertFalse(bad_file.exists_on_disk())


class TestBase64File(KanivinTestCase):
	def setUp(self):
		self.attached_to_doctype, self.attached_to_docname = make_test_doc()
		self.test_content = base64.b64encode(test_content1.encode("utf-8"))
		_file: "File" = kanivin.get_doc(
			{
				"doctype": "File",
...
class TestSameFileName(KanivinTestCase):
	def test_saved_content(self):
		...
		_file = kanivin.get_doc("File", {"file_url": self.saved_file_url1})
		content1 = _file.get_content()
		self.assertEqual(content1, self.test_content1)
		_file = kanivin.get_doc("File", {"file_url": self.saved_file_url2})
	...
	def test_saved_content_private(self):
		...
		_file = kanivin.get_doc("File", {"file_url": _file1.file_url})
		self.assertEqual(_file.get_content(), test_content1)

		_file = kanivin.get_doc("File", {"file_url": _file2.file_url})
		self.assertEqual(_file.get_content(), test_content2)


class TestSameContent(KanivinTestCase):
	def setUp(self):
	...
	def test_attachment_limit(self):
		...
		self.assertRaises(kanivin.exceptions.AttachmentLimitReached, file2.insert)
		limit_property.delete()
		kanivin.clear_cache(doctype="ToDo")


class TestFile(KanivinTestCase):
	def setUp(self):
	...
	def tearDown(self):
		try:
			kanivin.get_doc("File", {"file_name": "file_copy.txt"}).delete()
		except kanivin.DoesNotExistError:
			pass

	def delete_test_data(self):
		test_file_data = kanivin.get_all(
			"File",
			pluck="name",
	...
	def tests_after_upload(self):
		...
		file_folder = kanivin.db.get_value("File", self.saved_name, "folder")
		self.assertEqual(file_folder, _("Home/Test Folder 1"))

	def test_file_copy(self):
		folder = self.get_folder("Test Folder 2", "Home")

		file = kanivin.get_doc("File", {"file_name": "file_copy.txt"})
	...
	def test_folder_copy(self):
		...
		file = kanivin.get_doc("File", {"file_name": "folder_copy.txt"})
		file_copy_txt = kanivin.get_value("File", {"file_name": "file_copy.txt"})
		if file_copy_txt:
			kanivin.get_doc("File", file_copy_txt).delete()

		self.assertEqual(_("Home/Test Folder 1/Test Folder 3"), file.folder)

	def test_default_folder(self):
		d = kanivin.get_doc({"doctype": "File", "file_name": _("Test_Folder"), "is_folder": 1})
		d.save()
	...
	def test_on_delete(self):
		...
		self.assertEqual(kanivin.db.get_value("File", _("Home/Test Folder 1"), "file_size"), 0)
		...
		folder = kanivin.get_doc("File", "Home/Test Folder 1/Test Folder 3")
	...
	def test_same_file_url_update(self):
		...
		file2 = kanivin.get_doc("File", file2.name)
	...
	def test_file_url_validation(self):
		test_file: "File" = kanivin.new_doc("File")
		test_file.update({"file_name": "logo", "file_url": "https://kanivin.io/files/kanivin.png"})
	...
	def test_make_thumbnail(self):
		# test web image
		test_file: "File" = kanivin.get_doc(
			{
				"doctype": "File",
				"file_name": "logo",
				"file_url": kanivin.utils.get_url("/_test/assets/image.jpg"),
			}
		...
		test_file = kanivin.get_doc(
			{
				"doctype": "File",
				"file_name": "logo",
				"file_url": kanivin.utils.get_url("/_test/assets/image"),
			}
		).insert(ignore_permissions=True)
		...
		test_file.db_set("thumbnail_url", None)
		test_file.reload()
		test_file.file_url = kanivin.utils.get_url("unknown.jpg")
		test_file.make_thumbnail(suffix="xs")
		self.assertEqual(
			kanivin.message_log[0].get("message"),
			f"File '{kanivin.utils.get_url('unknown.jpg')}' not found",
		)
		self.assertEqual(test_file.thumbnail_url, None)

	def test_file_unzip(self):
		file_path = kanivin.get_app_path("kanivin", "www/_test/assets/file.zip")
		public_file_path = kanivin.get_site_path("public", "files")
		try:
			import shutil
...
class TestAttachment(KanivinTestCase):
	...
	def tearDownClass(cls):
		kanivin.db.rollback()
		kanivin.delete_doc("DocType", cls.test_doctype)

	def test_file_attachment_on_update(self):
		doc = kanivin.get_doc(doctype=self.test_doctype, title="test for attachment on update").insert()
...
class TestAttachmentsAccess(KanivinTestCase):
	def setUp(self) -> None:
		kanivin.db.delete("File", {"is_folder": 0})

	def test_list_private_attachments(self):
		kanivin.set_user("test4@example.com")
		self.attached_to_doctype, self.attached_to_docname = make_test_doc()
...
class TestFileUtils(KanivinTestCase):
	def test_extract_images_from_doc(self):
		is_private = not kanivin.get_meta("ToDo").make_attachments_public
		...
		self.assertTrue(kanivin.db.exists("File", {"attached_to_name": todo.name, "is_private": is_private}))
		...
		filename = kanivin.db.exists("File", {"attached_to_name": todo.name})
		self.assertIn(f'<img src="{kanivin.get_doc("File", filename).file_url}', todo.description)
	...
	def test_extract_images_from_comment(self):
		...
		"""
		is_private = not kanivin.get_meta("ToDo").make_attachments_public
		test_doc = kanivin.get_doc(doctype="ToDo", description="comment test").insert()
		comment = add_comment(
			"ToDo",
			test_doc.name,
		...
		self.assertTrue(
			kanivin.db.exists("File", {"attached_to_name": test_doc.name, "is_private": is_private})
		)
	...
	def test_extract_images_from_communication(self):
		...
		"""
		is_private = not kanivin.get_meta("Communication").make_attachments_public
		communication = kanivin.get_doc(
		...
		self.assertTrue(
			kanivin.db.exists("File", {"attached_to_name": communication.name, "is_private": is_private})
		)
...
class TestFileOptimization(KanivinTestCase):
	...
	def test_optimize_svg(self):
		file_path = kanivin.get_app_path("kanivin", "tests/data/sample_svg.svg")
		with open(file_path, "rb") as f:
			file_content = f.read()
		test_file = kanivin.get_doc(
	...
	def test_optimize_folder(self):
		test_folder = kanivin.get_doc("File", "Home/Attachments")
...
class TestGuestFileAndAttachments(KanivinTestCase):
	def setUp(self) -> None:
		kanivin.db.delete("File", {"is_folder": 0})
		kanivin.get_doc(
			doctype="DocType",
	...
	def tearDown(self) -> None:
		kanivin.set_user("Administrator")
		kanivin.db.rollback()
		kanivin.delete_doc("DocType", "Test For Attachment")
	...
	def test_attach_unattached_guest_file(self):
		...
		d = kanivin.new_doc("Test For Attachment")
	...
	def test_private_remains_private_even_if_same_hash(self):
		file_name = "test" + kanivin.generate_hash()
		content = file_name.encode()

		doc_pub: "File" = kanivin.new_doc("File")  # type: ignore
		doc_pub.file_url = f"/files/{file_name}.txt"
		doc_pub.content = content
		doc_pub.save()

		doc_pri: "File" = kanivin.new_doc("File")  # type: ignore
