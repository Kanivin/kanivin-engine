...
# this is a separate file since it is imported in kanivin.model.document
...
def run_server_script_for_doc_event(doc, event):
	...
	if scripts:
		...
		for script_name in scripts:
			kanivin.get_doc("Server Script", script_name).execute_doc(doc)
...
def get_server_script_map():
	...
	# 	}
	# }
	if kanivin.flags.in_patch and not kanivin.db.table_exists("Server Script"):
		return {}

	script_map = kanivin.cache.get_value("server_script_map")
	if script_map is None:
		script_map = {"permission_query": {}}
		enabled_server_scripts = kanivin.get_all(
			"Server Script",
		...
		kanivin.cache.set_value("server_script_map", script_map)
