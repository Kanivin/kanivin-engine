...
kanivin.ui.form.on("Server Script", {
	setup: function (frm) {
	...
	refresh: function (frm) {
		...
		if (!frm.is_new()) {
			frm.add_custom_button(__("Compare Versions"), () => {
				new kanivin.ui.DiffView("Server Script", "script", frm.doc.name);
			});
		}
	...
	check_safe_exec(frm) {
		kanivin.xcall("kanivin.core.doctype.server_script.server_script.enabled").then((enabled) => {
			if (enabled === false) {
				let docs_link =
					"https://kanivinframework.com/docs/user/en/desk/scripting/server-script";
...
if kanivin.form_dict.message == "ping":
	kanivin.response['message'] = "pong"
else:
	kanivin.response['message'] = "ok"
</code></pre>
...
tenant_id = kanivin.db.get_value(...)
