...
kanivin.ui.form.on("Prepared Report", {
	render_filter_values: function (frm, filters) {
		...
		Object.keys(filters).forEach((key) => {
			const filter_row = $(`<tr>
				<td>${kanivin.model.unscrub(key)}</td>
				<td>${filters[key]}</td>
			</tr>`);
	...
	refresh: function (frm) {
		...
		if (frm.doc.status == "Completed") {
			frm.page.set_primary_action(__("Show Report"), () => {
				kanivin.route_options = { prepared_report_name: frm.doc.name };
				kanivin.set_route("query-report", frm.doc.report_name);
			});
		}
	},
});
