...
import kanivin
from kanivin import _, conf, safe_decode
from kanivin.build import html_to_js_template
from kanivin.core.doctype.custom_role.custom_role import get_custom_allowed_roles
from kanivin.desk.form.meta import get_code_files_via_hooks, get_js
...
class Page(Document):
	...
	def autoname(self):
		...
		if (self.name and self.name.startswith("New Page")) or not self.name:
			...
			if kanivin.db.exists("Page", self.name):
				cnt = kanivin.db.sql(
	...
	def validate(self):
		...
		if self.is_new() and not getattr(conf, "developer_mode", 0):
			kanivin.throw(_("Not in Developer Mode"))

		# setting ignore_permissions via update_setup_wizard_access (setup_wizard.py)
		if kanivin.session.user != "Administrator" and not self.flags.ignore_permissions:
			kanivin.throw(_("Only Administrator can edit"))
	...
	def on_update(self):
		...
		from kanivin.core.doctype.doctype.doctype import make_module_and_roles
		...
		if path:
			...
			if not os.path.exists(path + ".js"):
				with open(path + ".js", "w") as f:
					f.write(
						f"""kanivin.pages['{self.name}'].on_page_load = function(wrapper) {{
			...
			if fname.endswith(".html"):
				with open(os.path.join(path, fname)) as f:
					...
					if "<!-- jinja -->" in template:
						...
						try:
							out = kanivin.get_attr(
								"{app}.{module}.page.{page}.{page}.get_context".format(
									app=kanivin.local.module_app[scrub(self.module)],
						...
						template = kanivin.render_template(template, context)
...
def delete_custom_role(field, docname):
	name = kanivin.db.get_value("Custom Role", {field: docname}, "name")
	if name:
		kanivin.delete_doc("Custom Role", name)
