...
from kanivin.core.doctype.data_import.exporter import Exporter
from kanivin.core.doctype.data_import.importer import Importer
from kanivin.model import CORE_DOCTYPES
from kanivin.model.document import Document
from kanivin.modules.import_file import import_file_by_path
from kanivin.utils.background_jobs import enqueue, is_job_enqueued
from kanivin.utils.csvutils import validate_google_sheets_url
...
class DataImport(Document):
	...
	def validate_doctype(self):
		if self.reference_doctype in BLOCKED_DOCTYPES:
			kanivin.throw(_("Importing {0} is not allowed.").format(self.reference_doctype))
	...
	def start_import(self):
		...
		run_now = kanivin.flags.in_test or kanivin.conf.developer_mode
		if is_scheduler_inactive() and not run_now:
			kanivin.throw(_("Scheduler is inactive. Cannot import data."), title=_("Scheduler Inactive"))
	...
	def on_trash(self):
		kanivin.db.delete("Data Import Log", {"data_import": self.name})


@kanivin.whitelist()
def get_preview_from_template(data_import, import_file=None, google_sheets_url=None):
	return kanivin.get_doc("Data Import", data_import).get_preview_from_template(
...
def form_start_import(data_import: str):
	return kanivin.get_doc("Data Import", data_import).start_import()


def start_import(data_import):
	"""This method runs in background job"""
	data_import = kanivin.get_doc("Data Import", data_import)
	try:
	...
	kanivin.publish_realtime("data_import_refresh", {"data_import": data_import.name})
...
def download_template(doctype, export_fields=None, export_records=None, export_filters=None, file_type="CSV"):
	...
	export_fields = kanivin.parse_json(export_fields)
	export_filters = kanivin.parse_json(export_filters)
...
def download_errored_template(data_import_name):
	data_import = kanivin.get_doc("Data Import", data_import_name)
...
def get_import_status(data_import_name):
	...
	total_payload_count = kanivin.db.get_value("Data Import", data_import_name, "payload_count")
...
def get_import_logs(data_import: str):
	doc = kanivin.get_doc("Data Import", data_import)
...
def import_file(doctype, file_path, import_type, submit_after_import=False, console=False):
	...
	data_import = kanivin.new_doc("Data Import")
...
def import_doc(path, pre_process=None):
	...
	for f in files:
		if f.endswith(".json"):
			kanivin.flags.mute_emails = True
			import_file_by_path(
...
def export_json(doctype, path, filters=None, or_filters=None, name=None, order_by="creation asc"):
	...
	out = []
	if name:
		out.append(kanivin.get_doc(doctype, name).as_dict())
	elif kanivin.db.get_value("DocType", doctype, "issingle"):
		out.append(kanivin.get_doc(doctype).as_dict())
	else:
		for doc in kanivin.get_all(
			doctype,
			fields=["name"],
			filters=filters,
			...
			out.append(kanivin.get_doc(doctype, doc.name).as_dict())
	post_process(out)
	...
	with open(path, "w") as outfile:
		outfile.write(kanivin.as_json(out, ensure_ascii=False))


def export_csv(doctype, path):
	from kanivin.core.doctype.data_export.exporter import export_data

	with open(path, "wb") as csvfile:
		export_data(doctype=doctype, all_doctypes=True, template=True, with_data=True)
		csvfile.write(kanivin.response.result.encode("utf-8"))
