...
kanivin.ui.form.on("Data Import", {
	setup(frm) {
		kanivin.realtime.on("data_import_refresh", ({ data_import }) => {
			frm.import_in_progress = false;
			if (data_import !== frm.doc.name) return;
			kanivin.model.clear_doc("Data Import", frm.doc.name);
			kanivin.model.with_doc("Data Import", frm.doc.name).then(() => {
				frm.refresh();
			});
		});
		kanivin.realtime.on("data_import_progress", (data) => {
		...
		frm.set_query("reference_doctype", () => {
			return {
				filters: {
					name: ["in", kanivin.boot.user.can_import],
				},
			};
		});
	...
	refresh(frm) {
		...
		if (frm.doc.status.includes("Success")) {
			frm.add_custom_button(__("Go to {0} List", [__(frm.doc.reference_doctype)]), () =>
				kanivin.set_route("List", frm.doc.reference_doctype)
			);
		}
	},
	...
	update_indicators(frm) {
		const indicator = kanivin.get_indicator(frm.doc);
		if (indicator) {
	...
	show_import_status(frm) {
		kanivin.call({
			method: "kanivin.core.doctype.data_import.data_import.get_import_status",
			args: {
				...
				.then((result) => {
					if (result.length > 0) {
						frm.add_custom_button("Report Error", () => {
							...
							kanivin.request.report_error(fake_xhr, {});
						});
					}
				});
		}
	...
	download_template(frm) {
		kanivin.require("data_import_tools.bundle.js", () => {
			frm.data_exporter = new kanivin.data_import.DataExporter(
	...
	show_import_preview(frm, preview_data) {
		...
		kanivin.require("data_import_tools.bundle.js", () => {
			frm.import_preview = new kanivin.data_import.ImportPreview({
	...
	export_errored_rows(frm) {
		open_url_post(
			"/api/method/kanivin.core.doctype.data_import.data_import.download_errored_template",
			{
				data_import_name: frm.doc.name,
			}
		);
	...
	export_import_log(frm) {
		open_url_post(
			"/api/method/kanivin.core.doctype.data_import.data_import.download_import_log",
			{
				data_import_name: frm.doc.name,
			}
		);
	...
	render_import_log(frm) {
		kanivin.call({
			method: "kanivin.core.doctype.data_import.data_import.get_import_logs",
			args: {
				data_import: frm.doc.name,
			},
			callback: function (r) {
						...
						if (log.success) {
							if (frm.doc.import_type === "Insert New Records") {
								html = __("Successfully imported {0}", [
									`<span class="underline">${kanivin.utils.get_form_link(
										frm.doc.reference_doctype,
										log.docname,
										true
									)}<span>`,
							...
							let messages = JSON.parse(log.messages || "[]")
								...
								.join("");
							let id = kanivin.dom.get_unique_id();
							html = `${messages}
	...
	show_import_log(frm) {
		...
		kanivin.call({
			method: "kanivin.client.get_count",
			args: {
