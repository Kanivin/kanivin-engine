...
from kanivin.core.doctype.version.version import get_diff
from kanivin.model import no_value_fields
from kanivin.utils import cint, cstr, duration_to_seconds, flt, update_progress_bar
from kanivin.utils.csvutils import get_csv_content_from_google_sheets, read_csv_content
from kanivin.utils.xlsxutils import (
...
class Importer:
	def __init__(self, doctype, data_import=None, file_path=None, import_type=None, console=False):
		...
		if not self.data_import:
			self.data_import = kanivin.get_doc(doctype="Data Import")
			if import_type:
				self.data_import.import_type = import_type

		self.template_options = kanivin.parse_json(self.data_import.template_options or "{}")
	...
	def get_data_for_import_preview(self):
		...
		out.import_log = kanivin.get_all(
			"Data Import Log",
	...
	def before_import(self):
		...
		kanivin.cache.hdel("lang", kanivin.session.user)
		kanivin.set_user_lang(kanivin.session.user)

		# set flags
		kanivin.flags.in_import = True
		kanivin.flags.mute_emails = self.data_import.mute_emails
	...
	def import_data(self):
		...
		if (
			...
			kanivin.db.delete("Data Import Log", {"success": 0, "data_import": self.data_import.name})

		# get successfully imported rows
		imported_rows = []
		for log in import_log:
			log = kanivin._dict(log)
		...
		batch_size = kanivin.conf.data_import_batch_size or 1000

		for batch_index, batched_payloads in enumerate(kanivin.utils.create_batch(payloads, batch_size)):
			for i, payload in enumerate(batched_payloads):
				...
				try:
					...
					messages = kanivin.local.message_log
					...
					create_import_log(
						...
						{
							"success": False,
							"exception": kanivin.get_traceback(),
							"messages": messages,
							"row_indexes": row_indexes,
						},
					)
	...
	def after_import(self):
		...
		kanivin.flags.mute_emails = False
	...
	def insert_record(self, doc):
		meta = kanivin.get_meta(self.doctype)
		new_doc = kanivin.new_doc(self.doctype)
		new_doc.update(doc)
	...
	def update_record(self, doc):
		...
		existing_doc = kanivin.get_doc(self.doctype, doc.get(id_field.fieldname))

		updated_doc = kanivin.get_doc(self.doctype, doc.get(id_field.fieldname))
		...
		if get_diff(existing_doc, updated_doc):
			...
			# throw if no changes
			kanivin.throw(_("No changes to update"))
	...
	def export_errored_rows(self):
		from kanivin.utils.csvutils import build_csv_response
	...
	def export_import_log(self):
		...
		for log in import_log:
			...
			exception = kanivin.utils.cstr(log.get("exception", ""))
	...
	def print_import_log(self, import_log):
		...
		if failed_records:
			...
			file_name = f"{self.doctype}_import_on_{kanivin.utils.now()}.txt"
...
class ImportFile:
	def __init__(self, doctype, file, template_options=None, import_type=None, *, console=False):
		self.doctype = doctype
		self.template_options = template_options or kanivin._dict(column_to_field_map=kanivin._dict())
		...
		if isinstance(file, str):
			if kanivin.db.exists("File", {"file_url": file}):
				self.file_doc = kanivin.get_doc("File", {"file_url": file})
		...
		if not self.file_doc and not self.file_path and not self.google_sheets_url:
			kanivin.throw(_("Invalid template file for import"))
	...
	def get_data_from_template_file(self):
		...
		if not content:
			kanivin.throw(_("Invalid or corrupted content for import"))
	...
	def get_data_for_import_preview(self):
		...
		columns = [kanivin._dict({"header_title": "Sr. No", "skip_import": True})]
	...
	def get_payloads_for_import(self):
		...
		while data:
			...
			payloads.append(kanivin._dict(doc=doc, rows=rows))
		return payloads
	...
	def read_file(self, file_path: str):
		...
		if self.console:
			file_content = kanivin.read_file(file_path, True)
			return file_content, extn

		file_name = kanivin.db.get_value("File", {"file_url": file_path})
		if file_name:
			file = kanivin.get_doc("File", file_name)
	...
	def read_content(self, content, extension):
		...
		if extension not in ("csv", "xlsx", "xls"):
			kanivin.throw(_("Import template should be of type .csv, .xlsx or .xls"), title=error_title)
...
class Row:
	...
	def _parse_doc(self, doctype, columns, values, parent_doc=None, table_df=None):
		...
		for key in kanivin.model.default_fields + kanivin.model.child_table_fields + ("__islocal",):
			doc.pop(key, None)
		...
		is_table = kanivin.get_meta(doctype).istable
		...
		if is_table and is_update:
			...
			if id_value and kanivin.db.exists(doctype, id_value):
				existing_doc = kanivin.get_doc(doctype, id_value)
				existing_doc.update(doc)
				doc = existing_doc
			else:
				# for table rows being inserted in update
				# create a new doc with defaults set
				new_doc = kanivin.new_doc(doctype, as_dict=True)
				new_doc.update(doc)
				doc = new_doc
	...
	def validate_value(self, value, col):
		...
		if df.fieldtype == "Select":
			...
			if select_options and cstr(value) not in select_options:
				options_string = ", ".join(kanivin.bold(d) for d in select_options)
			...
			if not exists:
				msg = _("Value {0} missing for {1}").format(kanivin.bold(value), kanivin.bold(df.options))
				self.warnings.append(
					{
						"row": self.row_number,
						"field": df_as_json(df),
				...
				self.warnings.append(
					{
						...
						"message": _("Value {0} must in {1} format").format(
							kanivin.bold(value), kanivin.bold(get_user_format(col.date_format))
						),
					}
				)
				return
	...
	def link_exists(self, value, df):
		return bool(kanivin.db.exists(df.options, value, cache=True))
...
class Header(Row):
	def __init__(self, index, row, doctype, raw_data, column_to_field_map=None):
		...
		self.data = row
		self.doctype = doctype
		column_to_field_map = column_to_field_map or kanivin._dict()
...
class Column:
	def __init__(self, index, header, doctype, column_values, map_to_field=None, seen=None):
		...
		self.meta = kanivin.get_meta(doctype)
		self.parse()
	...
	def parse(self):
				...
				self.warnings.append(
					{
						"message": _("Mapping column {0} to field {1}").format(
							kanivin.bold(header_title or "<i>Untitled Column</i>"), kanivin.bold(df.label)
						),
						"type": "info",
					}
				)
		...
		if header_title in self.seen:
			self.warnings.append(
				{
					"col": column_number,
					"message": _("Skipping Duplicate Column {0}").format(kanivin.bold(header_title)),
					"type": "info",
				}
			)
			df = None
			...
			self.warnings.append(
				{
					"col": column_number,
					"message": _("Skipping column {0}").format(kanivin.bold(header_title)),
					"type": "info",
				}
			)
			...
			self.warnings.append(
				{
					"col": column_number,
					"message": _("Cannot match column {0} with any field").format(kanivin.bold(header_title)),
					"type": "info",
				}
			)
		...
		def guess_date_format(d):
			if isinstance(d, datetime | date | time):
				...
				if self.df.fieldtype == "Time":
					return "%H:%M:%S"
			if isinstance(d, str):
				return kanivin.utils.guess_date_format(d)
			...
			self.warnings.append(
				{
					...
					"message": message.format(
						kanivin.bold(self.header_title),
						len(unique_date_formats),
						kanivin.bold(user_date_format),
					),
					"type": "info",
				}
			)
	...
	def validate_values(self):
		...
		if self.df.fieldtype == "Link":
			...
			exists = [cstr(d.name) for d in kanivin.get_all(self.df.options, filters={"name": ("in", values)})]
			...
			if options:
				...
				if invalid:
					valid_values = ", ".join(kanivin.bold(o) for o in options)
					invalid_values = ", ".join(kanivin.bold(i) for i in invalid)
...
def build_fields_dict_for_column_matching(parent_doctype):
	...
	for doctype, table_df in doctypes:
		...
		fields = get_standard_fields(doctype) + kanivin.get_meta(doctype).fields
		for df in fields:
...
def get_df_for_column_header(doctype, header):
	...
	df_by_labels_and_fieldname = kanivin.cache.hget(
...
def get_id_field(doctype):
	...
	return kanivin._dict({"label": "ID", "fieldname": "name", "fieldtype": "Data"})
