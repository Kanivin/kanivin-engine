...
from kanivin.utils import format_duration, getdate

doctype_name = "DocType for Import"


class TestImporter(KanivinTestCase):
	@classmethod
	def setUpClass(cls):
		super().setUpClass()
	...
	def test_data_import_from_file(self):
		...
		doc1 = kanivin.get_doc(doctype_name, "Test")
		doc2 = kanivin.get_doc(doctype_name, "Test 2")
		doc3 = kanivin.get_doc(doctype_name, "Test 3")
	...
	def test_data_import_without_mandatory_values(self):
		...
		self.assertEqual(kanivin.parse_json(import_log[0]["row_indexes"]), [2, 3])
		expected_error = (
			"Error: <strong>Child 1 of DocType for Import</strong> Row #1: Value missing for: Child Title"
		)
		self.assertEqual(
			kanivin.parse_json(kanivin.parse_json(import_log[0]["messages"])[0])["message"], expected_error
		)
		expected_error = (
			"Error: <strong>Child 1 of DocType for Import</strong> Row #2: Value missing for: Child Title"
		)
		self.assertEqual(
			kanivin.parse_json(kanivin.parse_json(import_log[0]["messages"])[1])["message"], expected_error
		)

		self.assertEqual(kanivin.parse_json(import_log[1]["row_indexes"]), [4])
		self.assertEqual(
			kanivin.parse_json(kanivin.parse_json(import_log[1]["messages"])[0])["message"],
			"Title is required",
		)

	def test_data_import_update(self):
		existing_doc = kanivin.get_doc(
		...
		updated_doc = kanivin.get_doc(doctype_name, existing_doc.name)
...
def create_doctype_if_not_exists(doctype_name, force=False):
	if force:
		kanivin.delete_doc_if_exists("DocType", doctype_name)
		kanivin.delete_doc_if_exists("DocType", "Child 1 of " + doctype_name)
		kanivin.delete_doc_if_exists("DocType", "Child 2 of " + doctype_name)

	if kanivin.db.exists("DocType", doctype_name):
		return
...
def get_import_file(csv_file_name, force=False):
	...
	_file = kanivin.db.exists("File", {"file_name": file_name})
	if force and _file:
		kanivin.delete_doc_if_exists("File", _file)

	if kanivin.db.exists("File", {"file_name": file_name}):
		f = kanivin.get_doc("File", {"file_name": file_name})
	else:
		full_path = get_csv_file_path(file_name)
		f = kanivin.get_doc(
			doctype="File", content=kanivin.read_file(full_path), file_name=file_name, is_private=1
		)
...
def get_csv_file_path(file_name):
	return kanivin.get_app_path("kanivin", "core", "doctype", "data_import", "fixtures", file_name)
