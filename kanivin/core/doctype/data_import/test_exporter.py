...
from kanivin.core.doctype.data_import.test_importer import create_doctype_if_not_exists
from kanivin.tests.utils import KanivinTestCase

doctype_name = "DocType for Export"


class TestExporter(KanivinTestCase):
	def setUp(self):
		create_doctype_if_not_exists(doctype_name)

	def test_exports_specified_fields(self):
		if not kanivin.db.exists(doctype_name, "Test"):
			doc = kanivin.get_doc(
			...
			doc = kanivin.get_doc(doctype_name, "Test")
	...
	def test_export_csv_response(self):
		...
		self.assertEqual(kanivin.response["doctype"], doctype_name)
