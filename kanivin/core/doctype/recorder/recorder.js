...
kanivin.ui.form.on("Recorder", {
	...
	refresh: function (frm) {
		...
		frm.add_custom_button(__("Suggest Optimizations"), () => {
			kanivin.xcall("kanivin.core.doctype.recorder.recorder.optimize", {
				recorder_id: frm.doc.name,
			});
		});

		kanivin.realtime.on("recorder-analysis-complete", () => {
			frm.reload_doc();
		...
		index_grid.add_custom_button(__("Add Indexes"), function () {
			...
			if (!indexes_to_add.length) {
				kanivin.toast(__("You need to select indexes you want to add first."));
				return;
			}
			kanivin.xcall("kanivin.core.doctype.recorder.recorder.add_indexes", {
...
kanivin.ui.form.on("Recorder Query", "form_render", function (frm, cdt, cdn) {
	let row = kanivin.get_doc(cdt, cdn);
