"""Basic DB optimizer for Kanivin Framework based app.
...
class DBColumn:
	...
	@classmethod
	def from_kanivin_ouput(cls, data) -> "DBColumn":
		"Parse DBColumn from output of describe-database-table command in Kanivin"
		return cls(
			name=data["column"],
...
class DBIndex:
	...
	@classmethod
	def from_kanivin_ouput(cls, data, table) -> "DBIndex":
		"Parse DBIndex from output of describe-database-table command in Kanivin"
		return cls(
			name=data["name"],
			table=table,
			unique=data["unique"],
...
class ColumnStat:
	...
	@classmethod
	def from_kanivin_ouput(cls, data) -> "ColumnStat":
		return cls(
...
class DBTable:
	...
	@classmethod
	def from_kanivin_ouput(cls, data) -> "DBTable":
		"Parse DBTable from output of describe-database-table command in Kanivin"
		table_name = data["table_name"]
		return cls(
			name=table_name,
			total_rows=data["total_rows"],
			schema=[DBColumn.from_kanivin_ouput(c) for c in data["schema"]],
			indexes=[DBIndex.from_kanivin_ouput(i, table_name) for i in data["indexes"]],
		)
...
class DBOptimizer:
	...
	def suggest_index(self) -> DBIndex | None:
		...
		if missing_tables := (set(self.tables_examined()) - set(self.tables.keys())):
			kanivin.throw("DBTable infomation missing for: " + ", ".join(missing_tables))
