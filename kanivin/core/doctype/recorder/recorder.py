...
from kanivin.core.doctype.recorder.db_optimizer import DBOptimizer, DBTable
...
from kanivin.recorder import RECORDER_REQUEST_HASH
from kanivin.recorder import get as get_recorder_data
from kanivin.utils import cint, cstr, evaluate_filters, get_table_name
...
class Recorder(Document):
	...
	if TYPE_CHECKING:
		from kanivin.core.doctype.recorder_query.recorder_query import RecorderQuery
		from kanivin.core.doctype.recorder_suggested_index.recorder_suggested_index import (
...
def serialize_request(request):
	request = kanivin._dict(request)
	if request.get("calls"):
		for i in request.calls:
			i["stack"] = kanivin.as_json(i["stack"])
			i["explain_result"] = kanivin.as_json(i["explain_result"])
	request.update(
		name=request.get("uuid"),
		number_of_queries=request.get("queries"),
		time_in_queries=request.get("time_queries"),
		request_headers=kanivin.as_json(request.get("headers"), indent=4),
		form_dict=kanivin.as_json(request.get("form_dict"), indent=4),
...
def add_indexes(indexes):
	...
	for index in indexes:
		kanivin.enqueue(_add_index, table=index["table"], column=index["column"])
	kanivin.msgprint(_("Enqueued creation of indexes"), alert=True)


def _add_index(table, column):
	doctype = get_doctype_name(table)
	kanivin.db.add_index(doctype, [column])
	kanivin.msgprint(
...
def optimize(recorder_id: str):
	kanivin.only_for("Administrator")
	kanivin.enqueue(_optimize, recorder_id=recorder_id, queue="long")


def _optimize(recorder_id):
	record: Recorder = kanivin.get_doc("Recorder", recorder_id)
	...
	data = kanivin.cache.hget(RECORDER_REQUEST_HASH, record.name)
	data["suggested_indexes"] = [{"table": idx[0][0], "column": idx[0][1]} for idx in suggested_indexes]
	kanivin.cache.hset(RECORDER_REQUEST_HASH, record.name, data)
	kanivin.publish_realtime("recorder-analysis-complete", user=kanivin.session.user)
	kanivin.msgprint(_("Query analysis complete. Check suggested indexes."), realtime=True, alert=True)
...
def _optimize_query(query):
	...
	for table in tables:
		...
		if not stats:
			return
		db_table = DBTable.from_kanivin_ouput(stats)
...
def _fetch_table_stats(doctype: str, columns: list[str]) -> dict | None:
	...
	if not kanivin.db.table_exists(doctype):
		return

	table = get_table_name(doctype, wrap_in_backticks=True)

	schema = []
	for field in kanivin.db.sql(f"describe {table}", as_dict=True):
		schema.append(
			{
				"column": field["Field"],
				"type": field["Type"],
	...
	indexes = []
	for idx in kanivin.db.sql(f"show index from {table}", as_dict=True):
		indexes.append(
			{
...
def _get_column_cardinality(table, column):
	return kanivin.db.sql(f"select count(distinct {column}) from {table}")[0][0]
