kanivin.listview_settings["Recorder"] = {
	...
	onload(listview) {
		...
		listview.page.add_button(__("Clear"), () => {
			kanivin.xcall("kanivin.recorder.delete").then(listview.refresh);
		});
		...
		listview.page.add_menu_item(__("Import"), () => {
			new kanivin.ui.FileUploader({
				...
				on_success: (file) => {
					...
					kanivin.call({
						method: "kanivin.recorder.import_data",
						args: {
							file: file.file_url,
						},
						callback: function () {
		...
		listview.page.add_menu_item(__("Export"), () => {
			kanivin.call({
				method: "kanivin.recorder.export_data",
				callback: function (r) {
	...
	fetch_recorder_status(listview) {
		return kanivin.xcall("kanivin.recorder.status").then((status) => {
	...
	stop_recorder(listview) {
		let me = this;
		kanivin.xcall("kanivin.recorder.stop", {}).then(() => {
			listview.refresh();
	...
	start_recorder(listview) {
		...
		kanivin.prompt(
			...
			(values) => {
				kanivin.xcall("kanivin.recorder.start", values).then(() => {
					listview.refresh();
