...
class TestPackage(KanivinTestCase):
	def test_package_release(self):
		...
		# make release
		kanivin.get_doc(dict(doctype="Package Release", package="Test Package", publish=1)).insert()

		self.assertTrue(os.path.exists(kanivin.get_site_path("packages", "test-package")))
		self.assertTrue(
			os.path.exists(kanivin.get_site_path("packages", "test-package", "test_module_for_package"))
		)
		self.assertTrue(
			os.path.exists(
				kanivin.get_site_path(
					"packages",
					"test-package",
...
def make_test_package():
	if not kanivin.db.exists("Package", "Test Package"):
		kanivin.get_doc(
...
def make_test_module():
	if not kanivin.db.exists("Module Def", "Test Module for Package"):
		kanivin.get_doc(
			dict(
				doctype="Module Def",
...
def make_test_doctype():
	if not kanivin.db.exists("DocType", "Test DocType for Package"):
		kanivin.get_doc(
			dict(
				doctype="DocType",
...
def make_test_server_script():
	if not kanivin.db.exists("Server Script", "Test Script for Package"):
		kanivin.get_doc(
			dict(
				...
				script='kanivin.msgprint("Test")',
			)
		).insert()


def make_test_web_page():
	if not kanivin.db.exists("Web Page", "test-web-page-for-package"):
		kanivin.get_doc(
			dict(
				doctype="Web Page",
				module="Test Module for Package",
