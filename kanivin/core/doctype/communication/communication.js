kanivin.ui.form.on("Communication", {
	onload: function (frm) {
		if (frm.doc.content) {
			frm.doc.content = kanivin.dom.remove_script_and_style(frm.doc.content);
		}
	...
	refresh: function (frm) {
		...
		if (frm.doc.reference_doctype && frm.doc.reference_name) {
			frm.add_custom_button(__(frm.doc.reference_name), function () {
				kanivin.set_route("Form", frm.doc.reference_doctype, frm.doc.reference_name);
			});
		} else {
			...
			fields: [
				{
					...
					get_query: function () {
						return { query: "kanivin.email.get_communication_doctype" };
					},
				},
				{
	...
	show_move_dialog: function (frm) {
		var d = new kanivin.ui.Dialog({
			...
			primary_action(values) {
				d.hide();
				kanivin.call({
					method: "kanivin.email.inbox.move_email",
					args: {
	...
	mark_as_read_unread: function (frm) {
		...
		return kanivin.call({
			method: "kanivin.email.inbox.create_email_flag_queue",
			args: {
				names: [frm.doc.name],
				action: action,
				flag: flag,
	...
	mark_as_closed_open: function (frm) {
		...
		return kanivin.call({
			method: "kanivin.email.inbox.mark_as_closed_open",
			args: {
				communication: frm.doc.name,
				status: status,
			},
	...
	get_mail_args: function (frm) {
		var sender_email_id = "";
		$.each(kanivin.boot.email_accounts, function (idx, account) {
	...
	add_to_contact: function (frm) {
		...
		kanivin.new_doc("Contact");
	},

	mark_as_spam: function (frm) {
		kanivin.call({
			method: "kanivin.email.inbox.mark_as_spam",
			args: {
				communication: frm.doc.name,
			...
			freeze: true,
			callback: function (r) {
				kanivin.msgprint(__("Email has been marked as spam"));
			},
		});
	},
	...
	move_to_trash: function (frm) {
		kanivin.call({
			method: "kanivin.email.inbox.mark_as_trash",
			args: {
				communication: frm.doc.name,
			},
			freeze: true,
			callback: function (r) {
				kanivin.msgprint(__("Email has been moved to trash"));
			},
		});
	},
});
