...
import kanivin
from kanivin.core.doctype.communication.communication import Communication, get_emails, parse_email
from kanivin.core.doctype.communication.email import add_attachments, make
from kanivin.email.doctype.email_queue.email_queue import EmailQueue
from kanivin.tests.utils import KanivinTestCase

if TYPE_CHECKING:
	from kanivin.contacts.doctype.contact.contact import Contact
	from kanivin.email.doctype.email_account.email_account import EmailAccount

test_records = kanivin.get_test_records("Communication")


class TestCommunication(KanivinTestCase):
	def test_email(self):
		valid_email_list = [
		...
		for i, x in enumerate(valid_email_list):
			with self.subTest(i=i, x=x):
				self.assertTrue(kanivin.utils.parse_addr(x)[1])

		for i, x in enumerate(invalid_email_list):
			with self.subTest(i=i, x=x):
				self.assertFalse(kanivin.utils.parse_addr(x)[0])
	...
	def test_name(self):
		...
		for x in valid_email_list:
			self.assertTrue(kanivin.utils.parse_addr(x)[0])
	...
	def test_circular_linking(self):
		...
		a = kanivin.get_doc("Communication", a.name)
		a.reference_doctype = "Communication"
		a.reference_name = c.name

		self.assertRaises(kanivin.CircularLinkingError, a.save)

	def test_deduplication_timeline_links(self):
		kanivin.delete_doc_if_exists("Note", "deduplication timeline links")
	...
	def test_contacts_attached(self):
		contact_sender: "Contact" = kanivin.get_doc(
			{
				"doctype": "Contact",
		...
		contact_recipient: "Contact" = kanivin.get_doc(
			{
				"doctype": "Contact",
		...
		contact_cc: "Contact" = kanivin.get_doc(
			{
				"doctype": "Contact",
		...
		comm: Communication = kanivin.get_doc(
			{
				"doctype": "Communication",
				"communication_medium": "Email",
	...
	def test_get_communication_data(self):
		from kanivin.desk.form.load import get_communication_data

		kanivin.delete_doc_if_exists("Note", "get communication data")

		note = kanivin.get_doc(
			{"doctype": "Note", "title": "get communication data", "content": "get communication data"}
		).insert(ignore_permissions=True)

		comm_note_1 = kanivin.get_doc(
			{
				"doctype": "Communication",
				"communication_type": "Communication",
	...
	def test_signature_in_email_content(self):
		...
		comm_with_signature = kanivin.get_doc(
			base_communication
			| {
		...
		comm_without_signature = kanivin.get_doc(
			base_communication
			| {
	...
	def test_mark_as_spam(self):
		...
		spam_comm: Communication = kanivin.get_doc(
			{
				"doctype": "Communication",
				"communication_medium": "Email",
		...
		normal_comm: Communication = kanivin.get_doc(
			{
				"doctype": "Communication",
				"communication_medium": "Email",
				"subject": "This is spam",
...
class TestCommunicationEmailMixin(KanivinTestCase):
	...
	def test_add_attachments_by_filename(self):
		...
		attached_file_name, attached_content_hash = kanivin.db.get_value(
			"File",
	...
	def test_add_attachments_by_file_content(self):
		...
		attached_file_name = kanivin.db.get_value(
			"File",
			{"attached_to_name": comm.name, "attached_to_doctype": comm.doctype},
		)
		attached_file = kanivin.get_doc("File", attached_file_name)
...
def create_email_account() -> "EmailAccount":
	kanivin.delete_doc_if_exists("Email Account", "_Test Comm Account 1")
