...
from kanivin.utils import get_formatted_email, get_url, parse_addr
...
class CommunicationEmailMixin:
	...
	def mail_cc(self, is_inbound_mail_communcation=False, include_sender=False):
		...
		* Remove standard users from email list
		...
		if include_sender:
			...
			if kanivin.session.user not in kanivin.STANDARD_USERS:
				sender = kanivin.db.get_value("User", kanivin.session.user, "email")
			cc.append(sender)
		...
		self._final_cc = [m for m in cc if m and m not in kanivin.STANDARD_USERS]
		return self._final_cc
	...
	def mail_bcc(self, is_inbound_mail_communcation=False):
		...
		* remove standard users.
		...
		self._final_bcc = [m for m in bcc if m not in kanivin.STANDARD_USERS]
		return self._final_bcc
	...
	def get_content(self, print_format=None):
		if print_format and kanivin.db.get_single_value("System Settings", "attach_view_link"):
			return self.content + self.get_attach_link(print_format)
		return self.content

	def get_attach_link(self, print_format):
		"""Returns public link for the attachment via `templates/emails/print_link.html`."""
		return kanivin.get_template("templates/emails/print_link.html").render(
			{
				"url": get_url(),
		...
		if not hasattr(self, "_outgoing_email_account"):
			if self.email_account:
				...
				if self.sent_or_received == "Sent" and self._outgoing_email_account:
					if kanivin.db.exists("Email Account", self._outgoing_email_account.name):
	...
	def mail_attachments(self, print_format=None, print_html=None, print_language=None):
		...
		if print_format or print_html:
			d = {
				...
				"doctype": self.reference_doctype,
				"name": self.reference_name,
				"lang": print_language or kanivin.local.lang,
			}
	...
	def filter_thread_notification_disbled_users(emails):
		...
		return kanivin.get_all("User", pluck="email", filters={"email": ["in", emails], "thread_notify": 0})
	...
	def filter_disabled_users(emails):
		...
		return kanivin.get_all("User", pluck="email", filters={"email": ["in", emails], "enabled": 0})
	...
	def send_email(
		...
		if input_dict := self.sendmail_input_dict(
			...
			kanivin.sendmail(now=now, **input_dict)
