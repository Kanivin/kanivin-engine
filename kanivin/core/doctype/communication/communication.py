...
from kanivin.automation.doctype.assignment_rule.assignment_rule import (
	apply as apply_assignment_rule,
)
from kanivin.contacts.doctype.contact.contact import get_contact_name
from kanivin.core.doctype.comment.comment import update_comment_in_doc
from kanivin.core.doctype.communication.email import validate_email
from kanivin.core.doctype.communication.mixins import CommunicationEmailMixin
from kanivin.core.utils import get_parent_doc
...
class Communication(Document, CommunicationEmailMixin):
	...
	if TYPE_CHECKING:
		from kanivin.core.doctype.communication_link.communication_link import CommunicationLink
	...
	def onload(self):
		...
		if (
			...
			email_flag_queue = kanivin.db.get_value(
	...
	def validate_reference(self):
		if self.reference_doctype and self.reference_name:
			if not self.reference_owner:
				self.reference_owner = kanivin.db.get_value(
					self.reference_doctype, self.reference_name, "owner"
				)

			# prevent communication against a child table
			if kanivin.get_meta(self.reference_doctype).istable:
				kanivin.throw(
	...
	def after_insert(self):
		...
		if self.reference_doctype == "Communication" and self.sent_or_received == "Sent":
			kanivin.db.set_value("Communication", self.reference_name, "status", "Replied")

		if self.communication_type == "Communication":
			self.notify_change("add")

		elif self.communication_type in ("Chat", "Notification"):
			if self.reference_name == kanivin.session.user:
				message = self.as_dict()
				message["broadcast"] = True
				kanivin.publish_realtime("new_message", message, after_commit=True)
			else:
	...
	def set_signature_in_email_content(self):
		...
		signature = user_email_signature or kanivin.db.get_value(
			"Email Account",
	...
	def mark_email_as_spam(self):
		if (
			...
			and self.communication_medium == "Email"
			and self.sent_or_received == "Received"
			and kanivin.db.exists("Email Rule", {"email_id": self.sender, "is_spam": 1})
		):
	...
	def find_one_by_filters(cls, *, order_by=None, **kwargs):
		name = kanivin.db.get_value(cls.DOCTYPE, kwargs, order_by=order_by)
	...
	def set_sender_full_name(self):
		if not self.sender_full_name and self.sender:
			if self.sender == "Administrator":
				self.sender_full_name = kanivin.db.get_value("User", "Administrator", "full_name")
				self.sender = kanivin.db.get_value("User", "Administrator", "email")
				...
				if not self.sender_full_name:
					self.sender_full_name = kanivin.db.get_value("User", self.sender, "full_name")

				if not self.sender_full_name:
					first_name, last_name = kanivin.db.get_value(
	...
	def set_delivery_status(self, commit=False):
		...
		status_counts = Counter(
			kanivin.get_all("Email Queue", pluck="status", filters={"communication": self.name})
		)
	...
	def parse_email_for_timeline_links(self):
		if not kanivin.db.get_value("Email Account", filters={"enable_automatic_linking": 1}):
			return

		for doctype, docname in parse_email([self.recipients, self.cc, self.bcc]):
			if not kanivin.db.get_value(doctype, docname, ignore=True):
				continue
	...
	def set_timeline_links(self):
		contacts = []
		create_contact_enabled = self.email_account and kanivin.db.get_value(
...
def on_doctype_update():
	...
	kanivin.db.add_index("Communication", ["reference_doctype", "reference_name"])
	kanivin.db.add_index("Communication", ["status", "communication_type"])
	kanivin.db.add_index("Communication", ["message_id(140)"])
...
def get_contacts(email_strings: list[str], auto_create_contact=False) -> list[str]:
	...
	for email in email_addrs:
		...
		if not contact_name and email and auto_create_contact:
			...
			first_name = kanivin.unscrub(email_parts[0])
...
def get_email_without_link(email):
	...
	"""
	if not kanivin.get_all("Email Account", filters={"enable_automatic_linking": 1}):
		return email
...
def update_first_response_time(parent, communication):
	if parent.meta.has_field("first_response_time") and not parent.get("first_response_time"):
		if (
			...
			or kanivin.get_cached_value("User", kanivin.session.user, "user_type") == "System User"
		):
...
def set_avg_response_time(parent, communication):
	if parent.meta.has_field("avg_response_time") and communication.sent_or_received == "Sent":
		...
		communications = kanivin.get_list(
			"Communication",
