...
def make(
	...
	**kwargs,
	...
	"""
	if kwargs:
		from kanivin.utils.commands import warn

		warn(
			f"Options {kwargs} used in kanivin.core.doctype.communication.email.make "
			"are deprecated or unsupported",
			category=DeprecationWarning,
		)

	if doctype and name:
		kanivin.has_permission(doctype, doc=name, ptype="email", throw=True)
...
def _make(
	...
	sender = sender or get_formatted_email(kanivin.session.user)
	...
	if cint(send_email):
		if not comm.get_outgoing_email_account():
			kanivin.throw(
				...
				exc=kanivin.OutgoingEmailError,
			)
...
def add_attachments(name: str, attachments: Iterable[str | dict]) -> None:
	...
	for a in attachments:
		if isinstance(a, str):
			attach = kanivin.db.get_value("File", {"name": a}, ["file_url", "is_private"], as_dict=1)
			file_args = {
				"file_url": attach.file_url,
				"is_private": attach.is_private,
			}
		elif isinstance(a, dict) and "fcontent" in a and "fname" in a:
			# dict returned by kanivin.attach_print()
			file_args = {
				"file_name": a["fname"],
				"content": a["fcontent"],
				"is_private": 1,
...
def mark_email_as_seen(name: str | None = None):
	kanivin.request.after_response.add(lambda: _mark_email_as_seen(name))
...
def _mark_email_as_seen(name):
	try:
		...
		kanivin.log_error("Unable to mark as seen", None, "Communication", name)

	kanivin.db.commit()  # nosemgrep: after_response requires explicit commit
...
def update_communication_as_read(name):
	...
	communication = kanivin.db.get_value("Communication", name, "read_by_recipient", as_dict=True)
