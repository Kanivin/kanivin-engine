...
class AuditTrail(Document):
	...
	def validate_doctype_name(self):
		if not self.doctype_name:
			kanivin.throw(_("{} field cannot be empty.").format(kanivin.bold("Doctype")))

	def validate_document(self):
		if not self.document:
			kanivin.throw(_("{} field cannot be empty.").format(kanivin.bold("Document")))

	@kanivin.whitelist()
	def compare_document(self):
		self.validate()
		amended_document_names = self.get_amended_documents()
		self.amended_docs = [kanivin.get_doc(self.doctype_name, name) for name in amended_document_names]
	...
	def get_amended_documents(self):
		...
		while curr_doc and len(amended_document_names) < 5:
			...
			curr_doc = kanivin.db.get_value(self.doctype_name, curr_doc, "amended_from")
...
def filter_fields_for_gridview(row):
	grid_row = {}
	meta = kanivin.get_meta(row.doctype)
	for field in meta.fields:
