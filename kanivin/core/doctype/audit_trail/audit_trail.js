...
kanivin.ui.form.on("Audit Trail", {
	refresh(frm) {
	...
	render_changed_fields(frm, document_names, changed_fields) {
		...
		$(kanivin.render_template("audit_trail", render_dict)).appendTo(
	...
	render_rows_added_or_removed(frm, changed_fields) {
		...
		for (let key in added_or_removed) {
			...
			$(kanivin.render_template("audit_trail_rows_added_removed", section_dict)).appendTo(
