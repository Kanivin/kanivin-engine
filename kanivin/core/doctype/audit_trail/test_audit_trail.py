...
class TestAuditTrail(KanivinTestCase):
	def setUp(self):
	...
	def test_compare_changed_fields(self):
		doc = kanivin.new_doc("Test Custom Doctype for Doc Comparator")
		doc.test_field = "first value"
		doc.submit()
		doc.cancel()

		changed_fields = kanivin._dict(test_field="second value")
		amended_doc = amend_document(doc, changed_fields, {}, 1)
		amended_doc.cancel()

		changed_fields = kanivin._dict(test_field="third value")
	...
	def test_compare_rows(self):
		...
		rows_updated = kanivin._dict(child_table_field=child_table_new)
...
def amend_document(amend_from, changed_fields, rows_updated, submit=False):
	amended_doc = kanivin.copy_doc(amend_from)
...
def create_comparator_doc(doctype_name, document):
	comparator = kanivin.new_doc("Audit Trail")
