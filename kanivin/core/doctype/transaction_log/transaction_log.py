...
from kanivin.utils import cint, now_datetime
...
class TransactionLog(Document):
	...
	def hash_line(self):
		sha = hashlib.sha256()
		sha.update(
			kanivin.safe_encode(str(self.row_index))
			+ kanivin.safe_encode(str(self.timestamp))
			+ kanivin.safe_encode(str(self.data))
		)
	...
	def hash_chain(self):
		sha = hashlib.sha256()
		sha.update(
			kanivin.safe_encode(str(self.transaction_hash)) + kanivin.safe_encode(str(self.previous_hash))
		)
		return sha.hexdigest()
...
def get_current_index():
	...
	current = (
		kanivin.qb.from_(series).where(series.name == "TRANSACTLOG").for_update().select("current")
	).run()
	...
	if current and current[0][0] is not None:
		...
		kanivin.db.sql("INSERT INTO `tabSeries` (name, current) VALUES ('TRANSACTLOG', 1)")
		current = 1
	return current
