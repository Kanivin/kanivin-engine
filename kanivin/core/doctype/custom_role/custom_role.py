...
class CustomRole(Document):
	...
	def validate(self):
		if self.report and not self.ref_doctype:
			self.ref_doctype = kanivin.db.get_value("Report", self.report, "ref_doctype")


def get_custom_allowed_roles(field, name):
	allowed_roles = []
	custom_role = kanivin.db.get_value("Custom Role", {field: name}, "name")
	if custom_role:
		custom_role_doc = kanivin.get_doc("Custom Role", custom_role)
