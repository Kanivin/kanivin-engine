...
kanivin.ui.form.on("Installed Applications", {
	...
	show_update_order_dialog() {
		const dialog = new kanivin.ui.Dialog({
			...
			primary_action: function () {
				...
				kanivin.call({
					method: "kanivin.core.doctype.installed_applications.installed_applications.update_installed_apps_order",
					freeze: true,
					args: {
						new_order: new_order,
					},
		...
		kanivin
			.xcall(
				"kanivin.core.doctype.installed_applications.installed_applications.get_installed_app_order"
			)
			.then((data) => {
				data.forEach((app) => {
				...
				grid.find(".grid-row-check:first").remove() &&
					grid.find(".grid-row-check").replaceWith(kanivin.utils.icon("menu"));
				dialog.show();
			});
	},
});
