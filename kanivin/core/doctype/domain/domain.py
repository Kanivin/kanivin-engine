...
class Domain(Document):
	...
	def setup_domain(self):
		...
		if self.data.on_setup:
			...
			kanivin.get_attr(self.data.on_setup)()
	...
	def remove_domain(self):
		...
		if self.data.restricted_roles:
			for role_name in self.data.restricted_roles:
				if kanivin.db.exists("Role", role_name):
					role = kanivin.get_doc("Role", role_name)
					role.disabled = 1
					role.save()
		...
		if self.data.custom_fields:
			for doctype in self.data.custom_fields:
				...
				for custom_field_detail in custom_fields:
					custom_field_name = kanivin.db.get_value(
	...
	def setup_roles(self):
		...
		if self.data.restricted_roles:
			...
			for role_name in self.data.restricted_roles:
				...
				if not kanivin.db.get_value("Role", role_name):
					kanivin.get_doc(dict(doctype="Role", role_name=role_name)).insert()
					continue
	...
	def setup_data(self, domain=None):
		...
		self.data = kanivin.get_domain_data(self.name)

	def get_domain_data(self, module):
		return kanivin.get_attr(kanivin.get_hooks("domains")[self.name] + ".data")
	...
	def setup_properties(self):
		if self.data.properties:
			for args in self.data.properties:
				kanivin.make_property_setter(args)

	def set_values(self):
		"""set values based on `data.set_value`"""
		if self.data.set_value:
			for args in self.data.set_value:
				kanivin.reload_doctype(args[0])
				doc = kanivin.get_doc(args[0], args[1] or args[0])
	...
	def setup_sidebar_items(self):
		...
		if self.data.allow_sidebar_items:
			# disable all
			kanivin.db.sql("update `tabPortal Menu Item` set enabled=0")
		...
		if self.data.remove_sidebar_items:
			# disable all
			kanivin.db.sql("update `tabPortal Menu Item` set enabled=1")
