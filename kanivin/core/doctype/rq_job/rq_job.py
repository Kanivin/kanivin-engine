...
from kanivin.utils.background_jobs import get_queues, get_redis_conn
...
class RQJob(Document):
	...
	def stop_job(self):
		try:
			...
			kanivin.msgprint(_("Job is not running."), title=_("Invalid Operation"))
...
def serialize_job(job: Job) -> kanivin._dict:
	...
	if job_name == "kanivin.utils.background_jobs.run_doc_method":
	...
	return kanivin._dict(
		...
		arguments=kanivin.as_json(job.kwargs),
		timeout=job.timeout,
...
def for_current_site(job: Job) -> bool:
	return job.kwargs.get("site") == kanivin.local.site
...
@kanivin.whitelist()
def stop_job(job_id):
	kanivin.get_doc("RQ Job", job_id).stop_job()
