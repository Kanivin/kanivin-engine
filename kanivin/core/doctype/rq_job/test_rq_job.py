...
import kanivin
from kanivin.core.doctype.rq_job.rq_job import RQJob, remove_failed_jobs, stop_job
from kanivin.installer import update_site_config
from kanivin.tests.utils import KanivinTestCase, timeout
from kanivin.utils import cstr, execute_in_shell
from kanivin.utils.background_jobs import get_job_status, is_job_enqueued
...
class TestRQJob(KanivinTestCase):
	BG_JOB = "kanivin.core.doctype.rq_job.test_rq_job.test_func"
	...
	def test_serialization(self):
		job = kanivin.enqueue(method=self.BG_JOB, queue="short")
		rq_job = kanivin.get_doc("RQ Job", job.id)
	...
	def test_configurable_ttl(self):
		kanivin.conf.rq_job_failure_ttl = 600
		job = kanivin.enqueue(method=self.BG_JOB, queue="short")

		self.assertEqual(job.failure_ttl, 600)

	def test_func_obj_serialization(self):
		job = kanivin.enqueue(method=test_func, queue="short")
		rq_job = kanivin.get_doc("RQ Job", job.id)
		self.assertEqual(rq_job.job_name, "kanivin.core.doctype.rq_job.test_rq_job.test_func")
	...
	def test_get_list_filtering(self):
		...
		# Fail a job
		job = kanivin.enqueue(method=self.BG_JOB, queue="short", fail=True)
		...
		job = kanivin.enqueue(method=self.BG_JOB, queue="short", sleep=10)
		time.sleep(3)
	...
	def test_delete_doc(self):
		job = kanivin.enqueue(method=self.BG_JOB, queue="short")
		kanivin.get_doc("RQ Job", job.id).delete()
	...
	def test_multi_queue_burst_consumption(self):
		for _ in range(3):
			for q in ["default", "short"]:
				kanivin.enqueue(self.BG_JOB, sleep=1, queue=q)
	...
	def test_job_id_manual_dedup(self):
		job_id = "test_dedup"
		job = kanivin.enqueue(self.BG_JOB, sleep=5, job_id=job_id)
	...
	def test_auto_job_dedup(self):
		job_id = "test_dedup"
		job1 = kanivin.enqueue(self.BG_JOB, sleep=2, job_id=job_id, deduplicate=True)
		job2 = kanivin.enqueue(self.BG_JOB, sleep=5, job_id=job_id, deduplicate=True)
		...
		job3 = kanivin.enqueue(self.BG_JOB, fail=True, job_id=job_id, deduplicate=True)
		self.check_status(job3, "failed")
		job4 = kanivin.enqueue(self.BG_JOB, sleep=1, job_id=job_id, deduplicate=True)
		self.check_status(job4, "finished")

	@timeout
	def test_enqueue_after_commit(self):
		job_id = kanivin.generate_hash()

		kanivin.enqueue(self.BG_JOB, enqueue_after_commit=True, job_id=job_id)
	...
	def test_memory_usage(self):
		if kanivin.db.db_type != "mariadb":
			return
		job = kanivin.enqueue("kanivin.utils.data._get_rss_memory_usage")
		self.check_status(job, "finished")
		...
		# Refer this PR: https://github.com/kanivin/kanivin/pull/21467
	...
	def test_clear_failed_jobs(self):
		...
		jobs = [kanivin.enqueue(method=self.BG_JOB, queue="short", fail=True) for _ in range(limit * 2)]
