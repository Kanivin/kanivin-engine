...
class DomainSettings(Document):
	...
	if TYPE_CHECKING:
		from kanivin.core.doctype.has_domain.has_domain import HasDomain
	...
	def on_update(self):
		for i, d in enumerate(self.active_domains):
			...
			if i >= 1:
				kanivin.flags.keep_desktop_icons = True
			domain = kanivin.get_doc("Domain", d.domain)
			domain.setup_domain()
	...
	def restrict_roles_and_modules(self):
		...
		all_domains = list(kanivin.get_hooks("domains") or {})

		def remove_role(role):
			kanivin.db.delete("Has Role", {"role": role})
			kanivin.set_value("Role", role, "disabled", 1)

		for domain in all_domains:
			data = kanivin.get_domain_data(domain)
			if not kanivin.db.get_value("Domain", domain):
				kanivin.get_doc(dict(doctype="Domain", domain=domain)).insert()
			if "modules" in data:
				for module in data.get("modules"):
					kanivin.db.set_value("Module Def", module, "restrict_to_domain", domain)

			if "restricted_roles" in data:
				for role in data["restricted_roles"]:
					if not kanivin.db.get_value("Role", role):
						kanivin.get_doc(dict(doctype="Role", role_name=role)).insert()
					kanivin.db.set_value("Role", role, "restrict_to_domain", domain)
			...
			if "custom_fields" in data:
				if domain not in active_domains:
					inactive_domain = kanivin.get_doc("Domain", domain)
...
def get_active_domains():
	...
	return kanivin.cache.get_value("active_domains", _get_active_domains)
...
def get_active_modules():
	...
	def _get_active_modules():
		...
		for m in kanivin.get_all("Module Def", fields=["name", "restrict_to_domain"]):
			if (not m.restrict_to_domain) or (m.restrict_to_domain in active_domains):
				active_modules.append(m.name)
		return active_modules

	return kanivin.cache.get_value("active_modules", _get_active_modules)
