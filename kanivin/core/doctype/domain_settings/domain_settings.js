...
kanivin.ui.form.on("Domain Settings", {
	before_load: function (frm) {
		if (!frm.domains_multicheck) {
			frm.domains_multicheck = kanivin.ui.form.make_control({
				...
				df: {
					...
					get_data: () => {
						...
						return kanivin.boot.all_domains.map((domain) => {
							return {
								label: domain,
								value: domain,
	...
	set_options_in_table: function (frm) {
		...
		unselected_options.map((option) => {
			if (list.includes(option)) {
				kanivin.model.clear_doc("Has Domain", map[option]);
			}
		});
		...
		selected_options.map((option) => {
			if (!list.includes(option)) {
				kanivin.model.clear_doc("Has Domain", map[option]);
				let row = kanivin.model.add_child(frm.doc, "Has Domain", "active_domains");
				row.domain = option;
			}
		});
