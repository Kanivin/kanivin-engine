...
import kanivin
import kanivin.desk.query_report
from kanivin import _, scrub
from kanivin.core.doctype.custom_role.custom_role import get_custom_allowed_roles
from kanivin.core.doctype.page.page import delete_custom_role
from kanivin.desk.reportview import append_totals_row
from kanivin.model.document import Document
from kanivin.modules import make_boilerplate
from kanivin.modules.export_file import export_to_files
from kanivin.utils import cint, cstr
from kanivin.utils.safe_exec import check_safe_sql_query, safe_exec
...
class Report(Document):
	...
	if TYPE_CHECKING:
		...
		from kanivin.core.doctype.report_column.report_column import ReportColumn
		from kanivin.core.doctype.report_filter.report_filter import ReportFilter
	...
	def validate(self):
		...
		if not self.module:
			self.module = kanivin.db.get_value("DocType", self.ref_doctype, "module")

		if not self.is_standard:
			self.is_standard = "No"
			if (
				kanivin.session.user == "Administrator"
				and getattr(kanivin.local.conf, "developer_mode", 0) == 1
			):
		...
		if self.is_standard == "No":
			...
			if kanivin.db.get_value("Report", self.name, "is_standard") == "Yes":
				kanivin.throw(_("Cannot edit a standard report. Please duplicate and create a new report"))

		if self.is_standard == "Yes" and kanivin.session.user != "Administrator":
			kanivin.throw(_("Only Administrator can save a standard report. Please rename and save."))
	...
	def on_trash(self):
		if (
			self.is_standard == "Yes"
			and not cint(getattr(kanivin.local.conf, "developer_mode", 0))
			and not (kanivin.flags.in_migrate or kanivin.flags.in_patch)
		):
			kanivin.throw(_("You are not allowed to delete Standard Report"))
	...
	def set_doctype_roles(self):
		if not self.get("roles") and self.is_standard == "No":
			meta = kanivin.get_meta(self.ref_doctype)
			if not meta.istable:
	...
	def is_permitted(self):
		...
		from kanivin.utils import has_common

		allowed = [d.role for d in kanivin.get_all("Has Role", fields=["role"], filters={"parent": self.name})]
		...
		if has_common(kanivin.get_roles(), allowed):
			return True
	...
	def export_doc(self):
		...
		if self.is_standard == "Yes" and (kanivin.local.conf.get("developer_mode") or 0) == 1:
	...
	def execute_query_report(self, filters):
		if not self.query:
			kanivin.throw(_("Must specify a Query to run"), title=_("Report Document Error"))

		check_safe_sql_query(self.query)

		result = [list(t) for t in kanivin.db.sql(self.query, filters)]
		columns = self.get_columns() or [cstr(c[0]) for c in kanivin.db.get_description()]
	...
	def execute_script_report(self, filters):
		...
		if not self.prepared_report:
			prepared_report_watcher = threading.Timer(
				...
				kwargs={"report": self.name, "site": kanivin.local.site},
			)
		...
		kanivin.cache.hset("report_execution_time", self.name, execution_time)

		return res

	def execute_module(self, filters):
		# report in python module
		module = self.module or kanivin.db.get_value("DocType", self.ref_doctype, "module")
		method_name = get_report_module_dotted_path(module, self.name) + ".execute"
		return kanivin.get_attr(method_name)(kanivin._dict(filters))

	def execute_script(self, filters):
		# server script
		loc = {"filters": kanivin._dict(filters), "data": None, "result": None}
	...
	def run_query_report(
		...
		columns, result = [], []
		data = kanivin.desk.query_report.run(
			self.name,
			filters=filters,
			user=user,
		...
		for d in data.get("columns"):
			if isinstance(d, dict):
				...
				columns.append(
					kanivin._dict(label=parts[0], fieldtype=fieldtype, fieldname=parts[0], options=options)
				)
	...
	def get_standard_report_columns(self, params):
		if params.get("fields"):
			...
			columns.extend(
				...
				for df in kanivin.get_meta(self.ref_doctype).fields
				if df.in_list_view
			)
		return columns
	...
	def get_standard_report_order_by(self, params):
		...
		if params.get("group_by"):
			group_by_args = kanivin._dict(params["group_by"])
	...
	def build_standard_report_columns(self, columns, group_by_args):
		...
		for fieldname, doctype in columns:
			...
			if meta.get_field(fieldname):
				...
				field = kanivin._dict(fieldname=fieldname, label=label)
	...
	def toggle_disable(self, disable: bool):
		if not self.has_permission("write"):
			kanivin.throw(_("You are not allowed to edit the report."))

		self.db_set("disabled", cint(disable))


def is_prepared_report_enabled(report):
	return cint(kanivin.db.get_value("Report", report, "prepared_report")) or 0


def get_report_module_dotted_path(module, report_name):
	return (
		kanivin.local.module_app[scrub(module)]
		+ "."
		+ scrub(module)
		+ ".report."
		+ scrub(report_name)
...
def enable_prepared_report(report: str, site: str):
	kanivin.init(site)
	kanivin.connect()
	kanivin.db.set_value("Report", report, "prepared_report", 1)
	kanivin.db.commit()
	kanivin.destroy()
