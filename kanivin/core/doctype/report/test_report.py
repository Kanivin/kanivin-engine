...
import kanivin
from kanivin.core.doctype.user_permission.test_user_permission import create_user
from kanivin.custom.doctype.customize_form.customize_form import reset_customization
from kanivin.desk.query_report import add_total_row, run, save_report
from kanivin.desk.reportview import delete_report
from kanivin.desk.reportview import save_report as _save_report
from kanivin.tests.utils import KanivinTestCase

test_records = kanivin.get_test_records("Report")
test_dependencies = ["User"]


class TestReport(KanivinTestCase):
	@classmethod
	...
	def test_report_builder(self):
		if kanivin.db.exists("Report", "User Activity Report"):
			kanivin.delete_doc("Report", "User Activity Report")

		with open(os.path.join(os.path.dirname(__file__), "user_activity_report.json")) as f:
			kanivin.get_doc(json.loads(f.read())).insert()

		report = kanivin.get_doc("Report", "User Activity Report")
	...
	def test_query_report(self):
		report = kanivin.get_doc("Report", "Permitted Documents For User")
	...
	def test_save_or_delete_report(self):
		...
		try:
			...
			kanivin.set_user("test_report_owner@example.com")
			self.assertRaises(kanivin.PermissionError, delete_report, report.name)
	...
	def test_custom_report(self):
		...
		custom_report = kanivin.get_doc("Report", custom_report_name)
		columns, result = custom_report.run_query_report(user=kanivin.session.user)

		self.assertListEqual(["email"], [column.get("fieldname") for column in columns])
		admin_dict = kanivin.core.utils.find(result, lambda d: d["name"] == "Administrator")
	...
	def test_report_permissions(self):
		kanivin.set_user("test@example.com")
		kanivin.db.delete("Has Role", {"parent": kanivin.session.user, "role": "Test Has Role"})
		kanivin.db.commit()
		if not kanivin.db.exists("Role", "Test Has Role"):
			kanivin.get_doc({"doctype": "Role", "role_name": "Test Has Role"}).insert(ignore_permissions=True)

		if not kanivin.db.exists("Report", "Test Report"):
			report = kanivin.get_doc(
				{
					"doctype": "Report",
					"ref_doctype": "User",
			...
			report = kanivin.get_doc("Report", "Test Report")
	...
	def test_report_custom_permissions(self):
		kanivin.set_user("test@example.com")
		kanivin.db.delete("Custom Role", {"report": "Test Custom Role Report"})
		kanivin.db.commit()  # nosemgrep
		if not kanivin.db.exists("Report", "Test Custom Role Report"):
			report = kanivin.get_doc(
				{
					"doctype": "Report",
					"ref_doctype": "User",
			...
			report = kanivin.get_doc("Report", "Test Custom Role Report")
	...
	def test_format_method(self):
		if kanivin.db.exists("Report", "User Activity Report Without Sort"):
			kanivin.delete_doc("Report", "User Activity Report Without Sort")
		with open(os.path.join(os.path.dirname(__file__), "user_activity_report_without_sort.json")) as f:
			kanivin.get_doc(json.loads(f.read())).insert()

		report = kanivin.get_doc("Report", "User Activity Report Without Sort")
		columns, data = report.get_data()
	...
	def test_non_standard_script_report(self):
		...
		if not kanivin.db.exists("Report", report_name):
			report = kanivin.get_doc(
				{
					"doctype": "Report",
					"ref_doctype": "User",
...
totals = {}
for user in kanivin.get_all('User', fields = ['name', 'user_type', 'creation']):
...
"""
	...
	def test_script_report_with_columns(self):
		...
		if kanivin.db.exists("Report", report_name):
			kanivin.delete_doc("Report", report_name)
...
"""
	...
	def test_toggle_disabled(self):
		...
		reports = kanivin.get_all(doctype="Report", limit=1)
		...
		doc = kanivin.get_doc("Report", report_name)
		with self.assertRaises(kanivin.exceptions.ValidationError):
	...
	def test_cte_in_query_report(self):
		...
		if kanivin.db.db_type == "mariadb":
			...
			self.assertRaises(kanivin.PermissionError, report.execute_query_report, filters={})
