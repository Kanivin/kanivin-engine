kanivin.ui.form.on("Report", {
	refresh: function (frm) {
		if (frm.doc.is_standard === "Yes" && !kanivin.boot.developer_mode) {
			...
			frm.add_custom_button(
				...
				function () {
					switch (doc.report_type) {
						case "Report Builder":
							kanivin.set_route("List", doc.ref_doctype, "Report", doc.name);
							break;
						case "Query Report":
							kanivin.set_route("query-report", doc.name);
							break;
						case "Script Report":
							kanivin.set_route("query-report", doc.name);
							break;
		...
		if (["en", "en-GB", "en-US"].includes(kanivin.boot.lang)) {
