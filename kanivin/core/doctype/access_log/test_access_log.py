...
from kanivin.utils import cstr, get_site_url


class TestAccessLog(KanivinTestCase):
	def setUp(self):
		# generate keys for current user to send requests for the following tests
		generate_keys(kanivin.session.user)
		kanivin.db.commit()
		generated_secret = kanivin.utils.password.get_decrypted_password(
			"User", kanivin.session.user, fieldname="api_secret"
		)
		...
		self.file_name = kanivin.utils.random_string(10) + ".txt"
		self.test_content = kanivin.utils.random_string(1024)
	...
	def test_make_full_access_log(self):
		...
		last_doc = kanivin.get_last_doc("Access Log")
	...
	def test_private_file_download(self):
		...
		new_private_file = kanivin.get_doc(
			{
				"doctype": self.test_doctype,
		...
		private_file_link = get_site_url(kanivin.local.site) + new_private_file.file_url
