...
from kanivin.utils import now, time_diff_in_seconds
...
class SubmissionQueue(Document):
	...
	@property
	def queued_doc(self):
		return getattr(self, "to_be_queued_doc", kanivin.get_doc(self.ref_doctype, self.ref_docname))
	...
	def clear_old_logs(days=30):
		...
		table = kanivin.qb.DocType("Submission Queue")
	...
	def background_submission(self, to_be_queued_doc: Document, action_for_queuing: str):
		...
		try:
			...
			values = {"status": "Failed", "exception": kanivin.get_traceback(with_context=True)}
			kanivin.db.rollback()

		values["ended_at"] = now()
		kanivin.db.set_value(self.doctype, self.name, values, update_modified=False)
	...
	def notify(self, submission_status: str, action: str):
		...
		message_replacements = (
			kanivin.bold(action),
			kanivin.bold(str(self.ref_doctype)),
			kanivin.bold(str(self.ref_docname)),
		)
		...
		if cint(time_diff) <= 60:
			...
			notify_to = kanivin.db.get_value("User", self.enqueued_by, fieldname="email")
	...
	def unlock_doc(self):
		...
		self.queued_doc.unlock()
		kanivin.msgprint(_("Document Unlocked"))


def queue_submission(doc: Document, action: str, alert: bool = True):
	queue = kanivin.new_doc("Submission Queue")
...
def get_latest_submissions(doctype, docname):
	...
	latest_submission = kanivin.db.get_value(
		"Submission Queue",
