...
from kanivin.desk.notifications import notify_mentions
from kanivin.exceptions import ImplicitCommitError
...
class Comment(Document):
	...
	def validate(self):
		if not self.comment_email:
			self.comment_email = kanivin.session.user
		self.content = kanivin.utils.sanitize_html(self.content, always_sanitize=True)
...
def on_doctype_update():
	kanivin.db.add_index("Comment", ["reference_doctype", "reference_name"])
...
def get_comments_from_parent(doc):
	...
	try:
		if is_virtual_doctype(doc.reference_doctype):
			...
			_comments = kanivin.db.get_value(doc.reference_doctype, doc.reference_name, "_comments") or "[]"

	except Exception as e:
		if kanivin.db.is_missing_table_or_column(e):
			_comments = "[]"
...
def update_comments_in_parent(reference_doctype, reference_name, _comments):
	...
	if (
		...
		or kanivin.db.get_value("DocType", reference_doctype, "issingle")
	...
	try:
		...
		if kanivin.db.is_missing_column(e) and getattr(kanivin.local, "request", None):
			pass
		elif kanivin.db.is_data_too_long(e):
			raise kanivin.DataTooLongException
		else:
			raise
	else:
		...
		# Clear route cache
		if route := kanivin.get_cached_value(reference_doctype, reference_name, "route"):
			clear_cache(route)
