...
import kanivin
from kanivin.templates.includes.comments.comments import add_comment
from kanivin.tests.test_model_utils import set_user
...
class TestComment(KanivinTestCase):
	def test_comment_creation(self):
		test_doc = kanivin.get_doc(dict(doctype="ToDo", description="test"))
		test_doc.insert()
	...
	def test_public_comment(self):
		...
		kanivin.db.delete("Comment", {"reference_doctype": "Blog Post"})
		add_comment_args = {
	...
	def test_user_not_logged_in(self):
		some_system_user = kanivin.db.get_value("User", {"name": ("not in", kanivin.STANDARD_USERS)})
