...
import kanivin
from kanivin import _, msgprint, throw
...
class SMSSettings(Document):
	...
	if TYPE_CHECKING:
		from kanivin.core.doctype.sms_parameter.sms_parameter import SMSParameter
...
def send_sms(receiver_list, msg, sender_name="", success_msg=True):
	...
	arg = {
		"receiver_list": receiver_list,
		"message": kanivin.safe_decode(msg).encode("utf-8"),
		"success_msg": success_msg,
	}

	if kanivin.db.get_single_value("SMS Settings", "sms_gateway_url"):
...
def send_via_gateway(arg):
	...
	message = kanivin.safe_decode(arg.get("message"))
	...
	if len(success_list) > 0:
		...
		if arg.get("success_msg"):
			kanivin.msgprint(_("SMS sent successfully"))


def get_headers(sms_settings=None):
	if not sms_settings:
		sms_settings = kanivin.get_doc("SMS Settings", "SMS Settings")
