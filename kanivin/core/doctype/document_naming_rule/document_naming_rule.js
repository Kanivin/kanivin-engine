...
kanivin.ui.form.on("Document Naming Rule", {
	...
	document_type: (frm) => {
		...
		if (frm.doc.document_type) {
			kanivin.model.with_doctype(frm.doc.document_type, () => {
				let fieldnames = kanivin
					.get_meta(frm.doc.document_type)
					.fields.filter((d) => {
						return kanivin.model.no_value_type.indexOf(d.fieldtype) === -1;
					})
					.map((d) => {
