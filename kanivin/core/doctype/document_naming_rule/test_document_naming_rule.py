...
class TestDocumentNamingRule(KanivinTestCase):
	...
	def test_naming_rule_by_condition(self):
		...
		# another rule
		naming_rule_1 = kanivin.copy_doc(naming_rule)
		...
		naming_rule_2 = kanivin.copy_doc(naming_rule)
		...
		todo = kanivin.get_doc(
			dict(doctype="ToDo", priority="High", description="Is this my name " + kanivin.generate_hash())
		).insert()

		todo_1 = kanivin.get_doc(
			dict(doctype="ToDo", priority="Medium", description="Is this my name " + kanivin.generate_hash())
		).insert()

		todo_2 = kanivin.get_doc(
			dict(doctype="ToDo", priority="Low", description="Is this my name " + kanivin.generate_hash())
		).insert()
