...
from kanivin.model.naming import parse_naming_series
from kanivin.utils.data import evaluate_filters
...
class DocumentNamingRule(Document):
	...
	if TYPE_CHECKING:
		from kanivin.core.doctype.document_naming_rule_condition.document_naming_rule_condition import (
	...
	def clear_doctype_map(self):
		kanivin.cache_manager.clear_doctype_map(self.doctype, self.document_type)
	...
	def validate_fields_in_conditions(self):
		if self.has_value_changed("document_type"):
			docfields = [x.fieldname for x in kanivin.get_meta(self.document_type).fields]
			for condition in self.conditions:
				if condition.field not in docfields:
					kanivin.throw(
						_("{0} is not a field of doctype {1}").format(
							kanivin.bold(condition.field), kanivin.bold(self.document_type)
						)
					)
	...
	def apply(self, doc):
		...
		counter = kanivin.db.get_value(self.doctype, self.name, "counter", for_update=True) or 0
		naming_series = parse_naming_series(self.prefix, doc=doc)

		doc.name = naming_series + ("%0" + str(self.prefix_digits) + "d") % (counter + 1)
		kanivin.db.set_value(self.doctype, self.name, "counter", counter + 1)
