...
kanivin.ui.form.on("Success Action", {
	on_load: (frm) => {
	...
	validate: (frm) => {
		...
		if (checked_actions.length < 2) {
			kanivin.msgprint(__("Select atleast 2 actions"));
		} else {
			return true;
		}
	},
	...
	after_save: (frm) => {
		kanivin.boot.success_action.push(frm.doc);
	...
	set_next_action_multicheck: (frm) => {
		...
		frm.action_multicheck = kanivin.ui.form.make_control({
