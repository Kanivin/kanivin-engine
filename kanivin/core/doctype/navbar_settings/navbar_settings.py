...
class NavbarSettings(Document):
	...
	if TYPE_CHECKING:
		from kanivin.core.doctype.navbar_item.navbar_item import NavbarItem
	...
	def validate_standard_navbar_items(self):
		...
		if not kanivin.flags.in_patch and (len(before_save_items) > len(after_save_items)):
			kanivin.throw(_("Please hide the standard navbar items instead of deleting them"))


def get_app_logo():
	app_logo = kanivin.get_website_settings("app_logo") or kanivin.db.get_single_value(
		"Navbar Settings", "app_logo", cache=True
	)

	if not app_logo:
		logos = kanivin.get_hooks("app_logo_url")
		app_logo = logos[0]
		if len(logos) == 2:
			app_logo = logos[1]
...
def get_navbar_settings():
	return kanivin.get_single("Navbar Settings")
