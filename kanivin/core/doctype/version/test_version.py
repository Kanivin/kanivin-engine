...
class TestVersion(KanivinTestCase):
	...
	def test_no_version_on_new_doc(self):
		from kanivin.desk.form.load import get_versions

		t = kanivin.get_doc(doctype="ToDo", description="something")
		t.save(ignore_version=False)

		self.assertFalse(get_versions(t))

		t = kanivin.get_doc(t.doctype, t.name)
