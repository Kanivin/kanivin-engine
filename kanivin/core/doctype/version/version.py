...
class Version(Document):
	...
	def set_impersonator(data):
		if not kanivin.session:
			return
		if impersonator := kanivin.session.data.get("impersonated_by"):
			data["impersonated_by"] = impersonator

		if audit_user := kanivin.session.data.get("audit_user"):
	...
	def set_diff(self, old: Document, new: Document) -> bool:
		...
		if diff:
			...
			self.data = kanivin.as_json(diff, indent=None, separators=(",", ":"))
			return True
		else:
			return False
	...
	def for_insert(self, doc: Document) -> bool:
		...
		self.data = kanivin.as_json(data, indent=None, separators=(",", ":"))
		return True
...
def on_doctype_update():
	kanivin.db.add_index("Version", ["ref_doctype", "docname"])
