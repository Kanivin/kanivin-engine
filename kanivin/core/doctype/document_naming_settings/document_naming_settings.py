...
from kanivin.core.doctype.doctype.doctype import validate_series
from kanivin.model.document import Document
from kanivin.model.naming import NamingSeries
from kanivin.permissions import get_doctypes_with_read


class NamingSeriesNotSetError(kanivin.ValidationError):
	pass
...
class DocumentNamingSettings(Document):
	...
	if TYPE_CHECKING:
		from kanivin.core.doctype.amended_document_naming_settings.amended_document_naming_settings import (
	...
	def _get_transactions(self) -> list[str]:
		...
		standard = kanivin.get_all("DocField", {"fieldname": "naming_series"}, "parent", pluck="parent")
		custom = kanivin.get_all("Custom Field", {"fieldname": "naming_series"}, "dt", pluck="dt")
	...
	def _get_prefixes(self, doctypes) -> list[str]:
		...
		for d in doctypes:
			try:
				options = kanivin.get_meta(d).get_naming_series_options()
				series_templates.update(options)
			except kanivin.DoesNotExistError:
				kanivin.msgprint(_("Unable to find DocType {0}").format(d))
				continue

		custom_templates = kanivin.get_all(
			"DocType",
			fields=["autoname"],
			filters={
	...
	def _evaluate_and_clean_templates(self, series_templates: set[str]) -> list[str]:
		...
		prefixes_from_db = kanivin.qb.from_(series).select(series.name).run(pluck=True)
		...
		for series_template in series_templates:
			try:
				...
				kanivin.log_error(f"Invalid naming series {series_template}")
	...
	def validate_set_series(self):
		if self.transaction_type and not self.naming_series_options:
			kanivin.throw(_("Please set the series to be used."))
	...
	def check_duplicate(self):
		...
		for doctype in all_doctypes_with_naming_series:
			for series in kanivin.get_meta(doctype).get_naming_series_options():
				existing_series[stripped_series(series)] = doctype

		dt = kanivin.get_doc("DocType", self.transaction_type)

		options = self.get_options_list(self.naming_series_options)
		for series in options:
			if stripped_series(series) in existing_series:
				kanivin.throw(_("Series {0} already used in {1}").format(series, existing_series[series]))
	...
	def get_options(self, doctype=None):
		...
		if kanivin.get_meta(doctype or self.transaction_type).get_field("naming_series"):
			return kanivin.get_meta(doctype or self.transaction_type).get_field("naming_series").options
	...
	def update_amendment_rule(self):
		...
		existing_overrides = kanivin.db.get_all(
		...
		for override in existing_overrides:
			kanivin.delete_doc("Amended Document Naming Settings", override)

		for row in self.amend_naming_override:
			row.save()

		kanivin.msgprint(_("Amendment naming rules updated."), indicator="green", alert=True)
	...
	def update_series_start(self):
		...
		if self.prefix is None:
			kanivin.throw(_("Please select prefix first"))
	...
	def create_version_log_for_change(self, series, old, new):
		...
		version.data = kanivin.as_json({"changed": [["current", old, new]]})
	...
	def _fetch_last_doc_if_available(self):
		...
		try:
			return kanivin.get_last_doc(self.transaction_type)
		except Exception:
			return None
