...
from kanivin.core.doctype.document_naming_settings.document_naming_settings import (
	DocumentNamingSettings,
)
from kanivin.model.naming import NamingSeries, get_default_naming_series
from kanivin.tests.utils import KanivinTestCase
from kanivin.utils import cint


class TestNamingSeries(KanivinTestCase):
	@classmethod
	def setUpClass(cls):
		super().setUpClass()
		cls.ns_doctype = (
	...
	def setUp(self):
		self.dns: DocumentNamingSettings = kanivin.get_doc("Document Naming Settings")
	...
	def test_get_transactions(self):
		...
		existing_naming_series = kanivin.get_meta(self.ns_doctype).get_field("naming_series").options
	...
	def test_updates_naming_options(self):
		...
		self.assertIn(test_series, kanivin.get_meta(self.ns_doctype).get_naming_series_options())
	...
	def test_amended_naming(self):
		...
		new_amended_doc = kanivin.get_doc(
			dict(
				doctype=self.ns_doctype,
				some_fieldname="test doc with submit",
				amended_from=submittable_doc.name,
