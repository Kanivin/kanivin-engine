...
from kanivin.utils import cint, today
...
class SystemSettings(Document):
	...
	def validate(self):
		from kanivin.twofactor import toggle_two_factor_auth
		...
		if enable_password_policy and minimum_password_score <= 0:
			kanivin.throw(_("Please select Minimum Password Score"))
		...
		if self.session_expiry:
			...
			if len(parts) != 2 or not (cint(parts[0]) or cint(parts[1])):
				kanivin.throw(_("Session Expiry must be in format {0}").format("hh:mm"))

		if self.enable_two_factor_auth:
			if self.two_factor_method == "SMS":
				if not kanivin.db.get_single_value("SMS Settings", "sms_gateway_url"):
					kanivin.throw(
		...
		kanivin.flags.update_last_reset_password_date = False
		if self.force_user_to_reset_password and not cint(
			...
			kanivin.flags.update_last_reset_password_date = True
	...
	def validate_user_pass_login(self):
		...
		social_login_enabled = kanivin.db.exists("Social Login Key", {"enable_social_login": 1})
		ldap_enabled = kanivin.db.get_single_value("LDAP Settings", "enabled")
		login_with_email_link_enabled = kanivin.db.get_single_value("System Settings", "login_with_email_link")
	...
	def validate_backup_limit(self):
		if not self.backup_limit or self.backup_limit < 1:
			kanivin.msgprint(_("Number of backups must be greater than zero."), alert=True)
			self.backup_limit = 1
	...
	def on_update(self):
		...
		kanivin.cache.delete_value("system_settings")
		kanivin.cache.delete_value("time_zone")

		if kanivin.flags.update_last_reset_password_date:
			update_last_reset_password_date()

	def set_defaults(self):
		from kanivin.translate import set_default_language

		for df in self.meta.get("fields"):
			if df.fieldtype not in no_value_fields and self.has_value_changed(df.fieldname):
				kanivin.db.set_default(df.fieldname, self.get(df.fieldname))
...
@kanivin.whitelist()
def load():
	from kanivin.utils.momentjs import get_all_timezones

	if "System Manager" not in kanivin.get_roles():
		kanivin.throw(_("Not permitted"), kanivin.PermissionError)

	all_defaults = kanivin.db.get_defaults()
	defaults = {}

	for df in kanivin.get_meta("System Settings").get("fields"):
