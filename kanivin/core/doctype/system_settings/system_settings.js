kanivin.ui.form.on("System Settings", {
	refresh: function (frm) {
		kanivin.call({
			method: "kanivin.core.doctype.system_settings.system_settings.load",
			callback: function (data) {
				kanivin.all_timezones = data.message.timezones;
				frm.set_df_property("time_zone", "options", kanivin.all_timezones);

				$.each(data.message.defaults, function (key, val) {
					frm.set_value(key, val, null, true);
					kanivin.sys_defaults[key] = val;
				});
				if (frm.re_setup_moment) {
					kanivin.app.setup_moment();
	...
	after_save: function (frm) {
		...
		const attr_tuples = [
			[frm.doc.language, kanivin.boot.sysdefaults.language, kanivin.boot.user.language],
			[frm.doc.rounding_method, kanivin.boot.sysdefaults.rounding_method], // no user override.
		];
	...
	rounding_method: function (frm) {
		if (frm.doc.rounding_method == kanivin.boot.sysdefaults.rounding_method) return;
		let msg = __(
		...
		kanivin.confirm(
			...
			() => {},
			() => {
				frm.set_value("rounding_method", kanivin.boot.sysdefaults.rounding_method);
			}
		);
	},
