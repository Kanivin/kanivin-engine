...
class Role(Document):
	...
	def before_rename(self, old, new, merge=False):
		if old in STANDARD_ROLES:
			kanivin.throw(kanivin._("Standard roles cannot be renamed"))

	def after_insert(self):
		kanivin.cache.hdel("roles", "Administrator")
	...
	def disable_role(self):
		if self.name in STANDARD_ROLES:
			kanivin.throw(kanivin._("Standard roles cannot be disabled"))
		else:
			self.remove_roles()

	def validate_homepage(self):
		if kanivin.request and self.home_page:
	...
	def remove_roles(self):
		kanivin.db.delete("Has Role", {"role": self.name})
		kanivin.clear_cache()
	...
	def update_user_type_on_change(self):
		...
		users_with_same_user_type = kanivin.get_all("User", {"user_type": role_user_type}, pluck="name")

		for user_name in set(users_with_role) - set(users_with_same_user_type):
			user = kanivin.get_doc("User", user_name)
...
def get_user_info(users, field="email"):
	...
	info_list = []
	for user in users:
		user_info, enabled = kanivin.db.get_value("User", user.get("user_name"), [field, "enabled"])
...
def get_users(role):
	return [
		d.parent
		for d in kanivin.get_all("Has Role", filters={"role": role, "parenttype": "User"}, fields=["parent"])
	]
