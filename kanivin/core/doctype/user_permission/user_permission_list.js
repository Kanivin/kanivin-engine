kanivin.listview_settings["User Permission"] = {
		...
		list_view.page.add_inner_button(__("Add / Update"), function () {
			let dialog = new kanivin.ui.Dialog({
				...
				primary_action: (data) => {
					...
					kanivin.call({
						async: false,
						method: "kanivin.core.doctype.user_permission.user_permission.add_user_permissions",
						args: {
							data: data,
						},
						callback: function (r) {
		...
		list_view.page.add_inner_button(__("Bulk Delete"), function () {
			const dialog = new kanivin.ui.Dialog({
				...
				primary_action: (data) => {
					...
					kanivin.confirm(__("Are you sure?"), () => {
						kanivin
							.xcall(
								"kanivin.core.doctype.user_permission.user_permission.clear_user_permissions",
								data
							)
							.then((data) => {
								dialog.hide();
	...
	validate: function (dialog, data) {
		...
		if (data.apply_to_all_doctypes == 0 && !("applicable_doctypes" in data)) {
			kanivin.throw(__("Please select applicable Doctypes"));
		}
		return data;
	},
	...
	get_applicable_doctype: function (dialog) {
		return new Promise((resolve) => {
			kanivin
				.call({
					method: "kanivin.core.doctype.user_permission.user_permission.check_applicable_doc_perm",
					async: false,
					args: {
	...
	get_multi_select_options: function (dialog, applicable) {
		return new Promise((resolve) => {
			kanivin
				.call({
					method: "kanivin.desk.form.linked_with.get_linked_doctypes",
					async: false,
					args: {
						user: dialog.fields_dict.user.value,
						doctype: dialog.fields_dict.doctype.value,
	...
	on_doctype_change: function (dialog) {
		...
		let show = kanivin.boot.nested_set_doctypes.includes(dialog.get_value("doctype"));
