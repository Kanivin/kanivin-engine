...
kanivin.ui.form.on("User Permission", {
	setup: (frm) => {
		frm.set_query("allow", () => {
			return {
				filters: {
		...
		frm.set_query("applicable_for", () => {
			return {
				query: "kanivin.core.doctype.user_permission.user_permission.get_applicable_for_doctype_list",
	...
	toggle_hide_descendants: (frm) => {
		let show = kanivin.boot.nested_set_doctypes.includes(frm.doc.allow);
