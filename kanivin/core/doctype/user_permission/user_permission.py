...
from kanivin.desk.form.linked_with import get_linked_doctypes
...
class UserPermission(Document):
	...
	def on_update(self):
		kanivin.cache.hdel("user_permissions", self.user)
		kanivin.publish_realtime("update_user_permissions", user=self.user, after_commit=True)
	...
	def validate_user_permission(self):
		...
		duplicate_exists = kanivin.get_all(
			self.doctype,
			filters={
				"allow": self.allow,
	...
	def validate_default_permission(self):
		...
		if overlap_exists:
			ref_link = kanivin.get_desk_link(self.doctype, overlap_exists[0].name)
			kanivin.throw(_("{0} has already assigned default value for {1}.").format(ref_link, self.allow))
...
def get_user_permissions(user=None):
	...
	if kanivin.request and kanivin.local.form_dict.cmd == "get_user_permissions":
	...
	cached_user_permissions = kanivin.cache.hget("user_permissions", user)
	...
	try:
		for perm in kanivin.get_all(
			...
			meta = kanivin.get_meta(perm.allow)
			add_doc_to_perm(perm, perm.for_value, perm.is_default)

			if meta.is_nested_set() and not perm.hide_descendants:
				decendants = kanivin.db.get_descendants(perm.allow, perm.for_value)
				for doc in decendants:
					add_doc_to_perm(perm, doc, False)

		out = kanivin._dict(out)
		kanivin.cache.hset("user_permissions", user, out)
	except kanivin.db.SQLError as e:
...
def clear_user_permissions(user, for_doctype):
	kanivin.only_for("System Manager")
	total = kanivin.db.count("User Permission", {"user": user, "allow": for_doctype})
...
def insert_user_perm(
	...
	user_perm = kanivin.new_doc("User Permission")
	user_perm.user = user
