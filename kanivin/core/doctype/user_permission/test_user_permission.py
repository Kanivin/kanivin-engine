...
from kanivin.core.doctype.user_permission.user_permission import (
	add_user_permissions,
	remove_applicable,
)
from kanivin.permissions import add_permission, has_user_permission
from kanivin.tests.utils import KanivinTestCase
from kanivin.website.doctype.blog_post.test_blog_post import make_test_blog


class TestUserPermission(KanivinTestCase):
	def setUp(self):
		test_users = (
		...
		kanivin.db.delete("User Permission", {"user": ("in", test_users)})
		kanivin.delete_doc_if_exists("DocType", "Person")
		kanivin.db.sql_ddl("DROP TABLE IF EXISTS `tabPerson`")
		kanivin.delete_doc_if_exists("DocType", "Doc A")
		kanivin.db.sql_ddl("DROP TABLE IF EXISTS `tabDoc A`")
	...
	def test_default_user_permission_validation(self):
		...
		self.assertRaises(kanivin.ValidationError, add_user_permissions, param)
	...
	def test_default_user_permission_corectness(self):
		...
		add_user_permissions(param)
		kanivin.db.delete("User Permission", filters={"for_value": test_blog.name})
		kanivin.delete_doc("Blog Post", test_blog.name)
	...
	def test_default_user_permission(self):
		...
		for category in ["general", "public"]:
			if not kanivin.db.exists("Blog Category", category):
				kanivin.get_doc({"doctype": "Blog Category", "title": category}).insert()
		...
		kanivin.set_user("test_user_perm1@example.com")
	...
	def test_for_applicable_on_update_from_apply_to_all(self):
		...
		removed_apply_to_all = kanivin.db.exists("User Permission", get_exists_param(user))
		is_created_applicable_first = kanivin.db.exists(
			"User Permission", get_exists_param(user, applicable="Comment")
		)
		is_created_applicable_second = kanivin.db.exists(
	...
	def test_for_apply_to_all_on_update_from_applicable(self):
		...
		is_created = add_user_permissions(param)
		is_created_apply_to_all = kanivin.db.exists("User Permission", get_exists_param(user))
		removed_applicable_first = kanivin.db.exists(
			"User Permission", get_exists_param(user, applicable="Comment")
		)
		removed_applicable_second = kanivin.db.exists(
			"User Permission", get_exists_param(user, applicable="Contact")
		)
	...
	def test_user_perm_for_nested_doctype(self):
		...
		if not kanivin.db.exists("DocType", "Person"):
			doc = new_doctype(
				"Person",
		...
		parent_record = kanivin.get_doc({"doctype": "Person", "person_name": "Parent", "is_group": 1}).insert()

		child_record = kanivin.get_doc(
			{
				"doctype": "Person",
				"person_name": "Child",
				"is_group": 0,
		...
		self.assertTrue(has_user_permission(kanivin.get_doc("Person", parent_record.name), user.name))
		self.assertTrue(has_user_permission(kanivin.get_doc("Person", child_record.name), user.name))

		#  give access of Parent DocType to Blogger role
		add_permission("Person", "Blogger")
		kanivin.set_user(user.name)
		visible_names = kanivin.get_list(
			doctype="Person",
			pluck="person_name",
		)

		user_permission = kanivin.get_doc(
		...
		# hides child records
		self.assertTrue(has_user_permission(kanivin.get_doc("Person", parent_record.name), user.name))
		self.assertFalse(has_user_permission(kanivin.get_doc("Person", child_record.name), user.name))

		visible_names_after_hide_descendants = kanivin.get_list(
			"Person",
			pluck="person_name",
		)
	...
	def test_user_perm_on_new_doc_with_field_default(self):
		...
		kanivin.set_user("new_doc_test@example.com")

		new_doc = kanivin.new_doc("Doc A")
	...
	def test_user_perm_on_new_doc_with_user_default(self):
		"""Test User Perm impact on kanivin.new_doc. with *user* default value"""
		from kanivin.core.doctype.session_default_settings.session_default_settings import (
		...
		if not kanivin.db.exists("Session Default", {"ref_doctype": "DocType"}):
		...
		kanivin.set_user("user_default_test@example.com")
...
def create_user(email, *roles):
	...
	if kanivin.db.exists("User", email):
		return kanivin.get_doc("User", email)
