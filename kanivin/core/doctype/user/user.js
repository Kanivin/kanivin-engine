kanivin.ui.form.on("User", {
	setup: function (frm) {
		frm.set_query("default_workspace", () => {
			return {
				filters: {
					for_user: ["in", [null, kanivin.session.user]],
	...
	before_load: function (frm) {
		let update_tz_options = function () {
			frm.fields_dict.time_zone.set_data(kanivin.all_timezones);
		};

		if (!kanivin.all_timezones) {
			kanivin.call({
				method: "kanivin.core.doctype.user.user.get_timezones",
				callback: function (r) {
					kanivin.all_timezones = r.message.timezones;
					update_tz_options();
				},
			});
		} else {
	...
	role_profile_name: function (frm) {
		if (frm.doc.role_profile_name) {
			kanivin.call({
				method: "kanivin.core.doctype.user.user.get_role_profile",
				args: {
	...
	module_profile: function (frm) {
		if (frm.doc.module_profile) {
			kanivin.call({
				method: "kanivin.core.doctype.user.user.get_module_profile",
				args: {
	...
	onload: function (frm) {
		...
		if (
			...
			if (!frm.roles_editor) {
				...
				frm.roles_editor = new kanivin.RoleEditor(
					role_area,
					frm,
	...
	refresh: function (frm) {
		...
		kanivin.xcall("kanivin.apps.get_apps").then((r) => {
			let apps = r?.map((r) => r.name) || [];
			frm.set_df_property("default_app", "options", [" ", ...apps]);
		});

		if (frm.is_new()) {
			frm.set_value("time_zone", kanivin.sys_defaults.time_zone);
		}
		...
		if (!frm.is_new()) {
			if (has_access_to_edit_user()) {
				...
				frm.add_custom_button(
					...
					() =>
						kanivin.set_route("query-report", "Permitted Documents For User", {
							user: frm.doc.name,
						}),
					__("Permissions")
				);
			...
			frm.add_custom_button(
				...
				function () {
					kanivin.call({
						method: "kanivin.core.doctype.user.user.reset_password",
						args: {
							user: frm.doc.name,
						},
					});
			...
			if (kanivin.user.has_role("System Manager")) {
				kanivin.db.get_single_value("LDAP Settings", "enabled").then((value) => {
								...
								const d = new kanivin.ui.Dialog({
									...
									primary_action: (values) => {
										...
										if (values.new_password !== values.confirm_password) {
											kanivin.throw(__("Passwords do not match!"));
										}
										kanivin.call(
											"kanivin.integrations.doctype.ldap_settings.ldap_settings.reset_password",
											{
												user: frm.doc.email,
			...
			if (
				cint(kanivin.boot.sysdefaults.enable_two_factor_auth) &&
				(kanivin.session.user == doc.name || kanivin.user.has_role("System Manager"))
			) {
				frm.add_custom_button(
					__("Reset OTP Secret"),
					function () {
						kanivin.call({
							method: "kanivin.twofactor.reset_otp_secret",
							args: {
								user: frm.doc.name,
							},
						});
			...
			if (kanivin.session.user == doc.name) {
				// update display settings
				if (doc.user_image) {
					kanivin.boot.user_info[kanivin.session.user].image = kanivin.utils.get_file_link(
						doc.user_image
					);
				}
			}
		}
		if (frm.doc.user_emails && kanivin.model.can_create("Email Account")) {
			var found = 0;
		...
		if (kanivin.route_flags.unsaved === 1) {
			delete kanivin.route_flags.unsaved;
	...
	create_user_email: function (frm) {
		kanivin.call({
			method: "kanivin.core.doctype.user.user.has_email_account",
			args: {
				email: frm.doc.email,
			},
			callback: function (r) {
				if (!Array.isArray(r.message) || !r.message.length) {
					...
					kanivin.model.with_doctype("Email Account", function (doc) {
						doc = kanivin.model.get_new_doc("Email Account");
						kanivin.route_flags.linked_user = frm.doc.name;
						kanivin.route_flags.delete_user_from_locals = true;
						kanivin.set_route("Form", "Email Account", doc.name);
					});
				} else {
					kanivin.route_flags.create_user_account = frm.doc.name;
					kanivin.set_route("Form", "Email Account", r.message[0]["name"]);
				}
			},
		});
	},
	generate_keys: function (frm) {
		kanivin.call({
			method: "kanivin.core.doctype.user.user.generate_keys",
			args: {
				user: frm.doc.name,
			},
			callback: function (r) {
				if (r.message) {
					kanivin.msgprint(__("Save API Secret: {0}", [r.message.api_secret]));
					frm.reload_doc();
				}
			},
		});
	...
	after_save: function (frm) {
		...
		if (doc.name === kanivin.session.user && attr_tuples.some(has_effectively_changed)) {
			kanivin.msgprint(__("Refreshing..."));
			window.location.reload();
		}
	},
	setup_impersonation: function (frm) {
		if (kanivin.session.user === "Administrator" && frm.doc.name != "Administrator") {
			frm.add_custom_button(__("Impersonate"), () => {
				...
				kanivin.prompt(
					...
					(values) => {
						kanivin
							.xcall("kanivin.core.doctype.user.user.impersonate", {
								user: frm.doc.name,
...
function has_access_to_edit_user() {
	return has_common(kanivin.user_roles, get_roles_for_editing_user());
}
