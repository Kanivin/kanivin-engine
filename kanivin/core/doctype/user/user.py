...
from kanivin import STANDARD_USERS, _, msgprint, throw
from kanivin.apps import get_default_path
from kanivin.auth import MAX_PASSWORD_SIZE
from kanivin.core.doctype.user_type.user_type import user_linked_with_permission_on_doctype
from kanivin.desk.doctype.notification_settings.notification_settings import (
	create_notification_settings,
	toggle_notifications,
)
from kanivin.desk.notifications import clear_notifications
...
from kanivin.utils.data import sha256_hash
from kanivin.utils.deprecations import deprecated
from kanivin.utils.html_utils import sanitize_html
from kanivin.utils.password import check_password, get_password_reset_limit
from kanivin.utils.password import update_password as _update_password
from kanivin.utils.user import get_system_managers
from kanivin.website.utils import get_home_page, is_signup_disabled
...
class User(Document):
	...
	if TYPE_CHECKING:
		...
		from kanivin.core.doctype.defaultvalue.defaultvalue import DefaultValue
		from kanivin.core.doctype.has_role.has_role import HasRole
		from kanivin.core.doctype.user_email.user_email import UserEmail
		from kanivin.core.doctype.user_social_login.user_social_login import UserSocialLogin
	...
	def after_insert(self):
		...
		kanivin.cache.delete_key("users_for_mentions")
		kanivin.cache.delete_key("enabled_users")
	...
	def validate(self):
		...
		if (self.name not in ["Administrator", "Guest"]) and (not self.get_social_login_userid("kanivin")):
			self.set_social_login_userid("kanivin", kanivin.generate_hash(length=39))

	def populate_role_profile_roles(self):
		if self.role_profile_name:
			role_profile = kanivin.get_doc("Role Profile", self.role_profile_name)
			self.set("roles", [])
	...
	def validate_allowed_modules(self):
		if self.module_profile:
			module_profile = kanivin.get_doc("Module Profile", self.module_profile)
	...
	def validate_user_image(self):
		if self.user_image and len(self.user_image) > 2000:
			kanivin.throw(_("Not a valid User Image."))

	def on_update(self):
		# clear new password
		self.share_with_self()
		clear_notifications(user=self.name)
		kanivin.clear_cache(user=self.name)
		now = kanivin.flags.in_test or kanivin.flags.in_install
		self.send_password_notification(self.__new_password)
		kanivin.enqueue(
			"kanivin.core.doctype.user.user.create_contact",
			user=self,
		...
		if self.name not in STANDARD_USERS and not self.user_image:
			kanivin.enqueue(
				"kanivin.core.doctype.user.user.update_gravatar",
				name=self.name,
				now=now,
				enqueue_after_commit=True,
			)
		...
		if self.time_zone:
			kanivin.defaults.set_default("time_zone", self.time_zone, self.name)
	...
	def has_website_permission(self, ptype, user, verbose=False):
		...
		return self.name == kanivin.session.user
	...
	def check_enable_disable(self):
		...
		if not cint(self.enabled) and self.name in STANDARD_USERS:
			kanivin.throw(_("User {0} cannot be disabled").format(self.name))

		# clear sessions if disabled
		if not cint(self.enabled) and getattr(kanivin.local, "login_manager", None):
			kanivin.local.login_manager.logout(user=self.name)
	...
	def set_system_user(self):
		...
		if self.user_type and not kanivin.get_cached_value("User Type", self.user_type, "is_standard"):
	...
	def set_roles_and_modules_based_on_user_type(self):
		user_type_doc = kanivin.get_cached_doc("User Type", self.user_type)
	...
	def validate_share(self, docshare):
		...
		# 		if docshare.share != 1:
		# 			kanivin.throw(_("Sorry! User should have complete access to their own record."))
		# 	else:
		# 		kanivin.throw(_("Sorry! Sharing with Website User is prohibited."))
	...
	def send_password_notification(self, new_password):
		...
		except kanivin.OutgoingEmailError:
	...
	def reset_password(self, send_email=False, password_expired=False):
		from kanivin.utils import get_url
	...
	def password_reset_mail(self, link):
		reset_password_template = kanivin.db.get_system_setting("reset_password_template")
	...
	def send_welcome_mail_to_user(self):
		...
		subject = None
		method = kanivin.get_hooks("welcome_email")
		if method:
			subject = kanivin.get_attr(method[-1])()
		if not subject:
			site_name = kanivin.db.get_default("site_name") or kanivin.get_conf().get("site_name")
			if site_name:
				subject = _("Welcome to {0}").format(site_name)
			else:
				subject = _("Complete Registration")

		welcome_email_template = kanivin.db.get_system_setting("welcome_email_template")
	...
	def send_login_mail(self, subject, template, add_args, now=None, custom_template=None):
		...
		from kanivin.utils import get_url
		from kanivin.utils.user import get_user_fullname

		created_by = get_user_fullname(kanivin.session["user"])
		if created_by == "Guest":
		...
		sender = (
			kanivin.session.user not in STANDARD_USERS and get_formatted_email(kanivin.session.user) or None
		)

		if custom_template:
			from kanivin.email.doctype.email_template.email_template import get_email_template
	...
	def on_trash(self):
		...
		self.enabled = 0
		if getattr(kanivin.local, "login_manager", None):
			kanivin.local.login_manager.logout(user=self.name)

		# delete todos
		kanivin.db.delete("ToDo", {"allocated_to": self.name})
		todo_table = DocType("ToDo")
		(
			kanivin.qb.update(todo_table)
			.set(todo_table.assigned_by, None)
			.where(todo_table.assigned_by == self.name)
		).run()

		# delete events
		kanivin.db.delete("Event", {"owner": self.name, "event_type": "Private"})

		# delete shares
		kanivin.db.delete("DocShare", {"user": self.name})
		# delete messages
		...
		kanivin.qb.update(table).where(table.user == self.name).set(table.user, None).run()

		# delete notification settings
		kanivin.delete_doc("Notification Settings", self.name, ignore_permissions=True)
		...
		kanivin.db.delete("User Permission", {"user": self.name})

		# Delete OAuth data
		kanivin.db.delete("OAuth Authorization Code", {"user": self.name})
		kanivin.db.delete("Token Cache", {"user": self.name})

		# Delete EPS data
		kanivin.db.delete("Energy Point Log", {"user": self.name})

		# Remove user link from Workflow Action
		kanivin.db.set_value("Workflow Action", {"user": self.name}, "user", None)

		# Delete user's List Filters
		kanivin.db.delete("List Filter", {"for_user": self.name})

		# Remove user from Note's Seen By table
		seen_notes = kanivin.get_all("Note", filters=[["Note Seen By", "user", "=", self.name]], pluck="name")
		for note_id in seen_notes:
			note = kanivin.get_doc("Note", note_id)
	...
	def before_rename(self, old_name, new_name, merge=False):
		...
		if merge:
			kanivin.delete_doc("Notification Settings", old_name, ignore_permissions=True)

		kanivin.clear_cache(user=old_name)
	...
	def after_rename(self, old_name, new_name, merge=False):
		tables = kanivin.db.get_tables()
		for tab in tables:
			desc = kanivin.db.get_table_columns_description(tab)
		...
		if kanivin.db.exists("Notification Settings", old_name):
			kanivin.rename_doc("Notification Settings", old_name, new_name, force=True, show_alert=False)

		# set email
		kanivin.db.set_value("User", new_name, "email", new_name)
	...
	def remove_disabled_roles(self):
		disabled_roles = [d.name for d in kanivin.get_all("Role", filters={"disabled": 1})]
	...
	def validate_username(self):
		if not self.username and self.is_new() and self.first_name:
			self.username = kanivin.scrub(self.first_name)
		...
		if self.username_exists():
			if self.user_type == "System User":
				kanivin.msgprint(_("Username {0} already exists").format(self.username))
	...
	def suggest_username(self):
		...
		# @firstname
		username = _check_suggestion(kanivin.scrub(self.first_name))

		if not username:
			# @firstname_last_name
			username = _check_suggestion(kanivin.scrub("{} {}".format(self.first_name, self.last_name or "")))

		if username:
			kanivin.msgprint(_("Suggested Username: {0}").format(username))

		return username

	def username_exists(self, username=None):
		return kanivin.db.get_value("User", {"username": username or self.username, "name": ("!=", self.name)})
	...
	def validate_user_email_inbox(self):
		...
		if len(email_accounts) != len(set(email_accounts)):
			kanivin.throw(_("Email Account added multiple times"))
	...
	def find_by_credentials(cls, user_name: str, password: str, validate_password: bool = True):
		...
		login_with_mobile = cint(
			kanivin.db.get_single_value("System Settings", "allow_login_using_mobile_number")
		)
		login_with_username = cint(
			kanivin.db.get_single_value("System Settings", "allow_login_using_user_name")
		)
		...
		users = kanivin.get_all("User", fields=["name", "enabled"], or_filters=or_filters, limit=1)
		if not users:
			return
		...
		if validate_password:
			...
			except kanivin.AuthenticationError:
	...
	def check_roles_added(self):
		...
		kanivin.msgprint(
			_("Newly created user {0} has no roles enabled.").format(kanivin.bold(self.name)),
			title=_("No Roles Specified"),
			indicator="orange",
			primary_action={
				"label": _("Add Roles"),
				"client_action": "kanivin.set_route",
...
def get_all_roles():
	...
	roles = kanivin.get_all(
		"Role",
		filters={
			"name": ("not in", kanivin.permissions.AUTOMATIC_ROLES),
			"disabled": 0,
		},
...
def get_roles(arg=None):
	...
	return kanivin.get_roles(kanivin.form_dict["uid"])


@kanivin.whitelist()
def get_perm_info(role):
	"""get permission info"""
	from kanivin.permissions import get_all_perms

	return get_all_perms(role)


@kanivin.whitelist(allow_guest=True, methods=["POST"])
def update_password(
	...
	if len(new_password) > MAX_PASSWORD_SIZE:
		kanivin.throw(_("Password size exceeded the maximum allowed size."))
	...
	if res.get("message"):
		kanivin.local.response.http_status_code = 410
		return res["message"]
	else:
		user = res["user"]

	logout_all_sessions = cint(logout_all_sessions) or kanivin.db.get_single_value(
	...
	redirect_to = kanivin.cache.hget("redirect_after_login", user)
	if redirect_to:
		redirect_url = redirect_to
		kanivin.cache.hdel("redirect_after_login", user)

	kanivin.local.login_manager.login_as(user)

	kanivin.db.set_value("User", user, "last_password_reset_date", today())
	kanivin.db.set_value("User", user, "reset_password_key", "")
...
def test_password_strength(new_password: str, key=None, old_password=None, user_data: tuple | None = None):
	...
	from kanivin.utils.password_strength import test_password_strength as _test_password_strength
	...
	enable_password_policy = kanivin.get_system_settings("enable_password_policy") or 0

	if not enable_password_policy:
		return {}

	if not user_data:
		user_data = kanivin.db.get_value(
			"User", kanivin.session.user, ["first_name", "middle_name", "last_name", "email", "birth_date"]
		)

	if new_password:
		result = _test_password_strength(new_password, user_inputs=user_data)
		password_policy_validation_passed = False
		minimum_password_score = cint(kanivin.get_system_settings("minimum_password_score")) or 0
...
def has_email_account(email: str):
	return kanivin.get_list("Email Account", filters={"email_id": email})


@kanivin.whitelist(allow_guest=False)
...
def ask_pass_update():
	...
	from kanivin.utils import set_default

	password_list = kanivin.get_all(
...
def _get_user_for_update_password(key, old_password):
	# verify old password
	result = kanivin._dict()
	if key:
		hashed_key = sha256_hash(key)
		user = kanivin.db.get_value(
		...
		if result.user:
			reset_password_link_expiry = cint(
				kanivin.db.get_single_value("System Settings", "reset_password_link_expiry_duration")
			)
			if (
		...
		# verify old password
		kanivin.local.login_manager.check_password(kanivin.session.user, old_password)
...
def reset_user_data(user):
	user_doc = kanivin.get_doc("User", user)
...
@kanivin.whitelist(methods=["POST"])
def verify_password(password):
	kanivin.local.login_manager.check_password(kanivin.session.user, password)


@kanivin.whitelist(allow_guest=True)
def sign_up(email: str, full_name: str, redirect_to: str) -> tuple[int, str]:
	if is_signup_disabled():
		kanivin.throw(_("Sign Up is disabled"), title=_("Not Allowed"))

	user = kanivin.db.get("User", {"email": email})
	if user:
		if user.enabled:
		...
		if kanivin.db.get_creation_count("User", 60) > 300:
		...
		default_role = kanivin.db.get_single_value("Portal Settings", "default_role")
		if default_role:
			user.add_roles(default_role)

		if redirect_to:
			kanivin.cache.hset("redirect_after_login", user.name, redirect_to)
...
def reset_password(user: str) -> str:
	try:
		user: User = kanivin.get_doc("User", user)
		...
		kanivin.local.response["http_status_code"] = 404
		kanivin.clear_messages()
		return "not found"


@kanivin.whitelist()
@kanivin.validate_and_sanitize_search_inputs
def user_query(doctype, txt, searchfield, start, page_len, filters):
	...
	if filters:
		if not (filters.get("ignore_user_type") and kanivin.session.data.user_type == "System User"):
...
def get_website_users():
	...
	return kanivin.db.count("User", filters={"enabled": True, "user_type": "Website User"})
...
def get_permission_query_conditions(user):
	if user == "Administrator":
		...
		return """(`tabUser`.name not in ({standard_users}))""".format(
			standard_users=", ".join(kanivin.db.escape(user) for user in STANDARD_USERS)
		)
...
def notify_admin_access_to_system_manager(login_manager=None):
	if (
		...
		and kanivin.local.conf.notify_admin_access_to_system_manager
	):
		site = '<a href="{0}" target="_blank">{0}</a>'.format(kanivin.local.request.host_url)
		date_and_time = "<b>{}</b>".format(format_datetime(now_datetime(), format_string="medium"))
		ip_address = kanivin.local.request_ip
...
def handle_password_test_fail(feedback: dict):
	...
	kanivin.throw(msg=" ".join([warning, *suggestions]), title=_("Invalid Password"))


def update_gravatar(name):
	gravatar = has_gravatar(name)
	if gravatar:
		kanivin.db.set_value("User", name, "user_image", gravatar)
...
def throttle_user_creation():
	...
	if kanivin.db.get_creation_count("User", 60) > kanivin.local.conf.get("throttle_user_limit", 60):
		kanivin.throw(_("Throttled"))


@kanivin.whitelist()
def get_role_profile(role_profile: str):
	return kanivin.get_doc("Role Profile", {"role_profile": role_profile}).roles


@kanivin.whitelist()
def get_module_profile(module_profile: str):
	module_profile = kanivin.get_doc("Module Profile", {"module_profile_name": module_profile})
	return module_profile.get("block_modules")


def create_contact(user, ignore_links=False, ignore_mandatory=False):
	from kanivin.contacts.doctype.contact.contact import get_contact_name
	...
	if not contact_name:
		...
		except kanivin.DuplicateEntryError:
			pass
	else:
		try:
			contact = kanivin.get_doc("Contact", contact_name)
		...
		except kanivin.TimestampMismatchError:
			raise kanivin.RetryBackgroundJobError
...
def generate_keys(user: str):
	...
	user_details: User = kanivin.get_doc("User", user)
	api_secret = kanivin.generate_hash(length=15)
	# if api key is not set generate api key
	if not user_details.api_key:
		api_key = kanivin.generate_hash(length=15)
...
def switch_theme(theme):
	if theme in ["Dark", "Light", "Automatic"]:
		kanivin.db.set_value("User", kanivin.session.user, "desk_theme", theme)


def get_enabled_users():
	def _get_enabled_users():
		enabled_users = kanivin.get_all("User", filters={"enabled": "1"}, pluck="name")
		return enabled_users

	return kanivin.cache.get_value("enabled_users", _get_enabled_users)
...
def impersonate(user: str, reason: str):
	...
	impersonator = kanivin.session.user
	kanivin.get_doc(
		{
	...
	notification = kanivin.new_doc(
		"Notification Log",
		for_user=user,
		from_user=kanivin.session.user,
		subject=_("{0} just impersonated as you. They gave this reason: {1}").format(impersonator, reason),
	)
	notification.set("type", "Alert")
	notification.insert(ignore_permissions=True)
	kanivin.local.login_manager.impersonate(user)
