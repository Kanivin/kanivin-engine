...
kanivin.ui.form.on("Role Permission for Page and Report", {
	setup: function (frm) {
	...
	setup_buttons: function (frm) {
		...
		if (frm.doc.set_role_for && frm.doc[kanivin.model.scrub(frm.doc.set_role_for)]) {
	...
	onload: function (frm) {
		if (!frm.roles_editor) {
			...
			frm.roles_editor = new kanivin.RoleEditor(frm.role_area, frm);
		}
	},
	...
	update_report_page_data: function (frm) {
		...
		return frm.call({
			...
			callback: function (r) {
				refresh_field("roles");
				frm.roles_editor.show();
				kanivin.msgprint(__("Successfully Updated"));
			},
		});
	},
	...
	validate_mandatory_fields: function (frm) {
		if (!frm.doc.set_role_for) {
			kanivin.throw(__("Mandatory field: set role for"));
		}

		if (frm.doc.set_role_for && !frm.doc[frm.doc.set_role_for.toLocaleLowerCase()]) {
			kanivin.throw(__("Mandatory field: {0}", [frm.doc.set_role_for]));
		}
	},
});
