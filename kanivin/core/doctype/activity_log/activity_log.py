...
import kanivin
from kanivin.core.utils import set_timeline_doc
...
from kanivin.utils import get_fullname, now, strip_html
...
class ActivityLog(Document):
	...
	def set_ip_address(self):
		if self.operation in ("Login", "Logout"):
			self.ip_address = kanivin.local.request_ip
	...
	def clear_old_logs(days=None):
		...
		kanivin.db.delete(doctype, filters=(doctype.modified < (Now() - Interval(days=days))))


def on_doctype_update():
	"""Add indexes in `tabActivity Log`"""
	kanivin.db.add_index("Activity Log", ["reference_doctype", "reference_name"])
	kanivin.db.add_index("Activity Log", ["timeline_doctype", "timeline_name"])
