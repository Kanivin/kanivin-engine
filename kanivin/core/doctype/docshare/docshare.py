...
from kanivin.utils import cint, get_fullname
...
class DocShare(Document):
	...
	def get_doc(self):
		if not getattr(self, "_doc", None):
			self._doc = kanivin.get_doc(self.share_doctype, self.share_name)
		return self._doc
	...
	def validate_user(self):
		if self.everyone:
			...
			kanivin.throw(_("User is mandatory for Share"), kanivin.MandatoryError)

	def check_share_permission(self):
		if not self.flags.ignore_share_permission and not kanivin.has_permission(
			self.share_doctype, "share", self.get_doc()
		):
			kanivin.throw(_('You need to have "Share" permission'), kanivin.PermissionError)

	def check_is_submittable(self):
		if self.submit and not cint(kanivin.db.get_value("DocType", self.share_doctype, "is_submittable")):
			kanivin.throw(
				_("Cannot share {0} with submit permission as the doctype {1} is not submittable").format(
					kanivin.bold(self.share_name), kanivin.bold(self.share_doctype)
				)
			)
...
def on_doctype_update():
	...
	kanivin.db.add_index("DocShare", ["user", "share_doctype"])
	kanivin.db.add_index("DocShare", ["share_doctype", "share_name"])
