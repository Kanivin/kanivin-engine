...
import kanivin
from kanivin.core.doctype.log_settings.log_settings import _supports_log_clearing, run_log_clean_up
from kanivin.tests.utils import KanivinTestCase
from kanivin.utils import add_to_date, now_datetime


class TestLogSettings(KanivinTestCase):
	@classmethod
	def setUpClass(cls):
		super().setUpClass()
	...
	def test_delete_logs(self):
		...
		activity_log_count = kanivin.db.count("Activity Log", {"creation": ("<=", self.datetime.past)})
		error_log_count = kanivin.db.count("Error Log", {"creation": ("<=", self.datetime.past)})
		email_queue_count = kanivin.db.count("Email Queue", {"creation": ("<=", self.datetime.past)})
