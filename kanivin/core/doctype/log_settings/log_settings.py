...
class LogSettings(Document):
	...
	if TYPE_CHECKING:
		from kanivin.core.doctype.logs_to_clear.logs_to_clear import LogsToClear
	...
	def remove_unsupported_doctypes(self):
		for entry in list(self.logs_to_clear):
			...
			msg = _("{} does not support automated log clearing.").format(kanivin.bold(entry.ref_doctype))
			if kanivin.conf.developer_mode:
				msg += "<br>" + _("Implement `clear_old_logs` method to enable auto error clearing.")
			kanivin.msgprint(msg, title=_("DocType not supported by Log Settings."))
			self.remove(entry)
	...
	def add_default_logtypes(self):
		...
		added_logtypes = set()
		default_logtypes_retention = kanivin.get_hooks("default_log_clearing_doctypes", {})
		...
		for logtype, retentions in default_logtypes_retention.items():
			if logtype not in existing_logtypes and _supports_log_clearing(logtype):
				if not kanivin.db.exists("DocType", logtype):
					continue
		...
		if added_logtypes:
			kanivin.msgprint(_("Added default log doctypes: {}").format(",".join(added_logtypes)), alert=True)
	...
	def clear_logs(self):
		...
		for entry in self.logs_to_clear:
			...
			kwargs = kanivin.get_newargs(func, {"days": entry.days})
			func(**kwargs)
			kanivin.db.commit()
...
def run_log_clean_up():
	doc = kanivin.get_doc("Log Settings")
...
def has_unseen_error_log():
	if kanivin.get_all("Error Log", filters={"seen": 0}, limit=1):
		return {
			"show_alert": True,
...
def get_log_doctypes(doctype, txt, searchfield, start, page_len, filters):
	...
	doctypes = kanivin.get_list("DocType", filters=filters, pluck="name")
...
def clear_log_table(doctype, days=90):
	...
	"""
	from kanivin.utils import get_table_name
	...
	try:
		kanivin.db.sql_ddl(f"CREATE TABLE `{temporary}` LIKE `{original}`")
		...
		kanivin.db.sql_ddl(f"RENAME TABLE `{original}` TO `{backup}`, `{temporary}` TO `{original}`")
	except Exception:
		kanivin.db.rollback()
		kanivin.db.sql_ddl(f"DROP TABLE IF EXISTS `{temporary}`")
		raise
	else:
		kanivin.db.sql_ddl(f"DROP TABLE `{backup}`")
