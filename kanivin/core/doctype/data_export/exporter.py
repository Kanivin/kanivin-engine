...
from kanivin.utils import cint, cstr, format_datetime, format_duration, formatdate, parse_json
from kanivin.utils.csvutils import UnicodeWriter
...
class DataExporter:
	...
	def prepare_args(self):
		...
		if self.all_doctypes:
			self.child_doctypes = [
				...
				for df in kanivin.get_meta(self.doctype).get_table_fields()
			]
	...
	def build_response(self):
		...
		if self.file_type == "Excel":
			...
			kanivin.response["result"] = cstr(self.writer.getvalue())
			kanivin.response["type"] = "csv"
			kanivin.response["doctype"] = self.doctype
	...
	def build_field_columns(self, dt, parentfield=None):
		...
		for f in kanivin.db.get_table_columns_description(table_name):
			field = meta.get_field(f.name)
			if f.name in ["owner", "creation"]:
				std_field = next((x for x in kanivin.model.std_fields if x["fieldname"] == f.name), None)
				if std_field:
					field = kanivin._dict(
						{
		...
		_column_start_end = kanivin._dict(start=0)
		...
		if dt == self.doctype:
			...
			_column_start_end = kanivin._dict(start=len(self.columns))
	...
	def getinforow(docfield):
		...
		if docfield.fieldtype == "Select":
			...
			return cstr(kanivin.defaults.get_defaults().date_format)
	...
	def add_data(self):
		...
		kanivin.permissions.can_export(self.parent_doctype, raise_exception=True)

		# sort nested set doctypes by `lft asc`
		order_by = None
		table_columns = kanivin.db.get_table_columns(self.parent_doctype)
		if "lft" in table_columns and "rgt" in table_columns:
			order_by = f"`tab{self.parent_doctype}`.`lft` asc"
		# get permitted data only
		self.data = kanivin.get_list(
		...
		for doc in self.data:
			...
			if self.all_doctypes:
				...
				for c in self.child_doctypes:
					...
					data_row = (
						kanivin.qb.from_(child_doctype_table)
						.select("*")
	...
	def build_response_as_excel(self):
		...
		filename = kanivin.generate_hash(length=10)
