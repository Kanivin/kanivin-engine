...
kanivin.ui.form.on("Data Export", {
	refresh: (frm) => {
		frm.disable_save();
		...
		frm.set_query("reference_doctype", () => {
			return {
				filters: {
					issingle: 0,
					istable: 0,
					name: ["in", kanivin.boot.user.can_export],
				},
			};
		});
	},
	reference_doctype: (frm) => {
		...
		if (doctype) {
			kanivin.model.with_doctype(doctype, () => set_field_options(frm));
		} else {
...
const can_export = (frm) => {
	...
	if (!doctype) {
		kanivin.msgprint(__("Please select the Document Type."));
	} else if (!parent_multicheck_options.length) {
		kanivin.msgprint(__("Atleast one field of Parent Document Type is mandatory"));
	} else {
		is_valid_form = true;
	}
...
const export_data = (frm) => {
	let get_template_url = "/api/method/kanivin.core.doctype.data_export.exporter.export_data";
	var export_params = () => {
		let columns = {};
...
const set_field_options = (frm) => {
	...
	frm.filter_list = new kanivin.ui.FilterGroup({
...
const make_multiselect_buttons = (parent_wrapper) => {
	...
	["Select All", "Unselect All"].map((d) => {
		kanivin.ui.form.make_control({
			parent: $(button_container),
			df: {
				label: __(d),
				fieldname: kanivin.scrub(d),
				fieldtype: "Button",
				click: () => {
...
const get_doctypes = (parentdt) => {
	return [parentdt].concat(kanivin.meta.get_table_fields(parentdt).map((df) => df.options));
};

const add_doctype_field_multicheck_control = (doctype, parent_wrapper) => {
	const fields = get_fields(doctype);

	kanivin.model.std_fields
	...
	const multicheck_control = kanivin.ui.form.make_control({
...
const filter_fields = (df) => kanivin.model.is_value_type(df) && !df.hidden;
const get_fields = (dt) => kanivin.meta.get_docfields(dt).filter(filter_fields);
