...
kanivin.ui.form.on("User Type", {
	refresh: function (frm) {
		if (frm.is_new() && !kanivin.boot.developer_mode) frm.set_value("is_standard", 1);
		...
		frm.set_query("apply_user_permission_on", function () {
			return {
				query: "kanivin.core.doctype.user_type.user_type.get_user_linked_doctypes",
			};
		});
	},
	...
	get_user_id_fields: function (frm) {
		if (frm.doc.apply_user_permission_on) {
			kanivin.call({
				method: "kanivin.core.doctype.user_type.user_type.get_user_id",
				args: {
