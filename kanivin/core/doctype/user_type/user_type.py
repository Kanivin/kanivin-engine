...
from kanivin.config import get_modules_from_app
from kanivin.model.document import Document
from kanivin.permissions import add_permission, add_user_permission
from kanivin.utils import get_link_to_form
...
class UserType(Document):
	...
	if TYPE_CHECKING:
		from kanivin.core.doctype.user_document_type.user_document_type import UserDocumentType
		from kanivin.core.doctype.user_select_document_type.user_select_document_type import (
			UserSelectDocumentType,
		)
		from kanivin.core.doctype.user_type_module.user_type_module import UserTypeModule
	...
	def clear_cache(self):
		...
		if not self.is_standard:
			kanivin.cache.delete_value("non_standard_user_types")
	...
	def on_trash(self):
		if self.is_standard:
			kanivin.throw(_("Standard user type {0} can not be deleted.").format(kanivin.bold(self.name)))

	def set_modules(self):
		if not self.user_doctypes:
			return

		modules = kanivin.get_all(
			"DocType",
	...
	def validate_document_type_limit(self):
		limit = kanivin.conf.get("user_type_doctype_limit", {}).get(kanivin.scrub(self.name))

		if not limit and kanivin.session.user != "Administrator":
			kanivin.throw(
				_("User does not have permission to create the new {0}").format(kanivin.bold(_("User Type"))),
	...
	def validate_role(self):
		if not self.role:
			kanivin.throw(_("The field {0} is mandatory").format(kanivin.bold(_("Role"))))

		if not kanivin.db.get_value("Role", self.role, "is_custom"):
			kanivin.throw(
				_("The role {0} should be a custom role.").format(
					kanivin.bold(get_link_to_form("Role", self.role))
				)
			)

	def update_users(self):
		for row in kanivin.get_all("User", filters={"user_type": self.name}):
			user = kanivin.get_cached_doc("User", row.name)
	...
	def add_role_permissions_for_user_doctypes(self):
		...
		for row in self.user_doctypes:
			...
			kanivin.db.set_value("Custom DocPerm", docperm, values)

	def add_select_perm_doctypes(self):
		if kanivin.flags.ignore_select_perm:
			return
	...
	def add_role_permissions_for_file(self):
		...
		kanivin.db.set_value("Custom DocPerm", docperm, {"read": 1, "create": 1, "write": 1})
	...
	def remove_permission_for_deleted_doctypes(self):
		...
		for perm in kanivin.get_all(
			"Custom DocPerm", filters={"role": self.role, "parent": ["not in", doctypes]}
		):
			kanivin.delete_doc("Custom DocPerm", perm.name)


def add_role_permissions(doctype, role):
	name = kanivin.get_value("Custom DocPerm", dict(parent=doctype, role=role, permlevel=0))
...
def get_user_linked_doctypes(doctype, txt, searchfield, start, page_len, filters):
	modules = [d.get("module_name") for d in get_modules_from_app("kanivin")]
	...
	custom_doctypes = kanivin.get_all(
...
def user_linked_with_permission_on_doctype(doc, user):
	...
	if not doc.user_id_field:
		kanivin.throw(_("User Id Field is mandatory in the user type {0}").format(kanivin.bold(doc.name)))

	if kanivin.db.get_value(doc.apply_user_permission_on, {doc.user_id_field: user}, "name"):
		return True
	else:
		label = kanivin.get_meta(doc.apply_user_permission_on).get_field(doc.user_id_field).label

		kanivin.msgprint(
			_(
				"To set the role {0} in the user {1}, kindly set the {2} field as {3} in one of the {4} record."
			).format(
				kanivin.bold(doc.role),
				kanivin.bold(user),
				kanivin.bold(label),
				kanivin.bold(user),
				kanivin.bold(doc.apply_user_permission_on),
			)
		)
...
def apply_permissions_for_non_standard_user_type(doc, method=None):
	...
	if not kanivin.db.table_exists("User Type") or kanivin.flags.in_migrate:
		return

	user_types = kanivin.cache.get_value(
	...
	for user_type, data in user_types.items():
		...
		if doc.get(data[1]) and (
			...
			if not perm_data:
				...
				kanivin.db.set_value("User Permission", perm_data[0], "user", doc.get(data[1]))
