...
import kanivin
from kanivin.query_builder import DocType, Field
from kanivin.utils import cstr, get_bench_path, is_html, strip, strip_html_tags, unique
...
def get_language(lang_list: list | None = None) -> str:
	"""Set `kanivin.local.lang` from HTTP headers at beginning of request
	...
	"""
	is_logged_in = kanivin.session.user != "Guest"

	# fetch language from form_dict
	if kanivin.form_dict._lang:
		language = get_lang_code(kanivin.form_dict._lang or get_parent_language(kanivin.form_dict._lang))
		if language:
			return language

	# use language set in User or System Settings if user is logged in
	if is_logged_in:
		return kanivin.local.lang
	...
	accept_language = list(kanivin.request.accept_languages.values())
	...
	return kanivin.db.get_default("lang") or "en"
...
def get_user_lang(user: str | None = None) -> str:
	"""Set kanivin.local.lang from user preferences on session beginning or resumption"""
	user = user or kanivin.session.user

	# User.language => Session Defaults => kanivin.local.lang => 'en'
	return (
		kanivin.get_cached_value("User", user, "language")
		or kanivin.db.get_default("lang")
		or kanivin.local.lang
		or "en"
	)


def get_lang_code(lang: str) -> str | None:
	return kanivin.db.get_value("Language", {"name": lang}) or kanivin.db.get_value(
...
def set_default_language(lang):
	...
	if kanivin.db.get_default("lang") != lang:
		kanivin.db.set_default("lang", lang)
	kanivin.local.lang = lang


def get_lang_dict():
	"""Return all languages in dict format, full name is the key e.g. `{"english":"en"}`."""
	return dict(
		kanivin.get_all("Language", fields=["language_name", "name"], order_by="creation", as_list=True)
	)
...
def get_messages_for_boot():
	...
	return get_all_translations(kanivin.local.lang)


@kanivin.whitelist(allow_guest=True)
def get_app_translations():
	if kanivin.session.user != "Guest":
		language = kanivin.db.get_value("User", kanivin.session.user, "language")
	else:
		language = kanivin.db.get_single_value("System Settings", "language")
...
def get_all_translations(lang: str) -> dict[str, str]:
	...
	def _merge_translations():
		from kanivin.geo.country_info import get_translated_countries
	...
	try:
		return kanivin.cache.hget(MERGED_TRANSLATION_KEY, lang, generator=_merge_translations)
	except Exception:
		if kanivin.flags and kanivin.flags.in_test:
			raise
		...
		kanivin.logger().error("Unable to load translations", exc_info=True)
		return {}
...
def get_translations_from_apps(lang, apps=None):
	...
	translations = {}
	from kanivin.gettext.translate import get_translations_from_mo

	for app in apps or kanivin.get_installed_apps(_ensure_on_bench=True):
...
def get_translations_from_csv(lang, app):
	return get_translation_dict_from_file(
		os.path.join(kanivin.get_app_path(app, "translations"), lang + ".csv"), lang, app
	)
	...
	if os.path.exists(path):
		...
		for item in csv_content:
			if len(item) == 3 and item[2]:
				...
				kanivin.log_error(message=msg, title="Error in translation file")
				if throw:
					kanivin.throw(msg, title="Error in translation file")
...
def get_user_translations(lang):
	...
	return kanivin.cache.hget(USER_TRANSLATION_KEY, lang, generator=_read_from_db)


def clear_cache():
	"""Clear all translation assets from :meth:`kanivin.cache`"""
	kanivin.cache.delete_value(
...
def get_messages_for_app(app, deduplicate=True):
	...
	messages = []
	modules = [kanivin.unscrub(m) for m in kanivin.local.app_modules[app]]
	...
	if modules:
		if isinstance(modules, str):
			modules = [modules]
		filtered_doctypes = (
			kanivin.qb.from_("DocType").where(Field("module").isin(modules)).select("name").run(pluck=True)
		)
		...
		# pages
		filtered_pages = (
			kanivin.qb.from_("Page").where(Field("module").isin(modules)).select("name", "title").run()
		)
...
def get_messages_from_navbar():
	...
	labels = kanivin.get_all("Navbar Item", filters={"item_label": ("is", "set")}, pluck="item_label")
...
def get_messages_from_doctype(name):
	...
	from kanivin.gettext.extractors.utils import is_translatable

	messages = []
	meta = kanivin.get_meta(name)
	...
	if not meta.custom:
		doctype_file_path = kanivin.get_module_path(meta.module, "doctype", meta.name, meta.name)
...
def get_messages_from_workflow(doctype=None, app_name=None):
	...
	workflows = []
	if doctype:
		workflows = kanivin.get_all("Workflow", filters={"document_type": doctype})
	else:
		fixtures = kanivin.get_hooks("fixtures", app_name=app_name) or []
		for fixture in fixtures:
			if isinstance(fixture, str) and fixture == "Worflow":
				workflows = kanivin.get_all("Workflow")
				break
			elif isinstance(fixture, dict) and fixture.get("dt", fixture.get("doctype")) == "Workflow":
				workflows.extend(kanivin.get_all("Workflow", filters=fixture.get("filters")))

	messages = []
	document_state = DocType("Workflow Document State")
	for w in workflows:
		states = kanivin.db.get_values(
			document_state,
...
def get_messages_from_page(name):
	"""Return all translatable strings from a :class:`kanivin.core.doctype.Page`."""
	return _get_messages_from_page_or_report("Page", name)


def get_messages_from_report(name):
	"""Return all translatable strings from a :class:`kanivin.core.doctype.Report`."""
	from kanivin.gettext.extractors.utils import is_translatable

	report = kanivin.get_doc("Report", name)
	messages = _get_messages_from_page_or_report(
		"Report", name, kanivin.db.get_value("DocType", report.ref_doctype, "module")
	)
...
def _get_messages_from_page_or_report(doctype, name, module=None):
	if not module:
		module = kanivin.db.get_value(doctype, name, "module")

	doc_path = kanivin.get_module_path(module, doctype, name)

	messages = get_messages_from_file(os.path.join(doc_path, kanivin.scrub(name) + ".py"))
...
def get_server_messages(app):
	"""Extracts all translatable strings (tagged with :func:`kanivin._`) from Python modules
	inside an app"""
	messages = []
	file_extensions = (".py", ".html", ".js", ".vue")
	app_walk = os.walk(kanivin.get_app_path(app))
...
def get_messages_from_include_files(app_name=None):
	...
	from kanivin.utils.jinja_globals import bundled_asset

	messages = []
	app_include_js = kanivin.get_hooks("app_include_js", app_name=app_name) or []
	web_include_js = kanivin.get_hooks("web_include_js", app_name=app_name) or []
	include_js = app_include_js + web_include_js

	for js_path in include_js:
		file_path = bundled_asset(js_path)
		relative_path = os.path.join(kanivin.local.sites_path, file_path.lstrip("/"))
...
def get_all_messages_from_js_files(app_name=None):
	...
	messages = []
	for app in [app_name] if app_name else kanivin.get_installed_apps(_ensure_on_bench=True):
		if os.path.exists(kanivin.get_app_path(app, "public")):
			for basepath, folders, files in os.walk(kanivin.get_app_path(app, "public")):  # noqa: B007
				if "kanivin/public/js/lib" in basepath:
					continue
...
def get_messages_from_file(path: str) -> list[tuple[str, str, str | None, int]]:
	...
	from kanivin.gettext.extractors.utils import extract_messages_from_code

	kanivin.flags.setdefault("scanned_files", set())
	# TODO: Find better alternative
	# To avoid duplicate scan
	if path in kanivin.flags.scanned_files:
		return []

	kanivin.flags.scanned_files.add(path)
...
def get_untranslated(lang, untranslated_file, get_all=False, app="_ALL_APPS"):
	...
	clear_cache()
	apps = kanivin.get_all_apps(True)
	if app != "_ALL_APPS":
		if app not in apps:
...
def update_translations(lang, untranslated_file, translated_file, app="_ALL_APPS"):
	...
	for key, value in zip(
		kanivin.get_file_items(untranslated_file, ignore_empty_lines=False),
		kanivin.get_file_items(translated_file, ignore_empty_lines=False),
		strict=False,
	):
...
def import_translations(lang, path):
	...
	for app in kanivin.get_all_apps(True):
...
def migrate_translations(source_app, target_app):
	...
	strings_in_source_app = [m[1] for m in kanivin.translate.get_messages_for_app(source_app)]
	strings_in_target_app = [m[1] for m in kanivin.translate.get_messages_for_app(target_app)]

	strings_in_target_app_but_not_in_source_app = list(
		set(strings_in_target_app) - set(strings_in_source_app)
	)

	languages = kanivin.translate.get_all_languages()

	source_app_translations_dir = kanivin.get_app_path(source_app, "translations")
	target_app_translations_dir = kanivin.get_app_path(target_app, "translations")
...
def rebuild_all_translation_files():
	...
	for lang in get_all_languages():
		for app in kanivin.get_all_apps():
...
def write_translations_file(app, lang, full_dict=None, app_messages=None):
	...
	tpath = kanivin.get_app_path(app, "translations")
	kanivin.create_folder(tpath)
	write_csv_file(os.path.join(tpath, lang + ".csv"), app_messages, full_dict or get_all_translations(lang))


def send_translations(translation_dict):
	"""Append translated dict in `kanivin.local.response`"""
	if "__messages" not in kanivin.local.response:
		kanivin.local.response["__messages"] = {}

	kanivin.local.response["__messages"].update(translation_dict)
...
def update_translations_for_source(source=None, translation_dict=None):
	...
	translation_records = kanivin.db.get_values(
...
def get_all_languages(with_language_name: bool = False) -> list:
	...
	def get_language_codes():
		return kanivin.get_all("Language", filters={"enabled": 1}, pluck="name")

	def get_all_language_with_name():
		return kanivin.get_all("Language", ["language_code", "language_name"], {"enabled": 1})

	if with_language_name:
		return kanivin.cache.get_value("languages_with_name", get_all_language_with_name)
	else:
		return kanivin.cache.get_value("languages", get_language_codes)


def get_preferred_language_cookie():
	return kanivin.request.cookies.get("preferred_language")


def get_translated_doctypes():
	dts = kanivin.get_all("DocType", {"translated_doctype": 1}, pluck="name")
	custom_dts = kanivin.get_all(
...
def print_language(language: str):
	...
	with print_language("de"):
	    html = kanivin.get_print(...)
	```
	"""
	if not language or language == kanivin.local.lang:
		# do nothing
		yield
		return

	# remember original values
	_lang = kanivin.local.lang
	_jenv = kanivin.local.jenv

	# set language, empty any existing lang_full_dict and jenv
	kanivin.local.lang = language
	kanivin.local.jenv = None

	yield

	# restore original values
	kanivin.local.lang = _lang
	kanivin.local.jenv = _jenv
